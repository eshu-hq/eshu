...
| --- | --- |
| Code intelligence | Find symbols, callers, call chains, imports, inheritance, complexity, and dead-code candidates across 20+ languages; production-tier deployed-scale validation for these reads is not yet committed evidence (tracked in issue 5552). <!-- capability-state: id=code_search.symbol_lookup state=experimental --> <!-- product-claim: id=readme.code-intelligence.language-breadth --> |
