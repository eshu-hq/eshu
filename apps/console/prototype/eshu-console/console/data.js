...
(function () {
  ...
  const collectors = [
    ...
    { kind: "cloudwatch", instance: "cw-bg", status: "healthy", facts: 47320, scopes: 2, lastRun: "1m ago", latencyMs: 880, freshness: "fresh", cadence: "5m poll", note: "Alarms, log groups & RDS/ElastiCache metrics across bg-prod, bg-qa" },
    ...
    { kind: "grafana_loki", instance: "loki-bg", status: "degraded", facts: 19240, scopes: 3, lastRun: "2m ago", latencyMs: 2600, freshness: "lagging", cadence: "stream", note: "Structured logs; ingester backpressure on bg-prod shard 2" },
    ...
    { kind: "grafana_synthetic", instance: "synthetics-bg", status: "stale", facts: 1840, scopes: 8, lastRun: "3h 40m ago", latencyMs: 0, freshness: "stale", cadence: "5m probe", note: "Synthetic uptime probes; checks paused after Grafana migration" },
  ...
  const graph = {
    nodes: [
      ...
      { id: "svc:boatsdotcom", kind: "service", label: "boatsdotcom", sub: "portal · Storefront", col: 0 },
    ...
    edges: [
      ...
      { s: "wl:boats", t: "mon:metrics", verb: "EMITS_METRICS", layer: "ops" },
      { s: "svc:boats", t: "mon:traces", verb: "TRACED_BY", layer: "ops" },
      { s: "wl:boats", t: "mon:logs", verb: "LOGS_TO", layer: "ops" },
      { s: "svc:boats", t: "mon:apm", verb: "TRACED_BY", layer: "ops" },
      { s: "svc:boatsdotcom", t: "svc:platform", verb: "DEPENDS_ON", layer: "runtime" },
      { s: "svc:boatsdotcom", t: "edge:cf", verb: "FRONTED_BY", layer: "infra" }
    ]
  };
  ...
  const nodeDetail = {
    ...
    "mon:logs": { evidence: ["LOGS_TO Grafana Loki (structured JSON)", "trace ⋈ log correlation by trace-id", "ingester backpressure on bg-prod shard 2", "collector: grafana_loki · streaming"], freshness: "lagging", truth: "derived" },
    ...
    "svc:boatsdotcom": { evidence: ["RUNS_IN bg-prod, bg-qa · Node SSR portal", "DEPENDS_ON api-node-platform, api-node-boats", "FRONTED_BY Cloudflare edge (WAF + cache)", "owner storefront · system Storefront"], freshness: "fresh", truth: "exact" },
    "edge:cf": { evidence: ["FRONTED_BY Cloudflare for 13 storefront portals", "WAF events + cache rules + edge routes", "collector: cloudflare · 5m poll", "fronts boatsdotcom, boattrader, yachtfocus…"], freshness: "fresh", truth: "exact" }
  };
  ...
  const cloudAccounts = [
    { id: "aws-prod", provider: "aws", label: "boats-prod", account: "479205017022", region: "us-east-1", env: "bg-prod" },
    { id: "aws-qa", provider: "aws", label: "boats-nonprod", account: "581745330914", region: "us-east-1", env: "bg-qa" },
    { id: "azure-edge", provider: "azure", label: "bg-edge-sub", account: "b7d1-edge", region: "eastus", env: "bg-prod" },
    { id: "gcp-data", provider: "gcp", label: "bg-data-warehouse", account: "bg-data-417", region: "us-central1", env: "bg-prod" }
  ];
  ...
  function buildCloudResources() {
    ...
    services.forEach((s, i) => {
      const accId = (s.envs && s.envs.includes("bg-qa") && i % 2) ? "aws-qa" : "aws-prod";
    ...
    add("aws", "aws-prod", "aws_synthetics_canary", "canary-boatsdotcom", "boatsdotcom uptime canary", { service: "boatsdotcom", family: "observability", signal: "synthetics", freshness: "stale", truth: "inferred" });
    // multi-cloud presence
    add("azure", "azure-edge", "azure_frontdoor_profile", "fd-storefronts", "Front Door · storefront edge", { service: "boatsdotcom" });
    add("azure", "azure-edge", "azure_monitor_workspace", "azmon-edge", "Azure Monitor · edge", { family: "observability", signal: "metrics" });
    add("gcp", "gcp-data", "gcp_bigquery_dataset", "bq-listings", "BigQuery · listings_analytics", { service: "api-node-datax" });
    add("gcp", "gcp-data", "gcp_cloud_run_service", "cr-export", "Cloud Run · datax-export", { service: "api-node-datax" });
    return out;
  }
  ...
  const deadCode = [
    { id: "dc1", repo: "api-node-fsbo", symbol: "legacySwigRenderer", file: "src/lib/render.js", line: 42, kind: "function", refs: 0, confidence: "exact", age: "live", loc: 58, reason: "No call sites — superseded by React SSR; still imports swig@1.4.2" },
    { id: "dc2", repo: "api-node-fsbo", symbol: "AwsS3LegacyClient", file: "src/lib/s3-legacy.js", line: 11, kind: "class", refs: 0, confidence: "exact", age: "live", loc: 124, reason: "aws-sdk v2 client; no references since v3 migration of sibling modules" },
    { id: "dc3", repo: "api-node-fsbo", symbol: "buildLegacyRequest", file: "src/lib/request-util.js", line: 7, kind: "function", refs: 0, confidence: "derived", age: "live", loc: 33, reason: "Wraps deprecated request@2.88.2; only reachable from removed routes" },
    { id: "dc4", repo: "api-node-boats", symbol: "experimentalRankV1", file: "src/services/rank.legacy.ts", line: 19, kind: "function", refs: 0, confidence: "exact", age: "live", loc: 91, reason: "Replaced by rankV2; export retained but never imported across the estate" },
    { id: "dc5", repo: "api-node-boats", symbol: "ES6FallbackMapper", file: "src/services/providers/es-fallback.ts", line: 23, kind: "class", refs: 0, confidence: "inferred", age: "5h", loc: 76, reason: "No CALLS edges observed; runtime trace coverage is partial here" },
    { id: "dc6", repo: "api-node-platform", symbol: "composeLegacyClients", file: "src/services/compose.legacy.ts", line: 14, kind: "function", refs: 0, confidence: "exact", age: "live", loc: 47, reason: "Old client-composition path; superseded by dmm-clients v19 bundle" },
    { id: "dc7", repo: "api-node-platform", symbol: "DEFAULT_TIMEOUTS_V1", file: "src/config/timeouts.ts", line: 3, kind: "const", refs: 0, confidence: "derived", age: "live", loc: 12, reason: "Constant exported but shadowed by env-driven config" },
    { id: "dc8", repo: "api-node-boattrader", symbol: "valkeyMigrationShim", file: "src/lib/valkey-shim.ts", line: 9, kind: "function", refs: 0, confidence: "exact", age: "live", loc: 64, reason: "Memcached→Valkey shim; migration complete, no callers remain" },
    { id: "dc9", repo: "api-node-conversation", symbol: "twilioWebhookV1", file: "src/routes/webhook.legacy.ts", line: 21, kind: "route", refs: 0, confidence: "exact", age: "live", loc: 88, reason: "Unmounted route handler; replaced by /v2/webhook" },
    { id: "dc10", repo: "api-node-forex", symbol: "staticRateTable", file: "src/lib/static-rates.ts", line: 5, kind: "const", refs: 0, confidence: "derived", age: "live", loc: 140, reason: "Hardcoded rate fallback table; no longer referenced after live FX feed" },
    { id: "dc11", repo: "api-node-saved-search", symbol: "dynamoScanAll", file: "src/services/scan.ts", line: 31, kind: "function", refs: 0, confidence: "inferred", age: "5h", loc: 41, reason: "Full-table scan helper; reachability inferred — deploy evidence stale" },
    { id: "dc12", repo: "api-node-fsbo", symbol: "OldSalesforceMapper", file: "src/services/sf-map.legacy.js", line: 16, kind: "class", refs: 0, confidence: "exact", age: "live", loc: 203, reason: "Pre-jsforce mapping; entire module unreferenced" },
    { id: "dc13", repo: "api-node-external-search", symbol: "partnerFeedV1", file: "src/routes/partner.legacy.ts", line: 12, kind: "route", refs: 0, confidence: "derived", age: "live", loc: 72, reason: "Syndication v1 endpoint; partners migrated to v2" },
    { id: "dc14", repo: "boatsdotcom", symbol: "ssrCacheWarmer", file: "src/server/warm.legacy.ts", line: 8, kind: "function", refs: 0, confidence: "inferred", age: "2d", loc: 55, reason: "Cache pre-warmer; no invocation found in current entrypoints" },
    { id: "dc15", repo: "api-node-make-model", symbol: "taxonomySeedV1", file: "src/data/seed.legacy.ts", line: 4, kind: "const", refs: 0, confidence: "exact", age: "live", loc: 318, reason: "One-time seed data committed to source; never imported at runtime" },
    { id: "dc16", repo: "api-node-fsbo", symbol: "renderXmlSitemapLegacy", file: "src/routes/sitemap.legacy.js", line: 27, kind: "route", refs: 0, confidence: "derived", age: "live", loc: 96, reason: "Superseded by job-node-sitemaps-generator" }
  ];
