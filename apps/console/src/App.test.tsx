...
describe("App shell", () => {
  ...
  it("renders the redesigned console navigation", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Eshu Console" })).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("Search repos, services, CVEs, evidence…"),
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole("link", { name: "Ask Eshu" })).toHaveAttribute("href", "/ask");
    expect(screen.getByRole("link", { name: "Graph Explorer" })).toHaveAttribute(
      "href",
      "/explorer",
    );
    ...
    expect(screen.getByRole("link", { name: "Dead code" })).toHaveAttribute("href", "/dead-code");
    expect(screen.getByRole("link", { name: "Code graph" })).toHaveAttribute("href", "/code-graph");
    expect(screen.getByRole("link", { name: "Catalog" })).toHaveAttribute("href", "/catalog");
    expect(screen.getByRole("link", { name: "Topology" })).toHaveAttribute("href", "/topology");
    ...
    expect(screen.getByRole("link", { name: /context graph/i })).toHaveAttribute("href", "/");
  });
  ...
  it("registers the replatforming console route", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Replatforming" })).toHaveAttribute(
      "href",
      "/replatforming",
    );
  });
  ...
  it("routes cloud drift to the live drift readback page", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Cloud Drift" })).toHaveAttribute(
      "href",
      "/cloud-drift",
    );
  });
  ...
  it("registers the changed-since console route", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Changed Since" })).toHaveAttribute(
      "href",
      "/changed-since",
    );
  });
  ...
  it("registers the secrets IAM posture route", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Secrets/IAM" })).toHaveAttribute(
      "href",
      "/secrets-iam",
    );
  });
  ...
  it("counts vulnerabilities in the Findings navigation worklist badge", async () => {
    ...
    expect(
      screen.getByRole("link", { name: "Findings" }).querySelector(".nav-count"),
    ).toHaveTextContent("2");
  });
        ...
        if (path === "/eshu-api/api/v0/repositories") {
          return Response.json({
            data: {
              repositories: [
                {
                  id: "repository:helm-charts",
                  name: "helm-charts",
                  repo_slug: "platform/helm-charts",
                },
              ],
            },
          });
