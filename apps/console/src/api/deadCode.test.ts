...
describe("deadCode", () => {
  it("posts a bounded scan request with optional repo, language, and exact-kind filters", async () => {
  ...
  it("uses repository names for labels without leaking unresolved internal repo ids", async () => {
    ...
    const page = await loadDeadCodePage(
      client,
      { limit: 100 },
      new Map([["repository:r1", "svc-platform"]]),
    );
