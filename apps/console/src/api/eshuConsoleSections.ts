...
interface IacResourcesResponse {
  readonly resources?: NonNullable<Parameters<typeof iacResourceRowsFromResponse>[0]>["resources"];
}

export { loadVulnerabilities, severityFromCvss } from "./eshuConsoleVulnerabilities";
...
export async function loadRuntime(
  ...
  try {
    ...
    overview = eco.data ?? {};
    if (eco.truth) {
      ctx.truth.runtime = eco.truth;
      profile = eco.truth.profile ?? profile;
    }
  } catch {
    /* optional */
  }
  ...
  try {
    const repoList = await client.getJson<RepositoryListBrief>(
      "/api/v0/repositories?limit=1&offset=0",
    );
  ...
  return {
    ...
    repositories:
      repoTotal ?? st.repository_count ?? overview.repo_count ?? overview.repository_count ?? 0,
...
export async function loadServices(
  client: EshuApiClient,
  ctx: SectionContext,
): Promise<readonly ServiceRow[] | null> {
...
export async function loadLanguages(client: EshuApiClient): Promise<readonly LanguageRow[] | null> {
  const env = await client.get<LanguageInventory>(
    "/api/v0/repositories/language-inventory?limit=100&offset=0",
  );
  if (env.error) throw new EshuEnvelopeError(env.error);
  const rows = (env.data?.languages ?? []).map((l) => ({
    language: l.language,
    count: l.repository_count ?? l.count ?? l.file_count ?? 0,
  }));
  return rows.length > 0 ? rows : null;
}
...
export async function loadFindings(
  client: EshuApiClient,
  ctx?: SectionContext,
): Promise<readonly FindingRow[] | null> {
...
export async function loadSbom(
  client: EshuApiClient,
  ctx: SectionContext,
): Promise<SbomEvidenceRow | null> {
  const env = await client.get<SBOMAttachmentCount>(
...
export async function loadDependenciesSection(
  client: EshuApiClient,
  ctx: SectionContext,
): Promise<readonly DependencyRow[] | null> {
...
export async function loadImagesSection(
  client: EshuApiClient,
  ctx: SectionContext,
): Promise<ReturnType<typeof imageRowsFromResponse> | null> {
  const env = await client.get<{ images?: readonly Record<string, unknown>[] }>(
    "/api/v0/images?limit=50&offset=0",
  );
  if (env.error) throw new EshuEnvelopeError(env.error);
...
export async function loadIacResources(
  client: EshuApiClient,
  ctx: SectionContext,
): Promise<readonly IacResourceRow[] | null> {
...
export { emptySeries, loadSeriesBundle } from "./eshuConsoleSeries";
...
export async function loadArgoCDApps(
  ...
  return results.map((r): ArgoCDAppRow => {
    ...
    const sourceIndexed =
      source !== "" &&
      (knownNames.has(source) || [...knownNames].some((name) => source.includes(name)));
    return {
      id: r.id ?? "",
      name: r.name ?? "",
      kind: r.kind ?? "",
