...
describe("impact review adapter", () => {
  ...
  it("keeps unsupported blast and error envelopes visible without fabricating graph state", async () => {
    ...
    expect(result.blast.status).toBe("skipped");
    expect(result.changeSurface.status).toBe("ready");
    expect(result.changeSurface.status === "ready" ? result.changeSurface.data.resolution.status : "").toBe("ambiguous");
    expect(result.changeSurface.status === "ready" ? result.changeSurface.truth?.freshness.state : "").toBe("stale");
    expect(result.deploymentTrace.status).toBe("unavailable");
    expect(result.deploymentTrace.status === "unavailable" ? result.deploymentTrace.error : "").toContain("unsupported_capability");
