    ...
    get: vi.fn(async (path: string) => {
      ...
      if (path.startsWith("/api/v0/investigations/drift/packet")) {
        return envelope(
          {
            answer: {
              summary: "Runtime drift packet is supported.",
              supported: true,
              truth_class: "exact",
            },
            bounds: { max_source_facts: 200, truncated: false },
            graph_answers: [
              { id: "graph:drift:1", present: true, relation: "MANAGED_BY_TERRAFORM" },
            ],
            identity: { family: "drift", scope: { account_id: "123456789012", provider: "aws" } },
            missing_evidence: [
              { hop: "terraform_state_resource", reason: "state evidence missing" },
            ],
            ...
            evidence_groups: [
              {
                ...
                evidence: [
                  {
                    id: "ev-1",
                    evidence_type: "aws_cloud_resource",
                    key: "arn",
                    value: "arn:aws:s3:::payments-prod",
                  },
                ],
                layer: "cloud",
              },
...
describe("CloudDriftPage", () => {
  it("renders runtime drift, unmanaged resources, import links, truth, and exact status explanations", async () => {
    ...
    render(
      <MemoryRouter
        initialEntries={["/cloud-drift?account_id=123456789012&provider=aws&region=us-east-1"]}
      >
    ...
    await waitFor(() =>
      expect(screen.getByText("cloud-resource:s3:payments-prod")).toBeInTheDocument(),
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Explain status for arn:aws:s3:::payments-prod" }),
    );
    ...
    await waitFor(() =>
      expect(screen.getByText("investigation-evidence-packet:drift-demo")).toBeInTheDocument(),
    );
    ...
    expect(
      calls.some(
        (call) =>
          call.path ===
          "/api/v0/investigations/drift/packet?account_id=123456789012&provider=aws&max_source_facts=50",
      ),
    ).toBe(true);
  ...
  it("forwards provider, account, region, and offset filters to bounded POST surfaces", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Account ID filter"), {
  ...
  it("keeps all summary tiles unavailable when a scoped read fails", async () => {
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent("Failed to load drift findings");
    for (const label of ["Multi-cloud drift", "AWS drift", "Unmanaged", "Import candidates"]) {
      ...
      expect(screen.getByText(label).closest(".stat-tile")).toHaveTextContent("unavailable");
    }
  });

  it("preserves successful surfaces when one scoped read fails", async () => {
    const calls: Array<{ path: string; body: unknown }> = [];
    const base = fakeDriftClient(calls);
    const client = {
      ...base,
      post: vi.fn(async (path: string, body: unknown) => {
        if (path === "/api/v0/aws/runtime-drift/findings") throw new Error("AWS read denied");
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent("AWS drift: AWS read denied");
    expect(screen.getByText("Multi-cloud drift").closest(".stat-tile")).toHaveTextContent("51");
    expect(screen.getByText("AWS drift").closest(".stat-tile")).toHaveTextContent("—");
    expect(screen.getByText("Unmanaged").closest(".stat-tile")).toHaveTextContent("1");
    expect(screen.getByText("Import candidates").closest(".stat-tile")).toHaveTextContent("1");
  });

  it("keeps loaded tiles available when only the next page fails", async () => {
    const calls: Array<{ path: string; body: unknown }> = [];
    const base = fakeDriftClient(calls);
    const client = {
      ...base,
      post: vi.fn(async (path: string, body: unknown) => {
        if (
          ...
          throw new Error("next page timed out");
        }
        return base.post(path, body);
      }),
    } as unknown as EshuApiClient;
    ...
    const next = await screen.findByRole("button", { name: "Next multi-cloud drift page" });
    fireEvent.click(next);
    expect(await screen.findByRole("alert")).toHaveTextContent("next page timed out");
    expect(screen.getByText("Multi-cloud drift").closest(".stat-tile")).toHaveTextContent("51");
    expect(screen.getByText("AWS drift").closest(".stat-tile")).toHaveTextContent("1");
  });
});
