...
import type {
  CodeRelationshipsResponse,
  CodeRelationshipStoryCoverage,
...
export function CodeGraphPage({
  ...
  useEffect(() => {
    ...
    setErr("");
    void client
      .post<CodeRelationshipStoryResponse>("/api/v0/code/relationships/story", {
      ...
      .then(async (env) => {
        ...
        try {
          const sourceEnv = await client.post<CodeRelationshipsResponse>(
            "/api/v0/code/relationships",
            { entity_id: selected.entityId, max_depth: 1 },
          );
          if (!sourceEnv.error) {
          ...
          /* source-link hydration is best-effort; story truth remains primary */
        }
        if (cancelled) return;
    ...
    <div className="page" style={{ maxWidth: "none" }}>
      ...
      >
        ...
        <select
          ...
          aria-label="Select repository for code graph"
        >
          ...
          {state.cycles.map((cycle, index) => (
            <div className="dead-row" key={`${cycle.sourceFile}:${cycle.targetFile}:${index}`}>
              ...
              {cycleSourceHref(cycle) ? (
                <Link className="mono" to={cycleSourceHref(cycle)!}>
