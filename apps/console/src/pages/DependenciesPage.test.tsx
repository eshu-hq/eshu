...
describe("DependenciesPage", () => {
  ...
  it("requires a package anchor before issuing a reverse lookup", async () => {
    ...
    const client = {
      get: async (path: string) => {
        ...
        if (path.includes("direction=reverse")) {
          return {
            data: { dependencies: [], direction: "reverse", truncated: false },
            error: null,
            truth: null,
          };
        }
    ...
    await waitFor(() =>
      expect(screen.getByText("Enter a package name to find its dependents.")).toBeInTheDocument(),
    );
  ...
  it("shows an honest empty state when the package graph has no edges", async () => {
    const client = {
      get: async () => ({
        data: { dependencies: [], direction: "forward", truncated: false },
        error: null,
        truth: null,
      }),
    } as unknown as EshuApiClient;
    ...
    expect(
      await screen.findByText(/No package dependencies in the indexed package graph/),
    ).toBeInTheDocument();
  });
  ...
  it("renders a repo dependency chain with the publisher leg labeled inferred when deep-linked by repo", async () => {
    ...
    const client = {
      get: async (path: string) => {
        if (path.includes("dependency-chains")) {
          chainPath = path;
