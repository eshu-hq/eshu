...
type CatalogState =
  ...
  | { readonly status: "ready"; readonly verbs: readonly RelationshipVerbTile[]; readonly totalEdges: number; readonly layerCount: number };
...
export function RelationshipsPage({ model, client }: {
  ...
  useEffect(() => {
    ...
    void loadRelationshipsCatalog(client)
      .then((data) => {
        if (cancelled) return;
        setCatalog({ status: "ready", verbs: data.verbs, totalEdges: data.totalEdges, layerCount: data.layerCount });
      })
      .catch((error) => {
        if (!cancelled) setCatalog({ status: "error", message: error instanceof Error ? error.message : "failed to load relationships" });
      });
    return () => { cancelled = true; };
  }, [client]);
  ...
  const demoVerbs = useMemo<readonly RelationshipVerbTile[]>(
    () => model.relationships.map((row) => ({ verb: row.verb, layer: row.layer, count: row.count, evidence: row.detail, detail: row.detail })),
    [model.relationships]
  );

  const verbs = catalog.status === "ready" ? catalog.verbs : live ? [] : demoVerbs;
  const totalEdges = catalog.status === "ready" ? catalog.totalEdges : verbs.reduce((sum, verb) => sum + verb.count, 0);
  const layerCount = catalog.status === "ready" ? catalog.layerCount : new Set(verbs.map((verb) => verb.layer)).size;
  ...
  function selectVerb(verb: string): void {
    ...
    void loadRelationshipEdges(client, verb)
      .then((data) => setEdges({ status: "ready", data }))
      .catch((error) => setEdges({ status: "error", message: error instanceof Error ? error.message : "failed to load edges" }));
  }
  ...
  return (
    <div className="page rel-page">
      <div className="page-intro">
        <h2>Relationships</h2>
        <p>
          Every typed edge Eshu has observed across the code-to-cloud graph — {fmt(totalEdges)} edges spanning {verbs.length} verbs.
          Each verb is a query: select one to list its concrete relationships with endpoints and the evidence behind them.
          This is how you answer who imports this, what deploys here, which workloads assume this role, what is affected by this change.
        </p>
      </div>

      <div className="rel-stats">
        <StatTile label="Relationships" value={fmt(totalEdges)} sub="typed edges observed" color={LAYER_COLOR.code} />
        <StatTile label="Typed verbs" value={verbs.length} sub="relationship types" color={LAYER_COLOR.deploy} />
        <StatTile label="Layers" value={layerCount} sub={LAYERS.join(" · ")} color={LAYER_COLOR.infra} />
        <div className="rel-nodes-tile">
          <StatTile label="Nodes" value={fmt(model.runtime.repositories)} sub="repositories" color={LAYER_COLOR.runtime} />
          <Link className="rel-nodes-browse" to="/explorer">Browse graph →</Link>
        </div>
      </div>
      ...
      <div className="explorer-filters rel-layer-filter">
        <button className={`layer-toggle ${layer === "all" ? "on" : "off"}`} onClick={() => setLayer("all")}>
          <span>All</span><strong className="rel-layer-count">{verbs.length}</strong>
        </button>
        {LAYERS.map((key) => (
          <button
            key={key}
      ...
      <div className="rel-layout">
        <Panel title="Verb catalog" sub={catalog.status === "loading" ? "Loading…" : `${visibleVerbs.length} relationship types`}>
          {visibleVerbs.length === 0 ? (
            <p className="empty">{catalog.status === "loading" ? "Loading verb catalog…" : "No relationship verbs for this layer."}</p>
          ) : (
                ...
                <li key={verb.verb}>
                  ...
                  >
                    ...
                    <span className="rel-verb-meta">
                      <span className="rel-verb-layer" style={{ color: LAYER_COLOR[verb.layer] }}>{verb.layer}</span>
        ...
        <Panel title={selectedVerb ? `${selectedVerb} edges` : "Concrete edges"} sub={selectedVerb ? "endpoints + evidence" : "Select a verb to list its edges"}>
          {selectedVerb === null ? (
            <p className="empty">Select a verb to list its concrete relationships with endpoints and evidence.</p>
            ...
            edges.data.edges.length === 0 ? (
              <p className="empty">No concrete edges recorded for this verb yet.</p>
            ) : (
              <>
                  ...
                  {edges.data.edges.map((edge, index) => (
                    <li className="rel-edge-row" key={`${edge.sourceId}-${edge.targetId}-${index}`}>
                      <span className="rel-edge-endpoints">
                        <span className="rel-edge-node mono" title={edge.sourceId}>{edge.sourceName}</span>
                        <span className="rel-edge-arrow">→</span>
                        <span className="rel-edge-node mono" title={edge.targetId}>{edge.targetName}</span>
                      </span>
                      {edge.evidence ? <span className="rel-edge-evidence t-mut">{edge.evidence}</span> : null}
                    </li>
                  ))}
                </ul>
                {edges.data.truncated ? (
                  <p className="t-mut rel-edge-trunc">Showing the first {edges.data.limit} edges.</p>
                ) : null}
              </>
            )
          ) : (
