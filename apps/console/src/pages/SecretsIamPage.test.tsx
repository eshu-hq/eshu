...
describe("SecretsIamPage", () => {
  it("loads a deep-linked posture review with truth, buckets, and drilldowns", async () => {
      ...
      get: async (path: string) => {
        ...
        if (path.includes("/posture-summary")) {
          return {
            data: summaryPayload(),
            error: null,
            truth: truthEnvelope("secrets_iam.posture_summary.read"),
          };
        }
        if (path.includes("/identity-trust-chains")) {
          return {
            data: trustChainsPayload(),
            error: null,
            truth: truthEnvelope("secrets_iam.identity_trust_chains.list"),
          };
        }
        if (path.includes("/privilege-posture-observations")) {
          return {
            ...
            error: null,
            truth: truthEnvelope("secrets_iam.privilege_posture_observations.list"),
          };
        }
        if (path.includes("/secret-access-paths")) {
          return {
            data: accessPathsPayload(),
            error: null,
            truth: truthEnvelope("secrets_iam.secret_access_paths.list"),
          };
        }
        return {
          data: gapsPayload(),
          error: null,
          truth: truthEnvelope("secrets_iam.posture_gaps.list"),
        };
      },
    ...
    expect(calls).toContain(
      "/api/v0/secrets-iam/identity-trust-chains?scope_id=scope-prod&limit=25&state=partial",
    );
  ...
  it("does not call the API until a scope is provided", async () => {
    ...
    expect(
      screen.getByText("Add scope_id to load reducer-owned secrets/IAM posture."),
    ).toBeInTheDocument();
  });
  ...
  it("renders an unavailable section instead of a fabricated empty state", async () => {
    ...
    expect(
      await screen.findByText(
        "unsupported_capability: identity trust chains require local-authoritative profile",
      ),
    ).toBeInTheDocument();
  ...
  return {
    ...
    summary: {
      identity_trust_chains_by_state: [
        { bucket: "exact", count: 3 },
        { bucket: "partial", count: 1 },
      ],
