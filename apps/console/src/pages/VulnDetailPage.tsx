...
const sevColor = (s: string): string =>
  SEVERITY_COLOR[(s as Severity) in SEVERITY_COLOR ? (s as Severity) : "medium"];
...
export function VulnDetailPage({
  ...
  const packetQuery = useMemo(
    () => supplyChainPacketQueryFromDetail(id, effective),
    [effective, id],
  );
  ...
  useEffect(() => {
    ...
    setPacket(null);
    setPacketError("");
    if (!client || !packetQuery)
      return () => {
        cancelled = true;
      };
    void loadSupplyChainImpactPacket(client, packetQuery)
      .then((result) => {
        if (!cancelled) setPacket(result);
      })
      .catch((err: unknown) => {
        if (!cancelled)
          setPacketError(err instanceof Error ? err.message : "failed to load impact packet");
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    <div className="page">
      <div className="page-intro">
        <Link to="/vulnerabilities" className="link-btn">
          ← Vulnerabilities
        </Link>
        <h2 style={{ marginTop: 8 }}>{id}</h2>
        <p>
          Advisory detail from{" "}
          <span className="mono">GET /api/v0/supply-chain/vulnerabilities/{"{id}"}</span>, with
          affected services joined from impact findings.
        </p>
      </div>
      ...
      {loading ? (
        ...
        effective.provenance === "unavailable" ||
        effective.provenance === "empty" ? (
        <div className="conn-state">
          <h2>Advisory unavailable</h2>
          <p>
            The vulnerability detail API did not return this advisory. It requires the
            vulnerability-intelligence collector and a live connection.
          </p>
        </div>
      ) : (
        <>
          {effective.provenance === "derived" ? (
            <p className="t-mut" style={{ marginBottom: 12, fontSize: ".82rem" }}>
              ...
              <span className="mono">GET /api/v0/supply-chain/vulnerabilities/{"{id}"}</span>.
              Showing reachable impact facts from{" "}
              <span className="mono">GET /api/v0/supply-chain/impact/findings</span> until issue
              #2217 proves the vulnerability-intelligence collector runtime path.
            </p>
          ) : null}
          <div className="grid g-4">
            <StatTile
              ...
              color={sevColor(effective.severity)}
              sub={effective.kev ? "KEV-listed" : "not on KEV"}
            />
            <StatTile
              label="CVSS"
              value={effective.cvss ?? "—"}
              color="var(--crit)"
              sub={effective.cvssVector ?? "no vector"}
            />
            <StatTile
              label="EPSS"
              value={effective.epss !== null ? `${(effective.epss * 100).toFixed(0)}%` : "—"}
              color="var(--ember)"
              sub="exploit probability"
            />
            <StatTile
              label="Fix"
              value={effective.fixedVersion ?? "—"}
              color={effective.fixedVersion ? "var(--teal)" : "var(--crit)"}
              sub={effective.fixedVersion ? "patch available" : "no fixed version"}
            />
          </div>
          ...
          >
            <div className="grid" style={{ gap: "var(--gap)", alignContent: "start" }}>
              <Panel title="Identity">
                <div className="kv">
                  <div>
                    <span className="t-mut">Canonical</span>
                    <span className="mono">{effective.canonicalId}</span>
                  </div>
                  <div>
                    <span className="t-mut">CVE</span>
                    <span className="mono">{effective.cveIds.join(", ") || "—"}</span>
                  </div>
                  <div>
                    <span className="t-mut">GHSA</span>
                    <span className="mono">{effective.ghsaIds.join(", ") || "—"}</span>
                  </div>
                  <div>
                    <span className="t-mut">CWE</span>
                    <span className="mono">{effective.cwes.join(", ") || "—"}</span>
                  </div>
                </div>
              </Panel>
              <Panel title={`Affected packages (${effective.packages.length})`}>
                {effective.packages.length === 0 ? (
                  <p className="empty">No affected packages reported.</p>
                ) : (
                  <ul className="plain-list">
                    {effective.packages.map((p, i) => (
                      ...
                      >
                        ...
                        {p.fixedVersion ? (
                          <Badge tone="teal">{p.fixedVersion}</Badge>
                        ) : (
            ...
            <Panel
              ...
              sub={`${services.length} reachable · AFFECTED_BY`}
            >
              {services.length === 0 ? (
                <p className="empty" style={{ padding: 20 }}>
                  No affected services joined from impact findings for this advisory.
                </p>
              ) : (
