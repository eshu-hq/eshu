  ...
  return (
    <div className="page">
      ...
      <section className="vulnerability-view-status" aria-label="Vulnerability view status">
        <ViewStatusCard label="Reachable impact" status={reachableStatus} />
        <ViewStatusCard label="Known intelligence" status={catalogStatus} />
      </section>
...
function ViewStatusCard({
  ...
  return (
    <article className="vulnerability-view-status-card" data-state={status.state}>
      <span>{label}</span>
...
function statusForReachable(model: ConsoleModel): ViewStatus {
  ...
  const count = model.vulnerabilities.length;
  return {
    state: "populated",
    value: `${count} reachable ${count === 1 ? "finding" : "findings"}`,
    detail: "These findings have admitted service-impact evidence.",
  };
}
