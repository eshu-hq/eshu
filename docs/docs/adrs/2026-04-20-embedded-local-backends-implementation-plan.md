...
- treat the old `ESHU_REDUCER_WORKERS=1` and single-item claim window as historical stability baselines, not the final performance answer. The active design now uses NumCPU NornicDB reducer workers, NumCPU local-authoritative projector workers, and durable conflict keys; the next evidence should decide whether shared projection needs the same partition-lease seam.
