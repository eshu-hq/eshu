...
| `eshu docs verify [path] [--fail-on <statuses>] [--limit <n>] [--max-bytes <n>] [--persist]` | Verify local Markdown-family documentation claims against the current Eshu CLI command tree, OpenAPI endpoint inventory, known Eshu environment variables, explicit local repo paths in backticks or Markdown links, and tagged or digested container image refs found in local deployment manifests. Emits documentation finding and evidence-packet facts in memory; with `--persist`, writes them through the Postgres fact store and reuses unchanged persisted findings for `--fail-on` checks. | No |
