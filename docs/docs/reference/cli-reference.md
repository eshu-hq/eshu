...
| `eshu docs verify [path] [--fail-on <statuses>] [--limit <n>] [--max-bytes <n>]` | Verify local Markdown-family documentation claims against the current Eshu CLI command tree, OpenAPI endpoint inventory, and known Eshu environment variables. Emits documentation finding and evidence-packet facts in memory and supports canonical `--json` output. | No |
