...
| `eshu trace service <name> [--repo <selector>] [--env <environment>] [--service-id <id>]` | Render the `/api/v0/services/{service_name}/story` dossier as an operator trace with canonical `--json` envelope support. The human view includes a code-to-runtime segment summary that marks exact, derived, and missing evidence instead of guessing. Duplicate service names return disambiguation candidates instead of a guessed trace. | Yes |
