...
API-backed typed graph-neighborhood map. `eshu docs verify` is available as the
first active documentation-truth verifier slice for local Markdown-family
documents. Their shared design lives in
...
| `eshu trace service <name> [--repo <selector>] [--env <environment>] [--service-id <id>]` | Render the `/api/v0/services/{service_name}/story` dossier as an operator trace with canonical `--json` envelope support. The human view includes truth freshness plus a code-to-runtime segment summary that marks exact, derived, and missing evidence instead of guessing. Duplicate service names return disambiguation candidates instead of a guessed trace; stale freshness exits `4`, and partial code-to-runtime traces exit `5` while still printing usable evidence. | Yes |
...
| `eshu docs verify [path] [--fail-on <statuses>] [--limit <n>] [--max-bytes <n>] [--image-truth auto\|local\|api] [--persist]` | Verify local Markdown-family documentation claims against the current Eshu CLI command tree, OpenAPI endpoint inventory, known Eshu environment variables, explicit local repo paths in backticks or Markdown links, tagged or digested container image refs, and explicit Terraform block addresses such as `aws_s3_bucket.logs`, `data.aws_iam_policy_document.reader`, or `module.network`. Image refs default to local deployment-manifest truth; `--image-truth api` checks `/api/v0/supply-chain/container-images/identities` through `--service-url`, `--api-key`, and `--profile`, while `auto` resolves to API truth only when remote API config is explicitly present. Emits documentation finding and evidence-packet facts in memory; with `--persist`, writes them through the Postgres fact store and reuses unchanged persisted findings for `--fail-on` checks only when the docs fingerprint and effective image truth source match. | Optional |
...
24-hour recent completed generations, last completed timestamps,
