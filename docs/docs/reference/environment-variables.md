...
| `ESHU_NORNICDB_ENTITY_PHASE_CONCURRENCY` | `NumCPU` clamped to `4` | graph writer | Worker count for parallel canonical entity-phase chunk dispatch. | Raise when canonical_write is dominated by serial entity chunks and NornicDB has commit headroom. Clamped to `16` to bound Bolt session demand. |
