...
| `ESHU_SNAPSHOT_WORKERS` | `min(NumCPU, 8)` | collector | Concurrent repo snapshot workers. | Raise when CPU/disk/network have headroom and many small repos wait; lower when memory or disk I/O saturates. |
| `ESHU_PARSE_WORKERS` | `min(NumCPU, 8)` | collector snapshotter | Concurrent file parse workers inside each snapshot. | Raise for CPU headroom; lower when parser memory or disk reads saturate. |
...
| Variable | Default | Read By | Purpose | Tune When |
| --- | --- | --- | --- | --- |
| `ESHU_PROJECTOR_WORKERS` | Default: `min(NumCPU, 8)`; NornicDB local-authoritative: `1` | ingester projector | Source-local projector worker count. | Raise when source-local projection is CPU-bound and graph backend can absorb writes; keep NornicDB local-authoritative serialized until backend write concurrency is proven safe. |
...
| `ESHU_REDUCER_WORKERS` | Neo4j: `min(NumCPU, 4)`; NornicDB: `min(NumCPU, 8)` | reducer | Reducer intent worker count. | Lower only when conflict-domain fencing still shows graph write conflicts or backend saturation. Raise explicitly only after measuring queue tail, CPU, disk, and graph-write timeout rates. |
