...
| **`inspect_call_graph_metrics`** | Inspect recursive functions and hub functions with a required `repo_id`, optional `language`, `limit`, `offset`, source handles, call-degree counts, recursion evidence, and truncation metadata. Returns canonical `functions` rows. | "Find recursive functions in this repo." |
