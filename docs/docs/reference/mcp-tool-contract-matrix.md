...
| `investigate_import_dependencies` | repository, language, source-file, target-file, source-module, and target-module filters optional by prompt shape; at least one scope anchor required | `limit` and `offset` | yes | prompt-ready for imports by file, module importers, package imports, Python file import cycles, and cross-module calls without raw Cypher |
