...
| `inspect_call_graph_metrics` | repository required; language optional | non-negative `limit` and `offset` | yes | prompt-ready for recursive function and high-degree hub-function prompts without raw Cypher; returns canonical `functions` rows with call counts, source handles, recursion evidence, and truncation |
