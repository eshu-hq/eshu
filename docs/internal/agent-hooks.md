...
### The trailing-CONSENT pitfall

`CONSENT:` is parsed only as a **leading metadata block** in both hooks — the
...
`goal-refresh.sh` scans the goal text on every restatement for a directory
name under `.agents/skills/` and appends "load `<id>` now" for any this
session has not loaded, reusing the `/tmp/claude-skill-loaded-<sid12>-<id>`
marker `skill-nudge.sh`/`skill-loaded.sh` already own. Matching is
word-bounded. The scan and note assembly live in `.claude/hooks/lib/` to keep
`goal-refresh.sh` under the 500-line cap;
`scripts/test-goal-refresh-hook-skill-nudge-cases.sh` covers nudged, loaded,
and not-a-skill.
...
Activation, the worktree-vs-main-checkout wiring, and how to diagnose a hook
that should have fired but did not are in
[Installing And Activating Agent Hooks](agent-hooks-installing.md), split out
to keep this file under the repo's 500-line Markdown cap.
