...
| --- | --- | --- | --- |
| CloudWatch metric alarm | `go/internal/collector/awscloud/service/cloudwatch/scanner.go` | `aws_resource` (`ResourceTypeCloudWatchAlarm`, `aws_cloudwatch_alarm`) + `aws_relationship` (`cloudwatch_alarm_observes_metric`, `cloudwatch_alarm_notifies_sns_topic`) | ARN + name; `CorrelationAnchors: [alarmARN, name]` |
...
for a scope generation, mirroring `awsdrift.BuildReducerIntent` (the
