...
Phase-1 stabilization status: Compose and Helm now pin NornicDB `v1.1.6` and
...
Eshu pins NornicDB `v1.1.6` for Compose graph startup. The per-database
...
preserved in later releases; v1.1.6 is the latest published multi-arch Docker Hub
...
truth change. Compose and Helm now pin NornicDB `v1.1.6`, disable BM25/vector
