...
`go/internal/reducer/supplychain/core/writer.go`; the shared
...
| --- | --- | --- |
| `reducer_supply_chain_impact_finding` | Full governance | Producer writes the supply-chain finding payload in `go/internal/reducer/supplychain/core/writer.go`. It is a public HTTP/MCP finding source (`docs/public/reference/mcp-prompt-surface-audit.md:131`, `go/internal/query/supply/chain/impact/findings_queries.go`) and part of #4585. W1 adds struct/schema/typed writer; W2 moves query, aggregate, explanation, and Postgres loader reads to typed decode. |
| `reducer_aws_cloud_runtime_drift_finding` | Full governance | Producer writes AWS runtime drift findings in `go/internal/reducer/awscloud/aws_cloud_runtime_drift_writer.go`; #4585 names this as one of the existing domain finding models. The AWS-specific store already treats it as an exported Postgres read surface (`go/internal/storage/postgres/aws_cloud_runtime_drift_findings.go`). W1 governs it with the drift finding family; W2 aligns it with the #4585 envelope. |
| `reducer_multi_cloud_runtime_drift_finding` | Full governance | Producer writes provider-neutral drift findings in `multicloudruntimedrift.PostgresMultiCloudRuntimeDriftWriter.WriteMultiCloudRuntimeDriftFindings` and `multiCloudRuntimeDriftTypedPayload` (`go/internal/reducer/multicloudruntimedrift/multi_cloud_runtime_drift_writer.go`). It is explicitly supported in the capability matrix and MCP/query readback (`specs/capability-matrix/cloud-runtime-drift-readback.v1.yaml:6,24-27`, `go/internal/query/cloud_runtime_drift.go`, `go/internal/mcp/cloud/runtime_drift_tools.go`). W1 governs; W2 typed-decodes the cloud runtime drift store and #4585 envelope. |
| `reducer_cloud_resource_identity` | Full governance | Producer writes canonical cloud inventory rows in `cloudinventory.PostgresCloudInventoryAdmissionWriter.WriteCloudInventoryAdmission` (`go/internal/reducer/cloudinventory/cloud_inventory_admission_writer.go`). It is a public cloud inventory readback (`specs/capability-matrix/cloud-inventory-readback.v1.yaml:6,21-23`, `go/internal/query/cloud_inventory_readback.go`, `go/internal/mcp/cloud/inventory_tools.go`). W1 governs; W2 typed-decodes cloud inventory query/MCP/Postgres reads. |
| `reducer_container_image_identity` | Full governance | Producer writes digest identity decisions in `go/internal/reducer/containerimage/container_image_identity_writer.go`. It feeds public supply-chain image identity reads and SBOM/impact explanations (`docs/public/reference/mcp-prompt-surface-audit.md:132`, `go/internal/query/container_image_identities.go`, `go/internal/query/sbom_attestation_attachments.go`). W1 governs; W2 typed-decodes image identity and supply-chain evidence consumers. |
| `reducer_sbom_attestation_attachment` | Full governance | Producer writes SBOM/attestation attachment decisions in `go/internal/reducer/sbomattest/sbom_attestation_attachment_writer.go`. It has public API/MCP read surfaces and aggregate indexes (`docs/public/reference/mcp-prompt-surface-audit.md:136`, `go/internal/query/sbom_attestation_attachments.go`, `go/internal/storage/postgres/schema_fact_records_sbom_test.go`). W1 governs; W2 typed-decodes attachment reads and aggregate stores. |
| `reducer_security_alert_reconciliation` | Full governance | Producer writes provider alert reconciliation payloads in `go/internal/reducer/securityalert/reconciliation_writer.go`. It is a public supply-chain security-alert read surface (`docs/public/reference/mcp-prompt-surface-audit.md:135`, `go/internal/query/supply/chain/alerts/store.go`, `go/internal/storage/postgres/facts_active_security_alert_reconciliation.go`). W1 governs; W2 typed-decodes reconciliation query and Postgres consumers. |
| `reducer_ci_cd_run_correlation` | Full governance | Producer writes CI/CD run correlation rows in `go/internal/reducer/cicdrun/ci_cd_run_correlation_writer.go`. It is a public list/aggregate surface and incident runtime evidence source (`docs/public/reference/mcp-prompt-surface-audit.md:133`, `go/internal/query/ci_cd_run_correlations.go`, `go/internal/query/incident/sql/runtime.go`). W1 governs; W2 typed-decodes CI/CD query and incident context consumers. |
| `reducer_incident_repository_correlation` | Full governance | Producer writes incident-to-repository decisions in `go/internal/reducer/incident/incident_repository_correlation_writer.go`. It gates scoped incident auth and service incident evidence (`go/internal/query/incident/store/authorizer.go`, `go/internal/query/auth_scoped_routes.go`, `go/internal/storage/postgres/service_incident_evidence_loader.go`). W1 governs; W2 typed-decodes incident context and authz consumers. |
| `reducer_kubernetes_correlation` | Full governance | Producer writes Kubernetes runtime correlation decisions in `go/internal/reducer/kubernetescorrelation/kubernetes_correlation_writer.go` (issue #6061). It has a dedicated query surface and incident runtime consumers (`docs/internal/design/388-kubernetes-correlation-readmodel.md:20,41`, `go/internal/query/kubernetes/correlations.go`, `go/internal/query/incident/sql/runtime.go`). W1 governs; W2 typed-decodes Kubernetes query and incident runtime reads. |
| `reducer_observability_coverage_correlation` | Full governance | Producer writes coverage/gap decisions in `go/internal/reducer/obscoverage/observability_coverage_correlation_writer.go`. It is a documented read model with a query surface (`docs/internal/design/391-observability-coverage-correlation.md:172,486`, `docs/public/reference/observability-evidence.md:196`, `go/internal/query/observability/coverage/correlations.go`). W1 governs; W2 typed-decodes observability coverage reads. |
| `reducer_package_consumption_correlation` | Full governance | Producer writes consumption decisions in `go/internal/reducer/packages/correlation/writer.go`. It is a supply-chain evidence and package registry read surface (`go/internal/query/package/registry/correlations.go`, `go/internal/storage/postgres/facts_active_supply_chain_impact.go`, `go/internal/query/read-models.md`). W1 governs all three package correlation kinds together; W2 typed-decodes package, supply-chain, and security-alert consumers. |
| `reducer_package_ownership_correlation` | Full governance | Producer writes ownership decisions in `go/internal/reducer/packages/correlation/writer.go`. It is consumed by package registry correlations and code-import owner resolution (`go/internal/query/package/registry/correlations.go`, `go/internal/storage/postgres/facts_active_package_ownership.go`, `go/internal/reducer/code_import_owner_facts.go`). W1 governs with the package correlation family; W2 typed-decodes owner consumers. |
| `reducer_package_publication_correlation` | Full governance | Producer writes publication decisions in `go/internal/reducer/packages/correlation/writer.go`. It shares the package registry and code-import owner read path (`go/internal/query/package/registry/correlations.go`, `go/internal/storage/postgres/facts_active_package_ownership.go`, `go/internal/reducer/code_import_owner_facts.go`). W1 governs with the package correlation family; W2 typed-decodes publication consumers. |
| `reducer_service_catalog_correlation` | Full governance | Producer writes service catalog correlation rows in `go/internal/reducer/servicecatalog/service_catalog_correlation_writer.go` (issue #6061 moved this domain out of the flat reducer root into the `servicecatalog` family subpackage). It is an API/MCP read model and supply-chain/incident evidence source (`docs/internal/design/563-service-catalog-manifest-fact-emitter.md:64,73,235`, `go/internal/query/service/catalog_correlations.go`, `go/internal/storage/postgres/service_catalog_id_resolver.go`). W1 governs; W2 typed-decodes service catalog, incident, and supply-chain consumers. |
| `reducer_workload_identity` | Full governance | Producer writes workload identity canonical facts in `go/internal/reducer/workload_identity_writer.go`. It is used by repository summaries and supply-chain runtime evidence (`go/internal/query/repository_read_model_summary.go`, `go/internal/query/content_reader_repository_catalog.go`, `docs/public/reference/security-intelligence-release-gate.md:69`). W1 governs; W2 typed-decodes repository summary and supply-chain evidence consumers. |
| `reducer_platform_materialization` | Full governance | Producer writes deployment/platform materialization facts in `go/internal/reducer/platformfam/platform_materialization_writer.go`. It is used in repository summaries and supply-chain deployment evidence (`go/internal/query/repository_read_model_summary.go`, `go/internal/query/supply_chain_impact_explain_runtime_test.go`, `go/internal/query/openapi/paths/supply/chain/impact_findings.go`). W1 governs; W2 typed-decodes repository and supply-chain consumers. |
| `reducer_eshu_search_document` | Full governance | Producer writes curated search documents in `go/internal/reducer/eshusearch/eshu_search_document_domain.go` and `go/internal/reducer/eshusearch/eshu_search_document_writer.go`. It is the design-430 search lane read model (`docs/internal/design/430-nornicdb-graph-search-split.md:246`, `docs/public/reference/search-document-projection.md:99`, `go/internal/storage/postgres/eshu_search_document.go`). W1 governs; W2 typed-decodes search document reads. |
| `reducer_secrets_iam_identity_trust_chain` | Full governance | Producer writes secrets/IAM read models in `go/internal/reducer/secretsiam/secrets_iam_trust_chain_writer.go`. It is a public posture read model (`docs/public/reference/secrets-iam-posture-collector-contract.md:162`, `go/internal/query/secrets/trust_chain.go`). W1 governs all four secrets/IAM reducer read models together; W2 typed-decodes posture query stores. |
| `reducer_secrets_iam_privilege_posture_observation` | Full governance | Producer writes the privilege posture model in `go/internal/reducer/secretsiam/secrets_iam_trust_chain_writer.go`; public docs list it as a posture output (`docs/public/reference/secrets-iam-posture-collector-contract.md:163`, `docs/internal/design/1314-secrets-iam-graph-promotion-gate.md:46-49`). W1 governs with secrets/IAM; W2 typed-decodes posture query stores. |
| `reducer_secrets_iam_secret_access_path` | Full governance | Producer writes secret access path read models in `go/internal/reducer/secretsiam/secrets_iam_trust_chain_writer.go`; public docs list it as a posture output (`docs/public/reference/secrets-iam-posture-collector-contract.md:164`, `go/internal/query/secrets/posture_stores.go`). W1 governs with secrets/IAM; W2 typed-decodes posture query stores. |
| `reducer_secrets_iam_posture_gap` | Full governance | Producer writes posture gap read models in `go/internal/reducer/secretsiam/secrets_iam_trust_chain_writer.go`; query stores consume the kind (`go/internal/query/secrets/posture_stores.go`) and tests already treat the payload as graph-projection evidence (`go/internal/reducer/secretsiam/secrets_iam_graph_projection_extract_test.go`). W1 governs with secrets/IAM; W2 typed-decodes posture query stores. |
| `reducer_cloud_asset_resolution` | Documented exemption | Producer writes one canonical cloud-asset reconciliation row in `cloudasset.PostgresCloudAssetResolutionWriter.WriteCloudAssetResolution` (`go/internal/reducer/cloudasset/cloud_asset_resolution_writer.go`), but repository search on 2026-07-06 found no non-test query, MCP, storage, docs, or capability-matrix consumer for this kind. The public cloud inventory read contract has moved to `reducer_cloud_resource_identity` (`go/internal/query/cloud_inventory_readback.go`). W1 does not author a struct/schema for this kind; if a reader is added, promote it to full governance first. |
