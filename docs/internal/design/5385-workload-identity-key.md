...
| Shape | Where | Note |
| --- | --- | --- |
| Property name from a Go variable | `go/internal/query/impact/change_surface_resolvers.go`, callers at `:29,32,41,42,80,83,88,93` | Builders carry no literal `id`; call sites carry no `$`. Neither end matches. |
...
| Label-less `impacted.id <> $target_id`, can bind a Workload | `go/internal/query/impact/change_surface_traversal.go`, `impact/change_surface_legacy.go:126` | `:34` matches unlabelled `(impacted)` with the label predicate at `:36`; the legacy statement names both labels inside `any(label IN labels(impacted) …)`. **`:36` is textually present and operationally inert** — see below. |
| Same `impacted.id <> $target_id` shape, but **`Repository`-anchored** | `go/internal/query/impact/change_surface_traversal.go`, `:42` | Both label `impacted` inline on the MATCH as `(impacted:Repository)`, so neither is label-less and neither names `Workload` or `WorkloadInstance`. `:18` is a `(start:Repository)<-[:DEPENDS_ON*]-(impacted:Repository)` traversal and cannot bind a workload at all. Listed only because they share the shape a reader would grep for. |
...
**On `:36` being inert.** `impact/change_surface_traversal.go:150-156` states that the
...
4. **Re-key `WorkloadInstance.workload_id` in the same change.** The item most
   ...
   `:168`), and `go/internal/query/impact/change_surface_resolvers.go` (the phase-2
   ...
   A fourth site, `go/internal/query/impact/resource_investigation_reads.go`, only *projects*
