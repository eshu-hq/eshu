...
| get_workload_story | ci_cd/image/package chain absent | `entity/workload_handlers.go` |
| get_repo_story | publication/ownership/image links | `repository/story.go` |
| trace_deployment_chain | image_ref→digest identity + which-CI-run-produced-image hop | `impact/trace_deployment.go`, `impact/trace_deployment_resources.go` |
