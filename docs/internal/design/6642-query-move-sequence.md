...
Owner calls this plan does not make.

1. **The four `content/read` merges are not `git mv`.** The issue's Scope says
   "Move and rename"; concatenating two files is neither. Without them the leaf
   lands at 41 and cannot pass the cap; with one merge it lands at 40 with zero
   headroom. Four is the recommendation. This needs an explicit yes.
2. **`contract/` ← `querycontract` forces the current `contract/` to move.**
   The issue settled the rename but did not say where the 40 capability-matrix
   files go. `capability/matrix/` is this document's proposal, not the issue's.
3. **`tracing` is treated as the issue's `queryspan`.** No package named
...
4. **The root test rule.** 403 of 728 root test files match no moving root
...
- [ ] PR 1 — spine repoint
- [ ] PR 2 — `contract/` ← `querycontract` (closes #6597)
- [ ] PR 3 — `capability/matrix/` + `capability/`
