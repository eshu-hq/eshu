...
| `facts/schema/` | 7 | `fact_records` DDL constants, read only by tests; keeps `facts/` at 35. |
| `freshness/` and its `aws/`, `gcp/`, `incident/`, `vulnerability/` children | 2 + 10 | Repository freshness, plus four collector freshness-trigger stores with one schema/claim/reap shape (naming question N4). |
...
| `service/catalog/`, `service/evidence/`, `service/materialization/` | 1, 2, 1 | Service catalog id resolution, service documentation and incident evidence, and service re-materialization. |
