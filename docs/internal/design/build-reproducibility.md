...
2. Go compilation is deterministic by default — `go build` does not embed
   wall-clock timestamps, and `-trimpath` removes filesystem-path variance.
   The `SOURCE_DATE_EPOCH` build-arg feeds the cgo toolchain (clang/lld) and
   the OCI label rewrite, not the Go compiler itself.
...
though the tag text has not changed. Separately, the Docker image labels
generated by `docker/metadata-action` embed the current build-time clock in
`org.opencontainers.image.created`, and the Go build uses cgo for
tree-sitter bindings, whose native toolchain (clang/lld) can produce
non-deterministic artifacts when the build timestamp varies. The result: a
rebuild of the same commit on a different day can produce a different image
digest.
...
the output binary. The existing `-trimpath` flag strips filesystem-path variance
from the compiled artifact.

The `SOURCE_DATE_EPOCH` environment variable is consumed by the cgo toolchain
(clang/lld) for tree-sitter native bindings, and is reused in §3.3 for the OCI
label rewrite. Feeding it a fixed value removes the last per-run variable from
the build.
...
This ties the binary's embedded cgo artifacts to the source commit, not the
...
- [Go Build Reproducibility](https://pkg.go.dev/cmd/go#hdr-Build_Reproducibility)
