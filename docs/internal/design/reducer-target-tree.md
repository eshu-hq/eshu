...
| `packages/` | `correlation` (new: `consumption*`, `source*`, `publication.go`, `provenance_edges.go`, `writer*`, `payloads.go` + `security_alert_manifest_dependency_match.go`), `source` (`packagesourcecore`, 2) | — (family fully moved; nothing remains at root) |
