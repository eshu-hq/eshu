...
**CORRECTION (#6059, during execution).** That sentence overstates several of these, and the tool that produced it under-reported. A compile probe of each family in isolation silently truncated at the Go compiler's 10-error limit, and reported "self-contained" whenever the build failed for any reason other than an undefined symbol — for graph_install it died on a missing `//go:embed` asset before resolving a single symbol. Re-measured with `-gcflags=-e` and the embed assets copied: graph_install needs `printJSON` and `localGraphReadVersion` (the latter from the local_host/local_graph supervisor cluster); vuln_scan needs 38 symbols across 73 sites including a dozen from that same cluster; first_run needs ~30 across 68 sites; freshness 49 sites; change_impact 35; component 27. The genuinely wiring-only leaves are service_report_cmd, assistant_hook_preflight, operator_digest, admin, and evidence_packet. `scan` is a third shared-core layer this appendix does not name, blocking vuln_scan, first_run, hosted_setup and evidence_bundle. Treat `clean` in the table below as unverified until re-probed. Six committed artifacts hardcode paths or test-run targets against this package's current flat layout (two CI gate files, one lint exception file, one backend-conformance spec, one evidence-continuity spec, one verify script) and must be updated in lockstep with any extraction — none of them are covered by a broad enough glob to survive a move into go/internal/cli/\*\* unmodified.
