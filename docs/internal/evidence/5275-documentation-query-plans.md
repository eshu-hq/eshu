...
4.20 times slower while the production-shaped 1.6-million-row search completed
in 1.150 seconds without it.
...
| Redesign free-text search around an indexable column | Superseded by measurement | Three index hypotheses were tested. The fastest plan added a 4.20x write cost, while the current production-shaped search stayed below the local interactive target. No search schema or query change lands. |
...
`c80c86c00148f36b4e571100508ec85659ee1728fa6e13d1f075b6bcbc707340`.

The measured SQL is mechanically bound to production, rather than trusted as
a hand-maintained copy. Query-package tests render the actual
`buildDocumentationFactsSQL` predicate, seven-kind allowlist, scope, ordering,
limit, and offset and compare them with the prepared search. Storage-package
tests compare the actual 17-column `upsertFactBatchPrefix`, complete
`upsertFactBatchSuffix`, and fencing predicate with the write probe. Mutation
tests prove that changes on either production side break the guard. The source
hashes for this run are:
...
| --- | ---: | ---: | --- |
| Execution time | 13.994 ms | 0.286 ms | 13.708 ms faster; 48.93x |
...
| Planning | 0.191 ms, 17 shared hits | 0.308 ms, 53 hits and 1 read | recorded, not optimized |
...
| Planning / execution | 0.182 ms / 1,150.248 ms |
| Terminal result | 51 rows; digest `2c5a286517da306b3461ba075696a3fa` |

The 1.150-second result is below the 2-3 second target on this named local
...
| --- | --- | --- | --- |
| Broad expression GIN with `gin_trgm_ops` | 16.636 s, 315 MB | `Bitmap Index Scan`, 1,600 rows x 1 loop, 22 index hits; whole plan 1,622 hits; 4.855 ms execution | Fastest read, but its repeated write cost was 4.20x baseline. |
...
| Scoped multicolumn GIN using `btree_gin` | 16.327 s, 320 MB | `Bitmap Index Scan`, 1,600 rows x 1 loop, 476 index hits; whole plan 2,076 hits; 8.928 ms execution | Slower and larger than the broad GIN; repeated write cost was 4.40x baseline. |
...
| --- | ---: | ---: | ---: | ---: |
| No candidate | 86.589 ms | 122,348 hits | 1,174 / 1,174 | 1.00x |
| Broad expression GIN | 363.894 ms | 233,343 hits | 1,255 / 1,255 | 4.20x |
| Scoped multicolumn GIN | 380.852 ms | 255,733 hits | 1,267 / 1,267 | 4.40x |

The fastest read candidate added 277.305 ms to the repeated 10,000-row insert.
...
Performance Evidence: the accepted findings index changed a 13.994 ms parallel
sequential scan into a 0.286 ms index scan with the same 66 rows and digest. The
production-shaped 1.6-million-row search completed in 1,150.248 ms. The fastest
rejected search index reduced it to 4.855 ms but increased the repeated
10,000-row production-shaped write from 86.589 ms to 363.894 ms, so it did not
land.
...
go test ./internal/storage/postgres \
  -run 'TestDocumentation(QueryPlanShim|ReadIndexFinalSchemaMatchesCurrentQueries|ReadIndexContractRejectsEveryMissingKey|ReadIndexConcurrentMigrationsAreIsolated|RejectedAndLegacyIndexesAreNotReplayed)' \
  -count=1
