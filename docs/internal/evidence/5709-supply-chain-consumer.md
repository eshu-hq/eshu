...
`git`, and `sbom_attestation` scopes (`candidateFactKinds` in `internal/projector/container/image/identity`,
`internal/projector/container/image/identity/intents.go`), so identity output can
