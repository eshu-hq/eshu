...
| B-7 golden corpus | `bash scripts/verify-golden-corpus-gate.sh` | pass 3/4 runs on the exact artifact (176s, 195s, 177s; 561 pass, 0 required-fail each); one intermittent `suppression ignored_hidden` miss on otherwise identical code+image (see note below) |
...
One of four v1.3.3 B-7 runs missed `suppression ignored_hidden query assertion
...
and twice more on the exact artifact; CI re-runs arbitrate.
