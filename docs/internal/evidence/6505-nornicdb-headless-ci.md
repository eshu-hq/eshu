# #6505 — Historical source-built NornicDB headless CI fix

Historical status: #6162 replaced this exact-source build with the published
v1.3.1 digest, so `NORNICDB_HEADLESS` and the upstream UI build are no longer
part of Eshu's Compose or CI paths. This note preserves the incident and the
fix that applied while the source-built image was active.
...
The fix at the time threaded upstream's documented `HEADLESS` build arg through
`docker-compose.yaml` (`args: HEADLESS: ${NORNICDB_HEADLESS:-false}`; local
default `false` kept the full UI build) and set `NORNICDB_HEADLESS=true`
everywhere CI built the image: step-level env on the three image-building
...
pin — including the #261/#290 fixes — was untouched. A pinning test asserted
...
operators inherited the local default unless they exported the knob.

No-Regression Evidence (#6505): backend behavior was unchanged by that fix. The
pin, build context, dockerfile, Go builder stage, and every backend flag and env
were byte-identical; only the UI static bundle was skipped when the knob was set,
and no gate served the NornicDB UI (all three jobs asserted graph determinism,
