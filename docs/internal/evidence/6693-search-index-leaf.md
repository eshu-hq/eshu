...
For each of the 17 moved test names, `go test ./internal/storage/postgres/search/index/... -list '^Name$' -count=1 | rg -q '^Name$'` exits 0 and the same command against `./internal/storage/postgres` exits 1 (confirmed absent from root). Ran as a loop over all 17 names; result: `newfail=0 oldfail=0` (zero names missing from the new package, zero names still present in root).
