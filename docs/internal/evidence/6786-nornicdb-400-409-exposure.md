...
| Code relationship lookup by name and `repo_id` (`codequery/relationship_handlers.go`) | X5 | Not reachable on the default NornicDB dispatch (`relationships.MetadataRow`, proven live and correct); affected only when `ESHU_GRAPH_BACKEND=neo4j` is set while the backing store is actually NornicDB (a mismatched configuration). Hardened in #6786 | Fixed/hardened in #6786; RED/GREEN in [6786-repository-dependency-marker-and-relationship-repo-anchor.md](6786-repository-dependency-marker-and-relationship-repo-anchor.md), including a live subtest of the real NornicDB-dispatch path |
