...
Moved to [Module node identity: review follow-ups on PR
#6106](module-node-identity-6106-review-follow-ups.md), which carries the
admission-versus-write-fence correction, the reducer graph writer inventory an
identity cutover has to check, the absent-language sweep key, and the two
test-quality findings. It was split out to keep this file under the
