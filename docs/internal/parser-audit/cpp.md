...
|---|---|
| Functions (name, class_context, line_number) | `cpp/engine_cpp_dead_code_roots_test.go:13`: `TestDefaultEngineParsePathCPPMarksDeadCodeRootKinds`; `engine_systems_test.go:68`: `TestDefaultEngineParsePathCPP` |
...
| Enums (name, `lang`, `line_number`, `end_line`) | `cpp/engine_cpp_entity_test.go:17` (`TestDefaultEngineParsePathCPPEnumExtraction`); `cpp/engine_cpp_entity_test.go:139` (`TestDefaultEngineParsePathCPPEnumPreScan`) |
| Unions (name, `lang`, `line_number`, `end_line`) | `cpp/engine_cpp_entity_test.go:64` (`TestDefaultEngineParsePathCPPUnionExtraction`); `cpp/engine_cpp_entity_test.go:109` (`TestDefaultEngineParsePathCPPUnionPreScan`) |
| `cpp.main_function` | `cpp/engine_cpp_dead_code_roots_test.go:88`; `cpp/dead_code_roots_test.go:126` |
| `cpp.public_header_api` (free functions) | `cpp/engine_cpp_dead_code_roots_test.go:194` (`TestDefaultEngineParsePathCPPMarksIncludedHeaderPublicAPI`); `cpp/dead_code_roots_test.go:127` |
| `cpp.public_header_api` (class methods) | `cpp/engine_cpp_dead_code_roots_test.go:195`; `cpp/dead_code_roots_test.go:128` |
| `cpp.public_header_api` (namespace-qualified) | `cpp/engine_cpp_dead_code_roots_test.go:248` (`TestDefaultEngineParsePathCPPMarksNamespaceQualifiedHeaderMethod`) |
| `cpp.virtual_method` | `cpp/engine_cpp_dead_code_roots_test.go:97`; `cpp/dead_code_roots_test.go:129` |
| `cpp.override_method` | `cpp/engine_cpp_dead_code_roots_test.go:98`; `cpp/dead_code_roots_test.go:130` |
| `cpp.callback_argument_target` | `cpp/engine_cpp_dead_code_roots_test.go:89`; `cpp/dead_code_roots_test.go:131` |
| `cpp.function_pointer_target` | `cpp/engine_cpp_dead_code_roots_test.go:91-93`; `cpp/dead_code_roots_test.go:132` |
| `cpp.node_addon_entrypoint` (NODE_MODULE_INIT/NAPI_MODULE_INIT) | `cpp/engine_cpp_dead_code_roots_test.go:94-96`; `cpp/dead_code_roots_test.go:133` |
...
| Out-of-line qualified methods end-to-end | `cpp/cpp_qualified_method_ast_test.go:18` (`TestDefaultEngineParsePathCPPOutOfLineQualifiedMethodsViaAST`) |
...
- Virtual and override methods: `cpp/engine_cpp_dead_code_roots_test.go:97-98`
- Namespace-qualified header methods (`api::Service::run`): `cpp/engine_cpp_dead_code_roots_test.go:248`
