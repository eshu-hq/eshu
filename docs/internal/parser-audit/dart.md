...
9. Create engine-level test files in the language package (e.g., `go/internal/parser/dart/engine_dart_dead_code_roots_test.go`, external `package dart_test`) following `go/internal/parser/cpp/engine_cpp_dead_code_roots_test.go`, to test the parser through the Engine dispatch path.
