...
Validation-Slug: prod-transitive-callers
...
Capability-Assertion: call_graph.transitive_callers passed its non-vacuous capability-specific assertion through the deployed API or MCP surface.
B12-Assertion: call_graph.transitive_callers -> mcp:analyze_code_relationships
...
curl -s -H "Authorization: Bearer $token" \
  ...
  -X POST "${ESHU_API_BASE_URL}/api/v0/code/relationships" \
