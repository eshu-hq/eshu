...
| GCP cloud | not in base remote E2E | `gcp-cloud-collector` | Helm template coverage for explicit claimed-live mode, read-only redaction Secret mount, and ServiceMonitor coverage; the scoped security smoke is complete, while partition-filtered handler and deployed-shape promotion proofs remain gated by the canonical collector contract. |
