...
| PagerDuty | Claim-driven. Selects one enabled `pagerduty` instance, resolves target `token_env` from the runtime environment, requires HTTPS for any configured `api_base_url`, fetches bounded incident, log-entry, and related change-event evidence, and commits only `incident.record`, `incident.lifecycle_event`, and `change.record` facts. Signed PagerDuty webhooks can wake the same configured `scope_id`, but they do not emit facts. PagerDuty state remains source evidence; reducers own runtime, image, build/commit, PR, and work-item correlation. |
