...
(`bash scripts/verify-ifa-fault-injection.sh`) drives twenty-four cells — a
...
scoped baseline and two recovery cells each for deployable_unit_edges and
codeowners_ownership_edges and repository-dependency edges — with zero durable dead letters throughout.

In CI the twenty-three cells other than the shared fault-free baseline are split
across four shards that run in
...
baseline captured on the same runner, so CI executes twenty-seven cells for a
twenty-four-cell matrix. Locally the command above runs all of them in one
