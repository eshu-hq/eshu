...
(`bash scripts/verify-ifa-fault-injection.sh`) drives twenty-seven cells — a
...
codeowners_ownership_edges, repository-dependency edges, and
submodule_pin_edges — with zero durable dead letters throughout.

In CI the twenty-six cells other than the shared fault-free baseline are
split across four shards that run in
...
baseline captured on the same runner, so CI executes thirty cell runs
for a twenty-seven-cell matrix. Locally the command above runs all of them in one
