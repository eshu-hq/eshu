...
| `ifa-fault-injection` | Lease reclaim, retry, and idempotent replay converge across twenty-one cells — fault-injecting cells, the fault-free baselines they compare against, and a delta-retract cell asserting an exact edge set — with the twenty non-baseline cells sharded four ways in CI and the fault-free baseline repeated in every shard, so CI executes twenty-four cell runs for a twenty-one-cell matrix. |
