...
| GitRepository url/ref | `flux-git-repository-url` / `flux-git-repository-ref` | supported | `flux_git_repositories` | `name, line_number, url, ref_branch, ref_tag, ref_semver, ref_commit` | `property:FluxGitRepository.url/ref_branch/ref_tag/ref_semver/ref_commit` | `go/internal/parser/yaml/flux_source_test.go::TestParseFluxGitRepositoryCapturesURLAndRef` | Compose-backed fixture verification | Flux resolves exactly one of branch/tag/semver/commit per revision; each is captured independently and only when present. |
