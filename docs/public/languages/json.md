...
- Unit test suite: `go/internal/parser/json/json_language_test.go`
...
| composer.json require sections | `composer-json-dependencies` | supported | `variables` | `name, line_number, value, section` | `node:Variable` | `go/internal/parser/json/json_language_test.go::TestDefaultEngineParsePathJSONPreservesDocumentOrderForMetadataAndConfigBuckets` | Compose-backed fixture verification | - |
| tsconfig targeted metadata | `tsconfig-targeted-metadata` | supported | `variables` | `name, line_number, value, config_kind` | `node:Variable` | `go/internal/parser/json/json_language_test.go::TestDefaultEngineParsePathJSONPreservesDocumentOrderForMetadataAndConfigBuckets` | Compose-backed fixture verification | - |
| Generic JSON metadata only | `generic-json-metadata-only` | partial | `json_metadata` | `top_level_keys` | `-` | `go/internal/parser/json/json_language_test.go::TestDefaultEngineParsePathJSONPreservesDocumentOrderForMetadataAndConfigBuckets` | Compose-backed fixture verification | Arbitrary JSON files stay intentionally quiet to avoid graph noise; `json_metadata.top_level_keys` is parsed but not exposed as a queryable graph node, matching the JSON Config row's Query Surfacing `-` in [Support Maturity](support-maturity.md). |
| JSON CloudFormation templates | `cloudformation-json-delegation` | supported | `cloudformation_resources` | `name, line_number, file_format` | `node:CloudFormationResource` | `go/internal/parser/cloudformation/parser_test.go::TestParsePersistsFileFormat` | Compose-backed fixture verification | JSON CloudFormation now shares the same parser path as YAML and persists `file_format` on CloudFormation rows. |
| dbt manifest lineage | `dbt-manifest-lineage` | supported | `analytics_models`, `data_assets`, `data_relationships` | `name, line_number` | parsed-only (`COMPILES_TO`, `ASSET_DERIVES_FROM`, `COLUMN_DERIVES_FROM`, `USES_MACRO` in the `data_relationships` bucket; not materialized to the graph or exposed through `content_relationships` — see [Edge Source-Tool Provenance](../reference/edge-source-tool-provenance.md#tier-3-no-edge-level-tool-intentional)) | `go/internal/parser/dbtsql/json_dbt_test.go::TestApplyDBTManifestDocumentIncludesMacroDependenciesAndLineage`, `go/internal/parser/dbtsql/json_dbt_test.go::TestDefaultEngineParsePathJSONDBTManifestWildcardExpansionAndCoalesce` | Compose-backed fixture verification | dbt `manifest.json` compiled-model lineage is parsed into the `analytics_models`/`data_assets`/`data_relationships` buckets, mirroring the SQL-side dbt lineage documented in [SQL](sql.md); the relationships stay in the parser payload and are not currently materialized to the graph. |
