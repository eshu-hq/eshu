...
| Spring route entries | `spring-mvc-route-truth` | supported | `go/internal/parser/java_kotlin_spring_route_semantics_test.go::TestDefaultEngineParsePathKotlinSpringRouteSemantics`, `go/internal/reducer/handles_route_intents_test.go::TestBuildHandlesRouteIntentRowsPreservesJVMFrameworkProvenance`, `go/internal/query/route_query_proof_matrix_test.go::TestRouteQueryProofMatrix/kotlin_spring` | Literal Spring MVC/WebFlux annotations emit exact `framework_semantics.spring.route_entries` with handler function names, and the reducer preserves that framework provenance into `HANDLES_ROUTE` intents (exact-only; ambiguous or unknown handlers are skipped). Proven at parser, reducer, AND query tiers: `TestRouteQueryProofMatrix/kotlin_spring` (#5361) parses a real Spring MVC fixture, drives it through the reducer's real `HANDLES_ROUTE` materialization, and asserts `trace_route_callers` returns the resolved handler end to end. JAX-RS, Micronaut, and Ktor below remain parser+reducer tier only; see #5395 (narrowed to those three frameworks now that Spring has query-tier proof). |
| JAX-RS route entries | `jax-rs-route-truth` | supported | `go/internal/parser/java_kotlin_spring_route_semantics_test.go::TestDefaultEngineParsePathKotlinJVMRouteSemantics`, `go/internal/reducer/handles_route_intents_test.go::TestBuildHandlesRouteIntentRowsPreservesJVMFrameworkProvenance` | Literal `@Path` plus HTTP method annotations emit exact `framework_semantics.jax_rs.route_entries` with handler function names (dynamic paths are not guessed), and the reducer preserves that framework provenance into `HANDLES_ROUTE` intents. Proven at parser and reducer tiers only; a dedicated query-tier proof test for this framework is tracked in #5395 (narrowed to JAX-RS/Micronaut/Ktor now that Spring has query-tier proof, `go/internal/query/route_query_proof_matrix_test.go::TestRouteQueryProofMatrix/kotlin_spring`). |
| Micronaut route entries | `micronaut-route-truth` | supported | `go/internal/parser/java_kotlin_spring_route_semantics_test.go::TestDefaultEngineParsePathKotlinJVMRouteSemantics`, `go/internal/reducer/handles_route_intents_test.go::TestBuildHandlesRouteIntentRowsPreservesJVMFrameworkProvenance` | Literal `@Controller` plus HTTP method annotations emit exact `framework_semantics.micronaut.route_entries` with handler function names (dynamic paths are not guessed), and the reducer preserves that framework provenance into `HANDLES_ROUTE` intents. Proven at parser and reducer tiers only; a dedicated query-tier proof test for this framework is tracked in #5395 (narrowed to JAX-RS/Micronaut/Ktor now that Spring has query-tier proof, `go/internal/query/route_query_proof_matrix_test.go::TestRouteQueryProofMatrix/kotlin_spring`). |
| Ktor route entries | `ktor-literal-handler-route-truth` | supported | `go/internal/parser/java_kotlin_spring_route_semantics_test.go::TestDefaultEngineParsePathKotlinJVMRouteSemantics`, `go/internal/reducer/handles_route_intents_test.go::TestBuildHandlesRouteIntentRowsPreservesJVMFrameworkProvenance` | Literal Ktor verb routes emit exact `framework_semantics.ktor.route_entries` only when the route lambda delegates to one exact bare handler function call, and the reducer preserves that framework provenance into `HANDLES_ROUTE` intents. Proven at parser and reducer tiers only; a dedicated query-tier proof test for this framework is tracked in #5395 (narrowed to JAX-RS/Micronaut/Ktor now that Spring has query-tier proof, `go/internal/query/route_query_proof_matrix_test.go::TestRouteQueryProofMatrix/kotlin_spring`). |
