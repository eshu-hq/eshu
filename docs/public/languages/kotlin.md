...
| Spring route entries | `spring-mvc-route-truth` | supported | `go/internal/parser/java_kotlin_spring_route_semantics_test.go::TestDefaultEngineParsePathKotlinSpringRouteSemantics`, `go/internal/reducer/handles_route_intents_test.go::TestBuildHandlesRouteIntentRowsPreservesJVMFrameworkProvenance` | Literal Spring MVC/WebFlux annotations emit exact `framework_semantics.spring.route_entries` with handler function names, and the reducer preserves that framework provenance into `HANDLES_ROUTE` intents (exact-only; ambiguous or unknown handlers are skipped). Proven at parser and reducer tiers; the resulting `HANDLES_ROUTE` edge is resolvable through the language-agnostic `trace_route_callers` query surface (route materialization is not language-specific), and a dedicated Kotlin query-tier proof test is tracked in #5395. |
| JAX-RS route entries | `jax-rs-route-truth` | supported | `go/internal/parser/java_kotlin_spring_route_semantics_test.go::TestDefaultEngineParsePathKotlinJVMRouteSemantics`, `go/internal/reducer/handles_route_intents_test.go::TestBuildHandlesRouteIntentRowsPreservesJVMFrameworkProvenance` | Literal `@Path` plus HTTP method annotations emit exact `framework_semantics.jax_rs.route_entries` with handler function names (dynamic paths are not guessed), and the reducer preserves that framework provenance into `HANDLES_ROUTE` intents. Proven at parser and reducer tiers; the resulting `HANDLES_ROUTE` edge is resolvable through the language-agnostic `trace_route_callers` query surface (route materialization is not language-specific), and a dedicated Kotlin query-tier proof test is tracked in #5395. |
| Micronaut route entries | `micronaut-route-truth` | supported | `go/internal/parser/java_kotlin_spring_route_semantics_test.go::TestDefaultEngineParsePathKotlinJVMRouteSemantics`, `go/internal/reducer/handles_route_intents_test.go::TestBuildHandlesRouteIntentRowsPreservesJVMFrameworkProvenance` | Literal `@Controller` plus HTTP method annotations emit exact `framework_semantics.micronaut.route_entries` with handler function names (dynamic paths are not guessed), and the reducer preserves that framework provenance into `HANDLES_ROUTE` intents. Proven at parser and reducer tiers; the resulting `HANDLES_ROUTE` edge is resolvable through the language-agnostic `trace_route_callers` query surface (route materialization is not language-specific), and a dedicated Kotlin query-tier proof test is tracked in #5395. |
| Ktor route entries | `ktor-literal-handler-route-truth` | supported | `go/internal/parser/java_kotlin_spring_route_semantics_test.go::TestDefaultEngineParsePathKotlinJVMRouteSemantics`, `go/internal/reducer/handles_route_intents_test.go::TestBuildHandlesRouteIntentRowsPreservesJVMFrameworkProvenance` | Literal Ktor verb routes emit exact `framework_semantics.ktor.route_entries` only when the route lambda delegates to one exact bare handler function call, and the reducer preserves that framework provenance into `HANDLES_ROUTE` intents. Proven at parser and reducer tiers; the resulting `HANDLES_ROUTE` edge is resolvable through the language-agnostic `trace_route_callers` query surface (route materialization is not language-specific), and a dedicated Kotlin query-tier proof test is tracked in #5395. |
