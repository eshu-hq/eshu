...
- Unit test suite: `go/internal/parser/yaml/engine_yaml_semantics_kustomize_test.go`
...
|-----------|----|--------|------------------------|-----------------|---------------|---------------|----------------------|-----------|
| Kustomization overlays (`kustomization.yaml`) | `kustomization-overlays-kustomization-yaml` | supported | `kustomize_overlays` | `name, line_number` | `node:KustomizeOverlay` | `go/internal/parser/yaml/engine_yaml_semantics_kustomize_test.go::TestDefaultEngineParsePathYAMLKustomizeAndHelm` | Compose-backed fixture verification | - |
| Namespace | `namespace` | supported | `variables` | `name, line_number, namespace` | `property:Overlay.property` | `go/internal/parser/yaml/engine_yaml_semantics_kustomize_test.go::TestDefaultEngineParsePathYAMLKustomizeAndHelm` | Compose-backed fixture verification | - |
| Resources list | `resources-list` | supported | `kustomize_overlays` | `name, line_number, resources` | `property:Overlay.resources` | `go/internal/parser/yaml/engine_yaml_semantics_kustomize_test.go::TestDefaultEngineParsePathYAMLKustomizeAndHelm` | Compose-backed fixture verification | - |
| Patches list | `patches-list` | supported | `kustomize_overlays` | `name, line_number, patches` | `property:Overlay.patches` | `go/internal/parser/yaml/engine_yaml_semantics_kustomize_test.go::TestDefaultEngineParsePathYAMLKustomizeAndHelm` | Compose-backed fixture verification | - |
| Patch targets (`patches[].target.kind/name`) | `patch-targets` | supported | `kustomize_overlays` | `name, line_number, patch_targets` | `property:KustomizeOverlay.patch_targets` | `go/internal/parser/yaml/engine_yaml_semantics_kustomize_test.go::TestDefaultEngineParsePathYAMLKustomizePatchTargets` | Compose-backed fixture verification | Inline Kustomize patch targets are normalized into stable `Kind/name` strings and now surface through Go query summaries. |
...
| Base references | `base-references` | supported | `kustomize_overlays` | `name, line_number, bases` | `property:KustomizeOverlay.bases` | `go/internal/parser/yaml/engine_yaml_semantics_kustomize_test.go::TestDefaultEngineParsePathYAMLKustomizeAndHelm` | Compose-backed fixture verification | `bases` is normalized into a stable, sorted list of base paths on the Kustomize payload, so the relation stays first-class instead of being flattened into a comma-delimited string. Only same-repo paths are bases: a remote target in any form kustomize accepts — including the scheme-less `github.com/org/repo//path?ref=v1` and the `git@host:org/repo` shorthand — is classified as a resource ref instead, since another repository is not a directory in this one. A remote target written under the legacy `bases:` key is classified the same way, and a versioned local directory such as `v1.2/base` stays a base despite the dot (#5609). |
| Typed deploy-source refs | `typed-deploy-source-refs` | supported | `kustomize_overlays` | `resource_refs, helm_refs, image_refs` | `property:KustomizeOverlay.resource_refs`, `property:KustomizeOverlay.helm_refs`, `property:KustomizeOverlay.image_refs` | `go/internal/parser/yaml/engine_yaml_semantics_kustomize_test.go::TestDefaultEngineParsePathYAMLKustomizeTypedDeployReferences` | Compose-backed fixture verification | Go now materializes non-base `resources`/`components`, `helmCharts`, and `images` into stable typed ref lists for downstream query and evidence promotion. The relationships engine reads these lists directly to raise `DEPLOYS_FROM` evidence, replacing a second raw-content parse of the same file. It reads `bases` alongside them, because a sibling directory can name another repository's config and confidence calibration is what discounts the paths that do not (#5609). |
