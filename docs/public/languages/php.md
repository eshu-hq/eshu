...
| Laravel Route:: facade route truth | `laravel-route-facade-truth` | partial | `go/internal/parser/php_route_entries_test.go::TestDefaultEngineParsePathPHPEmitsLaravelRouteEntries`, `go/internal/parser/php_route_entries_test.go::TestDefaultEngineParsePathPHPSkipsNonLaravelScopedGetCall`, `go/internal/parser/php_route_entries_test.go::TestDefaultEngineParsePathPHPEmitsLaravelNestedGroupRouteEntries`, `go/internal/parser/php_route_entries_test.go::TestDefaultEngineParsePathPHPEmitsLaravelGlobalBackslashRouteInNamespace`, `go/internal/query/route_query_proof_matrix_test.go::TestRouteQueryProofMatrix/php_laravel` | Laravel `Route::get()`, `Route::post()`, `Route::put()`, `Route::patch()`, `Route::delete()`, `Route::options()`, `Route::any()`, `Route::match()` scoped call expressions emit `framework_semantics.laravel.route_entries` when the scope resolves to the `Illuminate\Support\Facades\Route` facade — either through a `use` import, the fully-qualified `\Illuminate\Support\Facades\Route::`, the global alias `\Route::`, or bare `Route::` in a namespace-less file with no conflicting import. `match()` resolves HTTP methods from the first-arg array; `any()` emits an ANY entry. Phase-1 `scoped_call_expression` gathering gated on Laravel route verbs + post-walk facade-provenance resolution. `Route::group()` prefix concatenation is implemented for literal `'prefix'` array keys (nested groups supported). `Route::resource()` expansion and non-literal group prefixes are deferred. Route **emission** is exact, but route-to-caller **query truth** is not: Laravel's idiomatic `'Controller@method'` string handler (emitted verbatim as the `@`-joined token `UserController@index`) never survives `reducer.resolveHandlesRouteFunction`'s exact-name index. `go/internal/reducer/code_call_materialization_helpers.go` indexes each method under its bare name (`index`) and a `.`-joined class-qualified candidate (`UserController.index`), but never an `@`-joined one, so `HANDLES_ROUTE` is never projected for a standard Laravel string-callable route and `trace_route_callers` returns nothing for it (proven by `TestRouteQueryProofMatrix/php_laravel`, which also asserts the `@`-joined route WAS emitted before being dropped). By contrast, Symfony's `#[Route]` attribute emits the handler as the class-qualified, **dot-joined** string `ReportController.show`, which exact-matches the `.`-joined candidate the index builds and does resolve (`TestRouteQueryProofMatrix/php_symfony`) — so the passing/failing split is the handler-token separator (`.`-join vs `@`-join), not bare-vs-qualified. That working Symfony path is why PHP keeps `trace_route_callers` in its read_surfaces despite this Laravel gap. The `@`-join root cause is tracked in [#5513](https://github.com/eshu-hq/eshu/issues/5513). |
...
- Laravel `Route::resource()` expansion and non-literal group prefixes are
  deferred. Laravel route-to-handler query truth is a `partial` feature, not
  `supported`: the `@`-joined `Controller@method` handler token is never built
  as a resolution candidate, so `HANDLES_ROUTE` is not projected for Laravel
  string-callable routes. The live root-cause tracker is
  [#5513](https://github.com/eshu-hq/eshu/issues/5513) (earlier
  [#4162](https://github.com/eshu-hq/eshu/issues/4162) and
  [#4038](https://github.com/eshu-hq/eshu/issues/4038) are closed, kept here
  for provenance).
