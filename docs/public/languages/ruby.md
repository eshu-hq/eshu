...
- Literal Rails route entries are emitted when the HTTP method, path, and
  ...
  mountable engine's own route-set (`<Namespace>::Engine.routes.draw`, top-level
  `Engine.routes`, and the `.append`/`.prepend` variants engines and gems use).
  A controller routed only inside an internal engine's own `config/routes.rb` is
  seen for both exact capture and the route-liveness ambiguity scan, rather than
  being invisible and falsely downgraded to `route_unreachable`; an arbitrary
  `foo.routes.draw` on a non-Rails receiver is NOT promoted to Rails route truth
  (no false `route_entries`/`HANDLES_ROUTE`). The parser normalizes the
