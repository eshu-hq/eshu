...
- Literal Rails route entries are emitted when the HTTP method, path, and
  ...
  registration is recognized structurally by a `.routes`-suffixed receiver, so
  it covers both the main application (`Rails.application.routes.draw`) and a
  mountable engine's own route-set (`<Namespace>::Engine.routes.draw`, plus the
  `.append`/`.prepend` variants engines and gems use): a controller routed only
  inside an internal engine's own `config/routes.rb` is seen for both exact
  capture and the route-liveness ambiguity scan, rather than being invisible and
  falsely downgraded to `route_unreachable`. The parser normalizes the
