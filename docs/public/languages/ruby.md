...
- Unit test suite: `go/internal/parser/ruby/engine_ruby_semantics_test.go`
...
| Require/load imports | `require-load-imports` | supported | `imports` | `name, line_number` | `relationship:IMPORTS` | `go/internal/parser/ruby/engine_ruby_semantics_test.go::TestDefaultEngineParsePathRubyEmitsRequireAndLoadImports` | Compose-backed fixture verification | - |
| Method calls | `method-calls` | supported | `function_calls` | `name, line_number` | `relationship:CALLS` | `go/internal/parser/ruby/engine_ruby_semantics_test.go::TestDefaultEngineParsePathRubyCapturesGenericDslAndMethodCalls` | Compose-backed fixture verification | - |
| Instance variable assignments | `instance-variable-assignments` | supported | `variables` | `name, line_number` | `node:Variable` | `go/internal/parser/ruby/engine_ruby_semantics_test.go::TestDefaultEngineParsePathRubyCapturesLocalAndInstanceAssignments` | Compose-backed fixture verification | - |
| Local variable assignments | `local-variable-assignments` | supported | `variables` | `name, line_number` | `node:Variable` | `go/internal/parser/ruby/engine_ruby_semantics_test.go::TestDefaultEngineParsePathRubyCapturesLocalAndInstanceAssignments` | Compose-backed fixture verification | - |
...
- Literal Rails route entries are emitted when the HTTP method, path, and
  `to: "controller#action"` target are all source literals. Route-set
  registration is recognized on the source-proven Rails shapes: the main
  application (`Rails.application.routes.draw`, plus `.append`/`.prepend`) and a
  mountable engine's own route-set on the engine class constant
  (`<ConstantPath>.routes.draw`, plus `.append`/`.prepend`). A Rails engine is
  any `Rails::Engine` subclass, so the class may be conventionally named
  `MyEngine::Engine` or custom-named (`PaymentsEngine`, `Api`); the receiver of
  `.routes` must be a constant (class) reference. A controller routed only inside
  an internal engine's own `config/routes.rb` is seen for both exact capture and
  the route-liveness ambiguity scan, rather than being invisible and falsely
  downgraded to `route_unreachable`; an arbitrary lowercase `foo.routes.draw` (a
  local/method receiver, not a class constant) is NOT promoted to Rails route
  truth (no false `route_entries`/`HANDLES_ROUTE`). The parser normalizes the
  controller string to the matching `ControllerClass.action` handler name, and
  the reducer only projects `HANDLES_ROUTE` when that handler resolves to one
  indexed function.
