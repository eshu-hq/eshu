...
| `terraform {}` block metadata | `terraform-block-metadata` | supported | `terraform_blocks` | `name, line_number, required_providers` | `node:TerraformBlock` | `go/internal/parser/hcl/hcl_terraform_test.go::TestDefaultEngineParsePathHCLTerraformBlockMetadata` | Compose-backed fixture verification | The native HCL parser now materializes a standalone `terraform {}` block row that carries required-provider metadata and provider-source summaries. |
