...
- `ESHU_AUTH_RESOURCE_URI` is unset or fails the `https://` (or loopback
  ...
  document, so the probe `404`s permanently rather than intermittently — see
  [a non-loopback http deployment cannot serve
  discovery](mcp-oauth-discovery.md#a-non-loopback-http-deployment-cannot-serve-discovery).
