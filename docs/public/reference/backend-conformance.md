...
The per-backend snapshot cannot catch a statement that returns wrong rows on
...
row-total divergences with agreeing result sets are scheduling noise (the
backends drain at systematically different speeds); the
`-diff-executions-advisory-max` ceiling (200 in CI, ~2.9x the observed max)
