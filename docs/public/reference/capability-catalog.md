...
| Surface | Where | Notes |
| --- | --- | --- |
| HTTP API | `GET /api/v0/capabilities` | Bounded read with optional `maturity` and `owner` filters, `limit`/`offset` paging (default `limit=12`), and `view=compact\|full` / `include_authorization` detail toggles; exact truth, fresh freshness. |
