...
| Components | `eshu component` | Local component package inspect, verify, install, list, enable, disable, and uninstall. Each subcommand supports stable `--json` output; install and enable also support `--dry-run`. See [Component Package Manager](component-package-manager.md). |
