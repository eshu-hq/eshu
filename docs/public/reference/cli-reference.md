...
| Competitive parity gate | `eshu competitive-parity validate` | Validates that the shipped first-run report, operator digest, investigation evidence packet, and capability catalog surfaces remain reachable from CLI, API, MCP, console, and public docs. Emits `competitive_parity_gate.v1` as Markdown or JSON and links residual gaps to existing issues such as #3238. See [Competitive Parity Gate](local-testing/competitive-parity-gate.md). |
