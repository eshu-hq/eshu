...
| Operator digest | `eshu report` | Renders a deterministic `operator_digest.v1` model for an explicit share-safe scope such as `repo:owner/name` or `service:name`. This initial CLI path is offline: it validates the model, emits unsupported sections and suggested follow-up routes, and does not call providers, write graph state, claim reducer work, or infer graph truth. Use `--json` for the contract model. See [Operator Digest Contract](operator-digest.md). |
