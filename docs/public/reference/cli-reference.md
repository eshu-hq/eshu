...
| Components | `eshu component` | Local component scaffolding plus package inspect, verify, install, list, enable, disable, and uninstall. `component init collector` creates a minimal collector extension scaffold. Each subcommand supports stable `--json` output; install and enable also support `--dry-run`. See [Component Package Manager](component-package-manager.md). |
