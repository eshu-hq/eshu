...
| Pre-change impact | `eshu change impact` | Derives a local `git diff --name-status --find-renames --find-copies --find-copies-harder` from `--base`/`--head` or accepts repeated `--file` paths, then calls `/api/v0/impact/pre-change` for a bounded AnswerPacket-shaped impact summary. It requires `--repo-id` when changed files are provided, preserves deleted, renamed, and copied file status, and exits non-zero for stale, partial, or truncated evidence. |
