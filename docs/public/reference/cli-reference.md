...
| Evidence packet dogfood | `eshu evidence-packet-dogfood` | Scores the investigation evidence packet dogfood benchmark (`--from <benchmark.json>`): whether v2 evidence packets beat raw repository search and existing Eshu tool drilldowns on family coverage, answer correctness, answer time, token budget, and missing-evidence clarity across supply-chain impact, deployable drift, and service context. Exits non-zero when the packet approach loses any dimension. See [Evidence Packet Dogfood Benchmark](local-testing/evidence-packet-dogfood.md). |
| Competitive parity gate | `eshu competitive-parity validate` | Validates that the shipped first-run report, operator digest, investigation evidence packet, and capability catalog surfaces remain reachable from CLI, API, MCP, console, and public docs, then scores usefulness dimensions such as actionability, evidence clarity, reproducibility, reader usefulness, and peer-baseline coverage. Emits `competitive_parity_gate.v1` as Markdown or JSON and links any residual gaps to existing issues instead of opening duplicates. See [Competitive Parity Gate](local-testing/competitive-parity-gate.md). |
| Portable evidence bundle | `eshu evidence bundle export`, `eshu evidence bundle validate` | Exports and validates deterministic `evidence_bundle.v1` snapshots that package answer packet summaries, investigation packet summaries, capability catalog handles, freshness/readiness state, missing evidence, and reproduce calls. `--live` composes the snapshot from a running stack's read-only status routes instead of the built-in fixture; it is stack-wide, so it cannot be combined with `--scope`. Before writing, the bundle is screened for private endpoints, credentials, raw prompts, provider responses, and local paths — screening is best-effort, so review an artifact before sharing it. See [Portable Evidence Bundle](evidence-bundle.md). |
...
| Pre-change impact and developer plan | `eshu change impact`, `eshu change plan` | `change impact` derives a local `git diff --name-status --find-renames --find-copies --find-copies-harder` from `--base`/`--head` or accepts repeated `--file` paths, then calls `/api/v0/impact/pre-change` for a bounded AnswerPacket-shaped impact summary. `change plan` uses the same changed-file input plus optional `--intent`, calls `/api/v0/impact/developer-change-plan`, and renders the read-only `developer_change_plan.v1` actions, patch guidance, recommended tests, and bounded next calls. Both commands require `--repo-id` when changed files are provided, preserve deleted, renamed, and copied file status, and exit non-zero for stale, partial, or truncated evidence. |
