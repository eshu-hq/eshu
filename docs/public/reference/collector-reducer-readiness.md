...
| CI/CD runs | Fixture normalizer, reducer correlation, bounded GitHub Actions runtime source, and workflow planner exist. The hosted runtime strips token-bearing artifact URLs before fact emission and requires explicit repository allowlists plus run/job/artifact limits. It is not a deployed lane until a binary, coordinator service wiring, chart values, provider telemetry, status, and live target proof exist. |
