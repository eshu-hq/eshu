...
| PagerDuty incident context | `eshu-collector-pagerduty` is claim-driven for PagerDuty incidents, incident log entries, and related change events. It requires explicit credentials through `token_env`, bounded incident/log/change limits, an incident lookback window, and optional service allowlists before issuing provider requests. Public Helm chart support is pending. | `incident.record`, `incident.lifecycle_event`, and `change.record` facts preserve PagerDuty state as source truth. Incident-context reducers and reads are not part of this slice; runtime artifact, image, commit, pull-request, Jira, and deployment correlation must be proven separately before user-facing incident paths are reported. | Prove the configured PagerDuty target, credential environment, rate-limit behavior, redaction, claim handoff, fact counts, reducer/read-model follow-up, API/MCP reads, and private-data handling in the target environment. |
