...
| C# | `derived` | `Main`, constructors, overrides, same-file interface methods and implementations, ASP.NET controller actions, hosted-service callbacks, test methods, serialization callbacks. | Reflection, dependency injection, source generators, partial types, dynamic dispatch, project references, broad public API surfaces. |
