...
- For `supply_chain_impact` findings: an observed cloud compute resource (a
  ...
  probe (`go/internal/query/supply/chain/cloud_runtime_probe.go`), not a
...
- **Supply-chain impact** (`findings[].deployment_truth_tier`,
  `go/internal/query/supply/chain/impact/result.go`): now differentiates all
...
`version_resolution_corroboration[]` (`go/internal/query/supply/chain/impact/version_resolution.go`).
