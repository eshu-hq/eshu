...
| `ESHU_AWS_REDACTION_KEY` | unset | collector-aws-cloud | Deployment-scoped key for ECS task-definition and Lambda environment redaction markers. Required when a target scope enables `ecs` or `lambda`. |
