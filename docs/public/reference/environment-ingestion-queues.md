...
| `ESHU_BOOTSTRAP_COMMIT_LANES` | `4` | bootstrap-index | Concurrent commit lanes for the bootstrap collector drain (#5130). The default is the measured commit-throughput plateau from the #5122 lane shim (2.22x at 4 lanes on the accepted 896-repo fact distribution), not CPU count; invalid or non-positive values fall back to it; values above 64 clamp to 64 (every lane holds an open transaction, so a runaway value would exhaust the connection pool). |
