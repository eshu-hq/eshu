...
| Variable | Default | Read by | Purpose |
| --- | --- | --- | --- |
| `ESHU_CANONICAL_WRITE_TIMEOUT` | `30s` on NornicDB; unset (unbounded) on Neo4j | bootstrap-index, ingester, projector, reducer graph writers | On NornicDB, the client context and Bolt transaction timeout for graph writes. On Neo4j, the Bolt transaction timeout, applied only when set to a positive duration; the server terminates and rolls back a write that exceeds it. A timed-out write is not retried in place: it is deferred to the durable queue as `graph_write_timeout` for replay-safe `RUNS_ON` groups and fails terminally otherwise, on both backends. |
