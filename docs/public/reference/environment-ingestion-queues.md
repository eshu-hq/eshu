...
| `ESHU_DEFERRED_BACKFILL_CONCURRENCY` | `min(NumCPU, 8)`, hard cap `8` | bootstrap-index / ingester deferred maintenance | Concurrent per-repository batch transactions in the deferred relationship-evidence backfill. Each batch holds one pooled connection for its transaction and never nests a second, so a value above the connection pool throttles on `Begin` rather than deadlocking. The ingester may set this to `1` with a single-connection pool; bootstrap-index requires at least two pool connections because its bulk-load lock pins one for the run. The default uses the existing cap on roomy hosts to shorten the backfill long pole. |
...
| Variable | Default | Read by | Purpose |
| --- | --- | --- | --- |
| `ESHU_CANONICAL_WRITE_TIMEOUT` | `30s` on NornicDB; unset (unbounded) on Neo4j | bootstrap-index, ingester, projector, reducer graph writers | On NornicDB, the client context and Bolt transaction timeout for graph writes. On Neo4j, applied only when set to a positive duration: the Bolt transaction timeout (the server terminates and rolls back the write) plus, on the same paths as NornicDB, the client deadline. With the variable unset, each Neo4j graph writer logs one `graph.write_timeout.unbounded` WARN at startup. On both backends, ingester, bootstrap-index, projector, and reducer semantic-entity writes that exceed the client deadline requeue as retryable `graph_write_timeout`. Other reducer writes rely on the server timeout: it is not retried in place, it defers as `graph_write_timeout` for replay-safe `RUNS_ON` groups, and it fails terminally otherwise. On Neo4j, a write terminated while it waits on a lock reports `LockClientStopped` whatever the cause (timeout, operator kill, or shutdown); it is not retried in place and requeues as retryable `graph_write_timeout` in every group, whether or not this variable is set. |
...
| `NEO4J_IMAGE` | `neo4j:2026-community@sha256:eabfbb042bdaca2fd5e1950db1329b22c794eee80f0eacc4e7a729d44b2e863f` in `docker-compose.neo4j.yml` | Docker Compose | Neo4j compatibility image override. The default is the digest the B-7 Neo4j cell was proven on (#6782). |
| `NORNICDB_IMAGE` | `ghcr.io/eshu-hq/nornicdb-amd64-cpu:fix-500-e022384c@sha256:74a8ed7b36f37bdd1a7e32d8bc6aa3fa88908b7207bfa6568567ab94e4a4b3b1` in default Compose | Docker Compose | NornicDB image override. Set with `NORNICDB_PULL_POLICY` for controlled comparisons. |
| `NORNICDB_PLATFORM` | `linux/amd64` | Docker Compose | Platform override for the bundled backend. Only the amd64 default has live v1.3.3 proof; use a separate project and fresh graph volume for unsupported architecture experiments. |
