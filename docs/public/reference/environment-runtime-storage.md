...
| `ESHU_SCHEMA_BOOTSTRAP_OWNERSHIP_WAIT` | `10m` | `eshu-bootstrap-data-plane` | How long a schema bootstrap waits for another bootstrapper that owns the Postgres schema advisory lock before failing (#6956). While waiting it logs `bootstrap.postgres.ownership.waiting` with the holder every 15 s. |
| `ESHU_SCHEMA_LOCK_RETRY_BUDGET` | `10m` | `eshu-bootstrap-data-plane` | Total backoff one Postgres migration statement may spend retrying after `lock_timeout` (SQLSTATE 55P03, for example behind an anti-wraparound autovacuum) before the bootstrap fails (#6956). Each retry logs `bootstrap.postgres.migration.lock_wait`. |
