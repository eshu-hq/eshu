...
| `ESHU_SCHEMA_BOOTSTRAP_OWNERSHIP_WAIT` | `3m` | `eshu-bootstrap-data-plane`, bootstrap-index | How long a schema bootstrap waits for another bootstrapper that owns the Postgres schema advisory lock before failing (#6956). Unset keeps the default; a set value must be a positive duration. While waiting it logs `bootstrap.postgres.ownership.waiting` with the holder on the first failed try and then every 15 s. Keep it plus `ESHU_SCHEMA_LOCK_RETRY_BUDGET` under the bootstrap Job deadline. A `CREATE`/`DROP INDEX CONCURRENTLY` migration statement is outside this wait's scope (see `ESHU_SCHEMA_LOCK_RETRY_BUDGET` below): if a prior run's orphaned build is still holding the advisory lock when this one starts, this wait can time out even though the deadline never stopped that build (#7004). |
