...
| `ESHU_MCP_ALLOW_UNAUTHENTICATED` | `false` | MCP server | Dev/loopback escape hatch (issue #5168). With `ESHU_MCP_TRANSPORT=http` and no resolvable credential source (`ESHU_API_KEY`, `ESHU_SCOPED_TOKENS_FILE`, `ESHU_AUTH_RESOURCE_URI`), the server refuses to start unless this is `true`. When `true` with no credential source, every `initialize`/`tools/list`/`tools/call`/`ping` request and SSE session is unauthenticated; never set on a publicly reachable port. |
...
| `ESHU_SEMANTIC_EXTRACTION_POLICY_JSON` | unset | API, MCP, reducer | Optional hosted semantic extraction and semantic-search source policy. JSON must explicitly allow semantic provider egress plus provider profile ids, source classes, organization/tenant/project/repository scopes, source selectors, max chunk/token/cost limits, redaction mode, and retention posture. Without this policy or without semantic-provider egress allow, configured provider profiles remain visible but source policy stays disabled. |
| `ESHU_SEMANTIC_SEARCH_LOCAL_EMBEDDER` | unset | API, MCP, reducer | `hash` or `local_hash` explicitly forces deterministic no-network local semantic/hybrid retrieval for `POST /api/v0/search/semantic` and MCP `search_semantic_context`; on the reducer, the same setting enables the side runner that builds ready local vector rows for active curated search documents. `auto_hash` selects exactly one governed `search_documents` provider profile when available and otherwise falls back to local hash mode. When unset, API/MCP/reducer auto-select exactly one governed provider profile if available; otherwise semantic remains unavailable and hybrid remains BM25-degraded. API/MCP routes report explicit degraded state when rows are missing, stale, partial, rebuilding, failed, or incompatible. |
| `ESHU_SEMANTIC_SEARCH_PROVIDER_PROFILE_ID` | unset | API, MCP, reducer | Selects one governed `search_documents` provider profile when multiple eligible profiles are configured. Startup fails closed if multiple eligible profiles exist and this selector is unset or points to a non-search profile. |
...
| `ESHU_POSTGRES_MAX_OPEN_CONNS` | `30` | Runtimes using `internal/runtime.OpenPostgres` | Maximum open Postgres connections per process. Bootstrap-index requires at least `2` because its run-scoped bulk-load lock pins one connection. API/MCP currently open Postgres directly and do not apply this cap. |
...
| `ESHU_NORNICDB_PROFILE_FILE_GROUPS` | `false` | ingester | Logs sanitized per-statement timing for NornicDB canonical File groups. Diagnostic only; leave off for normal operation. Cannot be enabled with `ESHU_NEO4J_PROFILE_GROUP_STATEMENTS`. |
...
| `ESHU_SCHEMA_BOOTSTRAP_OWNERSHIP_WAIT` | `3m` | `eshu-bootstrap-data-plane`, bootstrap-index | How long a schema bootstrap waits for another bootstrapper that owns the Postgres schema advisory lock before failing (#6956). Unset keeps the default; a set value must be a positive duration. While waiting it logs `bootstrap.postgres.ownership.waiting` with the holder on the first failed try and then every 15 s. It also bounds bootstrap-index's wait for the secret-lines bulk-load lock (advisory key `(5318,1)`, #7125), which the same event reports with `lock=bulk_load`. That wait and the schema wait run one after the other, each with its own full bound, so a bootstrap-index run can wait up to twice this value before it starts work. Keep this value plus `ESHU_SCHEMA_LOCK_RETRY_BUDGET` (and a floor for pod start and migration work) under the schema bootstrap Job's `activeDeadlineSeconds`; `eshu-bootstrap-data-plane` waits once. A bootstrap-index run waits up to twice this value, and the chart sets no deadline for it, so size any Job you wrap it in for twice this value plus the retry budget. A `CREATE`/`DROP INDEX CONCURRENTLY` migration statement is outside this wait's scope (see `ESHU_SCHEMA_LOCK_RETRY_BUDGET` below): if a prior run's orphaned build is still holding the advisory lock when this one starts, this wait can time out even though the deadline never stopped that build (#7004). |
| `ESHU_SCHEMA_LOCK_RETRY_BUDGET` | `3m` | `eshu-bootstrap-data-plane`, bootstrap-index | Wall-clock deadline, shared by every migration statement of one bootstrap run, for retrying after `lock_timeout` (SQLSTATE 55P03, for example behind an anti-wraparound autovacuum) before the bootstrap fails (#6956). Unset keeps the default; a set value must be a positive duration. Each retry logs `bootstrap.postgres.migration.lock_wait`. A bare `CREATE`/`DROP INDEX CONCURRENTLY` statement is exempt from `lock_timeout` entirely and so never retries or draws on this budget (#7004); its wait is bounded only by however long Postgres takes to run it, not by this variable or by the bootstrap Job's `activeDeadlineSeconds` (which kills only the client, not the in-flight server-side statement) -- see [Bootstrap Runtime Services](../deployment/service-runtimes-bootstrap.md). |
