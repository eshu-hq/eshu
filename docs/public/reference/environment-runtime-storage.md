...
| `ESHU_SEMANTIC_SEARCH_LOCAL_EMBEDDER` | unset | API, MCP | Explicitly enables deterministic no-network local semantic/hybrid retrieval for `POST /api/v0/search/semantic` and MCP `search_semantic_context` when set to `hash` or `local_hash`. Unset keeps semantic unavailable and hybrid BM25-degraded. This reads active curated search documents only; it does not configure hosted providers, credentials, egress, graph writes, or ANN readiness. |
