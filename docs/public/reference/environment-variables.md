...
| `ESHU_REPO_DEPENDENCY_PROJECTION_WORKERS` | Set the dedicated repo-dependency projection lane to `1`, `2`, or `4` workers. The backend-aware default is the remotely proven `4` on NornicDB and the unscaled `1` on Neo4j; unsupported values fall back to that backend default. Stable source-repository acceptance-unit sharding keeps the same repository serialized and allows unrelated repositories to run concurrently. |
| `ESHU_REPO_DEPENDENCY_PROJECTION_LEASE_TTL` | Set the repo-dependency shard lease; default `5m`. It must exceed `ESHU_REPO_DEPENDENCY_PROJECTION_CYCLE_TIMEOUT + ESHU_CANONICAL_WRITE_TIMEOUT + 30s`, or reducer startup fails. An error, cycle deadline, or ambiguous commit leaves the shard quarantined until this lease expires; a process shutdown that interrupts a cycle before its acceptance-unit transaction opened releases the lease at once, and one that lands after that point keeps the quarantine. |
