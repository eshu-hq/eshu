...
| `ESHU_SEMANTIC_SEARCH_LOCAL_EMBEDDER` | Enable deterministic no-network local semantic/hybrid retrieval for API/MCP search with `hash` or `local_hash`; unset keeps no-provider keyword behavior. |
