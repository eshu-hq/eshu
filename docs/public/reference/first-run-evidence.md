...
That has a cost, and it lands on the commands, in two ways.
...
A recovery step that itself contains a credential-shaped pair loses it, even
though the step is a fixed instruction with no secret in it:
`export ESHU_API_KEY=<token>` is recorded as `export [redacted]`. Nothing checks
whether a value looks like a secret, so a placeholder is removed exactly like a
real key. Your terminal still prints the full recovery step — only the artifact
meant for sharing is shortened, and the diagnosis class and docs link beside it
