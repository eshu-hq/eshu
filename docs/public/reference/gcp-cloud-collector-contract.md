...
bounded per-rule summary of priority and action plus a rule count, the Cloud
Armor Adaptive Protection layer-7 DDoS defense enabled posture, and creation
time. The policy's graph edge is inbound: a Backend Service references it
through its own `securityPolicy`/`edgeSecurityPolicy` fields and resolves the
...
decoded — only the rule's priority and its action string (a small
Google-controlled vocabulary such as `allow`, `deny(403)`, `rate_based_ban`,
never user-supplied match data) leave the parser.
