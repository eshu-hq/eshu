...
`interconnect_attachment_uses_router` edge to the resolved Cloud `Router` and
`interconnect_attachment_uses_interconnect` edge to the resolved
`Interconnect`; and surfaces both resolved resource names as correlation
anchors. This extractor reuses `assetTypeComputeInterconnectAttachment` and
`assetTypeComputeRouter` (both declared by the Cloud Router extractor,
#4301), never redeclaring either, and declares `assetTypeComputeInterconnect`
for the Cloud Router extractor's own
`router_interface_linked_interconnect_attachment` edge to reuse. `partnerAsn`
is a string-encoded int64 per the compute API convention; it is decoded via
`json.RawMessage`/`parseFlexibleInt64` so an absent value (the common case for
a DEDICATED attachment, where the field is not available) is distinguished
from a legitimately present zero, rather than fabricated. Every
candidate/customer/cloud-router IP address field the Compute API exposes on
this resource (`candidateCloudRouterIpAddress`,
