...
| `POST /api/v0/code/divergence/findings` | Parallel-implementation findings (exact/renamed fingerprint groups, reducer-verified drifted pairs, graph-qualified wrapper-bypass targets, plus cohort-selected convention-outlier members) for one repository, ranked members x tokens with per-rule suppression counts; `kind` selects the family (blank reads all five); wrapper_bypass fingerprints carry the target entity id, convention_outlier fingerprints carry the cohort address plus the majority callee, and graph findings carry weakest-edge `confidence`; `POST /api/v0/code/divergence/investigate` drills into one finding by repo, kind, and fingerprint with bounded follow-up calls. `POST /api/v0/code/divergence/report` rolls all five families into one call: counts by kind, the total, the top `top_per_kind` findings per kind (default 3, max 10), per-rule suppression counts, and per-kind `truncated` flags. Operator signals for the fingerprint stage and the drifted-pair reducer domain live in [Code Divergence Signals](../telemetry/code-divergence.md). |
...
callers or callees; `max_depth` caps traversal. The response always carries `outgoing_truncated` and `incoming_truncated`: `true` means more neighbours exist in that direction than were returned (#7151). The NornicDB one-hop read returns at most 500 per direction and reads one extra row to know it clipped. The Neo4j read and the transitive walk have no row ceiling, and an excluded `direction` was never read, so those report `false`. The content fallback sets a flag when a candidate scan or lookup returned fewer neighbours than exist: the k8s `SELECTS` candidate scan, or a 20-row lookup for `REFERENCES` (JSX component usage), `PATCHES` (Kustomize patch targets) or `CONTAINS` (Rust impl blocks) edges. It reads one extra row to know, so exactly 20 is complete. A `relationship_type` that excludes the clipped edge type reports `false`. The ambiguous-target answer returns no neighbours and reports both flags `false`.
