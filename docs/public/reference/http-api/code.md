...
callers or callees; `max_depth` caps traversal. The response always carries `outgoing_truncated` and `incoming_truncated`: `true` means more neighbours exist in that direction than were returned (#7151). The NornicDB one-hop read returns at most 500 per direction and reads one extra row to know it clipped. The Neo4j read and the transitive walk have no row ceiling, and an excluded `direction` was never read, so those report `false`. The content fallback sets a flag only when its k8s `SELECTS` candidate scan overran its ceiling and `relationship_type` is empty or `SELECTS`; its other lookups do not report clipping yet.
