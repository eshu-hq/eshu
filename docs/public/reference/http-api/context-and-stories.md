...
- `result_limits` is a drilldown block with a bounded `limit`, deterministic
  ...
  `instance_count`, `dependent_count`, `consumer_count`, `hostname_count`, and
  `entrypoint_count` for workload/service context and story), a `truncated` flag,
  the `drilldown_tool` to call next (`get_relationship_evidence` for entity context,
  `get_workload_story` from workload context, `get_workload_context` from workload
  story), the `drilldown_basis`, and the `context_path` for re-reading the route.
  The entity and workload relationship fan-out is capped in place so the
  prompt-ready read stays within the route budget and exposes truncation explicitly.
