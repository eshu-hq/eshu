...
| `GET /api/v0/auth/admin/idp-group-mappings` | All-scopes admin read that lists up to 500 of the caller's tenant/workspace external group→role mappings per page in opaque `mapping_ref` order. On a full page, pass `next_after_ref` as `after_ref` until `truncated` is false or the page is empty. Omit `after_ref` for the first page; the cursor is read without normalization, so a present value that is not a lowercase 64-hex ref (empty, padded, or repeated included) or an undecodable query string returns `400`. Never returns the external group hash. |
...
| `DELETE /api/v0/auth/admin/idp-group-mappings/{mapping_ref}` | All-scopes admin mutation that deletes an external group→role mapping identified by its opaque 64-hex SHA-256 `mapping_ref` over the composite key. Scoped strictly to the caller's tenant/workspace. Idempotent: an absent or already-deleted mapping is a no-op. A saved 32-hex MD5 ref from before the FIPS transition returns `409`; page through the mapping list and retry with the current ref. The mapping row itself is unchanged. Every allowed and denied attempt is governance-audited. |
