# HTTP Repository Routes

Use these routes for repository catalog navigation and repository-scoped
context. The container image inventory, ingester status, and bundle search
routes moved to
...
`GET /api/v0/repositories/{repo_id}/context` and
`GET /api/v0/repositories/{repo_id}/story` both map a bounded graph-read
failure (`ErrGraphReadDeadline`/`ErrGraphUnavailable`) on their summary
counts, narrative rows, and deployment-evidence reads onto the shared
`503 backend_unavailable` / `504 backend_timeout` contract (see
[Graph-read safety](../telemetry/graph-read-safety.md)) rather than answering
a fabricated zero count or empty narrative. The story's workload/platform/
language narrative rows are also bounded at 500 rows
(`repositoryStoryStringRowLimit`); a healthy read that lands past that bound
