...
ownership and deployment families ship through
...
- `service_evidence_snapshots` holds generation-stable evidence rows keyed by a
  ...
  the resolution generation and is therefore not a stable diff key), with a
...
The **ownership** (#1943) and **deployment** (#1985) families ship. The
remaining families (runtime, dependencies, docs, incidents, vulnerabilities)
reuse this lineage and snapshot foundation and are tracked follow-ups. The
