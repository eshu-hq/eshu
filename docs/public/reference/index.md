...
contracts, gates, or source-of-truth details. If you are new to Eshu, start
with [Get Started](../index.md) or [Tutorials](../tutorials/index.md) first;
reference pages are optimized for lookup.

## Main Areas

| Area | Use it for |
| --- | --- |
| [API and MCP](http-api.md) | HTTP routes, MCP tool contracts, capability matrices, and client behavior. |
| [CLI and assistants](cli-reference.md) | CLI commands, assistant setup details, first-run evidence, and MCP client guidance. |
...
- Human onboarding pages link here only when a reader needs an exact contract.
- Operator procedures stay under [Operate](../operate/index.md) and link back to
  reference pages for deeper details.
- Maintainer, proof, and contract-heavy material remains searchable and
  linkable here, but it is grouped by lookup task instead of appearing as a
  beginner path.
