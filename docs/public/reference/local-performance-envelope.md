...
These rules replace scattered historical planning notes. Apply them to every
repo-scale performance claim.

- Apply schema first. For Compose, Kubernetes, and backend comparisons, run the
  data-plane schema bootstrap before indexing. Missing graph indexes or
  constraints can make the shared Cypher path look falsely slow.
- Record three terminal timings separately: collector stream complete,
  projection/bootstrap complete, and queue-zero. Do not use one timing as a
  substitute for another.
- Walk the proof ladder in order: focused fixture, single repo, representative
  medium subset, then full corpus. A full-corpus run is useful only after the
  smaller lanes explain the shape being tested.
- Capture the backend identity with the result: Eshu commit, NornicDB or Neo4j
  image tag or commit, clean-volume state, schema state, effective runtime
  knobs, queue counts, retry counts, and dead letters.
- Treat timeouts as symptoms. Classify whether the evidence points to query
  shape, missing schema or index, backend fallback, transaction validation,
  queue/retry/idempotency behavior, stale images, background backend work such
  as embeddings, or a real timeout-budget contract.
- Do not ship serialization as a performance fix. Lowering worker counts is
  acceptable as a baseline, temporary safeguard, or proven permanent serial
  contract. It is not a fix for a non-idempotent writer, conflict-key race, or
  MERGE/constraint concurrency defect.
- Keep performance and observability evidence in versioned docs. PR text alone
  is not enough because future maintainers need the proof without scraping
  GitHub.
