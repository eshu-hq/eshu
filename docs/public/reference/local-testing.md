...
definition described under [CI workflow shape](#ci-workflow-shape) below
(which also covers `.agents/**` and any root `*.md`, and is evaluated
per-workflow rather than per-path): the two serve different gates with
different blast radii, and this local classifier stays conservative on
purpose rather than inheriting CI's broader definition.
...
[documentation-only fast path](#documentation-only-fast-path) above; the table
is what the changed-path selector *adds* on top of those. You never have to
remember the matching verifier — the selector picks it.
