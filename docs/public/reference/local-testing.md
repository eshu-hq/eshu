...
| C-1 replay coverage manifest + lockstep gate (a new implemented-lane collector, fact-kind read surface, parser, or capability claim; the coverage manifest; or `go/internal/replaycoverage`) | `cd go && go test ./cmd/replay-coverage-gate ./internal/replaycoverage -count=1` and `bash scripts/test-verify-replay-coverage-gate.sh` (unit + static contract); `bash scripts/verify-replay-coverage-gate.sh --blocking` runs the same blocking gate CI enforces over all four registries and fails on any supported surface lacking a replay scenario while writing the C-7 coverage report. Omit `--blocking` only for local exploratory/advisory reports. Static, credential-free, Docker-free — composes with the golden-corpus-gate that actually replays the scenarios it counts |
