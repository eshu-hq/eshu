...
| Go micro-benchmark CI workflow (`.github/workflows/bench.yml`) or its runner `scripts/run-go-benchmarks.sh` | `bash scripts/test-run-go-benchmarks.sh` (static contract + fast hermetic functional check); `bash scripts/run-go-benchmarks.sh` for the full credential-free benchmark sweep that the workflow uploads as an artifact (visibility only — no regression gate; B-2/#3795 adds the `benchstat` baseline comparison) |
