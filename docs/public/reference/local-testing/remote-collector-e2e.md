...
`minimums.ci_cd_run_correlations` at `0` and set
...
That path still calls bounded API and MCP list readbacks, but prints only class
names. When image and SBOM minimums are both positive, the verifier requires the
