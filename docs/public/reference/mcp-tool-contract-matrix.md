...
| `list_supply_chain_impact_findings` | CVE, package id, repository id, subject digest, or impact status required; optional `profile=precise` (default) or `comprehensive` | `limit` default 50, optional cursor | yes | prompt-ready; keeps CVSS, EPSS, KEV, source snapshot freshness, reachability, fixed version, missing evidence, and per-row `detection_profile` separate |
