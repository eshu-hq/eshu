...
| `list_sbom_attestation_attachments` | source repository, workload, service, subject digest/image digest, document id, or document digest required | `limit` default 50, optional cursor | yes | prompt-ready; preserves attached verified, unverified, parse-only, subject mismatch, ambiguous subject, unknown subject, and unparseable states; scoped reads expose missing repository/workload/service-to-image and image-to-SBOM evidence without turning components into vulnerability impact |
