...
| `list_supply_chain_impact_findings` | CVE, package id, repository id, subject digest, or impact status required | `limit` default 50, optional cursor | yes | prompt-ready; keeps CVSS, EPSS, KEV, source snapshot freshness, reachability, fixed version, and missing evidence separate |
