...
| `list_supply_chain_impact_findings` | CVE, package id, repository id, subject digest, impact status, priority bucket, or minimum priority score required; optional `profile=precise` (default) or `comprehensive` | `limit` default 50, optional cursor; `sort` can use finding id or priority score | yes | prompt-ready; keeps CVSS, EPSS, KEV, priority contributions, source snapshot freshness, reachability, fixed version, missing evidence, and per-row `detection_profile` separate |
