...
| Incident response prompts | `get_incident_context` | Incident context returns PagerDuty source evidence, fallback change candidates, explicit missing Jira/PR/build/commit slots, and runtime/image/deployable slots only when explicit service-catalog, image-identity, and Kubernetes reducer evidence exists. | Work-item, PR, build, and commit enrichment remain separate evidence collectors/reducers. |
...
| `list_supply_chain_impact_findings` | CVE, package id, repository id or selector, subject digest, impact status, priority bucket, or `min_priority_score > 0` required; optional `profile=precise` (default) or `comprehensive` | `limit` default 50, optional cursor; `sort` can use finding id or priority score | yes | prompt-ready; keeps CVSS, EPSS, KEV, priority contributions, source snapshot freshness, reachability, fixed version, missing evidence, and per-row `detection_profile` separate |
| `count_supply_chain_impact_findings` | optional CVE, package id, repository id or selector, subject digest, or impact status scope | singleton totals envelope | yes | prompt-ready; cheap-summary count and by-priority/severity rollup over reducer-owned impact facts; replaces page-and-iterate caller workflow for ecosystem totals questions |
| `get_supply_chain_impact_inventory` | optional CVE, package id, repository id or selector, subject digest, or impact status scope; `group_by` selects dimension | `limit` default 100, max 500, deterministic ordering, `next_offset` on truncation | yes | prompt-ready; paginated grouped count of reducer-owned impact findings along one dimension (impact_status, priority_bucket, severity, repository_id) |
...
| `get_incident_context` | provider incident id required; optional provider, scope id, service id, and time window narrow the read | `limit` default 25, max 100 | yes | prompt-ready for on-call incident packets; separates exact provider evidence, fallback service/time change candidates, derived edges, ambiguous candidates, and explicit missing evidence |
...
- Incident context route contracts, explicit missing evidence slots, explicit
  PagerDuty service URL to runtime/image evidence, ambiguous provider-scope
  errors, OpenAPI fields, and MCP route mapping.
