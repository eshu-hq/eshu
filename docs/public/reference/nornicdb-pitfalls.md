...
"Serialization Is Not A Fix" in the root agent rules.
...
Per the **NornicDB Maintainer Patch Bar** in
`docs/internal/agent-guide.md`, patches against the NornicDB fork are
...
1. **Write a failing test in `NornicDB-New` first** per the NornicDB
   the root agent rules' mandatory bug-fix workflow. A patch without a failing test is
