...
`go/internal/query/impact/resource_investigation.go`) follows this shape.
...
`testutil/graph`'s `AssertCypherHasNoIgnoredLabelPredicate` rejects the ignored
...
`go/internal/query/impact/blast_radius.go` and the guard test
...
(`go/internal/query/impact/trace_deployment_oci.go`, guard test
...
read (`go/internal/query/impact/change_surface_response.go`,
`go/internal/query/impact/change_surface_legacy.go`, guard test
