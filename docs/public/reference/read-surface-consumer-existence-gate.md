...
4. **Reducer dispatch** — a `case facts.<Kind>:`, `== facts.<Kind>`, or
   ...
   `extractPackageSourceHints` in `go/internal/reducer/packages/correlation/source.go`
...
7. **`array.Array`-bound fact-kind slice** (round 2, `arrayBoundSliceFactKinds`) —
   a package-level `<name> = []string{"a", "b", ...}` declaration in
   `go/internal/query` that is passed as `array.Array(<name>)` into a live
   ...
   is the concrete round-2 case. Requiring the `array.Array` call site (not mere
...
reader signal; no `array.Array`-bound parameterized-query signal) and named 3
...
reader, or the `array.Array` slice signal), is no longer in the disclosure
