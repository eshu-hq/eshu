...
7. **`array.Of`-bound fact-kind slice** (round 2, `arrayBoundSliceFactKinds`) —
   a package-level `<name> = []string{"a", "b", ...}` declaration in
   `go/internal/query` that is passed as `array.Of(<name>)` into a live
   ...
   is the concrete round-2 case. Requiring the `array.Of` call site (not mere
...
reader signal; no `array.Of`-bound parameterized-query signal) and named 3
...
reader, or the `array.Of` slice signal), is no longer in the disclosure
