...
| Argo CD | Application sources, ApplicationSet discovery and deploy sources, destination platform hints. Turning a document into those typed values lives in `go/internal/relationships/argocd_document_parse.go`; emitting evidence from them stays in `yaml_iac_evidence.go`, and a test holds that boundary (#5573). | `DEPLOYS_FROM`, `DISCOVERS_CONFIG_IN`, `RUNS_ON` |
