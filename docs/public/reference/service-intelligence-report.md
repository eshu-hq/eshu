...
sections and carries the captured truth envelope verbatim. The `supply_chain`
and `incidents_support` sections are emitted `unsupported` (with their fallback
next calls) until their evidence is wired in. A response with no truth composes
an `unsupported` report rather than fabricating confidence.
