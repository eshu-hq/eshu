...
| `eshu_dp_code_fingerprint_entities_total` | Function body fingerprint volume for the code-divergence report (#6835), labeled by bounded `language` and `outcome` (`fingerprinted`, `skipped`); skipped rows carry bounded `reason` (`below_floor`, `has_error`, `no_body`). Absent fingerprint keys mean "not fingerprinted", never "unique". |
| `eshu_dp_code_fingerprint_duration_seconds` | Per-file fingerprinting time histogram, labeled by bounded `language`. Observed once per file that produced any fingerprint outcome. |
...
| `eshu_dp_git_repo_sync_failures_total` | Per-repository git sync operations that failed and were isolated to that one repository for the cycle — the failing repo is skipped and the rest of the fleet keeps syncing. With scheduled sync enabled it is retried on the next poll; in webhook-only mode (scheduled sync disabled; see [ingestion and queue environment](../environment-ingestion-queues.md)) it waits for its next webhook trigger, and the current trigger is still marked `handed_off`, the same as an isolated clone or fetch failure — labeled by bounded `operation` (`clone`, `fetch`, `list_refs`) (issue #7001). A sustained `list_refs` rate on a handful of repos points at remote/DNS trouble for those specific repos; a rate approaching the fleet size across all three operations points at a fleet-wide auth or network outage. Repository identity stays in the paired `git repository sync failed` log line (`failure_class=git_sync_failure`), not in the metric label. |
...
(`go/internal/collector/repo/git/snapshot_native_test.go`), which asserts the
