...
| `eshu_dp_git_repo_sync_failures_total` | Per-repository git sync operations that failed and were isolated to that one repository for the cycle — the failing repo is skipped and the rest of the fleet keeps syncing. With scheduled sync enabled it is retried on the next poll; in webhook-only mode (`ESHU_REPO_SCHEDULED_SYNC_ENABLED=false`) it waits for its next webhook trigger, and the current trigger is still marked `handed_off`, the same as an isolated clone or fetch failure — labeled by bounded `operation` (`clone`, `fetch`, `list_refs`) (issue #7001). A sustained `list_refs` rate on a handful of repos points at remote/DNS trouble for those specific repos; a rate approaching the fleet size across all three operations points at a fleet-wide auth or network outage. Repository identity stays in the paired `git repository sync failed` log line (`failure_class=git_sync_failure`), not in the metric label. |
