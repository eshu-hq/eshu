...
| `eshu_dp_git_repo_sync_failures_total` | Per-repository git sync operations that failed and were isolated to that one repository for the cycle — the failing repo is skipped and retried next cycle, and the rest of the fleet keeps syncing — labeled by bounded `operation` (`clone`, `fetch`, `list_refs`) (issue #7001). A sustained `list_refs` rate on a handful of repos points at remote/DNS trouble for those specific repos; a rate approaching the fleet size across all three operations points at a fleet-wide auth or network outage. Repository identity stays in the paired `git repository sync failed` log line (`failure_class=git_sync_failure`), not in the metric label. |
