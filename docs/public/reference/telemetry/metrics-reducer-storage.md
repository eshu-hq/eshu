...
| `eshu_dp_cross_repo_edges_resolved_total` | counter | Cross-repo edge ownership outcomes by `relationship_type` and bounded `outcome` (`owned_routed` or `foreign_owned_dropped`). |
