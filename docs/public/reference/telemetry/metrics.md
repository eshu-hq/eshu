...
| `eshu_dp_superseded_generation_fence_total` | Projector work stopped because its scope generation is superseded (#7130), by `failure_class`. `projector_ack_generation_superseded`: Ack refused to re-activate the generation and marked the work item superseded, so superseded-generation work reached a worker (for example through replay before the fence, or a claim that raced a supersede). `projector_replay_generation_superseded`: a replay or dead-letter drain left such rows terminal, or the admin replay refused explicit ids naming them. A nonzero Ack rate is worth tracing to the replay or claim that produced the work. |
