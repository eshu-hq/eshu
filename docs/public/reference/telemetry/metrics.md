...
| `eshu_dp_query_impact_ownership_check_duration_seconds` | Wall time of one impact ownership statement chunk (at most 500 keys), by `route`, `node_label` (`WorkloadInstance`, `CloudResource`, `TerraformStateResource`), and `outcome`. The per-class cost the ownership budget is sized from; p95 near the 10 s graph-read deadline means the budget is too generous for the graph. |
