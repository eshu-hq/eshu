...
| Source collectors and webhooks | `tfstate.*`, `webhook.handle`, `webhook.store`, `oci_registry.*`, `vulnerability_intelligence.*`, `security_alert.*`, `package_registry.*`, `scanner_worker.*`, and `aws.*` |
