...
The stable span families are grouped below.
...
The graph read wrapper can also emit `neo4j.query` and `neo4j.query.single`
around graph-backed read calls. Treat those as read-path dependency spans, not
as replacement names for the write-path `neo4j.execute` span.
