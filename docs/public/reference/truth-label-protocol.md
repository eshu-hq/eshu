...
| `basis` | `authoritative_graph`, `semantic_facts`, `content_index`, `hybrid`, `runtime_state`, or `no_backend_read`. `no_backend_read` is the one value that names an absence rather than an evidence source: the answer was produced without reading any backend. It always carries `level: fallback`. **Scope:** it is emitted today only by the empty-grant pages of `POST /api/v0/code/language-query`, `POST /api/v0/code/imports/investigate`, `GET /api/v0/repositories/by-language` and `GET /api/v0/repositories/language-inventory` — not by every no-read response. Other proven no-read paths still report a read-based basis and are not yet migrated (for example the empty observability-coverage correlations page reports `semantic_facts` and the empty infrastructure-search page reports `hybrid`, both without reading a backend). Clients MUST NOT treat `no_backend_read` as a universal marker for "nothing was read", nor its absence as proof that a backend was read. |
