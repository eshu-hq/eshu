...
These conventions are auto-generated from `skill-fragments/`. Do not edit them by hand; run `go run ./cmd/skillgen gen` to regenerate.

Aider does not discover this file on its own. Point at it from `.aider.conf.yml`:

```yaml
read: .aider/eshu-conventions.md
```
