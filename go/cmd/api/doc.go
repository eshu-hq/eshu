...
// impact explanation reads, and an optional Prometheus/Mimir metrics
// time-series source for console trends. It listens on ESHU_API_ADDR (default
// :8080) wrapped in otelhttp instrumentation. On SIGINT or SIGTERM it gives the
// HTTP server up to five seconds for graceful shutdown before exiting. The
// runtime serves reads only; it does not own repo sync, parsing, fact emission,
// or queued projection work.
//
