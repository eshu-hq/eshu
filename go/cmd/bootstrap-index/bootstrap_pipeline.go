...
func runPipelined(
	...
	//
	// container_image_identity is safe to reopen without ordering this slice.
	// Its writer publishes one complete immutable support set per scope, then
	// atomically moves the scope's active_set_id after checking the exact claim
	// and activation epoch. A reclassification replaces the active set, and a
	// demotion publishes an explicit empty set, so an older decision cannot stay
	// current. OCI completeness warnings carry only the affected prior supports
	// into the new set instead of turning missing evidence into a destructive
	// retirement. This closes the outcome-keyed replay gap from #5847 and keeps
	// the bounded-degradation and stale-writer safeguards from #5854 and #5740.
