...
func TestPipelinedBootstrapRunsDeferredBackfillWorkflow(t *testing.T) {
	...
	// container_image_identity must be replayed so a cross-scope ci.artifact →
	// OCI manifest join (and the #5423 ci_run_commit provenance) resolves once
	// the OCI generation is active on a later maintenance pass.
