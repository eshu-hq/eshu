...
// bootstrapKustomizeOverlayListCypher lists every KustomizeOverlay node in
// one repo. Single anchoring clause (MATCH -> WHERE -> RETURN, no
// intervening WITH or second MATCH) -- the safe read shape documented in
// docs/public/reference/nornicdb-query-pitfalls.md's "Multi-Clause Read
// Queries Silently Corrupt The Projection", mirroring
// bootstrapTerraformStateConfigMatchCandidateCountCypher. Anchored by the
// #5445 kustomize_overlay_repo_id index (go/internal/graph/schema_tables_indexes.go),
// not a bare label scan.
const bootstrapKustomizeOverlayListCypher = `MATCH (ko:KustomizeOverlay)
WHERE ko.repo_id = $repo_id
RETURN ko.uid AS uid, ko.path AS path, ko.base_refs AS base_refs`

// bootstrapKustomizeOverlayResolver adapts a read-only Bolt session to
// sourcecypher.KustomizeOverlayResolver (#5445 slice 3), so
// bootstrap-index's canonical writer can rebuild the #5445 EXTENDS_BASE edge
// set for a repo's FULL KustomizeOverlay set every cycle that touches or
// delta-deletes any of them, not only the entities present in the current
// materialization. Mirrors
// cmd/ingester/kustomize_overlay_resolver.go's
// ingesterKustomizeOverlayResolver.
type bootstrapKustomizeOverlayResolver struct {
...
func (r bootstrapKustomizeOverlayResolver) ListKustomizeOverlays(
	ctx context.Context,
	repoID string,
	...
	result, err := session.Run(ctx, bootstrapKustomizeOverlayListCypher, map[string]any{"repo_id": repoID})
	if err != nil {
	...
	for _, record := range records {
		...
		rows = append(rows, sourcecypher.KustomizeOverlayRow{
			UID:      uid,
			Path:     path,
			BaseRefs: bootstrapKustomizeOverlayBaseRefsFromBoltValue(baseRefsValue),
		})
	}
	return rows, nil
}

// bootstrapKustomizeOverlayBaseRefsFromBoltValue normalizes a Bolt-decoded
...
// not an error, matching every other tolerant decode in this codebase.
func bootstrapKustomizeOverlayBaseRefsFromBoltValue(value any) []string {
