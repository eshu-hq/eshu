...
// TestBootstrapCanonicalExecutorBoundsNeo4jWritesLikeNornicDB pins retry
// parity for a timed-out bootstrap canonical write: on both backends a
// non-RUNS_ON group that outlives ESHU_CANONICAL_WRITE_TIMEOUT must end as a
// retryable graph_write_timeout.
func TestBootstrapCanonicalExecutorBoundsNeo4jWritesLikeNornicDB(t *testing.T) {
	t.Parallel()
	...
	for _, backend := range []runtimecfg.GraphBackend{runtimecfg.GraphBackendNeo4j, runtimecfg.GraphBackendNornicDB} {
		t.Run(string(backend), func(t *testing.T) {
			...
			executor, err := bootstrapCanonicalExecutorForGraphBackend(
				blockingNeo4jTimeoutExecutor{}, backend, timeoutGetenv("20ms"), nil, nil, nil, nil,
			)
			if err != nil {
