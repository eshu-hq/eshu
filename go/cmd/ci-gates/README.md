...
`main` merges through a GitHub merge queue, so the publisher also aggregates
queue entries. A merge group has no pull request of its own (it can carry
several), so it is addressed by its commit:
...
re-verification; a queue commit never moves. `--event` defaults to
`pull_request`, which requires `--pr`; `merge_group` rejects `--pr`.
