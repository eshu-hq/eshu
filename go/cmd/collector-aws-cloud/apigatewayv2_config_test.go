...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForAPIGatewayV2(t *testing.T) {
	...
	if got, want := config.AWS.Targets[0].AllowedServices[0], awscloud.ServiceAPIGatewayV2; got != want {
		t.Fatalf("AllowedServices[0] = %q, want %q", got, want)
	}
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for API Gateway v2-only target, want zero key")
	}
}
