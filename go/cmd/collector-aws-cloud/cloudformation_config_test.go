...
// TestLoadRuntimeConfigRequiresRedactionKeyForCloudFormation confirms that a
// CloudFormation-only target scope requires a redaction key because secret-like
// stack output values route through the shared redact library before
// persistence. CloudFormation is the highest template-body redaction surface in
// the AWS collector.
func TestLoadRuntimeConfigRequiresRedactionKeyForCloudFormation(t *testing.T) {
		...
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			...
			"configuration":{
				"target_scopes":[{
					...
					"allowed_services":["cloudformation"],
					"credentials":{
	...
	if config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey zero for CloudFormation target, want configured key")
	}
}

// TestLoadRuntimeConfigRejectsMissingRedactionKeyForCloudFormation confirms the
// negative path: a CloudFormation-only target with no ESHU_AWS_REDACTION_KEY is
// rejected with an error naming the missing key.
func TestLoadRuntimeConfigRejectsMissingRedactionKeyForCloudFormation(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
