...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForSQS(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for SQS-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForSNS(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for SNS-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForEventBridge(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for EventBridge-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForGlue(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for Glue-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForMSK(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for MSK-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForStepFunctions(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
			...
			"configuration":{
				"target_scopes":[{
					"account_id":"123456789012",
					"allowed_regions":["us-east-1"],
					"allowed_services":["stepfunctions"],
					"credentials":{
						"mode":"local_workload_identity"
					}
				}]
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for Step Functions-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForAccessAnalyzer(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
			...
			"configuration":{
				"target_scopes":[{
					"account_id":"123456789012",
					"allowed_regions":["us-east-1"],
					"allowed_services":["accessanalyzer"],
					"credentials":{
						"mode":"local_workload_identity"
					}
				}]
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for Access Analyzer-only target, want zero key")
	}
}
...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForS3(t *testing.T) {
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for S3-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForRDS(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for RDS-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForDynamoDB(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for DynamoDB-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForRedshift(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for Redshift-only target, want zero key")
	}
}

func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForCloudWatchLogs(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
			...
			"configuration":{
				"target_scopes":[{
					"account_id":"123456789012",
					"allowed_regions":["us-east-1"],
					"allowed_services":["cloudwatchlogs"],
					"credentials":{
						"mode":"local_workload_identity"
					}
				}]
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for CloudWatch Logs-only target, want zero key")
	}
}
...
func TestLoadRuntimeConfigRequiresRedactionKeyForLambda(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
...
func TestLoadRuntimeConfigMapsRedactionKeyForLambda(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
