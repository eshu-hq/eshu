...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForEFS(t *testing.T) {
	...
	if got, want := config.AWS.Targets[0].AllowedServices[0], awscloud.ServiceEFS; got != want {
		t.Fatalf("AllowedServices[0] = %q, want %q", got, want)
	}
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for EFS-only target, want zero key")
	}
}
