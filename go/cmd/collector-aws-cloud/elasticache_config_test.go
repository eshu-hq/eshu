...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForElastiCache(t *testing.T) {
	...
	if got, want := config.AWS.Targets[0].AllowedServices[0], awscloud.ServiceElastiCache; got != want {
		t.Fatalf("AllowedServices[0] = %q, want %q", got, want)
	}
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for ElastiCache-only target, want zero key")
	}
}
