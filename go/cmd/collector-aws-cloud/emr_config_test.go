...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForEMR(t *testing.T) {
	...
	if got, want := config.AWS.Targets[0].AllowedServices[0], awscloud.ServiceEMR; got != want {
		t.Fatalf("AllowedServices[0] = %q, want %q", got, want)
	}
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for EMR-only target, want zero key (scanner emits no redacted fields)")
	}
}
