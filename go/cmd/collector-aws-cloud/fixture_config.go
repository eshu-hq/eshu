...
type fixtureFileScope struct {
	...
	Resources []runtime.FixtureResource `json:"resources"`
	// Relationships are the aws_relationship observations replayed for this scope.
	Relationships []runtime.FixtureRelationship `json:"relationships"`
}
...
func loadFixtureConfig(path string) (runtime.FixtureConfig, time.Duration, error) {
	raw, err := os.ReadFile(filepath.Clean(path))
	if err != nil {
		return runtime.FixtureConfig{}, 0, fmt.Errorf("read aws fixture config %q: %w", path, err)
	}
	var fileCfg fixtureFileConfig
	if err := json.Unmarshal(raw, &fileCfg); err != nil {
		return runtime.FixtureConfig{}, 0, fmt.Errorf("parse aws fixture config %q: %w", path, err)
	}
	...
	if rawInterval := strings.TrimSpace(fileCfg.PollInterval); rawInterval != "" {
		...
		if err != nil {
			return runtime.FixtureConfig{}, 0, fmt.Errorf("parse aws fixture poll_interval %q: %w", rawInterval, err)
		}
		pollInterval = interval
	}

	cfg := runtime.FixtureConfig{
		CollectorInstanceID: strings.TrimSpace(fileCfg.CollectorInstanceID),
		Scopes:              make([]runtime.FixtureScope, 0, len(fileCfg.Scopes)),
	}
	for i := range fileCfg.Scopes {
		cfg.Scopes = append(cfg.Scopes, fileCfg.Scopes[i].scope())
	}
	if err := cfg.Validate(); err != nil {
		return runtime.FixtureConfig{}, 0, err
	}
	return cfg, pollInterval, nil
}

func (s fixtureFileScope) scope() runtime.FixtureScope {
	return runtime.FixtureScope{
