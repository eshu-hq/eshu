...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForGlobalAccelerator(t *testing.T) {
		...
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			...
			"configuration":{
				"target_scopes":[{
					...
					"allowed_services":["globalaccelerator"],
					"credentials":{
	...
	if got, want := config.AWS.Targets[0].AllowedServices[0], awscloud.ServiceGlobalAccelerator; got != want {
		t.Fatalf("AllowedServices[0] = %q, want %q", got, want)
	}
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for Global-Accelerator-only target, want zero key")
	}
}
