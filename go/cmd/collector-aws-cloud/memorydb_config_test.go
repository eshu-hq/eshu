...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForMemoryDB(t *testing.T) {
	...
	if got, want := config.AWS.Targets[0].AllowedServices[0], awscloud.ServiceMemoryDB; got != want {
		t.Fatalf("AllowedServices[0] = %q, want %q", got, want)
	}
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for MemoryDB-only target, want zero key")
	}
}
