...
// TestLoadRuntimeConfigDoesNotRequireRedactionKeyForRAM confirms a RAM-only
// target loads without ESHU_AWS_REDACTION_KEY. The RAM scanner is metadata-only
// and redacts nothing, so it must not be derived into the redaction-key
// requirement set.
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForRAM(t *testing.T) {
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for RAM-only target, want zero key")
	}
}
