...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForSecretsManager(t *testing.T) {
		...
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			...
			"configuration":{
				"target_scopes":[{
					...
					"allowed_services":["secretsmanager"],
					"credentials":{
	...
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for Secrets Manager-only target, want zero key")
	}
}
