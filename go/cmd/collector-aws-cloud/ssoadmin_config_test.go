...
func TestLoadRuntimeConfigRequiresRedactionKeyForSSOAdmin(t *testing.T) {
...
func TestLoadRuntimeConfigMapsRedactionKeyForSSOAdmin(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
...
func TestLoadRuntimeConfigRequiresUSEast1ForSSOAdmin(t *testing.T) {
	getenv := mapEnv(map[string]string{
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			"instance_id":"collector-aws-1",
			"collector_kind":"aws",
	...
	_, err := loadRuntimeConfig(getenv)
	if err == nil {
		t.Fatalf("loadRuntimeConfig() error = nil, want region rejection")
	}
	if !strings.Contains(err.Error(), "ssoadmin scans require") {
		t.Fatalf("loadRuntimeConfig() error = %v, want ssoadmin region requirement", err)
	}
}
