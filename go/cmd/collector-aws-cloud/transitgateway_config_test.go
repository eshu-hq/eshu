...
func TestLoadRuntimeConfigDoesNotRequireRedactionKeyForTransitGateway(t *testing.T) {
		...
		"ESHU_COLLECTOR_INSTANCES_JSON": `[{
			...
			"configuration":{
				"target_scopes":[{
					...
					"allowed_services":["transitgateway"],
					"credentials":{
	...
	if got, want := config.AWS.Targets[0].AllowedServices[0], awscloud.ServiceTransitGateway; got != want {
		t.Fatalf("AllowedServices[0] = %q, want %q", got, want)
	}
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for transit-gateway-only target, want zero key")
	}
}
