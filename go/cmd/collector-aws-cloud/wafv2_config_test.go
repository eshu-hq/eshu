...
func TestLoadRuntimeConfigAcceptsWAFv2WithoutRedactionKey(t *testing.T) {
	...
	if got, want := config.AWS.Targets[0].AllowedServices[0], awscloud.ServiceWAFv2; got != want {
		t.Fatalf("AllowedServices[0] = %q, want %q", got, want)
	}
	if !config.AWSRedactionKey.IsZero() {
		t.Fatalf("AWSRedactionKey configured for WAFv2-only target, want zero key")
	}
}
