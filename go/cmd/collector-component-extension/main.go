...
func main() {
	if handled, err := buildinfo.PrintVersionFlag(os.Args[1:], os.Stdout, "eshu-collector-component-extension"); handled {
		if err != nil {
	...
	bootstrap, err := telemetry.NewBootstrap("collector-component-extension")
	if err != nil {
	...
	logger := telemetry.NewLogger(bootstrap, "collector-component-extension", "collector-component-extension")

	if err := run(context.Background()); err != nil {
		logger.Error("collector-component-extension failed", telemetry.EventAttr("runtime.startup.failed"), "error", err)
		os.Exit(1)
	}
}
...
func run(parent context.Context) error {
	...
	storeDB := &postgres.InstrumentedDB{
		...
		StoreName:   "collector_component_extension",
	}
