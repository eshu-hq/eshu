...
| `eshu_dp_confluence_http_requests_total` | Counter | `operation`, `result`, `status_class` | Counts bounded Confluence HTTP GET operations without page IDs, titles, URLs, or paths. Use it to separate permission failures, rate limits, retryable provider statuses, decode errors, and successful reads. |
