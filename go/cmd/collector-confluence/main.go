...
func main() {
	if handled, err := buildinfo.PrintVersionFlag(os.Args[1:], os.Stdout, "eshu-collector-confluence"); handled {
		if err != nil {
	...
	bootstrap, err := telemetry.NewBootstrap("collector-confluence")
	if err != nil {
	...
	logger := telemetry.NewLogger(bootstrap, "collector-confluence", "collector-confluence")

	if err := run(context.Background()); err != nil {
		logger.Error("collector-confluence failed", telemetry.EventAttr("runtime.startup.failed"), "error", err)
		os.Exit(1)
	}
}
