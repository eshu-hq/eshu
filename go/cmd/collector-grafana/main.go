...
const runtimeName = "collector-grafana"
...
func main() {
	if handled, err := buildinfo.PrintVersionFlag(os.Args[1:], os.Stdout, "eshu-collector-grafana"); handled {
		if err != nil {
	...
	if err := run(context.Background(), opts); err != nil {
		logger.Error("collector-grafana failed", telemetry.EventAttr("runtime.startup.failed"), "error", err)
		os.Exit(1)
	}
}
