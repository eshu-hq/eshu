...
// collector commit boundary. It requires no live Grafana credentials.
...
func buildClaimedService(
	...
	source, err := grafana.NewClaimedSource(config.Source)
	if err != nil {
		return collector.ClaimedService{}, err
	}
	committer := postgres.NewIngestionStore(database)
	...
	return collector.ClaimedService{
		...
		Source:              source,
		Committer:           committer,
		CollectorKind:       scope.CollectorKind(grafana.CollectorKind),
...
func newClaimID() string {
	...
	if _, err := rand.Read(raw[:]); err == nil {
		return "grafana-claim-" + hex.EncodeToString(raw[:])
	}
	next := atomic.AddUint64(&fallbackClaimSequence, 1)
	return fmt.Sprintf("grafana-claim-fallback-%d-%d", time.Now().UTC().UnixNano(), next)
}
