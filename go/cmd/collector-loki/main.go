...
const runtimeName = "collector-loki"
...
func main() {
	if handled, err := buildinfo.PrintVersionFlag(os.Args[1:], os.Stdout, "eshu-collector-loki"); handled {
		if err != nil {
	...
	if err := run(context.Background(), opts); err != nil {
		logger.Error("collector-loki failed", telemetry.EventAttr("runtime.startup.failed"), "error", err)
		os.Exit(1)
	}
}
