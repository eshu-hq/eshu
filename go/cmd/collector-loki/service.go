...
// collector commit boundary. It requires no live Loki credentials.
...
func buildClaimedService(
	...
	source, err := loki.NewClaimedSource(config.Source)
	if err != nil {
		return collector.ClaimedService{}, err
	}
	committer := postgres.NewIngestionStore(database)
	...
	return collector.ClaimedService{
		...
		Source:              source,
		Committer:           committer,
		CollectorKind:       scope.CollectorKind(loki.CollectorKind),
...
func newClaimID() string {
	...
	if _, err := rand.Read(raw[:]); err == nil {
		return "loki-claim-" + hex.EncodeToString(raw[:])
	}
	next := atomic.AddUint64(&fallbackClaimSequence, 1)
	return fmt.Sprintf("loki-claim-fallback-%d-%d", time.Now().UTC().UnixNano(), next)
}
