...
const (
	envCollectorInstanceID = "ESHU_PROMETHEUS_MIMIR_COLLECTOR_INSTANCE_ID"
	envPollInterval        = "ESHU_PROMETHEUS_MIMIR_COLLECTOR_POLL_INTERVAL"
	envClaimLeaseTTL       = "ESHU_PROMETHEUS_MIMIR_COLLECTOR_CLAIM_LEASE_TTL"
	envHeartbeatInterval   = "ESHU_PROMETHEUS_MIMIR_COLLECTOR_HEARTBEAT_INTERVAL"
	envCollectorOwnerID    = "ESHU_PROMETHEUS_MIMIR_COLLECTOR_OWNER_ID"
...
func loadClaimedRuntimeConfig(getenv func(string) string) (claimedRuntimeConfig, error) {
	...
	instance, err := selectPrometheusMimirInstance(instances, getenv(envCollectorInstanceID))
	if err != nil {
		return claimedRuntimeConfig{}, err
	}
	if err := validatePrometheusMimirInstance(instance); err != nil {
		return claimedRuntimeConfig{}, err
	}
	sourceConfig, err := parsePrometheusMimirRuntimeConfiguration(instance, getenv)
	if err != nil {
		return claimedRuntimeConfig{}, err
	}
	...
	if heartbeatInterval >= claimLeaseTTL {
		return claimedRuntimeConfig{}, fmt.Errorf("prometheus/mimir collector heartbeat interval must be less than claim lease TTL")
	}
...
func selectPrometheusMimirInstance(
	...
	case 0:
		if requestedInstanceID != "" {
			return workflow.DesiredCollectorInstance{}, fmt.Errorf("prometheus/mimir collector instance %q not found", requestedInstanceID)
		}
		return workflow.DesiredCollectorInstance{}, fmt.Errorf("no prometheus/mimir collector instance configured")
	case 1:
		return matches[0], nil
	default:
		return workflow.DesiredCollectorInstance{}, fmt.Errorf("multiple prometheus/mimir collector instances configured; set %s", envCollectorInstanceID)
	}
}

func validatePrometheusMimirInstance(instance workflow.DesiredCollectorInstance) error {
	...
	if !instance.ClaimsEnabled {
		return fmt.Errorf("prometheus/mimir collector requires claim-enabled collector instance")
	}
	return nil
}
...
func ownerID(getenv func(string) string) string {
	...
	return "collector-prometheus-mimir"
}
