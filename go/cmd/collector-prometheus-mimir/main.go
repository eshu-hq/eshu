...
const runtimeName = "collector-prometheus-mimir"

func main() {
	if handled, err := buildinfo.PrintVersionFlag(os.Args[1:], os.Stdout, "eshu-collector-prometheus-mimir"); handled {
		if err != nil {
	...
	if err := run(context.Background()); err != nil {
		logger.Error("collector-prometheus-mimir failed", telemetry.EventAttr("runtime.startup.failed"), "error", err)
		os.Exit(1)
	}
}
...
func run(parent context.Context) error {
	...
	storeDB := &postgres.InstrumentedDB{
		...
		StoreName:   "collector_prometheus_mimir",
	}
