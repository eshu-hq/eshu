...
// collector commit boundary. It requires no live Prometheus/Mimir credentials.
...
func newClaimID() string {
	...
	if _, err := rand.Read(raw[:]); err == nil {
		return "prometheus-mimir-claim-" + hex.EncodeToString(raw[:])
	}
	next := atomic.AddUint64(&fallbackClaimSequence, 1)
	return fmt.Sprintf("prometheus-mimir-claim-fallback-%d-%d", time.Now().UTC().UnixNano(), next)
}
