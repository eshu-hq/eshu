...
const runtimeName = "collector-sbom-attestation"
...
func main() {
	if handled, err := buildinfo.PrintVersionFlag(os.Args[1:], os.Stdout, "eshu-collector-sbom-attestation"); handled {
		if err != nil {
	...
	bootstrap, err := telemetry.NewBootstrap("collector-sbom-attestation")
	if err != nil {
	...
	logger := telemetry.NewLogger(bootstrap, "collector-sbom-attestation", "collector-sbom-attestation")
	...
	if err := run(context.Background(), opts); err != nil {
		logger.Error("collector-sbom-attestation failed", telemetry.EventAttr("runtime.startup.failed"), "error", err)
		os.Exit(1)
	}
}
...
func run(parent context.Context, opts launchOptions) error {
	...
	storeDB := &postgres.InstrumentedDB{
		...
		StoreName:   "collector_sbom_attestation",
	}
