...
const runtimeName = "collector-tempo"

func main() {
	if handled, err := buildinfo.PrintVersionFlag(os.Args[1:], os.Stdout, "eshu-collector-tempo"); handled {
		if err != nil {
	...
	if err := run(context.Background()); err != nil {
		logger.Error("collector-tempo failed", telemetry.EventAttr("runtime.startup.failed"), "error", err)
		os.Exit(1)
	}
}
