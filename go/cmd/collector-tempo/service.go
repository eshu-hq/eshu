...
// collector commit boundary. It requires no live Tempo credentials.
...
func buildClaimedService(
	...
	source, err := tempo.NewClaimedSource(config.Source)
	if err != nil {
		return collector.ClaimedService{}, err
	}
	committer := postgres.NewIngestionStore(database)
	...
	return collector.ClaimedService{
		...
		Source:              source,
		Committer:           committer,
		CollectorKind:       scope.CollectorKind(tempo.CollectorKind),
...
func newClaimID() string {
	...
	if _, err := rand.Read(raw[:]); err == nil {
		return "tempo-claim-" + hex.EncodeToString(raw[:])
	}
	next := atomic.AddUint64(&fallbackClaimSequence, 1)
	return fmt.Sprintf("tempo-claim-fallback-%d-%d", time.Now().UTC().UnixNano(), next)
}
