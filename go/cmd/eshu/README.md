...
- `eshu docs verify [path]` is a local documentation-truth verifier. It scans
  ...
  Git worktree root or current working directory; missing files or Terraform
  blocks are contradicted findings, not valid docs. Without `--persist`, it
  does not open Postgres or graph connections. With
