...
func TestRunVulnScanRepoScopedModeFailsClosedOnUnknownFreshness(t *testing.T) {
	...
	server := vulnScanReportTestServer(t, repoPath, `{"data":{"findings":[],"count":0,"limit":50,"truncated":false,"readiness":{"readiness_state":"ready_zero_findings","target_scope":{"repository_id":"repo-local"},"evidence_sources":[{"family":"package.consumption","fact_count":2,"freshness":"unknown"},{"family":"vulnerability.advisory","fact_count":80,"freshness":"unknown"}],"freshness":"unknown","counts":{"findings_returned":0,"evidence_facts_total":82}}},"truth":{"level":"exact","freshness":{"state":"fresh"}},"error":null}`)
	defer server.Close()
