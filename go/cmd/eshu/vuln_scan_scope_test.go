...
func TestRunVulnScanRepoDefaultScopedModeAttachesScopePlanAndPerformance(t *testing.T) {
	...
	if got := plan["observed_packages"]; got == nil || toInt(t, got) != 4 {
		t.Fatalf("plan[observed_packages] = %#v, want 4", got)
	}
	if got := plan["advisory_sources"]; got == nil || toInt(t, got) != 120 {
		t.Fatalf("plan[advisory_sources] = %#v, want 120", got)
	}
	if got := plan["package_registry"]; got == nil || toInt(t, got) != 2 {
		t.Fatalf("plan[package_registry] = %#v, want 2", got)
	}
	...
	for _, key := range []string{"started_at", "completed_at", "wall_time_ms", "scope_mode", "stop_threshold", "observed_packages", "advisory_sources", "package_registry_packages"} {
...
func TestRunVulnScanRepoScopedModeFailsClosedOnStaleAdvisoryCache(t *testing.T) {
	// When the readiness envelope reports a stale advisory source snapshot for
	// an observed ecosystem, the scoped CLI must fail closed instead of
	// printing a clean report. Issue #655 requires preserving fail-closed
	// readiness for stale advisory evidence.
	repoPath := t.TempDir()
	reset := stubScanRuntime(t)
	defer reset()
	...
	if err == nil {
		t.Fatal("runVulnScanRepo() error = nil, want fail-closed for stale advisory cache")
	}
	...
	if got, want := data["readiness_state"], "evidence_incomplete"; got != want {
		t.Fatalf("data[readiness_state] = %#v, want %#v (stale advisory cache must fail closed in scoped mode)", got, want)
	}
	plan, ok := data["scope_plan"].(map[string]any)
	if !ok {
		t.Fatalf("data[scope_plan] = %T, want object", data["scope_plan"])
	...
	if !ok || len(missing) == 0 {
		t.Fatalf("plan[missing_evidence] = %#v, want stale advisory_cache marker", plan["missing_evidence"])
	}
	found := false
	for _, value := range missing {
		if reason, ok := value.(string); ok && (reason == "advisory_cache_stale" || reason == "stale_source_snapshot") {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("plan[missing_evidence] = %#v, want advisory_cache_stale or stale_source_snapshot", missing)
	}
}

func TestRunVulnScanRepoScopedModeFailsClosedOnIncompleteAdvisoryCache(t *testing.T) {
	repoPath := t.TempDir()
	reset := stubScanRuntime(t)
	defer reset()
	...
	if err := runVulnScanRepo(cmd, []string{repoPath}); err == nil {
		t.Fatal("runVulnScanRepo() error = nil, want fail-closed for incomplete advisory snapshot")
	}
	...
	if got, want := data["readiness_state"], "target_incomplete"; got != want {
		t.Fatalf("data[readiness_state] = %#v, want %#v (incomplete snapshot must fail closed in scoped mode)", got, want)
	}
}
...
func TestRunVulnScanRepoScopedModeSurfacesEvidenceIncompleteWhenNoOwnedDeps(t *testing.T) {
	...
	// through unmodified. The scope plan still records that observed_packages
	// is zero so operators can see why the server fell back to
	// evidence_incomplete without a follow-up call.
	repoPath := t.TempDir()
	reset := stubScanRuntime(t)
	defer reset()
	...
	plan := data["scope_plan"].(map[string]any)
	if got := plan["observed_packages"]; toInt(t, got) != 0 {
		t.Fatalf("plan[observed_packages] = %#v, want 0", got)
	}
	if got, want := plan["stop_threshold"], "evidence_incomplete"; got != want {
		t.Fatalf("plan[stop_threshold] = %#v, want %#v", got, want)
	}
