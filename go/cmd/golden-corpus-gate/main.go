...
func parseFlags(args []string) (options, error) {
	...
	fs.BoolVar(&o.drainAllowReadinessDeferred, "drain-allow-readiness-deferred", false, "pre-maintenance quiescence mode (#6184): pass when no live, dead-letter, or failed fact rows and no pending completion events remain, reporting readiness-deferred rows and nonterminal shared intents instead of blocking on them. Post-maintenance drains must stay strict")
