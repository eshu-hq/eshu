...
type options struct {
	...
	drainAllowReadinessDeferred bool
...
func parseFlags(args []string) (options, error) {
	...
	fs.BoolVar(&o.drainAllowReadinessDeferred, "drain-allow-readiness-deferred", false, "pre-maintenance quiescence mode (#6184): pass when no live, dead-letter, or failed fact rows, no pending completion events, and no nonterminal shared intents outside repo_dependency remain, reporting readiness-deferred rows and repo_dependency intents instead of blocking on them (#6747). Post-maintenance drains must stay strict")
	...
	fs.IntVar(&o.diffExecutionsAdvisoryMax, "diff-executions-advisory-max", 0, "quorum mode only (#6941): ceiling on the reproduced advisory scheduling-noise divergence total (execution counts or row totals with agreeing results); above it the gate adds a required failing nornicdb_vs_neo4j_executions_ceiling finding instead of leaving the total unbounded. 0 (default) disables the ceiling; the existing nornicdb_vs_neo4j_executions advisory finding is unchanged either way")
