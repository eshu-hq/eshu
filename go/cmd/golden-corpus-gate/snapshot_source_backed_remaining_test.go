...
func sourceBackedContextAndCallSpecs() []sourceBackedShapeSpec {
	return []sourceBackedShapeSpec{
		...
		{slug: "prod-transitive-callers", transport: "http", key: "POST /api/v0/code/relationships?assert=transitive-callers", minimum: 1, maximum: 1, resultsField: "incoming", requestBody: map[string]any{"name": "mutualPing", "repo_id": dartRepo, "direction": "incoming", "relationship_type": "CALLS", "transitive": true, "max_depth": float64(4)}, objectMatches: map[string][]map[string]any{"incoming[]": {{"source_name": "mutualPong", "type": "CALLS", "depth": float64(1), "reason": "transitive_call_graph"}}}},
		{slug: "prod-transitive-callees", transport: "http", key: "POST /api/v0/code/relationships?assert=transitive-callees", minimum: 1, maximum: 1, resultsField: "outgoing", requestBody: map[string]any{"name": "mutualPing", "repo_id": dartRepo, "direction": "outgoing", "relationship_type": "CALLS", "transitive": true, "max_depth": float64(4)}, objectMatches: map[string][]map[string]any{"outgoing[]": {{"target_name": "mutualPong", "type": "CALLS", "depth": float64(1), "reason": "transitive_call_graph"}}}},
	}
}
