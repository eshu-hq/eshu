...
func sourceBackedCoreSpecs() []sourceBackedShapeSpec {
	return []sourceBackedShapeSpec{
		...
		{slug: "prod-aws-runtime-drift-read-model", transport: "mcp", key: "find_unmanaged_resources", minimum: 1, maximum: 10, resultsField: "findings", arguments: map[string]any{"scope_id": awsDriftScope, "limit": float64(10)}, values: map[string]any{"scope_id": awsDriftScope, "findings[].arn": awsDriftARN, "findings[].finding_kind": "image_version_drift"}},
		{slug: "prod-aws-runtime-drift-read-model", transport: "mcp", key: "get_iac_management_status", arguments: map[string]any{"scope_id": awsDriftScope, "resource_id": awsDriftARN}, values: map[string]any{"scope_id": awsDriftScope, "arn": awsDriftARN, "analysis_status": "active_management_finding", "finding.arn": awsDriftARN, "finding.finding_kind": "image_version_drift"}},
		{slug: "prod-aws-runtime-drift-read-model", transport: "mcp", key: "explain_iac_management_status", arguments: map[string]any{"scope_id": awsDriftScope, "resource_id": awsDriftARN}, values: map[string]any{"scope_id": awsDriftScope, "arn": awsDriftARN, "finding.arn": awsDriftARN, "finding.finding_kind": "image_version_drift"}, paths: []string{"evidence_groups[]"}},
		{slug: "prod-aws-runtime-drift-read-model", transport: "mcp", key: "propose_terraform_import_plan", minimum: 1, maximum: 10, resultsField: "candidates", arguments: map[string]any{"scope_id": awsDriftScope, "resource_id": awsDriftARN, "limit": float64(10)}, values: map[string]any{"scope_id": awsDriftScope, "arn": awsDriftARN, "candidates[].arn": awsDriftARN}},
		...
		{slug: "prod-change-surface", transport: "mcp", key: "find_change_surface", minimum: 1, maximum: 10, resultsField: "impacted", arguments: map[string]any{"target": libCommon, "limit": float64(10)}, values: map[string]any{"target.name": libCommon}, objectMatches: map[string][]map[string]any{"impacted[]": {{"name": "orders-api", "rel_type": "DEPENDS_ON"}}}},
		{slug: "prod-change-surface", transport: "mcp", key: "investigate_change_surface", minimum: 1, maximum: 10, resultsField: "direct_impact", arguments: map[string]any{"target": libCommon, "target_type": "repository", "limit": float64(10)}, values: map[string]any{"scope.target": libCommon, "scope.target_type": "repository", "target_resolution.status": "resolved"}, objectMatches: map[string][]map[string]any{"direct_impact[]": {{"name": "orders-api", "depth": float64(1)}}}},
		...
		{slug: "prod-relationships-catalog", transport: "http", key: "POST /api/v0/relationships/edges?assert=depends-on", minimum: 1, maximum: 10, resultsField: "edges", requestBody: map[string]any{"verb": "DEPENDS_ON", "limit": float64(10)}, values: map[string]any{"verb": "DEPENDS_ON"}, objectMatches: map[string][]map[string]any{"edges[]": {{"source_name": "orders-api", "target_name": libCommon}}}},
		{slug: "prod-resource-to-code", transport: "mcp", key: "trace_resource_to_code", minimum: 1, maximum: 10, resultsField: "paths", arguments: map[string]any{"start": "arn:aws:lambda:us-east-1:123456789012:function:supply-chain-demo", "max_depth": float64(4), "limit": float64(10)}, values: map[string]any{"start.id": "arn:aws:lambda:us-east-1:123456789012:function:supply-chain-demo", "paths[].repo_id": "repository:r_69256c06"}, objectMatches: map[string][]map[string]any{"paths[].hops[]": {{"type": "AWS_lambda_function_uses_image"}, {"type": "BUILT_FROM"}}}},
	}
}
