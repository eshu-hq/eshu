...
- `ESHU_CANONICAL_RETRACT_BATCH` controls the per-iteration delete limit for
  ...
  Directory removal, Entity removal) is rewritten at runtime into a
  `WITH <var> LIMIT $__retract_batch DETACH DELETE <var> RETURN count(...)` loop
  that iterates until the graph reports zero nodes deleted, preventing the
  unbounded `DETACH DELETE` from timing out on large corpora (5000+ files,
  10000+ entities). Delta retracts and positive-list retracts are unaffected.
