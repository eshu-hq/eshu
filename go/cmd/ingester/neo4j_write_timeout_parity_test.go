...
// TestCanonicalExecutorForGraphBackendBoundsNeo4jWritesLikeNornicDB pins
// retry parity for a timed-out ingester canonical write: on both backends a
// non-RUNS_ON group that outlives ESHU_CANONICAL_WRITE_TIMEOUT must end as a
// retryable graph_write_timeout, so it requeues instead of dead-lettering.
func TestCanonicalExecutorForGraphBackendBoundsNeo4jWritesLikeNornicDB(t *testing.T) {
	t.Parallel()
	...
	for _, backend := range []runtimecfg.GraphBackend{runtimecfg.GraphBackendNeo4j, runtimecfg.GraphBackendNornicDB} {
		t.Run(string(backend), func(t *testing.T) {
			...
			executor := canonicalExecutorForGraphBackend(
				...
				backend,
				canonicalTransactionTimeout(backend, timeoutGetenv("20ms")),
				false,
