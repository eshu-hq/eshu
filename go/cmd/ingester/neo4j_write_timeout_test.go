...
// TestCanonicalTransactionTimeoutAppliesConfiguredTimeoutToBothBackends pins the server-side
...
func TestCanonicalTransactionTimeoutAppliesConfiguredTimeoutToBothBackends(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := canonicalTransactionTimeout(tt.backend, getenv); got != tt.want {
				t.Fatalf("canonicalTransactionTimeout(%s, %q) = %s, want %s", tt.backend, tt.raw, got, tt.want)
			}
		})
	}
}
