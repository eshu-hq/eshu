...
// This wrapper applies a fresh child context to every call, so a stalled
// iteration fails after the per-statement budget while a drain that keeps
// making progress resets the budget each iteration and is never canceled by an
// earlier one. On the client deadline it returns the shared retryable
// GraphWriteTimeoutError so the reducer queue keeps its graph_write_timeout
// retry classification. A non-positive timeout is a passthrough (mirroring
// TimeoutExecutor), so an unset budget leaves drain behavior unchanged.
//
// The bounded existence probe that precedes a bare-label drain (#6822) is NOT
// bounded by this wrapper: it dispatches through the grouped TimeoutExecutor
// via PhaseGroupExecutor.Inner as a sourcecypher.ProbeExecutor (#6852), which
// gives it the same canonical-write timeout budget under a distinct
// "neo4j execute probe timed out" operation label.
...
// gatedDrainReader wraps a retractDrainReader so each full-refresh DETACH
// DELETE drain write draws a permit from the shared canonical graph-write gate
...
// gated inner GroupExecutor layer; without this wrapper those calls run
// ungated, so with multiple projector workers and
...
// NornicDB overload the gate is meant to close. Each drain iteration is a
// small bounded write, so acquiring one permit per call keeps the total
// concurrent canonical writes (fan-out ExecuteGroup + drains, across all
// workers) bounded to the ceiling. The permit is released before the next
// call, so a multi-iteration per-scope drain holds at most one permit at a
// time. A nil gate makes Acquire a no-op (passthrough), so the wrapper is only
// installed when the ceiling is configured.
//
// The bounded existence probe that precedes a bare-label drain (#6822) is NOT
// gated by this wrapper: it dispatches through the gated inner GroupExecutor
// layer via PhaseGroupExecutor.Inner as a sourcecypher.ProbeExecutor (#6852)
// carrying its own `canonical_probe` operation label, so backpressure
// telemetry still attributes it separately from `canonical_retract_drain`.
