...
// sequence is exhausted it returns 0. The bounded existence probe (#6822) that
// precedes a bare-label drain dispatches through PhaseGroupExecutor.Inner
// (recordingGroupChunkExecutor.ExecuteProbe, #6852) instead of this reader, so
// this fake only ever sees RunWrite calls.
...
DETACH DELETE d`,
	...
	if inner.probeCalls != 1 {
		t.Fatalf("bare-label probes = %d, want 1", inner.probeCalls)
	}
}
