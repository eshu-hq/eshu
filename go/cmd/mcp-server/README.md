...
| `ESHU_SEMANTIC_SEARCH_LOCAL_EMBEDDER` | unset | Optional deterministic no-network local semantic-search embedder for `search_semantic_context`. Accepted values are `hash` and `local_hash`; unset keeps semantic unavailable and hybrid keyword-degraded. |
