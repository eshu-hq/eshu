...
// a machine-readable manifest of which payload fields each reducer-decoded
// fact kind's handlers actually read, derived from the typed
// factschema.Decode* calls in go/internal/reducer/factschema_decode.go, and a
// gate that fails when a handler reads a field no checked-in JSON Schema
// (sdk/go/factschema/schema/*.json) declares — the reverse break the
// forward-direction factschema-diff gate (issue #4569) cannot catch.
//
