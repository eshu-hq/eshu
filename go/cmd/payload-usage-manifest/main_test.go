...
func TestRunGenerateAgainstRealRepoProducesNonTrivialManifest(t *testing.T) {
	...
	if len(manifest.Kinds) != 52 {
		t.Fatalf("len(Kinds) = %d, want 52 (8 aws/iam + 4 wired incident + 2 wired gcp + 2 wired azure + 3 wired kubernetes_live + 8 wired vulnerability + 5 wired sbom_attestation + 6 wired ci_cd_run reducer kinds + 6 projector oci_registry kinds + 5 projector terraform_state kinds + 3 projector package_registry kinds via the reducer and projector factschema_decode*.go globs)", len(manifest.Kinds))
	}
