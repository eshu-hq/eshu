...
// TestProjectorCanonicalTransactionTimeoutAppliesConfiguredTimeoutToBothBackends pins the server-side
...
func TestProjectorCanonicalTransactionTimeoutAppliesConfiguredTimeoutToBothBackends(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := projectorCanonicalTransactionTimeout(tt.backend, getenv); got != tt.want {
				t.Fatalf("projectorCanonicalTransactionTimeout(%s, %q) = %s, want %s", tt.backend, tt.raw, got, tt.want)
			}
		})
	}
}
