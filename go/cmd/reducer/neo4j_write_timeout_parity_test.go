...
// TestSemanticEntityExecutorForGraphBackendBoundsNeo4jWritesLikeNornicDB pins
// retry parity for a timed-out reducer semantic write, the one reducer path
// NornicDB bounds with TimeoutExecutor: on both backends a group that outlives
// ESHU_CANONICAL_WRITE_TIMEOUT must end as a retryable graph_write_timeout.
func TestSemanticEntityExecutorForGraphBackendBoundsNeo4jWritesLikeNornicDB(t *testing.T) {
	t.Parallel()
	...
	for _, backend := range []runtimecfg.GraphBackend{runtimecfg.GraphBackendNeo4j, runtimecfg.GraphBackendNornicDB} {
		t.Run(string(backend), func(t *testing.T) {
			...
			executor := semanticEntityExecutorForGraphBackend(
				blockingNeo4jTimeoutExecutor{}, backend, reducerTransactionTimeout(backend, timeoutGetenv("20ms")), true,
			)
