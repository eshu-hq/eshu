...
func main() {
	if handled, err := buildinfo.PrintVersionFlag(os.Args[1:], os.Stdout, "eshu-scanner-worker"); handled {
		if err != nil {
	...
	bootstrap, err := telemetry.NewBootstrap("scanner-worker")
	if err != nil {
		fallback := slog.New(slog.NewJSONHandler(os.Stderr, nil))
		fallback.Error("scanner-worker bootstrap failed", "event_name", "runtime.startup.failed", "error", err)
		os.Exit(1)
	}
	logger := telemetry.NewLogger(bootstrap, "scanner-worker", "scanner-worker")

	if err := run(context.Background()); err != nil {
		logger.Error("scanner-worker failed", telemetry.EventAttr("runtime.startup.failed"), "error", err)
		os.Exit(1)
	}
}
...
func run(parent context.Context) error {
	...
	storeDB := &postgres.InstrumentedDB{
		...
		StoreName:   "scanner_worker",
	}
	runner, err := buildService(storeDB, os.Getenv, tracer, instruments, logger)
	if err != nil {
		return err
	}
