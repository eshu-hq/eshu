...
3. `go/internal/ask/catalog/annotations_routes.go` — curated annotations for the
   167 implemented HTTP API routes; read owning handlers in `internal/query/`
   before adding or changing entries.
...
5. `go/internal/capabilitycatalog/data/surface-inventory.generated.json` —
   the canonical surface inventory; never hand-edit it; it is generated by
   `cmd/capability-inventory`.

## Invariants this package enforces

- **Overlay completeness** — every implemented `api_route` and `mcp_tool` surface
  in the inventory must have an entry in `askRouteAnnotations()` or
  `askToolAnnotations()`. The coverage drift test (`TestOverlayCoversInventory`)
  enforces this. Any PR that adds implemented surfaces to the inventory without
  updating the overlay will fail CI.
