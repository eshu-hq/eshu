...
func TestStartLocalChildProcessKeepsStdioForTerminalMode(t *testing.T) {
	cmd := &exec.Cmd{}
	closeIO, err := configureLocalChildProcessIOWithCleanup(cmd, "eshu-ingester", []string{"ESHU_LOCAL_LOG_MODE=terminal"})
	if err != nil {
		t.Fatalf("configureLocalChildProcessIOWithCleanup() error = %v, want nil", err)
	}
	if closeIO != nil {
		t.Fatal("closeIO != nil; terminal mode opens no log file to close")
	}
