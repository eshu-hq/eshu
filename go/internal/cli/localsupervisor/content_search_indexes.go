...
var localContentSearchDiscoverRepos = git.DiscoverFilesystemRepositoryIDs
...
func runDeferredContentSearchIndexes(ctx context.Context, out io.Writer, database localContentSearchIndexDB, interval time.Duration, expectedProjectors int) error {
	...
	for {
		...
		if ready {
			...
			_, _ = fmt.Fprintf(out, "deferred content search indexes ready expected_projectors=%d duration=%s\n", expectedProjectors, time.Since(start).Round(time.Millisecond))
			return nil
		}
