...
var (
	...
	// version reader graphinstall is wired with, so an install and a status read
	// agree on what a binary reports.
	ReadGraphVersion            = readLocalGraphVersion
...
// ManagedGraph is a local graph backend this process is responsible for: either
// a child process it spawned or an in-process runtime it linked in. The zero
// value is not usable; construct one through StartManagedNornicDB or
// ManagedGraphFromRecord.
...
func startManagedLocalGraph(ctx context.Context, layout eshulocal.Layout, runtimeConfig RuntimeConfig) (*ManagedGraph, error) {
	switch runtimeConfig.GraphBackend {
	case query.GraphBackendNornicDB:
		return StartManagedNornicDB(ctx, layout)
	default:
		return nil, fmt.Errorf("graph backend %q is not supported by local_authoritative host", runtimeConfig.GraphBackend)
	}
}

// StartManagedNornicDB starts the workspace graph backend, choosing the
// in-process runtime or a managed child process according to
// ESHU_NORNICDB_RUNTIME and what this build actually links in.
func StartManagedNornicDB(ctx context.Context, layout eshulocal.Layout) (*ManagedGraph, error) {
...
func waitForManagedLocalGraph(ctx context.Context, graph *ManagedGraph, timeout time.Duration) error {
...
// StopManagedGraph shuts a managed graph backend down within timeout, using the
// in-process shutdown hook when there is one and SIGTERM-then-kill otherwise. A
// nil graph is a no-op.
func StopManagedGraph(graph *ManagedGraph, timeout time.Duration) error {
	if graph == nil {
		return nil
	}
	defer func() {
...
func stopRecordedLocalGraph(record eshulocal.OwnerRecord) error {
	...
	deadline := time.Now().Add(GraphShutdownTimeout)
...
// ManagedGraphFromRecord rebuilds the connection details of a graph backend
// another process owns, so a client can reach it without supervising it. It
// returns nil when the record names no backend.
func ManagedGraphFromRecord(record eshulocal.OwnerRecord) *ManagedGraph {
...
func graphEnvOverrides(graph *ManagedGraph) map[string]string {
	...
	return map[string]string{
		...
		"ESHU_NEO4J_DATABASE": GraphDatabaseName,
		"NEO4J_URI":           boltURI,
		"NEO4J_USERNAME":      username,
		"NEO4J_PASSWORD":      graph.Password,
		"NEO4J_DATABASE":      GraphDatabaseName,
		"DEFAULT_DATABASE":    GraphDatabaseName,
	}
}

func graphAddress(graph *ManagedGraph) string {
	if graph == nil {
		return ""
	}
...
func graphPID(graph *ManagedGraph) int {
	if graph == nil {
		return 0
	}
	return graph.PID
...
func graphBoltPort(graph *ManagedGraph) int {
	if graph == nil {
		return 0
	}
...
func graphHTTPPort(graph *ManagedGraph) int {
	if graph == nil {
		return 0
	}
...
func graphDataDir(graph *ManagedGraph) string {
	if graph == nil {
		return ""
	}
...
func graphVersion(graph *ManagedGraph) string {
	if graph == nil {
		return ""
	}
...
func graphUsername(graph *ManagedGraph) string {
	if graph == nil {
		return ""
	}
...
func graphPassword(graph *ManagedGraph) string {
	if graph == nil {
		return ""
	}
...
func graphHealthyFromOwnerRecord(record eshulocal.OwnerRecord) bool {
	...
	return ProcessAlive(record.GraphPID) &&
		localGraphHTTPHealthy(address, record.GraphHTTPPort, GraphHealthTimeout) &&
		localGraphBoltHealthy(address, record.GraphBoltPort, GraphHealthTimeout)
}
