...
func TestEmbeddedLocalNornicDBRuntimeStartsHTTPAndBolt(t *testing.T) {
	...
	t.Cleanup(func() {
		ctx, cancel := context.WithTimeout(context.Background(), GraphShutdownTimeout)
		defer cancel()
	...
	for time.Now().Before(deadline) {
		if graphHTTPHealthy(localNornicDBBindAddress, httpPort, GraphHealthTimeout) &&
			graphBoltHealthy(localNornicDBBindAddress, boltPort, GraphHealthTimeout) {
			return
		}
...
func assertEmbeddedBoltDefaultDatabaseWriteAccepted(t *testing.T, boltPort int, credentials localGraphCredentials) {
	...
	session := driver.NewSession(ctx, neo4jdriver.SessionConfig{
		...
		DatabaseName: GraphDatabaseName,
	})
	defer func() {
		_ = session.Close(ctx)
	}()
	...
	if err != nil {
		t.Fatalf("Run() error = %v, want write access to %q", err, GraphDatabaseName)
	}
	if _, err := result.Consume(ctx); err != nil {
		t.Fatalf("Consume() error = %v, want write access to %q", err, GraphDatabaseName)
	}
}
