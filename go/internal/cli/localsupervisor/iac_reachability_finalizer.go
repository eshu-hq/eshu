...
func startLocalIaCReachabilityFinalizer(ctx context.Context, out io.Writer, dsn string, expectedProjectors int) (func() error, error) {
...
func runLocalIaCReachabilityFinalizer(
	...
	for {
		...
		if err != nil {
			_, _ = fmt.Fprintf(out, "warning: IaC reachability finalizer readiness check failed: %v\n", err)
		}
		if ready {
			start := time.Now()
			...
			_, _ = fmt.Fprintf(out, "IaC reachability rows materialized expected_projectors=%d duration=%s\n", expectedProjectors, time.Since(start).Round(time.Millisecond))
			return nil
		}
