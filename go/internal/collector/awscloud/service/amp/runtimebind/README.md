...
`internal/collector/awscloud/service/amp/runtimebind` registers the AMP scanner
...
`internal/collector/awscloud/service/amp` and its `awssdk` adapter.
...
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/service/amp` for the scanner struct.
- `internal/collector/awscloud/service/amp/awssdk` for the SDK adapter
  constructor.
