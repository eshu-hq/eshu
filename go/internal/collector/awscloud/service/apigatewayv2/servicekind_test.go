...
// joins/filters that key on the canonical "apigatewayv2".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceAPIGatewayV2 + "  "
	client := fakeClient{snapshot: sampleSnapshot()}
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceAPIGatewayV2; got != want {
