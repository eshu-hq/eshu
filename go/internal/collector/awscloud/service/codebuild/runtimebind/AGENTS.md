# AGENTS.md - services/codebuild/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - CodeBuild scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceCodeBuild`.
- Keep the redaction-key guard: return a typed error when
  `ScannerDeps.RedactionKey` is zero. CodeBuild redacts PLAINTEXT
  environment-variable values, so a missing key is a configuration error, not a
  silent fallback.
...
- Update the builder body only when the CodeBuild scanner constructor signature
