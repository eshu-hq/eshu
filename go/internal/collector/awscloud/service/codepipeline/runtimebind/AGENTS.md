# AGENTS.md - services/codepipeline/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - CodePipeline scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceCodePipeline`.
- Keep the redaction-key guard: return a typed error when
  `ScannerDeps.RedactionKey` is zero. CodePipeline redacts source-revision
  summaries, so a missing key is a configuration error, not a silent fallback.
...
- Update the builder body only when the CodePipeline scanner constructor
