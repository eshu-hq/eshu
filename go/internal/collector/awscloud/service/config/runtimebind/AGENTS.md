# AGENTS.md - services/config/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - AWS Config scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceConfig`.
- AWS Config is metadata-only. Keep the builder the minimal constructor form;
...
- Update the builder body only when the AWS Config scanner constructor signature
