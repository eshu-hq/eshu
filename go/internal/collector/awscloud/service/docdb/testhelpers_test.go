...
func testBoundary() awscloud.Boundary {
	return awscloud.Boundary{
		...
		GenerationID:        "aws:123456789012:us-east-1:docdb:1",
...
func (c fakeClient) ListEventSubscriptions(context.Context) ([]EventSubscription, error) {
	return c.eventSubscriptions, nil
}
	...
	for key := range attributes {
		for _, forbidden := range forbiddenSubstrings() {
			if contains(key, forbidden) {
				t.Fatalf("attribute %q contains forbidden substring %q; DocDB scanner must stay metadata-only", key, forbidden)
			}
		}
	}
}
