...
// joins/filters that key on the canonical "eks".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceEKS + "  "
	client := fakeClient{
		clusters: []Cluster{{
			ARN:    "arn:aws:eks:us-east-1:123456789012:cluster/padded",
			Name:   "padded",
			Status: "ACTIVE",
		}},
	}
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceEKS; got != want {
