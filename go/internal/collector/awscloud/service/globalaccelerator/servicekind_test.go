...
// joins/filters that key on the canonical "globalaccelerator".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceGlobalAccelerator + "  "
	client := fakeClient{accelerators: []Accelerator{fullTopologyAccelerator()}}
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceGlobalAccelerator; got != want {
