# AGENTS.md - services/inspector2/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Inspector v2 scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceInspector2`.
- Inspector v2 is metadata-only. Keep the builder the minimal constructor form;
  do not add `RedactionKey` validation or `Checkpoints` usage.
...
- Update the builder body only when the Inspector v2 scanner constructor
