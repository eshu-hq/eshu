# AGENTS.md - services/kinesis/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Kinesis scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceKinesis`.
- Do not load AWS configuration or build SDK clients at init time. Builders
  construct clients per claim from `ScannerDeps`.
- Do not pass a `RedactionKey`. The Kinesis scanner persists no redacted
  fields; it omits secret-bearing destination material entirely.
...
- Update the builder body only when the Kinesis scanner constructor signature
