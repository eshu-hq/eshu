...
`internal/collector/awscloud/service/mq/runtimebind` registers the Amazon MQ
...
`internal/collector/awscloud/service/mq` and its `awssdk` adapter.
...
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/service/mq` for the scanner struct.
- `internal/collector/awscloud/service/mq/awssdk` for the SDK adapter
  constructor.
