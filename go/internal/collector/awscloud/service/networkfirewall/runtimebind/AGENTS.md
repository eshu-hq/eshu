# AGENTS.md - services/networkfirewall/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Network Firewall scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceNetworkFirewall`.
- Do not set `RequiresRedactionKey`. The scanner needs no redaction key because
  it never reads sensitive bodies.
...
- Update the builder body only when the Network Firewall scanner constructor
