...
func TestRoute53RecoveryControlConfigRuntimeBindRegisters(t *testing.T) {
	build, ok := awsruntime.LookupBuilder(awscloud.ServiceRoute53RecoveryControlConfig)
	if !ok {
		t.Fatalf("LookupBuilder(%q) ok = false, want true", awscloud.ServiceRoute53RecoveryControlConfig)
	}
