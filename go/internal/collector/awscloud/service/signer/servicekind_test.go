...
// joins/filters that key on the canonical "signer".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceSigner + "  "
	client := fakeClient{snapshot: Snapshot{
		Platforms: []SigningPlatform{{PlatformID: testPlatformID, DisplayName: "AWS Lambda"}},
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceSigner; got != want {
