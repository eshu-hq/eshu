# AGENTS.md - services/verifiedpermissions/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Verified Permissions scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with
  `awscloud.ServiceVerifiedPermissions`.
...
- Update the builder body only when the Verified Permissions scanner
  constructor signature changes. Keep the change scoped to the constructor call.
