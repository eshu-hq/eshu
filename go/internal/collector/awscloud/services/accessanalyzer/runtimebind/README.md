# AWS Access Analyzer Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/accessanalyzer/runtimebind` registers the
Access Analyzer scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the Access Analyzer scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceAccessAnalyzer` to the Access Analyzer scanner builder. It does not own
AWS API calls, Access Analyzer domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/accessanalyzer` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceAccessAnalyzer` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/accessanalyzer` for the scanner struct.
- `internal/collector/awscloud/services/accessanalyzer/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The Access Analyzer scanner and its
...
- `../README.md` for the Access Analyzer scanner contract.
