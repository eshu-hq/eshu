...
`internal/collector/awscloud/services/amp/runtimebind` registers the AMP scanner
...
`awscloud.ServiceAMP` to the AMP scanner builder. It does not own AWS API calls,
AMP domain types, redaction policy, or fact emission. Those belong to
`internal/collector/awscloud/services/amp` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceAMP` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/amp` for the scanner struct.
- `internal/collector/awscloud/services/amp/awssdk` for the SDK adapter
  constructor.
