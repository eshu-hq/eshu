# AWS Amplify Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/amplify/runtimebind` registers the Amplify
scanner with the awsruntime registry from a package `init()`. Importing this
package for its blank side effect is the only way a runtime brings the Amplify
scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceAmplify` to the Amplify scanner builder. It does not own AWS API
calls, Amplify domain types, or fact emission. Those belong to
`internal/collector/awscloud/services/amplify` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceAmplify` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/amplify` for the scanner struct.
- `internal/collector/awscloud/services/amplify/awssdk` for the SDK adapter
  constructor.

## Telemetry

This binding emits no telemetry of its own. The Amplify scanner and its SDK
...
- The builder needs no redaction key: the SDK adapter drops every secret-bearing
  field at the boundary, so the registration leaves `RequiresRedactionKey` unset.
