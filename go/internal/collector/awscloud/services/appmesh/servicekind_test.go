...
// joins/filters that key on the canonical "appmesh".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceAppMesh + "  "
	client := fakeClient{meshes: []Mesh{{
		ARN:    meshARN,
		Name:   "checkout",
		Status: "ACTIVE",
	}}}

	envelopes, err := (Scanner{Client: client, RedactionKey: testRedactionKey(t)}).Scan(context.Background(), boundary)
	if err != nil {
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceAppMesh; got != want {
