# AGENTS.md - services/apprunner/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - App Runner scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceAppRunner`.
- Leave `RequiresRedactionKey` unset. App Runner drops environment-variable
  values rather than redacting them; the builder needs no redaction-key guard.
...
- Update the builder body only when the App Runner scanner constructor signature
...
- Do not introduce side effects (network, file IO, config parsing) at package
  load time.
- Do not add a redaction-key requirement.
