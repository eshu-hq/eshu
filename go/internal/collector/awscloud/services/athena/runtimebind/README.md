# AWS Athena Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/athena/runtimebind` registers the
Athena scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the Athena scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceAthena` to the Athena scanner builder. It does not own
AWS API calls, Athena domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/athena` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceAthena` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/athena` for the scanner struct.
- `internal/collector/awscloud/services/athena/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The Athena scanner and its
...
- `../README.md` for the Athena scanner contract.
