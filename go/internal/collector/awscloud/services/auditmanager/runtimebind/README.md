...
`internal/collector/awscloud/services/auditmanager/runtimebind` registers the
Audit Manager scanner with the awsruntime registry from a package `init()`.
...
`awscloud.ServiceAuditManager` to the Audit Manager scanner builder. It does not
own AWS API calls, Audit Manager domain types, or fact emission. Those belong to
`internal/collector/awscloud/services/auditmanager` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceAuditManager` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/auditmanager` for the scanner struct.
- `internal/collector/awscloud/services/auditmanager/awssdk` for the SDK adapter
  constructor.
