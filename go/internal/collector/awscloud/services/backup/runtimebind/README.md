...
`internal/collector/awscloud/services/backup/runtimebind` registers the AWS
Backup scanner with the awsruntime registry from a package `init()`.
...
`awscloud.ServiceBackup` to the AWS Backup scanner builder. It does not own
AWS API calls, AWS Backup domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/backup` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceBackup` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/backup` for the scanner struct.
- `internal/collector/awscloud/services/backup/awssdk` for the SDK adapter
  constructor.
