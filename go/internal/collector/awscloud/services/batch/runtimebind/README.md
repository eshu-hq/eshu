...
`internal/collector/awscloud/services/batch/runtimebind` registers the AWS
Batch scanner with the awsruntime registry from a package `init()`. Importing
...
`awscloud.ServiceBatch` to the Batch scanner builder. It does not own AWS API
calls, Batch domain types, redaction policy, or fact emission. Those belong to
`internal/collector/awscloud/services/batch` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceBatch` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/batch` for the scanner struct.
- `internal/collector/awscloud/services/batch/awssdk` for the SDK adapter
  constructor.
