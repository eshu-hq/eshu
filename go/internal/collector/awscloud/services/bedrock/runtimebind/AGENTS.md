# AGENTS.md - services/bedrock/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Bedrock scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceBedrock`.
- Keep the builder body a plain constructor call. Bedrock takes no optional
...
- Update the builder body only when the Bedrock scanner constructor signature
