...
`internal/collector/awscloud/services/cloudformation/runtimebind` registers the
CloudFormation scanner with the awsruntime registry from a package `init()`.
...
`awscloud.ServiceCloudFormation` to the CloudFormation scanner builder. It does
not own AWS API calls, CloudFormation domain types, redaction policy, or fact
emission. Those belong to
`internal/collector/awscloud/services/cloudformation` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceCloudFormation` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/cloudformation` for the scanner struct.
- `internal/collector/awscloud/services/cloudformation/awssdk` for the SDK
  adapter constructor.
