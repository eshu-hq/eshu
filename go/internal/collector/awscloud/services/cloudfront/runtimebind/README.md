# AWS CloudFront Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/cloudfront/runtimebind` registers the
CloudFront scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the CloudFront scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceCloudFront` to the CloudFront scanner builder. It does not own
AWS API calls, CloudFront domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/cloudfront` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceCloudFront` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/cloudfront` for the scanner struct.
- `internal/collector/awscloud/services/cloudfront/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The CloudFront scanner and its
...
- `../README.md` for the CloudFront scanner contract.
