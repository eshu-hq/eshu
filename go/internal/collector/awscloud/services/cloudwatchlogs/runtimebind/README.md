# AWS CloudWatch Logs Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/cloudwatchlogs/runtimebind` registers the
CloudWatch Logs scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the CloudWatch Logs scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceCloudWatchLogs` to the CloudWatch Logs scanner builder. It does not own
AWS API calls, CloudWatch Logs domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/cloudwatchlogs` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceCloudWatchLogs` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/cloudwatchlogs` for the scanner struct.
- `internal/collector/awscloud/services/cloudwatchlogs/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The CloudWatch Logs scanner and its
...
- `../README.md` for the CloudWatch Logs scanner contract.
