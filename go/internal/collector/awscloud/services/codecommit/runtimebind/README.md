# AWS CodeCommit Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/codecommit/runtimebind` registers the
CodeCommit scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the CodeCommit scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceCodeCommit` to the CodeCommit scanner builder. It does not own
AWS API calls, CodeCommit domain types, or fact emission. Those belong to
`internal/collector/awscloud/services/codecommit` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceCodeCommit` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/codecommit` for the scanner struct.
- `internal/collector/awscloud/services/codecommit/awssdk` for the SDK adapter
  constructor.

## Telemetry

This binding emits no telemetry of its own. The CodeCommit scanner and its SDK
...
- CodeCommit needs no redaction key (it persists no secret-shaped field), so the
