...
// joins/filters that key on the canonical "codecommit".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceCodeCommit + "  "
	client := fakeClient{repositories: []Repository{fullRepository()}}
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceCodeCommit; got != want {
