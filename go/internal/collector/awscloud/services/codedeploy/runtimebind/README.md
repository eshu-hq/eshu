# AWS CodeDeploy Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/codedeploy/runtimebind` registers the
CodeDeploy scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the CodeDeploy scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceCodeDeploy` to the CodeDeploy scanner builder. It does not own
AWS API calls, CodeDeploy domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/codedeploy` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceCodeDeploy` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/codedeploy` for the scanner struct.
- `internal/collector/awscloud/services/codedeploy/awssdk` for the SDK adapter
  constructor.

## Telemetry

This binding emits no telemetry of its own. The CodeDeploy scanner and its SDK
...
- The builder returns a typed error when `ScannerDeps.RedactionKey` is zero
  because CodeDeploy redacts on-premises instance tag values. The command
...
- `../README.md` for the CodeDeploy scanner contract.
