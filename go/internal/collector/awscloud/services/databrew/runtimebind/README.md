# AWS Glue DataBrew Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/databrew/runtimebind` registers the
...
belong to `internal/collector/awscloud/services/databrew` and its `awssdk`
adapter.
...
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/databrew` for the scanner struct.
- `internal/collector/awscloud/services/databrew/awssdk` for the SDK adapter
  constructor.
