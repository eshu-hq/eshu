# AWS DataSync Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/datasync/runtimebind` registers the
DataSync scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the DataSync scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceDataSync` to the DataSync scanner builder. It does not own
AWS API calls, DataSync domain types, partition policy, or fact emission.
Those belong to `internal/collector/awscloud/services/datasync` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceDataSync` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/datasync` for the scanner struct.
- `internal/collector/awscloud/services/datasync/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The DataSync scanner and its
...
- `../README.md` for the DataSync scanner contract.
