# AGENTS.md - services/dax/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - DAX scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceDAX`.
- Do not set `RequiresRedactionKey`. DAX never persists secret material, so it
  carries no redaction-key requirement.
...
- Update the builder body only when the DAX scanner constructor signature
