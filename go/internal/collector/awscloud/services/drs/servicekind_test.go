...
// joins/filters that key on the canonical "drs".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceDRS + "  "
	client := fakeClient{snapshot: Snapshot{SourceServers: []SourceServer{{
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceDRS; got != want {
