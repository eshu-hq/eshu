# AGENTS.md - services/ds/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Directory Service scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceDirectoryService`.
- Leave `RequiresRedactionKey` unset: the Directory Service scanner never reads
  directory admin passwords or the RADIUS shared secret, so it needs no
  redaction key.
...
- Update the builder body only when the Directory Service scanner constructor
