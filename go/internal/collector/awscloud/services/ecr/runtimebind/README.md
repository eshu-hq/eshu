# AWS ECR Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/ecr/runtimebind` registers the
ECR scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the ECR scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceECR` to the ECR scanner builder. It does not own
AWS API calls, ECR domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/ecr` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceECR` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/ecr` for the scanner struct.
- `internal/collector/awscloud/services/ecr/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The ECR scanner and its
...
- `../README.md` for the ECR scanner contract.
