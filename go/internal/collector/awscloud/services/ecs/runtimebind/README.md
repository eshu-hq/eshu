# AWS ECS Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/ecs/runtimebind` registers the
ECS scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the ECS scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceECS` to the ECS scanner builder. It does not own
AWS API calls, ECS domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/ecs` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceECS` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/ecs` for the scanner struct.
- `internal/collector/awscloud/services/ecs/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The ECS scanner and its
...
- `../README.md` for the ECS scanner contract.
