# AGENTS.md - services/elasticbeanstalk/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Elastic Beanstalk scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceElasticBeanstalk`.
- Keep `RequiresRedactionKey: true`. The config command derives the
  `ESHU_AWS_REDACTION_KEY` requirement from the registry, not from a switch in
  `config.go`. The builder must still fail closed when `d.RedactionKey.IsZero()`.
- Do not load AWS configuration or build SDK clients at init time. Builders
  construct clients per claim from `ScannerDeps`.
- Do not validate or transform claims here. Validation belongs to awsruntime and
  the scanner. The builder body stays a constructor call plus the redaction-key
  guard.
- Do not import anything else from `internal/collector/awscloud/services`.
  Cross-service knowledge belongs upstream.

## Common Changes

- Update the builder body only when the Elastic Beanstalk scanner constructor
  signature changes. Keep the change scoped to the constructor call.

## What Not To Change Without An ADR

- Do not move the `Register` call out of `init()`.
- Do not drop `RequiresRedactionKey` while the scanner still redacts
  option-setting values.
- Do not introduce side effects (network, file IO, config parsing) at package
  load time.
