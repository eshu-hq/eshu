...
`internal/collector/awscloud/services/elbv2/runtimebind` registers the
ELBv2 scanner with the awsruntime registry from a package `init()`.
...
`awscloud.ServiceELBv2` to the ELBv2 scanner builder. It does not own
AWS API calls, ELBv2 domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/elbv2` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceELBv2` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/elbv2` for the scanner struct.
- `internal/collector/awscloud/services/elbv2/awssdk` for the SDK
  adapter constructor.
