...
`internal/collector/awscloud/services/fsx/runtimebind` registers the FSx scanner
...
`awscloud.ServiceFSx` to the FSx scanner builder. It does not own AWS API calls,
FSx domain types, or fact emission. Those belong to
`internal/collector/awscloud/services/fsx` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceFSx` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/fsx` for the scanner struct.
- `internal/collector/awscloud/services/fsx/awssdk` for the SDK adapter
  constructor.
