...
`internal/collector/awscloud/services/keyspaces/runtimebind` registers the
Amazon Keyspaces scanner with the awsruntime registry from a package `init()`.
...
`awscloud.ServiceKeyspaces` to the Keyspaces scanner builder. It does not own
AWS API calls, Keyspaces domain types, or fact emission. Those belong to
`internal/collector/awscloud/services/keyspaces` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceKeyspaces` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/keyspaces` for the scanner struct.
- `internal/collector/awscloud/services/keyspaces/awssdk` for the SDK adapter
  constructor.
