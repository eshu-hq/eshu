...
`internal/collector/awscloud/services/kinesis/runtimebind` registers the
Kinesis scanner with the awsruntime registry from a package `init()`.
...
`awscloud.ServiceKinesis` to the Kinesis scanner builder. It does not own AWS
API calls, Kinesis domain types, or fact emission. Those belong to
`internal/collector/awscloud/services/kinesis` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceKinesis` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/kinesis` for the scanner struct.
- `internal/collector/awscloud/services/kinesis/awssdk` for the SDK adapter
  constructor.
