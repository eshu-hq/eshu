...
`internal/collector/awscloud/services/kms/runtimebind` registers the KMS
scanner with the awsruntime registry from a package `init()`. Importing
...
`awscloud.ServiceKMS` to the KMS scanner builder. It does not own AWS
API calls, KMS domain types, redaction policy, or fact emission. Those
belong to `internal/collector/awscloud/services/kms` and its `awssdk`
adapter.
...
- `internal/collector/awscloud` for the `ServiceKMS` constant.
- `internal/collector/awscloud/awsruntime` for `Register`,
  `ScannerDeps`, and `ScannerRegistration`.
- `internal/collector/awscloud/services/kms` for the scanner struct.
- `internal/collector/awscloud/services/kms/awssdk` for the SDK adapter
  constructor.
