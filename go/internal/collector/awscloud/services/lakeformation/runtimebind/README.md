# AWS Lake Formation Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/lakeformation/runtimebind` registers the
Lake Formation scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the Lake Formation scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceLakeFormation` to the Lake Formation scanner builder. It does
not own AWS API calls, Lake Formation domain types, data-sensitivity policy, or
fact emission. Those belong to
`internal/collector/awscloud/services/lakeformation` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceLakeFormation` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/lakeformation` for the scanner struct.
- `internal/collector/awscloud/services/lakeformation/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The Lake Formation scanner and its
...
- `../README.md` for the Lake Formation scanner contract.
