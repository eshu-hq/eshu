# AWS Lambda Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/lambda/runtimebind` registers the
Lambda scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the Lambda scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceLambda` to the Lambda scanner builder. It does not own
AWS API calls, Lambda domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/lambda` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceLambda` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/lambda` for the scanner struct.
- `internal/collector/awscloud/services/lambda/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The Lambda scanner and its
...
- `../README.md` for the Lambda scanner contract.
