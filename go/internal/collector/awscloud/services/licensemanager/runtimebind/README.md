...
`internal/collector/awscloud/services/licensemanager/runtimebind` registers the
License Manager scanner with the awsruntime registry from a package `init()`.
...
`awscloud.ServiceLicenseManager` to the License Manager scanner builder. It does
not own AWS API calls, License Manager domain types, redaction policy, or fact
emission. Those belong to `internal/collector/awscloud/services/licensemanager`
...
- `internal/collector/awscloud` for the `ServiceLicenseManager` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/licensemanager` for the scanner struct.
- `internal/collector/awscloud/services/licensemanager/awssdk` for the SDK
  adapter constructor.
