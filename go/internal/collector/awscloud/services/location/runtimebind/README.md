...
`internal/collector/awscloud/services/location/runtimebind` registers the
Location Service scanner with the awsruntime registry from a package `init()`.
...
`awscloud.ServiceLocation` to the Location Service scanner builder. It does not
own AWS API calls, Location Service domain types, redaction policy, or fact
emission. Those belong to
`internal/collector/awscloud/services/location` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceLocation` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/location` for the scanner struct.
- `internal/collector/awscloud/services/location/awssdk` for the SDK adapter
  constructor.
