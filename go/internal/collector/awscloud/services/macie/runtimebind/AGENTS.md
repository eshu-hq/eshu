# AGENTS.md - services/macie/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Macie scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceMacie`.
- Macie is metadata-only. Keep the builder the minimal constructor form; do not
  add `RedactionKey` validation or `Checkpoints` usage.
...
- Update the builder body only when the Macie scanner constructor signature
