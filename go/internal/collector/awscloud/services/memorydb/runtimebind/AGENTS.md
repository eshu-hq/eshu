# AGENTS.md - services/memorydb/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - MemoryDB scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceMemoryDB`.
- Do not set `RequiresRedactionKey`. MemoryDB never persists password material,
  so it carries no redaction-key requirement, matching ElastiCache.
...
- Update the builder body only when the MemoryDB scanner constructor signature
