# AWS MemoryDB Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/memorydb/runtimebind` registers the
...
belong to `internal/collector/awscloud/services/memorydb` and its `awssdk`
adapter.
...
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/memorydb` for the scanner struct.
- `internal/collector/awscloud/services/memorydb/awssdk` for the SDK adapter
  constructor.
...
- The registration does not set `RequiresRedactionKey`. MemoryDB drops password
  material by not mapping it (no `ESHU_AWS_REDACTION_KEY` requirement), matching
  the ElastiCache binding.
