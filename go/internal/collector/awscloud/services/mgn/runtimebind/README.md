# AWS Application Migration Service (MGN) Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/mgn/runtimebind` registers the MGN scanner
...
`internal/collector/awscloud/services/mgn` and its `awssdk` adapter.
...
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/mgn` for the scanner struct.
- `internal/collector/awscloud/services/mgn/awssdk` for the SDK adapter
  constructor.
