# AWS Organizations Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/organizations/runtimebind` registers the
Organizations scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the Organizations scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceOrganizations` to the Organizations scanner builder. It does not own
AWS API calls, Organizations domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/organizations` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceOrganizations` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/organizations` for the scanner struct.
- `internal/collector/awscloud/services/organizations/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The Organizations scanner and its
...
- `../README.md` for the Organizations scanner contract.
