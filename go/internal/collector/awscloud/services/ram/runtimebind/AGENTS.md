# AGENTS.md - services/ram/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - RAM scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceRAM`.
- Do not set `RequiresRedactionKey`. The RAM scanner redacts nothing; adding the
  flag would force an unnecessary `ESHU_AWS_REDACTION_KEY` pre-flight.
...
- Update the builder body only when the RAM scanner constructor signature
