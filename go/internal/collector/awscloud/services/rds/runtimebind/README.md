# AWS RDS Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/rds/runtimebind` registers the
RDS scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the RDS scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceRDS` to the RDS scanner builder. It does not own
AWS API calls, RDS domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/rds` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceRDS` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/rds` for the scanner struct.
- `internal/collector/awscloud/services/rds/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The RDS scanner and its
...
- `../README.md` for the RDS scanner contract.
