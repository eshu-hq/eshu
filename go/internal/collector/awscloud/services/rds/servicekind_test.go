...
// joins/filters that key on the canonical "rds".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceRDS + "  "
	client := fakeClient{instances: []DBInstance{{
		ARN:        "arn:aws:rds:us-east-1:123456789012:db:padded-writer",
		Identifier: "padded-writer",
		Engine:     "postgres",
		Status:     "available",
	}}}
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceRDS; got != want {
