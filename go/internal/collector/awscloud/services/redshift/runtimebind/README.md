# AWS Redshift Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/redshift/runtimebind` registers the
Redshift scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the Redshift scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceRedshift` to the Redshift scanner builder. It does not own
AWS API calls, Redshift domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/redshift` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceRedshift` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/redshift` for the scanner struct.
- `internal/collector/awscloud/services/redshift/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The Redshift scanner and its
...
- `../README.md` for the Redshift scanner contract.
