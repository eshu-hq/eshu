...
// joins/filters that key on the canonical "resourcegroups".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceResourceGroups + "  "
	client := fakeClient{groups: []Group{{
		ARN:       "arn:aws:resource-groups:us-east-1:123456789012:group/padded",
		Name:      "padded",
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceResourceGroups; got != want {
