# AGENTS.md - services/route53resolver/runtimebind guidance

## Read First

1. `README.md` - one-binding contract and ownership boundary.
2. `bind.go` - the actual registration.
3. `../README.md` - Route 53 Resolver scanner contract.
4. `../../../awsruntime/README.md` - awsruntime registry and runtime surface.

## Invariants

- Register exactly once from `init()` with `awscloud.ServiceRoute53Resolver`.
- Do not set `RequiresRedactionKey`. The scanner drops DNS Firewall domain list
  contents rather than redacting them, so no redaction key is needed. Adding
  the flag without a redaction path in the scanner would break the
  registry-derived redaction-key config test.
...
- Update the builder body only when the Route 53 Resolver scanner constructor
