# AWS S3 Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/s3/runtimebind` registers the
S3 scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the S3 scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceS3` to the S3 scanner builder. It does not own
AWS API calls, S3 domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/s3` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceS3` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/s3` for the scanner struct.
- `internal/collector/awscloud/services/s3/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The S3 scanner and its
...
- `../README.md` for the S3 scanner contract.
