# Amazon SES Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/ses/runtimebind` registers the SES (v2)
scanner with the awsruntime registry from a package `init()`. Importing this
package for its blank side effect is the only way a runtime brings the SES
scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceSES` to the SES scanner builder. It does not own AWS API calls,
SES domain types, redaction policy, or fact emission. Those belong to
`internal/collector/awscloud/services/ses` and its `awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceSES` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/ses` for the scanner struct.
- `internal/collector/awscloud/services/ses/awssdk` for the SDK adapter
  constructor.

## Telemetry

This binding emits no telemetry of its own. The SES scanner and its SDK adapter
...
- `../README.md` for the SES scanner contract.
