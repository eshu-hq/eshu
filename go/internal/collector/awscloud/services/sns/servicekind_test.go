...
// joins/filters that key on the canonical "sns".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceSNS + "  "
	client := fakeClient{topics: []Topic{{
		ARN:  "arn:aws:sns:us-east-1:123456789012:padded",
		Name: "padded",
	}}}
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceSNS; got != want {
