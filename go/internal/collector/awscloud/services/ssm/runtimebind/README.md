# AWS SSM Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/ssm/runtimebind` registers the
SSM scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the SSM scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceSSM` to the SSM scanner builder. It does not own
AWS API calls, SSM domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/ssm` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceSSM` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/ssm` for the scanner struct.
- `internal/collector/awscloud/services/ssm/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The SSM scanner and its
...
- `../README.md` for the SSM scanner contract.
