# AWS Step Functions Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/stepfunctions/runtimebind` registers the
Step Functions scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the Step Functions scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceStepFunctions` to the Step Functions scanner builder. It does not own
AWS API calls, Step Functions domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/stepfunctions` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceStepFunctions` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/stepfunctions` for the scanner struct.
- `internal/collector/awscloud/services/stepfunctions/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The Step Functions scanner and its
...
- `../README.md` for the Step Functions scanner contract.
