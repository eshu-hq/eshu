# AWS Transfer Family Scanner Runtime Binding

## Purpose

`internal/collector/awscloud/services/transfer/runtimebind` registers the
Transfer Family scanner with the awsruntime registry from a package `init()`.
Importing this package for its blank side effect is the only way a runtime
brings the Transfer scanner into the production registry.

## Ownership boundary

This package owns one thing: the `awsruntime.Register` call that wires
`awscloud.ServiceTransfer` to the Transfer scanner builder. It does not own
AWS API calls, Transfer domain types, exclusion policy, or fact emission.
Those belong to `internal/collector/awscloud/services/transfer` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceTransfer` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/awscloud/services/transfer` for the scanner struct.
- `internal/collector/awscloud/services/transfer/awssdk` for the SDK
  adapter constructor.

## Telemetry

This binding emits no telemetry of its own. The Transfer scanner and its
...
- `../README.md` for the Transfer scanner contract.
