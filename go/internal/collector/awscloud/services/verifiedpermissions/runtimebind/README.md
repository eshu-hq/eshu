...
`internal/collector/awscloud/services/verifiedpermissions/runtimebind` registers
the Verified Permissions scanner with the awsruntime registry from a package
...
`awscloud.ServiceVerifiedPermissions` to the Verified Permissions scanner
builder. It does not own AWS API calls, Verified Permissions domain types,
redaction policy, or fact emission. Those belong to
`internal/collector/awscloud/services/verifiedpermissions` and its `awssdk`
adapter.
...
- `internal/collector/awscloud` for the `ServiceVerifiedPermissions` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/verifiedpermissions` for the scanner
  struct.
- `internal/collector/awscloud/services/verifiedpermissions/awssdk` for the SDK
  adapter constructor.
