...
`internal/collector/awscloud/services/vpclattice/runtimebind` registers the VPC
Lattice scanner with the awsruntime registry from a package `init()`. Importing
...
`awscloud.ServiceVPCLattice` to the VPC Lattice scanner builder. It does not own
AWS API calls, VPC Lattice domain types, redaction policy, or fact emission.
Those belong to `internal/collector/awscloud/services/vpclattice` and its
`awssdk` adapter.
...
- `internal/collector/awscloud` for the `ServiceVPCLattice` constant.
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/services/vpclattice` for the scanner struct.
- `internal/collector/awscloud/services/vpclattice/awssdk` for the SDK adapter
  constructor.
