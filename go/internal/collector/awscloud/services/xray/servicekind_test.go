...
// joins/filters that key on the canonical "xray".
func TestScannerCanonicalizesPaddedServiceKind(t *testing.T) {
	boundary := testBoundary()
	boundary.ServiceKind = "  " + awscloud.ServiceXRay + "  "
	client := fakeClient{
		groups: []Group{{
			ARN:  "arn:aws:xray:us-east-1:123456789012:group/padded/abc123",
			Name: "padded",
		}},
	}
	...
	for _, envelope := range envelopes {
		if got, want := envelope.Payload["service_kind"], awscloud.ServiceXRay; got != want {
