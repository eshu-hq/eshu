...
- `Source`: implements `collector.Source.Next`, yielding one
  ...
  resume), and delegates emission to `azurecloud.Collector`.
...
remaining Azure fact families, API/MCP readback, and Helm/chart wiring are
deferred follow-ups (issue #1998) gated by the Azure cloud collector contract.

## Scope and generation

Each target maps to a stable scope ID
`azure:<tenant>:<scope_kind>:<provider_scope>:<resource_family>:<location>:resource_graph`
...
No-Regression Evidence: This is fixture-only scaffolding that adds a new
...
Why safe: the runtime is single-pass over a fixed target slice, holds no shared
mutable state across targets, and adds no concurrency primitive; the only
provider is fixture-backed under test and gated in production.

Observability Evidence: The runtime emits a bounded per-target span
...
scope_id, generation_id, scope_kind, source_lane, resource/warning/page/resume
counts, partial-scope and truncation flags, and duration. Fact emission and
