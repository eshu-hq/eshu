...
// reads Resource Graph pages through the azurecloud.PageProvider seam with
// $skipToken resume, and yields one collector.CollectedGeneration per target for
// atomic durable commit by the shared collector.Service.
//
...
// pre-parsed or file-backed Resource Graph pages with no network calls. A future
// PR replaces LiveProviderFactory with a real read-only adapter proven by its
// own credential, quota, throttle, and fixture gates.
//
// Reducer admission, the remaining Azure fact families, API and MCP readback,
// and Helm or chart wiring are deferred follow-ups gated by the Azure cloud
// collector contract; this package adds none of them. Credentials are referenced
// by name only in TargetConfig.CredentialRef, never inlined, so configuration is
// safe to log and persist, and no secret or credential name reaches telemetry.
