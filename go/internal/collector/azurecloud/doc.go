...
// and emits provider-specific azure_cloud_resource, azure_resource_change, and
// azure_collection_warning source facts for one bounded subscription,
// management-group, or tenant shard.
//
...
// resource-change actor fingerprinting, extension redaction, and bounded-label
// telemetry. It does not call live Azure Resource Graph or ARM APIs (the
// PageProvider seam is fed by fixtures in this slice), schedule workflow claims,
// choose credentials, commit facts, write graph truth, or answer queries.
//
...
// bodies. Resource-change facts carry changed property paths only, never
// before/after values or raw response bodies, and delete changes are tombstone
// candidates until reducer-owned evidence confirms final state. Telemetry labels
// are bounded enums only (collector kind, scope kind, source lane, operation,
// status class, fact kind, warning reason) and never carry ARM IDs,
// subscription or tenant IDs, resource group or resource names, locations, tags,
// KQL query text, URLs, or credential names.
package azurecloud
