...
// and azure_collection_warning source facts for one bounded subscription,
...
// bodies. Tag and identity values are stored as keyed fingerprints;
// relationship facts stay provenance-only; resource-change facts carry changed
// property paths only, never before/after values or raw response bodies, and
// delete changes are tombstone candidates until reducer-owned evidence confirms
// final state. Telemetry labels are bounded enums only (collector kind, scope
// kind, source lane, operation, status class, fact kind, warning reason) and
// never carry ARM IDs, subscription or tenant IDs, resource group or resource
// names, locations, tags, KQL query text, URLs, or credential names.
package azurecloud
