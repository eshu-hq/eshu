...
type TargetConfig struct {
	...
	MaxJobs             int
	MaxArtifacts        int
	// MaxDeployments bounds the GitHub Deployments API window (#5425 STEP 3;
	// see source_deployments.go).
	MaxDeployments int
}
...
func (s ClaimedSource) NextClaimed(
	...
	// #5425 STEP 3: deployment facts MUST land in this SAME generation as
	// the ci.run facts above -- see appendDeploymentEnvelopes' doc comment.
	envelopes, deploymentsTruncated, err := s.appendDeploymentEnvelopes(observeCtx, observeSpan, item, target, page, envelopes, observedAt)
	if err != nil {
		return collector.CollectedGeneration{}, false, err
	}
	...
	s.recordPartialGeneration(observeCtx, page, gapDetected, deploymentsTruncated)
