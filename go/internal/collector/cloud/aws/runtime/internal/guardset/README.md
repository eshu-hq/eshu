...
- `RuntimebindServiceDirs(servicesDir)` walks
  `services/<service>/bind/` directories and returns the sorted service
...
- `BindingsImportServices(bindingsFile)` parses `bindings.go` with `go/parser`
  ...
  `services/<service>/bind` blank imports. This is the set of scanners
