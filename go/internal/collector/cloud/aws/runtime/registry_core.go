...
// registrations happen from bind sub-packages so the runtime
// registry has no compile-time dependency on individual service packages.
type ScannerRegistration struct {
	// ServiceKind must equal an aws.ServiceXxx constant. The registry
	...
	// requirement only in its bind Register call. The builder still
...
// callers invoke Register from a service bind package init() so the
// runtime command surface stays additive when new scanners arrive.
//
...
func Register(reg ScannerRegistration) {
	if reg.ServiceKind == "" {
		panic("runtime.Register: ServiceKind is empty")
	}
	if reg.Build == nil {
		panic(fmt.Sprintf("runtime.Register: nil Build for service_kind %q", reg.ServiceKind))
	}
	registryMu.Lock()
	defer registryMu.Unlock()
	if _, exists := scannerRegistry[reg.ServiceKind]; exists {
		panic(fmt.Sprintf("runtime.Register: duplicate registration for service_kind %q", reg.ServiceKind))
	}
...
// list stays in lockstep with the bind registrations. The slice is
