...
`internal/collector/cloud/aws/service/accessanalyzer/bind` registers the
Access Analyzer scanner with the runtime registry from a package `init()`.
...
`aws.ServiceAccessAnalyzer` to the Access Analyzer scanner builder. It does not own
AWS API calls, Access Analyzer domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/accessanalyzer` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceAccessAnalyzer` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/accessanalyzer` for the scanner struct.
- `internal/collector/cloud/aws/service/accessanalyzer/sdk` for the SDK
  adapter constructor.
