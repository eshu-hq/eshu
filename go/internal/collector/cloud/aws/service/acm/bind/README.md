...
`internal/collector/cloud/aws/service/acm/bind` registers the ACM scanner
with the runtime registry from a package `init()`. Importing this package
...
`aws.ServiceACM` to the ACM scanner builder. It does not own AWS API
calls, ACM domain types, redaction policy, or fact emission. Those belong to
`internal/collector/cloud/aws/service/acm` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceACM` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/acm` for the scanner struct.
- `internal/collector/cloud/aws/service/acm/sdk` for the SDK adapter
  constructor.
