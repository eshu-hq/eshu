...
`internal/collector/cloud/aws/service/amplify/bind` registers the Amplify
...
`aws.ServiceAmplify` to the Amplify scanner builder. It does not own AWS API
calls, Amplify domain types, or fact emission. Those belong to
`internal/collector/cloud/aws/service/amplify` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceAmplify` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/amplify` for the scanner struct.
- `internal/collector/cloud/aws/service/amplify/sdk` for the SDK adapter
  constructor.
