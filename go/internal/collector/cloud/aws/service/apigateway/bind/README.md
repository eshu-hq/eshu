...
`internal/collector/cloud/aws/service/apigateway/bind` registers the
API Gateway scanner with the runtime registry from a package `init()`.
...
`aws.ServiceAPIGateway` to the API Gateway scanner builder. It does not own
AWS API calls, API Gateway domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/apigateway` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceAPIGateway` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/apigateway` for the scanner struct.
- `internal/collector/cloud/aws/service/apigateway/sdk` for the SDK
  adapter constructor.
