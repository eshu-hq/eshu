...
func testBoundary() aws.Boundary {
	return aws.Boundary{
		...
		Region:              "us-east-1",
		ServiceKind:         aws.ServiceAPIGatewayV2,
...
func TestScannerEmitsAPIGatewayV2Metadata(t *testing.T) {
	...
	api := resourceByType(t, envelopes, aws.ResourceTypeAPIGatewayV2API)
	...
	stage := resourceByType(t, envelopes, aws.ResourceTypeAPIGatewayStage)
	...
	route := resourceByType(t, envelopes, aws.ResourceTypeAPIGatewayV2Route)
	if got := attributesOf(t, route)["route_key"]; got != "POST /orders" && got != "$default" {
		t.Fatalf("route route_key = %#v", got)
	}

	resourceByType(t, envelopes, aws.ResourceTypeAPIGatewayV2Integration)
	resourceByType(t, envelopes, aws.ResourceTypeAPIGatewayV2Authorizer)
	resourceByType(t, envelopes, aws.ResourceTypeAPIGatewayV2VPCLink)
	resourceByType(t, envelopes, aws.ResourceTypeAPIGatewayDomainName)

	// API -> stage.
	apiStage := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2APIHasStage)
	...
	apiRoute := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2APIHasRoute)
	if got := payloadString(t, apiRoute, "source_resource_id"); got != "api-123" {
		t.Fatalf("api->route source = %q", got)
	}
	routeInt := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2RouteUsesIntegration)
	...
	lambdaEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2IntegrationTargetsLambda)
	...
	if got := payloadString(t, lambdaEdge, "target_type"); got != aws.ResourceTypeLambdaFunction {
		t.Fatalf("integration->lambda target_type = %q", got)
	}

	// integration -> HTTP endpoint.
	httpEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2IntegrationTargetsHTTP)
	...
	vpcEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2IntegrationUsesVPCLink)
	...
	poolEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2APIUsesUserPool)
	...
	if got := payloadString(t, poolEdge, "target_type"); got != aws.ResourceTypeCognitoUserPool {
		t.Fatalf("api->user-pool target_type = %q", got)
	}

	// Non-Cognito JWT issuer stays a jwt_issuer edge, not a dangling user pool.
	jwtEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2APIUsesJWTIssuer)
	...
	acmEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2DomainUsesACMCertificate)
	...
	// domain -> API mapping.
	mapEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2DomainMapsToAPI)
	...
	subnetEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2VPCLinkUsesSubnet)
	if got := payloadString(t, subnetEdge, "target_type"); got != aws.ResourceTypeEC2Subnet {
		t.Fatalf("vpc-link->subnet target_type = %q", got)
	}
	sgEdge := relationshipByType(t, envelopes, aws.RelationshipAPIGatewayV2VPCLinkUsesSecurityGroup)
