...
`internal/collector/cloud/aws/service/athena/bind` registers the
Athena scanner with the runtime registry from a package `init()`.
...
`aws.ServiceAthena` to the Athena scanner builder. It does not own
AWS API calls, Athena domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/athena` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceAthena` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/athena` for the scanner struct.
- `internal/collector/cloud/aws/service/athena/sdk` for the SDK
  adapter constructor.
