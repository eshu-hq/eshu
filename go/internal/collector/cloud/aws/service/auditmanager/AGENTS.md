# AGENTS.md - internal/collector/cloud/aws/service/auditmanager guidance
...
5. `sdk/README.md` - SDK adapter contract and the read-surface gate.
...
- Metadata-only. Never read or persist collected audit evidence, evidence finder
  ...
  assessment report URLs. The `sdk` adapter excludes every such read by
  construction; do not loosen it.
- Canonicalize `service_kind` with the merged `switch strings.TrimSpace(...)`
  case that writes `aws.ServiceAuditManager` back. The AST guard in
  `aws/servicekind_guard_test.go` fails the build otherwise.
- Every relationship sets a declared `aws.ResourceType*` (or allowlisted)
...
- Add a new Audit Manager metadata field by extending the scanner-owned type and
  the `sdk` mapper, writing a scanner or adapter test first. Persist only
