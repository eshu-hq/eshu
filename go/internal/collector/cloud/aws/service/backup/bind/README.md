...
`internal/collector/awscloud/service/backup/runtimebind` registers the AWS
...
Those belong to `internal/collector/awscloud/service/backup` and its
`awssdk` adapter.
...
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/service/backup` for the scanner struct.
- `internal/collector/awscloud/service/backup/awssdk` for the SDK adapter
  constructor.
