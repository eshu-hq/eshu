...
`internal/collector/cloud/aws/service/cloudfront/bind` registers the
CloudFront scanner with the runtime registry from a package `init()`.
...
`aws.ServiceCloudFront` to the CloudFront scanner builder. It does not own
AWS API calls, CloudFront domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/cloudfront` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceCloudFront` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/cloudfront` for the scanner struct.
- `internal/collector/cloud/aws/service/cloudfront/sdk` for the SDK
  adapter constructor.
