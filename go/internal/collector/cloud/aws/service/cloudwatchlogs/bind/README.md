...
`internal/collector/cloud/aws/service/cloudwatchlogs/bind` registers the
CloudWatch Logs scanner with the runtime registry from a package `init()`.
...
`aws.ServiceCloudWatchLogs` to the CloudWatch Logs scanner builder. It does not own
AWS API calls, CloudWatch Logs domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/cloudwatchlogs` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceCloudWatchLogs` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/cloudwatchlogs` for the scanner struct.
- `internal/collector/cloud/aws/service/cloudwatchlogs/sdk` for the SDK
  adapter constructor.
