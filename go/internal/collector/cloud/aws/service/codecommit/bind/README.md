...
`internal/collector/cloud/aws/service/codecommit/bind` registers the
CodeCommit scanner with the runtime registry from a package `init()`.
...
`aws.ServiceCodeCommit` to the CodeCommit scanner builder. It does not own
AWS API calls, CodeCommit domain types, or fact emission. Those belong to
`internal/collector/cloud/aws/service/codecommit` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceCodeCommit` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/codecommit` for the scanner struct.
- `internal/collector/cloud/aws/service/codecommit/sdk` for the SDK adapter
  constructor.
