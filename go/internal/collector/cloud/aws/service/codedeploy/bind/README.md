...
`internal/collector/cloud/aws/service/codedeploy/bind` registers the
CodeDeploy scanner with the runtime registry from a package `init()`.
...
`aws.ServiceCodeDeploy` to the CodeDeploy scanner builder. It does not own
AWS API calls, CodeDeploy domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/codedeploy` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceCodeDeploy` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/codedeploy` for the scanner struct.
- `internal/collector/cloud/aws/service/codedeploy/sdk` for the SDK adapter
  constructor.
