...
`internal/collector/awscloud/service/config/runtimebind` registers the AWS
...
to `internal/collector/awscloud/service/config` and its `awssdk` adapter.
...
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/service/config` for the scanner struct.
- `internal/collector/awscloud/service/config/awssdk` for the SDK adapter
  constructor.
