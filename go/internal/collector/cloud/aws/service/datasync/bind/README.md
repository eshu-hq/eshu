...
`internal/collector/cloud/aws/service/datasync/bind` registers the
DataSync scanner with the runtime registry from a package `init()`.
...
`aws.ServiceDataSync` to the DataSync scanner builder. It does not own
AWS API calls, DataSync domain types, partition policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/datasync` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceDataSync` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/datasync` for the scanner struct.
- `internal/collector/cloud/aws/service/datasync/sdk` for the SDK
  adapter constructor.
