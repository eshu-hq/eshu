...
`internal/collector/cloud/aws/service/datazone/bind` registers the
DataZone scanner with the runtime registry from a package `init()`. Importing
...
`aws.ServiceDatazone` to the DataZone scanner builder. It does not own AWS
API calls, DataZone domain types, redaction policy, or fact emission. Those
belong to `internal/collector/cloud/aws/service/datazone` and its `sdk`
adapter.
...
- `internal/collector/cloud/aws` for the `ServiceDatazone` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/datazone` for the scanner struct.
- `internal/collector/cloud/aws/service/datazone/sdk` for the SDK adapter
  constructor.
