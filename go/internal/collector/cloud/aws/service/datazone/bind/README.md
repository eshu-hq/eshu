...
`internal/collector/awscloud/service/datazone/runtimebind` registers the
...
belong to `internal/collector/awscloud/service/datazone` and its `awssdk`
adapter.
...
- `internal/collector/awscloud/awsruntime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/awscloud/service/datazone` for the scanner struct.
- `internal/collector/awscloud/service/datazone/awssdk` for the SDK adapter
  constructor.
