...
`internal/collector/cloud/aws/service/dax/bind` registers the DAX scanner
...
`aws.ServiceDAX` to the DAX scanner builder. It does not own AWS API calls,
DAX domain types, redaction policy, or fact emission. Those belong to
`internal/collector/cloud/aws/service/dax` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceDAX` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/dax` for the scanner struct.
- `internal/collector/cloud/aws/service/dax/sdk` for the SDK adapter
  constructor.
