...
`internal/collector/cloud/aws/service/directconnect/bind` registers the
Direct Connect scanner with the runtime registry from a package `init()`.
...
`aws.ServiceDirectConnect` to the Direct Connect scanner builder. It does
not own AWS API calls, Direct Connect domain types, or fact emission. Those
belong to `internal/collector/cloud/aws/service/directconnect` and its `sdk`
adapter.
...
- `internal/collector/cloud/aws` for the `ServiceDirectConnect` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/directconnect` for the scanner struct.
- `internal/collector/cloud/aws/service/directconnect/sdk` for the SDK
  adapter constructor.
