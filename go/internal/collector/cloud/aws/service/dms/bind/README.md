...
`internal/collector/cloud/aws/service/dms/bind` registers the DMS scanner
...
`aws.ServiceDMS` to the DMS scanner builder. It does not own AWS API calls,
DMS domain types, redaction policy, or fact emission. Those belong to
`internal/collector/cloud/aws/service/dms` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceDMS` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/dms` for the scanner struct.
- `internal/collector/cloud/aws/service/dms/sdk` for the SDK adapter
  constructor.
