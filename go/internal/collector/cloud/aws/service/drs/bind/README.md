...
`internal/collector/cloud/aws/service/drs/bind` registers the DRS scanner
...
`aws.ServiceDRS` to the DRS scanner builder. It does not own AWS API calls,
DRS domain types, redaction policy, or fact emission. Those belong to
`internal/collector/cloud/aws/service/drs` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceDRS` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/drs` for the scanner struct.
- `internal/collector/cloud/aws/service/drs/sdk` for the SDK adapter
  constructor.
