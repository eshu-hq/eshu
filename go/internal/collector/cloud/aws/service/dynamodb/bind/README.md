...
`internal/collector/cloud/aws/service/dynamodb/bind` registers the
DynamoDB scanner with the runtime registry from a package `init()`.
...
`aws.ServiceDynamoDB` to the DynamoDB scanner builder. It does not own
AWS API calls, DynamoDB domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/dynamodb` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceDynamoDB` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/dynamodb` for the scanner struct.
- `internal/collector/cloud/aws/service/dynamodb/sdk` for the SDK
  adapter constructor.
