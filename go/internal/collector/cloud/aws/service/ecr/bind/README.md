...
`internal/collector/cloud/aws/service/ecr/bind` registers the
ECR scanner with the runtime registry from a package `init()`.
...
`aws.ServiceECR` to the ECR scanner builder. It does not own
AWS API calls, ECR domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/ecr` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceECR` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/ecr` for the scanner struct.
- `internal/collector/cloud/aws/service/ecr/sdk` for the SDK
  adapter constructor.
