...
`internal/collector/cloud/aws/service/ecs/bind` registers the
ECS scanner with the runtime registry from a package `init()`.
...
`aws.ServiceECS` to the ECS scanner builder. It does not own
AWS API calls, ECS domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/ecs` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceECS` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/ecs` for the scanner struct.
- `internal/collector/cloud/aws/service/ecs/sdk` for the SDK
  adapter constructor.
