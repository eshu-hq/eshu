...
`internal/collector/cloud/aws/service/eks/bind` registers the
EKS scanner with the runtime registry from a package `init()`.
...
`aws.ServiceEKS` to the EKS scanner builder. It does not own
AWS API calls, EKS domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/eks` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceEKS` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/eks` for the scanner struct.
- `internal/collector/cloud/aws/service/eks/sdk` for the SDK
  adapter constructor.
