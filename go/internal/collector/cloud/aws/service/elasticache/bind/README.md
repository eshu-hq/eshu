...
`internal/collector/cloud/aws/service/elasticache/bind` registers the
ElastiCache scanner with the runtime registry from a package `init()`.
...
`aws.ServiceElastiCache` to the ElastiCache scanner builder. It does not own
AWS API calls, ElastiCache domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/elasticache` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceElastiCache` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/elasticache` for the scanner struct.
- `internal/collector/cloud/aws/service/elasticache/sdk` for the SDK
  adapter constructor.
