...
`internal/collector/cloud/aws/service/eventbridge/bind` registers the
EventBridge scanner with the runtime registry from a package `init()`.
...
`aws.ServiceEventBridge` to the EventBridge scanner builder. It does not own
AWS API calls, EventBridge domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/eventbridge` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceEventBridge` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/eventbridge` for the scanner struct.
- `internal/collector/cloud/aws/service/eventbridge/sdk` for the SDK
  adapter constructor.
