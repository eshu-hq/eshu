# AGENTS.md - service/firehose/bind guidance
...
- Update the builder body only when the Firehose scanner constructor
...
- Do not introduce side effects (network, file IO, config parsing) at
  package load time.
- Do not set `RequiresRedactionKey` without an ADR documenting a new
  secret-shaped Firehose field and its redaction policy.
