...
`internal/collector/cloud/aws/service/globalaccelerator/bind` registers
the Global Accelerator scanner with the runtime registry from a package
...
`aws.ServiceGlobalAccelerator` to the Global Accelerator scanner builder.
It does not own AWS API calls, Global Accelerator domain types, or fact
emission. Those belong to
`internal/collector/cloud/aws/service/globalaccelerator` and its `sdk`
adapter.
...
- `internal/collector/cloud/aws` for the `ServiceGlobalAccelerator` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/globalaccelerator` for the scanner
  struct.
- `internal/collector/cloud/aws/service/globalaccelerator/sdk` for the SDK
  adapter constructor.
