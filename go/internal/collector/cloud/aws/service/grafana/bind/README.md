...
`internal/collector/cloud/aws/service/grafana/bind` registers the Managed
Grafana scanner with the runtime registry from a package `init()`. Importing
...
`aws.ServiceGrafana` to the Grafana scanner builder. It does not own AWS API
calls, Grafana domain types, redaction policy, or fact emission. Those belong to
`internal/collector/cloud/aws/service/grafana` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceGrafana` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/grafana` for the scanner struct.
- `internal/collector/cloud/aws/service/grafana/sdk` for the SDK adapter
  constructor.
