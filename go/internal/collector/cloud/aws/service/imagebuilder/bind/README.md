...
`internal/collector/cloud/aws/service/imagebuilder/bind` registers the
Image Builder scanner with the runtime registry from a package `init()`.
...
`aws.ServiceImageBuilder` to the Image Builder scanner builder. It does not
own AWS API calls, Image Builder domain types, redaction policy, or fact
emission. Those belong to `internal/collector/cloud/aws/service/imagebuilder`
and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceImageBuilder` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/imagebuilder` for the scanner struct.
- `internal/collector/cloud/aws/service/imagebuilder/sdk` for the SDK adapter
  constructor.
