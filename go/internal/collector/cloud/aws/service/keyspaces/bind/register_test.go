...
func TestKeyspacesRuntimeBindRegisters(t *testing.T) {
	build, ok := runtime.LookupBuilder(aws.ServiceKeyspaces)
	if !ok {
		t.Fatalf("LookupBuilder(%q) ok = false, want true", aws.ServiceKeyspaces)
	}
	scanner, err := build(runtime.ScannerDeps{
		AWSConfig: awsv2.Config{Region: "us-east-1"},
		Boundary:  aws.Boundary{AccountID: "123456789012", Region: "us-east-1", ServiceKind: aws.ServiceKeyspaces},
	})
	if err != nil {
