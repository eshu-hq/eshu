...
`internal/collector/cloud/aws/service/lakeformation/bind` registers the
Lake Formation scanner with the runtime registry from a package `init()`.
...
`aws.ServiceLakeFormation` to the Lake Formation scanner builder. It does
not own AWS API calls, Lake Formation domain types, data-sensitivity policy, or
fact emission. Those belong to
`internal/collector/cloud/aws/service/lakeformation` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceLakeFormation` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/lakeformation` for the scanner struct.
- `internal/collector/cloud/aws/service/lakeformation/sdk` for the SDK
  adapter constructor.
