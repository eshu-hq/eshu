...
`internal/collector/cloud/aws/service/lambda/bind` registers the
Lambda scanner with the runtime registry from a package `init()`.
...
`aws.ServiceLambda` to the Lambda scanner builder. It does not own
AWS API calls, Lambda domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/lambda` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceLambda` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/lambda` for the scanner struct.
- `internal/collector/cloud/aws/service/lambda/sdk` for the SDK
  adapter constructor.
