...
`internal/collector/cloud/aws/service/location` owns the Amazon Location Service
...
The `sdk` adapter records Location Service API call counts, throttles, and
pagination spans.
...
`go test ./internal/collector/cloud/aws/service/location/...` covers the bounded
...
No-Regression Evidence: metadata-only control-plane scanner; new read path, no change to existing hot paths. `go test ./internal/collector/cloud/aws/service/location/...` green.
