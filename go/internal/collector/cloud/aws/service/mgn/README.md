...
`internal/collector/cloud/aws/service/mgn` owns the AWS Application Migration
...
The `sdk` adapter records MGN API call counts, throttles, and pagination
spans.
...
`go test ./internal/collector/cloud/aws/service/mgn/...` covers the bounded MGN
...
No-Regression Evidence: metadata-only control-plane scanner; new read path, no change to existing hot paths. `go test ./internal/collector/cloud/aws/service/mgn/...` green.
