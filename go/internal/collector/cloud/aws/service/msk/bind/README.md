...
`internal/collector/cloud/aws/service/msk/bind` registers the
MSK scanner with the runtime registry from a package `init()`.
...
`aws.ServiceMSK` to the MSK scanner builder. It does not own
AWS API calls, MSK domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/msk` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceMSK` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/msk` for the scanner struct.
- `internal/collector/cloud/aws/service/msk/sdk` for the SDK
  adapter constructor.
