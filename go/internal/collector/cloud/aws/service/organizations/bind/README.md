...
`internal/collector/cloud/aws/service/organizations/bind` registers the
Organizations scanner with the runtime registry from a package `init()`.
...
`aws.ServiceOrganizations` to the Organizations scanner builder. It does not own
AWS API calls, Organizations domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/organizations` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceOrganizations` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/organizations` for the scanner struct.
- `internal/collector/cloud/aws/service/organizations/sdk` for the SDK
  adapter constructor.
