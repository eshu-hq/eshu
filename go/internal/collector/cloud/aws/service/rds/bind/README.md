...
`internal/collector/cloud/aws/service/rds/bind` registers the
RDS scanner with the runtime registry from a package `init()`.
...
`aws.ServiceRDS` to the RDS scanner builder. It does not own
AWS API calls, RDS domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/rds` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceRDS` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/rds` for the scanner struct.
- `internal/collector/cloud/aws/service/rds/sdk` for the SDK
  adapter constructor.
