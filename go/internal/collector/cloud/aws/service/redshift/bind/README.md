...
`internal/collector/cloud/aws/service/redshift/bind` registers the
Redshift scanner with the runtime registry from a package `init()`.
...
`aws.ServiceRedshift` to the Redshift scanner builder. It does not own
AWS API calls, Redshift domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/redshift` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceRedshift` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/redshift` for the scanner struct.
- `internal/collector/cloud/aws/service/redshift/sdk` for the SDK
  adapter constructor.
