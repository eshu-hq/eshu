...
`internal/collector/cloud/aws/service/resourcegroups/bind` registers the
Resource Groups scanner with the runtime registry from a package `init()`.
...
`aws.ServiceResourceGroups` to the Resource Groups scanner builder. It does
not own AWS API calls, Resource Groups domain types, the membership classifier,
or fact emission. Those belong to
`internal/collector/cloud/aws/service/resourcegroups` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceResourceGroups` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/resourcegroups` for the scanner struct.
- `internal/collector/cloud/aws/service/resourcegroups/sdk` for the SDK
  adapter constructor.
