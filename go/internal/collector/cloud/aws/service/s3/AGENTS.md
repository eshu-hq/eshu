# AGENTS.md - internal/collector/cloud/aws/service/s3 guidance
...
- The scanner MAY emit the normalized, derived `aws_resource_policy_permission`
  ...
  owner-approved). The `sdk` adapter parses the policy transiently and hands
...
- Extend the derived `s3_bucket_posture` fact by adding a derived boolean or
  safe identifier to `aws.S3BucketPostureObservation` and the
...
- Extend `aws_resource_policy_permission` only with normalized/derived statement
  ...
  ARNs / types, public / cross-account). The derivation lives in the `sdk`
...
- Extend SDK pagination and optional-not-configured handling in the `sdk`
  adapter, not here.
