...
`internal/collector/cloud/aws/service/s3/bind` registers the
S3 scanner with the runtime registry from a package `init()`.
...
`aws.ServiceS3` to the S3 scanner builder. It does not own
AWS API calls, S3 domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/s3` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceS3` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/s3` for the scanner struct.
- `internal/collector/cloud/aws/service/s3/sdk` for the SDK
  adapter constructor.
