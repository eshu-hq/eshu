...
`internal/collector/cloud/aws/service/ses/bind` registers the SES (v2)
...
`aws.ServiceSES` to the SES scanner builder. It does not own AWS API calls,
SES domain types, redaction policy, or fact emission. Those belong to
`internal/collector/cloud/aws/service/ses` and its `sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceSES` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/ses` for the scanner struct.
- `internal/collector/cloud/aws/service/ses/sdk` for the SDK adapter
  constructor.
