...
`internal/collector/cloud/aws/service/sqs/bind` registers the
SQS scanner with the runtime registry from a package `init()`.
...
`aws.ServiceSQS` to the SQS scanner builder. It does not own
AWS API calls, SQS domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/sqs` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceSQS` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/sqs` for the scanner struct.
- `internal/collector/cloud/aws/service/sqs/sdk` for the SDK
  adapter constructor.
