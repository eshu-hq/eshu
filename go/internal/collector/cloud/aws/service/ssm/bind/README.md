...
`internal/collector/cloud/aws/service/ssm/bind` registers the
SSM scanner with the runtime registry from a package `init()`.
...
`aws.ServiceSSM` to the SSM scanner builder. It does not own
AWS API calls, SSM domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/ssm` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceSSM` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/ssm` for the scanner struct.
- `internal/collector/cloud/aws/service/ssm/sdk` for the SDK
  adapter constructor.
