...
`internal/collector/cloud/aws/service/stepfunctions/bind` registers the
Step Functions scanner with the runtime registry from a package `init()`.
...
`aws.ServiceStepFunctions` to the Step Functions scanner builder. It does not own
AWS API calls, Step Functions domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/stepfunctions` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceStepFunctions` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/stepfunctions` for the scanner struct.
- `internal/collector/cloud/aws/service/stepfunctions/sdk` for the SDK
  adapter constructor.
