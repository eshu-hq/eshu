...
`internal/collector/cloud/aws/service/timestream/bind` registers the
Timestream scanner with the runtime registry from a package `init()`.
...
`aws.ServiceTimestream` to the Timestream scanner builder. It does not own
AWS API calls, Timestream domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/timestream` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceTimestream` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/timestream` for the scanner struct.
- `internal/collector/cloud/aws/service/timestream/sdk` for the SDK adapter
  constructor.
