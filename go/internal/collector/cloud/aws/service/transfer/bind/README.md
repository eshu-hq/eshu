...
`internal/collector/cloud/aws/service/transfer/bind` registers the
Transfer Family scanner with the runtime registry from a package `init()`.
...
`aws.ServiceTransfer` to the Transfer scanner builder. It does not own
AWS API calls, Transfer domain types, exclusion policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/transfer` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceTransfer` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`,
  and `ScannerRegistration`.
- `internal/collector/cloud/aws/service/transfer` for the scanner struct.
- `internal/collector/cloud/aws/service/transfer/sdk` for the SDK
  adapter constructor.
