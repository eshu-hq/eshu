...
`internal/collector/cloud/aws/service/workspaces/bind` registers the
WorkSpaces scanner with the runtime registry from a package `init()`.
...
`aws.ServiceWorkSpaces` to the WorkSpaces scanner builder. It does not own
AWS API calls, WorkSpaces domain types, redaction policy, or fact emission.
Those belong to `internal/collector/cloud/aws/service/workspaces` and its
`sdk` adapter.
...
- `internal/collector/cloud/aws` for the `ServiceWorkSpaces` constant.
- `internal/collector/cloud/aws/runtime` for `Register`, `ScannerDeps`, and
  `ScannerRegistration`.
- `internal/collector/cloud/aws/service/workspaces` for the scanner struct.
- `internal/collector/cloud/aws/service/workspaces/sdk` for the SDK adapter
  constructor.
