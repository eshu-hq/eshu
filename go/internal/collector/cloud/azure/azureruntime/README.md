...
- `Source`: implements `collector.Source.Next`, yielding one
  ...
  `azure.Boundary`, derives durable scope and generation identity, reads
  Resource Graph pages through the `PageProvider` seam (with `$skipToken`
  resume), and delegates emission to `azure.Collector`. The default source
...
and bounded telemetry live in the parent `azure` package. Durable commit
...
```bash
cd go && go build ./...
cd go && go test ./internal/collector/cloud/azure/... ./cmd/collector-azure-cloud/... ./internal/facts/... -count=1
cd go && golangci-lint run ./internal/collector/cloud/azure/... ./cmd/collector-azure-cloud/...
```
...
`azure.Collector`, which still owns fact emission. ARM fallback enrichment
...
bounded by `azure.maxResourceGraphPages` (1000) so a malformed continuation
...
partial-scope counters reuse the parent `azure` bounded-label instruments
