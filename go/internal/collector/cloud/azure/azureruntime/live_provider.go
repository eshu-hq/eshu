...
type LiveProviderFactory struct {
	...
	// default, which projects only fields consumed by azure.ResourceRow.
	Query string
...
func (f LiveProviderFactory) PageProvider(
	...
	if lane == "" {
		lane = azure.SourceLaneResourceGraph
	}
	if lane != azure.SourceLaneResourceGraph {
		return nil, fmt.Errorf("azure live provider supports source lane %q only", azure.SourceLaneResourceGraph)
	}
	...
	for attempt := 0; ; attempt++ {
		...
		if err == nil {
			...
			if err := p.applyARMFallbacks(ctx, &page); err != nil {
				return azure.ResourceGraphPage{}, err
			}
			return page, nil
		}
		liveErr, ok := classifyLiveProviderError(err)
		if !ok {
			return azure.ResourceGraphPage{}, fmt.Errorf("query azure resource graph: %w", err)
		}
		switch liveErr.kind {
		case liveProviderErrorThrottled:
			p.mergeAccess(azure.ScopeAccess{
				Partial: true,
				Reason:  azure.WarningThrottled,
			...
			return azure.ResourceGraphPage{}, nil
		case liveProviderErrorSkipTokenExpired:
			p.mergeAccess(azure.ScopeAccess{
				Partial: true,
				Reason:  azure.WarningStale,
		...
		case liveProviderErrorUnsupported:
			p.mergeAccess(azure.ScopeAccess{
				Partial: true,
				Reason:  azure.WarningUnsupported,
...
func (p *liveResourceGraphProvider) ScopeAccess(context.Context) azure.ScopeAccess {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.access
}
...
func (p *liveResourceGraphProvider) request(skipToken string) LiveResourceGraphRequest {
	...
	case azure.ScopeKindSubscription:
		request.Subscriptions = []string{p.target.ProviderScopeID}
	case azure.ScopeKindManagementGroup:
...
func (p *liveResourceGraphProvider) mergeAccess(access azure.ScopeAccess) {
	if !access.Partial {
		return
	}
	p.mu.Lock()
...
func accessWarningPriority(reason string) int {
	if reason == azure.WarningFallbackSkipped {
		return 0
	}
	return 1
}
