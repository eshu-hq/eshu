...
func TestExplicitLiveProviderPaginatesBySkipToken(t *testing.T) {
	...
	provider, err := factory.PageProvider(context.Background(), azure.Boundary{}, testTarget())
	if err != nil {
		t.Fatalf("PageProvider() error = %v, want nil", err)
	}

	boundary := testBoundary()
	result, err := azure.NewCollector(provider, nil).Collect(context.Background(), boundary)
	if err != nil {
...
func TestExplicitLiveProviderThrottleBackoffIsBounded(t *testing.T) {
	client := &mockLiveResourceGraphClient{
		...
		responses: []LiveResourceGraphResponse{{Page: azure.ResourceGraphPage{
			TotalRecords: 1,
			Count:        1,
			Rows: []azure.ResourceRow{
				resourceRow(
	...
	result, err := azure.NewCollector(provider, nil).Collect(context.Background(), testBoundary())
	if err != nil {
		t.Fatalf("Collect() error = %v, want nil", err)
	}
...
func TestExplicitLiveProviderPermissionHiddenReportsScopeAccess(t *testing.T) {
	client := &mockLiveResourceGraphClient{responses: []LiveResourceGraphResponse{{Page: azure.ResourceGraphPage{
		TotalRecords: 0,
		Count:        0,
...
func TestExplicitLiveProviderRejectsMissingCredentialRef(t *testing.T) {
	...
	if _, err := factory.PageProvider(context.Background(), azure.Boundary{}, target); err == nil {
...
func testBoundary() azure.Boundary {
	return azure.Boundary{
		...
		SourceLane:          azure.SourceLaneResourceGraph,
