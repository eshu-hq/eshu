...
func testResourceChangesTarget() TargetConfig {
	target := testTarget()
	target.SourceLane = azure.SourceLaneResourceChanges
	return target
}
...
func resourceChangesFixture(t *testing.T) map[string]azure.ResourceChangesPage {
	t.Helper()
	...
	return map[string]azure.ResourceChangesPage{"": page}
}

func TestSourceYieldsResourceChangeGenerationFromFixturePages(t *testing.T) {
	provider := NewFixtureResourceChangesPageProvider(resourceChangesFixture(t), azure.ScopeAccess{})
	...
	if !strings.HasSuffix(collected.Scope.ScopeID, ":"+azure.SourceLaneResourceChanges) {
