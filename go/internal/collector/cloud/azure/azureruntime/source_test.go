...
func resourceRow(id, resourceType, location string) azure.ResourceRow {
	return azure.ResourceRow{
		ID:             id,
		Name:           "leaf",
		Type:           resourceType,
		TenantID:       "tenant-abc",
...
func twoPageFixture() map[string]azure.ResourceGraphPage {
	return map[string]azure.ResourceGraphPage{
		"": {
			TotalRecords: 2,
			Count:        1,
			SkipToken:    fixtureSkipToken,
			Rows: []azure.ResourceRow{
				resourceRow(
...
func testTarget() TargetConfig {
	return TargetConfig{
		TenantID:           "tenant-abc",
		ScopeKind:          azure.ScopeKindSubscription,
...
func newFixtureSource(t *testing.T, provider azure.PageProvider, targets ...TargetConfig) *Source {
	t.Helper()
	return &Source{
		Config: Config{
...
func TestSourceYieldsGenerationFromFixturePages(t *testing.T) {
	provider := NewFixturePageProvider(twoPageFixture(), azure.ScopeAccess{})
	...
	if collected.Scope.SourceSystem != azure.CollectorKind {
...
func TestSourceIdempotentReEmission(t *testing.T) {
	first := newFixtureSource(t, NewFixturePageProvider(twoPageFixture(), azure.ScopeAccess{}), testTarget())
	second := newFixtureSource(t, NewFixturePageProvider(twoPageFixture(), azure.ScopeAccess{}), testTarget())
...
func TestSourcePartialScopeAccounting(t *testing.T) {
	provider := NewFixturePageProvider(twoPageFixture(), azure.ScopeAccess{
		...
		Reason:              azure.WarningPermissionHidden,
	...
	if warnings[0].Payload["warning_kind"] != azure.WarningPermissionHidden {
		t.Fatalf("warning kind = %v", warnings[0].Payload["warning_kind"])
	}
	if warnings[0].Payload["outcome"] != azure.OutcomePartial {
...
func TestSourceRejectsInvalidConfig(t *testing.T) {
	src := &Source{
		...
		ProviderFactory: StaticFixtureFactory(NewFixturePageProvider(twoPageFixture(), azure.ScopeAccess{})),
	}
...
func TestLiveProviderFactoryIsGated(t *testing.T) {
	_, err := LiveProviderFactory{}.PageProvider(context.Background(), azure.Boundary{}, testTarget())
