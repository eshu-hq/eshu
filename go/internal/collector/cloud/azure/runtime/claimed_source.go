...
func (s *Source) targetForClaim(item workflow.WorkItem) (TargetConfig, string, error) {
	...
	if strings.TrimSpace(item.SourceSystem) != azure.CollectorKind {
		return TargetConfig{}, "", fmt.Errorf("claimed source_system %q must be %q", item.SourceSystem, azure.CollectorKind)
	}
