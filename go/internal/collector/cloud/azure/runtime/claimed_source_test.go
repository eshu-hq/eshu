...
func newClaimedFixtureSource(t *testing.T, provider azure.PageProvider, targets ...TargetConfig) *Source {
	t.Helper()
...
func TestSourceNextClaimedRejectsUnauthorizedScope(t *testing.T) {
	...
	src := newClaimedFixtureSource(t, NewFixturePageProvider(twoPageFixture(), azure.ScopeAccess{}), testTarget())
...
func TestSourceNextClaimedRecordsPartialClaimOnPartialScope(t *testing.T) {
	...
	if len(metrics.claims) != 1 || metrics.claims[0] != azure.ClaimStatusPartial {
		t.Fatalf("recorded claims = %v, want exactly [%q]", metrics.claims, azure.ClaimStatusPartial)
	}
}
...
func TestSourceNextClaimedRecordsSucceededClaimOnFullScope(t *testing.T) {
	...
	if len(metrics.claims) != 1 || metrics.claims[0] != azure.ClaimStatusSucceeded {
		t.Fatalf("recorded claims = %v, want exactly [%q]", metrics.claims, azure.ClaimStatusSucceeded)
	}
}
...
func TestSourceNextClaimedRejectsZeroRedactionKey(t *testing.T) {
	...
	src := newFixtureSource(t, NewFixturePageProvider(twoPageFixture(), azure.ScopeAccess{}), testTarget())
