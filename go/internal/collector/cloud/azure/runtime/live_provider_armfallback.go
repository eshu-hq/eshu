...
// allowlist and byte bound before the parent azure collector redacts and
...
func liveARMFallbackAccess(err error) azure.ScopeAccess {
	liveErr, ok := classifyLiveProviderError(err)
	if !ok {
		if errors.Is(err, context.DeadlineExceeded) {
			return azure.ScopeAccess{
				Partial: true,
