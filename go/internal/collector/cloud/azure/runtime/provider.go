...
// PageProviderFactory builds the azure.PageProvider that serves Resource
...
type PageProviderFactory interface {
	...
	PageProvider(ctx context.Context, boundary azure.Boundary, target TargetConfig) (azure.PageProvider, error)
}

// PageProviderFactoryFunc adapts a function into a PageProviderFactory.
type PageProviderFactoryFunc func(context.Context, azure.Boundary, TargetConfig) (azure.PageProvider, error)

// PageProvider calls the adapted function.
func (f PageProviderFactoryFunc) PageProvider(
	ctx context.Context,
	boundary azure.Boundary,
	target TargetConfig,
...
type FixturePageProvider struct {
	mu          sync.Mutex
	pages       map[string]azure.ResourceGraphPage
	changePages map[string]azure.ResourceChangesPage
...
func NewFixturePageProvider(
	...
	cloned := make(map[string]azure.ResourceGraphPage, len(pages))
...
func NewFixtureResourceChangesPageProvider(
	pages map[string]azure.ResourceChangesPage,
	access azure.ScopeAccess,
) *FixturePageProvider {
	cloned := make(map[string]azure.ResourceChangesPage, len(pages))
	for token, page := range pages {
		cloned[token] = page
	}
...
func NewFixturePageProviderFromFiles(
	...
	pages := make(map[string]azure.ResourceGraphPage, len(paths))
	token := ""
	for i, path := range paths {
		...
		page, err := azure.ParseResourceGraphPage(raw)
		if err != nil {
...
func NewFixtureResourceChangesPageProviderFromFiles(
	...
	pages := make(map[string]azure.ResourceChangesPage, len(paths))
	token := ""
	for i, path := range paths {
		raw, err := os.ReadFile(filepath.Clean(path))
		if err != nil {
		...
		page, err := azure.ParseResourceChangesPage(raw)
		if err != nil {
...
func (p *FixturePageProvider) NextPage(
	_ context.Context,
	skipToken string,
) (azure.ResourceGraphPage, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.calls = append(p.calls, skipToken)
	page, ok := p.pages[skipToken]
	if !ok {
		return azure.ResourceGraphPage{}, fmt.Errorf("no azure fixture page for skip token %q", skipToken)
	}
	return page, nil
}
...
func (p *FixturePageProvider) NextResourceChangesPage(
	_ context.Context,
	skipToken string,
) (azure.ResourceChangesPage, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.calls = append(p.calls, skipToken)
	page, ok := p.changePages[skipToken]
	if !ok {
		return azure.ResourceChangesPage{}, fmt.Errorf("no azure fixture resourcechanges page for skip token %q", skipToken)
	}
	return page, nil
}

// ScopeAccess reports the configured partial-scope access for the fixture.
func (p *FixturePageProvider) ScopeAccess(context.Context) azure.ScopeAccess {
	return p.access
}
...
// offline runs only.
func StaticFixtureFactory(provider azure.PageProvider) PageProviderFactory {
