...
func TestFixturePageProviderFromFilesRequiresPath(t *testing.T) {
	if _, err := NewFixturePageProviderFromFiles(azure.ScopeAccess{}); err == nil {
		t.Fatal("expected error with no fixture paths")
	}
}

func TestFixturePageProviderMissingTokenIsError(t *testing.T) {
	provider := NewFixturePageProvider(map[string]azure.ResourceGraphPage{
...
func TestStaticFixtureFactoryRejectsNilProvider(t *testing.T) {
	_, err := StaticFixtureFactory(nil).PageProvider(context.Background(), azure.Boundary{}, testTarget())
	if err == nil {
