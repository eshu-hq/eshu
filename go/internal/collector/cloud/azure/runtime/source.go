...
func (s *Source) scanTarget(
	...
	if s.Tracer != nil {
		...
		span.SetAttributes(
			telemetry.AttrCollectorKind(azure.CollectorKind),
	...
	result, err := azure.NewCollector(provider, s.Metrics, azure.WithRedactionKey(s.RedactionKey)).Collect(ctx, boundary)
	if err != nil {
		s.recordClaim(ctx, azure.ClaimStatusFailed)
	...
	if result.Partial || result.WarningCount > 0 {
		s.recordClaim(ctx, azure.ClaimStatusPartial)
	} else {
		s.recordClaim(ctx, azure.ClaimStatusSucceeded)
	}
