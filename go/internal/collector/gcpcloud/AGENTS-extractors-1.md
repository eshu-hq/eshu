# AGENTS-extractors-1.md — gcpcloud typed-depth extractor catalog (1 of 2)
...
file is part 1 of the per-asset-type extractor "Read First" catalog, covering
items 14–46. Part 2 is [`AGENTS-extractors-2.md`](AGENTS-extractors-2.md).

The two files are one continuous ascending list numbered from 14, so a
"#N above" cross-reference points to the lower-numbered entry — in part 1 when
N ≤ 46.

## Read First — typed-depth extractors (14–46)
...
45. `extractor_filestore_instance.go` - typed-depth extractor for
   ...
   typed depth).
46. `extractor_spanner_instance.go` - typed-depth extractor for
