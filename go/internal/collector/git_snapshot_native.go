...
func (s NativeRepositorySnapshotter) SnapshotRepository(
	...
	s.recordSnapshotStage(ctx, repoPath, telemetry.SnapshotStageMaterialize, materializeStartedAt,
		...
		// lockfile_cap_hit_count: number of files where the lockfile variable cap
		// (MaxLockfileVariableEntities) shed transitive dependency entries.
		// Non-zero values on the full corpus identify the repositories driving
		// content_entity volume from lockfile fan-out (#3676).
		slog.Int("lockfile_cap_hit_count", materialization.LockfileCapHits),
		...
		// source files with pathological symbol density.
