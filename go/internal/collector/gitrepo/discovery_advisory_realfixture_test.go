...
func TestDiscoveryAdvisoryClassifiesRealManifestAndConfigFixtures(t *testing.T) {
	...
	entities = append(entities, gitmodel.ContentEntitySnapshot{
...
// snapshots.
func snapshotsFromParsedPayload(t *testing.T, relPath string, payload map[string]any) []gitmodel.ContentEntitySnapshot {
	t.Helper()
	...
	var snapshots []gitmodel.ContentEntitySnapshot
...
func snapshotFromShapeEntity(relPath, label string, entity shape.Entity) gitmodel.ContentEntitySnapshot {
	return gitmodel.ContentEntitySnapshot{
...
func countVariableDependencyEntities(entities []gitmodel.ContentEntitySnapshot) int {
	count := 0
	for _, entity := range entities {
