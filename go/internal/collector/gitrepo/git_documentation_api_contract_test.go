...
`
	...
	if got, want := gitmodel.PayloadString(document.Payload, "format"), "openapi"; got != want {
		t.Fatalf("document format = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(document.Payload, "document_type"), "api_contract"; got != want {
	...
	for _, want := range []string{"listOrders", "Returns bounded order summaries", "Schema refs: #/components/schemas/OrderList"} {
		if content := gitmodel.PayloadString(operation.Payload, "content"); !strings.Contains(content, want) {
			t.Fatalf("operation section content missing %q: %q", want, content)
		}
	}
	if got := gitmodel.PayloadString(operation.Payload, "source_start_ref"); !strings.Contains(got, "/paths/~1orders/get") {
		t.Fatalf("operation source_start_ref = %q, want JSON pointer anchor", got)
	}
	if got := gitmodel.PayloadString(operation.Payload, "section_anchor"); got != "operation-get-orders" {
	...
	if got := gitmodel.PayloadString(schema.Payload, "content"); !strings.Contains(got, "A bounded order page.") {
	...
	if got, want := gitmodel.PayloadString(links[0].Payload, "target_uri"), "https://docs.example.test/payments"; got != want {
  ...
  total: Float
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			document := singleFact(t, envelopes, facts.DocumentationDocumentFactKind)
			if got := gitmodel.PayloadString(document.Payload, "format"); got != tt.format {
				t.Fatalf("document format = %q, want %q", got, tt.format)
			}
			section := findDocumentationSection(t, envelopes, tt.wantHeading)
			if got := gitmodel.PayloadString(section.Payload, "content"); !strings.Contains(got, tt.wantContent) {
...
func findDocumentationSection(t *testing.T, envelopes []facts.Envelope, heading string) facts.Envelope {
	...
	for _, section := range factsByKind(envelopes, facts.DocumentationSectionFactKind) {
		if gitmodel.PayloadString(section.Payload, "heading_text") == heading {
			return section
		}
	}
