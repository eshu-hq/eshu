...
func TestTextDiagramDocumentationFormatsAreDocumentationFiles(t *testing.T) {
	...
	metas := gitdocs.DocumentationFileMetasForPaths(repoPath, files, "commit")
...
`)
	...
	for _, document := range documentFacts {
		...
		if got, want := gitmodel.PayloadString(document.Payload, "document_type"), "diagram"; got != want {
	...
	for _, section := range sectionFacts {
		if got := gitmodel.PayloadString(section.Payload, "content_format"); got != "mermaid" && got != "d2" {
...
`)
	...
	writeCollectorTestFile(t, filepath.Join(repoPath, "docs", "truncated.mmd"),
		"flowchart LR\n  docs[Documentation] --> api[API]\n"+strings.Repeat("A", gitdocs.DocumentationMaxBodyBytes+1))
...
func TestTextDiagramDocumentationCanceledPreflightSuppressesContent(t *testing.T) {
	...
	envelopes, _ := gitdocs.GitDocumentationEnvelopesForContentFile(
		ctx,
		repoPath,
		repo,
		"scope:r",
...
func assertSectionContentContains(t *testing.T, sections []facts.Envelope, text string) {
	...
	for _, section := range sections {
		if strings.Contains(gitmodel.PayloadString(section.Payload, "content"), text) {
			return
		}
	}
