...
func TestStreamFactsEmitsDOCXDocumentationSections(t *testing.T) {
	...
	if got, want := gitmodel.PayloadString(document.Payload, "format"), "docx"; got != want {
		t.Fatalf("document format = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(document.Payload, "document_type"), "document"; got != want {
		t.Fatalf("document_type = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(document.Payload, "title"), "Migration Plan"; got != want {
	...
	if content := gitmodel.PayloadString(plan.Payload, "content"); !strings.Contains(content, "bounded rollout sequence") {
	...
	content := gitmodel.PayloadString(inventory.Payload, "content")
	...
	if got, want := gitmodel.PayloadString(inventory.Payload, "content_format"), "docx"; got != want {
...
func streamMultipleDocumentFacts(t *testing.T, repoPath string, relativePaths []string) []facts.Envelope {
	...
	metas := make([]gitmodel.ContentFileMeta, 0, len(relativePaths))
