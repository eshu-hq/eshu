...
func TestStreamFactsEmitsDocumentationTruthMentionsAndClaimCandidates(t *testing.T) {
	...
	for _, claim := range claimFacts {
		claimTypes[gitmodel.PayloadString(claim.Payload, "claim_type")] = true
		if got, want := gitmodel.PayloadString(claim.Payload, "authority"), facts.DocumentationClaimAuthorityDocumentEvidence; got != want {
...
func TestStreamFactsEmitsDocumentationDocumentsForMarkdownFamily(t *testing.T) {
	...
	for _, envelope := range documentFacts {
		if strings.Contains(gitmodel.PayloadString(envelope.Payload, "canonical_uri"), "/blob/sha256:") {
...
func TestStreamFactsDisambiguatesDuplicateMarkdownHeadingAnchors(t *testing.T) {
	...
	anchors := []string{
		gitmodel.PayloadString(sections[0].Payload, "section_anchor"),
		gitmodel.PayloadString(sections[1].Payload, "section_anchor"),
		gitmodel.PayloadString(sections[2].Payload, "section_anchor"),
	}
	if got, want := strings.Join(anchors, ","), "operate,step,step-2"; got != want {
		t.Fatalf("section anchors = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(sections[2].Payload, "parent_section_id"), gitmodel.PayloadString(sections[0].Payload, "section_id"); got != want {
