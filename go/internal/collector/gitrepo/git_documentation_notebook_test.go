...
func TestStreamFactsEmitsNotebookNarrativeDocumentation(t *testing.T) {
  ...
  "nbformat_minor": 5
	...
	if got, want := gitmodel.PayloadString(document.Payload, "format"), "notebook"; got != want {
		t.Fatalf("document format = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(document.Payload, "title"), "Analysis"; got != want {
...
func TestStreamFactsBoundsLargeNotebookTextOutput(t *testing.T) {
	...
	repoPath := t.TempDir()
	largeOutput := strings.Repeat("x", gitdocs.DocumentationMaxSectionChars+1024)
  ...
  "nbformat": 4
	...
	content := gitmodel.PayloadString(sections[0].Payload, "content")
	if len([]rune(content)) > gitdocs.DocumentationMaxSectionChars {
		t.Fatalf("output content length = %d, want <= %d", len([]rune(content)), gitdocs.DocumentationMaxSectionChars)
	}
...
func TestStreamFactsParsesNotebookLargerThanGenericDocumentationLimit(t *testing.T) {
	...
	repoPath := t.TempDir()
	filler := strings.Repeat("x", gitdocs.DocumentationMaxBodyBytes+1024)
  ...
  "nbformat": 4
	...
	if got, want := gitmodel.PayloadString(sections[0].Payload, "heading_text"), "Large Notebook"; got != want {
		t.Fatalf("section heading = %q, want %q", got, want)
	}
	if content := gitmodel.PayloadString(sections[0].Payload, "content"); !strings.Contains(content, "Narrative survives.") {
...
func TestNotebookDocumentationStableIDsAndParserSupport(t *testing.T) {
  ...
  "nbformat": 4
	...
	if got, want := gitmodel.PayloadString(sections[0].Payload, "section_anchor"), "cell-0-same"; got != want {
		t.Fatalf("first section anchor = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(sections[1].Payload, "section_anchor"), "cell-2-same"; got != want {
...
func assertNotebookSection(t *testing.T, envelope facts.Envelope, heading string, anchor string, startRef string, cellID string) {
	t.Helper()
	if got := gitmodel.PayloadString(envelope.Payload, "heading_text"); got != heading {
		t.Fatalf("section heading = %q, want %q", got, heading)
	}
	if got := gitmodel.PayloadString(envelope.Payload, "section_anchor"); got != anchor {
		t.Fatalf("section anchor = %q, want %q", got, anchor)
	}
	if got := gitmodel.PayloadString(envelope.Payload, "source_start_ref"); got != startRef {
...
func notebookSectionContent(sections []facts.Envelope) string {
	...
	for _, section := range sections {
		parts = append(parts, gitmodel.PayloadString(section.Payload, "content"))
	}
