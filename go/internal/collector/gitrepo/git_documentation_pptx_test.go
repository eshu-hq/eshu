...
func TestStreamFactsEmitsPPTXSlideDocumentation(t *testing.T) {
	...
	if got, want := gitmodel.PayloadString(document.Payload, "format"), "pptx"; got != want {
		t.Fatalf("document format = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(document.Payload, "document_type"), "presentation"; got != want {
		t.Fatalf("document_type = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(document.Payload, "title"), "Release Review"; got != want {
	...
	if got, want := gitmodel.PayloadString(review.Payload, "content_format"), "pptx"; got != want {
	...
	content := gitmodel.PayloadString(review.Payload, "content")
...
func TestStreamFactsBoundsLargePPTXDeck(t *testing.T) {
	...
	repoPath := t.TempDir()
	slides := make([]pptxTestSlide, 0, gitdocs.PptxMaxSlides+1)
	for i := 0; i < gitdocs.PptxMaxSlides; i++ {
	...
	assertPayloadWarning(t, document.Payload, "hidden_content_skipped")
	if got, want := payloadSourceMetadataValue(document.Payload, "slide_count"), fmt.Sprintf("%d", gitdocs.PptxMaxSlides+1); got != want {
		t.Fatalf("slide_count = %q, want %q", got, want)
	}
	if got, want := payloadSourceMetadataValue(document.Payload, "visible_slide_count"), fmt.Sprintf("%d", gitdocs.PptxMaxSlides); got != want {
	...
	sections := factsByKind(envelopes, facts.DocumentationSectionFactKind)
	if got, want := len(sections), gitdocs.PptxMaxSlides; got != want {
		t.Fatalf("documentation_section count = %d, want %d", got, want)
	}
	for _, section := range sections {
		if content := gitmodel.PayloadString(section.Payload, "content"); strings.Contains(content, "hidden body should not emit") {
