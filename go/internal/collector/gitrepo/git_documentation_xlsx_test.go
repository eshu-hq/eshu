...
func TestStreamFactsEmitsXLSXWorkbookDocumentation(t *testing.T) {
	...
	if got, want := gitmodel.PayloadString(document.Payload, "format"), "xlsx"; got != want {
		t.Fatalf("document format = %q, want %q", got, want)
	}
	if got, want := gitmodel.PayloadString(document.Payload, "document_type"), "spreadsheet"; got != want {
	...
	section := sections[0]
	if got, want := gitmodel.PayloadString(section.Payload, "heading_text"), "Inventory"; got != want {
	...
	content := gitmodel.PayloadString(section.Payload, "content")
...
func TestStreamFactsBoundsLargeXLSXWorkbook(t *testing.T) {
	...
	for i := 0; i < gitdocs.SpreadsheetMaxRows+5; i++ {
		row := make([]xlsxTestCell, 0, gitdocs.SpreadsheetMaxColumns+2)
		for col := 0; col < gitdocs.SpreadsheetMaxColumns+2; col++ {
	...
	if got, want := payloadSourceMetadataValue(section.Payload, "row_count"), fmt.Sprintf("%d", gitdocs.SpreadsheetMaxRows); got != want {
		t.Fatalf("row_count = %q, want %q", got, want)
	}
	if got, want := payloadSourceMetadataValue(section.Payload, "column_count"), fmt.Sprintf("%d", gitdocs.SpreadsheetMaxColumns); got != want {
		t.Fatalf("column_count = %q, want %q", got, want)
	}
	if got, want := payloadSourceMetadataValue(section.Payload, "sample_row_count"), fmt.Sprintf("%d", gitdocs.SpreadsheetSampleRows); got != want {
...
func TestReadDocumentationBodySkipsLegacyXLSBytes(t *testing.T) {
	...
	body, ok := gitdocs.ReadDocumentationBody(repoPath, "docs/legacy.xls", nil)
	if !ok {
