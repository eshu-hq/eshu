...
func TestStreamFactBodyReadCountBeforeAfter(t *testing.T) {
	...
	metas := make([]gitmodel.ContentFileMeta, len(relPaths))
	for i, rel := range relPaths {
		metas[i] = gitmodel.ContentFileMeta{RelativePath: rel, Digest: "sha256:doc", Language: "markdown", CommitSHA: "abc123"}
	}
	...
	freshSnapshot := func() RepositorySnapshot {
		fresh := make([]gitmodel.ContentFileMeta, len(metas))
		copy(fresh, metas)
