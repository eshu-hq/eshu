...
func resolveNativeSnapshotFileSet(
	...
	stats, fileSets, err := discovery.ResolveRepositoryFileSetsWithStats(
		...
		func(path string) bool {
			...
			if ok || gitdocs.IsGitDocumentationPath(path) {
				return true
			}
			return gitsubmodule.IsGitmodulesCandidatePath(repoPath, path) || gitcodeowners.IsCodeownersCandidatePath(repoPath, path)
		},
		opts,
	)
	if err != nil {
