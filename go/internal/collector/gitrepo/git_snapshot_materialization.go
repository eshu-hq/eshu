	...
	for _, target := range fileTargets {
		...
		if !isTerraformStateCandidateName(filepath.Base(absoluteTarget)) {
			if _, ok := registry.LookupByPath(absoluteTarget); !ok {
				...
				if !gitdocs.IsGitDocumentationPath(absoluteTarget) && !isCodeownersCandidate {
					continue
				}
			}
		}
...
func materializationRecordsToMetas(records []content.Record) []gitmodel.ContentFileMeta {
	metas := make([]gitmodel.ContentFileMeta, 0, len(records))
...
func materializationEntitiesToSnapshots(
	...
	indexedAt time.Time,
) []gitmodel.ContentEntitySnapshot {
	snapshots := make([]gitmodel.ContentEntitySnapshot, 0, len(entities))
	for _, entity := range entities {
		snapshots = append(snapshots, gitmodel.ContentEntitySnapshot{
