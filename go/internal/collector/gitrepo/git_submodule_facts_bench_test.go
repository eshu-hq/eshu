...
func BenchmarkNoteSubmoduleCandidate(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for _, p := range paths {
			...
			gitsubmodule.NoteSubmoduleCandidate(candidates, p, body)
		}
	}
}
...
func BenchmarkSubmoduleContentStreamHook(b *testing.B) {
	for _, withSubmodule := range []bool{false, true} {
		...
		b.Run(name, func(b *testing.B) {
			...
			w := gitmodel.NewFactStreamWriter(ch, &count, "")
			...
			for i := 0; i < b.N; i++ {
				...
				before := count.Load()
				gitsubmodule.EmitSubmoduleFactsForCandidates(goCtx, w, "repo-1", repoPath, "", "scope-1", "gen-1", time.Time{}, candidates)
...
func BenchmarkGitSubmoduleGitlinkSHA(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		gitsubmodule.GitSubmoduleGitlinkSHA(ctx, repoPath, "", submoduleBenchSubmodulePath)
	}
}
