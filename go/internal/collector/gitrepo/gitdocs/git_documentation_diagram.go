...
func mermaidClickLink(line string) (diagramLink, bool) {
	...
	return diagramLink{target: target, anchor: gitmodel.FirstNonEmptyString(anchor, target)}, true
}
...
func uniqueDiagramLinks(links []diagramLink) []diagramLink {
	...
	for _, link := range links {
		...
		link.anchor = cleanDiagramLabel(gitmodel.FirstNonEmptyString(link.anchor, link.target))
...
func diagramDocumentationLinks(
	...
	for _, link := range links {
		out = append(out, facts.DocumentationLinkPayload{
			...
			AnchorTextHash: documentationHashText(gitmodel.FirstNonEmptyString(link.anchor, link.target)),
			SourceMetadata: map[string]string{
				"path":          relativePath,
				"format_family": "diagram",
			},
