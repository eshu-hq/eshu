...
func plantUMLLinks(line string) []diagramLink {
	...
	for _, match := range matches {
		...
		links = append(links, diagramLink{target: target, anchor: gitmodel.FirstNonEmptyString(anchor, target)})
	}
	return links
}
	...
	for {
		...
		for _, attr := range start.Attr {
			...
			case "name", "label", "value":
				label = gitmodel.FirstNonEmptyString(label, drawIOLabel(attr.Value))
			case "link", "href":
		...
		if link != "" {
			result.links = append(result.links, diagramLink{target: link, anchor: gitmodel.FirstNonEmptyString(label, link)})
		}
	}
	result.labels = uniqueDiagramValues(result.labels)
	result.links = uniqueDiagramLinks(result.links)
...
func extractExcalidrawDiagramContent(body string) diagramExtraction {
	...
	for _, element := range document.Elements {
		...
		if target := CleanDiagramLinkTarget(element.Link); target != "" {
			result.links = append(result.links, diagramLink{target: target, anchor: gitmodel.FirstNonEmptyString(label, target)})
		}
	}
	result.labels = uniqueDiagramValues(result.labels)
	result.links = uniqueDiagramLinks(result.links)
