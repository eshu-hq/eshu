...
func GitDocumentationEnvelopesForContentFile(
	...
	if emitSource {
		sourcePayload := facts.DocumentationSourcePayload{
			...
			SourceSystem: "git",
			ExternalID:   repo.ID,
			DisplayName:  gitmodel.FirstNonEmptyString(repo.Name, repo.RepoSlug, repo.ID),
...
func DocumentationMetaRelativePaths(metas []gitmodel.ContentFileMeta) map[string]bool {
...
func extractMarkdownDocumentationWithFormat(
	...
	revisionID := gitmodel.FirstNonEmptyString(commitSHA, digest, "unknown")
	...
	document := facts.DocumentationDocumentPayload{
		...
		Format:       format,
		Language:     "en",
		ContentHash:  gitmodel.FirstNonEmptyString(digest, documentationHashText(bodyText)),
		SourceMetadata: map[string]string{
			"path":    relativePath,
			"repo_id": repo.ID,
		},
...
func gitDocumentationEnvelope(
	...
	envelope := gitmodel.FactEnvelope(factKind, scopeID, generationID, observedAt, factKey, payloadMap, sourceURI)
	...
	if sourceURI == repoPath {
		...
		envelope.SourceRef.SourceRecordID = gitmodel.RepositoryRelativePath(repoPath, sourceURI)
	}
	return envelope
}
