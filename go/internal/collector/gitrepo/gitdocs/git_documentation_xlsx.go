...
func workbookDocumentPayload(
	...
	document := facts.DocumentationDocumentPayload{
		...
		Format:       format,
		Language:     "en",
		ContentHash:  gitmodel.FirstNonEmptyString(digest, documentationHashText(string(body))),
...
func parseXLSXWorkbookSheetRefs(body []byte) ([]xlsxSheetRef, error) {
	...
	for {
		...
		refs = append(refs, xlsxSheetRef{
			name:   gitmodel.FirstNonEmptyString(xlsxXMLAttr(start, "name"), fmt.Sprintf("Sheet%d", len(refs)+1)),
