...
func observabilitySourceInstanceEnvelope(
	...
	payload["source_class"] = gitmodel.FirstNonEmptyString(sourceClass, "declared")
...
func observabilityRowEnvelope(
	...
	for key, value := range row {
		if key == "cluster_server" {
			if fingerprint := observabilityFingerprint(gitmodel.PayloadString(row, key)); fingerprint != "" {
				payload["cluster_server_fingerprint"] = fingerprint
			}
			continue
		}
		if key == "source_revision" && gitmodel.PayloadString(row, key) == "unknown" {
			continue
		}
	...
	sourceKind := gitmodel.PayloadString(payload, "source_kind")
	if sourceKind == "" {
		payload["source_kind"] = "git"
	}
...
func observabilityBasePayload(
	...
	payload := map[string]any{
		...
		"source_revision":   gitmodel.FirstNonEmptyString(sourceRevision, "unknown"),
	}
...
func observabilitySourceClassForFile(fileData map[string]any) string {
	...
	for _, mapping := range observabilitySourceBuckets {
		for _, row := range observabilityRows(fileData, mapping.bucket) {
			if sourceClass := strings.TrimSpace(gitmodel.PayloadString(row, "source_class")); sourceClass != "" {
...
func observabilityRecordIdentity(payload map[string]any) string {
	...
	for _, key := range keys {
		if value := gitmodel.PayloadString(payload, key); value != "" {
