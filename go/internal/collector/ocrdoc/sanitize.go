...
var (
	...
	unixPathPattern    = regexp.MustCompile(`(?i)(^|[[:space:]])/(users|home|var|etc|private|volumes|tmp)/\S+`)
...
func safeCanonicalURI(value string) (string, bool) {
	...
	if strings.HasPrefix(value, "git://") && !containsSensitiveMarker(strings.TrimPrefix(value, "git://")) {
		return value, false
	}
