...
func TestSanitizeURLDropsOpaqueCredentials(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "scp-style git ref is credential-shaped", value: "git@github.example:" + credentialSentinel + "/repo"},
...
// TestPackageArtifactEnvelopeNeverPersistsOpaqueCredential drives the real
// envelope constructor with a credentialed opaque SourceURI and ArtifactURL
// and asserts the sentinel is absent from the marshaled envelope — the bytes
// that persist. The second run is the positive control: a path-segment
// sentinel must reach the same bytes, so the absence assertion is reading a
// real envelope.
func TestPackageArtifactEnvelopeNeverPersistsOpaqueCredential(t *testing.T) {
	t.Parallel()

	build := func(t *testing.T, uri string) string {
		t.Helper()
		envelope, err := NewPackageArtifactEnvelope(PackageArtifactObservation{
		...
		if err != nil {
			t.Fatalf("NewPackageArtifactEnvelope() error = %v, want nil", err)
		}
	...
	persisted := build(t, "svc:"+credentialSentinel+"@h.internal:5432/maven2/")
	if strings.Contains(persisted, credentialSentinel) {
		t.Errorf("credential sentinel reached the persisted envelope:\n%s", persisted)
	}

	control := build(t, "https://h.internal/owners/"+credentialSentinel+"@example.com/maven2/")
