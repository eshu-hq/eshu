...
func (p *recordingMetadataProvider) FetchMetadata(ctx context.Context, target TargetConfig) (MetadataDocument, error) {
	p.targets = append(p.targets, target)
	return p.staticMetadataProvider.FetchMetadata(ctx, target)
}
