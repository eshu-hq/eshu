# AGENTS.md - collector/preflight/archive guidance for LLM assistants

## Read first

1. `go/internal/collector/preflight/archive/README.md`
2. `go/internal/collector/preflight/archive/doc.go`
3. `go/internal/collector/preflight/archive/preflight.go`
...
- Caller cancellation or deadline returns `timeout` and no partially trusted
  safe result.
- Gzip-compressed tar support in this package is still metadata preflight only.
  Contained-document routing permission belongs to the owning collector slice.
