...
func (e ProviderHTTPError) Error() string {
	if strings.TrimSpace(e.Message) == "" {
		return fmt.Sprintf("metric provider returned status %d", e.StatusCode)
	}
	return fmt.Sprintf("metric provider returned status %d: %s", e.StatusCode, e.Message)
}
...
// ProviderFailure wraps a metric provider failure with a bounded failure class.
...
func classifiedProviderFailure(err error) ProviderFailure {
	...
	if errors.As(err, &apiErr) {
		switch strings.TrimSpace(apiErr.ErrorType) {
		case "bad_data":
			return ProviderFailure{failureClass: FailureTerminal, cause: err}
		default:
			return ProviderFailure{failureClass: FailureRetryable, cause: err}
