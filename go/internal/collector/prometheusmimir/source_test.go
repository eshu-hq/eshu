...
func TestClaimedSourceEmitsObservedMetricFacts(t *testing.T) {
	t.Parallel()
	...
	if got, want := collected.Scope.ScopeKind, scope.ScopeKind(ScopeKindMetricSource); got != want {
		t.Fatalf("Scope.ScopeKind = %q, want %q", got, want)
	}
	if got, want := collected.Generation.FreshnessHint, "prometheus_mimir_observed_metadata"; got != want {
	...
	if got, want := counts[facts.ObservabilityObservedTargetFactKind], 1; got != want {
		t.Fatalf("observed_target facts = %d, want %d", got, want)
	}
...
func TestClaimedSourceRejectsUnknownProvider(t *testing.T) {
	t.Parallel()
	...
	_, err := NewClaimedSource(SourceConfig{
		...
		ClientFactory: func(TargetConfig) (EvidenceClient, error) {
			t.Fatal("unknown provider should fail before client construction")
			return nil, nil
		},
	})
	if err == nil {
		t.Fatal("NewClaimedSource() error = nil, want unknown provider rejected")
	}
}
