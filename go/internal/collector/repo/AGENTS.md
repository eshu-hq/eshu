# Collector repo namespace agent guide
...
- Keep repo implementation in leaf packages and collection behavior in
...
Move or add a repo leaf only with a production-and-test import edge
census. Update all consumers in the same change; do not leave forwarding
packages or duplicate contracts.

## Verification

Run the recursive leaf tests before claiming a repo change is safe:

```bash
env -u GOROOT go test -count=1 ./internal/collector/repo/...
```
