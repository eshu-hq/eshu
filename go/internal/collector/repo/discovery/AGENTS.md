# AGENTS.md — internal/collector/repo/discovery guidance for LLM assistants

## Read first

1. `go/internal/collector/repo/discovery/README.md` — purpose, exported surface,
   path invariants, and operational notes
2. `go/internal/collector/repo/discovery/discovery.go` — `ResolveRepositoryFileSetsWithStats`,
   `collectSupportedFiles`, `groupFilesByRepository`, and `nearestRepositoryRoot`
3. `go/internal/collector/repo/discovery/gitignore.go` — `.gitignore` and
   `.eshuignore` filtering implementations
4. `go/internal/collector/repo/discovery/path_globs.go` — `PathGlobRule` matching
   and `IgnoredPathGlobs` / `PreservedPathGlobs` logic
5. `go/internal/collector/repo/discovery/doc.go` — the package contract
...
- **Add a stat or counter to DiscoveryStats** → add the field to `DiscoveryStats`
  ...
  `internal/collector/repo/git/discovery_advisory.go` populates it.
