...
func TestDiscoveryAdvisoryClassifiesRealManifestAndConfigFixtures(t *testing.T) {
	...
	entities = append(entities, model.ContentEntitySnapshot{
		EntityType:   "TerraformResource",
		RelativePath: "main.tf",
		ArtifactType: "terraform_hcl",
	})

	contentFiles := []model.ContentFileMeta{
		{RelativePath: "go.mod"},
		{RelativePath: "main.tf"},
	}
...
// snapshots.
func snapshotsFromParsedPayload(t *testing.T, relPath string, payload map[string]any) []model.ContentEntitySnapshot {
	t.Helper()
	...
	var snapshots []model.ContentEntitySnapshot
...
func snapshotFromShapeEntity(relPath, label string, entity shape.Entity) model.ContentEntitySnapshot {
	return model.ContentEntitySnapshot{
...
func countVariableDependencyEntities(entities []model.ContentEntitySnapshot) int {
	count := 0
	for _, entity := range entities {
