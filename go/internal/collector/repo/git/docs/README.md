...
`go/internal/collector/repo/git/docs` owns documentation extraction for the git collector.
...
- `ReadDocumentationBody`, `DocumentationMetaRelativePaths`,
  `CleanDiagramLinkTarget` — helpers git still calls by name.
...
tests that assert them stayed in git. They are budget constants, not
...
`buildStreamingGeneration` stayed in git, because that is where the wiring
they assert lives.
