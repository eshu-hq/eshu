...
func workbookDocumentPayload(
	...
	document := facts.DocumentationDocumentPayload{
		...
		Format:       format,
		Language:     "en",
		ContentHash:  model.FirstNonEmptyString(digest, documentationHashText(string(body))),
...
func parseXLSXWorkbookSheetRefs(body []byte) ([]xlsxSheetRef, error) {
	...
	for {
		...
		refs = append(refs, xlsxSheetRef{
			name:   model.FirstNonEmptyString(xlsxXMLAttr(start, "name"), fmt.Sprintf("Sheet%d", len(refs)+1)),
