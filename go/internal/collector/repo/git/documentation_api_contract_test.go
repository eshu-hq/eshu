...
`
	...
	if got, want := model.PayloadString(document.Payload, "document_type"), "api_contract"; got != want {
	...
	for _, want := range []string{"listOrders", "Returns bounded order summaries", "Schema refs: #/components/schemas/OrderList"} {
		if content := model.PayloadString(operation.Payload, "content"); !strings.Contains(content, want) {
			t.Fatalf("operation section content missing %q: %q", want, content)
		}
	}
	if got := model.PayloadString(operation.Payload, "source_start_ref"); !strings.Contains(got, "/paths/~1orders/get") {
		t.Fatalf("operation source_start_ref = %q, want JSON pointer anchor", got)
	}
	if got := model.PayloadString(operation.Payload, "section_anchor"); got != "operation-get-orders" {
	...
	if got, want := model.PayloadString(links[0].Payload, "target_uri"), "https://docs.example.test/payments"; got != want {
  ...
  total: Float
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := model.PayloadString(section.Payload, "content"); !strings.Contains(got, tt.wantContent) {
