...
func TestStreamFactsEmitsZIPArchiveDocumentationPacket(t *testing.T) {
	...
	if got, want := model.PayloadString(outer.Payload, "format"), "zip"; got != want {
...
func oversizeArchiveMemberBody() string {
	...
	for i := 0; builder.Len() <= docs.DocumentationMaxBodyBytes+1024; i++ {
...
func documentByID(documents []facts.Envelope, documentID string) *facts.Envelope {
	for i := range documents {
		if model.PayloadString(documents[i].Payload, "document_id") == documentID {
			return &documents[i]
		}
	}
	return nil
