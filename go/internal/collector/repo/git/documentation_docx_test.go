...
func TestStreamFactsEmitsDOCXDocumentationSections(t *testing.T) {
	...
	if got, want := model.PayloadString(document.Payload, "document_type"), "document"; got != want {
		t.Fatalf("document_type = %q, want %q", got, want)
	}
	if got, want := model.PayloadString(document.Payload, "title"), "Migration Plan"; got != want {
	...
	if content := model.PayloadString(plan.Payload, "content"); !strings.Contains(content, "bounded rollout sequence") {
	...
	content := model.PayloadString(inventory.Payload, "content")
	...
	if got, want := model.PayloadString(inventory.Payload, "content_format"), "docx"; got != want {
...
func streamMultipleDocumentFacts(t *testing.T, repoPath string, relativePaths []string) []facts.Envelope {
	...
	metas := make([]model.ContentFileMeta, 0, len(relativePaths))
