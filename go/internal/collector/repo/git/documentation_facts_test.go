...
func TestStreamFactsEmitsDocumentationDocumentsForMarkdownFamily(t *testing.T) {
	...
	for _, envelope := range documentFacts {
		if strings.Contains(model.PayloadString(envelope.Payload, "canonical_uri"), "/blob/sha256:") {
...
func TestStreamFactsDisambiguatesDuplicateMarkdownHeadingAnchors(t *testing.T) {
	...
	anchors := []string{
		model.PayloadString(sections[0].Payload, "section_anchor"),
		model.PayloadString(sections[1].Payload, "section_anchor"),
		model.PayloadString(sections[2].Payload, "section_anchor"),
	}
	if got, want := strings.Join(anchors, ","), "operate,step,step-2"; got != want {
		t.Fatalf("section anchors = %q, want %q", got, want)
	}
	if got, want := model.PayloadString(sections[2].Payload, "parent_section_id"), model.PayloadString(sections[0].Payload, "section_id"); got != want {
