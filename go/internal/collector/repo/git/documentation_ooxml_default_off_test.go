...
func TestUnsupportedMacroEnabledOOXMLDocumentationFormatsRemainDefaultOff(t *testing.T) {
	...
	for _, relativePath := range relativePaths {
		...
		if metas := docs.DocumentationFileMetasForPaths(repoPath, []string{file}, "commit"); len(metas) != 0 {
...
func TestOfficeSpreadsheetDocumentationFormatsAreDocumentationFiles(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		_, format, ok := docs.GitDocumentationSourceURIAndFormat(tc.relativePath)
		if !ok {
		...
		if metas := docs.DocumentationFileMetasForPaths(repoPath, []string{file}, "commit"); len(metas) != 1 {
