...
func TestStreamFactsEmitsPPTXSlideDocumentation(t *testing.T) {
	...
	if got, want := model.PayloadString(document.Payload, "document_type"), "presentation"; got != want {
		t.Fatalf("document_type = %q, want %q", got, want)
	}
	if got, want := model.PayloadString(document.Payload, "title"), "Release Review"; got != want {
	...
	if got, want := model.PayloadString(review.Payload, "content_format"), "pptx"; got != want {
	...
	content := model.PayloadString(review.Payload, "content")
...
func TestStreamFactsBoundsLargePPTXDeck(t *testing.T) {
	...
	repoPath := t.TempDir()
	slides := make([]pptxTestSlide, 0, docs.PptxMaxSlides+1)
	for i := 0; i < docs.PptxMaxSlides; i++ {
	...
	if got, want := payloadSourceMetadataValue(document.Payload, "visible_slide_count"), fmt.Sprintf("%d", docs.PptxMaxSlides); got != want {
	...
	for _, section := range sections {
		if content := model.PayloadString(section.Payload, "content"); strings.Contains(content, "hidden body should not emit") {
