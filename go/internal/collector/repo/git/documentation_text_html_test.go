...
func TestStreamFactsEmitsLightweightTextDocumentationFormats(t *testing.T) {
	...
	snapshot := RepositorySnapshot{
		FileCount: 5,
		DocumentationFileMetas: []model.ContentFileMeta{
	...
	if got, want := model.PayloadString(sourceFacts[0].Payload, "source_type"), "repository_documentation"; got != want {
	...
	for _, document := range documentFacts {
		formats[model.PayloadString(document.Payload, "format")] = true
	...
	if got, want := model.PayloadString(claimFacts[0].Payload, "claim_type"), "cli_command"; got != want {
...
<html>
  ...
  <body>
	...
	for _, document := range documentFacts {
		if got, want := model.PayloadString(document.Payload, "format"), "html"; got != want {
	...
	if content := model.PayloadString(reference.Payload, "content"); strings.Contains(content, "window.internal") || strings.Contains(content, "display: none") {
	...
	if content := model.PayloadString(usage.Payload, "content"); !strings.Contains(content, "SLO") || !strings.Contains(content, "99.9%") {
...
func TestReadDocumentationBodyBoundsLargeDocumentationFile(t *testing.T) {
	...
	writeCollectorTestFile(
		...
		strings.Repeat("x", docs.DocumentationMaxBodyBytes+1024),
	)

	body, ok := docs.ReadDocumentationBody(repoPath, "docs/large.html", nil)
	if !ok {
		t.Fatal("readDocumentationBody() ok = false, want true")
	}
	if got, want := len(body), docs.DocumentationMaxBodyBytes+1; got != want {
...
func TestGitDocumentationSourceURIAndFormatRejectsParserOnlyFiles(t *testing.T) {
	...
	if _, _, ok := docs.GitDocumentationSourceURIAndFormat("app.py"); ok {
		t.Fatal("gitDocumentationSourceURIAndFormat(app.py) ok = true, want false")
	}
	sourceURI, format, ok := docs.GitDocumentationSourceURIAndFormat("docs/guide.rst")
	if !ok {
...
func sectionByHeading(sections []facts.Envelope, heading string) *facts.Envelope {
	for i := range sections {
		if model.PayloadString(sections[i].Payload, "heading_text") == heading {
			return &sections[i]
		}
	}
	return nil
...
func assertLinkTargetPresent(t *testing.T, links []facts.Envelope, target string) {
	...
	for _, link := range links {
		if model.PayloadString(link.Payload, "target_uri") == target {
			return
		}
	}
...
func gotParsedFilePathCount(files []map[string]any, suffix string) int {
	...
	for _, file := range files {
		if strings.HasSuffix(model.PayloadString(file, "path"), suffix) {
			count++
		}
	}
	return count
