...
func TestStreamFactsEmitsXLSXWorkbookDocumentation(t *testing.T) {
	...
	if got, want := model.PayloadString(document.Payload, "format"), "xlsx"; got != want {
		t.Fatalf("document format = %q, want %q", got, want)
	}
	if got, want := model.PayloadString(document.Payload, "document_type"), "spreadsheet"; got != want {
	...
	section := sections[0]
	if got, want := model.PayloadString(section.Payload, "heading_text"), "Inventory"; got != want {
...
func TestStreamFactsBoundsLargeXLSXWorkbook(t *testing.T) {
	...
	for i := 0; i < docs.SpreadsheetMaxRows+5; i++ {
		row := make([]xlsxTestCell, 0, docs.SpreadsheetMaxColumns+2)
		for col := 0; col < docs.SpreadsheetMaxColumns+2; col++ {
	...
	if got, want := payloadSourceMetadataValue(section.Payload, "column_count"), fmt.Sprintf("%d", docs.SpreadsheetMaxColumns); got != want {
		t.Fatalf("column_count = %q, want %q", got, want)
	}
	if got, want := payloadSourceMetadataValue(section.Payload, "sample_row_count"), fmt.Sprintf("%d", docs.SpreadsheetSampleRows); got != want {
...
func TestReadDocumentationBodySkipsLegacyXLSBytes(t *testing.T) {
	...
	body, ok := docs.ReadDocumentationBody(repoPath, "docs/legacy.xls", nil)
	if !ok {
