...
func TestStreamFactBodyReadCountBeforeAfter(t *testing.T) {
	...
	metas := make([]model.ContentFileMeta, len(relPaths))
	for i, rel := range relPaths {
		metas[i] = model.ContentFileMeta{RelativePath: rel, Digest: "sha256:doc", Language: "markdown", CommitSHA: "abc123"}
	}
	...
	freshSnapshot := func() RepositorySnapshot {
		fresh := make([]model.ContentFileMeta, len(metas))
		copy(fresh, metas)
