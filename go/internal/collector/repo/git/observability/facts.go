...
func observabilitySourceInstanceEnvelope(
	...
	payload["source_class"] = model.FirstNonEmptyString(sourceClass, "declared")
...
func observabilityRowEnvelope(
	...
	for key, value := range row {
		if key == "cluster_server" {
			if fingerprint := observabilityFingerprint(model.PayloadString(row, key)); fingerprint != "" {
				payload["cluster_server_fingerprint"] = fingerprint
			}
			continue
		}
		if key == "source_revision" && model.PayloadString(row, key) == "unknown" {
			continue
		}
	...
	sourceKind := model.PayloadString(payload, "source_kind")
	if sourceKind == "" {
		payload["source_kind"] = "git"
	}
...
func observabilityBasePayload(
	...
	payload := map[string]any{
		...
		"source_revision":   model.FirstNonEmptyString(sourceRevision, "unknown"),
	}
...
func observabilitySourceClassForFile(fileData map[string]any) string {
	...
	for _, mapping := range observabilitySourceBuckets {
		for _, row := range observabilityRows(fileData, mapping.bucket) {
			if sourceClass := strings.TrimSpace(model.PayloadString(row, "source_class")); sourceClass != "" {
...
func observabilityRecordIdentity(payload map[string]any) string {
	...
	for _, key := range keys {
		if value := model.PayloadString(payload, key); value != "" {
