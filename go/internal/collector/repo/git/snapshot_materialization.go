	...
	for _, target := range fileTargets {
		...
		if !isTerraformStateCandidateName(filepath.Base(absoluteTarget)) {
			if _, ok := registry.LookupByPath(absoluteTarget); !ok {
				...
				if !docs.IsGitDocumentationPath(absoluteTarget) && !isCodeownersCandidate {
					continue
				}
			}
		}
...
func materializationRecordsToMetas(records []content.Record) []model.ContentFileMeta {
	metas := make([]model.ContentFileMeta, 0, len(records))
...
func materializationEntitiesToSnapshots(
	...
	indexedAt time.Time,
) []model.ContentEntitySnapshot {
	snapshots := make([]model.ContentEntitySnapshot, 0, len(entities))
	for _, entity := range entities {
		snapshots = append(snapshots, model.ContentEntitySnapshot{
