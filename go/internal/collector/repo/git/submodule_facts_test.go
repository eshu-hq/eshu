...
func TestNoteSubmoduleCandidateIgnoresOtherPaths(t *testing.T) {
	...
	submodule.NoteSubmoduleCandidate(candidates, "vendor/.gitmodules", "ignored body")
	submodule.NoteSubmoduleCandidate(candidates, "README.md", "ignored body")
	if len(candidates) != 0 {
		t.Fatalf("candidates = %#v, want empty", candidates)
	}

	submodule.NoteSubmoduleCandidate(candidates, ".gitmodules", "real body")
