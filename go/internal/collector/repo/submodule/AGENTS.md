# AGENTS.md — internal/collector/repo/submodule guidance
...
8. `go/internal/collector/repo/git/submodule/facts.go` — the Git collector glue
...
9. `go/internal/collector/repo/git/submodule/pinned_sha.go` — the Git collector's
   `gitSubmoduleGitlinkSHA`, the `PinnedSHAResolver` implementation wired in
   by `facts.go` (issue #5420 Phase 2b).
...
- `Emit` fills `Pin.PinnedSHA` only through the caller-supplied
  ...
  `go/internal/collector/repo/git/submodule/pinned_sha.go`, issue #5420 Phase 2b);
