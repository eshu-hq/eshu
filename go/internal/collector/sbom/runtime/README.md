...
`internal/collector/sbom/runtime` is the claim-driven hosted runtime for SBOM
...
- No-Regression Evidence (#2384): configured document fetches still issue one
  ...
  `go test ./internal/collector/sbom/runtime -run
...
- No-Regression Evidence (#3113): the `oci_referrer` fetch now resolves its
  ...
  ./internal/collector/sbom/runtime/... ./internal/collector/ociregistry/...
