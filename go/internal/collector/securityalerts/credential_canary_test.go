...
// returning "" for everything — and if sanitizeURL stopped being called the
// absence rows would redden.
...
// TestDependabotEnvelopeNeverPersistsOpaqueCredential drives the real
...
func TestDependabotEnvelopeNeverPersistsOpaqueCredential(t *testing.T) {
	t.Parallel()
	...
	build := func(t *testing.T, sourceURI string) string {
		...
		envelope, err := NewGitHubDependabotAlertEnvelope(ctx, GitHubDependabotAlert{
			Number:    42,
			State:     "open",
			HTMLURL:   "https://github.com/eshu-hq/eshu/security/dependabot/42",
			CreatedAt: "2026-05-20T12:00:00Z",
			UpdatedAt: "2026-05-23T10:15:00Z",
			Dependency: GitHubDependabotDependency{
				ManifestPath: "package-lock.json",
				Package:      GitHubDependabotPackage{Ecosystem: "npm", Name: "left-pad"},
			},
		})
		if err != nil {
	...
	persisted := build(t, "svc:"+credentialSentinel+"@h.internal:5432/alerts")
	if strings.Contains(persisted, credentialSentinel) {
		t.Errorf("credential sentinel reached the persisted envelope:\n%s", persisted)
	}

	control := build(t, "https://h.internal/owners/"+credentialSentinel+"@example.com/alerts")
