...
`jiraplanner` turns one validated Jira collector instance into a deterministic
workflow run and one work item per selected target. It also checks whether an
incident-freshness scope belongs to a configured target; the root coordinator
combines that membership result with its authorization policy.

## Ownership boundary

The package owns request validation, Jira target decoding, duplicate and field
validation, requested-scope filtering, freshness-scope membership, trigger
selection, requested-scope serialization, stable IDs, and fairness metadata.
It performs no network, credential, or database work.
...
`internal/collector/jira` owns provider calls and fact emission after a worker
claims the planned item.

## Exported surface

- `PlanRequest` carries the collector instance, observation time, plan key,
  optional trigger override, and optional scope filter.
- `WorkPlanner` implements the root coordinator's `JiraPlanner` interface.
...
- `internal/scope` supplies the Jira collector kind.
...
- Requested-scope JSON contains target identity only. It omits Jira URLs,
  credential environment names, JQL, limits, and lookback settings.
- Fairness keys keep the site ID as the per-instance conflict partition.
...
precedence, configured item order, exact identity fields, site fairness
partitions, UTC timestamps, sorted privacy-safe metadata, membership, and empty
selections. Root tests pin all five request fields and scheduled and freshness
admission behavior. Planning still scans `n` configured targets in O(n) and
sorts at most `n` requested-scope rows in O(n log n). It opens no network or
database connection.
...
transitions. Site-based fairness keys retain their prior conflict domain.
