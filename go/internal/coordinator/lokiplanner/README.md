# Loki scheduler planner

## Purpose

`lokiplanner` plans one workflow work item per enabled Grafana Loki target
without contacting Loki or resolving credentials.
...
- `WorkPlanner` implements the parent's `LokiPlanner` interface.
