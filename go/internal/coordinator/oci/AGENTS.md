# Coordinator OCI namespace agent guide
...
- Do not describe the `registry` leaf as independently extractable while it
  imports collector identity and provider-adapter packages.

## Common changes

Move or add an OCI planner only with a production-and-test import edge census.
