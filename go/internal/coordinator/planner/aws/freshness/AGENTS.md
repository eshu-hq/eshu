# AGENTS.md — internal/coordinator/planner/aws/freshness guidance
...
4. `../scheduled/planner.go` — a DIFFERENT AWS family that shares this
   package's `target_scopes` parsing. It now lives in `../scheduled`; do not move its
   symbols here and do not conflate the two.
5. `../../contract/README.md` for plan-key grammar.
...
- Use package name `freshness`. Where both packages are referenced, alias
  `internal/collector/awscloud/freshness` as `awsfreshness`.
...
Run `go test ./internal/coordinator/planner/aws/freshness ./internal/coordinator -count=1`
