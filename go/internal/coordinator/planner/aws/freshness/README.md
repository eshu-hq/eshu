...
`freshness` turns a batch of claimed AWS freshness triggers into one
...
`freshness.PlanRequest` in its method signature; `WorkPlanner`
...
The last three are exported for the same reason `gcp` exports
...
collect. So this follows the `gcp` precedent instead — one definition,
exported — and keeps `firstNonBlank`-style duplication for genuinely tiny pure
helpers.

`scheduled`'s own exported `ScanEnabled` still decodes the
...
registered scanner implements. `contract` validates plan keys. `facts`,
...
No-Regression Evidence: `go test ./internal/coordinator/planner/aws/freshness ./internal/coordinator -count=1`
