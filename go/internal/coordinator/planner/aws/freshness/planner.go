...
func validatePlanRequest(request PlanRequest) error {
	...
	if err := contract.ValidateSafePlanKey("AWS freshness planner", request.PlanKey); err != nil {
		return err
	}
	return nil
}
...
// (scheduled/planner.go); both share this one
...
func authorizedTargets(
	triggers []awsfreshness.StoredTrigger,
	scopes []TargetScope,
) ([]awsfreshness.Target, error) {
	targetsByKey := make(map[string]awsfreshness.Target, len(triggers))
...
// itself enforces.
func TargetAuthorized(target awsfreshness.Target, scopes []TargetScope) bool {
