...
func TestAWSFreshnessWorkPlannerPlansTargetedAWSClaims(t *testing.T) {
	...
	run, items, err := WorkPlanner{}.PlanAWSFreshnessWork(context.Background(), PlanRequest{
		Instance:   instance,
		Triggers:   []awsfreshness.StoredTrigger{trigger},
		ObservedAt: observedAt,
...
func TestAWSFreshnessWorkPlannerCoalescesDuplicateTargetTriggers(t *testing.T) {
	...
	observedAt := time.Date(2026, time.May, 15, 10, 0, 0, 0, time.UTC)
	first, err := awsfreshness.NewStoredTrigger(testAWSPlannerTrigger("evt-1", observedAt), observedAt)
	if err != nil {
		t.Fatalf("NewStoredTrigger(first) error = %v, want nil", err)
	}
	second, err := awsfreshness.NewStoredTrigger(testAWSPlannerTrigger("evt-2", observedAt), observedAt)
	if err != nil {
	...
	_, items, err := WorkPlanner{}.PlanAWSFreshnessWork(context.Background(), PlanRequest{
		Instance:   testAWSCollectorInstance(observedAt),
		Triggers:   []awsfreshness.StoredTrigger{first, second},
		ObservedAt: observedAt,
		PlanKey:    "freshness-20260515T100000Z",
	})
	if err != nil {
...
func testAWSPlannerTrigger(eventID string, observedAt time.Time) awsfreshness.Trigger {
	return awsfreshness.Trigger{
		EventID:      eventID,
		Kind:         awsfreshness.EventKindConfigChange,
		AccountID:    "123456789012",
		Region:       "us-east-1",
