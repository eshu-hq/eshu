...
func (p WorkPlanner) PlanAWSScheduledWork(
	...
	scopes, err := freshness.ParseTargetScopes(request.Instance.Configuration)
	if err != nil {
		return workflow.Run{}, nil, err
	}
...
func validatePlanRequest(request PlanRequest) error {
	...
	if err := contract.ValidateSafePlanKey("AWS scheduled planner", request.PlanKey); err != nil {
		return err
	}
	return nil
}
...
func planAWSScheduledTargets(scopes []freshness.TargetScope) awsScheduledTargetPlan {
	byKey := map[string]awsfreshness.Target{}
...
func awsScheduledTargetAllowed(target awsfreshness.Target) (string, bool) {
...
func sortedAWSScheduledTargets(byKey map[string]awsfreshness.Target) []awsfreshness.Target {
	keys := make([]string, 0, len(byKey))
	for key := range byKey {
		keys = append(keys, key)
	}
	sort.Strings(keys)
	targets := make([]awsfreshness.Target, 0, len(keys))
