...
// PlanRequest carries one package-registry collector planning
// request.
...
// WorkPlanner plans workflow rows for configured
...
func validatePackageRegistryPlanRequest(request PlanRequest) error {
	...
	if err := contract.ValidateSafePlanKey("package registry planner", request.PlanKey); err != nil {
		return err
	}
	return nil
}
...
func parsePackageRegistryRuntimeTargets(raw string) ([]packageRegistryTargetConfiguration, error) {
	...
	for _, target := range decoded.Targets {
		...
		target.TargetClass = configuredTargetClass(target)
...
func packageRegistryRequestedScopeSet(
	...
	payload := struct {
		...
		SkippedTargets      []schedule.DerivedTargetSkipEvidence `json:"skipped_targets,omitempty"`
	}{
...
func appendPackageRegistryDerivedTargets(
	...
	sort.SliceStable(configured, func(i, j int) bool {
		return schedule.TargetClassRank(targetClass(configured[i])) <
			schedule.TargetClassRank(targetClass(configured[j]))
	})
	return configured
}
...
func derivePackageRegistryTargets(
	...
	limit := DerivedTargetLimit(derivation.TargetLimit)
	...
	ecosystems := DerivationEcosystems(derivation.Ecosystems)
	seen := make(map[string]struct{}, len(configured)+len(owned))
	for _, target := range configured {
		seen[strings.TrimSpace(target.ScopeID)] = struct{}{}
	}
	out := make([]packageRegistryTargetConfiguration, 0, min(limit, len(owned)))
	skipped := 0
	for _, target := range owned {
		if !schedule.StringSetContains(ecosystems, target.Ecosystem) {
			continue
		}
	...
	return packageRegistryDerivationResult{
		Targets: out,
		SkippedTargets: schedule.DerivedTargetBudgetSkipEvidence(
			string(scope.CollectorPackageRegistry),
			limit,
			len(out),
			skipped,
			schedule.SortedStringSetValues(ecosystems),
			nil,
		),
	}
}
...
func packageRegistryWorkItem(
	...
	scopeID := strings.TrimSpace(target.ScopeID)
	createdAt := schedule.TargetCreatedAt(observedAt, ordinal)
	...
	item := workflow.WorkItem{
		...
		FairnessKey:         fmt.Sprintf("%s:%s:%s:%s", scope.CollectorPackageRegistry, strings.TrimSpace(instance.InstanceID), targetClass(target), strings.TrimSpace(target.Ecosystem)),
