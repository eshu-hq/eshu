...
func (c *fakeSemanticProviderClient) Dispatch(
	...
	if c.dispatchErr != nil {
		return DispatchResult{}, c.dispatchErr
	}
	return DispatchResult{ResponseHash: c.responseHash}, nil
}
...
func BenchmarkSemanticWorkerEgressGatedClaimLoop(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		worker := newSemanticWorker(claimer, DisabledProviderClient{}, nil, ProviderWorkerConfig{
