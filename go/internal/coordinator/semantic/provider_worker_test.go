...
func TestSemanticWorkerSkipsDeniedProviderEgress(t *testing.T) {
	...
	worker := newSemanticWorker(claimer, client, audit, ProviderWorkerConfig{
...
func TestSemanticWorkerAllowedEgressDefaultDisabledClientTerminatesNoNetwork(t *testing.T) {
	...
	audit := &fakeGovernanceAuditAppender{}
	var logs bytes.Buffer
	// No client supplied -> worker uses DisabledProviderClient.
	worker := newSemanticWorker(claimer, nil, audit, ProviderWorkerConfig{
...
func TestSemanticWorkerDisabledClientNeverDispatchesEvenWhenExecutionEnabled(t *testing.T) {
	...
	disabled := DisabledProviderClient{}
	if disabled.Enabled() {
		t.Fatal("DisabledProviderClient.Enabled() = true, want false")
	}
	if _, err := disabled.Dispatch(context.Background(), DispatchRequest{}); !errors.Is(err, ErrProviderExecutionNotEnabled) {
		t.Fatalf("Dispatch() error = %v, want ErrProviderExecutionNotEnabled", err)
	}
}
...
func TestSemanticWorkerExecutionDisabledFlagBlocksEnabledClient(t *testing.T) {
	...
	client := &fakeSemanticProviderClient{enabled: true}
	worker := newSemanticWorker(claimer, client, &fakeGovernanceAuditAppender{}, ProviderWorkerConfig{
...
func TestSemanticWorkerDisabledByDefault(t *testing.T) {
	...
	worker := ProviderWorker{
		Config:  ProviderWorkerConfig{Enabled: false, ScopeIDs: []string{semanticWorkerScope}},
		Claimer: claimer,
		Client:  client,
	}
	if err := worker.Run(context.Background()); err != nil {
