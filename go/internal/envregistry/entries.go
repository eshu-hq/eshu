...
var coreEntries = []Entry{
	...
	{Name: "ESHU_GRAPH_GAUGE_REFRESH_INTERVAL", Type: VarDuration, Default: "5m", Subsystem: "reducer", Description: "Delay between background refreshes of the graph-backed observable gauges (eshu_dp_edges_by_source_tool, eshu_dp_files_by_language, eshu_dp_graph_orphan_nodes). /metrics serves the last snapshot, so those gauges lag the graph by up to this interval plus one read."},
	{Name: "ESHU_GRAPH_GAUGE_REFRESH_TIMEOUT", Type: VarDuration, Default: "30s", Subsystem: "reducer", Description: "Deadline for each background graph read that refreshes a graph-backed observable gauge. A read that exceeds it is cancelled and counted as a timeout in eshu_dp_gauge_snapshot_refreshes_total; the previous snapshot keeps being served."},
