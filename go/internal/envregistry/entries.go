...
var coreEntries = []Entry{
	...
	{Name: "ESHU_GRAPH_READ_SLOW_THRESHOLD", Type: VarDuration, Default: "1s", Subsystem: "graph", Description: "Duration a bounded Neo4jReader graph read must reach or exceed before it is logged and counted as a slow outcome (issue #7035). Defaults to 1 second (it was 2 seconds before #7035). An invalid or non-positive value logs once and falls back to the default; it never disables the warning."},
