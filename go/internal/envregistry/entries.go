...
var coreEntries = []Entry{
	...
	// backend-conformance
	{Name: "ESHU_BACKEND_CONFORMANCE_VALUE_FLOW", Type: VarBool, Default: "false", Subsystem: "backend-conformance", Description: "Contributor test knob, read by no Eshu service -- consumed only by go/internal/backendconformance, scripts/verify_backend_conformance_live.sh, and scripts/verify-value-flow-conformance-expectation.sh (the CI job that runs the pair on both backends with the expectation inverted). Includes the opt-in value-flow cloud sink read/seed pair in the backend conformance corpora. The pair reproduces defects open upstream in NornicDB, so it is absent from the corpus when unset (absent, not skipped) rather than red-lining the blocking live gate on unrelated changes."},
	// graph
