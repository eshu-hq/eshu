...
var coreEntries = []Entry{
	...
	{Name: "ESHU_REDUCER_ADMISSION_HIGH_WATER_MARK", Type: VarInt, Default: "10000", Subsystem: "reducer", Description: "Ingester and bootstrap-index source-local reducer-intent admission threshold; defers while outstanding reducer queue depth is at or above this value. Set to 0 to disable."},
	{Name: "ESHU_REDUCER_ADMISSION_POLL_INTERVAL", Type: VarDuration, Default: "1s", Subsystem: "reducer", Description: "Ingester and bootstrap-index queue-depth recheck interval while reducer admission is deferring."},
	{Name: "ESHU_REDUCER_ADMISSION_RETRYING_HIGH_WATER_MARK", Type: VarInt, Default: "500", Subsystem: "reducer", Description: "Ingester and bootstrap-index graph-write backpressure: defers source-local reducer-intent admission while retrying-state reducer depth (the durable signal of recurring graph-write timeouts) is at or above this value, so recoverable work is throttled instead of dead-lettered. Set to 0 to disable."},
	{Name: "ESHU_REDUCER_ADMISSION_RETRYING_LOW_WATER_MARK", Type: VarInt, Default: "100", Subsystem: "reducer", Description: "Ingester and bootstrap-index hysteresis floor for graph-write backpressure; admission resumes only after retrying-state reducer depth falls below this value. Must be less than ESHU_REDUCER_ADMISSION_RETRYING_HIGH_WATER_MARK."},
