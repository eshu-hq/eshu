...
var coreEntries = []Entry{
	...
	{Name: "ESHU_CANONICAL_WRITE_TIMEOUT", Type: VarDuration, Subsystem: "graph", Description: "Transaction timeout for canonical and reducer graph writes (Go duration). On NornicDB an unset value uses 30s. On Neo4j it applies only when set; unset leaves Neo4j writes unbounded and each writer logs graph.write_timeout.unbounded at startup (issue #7046). Keep lease TTLs that guard graph writes above this value."},
	...
	{Name: "ESHU_GOVERNANCE_MODE", Type: VarEnum, Default: "local_no_policy", Subsystem: "api", Allowed: []string{"local_no_policy", "hosted_single_tenant", "hosted_multi_tenant"}, Description: "Hosted governance posture for the deployment. Both cmd/api and cmd/mcp-server read it: it is echoed in the governance status readback, and it decides whether a tenant-bound all-scope credential may use routes whose handlers filter reads by repository or scope grant. hosted_multi_tenant -- and any unrecognized value -- refuses such a caller with 403 permission_denied and records scoped_route_all_scope_grant_required; local_no_policy, hosted_single_tenant, and an unset mode admit it, the intended posture where one graph belongs to one tenant. Credentials carrying real repository or scope ids are unaffected in every mode."},
	...
	{Name: "ESHU_GRAPH_GAUGE_REFRESH_INTERVAL", Type: VarDuration, Default: "5m", Subsystem: "reducer", Description: "Delay between background refreshes of the graph-backed observable gauges (eshu_dp_edges_by_source_tool, eshu_dp_files_by_language, eshu_dp_graph_orphan_nodes). /metrics serves the last snapshot, so while refreshes succeed those gauges lag the graph by up to this interval plus one read. If refreshes keep failing the gauges stop reporting once the snapshot is older than three intervals; eshu_dp_gauge_snapshot_age_seconds shows the age."},
	{Name: "ESHU_GRAPH_GAUGE_REFRESH_TIMEOUT", Type: VarDuration, Default: "30s", Subsystem: "reducer", Description: "Deadline for each background graph read that refreshes a graph-backed observable gauge. A read that exceeds it is cancelled and counted as a timeout in eshu_dp_gauge_snapshot_refreshes_total; the previous snapshot keeps being served until it is three intervals old."},
	...
	{Name: "ESHU_INFRA_INVENTORY_RECONCILE_ENABLED", Type: VarBool, Default: "true", Subsystem: "reducer", Description: "Run the infra read model reconcile loop, which re-derives repositories whose infra_resource_entities rows drifted from content_entities, and repairs rolling-upgrade fence marks. With it false, nothing repairs rolling-upgrade fence marks: one write from an older binary or manual SQL keeps unscoped infra aggregate reads on the graph until the loop runs again (see eshu_dp_infra_inventory_dirty_repos and the infra_inventory admin status field)."},
	{Name: "ESHU_INFRA_INVENTORY_RECONCILE_INTERVAL", Type: VarDuration, Default: "5m", Subsystem: "reducer", Description: "Wait between infra read model reconcile cycles."},
	{Name: "ESHU_INFRA_INVENTORY_RECONCILE_REPO_BUDGET", Type: VarInt, Default: "500", Subsystem: "reducer", Description: "Repositories one infra read model reconcile cycle checks."},
	...
	{Name: "ESHU_CODE_CALL_PROJECTION_LEASE_OWNER", Type: VarString, Default: "code-call-projection-runner", Subsystem: "reducer", Description: "Prefix for the code-call partition lease owner. The reducer appends hostname, PID, and a boot nonce so replicas and restarted processes never share one active owner identity."},
	...
	{Name: "ESHU_PRIVATE_IDENTIFIERS_FILE", Type: VarString, Subsystem: "backend-conformance", Description: "Path to a file outside git listing organisation and product identifiers, one per line, that the cassette private-data gate (scripts/verify-cassette-author.sh) must not find in testdata/cassettes; unset scans for the committed canary only and says so on stderr (#6965)."},
	{Name: "ESHU_PRIVATE_IDENTIFIERS_REQUIRED", Type: VarString, Subsystem: "backend-conformance", Description: "Set to 1 to make the cassette private-data gate fail when ESHU_PRIVATE_IDENTIFIERS_FILE is unset, instead of running the identifier check with the canary only (#6965)."},
	{Name: "ESHU_RECORD_PSEUDONYM_KEY", Type: VarString, Subsystem: "backend-conformance", Description: "Recording key for collector -mode=record pseudonymization (#6965 Phase 3): at least 32 bytes, generated with `openssl rand -hex 32`, one key per cassette corpus so cross-cassette joins survive, kept with the corpus's other secrets and never committed; only its 8-hex fingerprint is written to the cassette and logged."},
