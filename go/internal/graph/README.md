...
- Package-registry projection labels (`Package`, `PackageVersion`,
  ...
  `PackageRegistryRegistryEvent`) receive `uid` constraints. Secondary
  ...
  ID, artifact package ID, registry-event version ID, and registry-event
  package ID keep bounded package query surfaces from falling back to label
  scans.
