...
func TestDeployableUnitFamilyCassetteDerivesTheExpectedEdgeSet(t *testing.T) {
	...
	odu, err := ifa.LoadDeployableUnitFamilyOdu(ifa.DeployableUnitFamilyCassetteFullPath(repoRoot))
	if err != nil {
		t.Fatalf("ifa.LoadDeployableUnitFamilyOdu: %v", err)
	}
...
// ifa.CatalogByName()/the Resolve dispatch arm: those wire through
...
func TestDeployableUnitFamilyCassetteMatchesCompiledCatalog(t *testing.T) {
	...
	compiled := ifa.CatalogByName()[deployableUnitFamilyOduName]
	fromCassette, err := ifa.LoadDeployableUnitFamilyOdu(ifa.DeployableUnitFamilyCassetteFullPath(repoRoot))
	if err != nil {
		t.Fatalf("ifa.LoadDeployableUnitFamilyOdu: %v", err)
	}
...
func TestDeployableUnitFamilyOduPreservesEnvelopeFields(t *testing.T) {
	...
	raw, err := os.ReadFile(ifa.DeployableUnitFamilyCassetteFullPath(repoRoot))
	if err != nil {
