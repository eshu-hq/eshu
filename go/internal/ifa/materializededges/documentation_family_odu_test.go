...
func TestDocumentationFamilyIsCatalogedAndResolvable(t *testing.T) {
	...
	fromCassette, err := ifa.LoadDocumentationFamilyOdu(ifa.DocumentationFamilyCassetteFullPath(repoRoot))
	if err != nil {
		t.Fatalf("ifa.LoadDocumentationFamilyOdu: %v", err)
	}
...
func TestDocumentationFamilyCassetteDerivesTheExpectedEdgeSet(t *testing.T) {
	...
	odu, err := ifa.LoadDocumentationFamilyOdu(ifa.DocumentationFamilyCassetteFullPath(repoRoot))
	if err != nil {
		t.Fatalf("ifa.LoadDocumentationFamilyOdu: %v", err)
	}
...
func TestLoadDocumentationFamilyOduRejectsUnknownJSONField(t *testing.T) {
  ...
  "scopes": [
	...
	if _, err := ifa.LoadDocumentationFamilyOdu(goodPath); err != nil {
		t.Fatalf("ifa.LoadDocumentationFamilyOdu rejected a well-formed cassette carrying every real envelope field: %v", err)
	}
	...
	if _, err := ifa.LoadDocumentationFamilyOdu(badPath); err == nil {
		t.Fatal("ifa.LoadDocumentationFamilyOdu accepted a cassette with an unknown field (fact_knd typo)")
	}
}
