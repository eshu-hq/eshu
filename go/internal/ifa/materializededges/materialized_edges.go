...
// coverage always adds its own small vacuity-guard function too.
type MaterializedEdgeOduResolver struct {
	// Catalog indexes every cataloged Odù by name (Catalog()/ifa.CatalogByName()).
	Catalog map[string]ifa.Odu
...
func (r MaterializedEdgeOduResolver) Resolve(entry replaycoverage.CoverageEntry) (bool, string) {
	...
	case "inheritance_edges":
		return resolveInheritanceMaterializedEdges(odu, inheritanceFamilyExpectedEdgesPath(r.RepoRoot))
	...
	//
	// A registered guard is NOT a coverage claim. Since #6309 two of the four
	// families carry coverage rows instead of waivers in
	// specs/ifa-materialized-edge-coverage-direct.v1.yaml because both live
	// matrices drive them; workload_cloud_relationship and iam_can_assume
	// still carry their waiver rows. What these arms buy is that a coverage
	// row COULD resolve, which is one of the three things those waiver reasons
	// said was missing.
