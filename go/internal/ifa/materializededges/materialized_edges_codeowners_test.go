...
// field is NOT validated against Catalog()/ifa.CatalogByName() or anything else
...
func TestCodeownersFamilyCassetteDerivesTheExpectedEdgeSet(t *testing.T) {
	...
	odu, err := ifa.LoadCodeownersFamilyOdu(ifa.CodeownersFamilyCassetteFullPath(repoRoot))
	if err != nil {
		t.Fatalf("ifa.LoadCodeownersFamilyOdu: %v", err)
	}
...
// catalog_seed.go's catalogSeed slice adds ifa.CodeownersFamilyOdu(), and
...
func TestCodeownersFamilyIsCatalogedAndResolvable(t *testing.T) {
	...
	compiled, ok := catalog[ifa.CodeownersFamilyOduName]
	if !ok {
		t.Fatalf("CatalogByName omits %q -- restore `ifa.CodeownersFamilyOdu(),` in catalog_seed.go's catalogSeed; without it the installed binary cannot resolve this family's manifest row", ifa.CodeownersFamilyOduName)
	}
	fromCassette, err := ifa.LoadCodeownersFamilyOdu(ifa.CodeownersFamilyCassetteFullPath(repoRoot))
	if err != nil {
		t.Fatalf("ifa.LoadCodeownersFamilyOdu: %v", err)
	}
	...
	ok2, detail := (MaterializedEdgeOduResolver{Catalog: catalog, RepoRoot: repoRoot}).Resolve(replaycoverage.CoverageEntry{
		...
		Ref:          ifa.CodeownersFamilyOduName,
	})
	if !ok2 {
...
// TestCodeownersFamilyOduBuildsFiveRuleFacts exercises ifa.CodeownersFamilyOdu
...
func TestCodeownersFamilyOduBuildsFiveRuleFacts(t *testing.T) {
	t.Parallel()
	catalogOdu := ifa.CodeownersFamilyOdu()
	if catalogOdu.Odu.Name != ifa.CodeownersFamilyOduName {
		t.Fatalf("ifa.CodeownersFamilyOdu().Odu.Name = %q, want %q", catalogOdu.Odu.Name, ifa.CodeownersFamilyOduName)
	}
	...
	if len(catalogOdu.Odu.Facts) != 7 {
		t.Fatalf("ifa.CodeownersFamilyOdu() carries %d facts, want 7 (1 repository + 5 rules + 1 shared_followup)", len(catalogOdu.Odu.Facts))
	}
}
...
func TestResolveCodeownersOwnershipMaterializedEdgesAcceptsTheCompiledOdu(t *testing.T) {
	t.Parallel()
	repoRoot := repoRootDir(t)
	ok, detail := resolveCodeownersOwnershipMaterializedEdges(ifa.CodeownersFamilyOdu().Odu, codeownersFamilyExpectedEdgesPath(repoRoot))
	if !ok {
...
func TestResolveCodeownersOwnershipMaterializedEdgesReturnsZeroRowsFalse(t *testing.T) {
	...
	emptyOdu := ifa.Odu{Name: "odu:ifa-codeowners-empty-probe", Facts: []facts.Envelope{ifa.CodeownersFamilyRepositoryFact()}}
  ...
  "edges": [{"relationship_type": "DECLARES_CODEOWNER", "source_entity_id": "r", "target_entity_id": "t", "identity": {"pattern": "*.md", "source_path": ".github/CODEOWNERS", "order_index": "1"}}]
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ok, detail := resolveCodeownersOwnershipMaterializedEdges(ifa.CodeownersFamilyOdu().Odu, path)
			if ok {
