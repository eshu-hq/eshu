...
var (
	_ func(codegraphv1.Repository) facts.Envelope = ifa.RationaleFamilyRepositoryFact
	_ func(codegraphv1.File) facts.Envelope       = ifa.RationaleFamilyFileFact
)
...
func TestRationaleCanonicalTargetIDLiterals(t *testing.T) {
	...
	tests := []struct {
		...
		{ifa.RationaleFamilyChargePath, ifa.RationaleFamilyChargeName, ifa.RationaleFamilyChargeLine, "content-entity:e_763200c9adc3"},
		{ifa.RationaleFamilyInvoicePath, ifa.RationaleFamilyInvoiceName, ifa.RationaleFamilyInvoiceLine, "content-entity:e_2dc98238d686"},
		{ifa.RationaleFamilyRefundPath, "refund", 3, "content-entity:e_a6fb2b86785c"},
		{ifa.RationaleFamilyReconcilePath, "reconcile", 3, "content-entity:e_6861e03e94da"},
		{ifa.RationaleFamilyHealthcheckPath, "healthcheck", 1, "content-entity:e_02d9fec13a93"},
	}
	for _, test := range tests {
		got := content.CanonicalEntityID(ifa.RationaleFamilyRepoID, test.path, "Function", test.name, test.line)
		if got != test.want {
			t.Errorf("CanonicalEntityID(%q, %q, Function, %q, %d) = %q, want %q", ifa.RationaleFamilyRepoID, test.path, test.name, test.line, got, test.want)
		}
	}
}
...
func TestRationaleCassettePositivesAreParserReachablePython(t *testing.T) {
	...
	tests := []struct {
		...
		{
			path: ifa.RationaleFamilyChargePath, name: ifa.RationaleFamilyChargeName, line: ifa.RationaleFamilyChargeLine,
		...
		{
			path: ifa.RationaleFamilyInvoicePath, name: ifa.RationaleFamilyInvoiceName, line: ifa.RationaleFamilyInvoiceLine,
			source: "# HACK: Invoices are immutable once issued.\n" +
				"def issue_invoice():\n    pass\n",
			comments: []map[string]any{{"kind": "HACK", "text": "Invoices are immutable once issued."}},
		},
		{path: ifa.RationaleFamilyRefundPath, name: "refund", line: 3, source: "# why: lowercase markers are ignored\n# CAVEAT: unsupported markers are ignored\ndef refund():\n    pass\n"},
		{path: ifa.RationaleFamilyReconcilePath, name: "reconcile", line: 3, source: "# FIXME: detached by a blank line\n\ndef reconcile():\n    pass\n"},
		{path: ifa.RationaleFamilyHealthcheckPath, name: "healthcheck", line: 1, source: "def healthcheck():\n    pass\n"},
	}
	...
	for _, test := range tests {
		...
		t.Run(test.path, func(t *testing.T) {
			...
			wantID := content.CanonicalEntityID(ifa.RationaleFamilyRepoID, test.path, "Function", test.name, test.line)
...
func TestRationaleCassetteEnqueuesMaterializationHandler(t *testing.T) {
	...
	if intent.ScopeID != ifa.RationaleFamilyScopeID || intent.GenerationID != ifa.RationaleFamilyGenerationID {
		t.Fatalf("rationale cassette intent scope/generation = %q/%q, want %q/%q", intent.ScopeID, intent.GenerationID, ifa.RationaleFamilyScopeID, ifa.RationaleFamilyGenerationID)
	}
}
...
func loadRationaleCassetteFacts(t *testing.T) []facts.Envelope {
	t.Helper()
	source, err := cassette.NewSource(filepath.Join(repoRootDir(t), ifa.RationaleFamilyCassetteRelPath))
	if err != nil {
