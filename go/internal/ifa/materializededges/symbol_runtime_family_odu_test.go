...
func TestSymbolRuntimeFamilyOduPreservesEnvelopeFields(t *testing.T) {
	...
	if len(odu.Facts) != 10 {
		t.Fatalf("expected 10 facts (1 repository, 3 file, 3 content_entity, 3 shared_followup), got %d", len(odu.Facts))
	}
	...
	want := map[string]int{"repository": 1, "file": 3, "content_entity": 3, "shared_followup": 3}
