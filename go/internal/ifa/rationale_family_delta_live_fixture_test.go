...
func TestRationaleDeltaExpectedFixtureIsLiveReadable(t *testing.T) {
	...
	if repoID != rationaleFamilyRepoID || len(records) != 1 {
		t.Fatalf("live-readable rationale delta fixture = repo:%q edges:%d, want %q/1",
			repoID, len(records), rationaleFamilyRepoID)
	}
}
...
func TestRationaleDeltaCassettePinsFourFactRefreshAndSurvivors(t *testing.T) {
	...
	for index, want := range wantKinds {
		...
		if envelopes[index].GenerationID != rationaleFamilyDeltaGenerationID {
			t.Errorf("rationale delta fact %d generation = %q, want %q", index, envelopes[index].GenerationID, rationaleFamilyDeltaGenerationID)
		}
	}

	stage := projector.ProjectWorkloadStage(envelopes)
	if got := stage.SourceRunPairs[rationaleFamilyRepoID]; got != rationaleFamilySourceRunID {
		t.Fatalf("rationale delta repository source run = %q, want %q", got, rationaleFamilySourceRunID)
	}
	...
	if intent := stage.Intents[0]; intent.ScopeID != rationaleFamilyScopeID || intent.GenerationID != rationaleFamilyDeltaGenerationID {
		t.Fatalf("rationale delta intent scope/generation = %q/%q, want %q/%q",
			intent.ScopeID, intent.GenerationID, rationaleFamilyScopeID, rationaleFamilyDeltaGenerationID)
	}
	...
	if expected.GenerationID != rationaleFamilyDeltaGenerationID || len(expected.Edges) != 1 || len(expected.RequiredNodes) != 1 {
		t.Fatalf("rationale delta expected inventory = generation:%q edges:%d nodes:%d, want %s/1/1",
			expected.GenerationID, len(expected.Edges), len(expected.RequiredNodes), rationaleFamilyDeltaGenerationID)
	}
	...
	entities := projector.ExtractEntityRows(envelopes, rationaleFamilyRepoID, rationaleFamilyLocalPath)
	if len(entities) != 1 {
...
func rationaleDeltaNodeFromEntity(entity projector.EntityRow) rationaleDeltaNode {
	return rationaleDeltaNode{
		...
		Props: map[string]any{
			...
			"scope_id": rationaleFamilyScopeID, "generation_id": rationaleFamilyDeltaGenerationID,
