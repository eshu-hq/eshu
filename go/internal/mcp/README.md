...
| `get_repo_summary` | GET | `/api/v0/repositories/{repo_id}/stats` — lightweight identity and coverage summary (file count, workload count, platform count, dependency count, indexing coverage state); use before `get_repo_context` for a quick overview |
...
| `ReadOnlyTools` | `types.go:11` | returns all 147 tool definitions |
