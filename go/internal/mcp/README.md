...
`admissiondecisions`, `kubernetes`, `infrasearch`, `impact`, and `codeflow` children own
only family membership and pure request selection; root keeps the matching `*Route`
adapters and dispatch, and each family's definitions stay with their current owners. The `ecosystem`
