...
`secretsiam`, `observabilitycoverage`, `containerimage`, and `supplychainimpact`
children own only family membership and pure request selection; root keeps their
definitions, the matching `*Route` adapters, and dispatch. The `ecosystem` child
package owns the 23 ecosystem, repository-context, infrastructure-impact, and
