...
| `list_sbom_attestation_attachments` | GET | `/api/v0/supply-chain/sbom-attestations/attachments` (repository id is forwarded to query and rejected; use `subject_digest`/`digest`, `document_id`, `document_digest`, `workload_id`, or `service_id` for SBOM proof; scoped workload/service reads return explicit missing image or image-to-SBOM evidence instead of silently flattening empty pages) |
