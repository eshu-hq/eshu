...
`secretsiam`, `observabilitycoverage`, and `containerimage` children own only
family membership and pure request selection; root keeps their definitions, the
matching `*Route` adapters, and dispatch. The `ecosystem` child package owns the
23 ecosystem, repository-context, infrastructure-impact, and change-planning
registration definitions. Their existing split routers stay here.
