...
family route selection; `internal/query` retains validation and graph reads. The
...
`observabilitycoverage`, `containerimage`, `supplychainimpact`, `securityalert`, and
`admissiondecisions` children own only family membership and pure request selection;
root keeps their definitions, the matching `*Route` adapters, and dispatch. The `ecosystem`
child package owns the 23 ecosystem, repository-context, infrastructure-impact, and
change-planning registration definitions. Their existing split routers stay here. The
`ask` child owns natural-language answer registration and pure request selection; global
fanout and dispatch stay here, while query executes answers.
