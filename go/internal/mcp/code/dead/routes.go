...
// defaultLimit is the MCP default for `limit` on all three dead-code tools. It
// is sized to the MCP response budget, not to the HTTP handler: 100 candidate
// rows overran the 256 KiB dispatch budget on most measured repositories
// (#7168); 25 keeps the worst measured repository well inside it. The HTTP
// handlers keep their own default of 100 because an HTTP caller has no byte
...
// 100, and the response stays inside the dispatch byte budget. offset defaults to 0, the
