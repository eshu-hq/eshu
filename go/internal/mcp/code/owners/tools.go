...
// Tools returns the CODEOWNERS ownership tool definition owned by this
// package. The root package splices it into ReadOnlyTools at its
// long-standing position via the codeownersTools wrapper, so the
// client-visible registration order is unchanged.
func Tools() []toolcontract.ToolDefinition {
	return []toolcontract.ToolDefinition{
		codeownersOwnershipTool(),
	}
}

func codeownersOwnershipTool() toolcontract.ToolDefinition {
	return toolcontract.ToolDefinition{
