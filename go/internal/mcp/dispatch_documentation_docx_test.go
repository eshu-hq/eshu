...
func TestDispatchToolListsDOCXDocumentationFacts(t *testing.T) {
	t.Parallel()
	...
	mux.HandleFunc("GET /api/v0/documentation/facts", func(w http.ResponseWriter, r *http.Request) {
		...
		writeDOCXDocumentationEnvelope(w)
	})
	...
	if got, want := query.StringVal(payload, "content_format"), "docx"; got != want {
		t.Fatalf("payload.content_format = %q, want %q", got, want)
	}
}

func writeDOCXDocumentationEnvelope(w http.ResponseWriter) {
