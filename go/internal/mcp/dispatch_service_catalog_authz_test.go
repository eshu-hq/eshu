...
func TestDispatchToolServiceCatalogCorrelationsAllowsScopedRoute(t *testing.T) {
	t.Parallel()

	mux := http.NewServeMux()
	mux.HandleFunc("GET /api/v0/service-catalog/correlations", func(w http.ResponseWriter, r *http.Request) {
		...
		query.WriteSuccess(w, r, http.StatusOK, map[string]any{
			...
			"test service catalog correlations route",
		))
	})
