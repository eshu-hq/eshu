			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"generation_id": map[string]any{
						"type":        "string",
						"description": "Optional. Omit to read the scope's active generation (each scope's active generation for anchor-only calls); superseded generations are not returned as current facts. Set to read that exact generation, even if superseded; data.generation_binding and truth.freshness label it.",
					},
					"repo":        map[string]any{"type": "string"},
					"target_kind": map[string]any{"type": "string"},
					"target_id":   map[string]any{"type": "string"},
