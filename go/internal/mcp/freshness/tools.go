		...
		},
		{
			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"scope_id": map[string]any{
						"type":        "string",
						"description": "Ingestion scope whose lineage of service_id to diff. Needed only when the service id has a lineage in more than one scope the caller may read; the ambiguity error lists them.",
					},
					"since_generation_id": map[string]any{
						"type":        "string",
