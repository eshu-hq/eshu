...
Groups MCP infrastructure-as-code route selection. The `management` leaf
selects the five IaC tools (`find_dead_iac`, `find_unmanaged_resources`,
`get_iac_management_status`, `explain_iac_management_status`,
`propose_terraform_import_plan`), mapping decoded arguments to a
...
Rename-only `iacmanagement` to `iac/management` move. Every exported Go
