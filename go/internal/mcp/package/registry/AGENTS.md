# AGENTS.md — MCP package-registry route guidance
...
4. `../../dispatch_package_registry.go` and
   `../../dispatch_package_registry_contract_test.go` for the root adapter and the
...
- Keep only package-registry family membership and pure argument-to-request
  selection here. Global route fanout, the private adapter, and execution stay
  in the parent MCP package and `internal/query`.
- Keep the package clause as `package packageregistrytools`; the root imports it
  with an explicit alias.
- Preserve the six tool names and their exact method, path, and query keys:
  `/api/v0/package-registry/packages`, `.../packages/count`,
  `.../packages/inventory`, `.../versions`, `.../dependencies`, and
  `.../correlations`, all `GET` with no body.
- Preserve the `limit` defaults (50 on listings, 100 on inventory), the `offset`
  default of 0, and the `group_by` fallback to `ecosystem`.
- Return the zero request and `handled=false` for unrelated tools, including
  near-miss names that share the `list_package_registry` prefix.
...
```bash
go test ./internal/mcp/package/registry ./internal/mcp -count=1
go vet ./internal/mcp/package/registry ./internal/mcp
```
