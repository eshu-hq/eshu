...
func CodeTools() []toolcontract.ToolDefinition {
	return []toolcontract.ToolDefinition{
		codeRelationshipStoryTool(),
		{
			...
			Description: "Analyze code relationships like 'who calls this function' or 'class hierarchy'. Relationship-story query types return per-row provenance blocks. Supported query types include: find_callers, find_callees, find_all_callers, find_all_callees, find_cross_repo_callers, find_cross_repo_callees, find_importers, find_cross_repo_importers, who_modifies, class_hierarchy, cross_repo_class_hierarchy, overrides, cross_repo_overrides, dead_code, call_chain, find_cross_repo_call_chain, module_deps, variable_scope, find_complexity, find_functions_by_argument, find_functions_by_decorator. Every query type returns only granted repositories for a scoped token and rejects an ungranted repository selector. The remaining query types (who_modifies, module_deps, variable_scope, find_complexity, find_functions_by_argument, find_functions_by_decorator) dispatch to POST /api/v0/code/relationships with repo_id, target as the entity name, and an optional exact entity_id; that route has no limit or offset, so those are not applied to these query types (dead_code dispatches to POST /api/v0/code/dead-code): for a scoped token an entity outside the grant answers not found, and neighbours outside the grant, or with no repository, are omitted. Those responses carry outgoing_truncated and incoming_truncated; on the graph paths true means more neighbours exist in that direction than were returned, and on the content fallback it means a lookup reached its ceiling, so the list may be incomplete.",
