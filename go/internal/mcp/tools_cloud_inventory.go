...
// is read-only and never returns raw provider locators, tags, or credentials.
func cloudInventoryTools() []ToolDefinition {
	return []ToolDefinition{
		{
			Name:        "list_cloud_resource_inventory",
			Description: "List canonical multi-cloud resource identities (reducer_cloud_resource_identity) by bounded provider, scope, and management_origin filters. Returns provider, normalized identity, management_origin, per-layer evidence flags, and provider-neutral source state. Unsupported on lightweight local runtime.",
