...
// is read-only and never returns raw provider locators, tags, identities,
// assignment scopes, or credentials.
func cloudInventoryTools() []ToolDefinition {
	return []ToolDefinition{
		{
			Name:        "list_cloud_resource_inventory",
			Description: "List canonical multi-cloud resource identities (reducer_cloud_resource_identity) by bounded provider, scope, and management_origin filters. Returns provider, normalized identity, management_origin, per-layer evidence flags, provider-neutral source state, optional keyed tag fingerprints, and optional bounded identity-policy evidence. Unsupported on lightweight local runtime.",
