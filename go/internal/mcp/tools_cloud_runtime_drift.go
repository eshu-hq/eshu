	...
	return []ToolDefinition{
		{
			...
			Description: "List runtime drift findings for a bounded canonical scope across aws, gcp, and azure, aggregating reducer_multi_cloud_runtime_drift_finding (gcp, azure) and reducer_aws_cloud_runtime_drift_finding (aws) in one query. Filterable by provider, canonical cloud_resource_uid, and finding_kind; cloud_resource_uid filtering matches only gcp/azure findings. Returns provider, normalized identity, finding_kind, management_status, provider-neutral source state, and refusal-safety posture; AWS-origin findings also carry AWS-only IaC-source enrichment (matched_terraform_config_file, matched_terraform_module_path, matched_other_iac_source, service_candidates, environment_candidates, dependency_paths) that gcp/azure findings never carry. Unsafe findings are reported as rejected, not omitted. Unsupported on lightweight local runtime.",
