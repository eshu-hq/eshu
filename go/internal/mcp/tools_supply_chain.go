			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"cursor": map[string]any{
						"type":        "string",
						"description": "Continuation token from a truncated page's next_cursor. Pass it back verbatim: it is an authenticated-encryption envelope over a row key, so only a token this deployment issued for this image_ref opens. Edited, synthesized, truncated, foreign-image_ref and pre-rotation tokens all get a 400 -- restart from page one. There is no row position inside it, so it is not bound to the limit it was issued for, and a key matching no current row is not an error. This is the only continuation a scoped (personal-token) caller may use, and on a server with no sealing key configured that caller gets a 503 naming the variable instead.",
					},
