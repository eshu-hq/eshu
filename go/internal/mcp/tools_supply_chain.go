			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"cursor": map[string]any{
						"type":        "string",
						"description": "Continuation token from a truncated page's next_cursor. Pass it back verbatim and treat it as opaque by convention; it is not sealed, and nothing here claims an edit is detected. It names a row key rather than a row position, so it is bound to this image_ref but NOT to the limit it was issued for, and a key matching no current row is not an error. A malformed token, an unknown-version one (which includes every pre-keyset token), or one issued for another image is rejected with a 400. This is the only continuation a scoped (personal-token) caller may use.",
					},
