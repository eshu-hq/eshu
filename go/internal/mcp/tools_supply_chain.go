			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"cursor": map[string]any{
						"type":        "string",
						"description": "Opaque continuation token from a truncated page's next_cursor. Pass it back verbatim; it is bound to this image_ref and this limit, and a malformed token, or one issued for another image or another limit, is rejected with a 400. This is the only continuation a scoped (personal-token) caller may use.",
					},
