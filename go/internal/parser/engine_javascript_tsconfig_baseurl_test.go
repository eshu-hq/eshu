...
func TestDefaultEngineParsePathTypeScriptAnnotatesBaseURLImportFromJSONCTSConfig(t *testing.T) {
	t.Parallel()

	repoRoot := t.TempDir()
	filePath := filepath.Join(repoRoot, "src", "handlers", "token.ts")
	writeTestFile(t, filepath.Join(repoRoot, "tsconfig.json"), `{
  // TypeScript accepts JSONC comments in tsconfig.
  "compilerOptions": {
    "baseUrl": "src",
  },
}
`)
	writeTestFile(t, filepath.Join(repoRoot, "src", "resources", "jwt.ts"), `export const encode = () => "";
`)
	writeTestFile(t, filePath, `import * as jwt from "resources/jwt";
...
`)
	...
	jwtImport := findNamedBucketItem(t, got, "imports", "*")
	assertStringFieldValue(t, jwtImport, "source", "resources/jwt")
	assertStringFieldValue(t, jwtImport, "resolved_source", "src/resources/jwt.ts")
}
