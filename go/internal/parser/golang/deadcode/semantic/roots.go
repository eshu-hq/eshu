...
func CollectRoots(
	...
	structFieldTypes := symbols.StructFieldConcreteTypes(root, source, structTypes)
	structFieldTargets := goStructFieldInterfaceTargets(root, source, interfaceMethods)
	functionParamTargets := symbols.FunctionParamInterfaceTargets(root, source, interfaceMethods)
	...
	// follow-up).
	variableTypeIndex := symbols.BuildVariableTypeIndex(root, source, structTypes, constructorReturns, lookup)
	...
	for _, node := range gatheredVarSpecs {
		...
		for _, interfaceName := range symbols.ReferencedLocalInterfaces(typeNode, source, interfaceMethods) {
	...
	for _, node := range gatheredCompositeLiterals {
		scopedVariableTypes := variableTypeIndex.ForNode(root, node)
		if concreteType := symbols.ConcreteTypeFromTypeNode(node.ChildByFieldName("type"), source, structTypes); concreteType != "" {
			structRootKinds[concreteType] = symbols.AppendUniqueImportAlias(structRootKinds[concreteType], "go.type_reference")
		}
		typeNode := node.ChildByFieldName("type")
		for _, interfaceName := range symbols.ReferencedLocalInterfaces(typeNode, source, interfaceMethods) {
			interfaceRootKinds[interfaceName] = symbols.AppendUniqueImportAlias(interfaceRootKinds[interfaceName], "go.interface_type_reference")
	...
	for _, node := range gatheredParamDecls {
		for _, interfaceName := range symbols.ReferencedLocalInterfaces(node.ChildByFieldName("type"), source, interfaceMethods) {
			interfaceRootKinds[interfaceName] = symbols.AppendUniqueImportAlias(interfaceRootKinds[interfaceName], "go.interface_type_reference")
		}
	}
		...
		for interfaceName, methodNames := range interfaceMethods {
			...
			for _, methodName := range methodNames {
				for methodKey := range methodKeys {
					...
					functionRootKinds[methodKey] = symbols.AppendUniqueImportAlias(functionRootKinds[methodKey], "go.generic_constraint_method")
					structRootKinds[receiver] = symbols.AppendUniqueImportAlias(structRootKinds[receiver], "go.interface_implementation_type")
				}
			}
		}
	}
...
func goTypeParameterMentionsConstraint(text string, interfaceName string) bool {
	for _, field := range symbols.TypeParameterConstraintCandidates(text) {
...
func goCollectDependencyInjectionCallbacksFromCall(
	...
	argIndex := 0
	symbols.WalkDirectNamed(argumentsNode, func(child *tree_sitter.Node) {
		if child.Kind() == "," {
			return
		}
...
func goCollectFunctionCallbackArgument(
	...
	case "selector_expression":
		...
		if _, ok := methodKeys[key]; ok {
			functionRootKinds[key] = symbols.AppendUniqueImportAlias(functionRootKinds[key], "go.dependency_injection_callback")
		}
	}
}
