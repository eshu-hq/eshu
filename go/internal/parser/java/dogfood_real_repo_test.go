...
// header comment) -- and asserts the parser's per-file, row-level output
// matches the checked-in snapshot at
// testdata/dogfood_real_repo_snapshot.txt. The snapshot pins one line per
// emitted entity/relationship (functions, classes, interfaces, annotations,
...
// (Spring method, path, handler), not merely the per-bucket counts, so a
// regression that corrupts an identifier or route while preserving the counts
// still fails the snapshot (see
// TestDogfoodJavaRowLevelCatchesCountPreservingCorruption).
//
...
// TestDogfoodJavaRowLevelCatchesCountPreservingCorruption pins the reason the
...
func TestDogfoodJavaRowLevelCatchesCountPreservingCorruption(t *testing.T) {
	t.Parallel()

	clean := map[string]any{
		"functions": []map[string]any{
			{"name": "index", "line_number": 10, "lang": "java"},
			{"name": "show", "line_number": 20, "lang": "java"},
		},
	}
	...
	corrupt := map[string]any{
		"functions": []map[string]any{
			{"name": "index", "line_number": 20, "lang": "java"},
			{"name": "show", "line_number": 10, "lang": "java"},
		},
	}
