...
`)
	...
	got, err := engine.ParsePath(repoRoot, requestPath, false, parser.Options{})
	if err != nil {
...
`)
	...
	got, err := engine.ParsePath(repoRoot, contextPath, false, parser.Options{})
	if err != nil {
		t.Fatalf("ParsePath() error = %v, want nil", err)
	}
...
`)
	got, err = engine.ParsePath(repoRoot, typeProviderPath, false, parser.Options{})
	if err != nil {
