...
`,
	...
	seedPayload, err := engine.ParsePath(repoRoot, seedPath, false, parser.Options{})
	if err != nil {
		t.Fatalf("ParsePath(seed) error = %v, want nil", err)
	}
	consumerPayload, err := engine.ParsePath(repoRoot, consumerPath, false, parser.Options{})
	if err != nil {
