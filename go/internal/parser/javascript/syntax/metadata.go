...
// Docstring returns the normalized `/** ... */` JSDoc comment immediately
// preceding node, or "" when node has no leading JSDoc block. A blank line
// between the comment and node, or a `/* ... */` block that is not a JSDoc
// block, breaks the association.
...
func normalizeDocstring(lines []string) string {
	if len(lines) == 0 {
		return ""
	}
...
// FunctionKind classifies a function/method node as "async", "generator",
// "getter", or "setter" from its syntax, or "" when none of those modifiers
// apply.
func FunctionKind(node *tree_sitter.Node, source []byte) string {
	if node == nil {
		return ""
	}
	...
	case "function_declaration", "function_expression", "arrow_function":
		declaration := strings.TrimSpace(shared.NodeText(node, source))
	...
	case "method_definition", "method_signature":
		return methodKind(node)
	}

	return ""
}

// methodKind classifies a method_definition/method_signature node from its
// leading modifier tokens. The tree-sitter grammar emits get/set/async and
...
func methodKind(node *tree_sitter.Node) string {
	if node == nil {
		return ""
	}
