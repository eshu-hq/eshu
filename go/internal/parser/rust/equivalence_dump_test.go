...
// Throwaway before/after equivalence harness for epic #4831 child #4840. Run
// only when RUST_PARSE_DUMP is set; not part of the normal test suite.
...
// TestDumpRustParseCorpus parses every .rs fixture under RUST_PARSE_CORPUS
...
// RUST_PARSE_DUMP, sorted by that line text. Comparing dumps taken before and
// after a change proves byte-identical parser output.
func TestDumpRustParseCorpus(t *testing.T) {
	dumpPath := os.Getenv("RUST_PARSE_DUMP")
	if dumpPath == "" {
		t.Skip("RUST_PARSE_DUMP not set")
	}

	corpusRoot := os.Getenv("RUST_PARSE_CORPUS")
	if corpusRoot == "" {
	...
	if len(paths) == 0 {
		t.Fatalf("no .rs fixtures found under %q", corpusRoot)
	}
	sort.Strings(paths)
