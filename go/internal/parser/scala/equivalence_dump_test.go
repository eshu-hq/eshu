...
// consolidation). Not run by default: set SCALA_PARSE_DUMP to a destination
...
// scalaEquivalenceCorpus returns every ".scala" fixture under
// SCALA_PARSE_CORPUS (default "../../../../tests/fixtures"), sorted for
// deterministic output.
func scalaEquivalenceCorpus(tb testing.TB) []string {
	tb.Helper()
	root := os.Getenv("SCALA_PARSE_CORPUS")
	if root == "" {
		root = "../../../../tests/fixtures"
	}
	var paths []string
	...
	if len(paths) == 0 {
		tb.Skipf("no scala corpus under %q", root)
	}
	sort.Strings(paths)
	return paths
}

// scalaEquivalenceVariants are the Options combinations the dump proves
...
var scalaEquivalenceVariants = []struct {
	name    string
...
// TestDumpScalaParseCorpus is a guarded, non-default test: it only runs and
// writes a dump when SCALA_PARSE_DUMP names an output file. Use it to
...
func TestDumpScalaParseCorpus(t *testing.T) {
	out := os.Getenv("SCALA_PARSE_DUMP")
	if out == "" {
		t.Skip("SCALA_PARSE_DUMP not set; skipping corpus dump")
	}

	paths := scalaEquivalenceCorpus(t)
	...
	lines := make([]string, 0, len(paths)*len(scalaEquivalenceVariants))
	for _, path := range paths {
		for _, variant := range scalaEquivalenceVariants {
