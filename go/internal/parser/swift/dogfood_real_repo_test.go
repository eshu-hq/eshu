...
// file's own header comment) -- and asserts the parser's per-file, row-level
// output matches the checked-in snapshot at
// testdata/dogfood_real_repo_snapshot.txt. The snapshot pins one line per
// emitted entity/relationship (functions, classes, structs, protocols,
// imports, function_calls, variables) with its identifying fields -- name,
// line number, call target -- plus any framework_semantics route evidence
// (Vapor method, path, handler), not merely the per-bucket counts, so a
// regression that corrupts an identifier or route while preserving the counts
// still fails the snapshot (see
// TestDogfoodSwiftRowLevelCatchesCountPreservingCorruption).
//
...
// TestDogfoodSwiftRowLevelCatchesCountPreservingCorruption pins the reason the
...
func TestDogfoodSwiftRowLevelCatchesCountPreservingCorruption(t *testing.T) {
	t.Parallel()

	clean := map[string]any{
		"functions": []map[string]any{
			{"name": "index", "line_number": 10, "lang": "swift"},
			{"name": "show", "line_number": 20, "lang": "swift"},
		},
	}
	...
	corrupt := map[string]any{
		"functions": []map[string]any{
			{"name": "index", "line_number": 20, "lang": "swift"},
			{"name": "show", "line_number": 10, "lang": "swift"},
		},
	}
