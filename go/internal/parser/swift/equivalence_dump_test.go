...
// consolidation). Not run by default: set SWIFT_PARSE_DUMP to a destination
...
// swiftEquivalenceCorpus returns every ".swift" fixture under
// SWIFT_PARSE_CORPUS (default "../../../../tests/fixtures"), sorted for
// deterministic output.
func swiftEquivalenceCorpus(tb testing.TB) []string {
	tb.Helper()
	root := os.Getenv("SWIFT_PARSE_CORPUS")
	if root == "" {
		root = "../../../../tests/fixtures"
	}
	var paths []string
	...
	if len(paths) == 0 {
		tb.Skipf("no swift corpus under %q", root)
	}
	sort.Strings(paths)
	return paths
}

// swiftEquivalenceVariants are the Options combinations the dump proves
...
var swiftEquivalenceVariants = []struct {
	name    string
...
// TestDumpSwiftParseCorpus is a guarded, non-default test: it only runs and
// writes a dump when SWIFT_PARSE_DUMP names an output file. Use it to
...
func TestDumpSwiftParseCorpus(t *testing.T) {
	out := os.Getenv("SWIFT_PARSE_DUMP")
	if out == "" {
		t.Skip("SWIFT_PARSE_DUMP not set; skipping corpus dump")
	}

	paths := swiftEquivalenceCorpus(t)
	...
	lines := make([]string, 0, len(paths)*len(swiftEquivalenceVariants))
	for _, path := range paths {
		for _, variant := range swiftEquivalenceVariants {
