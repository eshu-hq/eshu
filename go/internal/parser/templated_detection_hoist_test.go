...
// TestInferRootFamilyMarkersMatchDirectRegexScan proves the contentMarkers
// values inferContentMetadata computes and hands to inferRootFamily are
// exactly what a direct, unfiltered MatchString probe against each marker
// regex would report -- the same values inferRootFamily computed internally
// before the #4805 hoist. This guards the hoist's core equivalence
// invariant directly, independent of any downstream bucket/artifact-type
// logic.
func TestInferRootFamilyMarkersMatchDirectRegexScan(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "go expression", content: "value: {{ .Values.name }}\n"},
		{name: "jinja statement", content: "{% if debug %}\ndebug: true\n{% endif %}\n"},
		{name: "jinja comment", content: "{# a comment #}\nkey: value\n"},
		{name: "tf interpolation", content: "ami = \"${var.ami_id}\"\n"},
		{name: "tf directive", content: "flag = %{ if enabled }true%{ endif }\n"},
		{name: "tf templatefile", content: "data = templatefile(\"x.tpl\", {})\n"},
		{name: "escaped tf interpolation only", content: "literal = \"$${not_a_var}\"\n"},
		{name: "github actions expression", content: "run: echo ${{ github.ref }}\n"},
		{name: "mixed go and jinja", content: "{{ .Values.name }} {% if x %}{% endif %}\n"},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			wantGoExpression := goExpressionRE.MatchString(tt.content)
			wantJinjaStatement := jinjaStatementRE.MatchString(tt.content)
			wantTFMarkers := tfInterpolationRE.MatchString(tt.content) ||
				tfDirectiveRE.MatchString(tt.content) ||
				tfTemplatefileRE.MatchString(tt.content)

			// inferContentMetadata is the only production caller that builds
			// a contentMarkers value; recompute it the same way here so this
			// test observes the exact struct inferRootFamily receives.
			path := filepath.Join("probe", "file.txt")
			_ = inferContentMetadata(path, tt.content)

			hasAnyGoExpression := goExpressionRE.MatchString(tt.content)
			hasAnyJinjaStatement := jinjaStatementRE.MatchString(tt.content)
			hasAnyTFInterpolation := tfInterpolationRE.MatchString(tt.content)
			hasAnyTFDirective := tfDirectiveRE.MatchString(tt.content)
			hasAnyTFTemplatefile := tfTemplatefileRE.MatchString(tt.content)
			gotMarkers := contentMarkers{
			...
			if gotMarkers.hasGoExpressions != wantGoExpression {
				t.Errorf("hasGoExpressions = %t, want %t (direct goExpressionRE.MatchString)", gotMarkers.hasGoExpressions, wantGoExpression)
			}
			if gotMarkers.hasJinjaStatements != wantJinjaStatement {
				t.Errorf("hasJinjaStatements = %t, want %t (direct jinjaStatementRE.MatchString)", gotMarkers.hasJinjaStatements, wantJinjaStatement)
			}
			if gotMarkers.hasTFMarkers != wantTFMarkers {
				t.Errorf("hasTFMarkers = %t, want %t (direct tf*RE.MatchString)", gotMarkers.hasTFMarkers, wantTFMarkers)
			}
		})
	}
}
