...
// TestParseKubernetesWorkloadSelectorNormalizesMatchLabels proves
// collectSpecSelector is shape-aware: a Service's spec.selector is a flat
// label map, but a workload's (Deployment/StatefulSet/DaemonSet/ReplicaSet)
// spec.selector is a nested Kubernetes LabelSelector
// (spec.selector.matchLabels), and both must normalize to the same clean
// "k=v,k=v" string. Before the fix, the nested shape was passed straight to
// collectLabelLikeMap, which stringified the inner map as garbage
// ("matchLabels=map[app:comprehensive-cache]") instead of extracting it. See
// #5343 review finding.
func TestParseKubernetesWorkloadSelectorNormalizesMatchLabels(t *testing.T) {
	t.Parallel()

	t.Run("flat service selector", func(t *testing.T) {
		t.Parallel()
...
`)
		...
		if gotSelector, want := resources[0]["selector"], "app=comprehensive-cache"; gotSelector != want {
			t.Fatalf("selector = %#v, want %#v", gotSelector, want)
		}
	})

	t.Run("nested workload matchLabels selector", func(t *testing.T) {
		t.Parallel()

		path := filepath.Join(t.TempDir(), "statefulset.yaml")
		source := []byte(`
apiVersion: apps/v1
kind: StatefulSet
metadata:
