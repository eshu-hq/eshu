...
// one file is used; otherwise it walks ReducerDir (at any depth, see
// globFilesRecursive) for every factschema_decode*.go file so a family whose
// wrappers live in a split file — including one relocated into a
// subdirectory — is covered. An empty result is an error rather than a
// silent zero-seam manifest.
func resolveDecodeFiles(p Paths) ([]string, error) {
	...
	pattern := filepath.Join(p.ReducerDir, decodeSeamFileNamePattern)
	matches, err := globFilesRecursive(p.ReducerDir, decodeSeamFileNamePattern)
	if err != nil {
...
// resolveOptionalDecodeFiles walks dir (at any depth, see globFilesRecursive)
// for every factschema_decode*.go file, excluding test files, for pipeline
...
func resolveOptionalDecodeFiles(label string, dir string, explicit []string) ([]string, error) {
	...
	pattern := filepath.Join(dir, decodeSeamFileNamePattern)
	matches, err := globFilesRecursive(dir, decodeSeamFileNamePattern)
	if err != nil {
