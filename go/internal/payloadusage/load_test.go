...
func TestLoadAgainstRealReducer(t *testing.T) {
	...
	if len(manifest.Kinds) != 46 {
		t.Fatalf("len(manifest.Kinds) = %d, want 46 (8 aws/iam + 4 wired incident + 2 wired gcp + 2 wired azure + 3 wired kubernetes_live + 8 wired vulnerability + 5 wired sbom_attestation reducer kinds + 6 projector oci_registry kinds + 5 projector terraform_state kinds + 3 projector package_registry kinds via the reducer and projector factschema_decode*.go globs); got %+v", len(manifest.Kinds), manifest.Kinds)
	}
