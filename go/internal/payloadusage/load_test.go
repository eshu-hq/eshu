...
func TestLoadAgainstRealReducer(t *testing.T) {
	...
	if len(manifest.Kinds) != 133 {
		t.Fatalf("len(manifest.Kinds) = %d, want 133 (10 aws/iam — the original 8 plus rds_instance_posture and s3_external_principal_grant wired to the reducer's typed decode seam, #4632 — + 8 wired secrets_iam trust-chain anchor kinds (aws_iam_trust_policy, aws_iam_permission_policy, aws_iam_policy_attachment, aws_iam_permission_boundary, gcp_iam_principal, gcp_iam_trust_policy, gcp_iam_permission_policy, k8s_service_account_token_posture, #6392) — + 3 cross-provider image_reference kinds (aws/azure/gcp, #4685) + 6 wired incident (4 reducer-decoded + incident.lifecycle_event and change.record decoded only by the query-layer incident-context read model, #4794 W2a) + 3 wired gcp (gcp_cloud_resource, gcp_cloud_relationship via the reducer; gcp_tag_observation via the shared cloud-tag-evidence storage loader, #4686) + 3 wired azure (azure_cloud_resource, azure_cloud_relationship via the reducer; azure_tag_observation via the shared cloud-tag-evidence storage loader, #4686) + 4 wired kubernetes_live (the original 3 plus kubernetes_live.namespace, #5434) + 8 wired vulnerability + 8 wired sbom_attestation (the original 5 plus sbom.dependency_relationship and sbom.external_reference wired to the reducer's typed decode seam, #5370, plus attestation.slsa_provenance wired to the reducer's typed decode seam, #5371) + 7 wired ci_cd_run (the original 6 plus ci.deployment_event, decoded via decodeCICDDeploymentEvent and joined onto runs by sha via attachDeploymentEventsToRuns rather than the provider/run_id key the other kinds use, issue #5425) + 8 wired secrets_iam vault/k8s reducer kinds + 1 wired security_alert reducer kind + 3 reducer_derived package correlation kinds + 17 wired observability reducer kinds (source_instance is typed but has no reducer decode wrapper, so it is intentionally excluded) + 2 wired code kinds (file, repository outer envelopes) + 6 wired codedataflow kinds (Wave 4f S2) + 4 wired service_catalog kinds (entity, ownership, repository_link via the reducer; operational_link decoded only by the query-layer incident-context read model, #4794 W2a) + 7 wired oci_registry kinds (six projector kinds plus warning in the retirement planner, #5854) + 6 projector terraform_state kinds (the original 5 plus terraform_state_provider_binding wired to the projector's typed decode seam, #5446) + 5 projector package_registry kinds (the original 3 plus package_artifact and registry_event wired to the projector's typed decode seam, #5458) + 9 wired work_item query kinds (issue_type_metadata added #4731) + 2 wired documentation kinds + 1 wired codeowners kind (codeowners.ownership via the reducer, issue #5419 Phase 3) + 1 wired submodule kind (submodule.pin via the reducer, issue #5420 Phase 3) + 1 wired scanner_worker kind (scanner_worker.analysis via the reducer, issue #5463) via the reducer, projector, and query factschema_decode*.go globs); got %+v", len(manifest.Kinds), manifest.Kinds)
	}
...
func TestGateAgainstRealReducerAndSchemas(t *testing.T) {
	...
	if len(manifest.Kinds) != 133 {
		t.Fatalf("len(manifest.Kinds) = %d, want 133", len(manifest.Kinds))
	}
}
