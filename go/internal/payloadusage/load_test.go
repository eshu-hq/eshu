...
func TestLoadAgainstRealReducer(t *testing.T) {
	...
	if len(manifest.Kinds) != 19 {
		t.Fatalf("len(manifest.Kinds) = %d, want 19 (8 aws/iam + 4 wired incident + 2 wired gcp + 2 wired azure + 3 wired kubernetes_live kinds via the factschema_decode*.go glob); got %+v", len(manifest.Kinds), manifest.Kinds)
	}
...
func TestGateAgainstRealReducerAndSchemas(t *testing.T) {
	...
	if len(manifest.Kinds) != 19 {
		t.Fatalf("len(manifest.Kinds) = %d, want 19", len(manifest.Kinds))
	}
}
