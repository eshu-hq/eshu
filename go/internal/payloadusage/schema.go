...
var factKindSchemaFile = map[string]string{ // #nosec G101 -- fact-kind identifier to JSON-schema-filename lookup (e.g. vault_secret_engine_mount, k8s_service_account_token_posture); these are schema file names, not credentials.
	...
	// package_registry family: the five kinds the projector's canonical
	...
	// package_registry_canonical_artifact.go) and registry_event (#5458,
	// decodePackageRegistryRegistryEvent wired into
	// package_registry_canonical_event.go). The four remaining
	// typed-but-not-yet-consumed kinds (source_hint, vulnerability_hint,
	// repository_hosting, warning) carry a schema but no decode-seam call from
	// the reducer or projector, so they are intentionally absent here —
	// mapping them would assert a gate contract for a kind no decode site
	// reads.
	...
	"FactKindPackageRegistryRegistryEvent":     "package_registry.registry_event.v1.schema.json",
