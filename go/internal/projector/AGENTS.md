...
- **Intent family dependency direction** — extracted family packages must
  ...
  incident-routing, AWS-relationship, IAM CAN_ASSUME, package-source-correlation, service-catalog-correlation, and secrets-IAM-trust-chain family builders consume the lookup. Root owns ordered family assembly and the public `ReducerIntent`
  incident-routing, AWS-relationship, IAM CAN_ASSUME, package-source-correlation, SBOM-attestation-attachment, and service-catalog-correlation family builders consume the lookup. Root owns ordered family assembly and the public `ReducerIntent`
  ...
  package-source-correlation, service-catalog-correlation, and
  secrets-IAM-trust-chain builders trigger on fact presence alone and carry no
  decode seam.
  package-source-correlation, SBOM-attestation-attachment, and
  service-catalog-correlation builders trigger
  on fact presence alone and carry no decode seam.
