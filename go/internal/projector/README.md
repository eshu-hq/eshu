...
`observability_coverage_correlation` reducer intent for the scope/generation, and when an AWS-native observability resource is present specifically, `observabilitycoveragematerialization.BuildObservabilityCoverageMaterializationReducerIntent` ([architecture](observabilitycoveragematerialization/README.md)) emits one `observability_coverage_materialization` intent alongside it, keyed to `aws_resource_materialization:<scope>` so the reducer waits for the AWS CloudResource canonical-nodes phase before writing COVERS edges.
