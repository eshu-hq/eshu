...
package canonical //nolint:filelength // Legacy canonical materialization coverage predates the 500-line hook; this change only adjusts existing assertions while new coverage lives in focused files.
...
func TestBuildCanonicalMaterializationMarksFirstGenerationFromScope(t *testing.T) {
	...
	result, _ := BuildMaterialization(sc, gen, nil)
...
func TestBuildCanonicalMaterializationHandlesEmptyFacts(t *testing.T) {
	...
	// empty slice input
	result, _ = BuildMaterialization(sc, gen, []facts.Envelope{})
	if !result.IsEmpty() {
