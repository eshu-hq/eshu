...
func extractRepositoryWithQuarantine(envelopes []facts.Envelope) (*RepositoryRow, []decode.QuarantinedFact) {
	...
	for i := range repoFacts {
		...
		if err != nil {
			q, isQuarantine, fatal := decode.PartitionFailures(repoFacts[i], err)
			if fatal != nil {
				continue
			}
			if isQuarantine {
		...
		name := decode.CodegraphDerefString(repository.Name)
		repoPath, _ := decode.PayloadString(p, "path")
		localPath := decode.CodegraphDerefString(repository.LocalPath)
		remoteURL := decode.CodegraphDerefString(repository.RemoteURL)
		repoSlug := decode.CodegraphDerefString(repository.RepoSlug)
		...
		hasRemote := false
		if ptr := decode.PayloadBoolPtr(p, "has_remote"); ptr != nil {
			hasRemote = *ptr
		} else {
			hasRemote = remoteURL != ""
		}
...
func extractFilesWithQuarantine(envelopes []facts.Envelope, repoID, repoPath string) ([]FileRow, []parsedFileRef, []decode.QuarantinedFact) {
	...
	for i := range fileFacts {
		...
		file, err := decode.CodegraphFile(fileFacts[i])
		if err != nil {
			q, isQuarantine, fatal := decode.PartitionFailures(fileFacts[i], err)
			if fatal != nil {
				continue
			}
			if isQuarantine {
		...
		if !isRepositoryLocalRelativePath(relativePath) {
			...
			quarantined = append(quarantined, decode.QuarantinedFact{
		...
		language := decode.CodegraphDerefString(file.Language)
