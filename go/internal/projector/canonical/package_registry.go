...
// package_registry_artifact.go and
// package_registry_event.go respectively, split out from this file
// to stay under the package's 500-line-per-file convention (mirrors
// terraform_state_types.go's split from terraform_state.go).
func ExtractPackageRegistryRows(mat *CanonicalMaterialization, envelopes []facts.Envelope) []decode.QuarantinedFact {
...
func packageRegistryPackageRow(envelope facts.Envelope) (PackageRegistryPackageRow, bool, error) {
	...
	pkg, err := decode.PackageRegistryPackage(envelope)
	if err != nil {
	...
	return PackageRegistryPackageRow{
		UID:                 packageID,
		Ecosystem:           decode.PackageRegistryDerefString(pkg.Ecosystem),
		Registry:            decode.PackageRegistryDerefString(pkg.Registry),
		RawName:             decode.PackageRegistryDerefString(pkg.RawName),
		NormalizedName:      decode.PackageRegistryDerefString(pkg.NormalizedName),
		Namespace:           decode.PackageRegistryDerefString(pkg.Namespace),
		Classifier:          decode.PackageRegistryDerefString(pkg.Classifier),
		PURL:                decode.PackageRegistryDerefString(pkg.PURL),
		BOMRef:              decode.PackageRegistryDerefString(pkg.BOMRef),
		PackageManager:      decode.PackageRegistryDerefString(pkg.PackageManager),
		SourcePath:          decode.PackageRegistryDerefString(pkg.SourcePath),
		SourceSpecificID:    decode.PackageRegistryDerefString(pkg.SourceSpecificID),
		Visibility:          decode.PackageRegistryDerefString(pkg.Visibility),
		...
		CollectorInstanceID: decode.PackageRegistryDerefString(pkg.CollectorInstanceID),
...
func packageRegistryVersionRow(envelope facts.Envelope) (PackageRegistryVersionRow, bool, error) {
	...
	version, err := decode.PackageRegistryPackageVersion(envelope)
	if err != nil {
	...
	return PackageRegistryVersionRow{
		UID:                 versionID,
		PackageID:           packageID,
		Ecosystem:           decode.PackageRegistryDerefString(version.Ecosystem),
		Registry:            decode.PackageRegistryDerefString(version.Registry),
		Version:             rawVersion,
		PURL:                decode.PackageRegistryDerefString(version.PURL),
		BOMRef:              decode.PackageRegistryDerefString(version.BOMRef),
		PackageManager:      decode.PackageRegistryDerefString(version.PackageManager),
		PublishedAt:         packageRegistryParsedTimestamp(version.PublishedAt),
		IsYanked:            decode.PackageRegistryDerefBool(version.IsYanked),
		IsUnlisted:          decode.PackageRegistryDerefBool(version.IsUnlisted),
		IsDeprecated:        decode.PackageRegistryDerefBool(version.IsDeprecated),
		IsRetracted:         decode.PackageRegistryDerefBool(version.IsRetracted),
		...
		CollectorInstanceID: decode.PackageRegistryDerefString(version.CollectorInstanceID),
		ObservedAt:          envelope.ObservedAt,
	}, true, nil
}
...
func packageRegistryDependencyRow(envelope facts.Envelope) (PackageRegistryDependencyRow, bool, error) {
	...
	dependency, err := decode.PackageRegistryPackageDependency(envelope)
	if err != nil {
	...
	return PackageRegistryDependencyRow{
		...
		PackageID:            packageID,
		VersionID:            versionID,
		Version:              decode.PackageRegistryDerefString(dependency.Version),
		DependencyPackageID:  dependencyPackageID,
		DependencyEcosystem:  decode.PackageRegistryDerefString(dependency.DependencyEcosystem),
		DependencyRegistry:   decode.PackageRegistryDerefString(dependency.DependencyRegistry),
		DependencyNamespace:  decode.PackageRegistryDerefString(dependency.DependencyNamespace),
		DependencyNormalized: decode.PackageRegistryDerefString(dependency.DependencyNormalized),
		DependencyPURL:       decode.PackageRegistryDerefString(dependency.DependencyPURL),
		DependencyBOMRef:     decode.PackageRegistryDerefString(dependency.DependencyBOMRef),
		DependencyManager:    decode.PackageRegistryDerefString(dependency.DependencyManager),
		DependencyRange:      decode.PackageRegistryDerefString(dependency.DependencyRange),
		DependencyType:       decode.PackageRegistryDerefString(dependency.DependencyType),
		TargetFramework:      decode.PackageRegistryDerefString(dependency.TargetFramework),
		Marker:               decode.PackageRegistryDerefString(dependency.Marker),
		Optional:             decode.PackageRegistryDerefBool(dependency.Optional),
		Excluded:             decode.PackageRegistryDerefBool(dependency.Excluded),
		SourceFactID:         envelope.FactID,
		...
		CollectorInstanceID:  decode.PackageRegistryDerefString(dependency.CollectorInstanceID),
		ObservedAt:           envelope.ObservedAt,
	}, true, nil
}
...
func PackageRegistryTrimmedStringMap(factKind, field string, values map[string]string) (map[string]string, error) {
	...
	for _, key := range originalKeys {
		...
		if existing, collided := out[trimmedKey]; collided && existing != trimmedValue {
			return nil, decode.NewError(factKind, &factschema.DecodeError{
				FactKind:       factKind,
