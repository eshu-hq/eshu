...
func packageRegistryArtifactRow(envelope facts.Envelope) (PackageRegistryArtifactRow, bool, error) {
	...
	artifact, err := decode.PackageRegistryPackageArtifact(envelope)
	if err != nil {
	...
	return PackageRegistryArtifactRow{
		...
		Version:             decode.PackageRegistryDerefString(artifact.Version),
		Ecosystem:           decode.PackageRegistryDerefString(artifact.Ecosystem),
		Registry:            decode.PackageRegistryDerefString(artifact.Registry),
		ArtifactType:        decode.PackageRegistryDerefString(artifact.ArtifactType),
		ArtifactURL:         decode.PackageRegistryDerefString(artifact.ArtifactURL),
		ArtifactPath:        decode.PackageRegistryDerefString(artifact.ArtifactPath),
		SizeBytes:           decode.PackageRegistryDerefInt64(artifact.SizeBytes),
		Hashes:              hashes,
		Classifier:          decode.PackageRegistryDerefString(artifact.Classifier),
		...
		CollectorInstanceID: decode.PackageRegistryDerefString(artifact.CollectorInstanceID),
