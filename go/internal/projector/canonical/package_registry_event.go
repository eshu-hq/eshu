...
func packageRegistryEventRow(envelope facts.Envelope) (PackageRegistryEventRow, bool, error) {
	...
	event, err := decode.PackageRegistryRegistryEvent(envelope)
	if err != nil {
		return PackageRegistryEventRow{}, false, err
	}
	packageID := strings.TrimSpace(decode.PackageRegistryDerefString(event.PackageID))
	versionID := strings.TrimSpace(decode.PackageRegistryDerefString(event.VersionID))
	...
	return PackageRegistryEventRow{
		...
		PackageID:           packageID,
		VersionID:           versionID,
		Version:             decode.PackageRegistryDerefString(event.Version),
		Ecosystem:           decode.PackageRegistryDerefString(event.Ecosystem),
		Registry:            decode.PackageRegistryDerefString(event.Registry),
		EventKey:            eventKey,
		EventType:           eventType,
		ArtifactKey:         decode.PackageRegistryDerefString(event.ArtifactKey),
		Actor:               decode.PackageRegistryDerefString(event.Actor),
		Message:             decode.PackageRegistryDerefString(event.Message),
		...
		CollectorInstanceID: decode.PackageRegistryDerefString(event.CollectorInstanceID),
