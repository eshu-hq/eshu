...
func ExtractTerraformStateRows(mat *CanonicalMaterialization, envelopes []facts.Envelope) []decode.QuarantinedFact {
	...
	if snapshotErr != nil {
		if q, isQuarantine, fatal := decode.PartitionFailures(snapshotEnvelope, snapshotErr); fatal == nil && isQuarantine {
...
func terraformStateSnapshot(envelopes []facts.Envelope) (terraformStateSnapshotContext, facts.Envelope, error) {
	for _, envelope := range envelopes {
		...
		lineage := decode.TerraformStateDerefString(snapshot.Lineage)
		backendKind := decode.TerraformStateDerefString(snapshot.BackendKind)
		locatorHash := decode.TerraformStateDerefString(snapshot.LocatorHash)
		return terraformStateSnapshotContext{
			Lineage:     lineage,
			Serial:      decode.TerraformStateDerefInt64(snapshot.Serial),
...
func terraformStateResourceRow(
	...
	resource, err := decode.TerraformStateResource(envelope)
	if err != nil {
	...
	return TerraformStateResourceRow{
		...
		Address:               address,
		Mode:                  decode.TerraformStateDerefString(resource.Mode),
		ResourceType:          decode.TerraformStateDerefString(resource.ResourceType),
		Name:                  decode.TerraformStateDerefString(resource.Name),
		ModuleAddress:         decode.TerraformStateDerefString(resource.Module),
		ProviderAddress:       decode.TerraformStateDerefString(resource.Provider),
...
func terraformStateModuleRow(
	...
	module, err := decode.TerraformStateModule(envelope)
	if err != nil {
	...
	return TerraformStateModuleRow{
		...
		ResourceCount:    decode.TerraformStateDerefInt64(module.ResourceCount),
		Lineage:          snapshot.Lineage,
		Serial:           snapshot.Serial,
		BackendKind:      snapshot.BackendKind,
		LocatorHash:      snapshot.LocatorHash,
...
func terraformStateOutputRow(
	...
	output, err := decode.TerraformStateOutput(envelope)
	if err != nil {
	...
	sensitive := decode.TerraformStateDerefBool(output.Sensitive)
	valueShape := decode.TerraformStateDerefString(output.ValueShape)
	...
	if valueShape == "" && decode.PayloadHasKey(envelope.Payload, "value") {
		valueShape = "scalar"
		if sensitive {
...
func terraformStateCorrelationAnchors(rawAnchors []map[string]any) []string {
	...
	for _, entry := range rawAnchors {
		kind, _ := decode.PayloadString(entry, "anchor_kind")
		hash, _ := decode.PayloadString(entry, "value_hash")
		if kind == "" || hash == "" {
			continue
		}
...
func terraformStateTagHashesByResource(envelopes []facts.Envelope) (map[string][]string, []decode.QuarantinedFact) {
	...
	for _, envelope := range envelopes {
		...
		if err != nil {
			if q, isQuarantine, fatal := decode.PartitionFailures(envelope, err); fatal == nil && isQuarantine {
				quarantined = append(quarantined, q)
			}
			continue
		}
...
func terraformStateProviderBindingsByResource(envelopes []facts.Envelope) (map[string]terraformStateProviderBindingInfo, []decode.QuarantinedFact) {
	...
	for _, envelope := range envelopes {
		...
		bindings[address] = terraformStateProviderBindingInfo{
			Provider:              decode.TerraformStateDerefString(binding.ProviderType),
			ProviderSourceAddress: decode.TerraformStateDerefString(binding.ProviderSourceAddress),
			ProviderAlias:         decode.TerraformStateDerefString(binding.ProviderAlias),
		}
	}
