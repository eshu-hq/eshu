...
// Package summary builds the code_function_summary reducer intent
...
//
// This package performs trigger-time typed decode only to derive a best-effort
// repo_id. It performs no schema-version admission or quarantine; a malformed
// payload simply yields an unresolved repo_id, never a dropped intent or a
// projector-side dead letter. The reducer's DomainCodeFunctionSummary handler
// owns materialization-time typed decode, quarantine, and durable
// summary/source/graph-id persistence. Root projector assembly owns lookup
// construction and lifetime, family invocation order, queue writes, retries,
// and telemetry.
package summary
