# AGENTS.md — interprocedural-evidence projector intent guidance
...
- `BuildReducerIntent` fires on a `code_interproc_evidence` finding, else on
  the `code_dataflow_scanned` marker. The finding outranks the marker regardless
  of input order.
- The marker fallback is required by #2919 so empty scans retract stale
  `TAINT_FLOWS_TO` edges.
- A `code_function_summary` fact must not trigger this family.
- Preserve both reason strings and `code_interproc_evidence:<scope>`.
...
Run the package contract tests, root interproc/marker cases, ordered fan-out and
