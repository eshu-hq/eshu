# Taint-evidence projector intent

## Purpose

This package recognizes value-flow taint evidence for one immutable scope
generation and builds the reducer intent that materializes or reconciles
`CodeTaintEvidence` graph truth. The marker fallback from #2919 lets an empty
scan retract stale evidence from an earlier generation.
...
node and edge writes, and stale-evidence retraction.
...
- `BuildReducerIntent` prefers `code_taint_evidence`; the
