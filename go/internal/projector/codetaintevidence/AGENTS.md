# AGENTS.md — code-taint-evidence projector intent guidance
...
4. `../scope_generation_intents.go` for root-owned assembly order; this probe
   runs after the incident-routing probe and before the code-interproc
   evidence probe.
5. `go/internal/reducer/codetaint/` for what the reducer does with the intent
   this package enqueues, including typed decode, quarantine, and the
   stale-evidence retraction the marker path exists for.
...
- `BuildCodeTaintEvidenceReducerIntent` fires on a `code_taint_evidence`
  ...
  `FactID` provenance for generations carrying both kinds.
- The marker fallback is load-bearing (#2919). Removing it means an empty
  finding set queues no intent, so stale CodeTaintEvidence from a prior
  generation is never retracted.
- Keep the two reason strings and the `code_taint_evidence:<scope>` entity key
...
- `SourceSystem` is the trimmed `CollectorKind` alone — a single tier. Do NOT
  ...
  package test `TestBuildCodeTaintEvidenceReducerIntentTrimsCollectorKind`
...
- **Adding a trigger kind.** Decide explicitly whether it joins the
  finding tier or the marker tier, keep the finding-over-marker rule, and
  update the child tests plus the root dispatcher tests in
...
- **Route-serves-data registry path citations.** The registry in
  ...
  serves the taint domain, its evidence entry will create that coupling here.
...
- **Root dispatcher tests live outside this directory.** The end-to-end
  `buildProjection` cases for this domain — including the #2919 both-domains
  marker case — are at root in `../code_taint_evidence_projection_test.go`.
...
- Do not widen the export surface past `BuildCodeTaintEvidenceReducerIntent`.
...
- Adding a decode seam. This family triggers on fact presence alone; families
  ...
  than a local call.
- Changing `reducer.DomainCodeTaintEvidence`, the finding-over-marker rule, or
  the single-tier source-system label. All three are contract surface the
