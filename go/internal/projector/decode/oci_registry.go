...
// *Error so PartitionFailures can quarantine the fact
...
// OCIRegistryRepository decodes one oci_registry.repository envelope into
// the typed struct through the contracts seam. A missing required field
// (repository_id) yields a self-classifying *Error.
func OCIRegistryRepository(env facts.Envelope) (ociregistryv1.Repository, error) {
	repository, err := factschema.DecodeOCIRegistryRepository(factschemaEnvelope(env))
	if err != nil {
		return ociregistryv1.Repository{}, NewError(factschema.FactKindOCIRegistryRepository, err)
	}
	return repository, nil
}

// OCIImageManifest decodes one oci_registry.image_manifest envelope into
// the typed struct. A missing required field (repository_id, digest) yields a
// self-classifying *Error.
func OCIImageManifest(env facts.Envelope) (ociregistryv1.ImageManifest, error) {
	manifest, err := factschema.DecodeOCIImageManifest(factschemaEnvelope(env))
	if err != nil {
		return ociregistryv1.ImageManifest{}, NewError(factschema.FactKindOCIImageManifest, err)
	}
	return manifest, nil
}

// OCIImageIndex decodes one oci_registry.image_index envelope into the
// typed struct. A missing required field (repository_id, digest) yields a
// self-classifying *Error.
func OCIImageIndex(env facts.Envelope) (ociregistryv1.ImageIndex, error) {
	index, err := factschema.DecodeOCIImageIndex(factschemaEnvelope(env))
	if err != nil {
		return ociregistryv1.ImageIndex{}, NewError(factschema.FactKindOCIImageIndex, err)
	}
	return index, nil
}

// OCIImageDescriptor decodes one oci_registry.image_descriptor envelope
// into the typed struct. A missing required field (repository_id, digest)
// yields a self-classifying *Error.
func OCIImageDescriptor(env facts.Envelope) (ociregistryv1.ImageDescriptor, error) {
	descriptor, err := factschema.DecodeOCIImageDescriptor(factschemaEnvelope(env))
	if err != nil {
		return ociregistryv1.ImageDescriptor{}, NewError(factschema.FactKindOCIImageDescriptor, err)
	}
	return descriptor, nil
}

// OCIImageTagObservation decodes one oci_registry.image_tag_observation
// envelope into the typed struct. A missing required field (repository_id, tag,
// resolved_digest) yields a self-classifying *Error.
func OCIImageTagObservation(env facts.Envelope) (ociregistryv1.TagObservation, error) {
	observation, err := factschema.DecodeOCIImageTagObservation(factschemaEnvelope(env))
	if err != nil {
		return ociregistryv1.TagObservation{}, NewError(factschema.FactKindOCIImageTagObservation, err)
	}
	return observation, nil
}

// OCIImageReferrer decodes one oci_registry.image_referrer envelope into
// the typed struct. A missing required field (repository_id, subject_digest,
// referrer_digest) yields a self-classifying *Error.
func OCIImageReferrer(env facts.Envelope) (ociregistryv1.ImageReferrer, error) {
	referrer, err := factschema.DecodeOCIImageReferrer(factschemaEnvelope(env))
	if err != nil {
		return ociregistryv1.ImageReferrer{}, NewError(factschema.FactKindOCIImageReferrer, err)
	}
	return referrer, nil
}
