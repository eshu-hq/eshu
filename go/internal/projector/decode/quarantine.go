...
// Stage labels for the canonical extractors. Each is the bounded value the
// extractor reports as the stage dimension on
// eshu_dp_projector_input_invalid_facts_total, and the value
// [QuarantinedFactStage] maps a quarantined fact's kind onto. The set is
// bounded on purpose: an unbounded stage dimension would make the metric
// unusable at repo scale.
//
// They live here rather than with their extractors because the quarantine path
// maps fact kinds onto them, and a label owned by the canonical package would
// make decode depend on canonical while canonical already depends on decode.
const (
	// CodegraphCanonicalStage labels the source-local codegraph extractor.
	CodegraphCanonicalStage = "codegraph_canonical"
	// OCIRegistryCanonicalStage labels the OCI registry extractor.
	OCIRegistryCanonicalStage = "oci_registry_canonical"
	// PackageRegistryCanonicalStage labels the package registry extractor.
	PackageRegistryCanonicalStage = "package_registry_canonical"
	// TerraformStateCanonicalStage labels the terraform_state extractor.
	TerraformStateCanonicalStage = "terraform_state_canonical"
)
