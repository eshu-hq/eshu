...
// terraformStateProviderBindingsByResource in canonical/terraform_state.go. The two
...
// TerraformStateSnapshot decodes one terraform_state_snapshot envelope
...
func TerraformStateSnapshot(env facts.Envelope) (tfstatev1.Snapshot, error) {
	snapshot, err := factschema.DecodeTerraformStateSnapshot(factschemaEnvelope(env))
	if err != nil {
		return tfstatev1.Snapshot{}, NewError(factschema.FactKindTerraformStateSnapshot, err)
	}
	return snapshot, nil
}

// TerraformStateResource decodes one terraform_state_resource envelope
// into the typed struct. A missing required field (address) yields a
// self-classifying *Error.
func TerraformStateResource(env facts.Envelope) (tfstatev1.Resource, error) {
	resource, err := factschema.DecodeTerraformStateResource(factschemaEnvelope(env))
	if err != nil {
		return tfstatev1.Resource{}, NewError(factschema.FactKindTerraformStateResource, err)
	}
	return resource, nil
}

// TerraformStateModule decodes one terraform_state_module envelope into
// the typed struct. A missing required field (module_address) yields a
// self-classifying *Error.
func TerraformStateModule(env facts.Envelope) (tfstatev1.Module, error) {
	module, err := factschema.DecodeTerraformStateModule(factschemaEnvelope(env))
	if err != nil {
		return tfstatev1.Module{}, NewError(factschema.FactKindTerraformStateModule, err)
	}
	return module, nil
}

// TerraformStateOutput decodes one terraform_state_output envelope into
// the typed struct. A missing required field (name) yields a self-classifying
// *Error.
func TerraformStateOutput(env facts.Envelope) (tfstatev1.Output, error) {
	output, err := factschema.DecodeTerraformStateOutput(factschemaEnvelope(env))
	if err != nil {
		return tfstatev1.Output{}, NewError(factschema.FactKindTerraformStateOutput, err)
	}
	return output, nil
}

// TerraformStateTagObservation decodes one
...
// self-classifying *Error.
func TerraformStateTagObservation(env facts.Envelope) (tfstatev1.TagObservation, error) {
	observation, err := factschema.DecodeTerraformStateTagObservation(factschemaEnvelope(env))
	if err != nil {
		return tfstatev1.TagObservation{}, NewError(factschema.FactKindTerraformStateTagObservation, err)
	}
	return observation, nil
}

// TerraformStateProviderBinding decodes one
...
// self-classifying *Error.
func TerraformStateProviderBinding(env facts.Envelope) (tfstatev1.ProviderBinding, error) {
	binding, err := factschema.DecodeTerraformStateProviderBinding(factschemaEnvelope(env))
	if err != nil {
		return tfstatev1.ProviderBinding{}, NewError(factschema.FactKindTerraformStateProviderBinding, err)
	}
	return binding, nil
}

// TerraformStateDerefString returns the value a *string points at, or "" when it is
...
func TerraformStateDerefString(value *string) string {
	if value == nil {
		return ""
	}
	return *value
...
// TerraformStateDerefInt64 returns the value a *int64 points at, or 0 when it is
// nil.
func TerraformStateDerefInt64(value *int64) int64 {
	if value == nil {
		return 0
	}
	return *value
...
// TerraformStateDerefBool returns the value a *bool points at, or false when it is
// nil.
func TerraformStateDerefBool(value *bool) bool {
	if value == nil {
		return false
	}
	return *value
