# projector Evidence Notes

Scoped `internal/projector` evidence that is too detailed for the package
orientation README, and too historical for the agent guidance in
`AGENTS.md`. Moved out of `AGENTS.md` so that file has room to keep
describing the package as families continue to move out of the root
(#6057), matching the sibling convention in `internal/reducer`.

Each note records what was proven at the time it was written. They are
point-in-time records, not current instructions: read `AGENTS.md` for what
the package requires today.
