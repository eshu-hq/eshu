...
func TestExtractPackageRegistryRowsQuarantinesMissingPackageID(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "package-registry-scope-1"}
	quarantined := canonical.ExtractPackageRegistryRows(mat, append(validFacts, malformed))
...
func TestExtractPackageRegistryRowsPresentButEmptyPackageIDIsDroppedNotQuarantined(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "package-registry-scope-1"}
	quarantined := canonical.ExtractPackageRegistryRows(mat, []facts.Envelope{emptyPackageID})
...
func TestExtractPackageRegistryRowsWhitespacePackageIDIsDroppedNotMaterialized(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "package-registry-scope-1"}
	quarantined := canonical.ExtractPackageRegistryRows(mat, append(validFacts, whitespacePackageID))
...
func TestExtractPackageRegistryRowsQuarantinesMissingDependencyJoinKey(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "package-registry-scope-1"}
	quarantined := canonical.ExtractPackageRegistryRows(mat, append(packageRegistryFacts(), malformed))
...
func TestExtractPackageRegistryRowsProjectsWithOmittedStatusBools(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "package-registry-scope-1"}
	quarantined := canonical.ExtractPackageRegistryRows(mat, []facts.Envelope{versionNoBools, dependencyNoBools})
