...
func TestExtractTerraformStateRowsQuarantinesMissingResourceAddress(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "tf-scope-1"}
	quarantined := canonical.ExtractTerraformStateRows(mat, append(validFacts, malformed))
...
func TestExtractTerraformStateRowsPresentButEmptyAddressIsDroppedNotQuarantined(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "tf-scope-1"}
	quarantined := canonical.ExtractTerraformStateRows(mat, []facts.Envelope{emptyAddress})
...
func TestExtractTerraformStateRowsWhitespaceAddressIsDroppedNotMaterialized(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "tf-scope-1"}
	quarantined := canonical.ExtractTerraformStateRows(mat, append(validFacts, whitespaceAddress))
...
func TestExtractTerraformStateRowsQuarantinesMissingTagObservationJoinKey(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "tf-scope-1"}
	quarantined := canonical.ExtractTerraformStateRows(mat, append(terraformStateFacts(), malformed))
...
func TestExtractTerraformStateRowsProviderBindingDuplicateAddressFirstWins(t *testing.T) {
	...
	mat := &canonical.CanonicalMaterialization{ScopeID: "tf-scope-1"}
	quarantined := canonical.ExtractTerraformStateRows(mat, append(terraformStateFacts(), first, duplicate))
