...
here, `browserSessionRouteAllowed` admits only a tenant-bound all-scopes
session under `BrowserSessionRoutePolicy.AllowTenantBoundAllScopes`, which
`cmd/api` sets for the default, `local_no_policy`, and
`hosted_single_tenant` modes, and rejects a restricted-scope session. On
the allowlist it splits by `scopedRouteClass` (#6450): an all-scope session
still needs that policy unless the route needs no caller grant.
