...
func (h *Handler) listDeadLetters(w http.ResponseWriter, r *http.Request) {
	...
	if updatedAfter != nil && updatedBefore != nil && !updatedAfter.Before(*updatedBefore) {
		querycontract.WriteError(w, http.StatusBadRequest, "updated_after must be before updated_before")
		return
	}
	...
	if err != nil {
		...
		querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("list dead letters: %v", err))
		return
	}
...
func writeDeadLetterList(w http.ResponseWriter, limit int, truncated bool, items []DeadLetterWorkItem) {
...
func deadLetterItemsToSlice(items []DeadLetterWorkItem) []map[string]any {
