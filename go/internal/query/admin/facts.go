...
func (h *Handler) listWorkItems(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("list work items: %v", err))
		return
	}
...
func (h *Handler) listDecisions(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(req.RepositoryID) == "" || strings.TrimSpace(req.SourceRunID) == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "repository_id and source_run_id are required")
		return
	}
	...
	if err != nil {
		querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("list decisions: %v", err))
		return
	}
...
func (h *Handler) deadLetter(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("dead-letter: %v", err))
		return
	}
...
func (h *Handler) skip(w http.ResponseWriter, r *http.Request) {
	...
	if repoID == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "repository_id is required and must not be empty")
		return
	}
...
func (h *Handler) backfill(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("backfill: %v", err))
		return
	}
...
func (h *Handler) listReplayEvents(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("list replay events: %v", err))
		return
	}
...
// decisionRowToMap converts an DecisionRow to a JSON-friendly map.
func decisionRowToMap(d DecisionRow) map[string]any {
...
// evidenceRowsToSlice converts a slice of EvidenceRow to JSON-friendly maps.
func evidenceRowsToSlice(rows []EvidenceRow) []map[string]any {
...
// workItemsToSlice converts a slice of WorkItem to JSON-friendly maps.
func workItemsToSlice(items []WorkItem) []map[string]any {
