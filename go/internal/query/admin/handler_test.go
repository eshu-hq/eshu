...
) ([]DeadLetterWorkItem, error) {
	s.deadLetterCalls++
	s.deadLetterFilter = f
	return s.deadLetterRows, s.deadLetterErr
}

func (s *stubAdminStore) DeadLetterWorkItems(_ context.Context, _ DeadLetterFilter) ([]WorkItem, error) {
	return s.deadLettered, s.deadLetterErr
}

func (s *stubAdminStore) SkipRepositoryWorkItems(_ context.Context, _ string, _ string) ([]WorkItem, error) {
...
func (s *stubAdminStore) RequestBackfill(_ context.Context, _ BackfillInput) (*BackfillRequest, error) {
	return s.backfillRow, s.backfillErr
}

func (s *stubAdminStore) ListReplayEvents(_ context.Context, _ ReplayEventFilter) ([]ReplayEvent, error) {
	return s.replayEvents, s.replayEvtErr
}

func (s *stubAdminStore) ListDecisions(_ context.Context, _ DecisionQueryFilter) ([]DecisionRow, error) {
	return s.decisions, s.decisionsErr
}

func (s *stubAdminStore) ListEvidence(_ context.Context, _ string) ([]EvidenceRow, error) {
...
func TestAdminHandler_RefinalizeStatusRouteRemovedFromPublicMount(t *testing.T) {
	h := &Handler{Recovery: &stubRecoveryHandler{}}
	mux := newAdminMux(h)
...
func TestAdminHandler_Mount_RegistersAllRoutes(t *testing.T) {
	...
	h := &Handler{Recovery: stub, Store: store}
	mux := newAdminMux(h)
