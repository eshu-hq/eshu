...
// the mode the request carries is the class the identity-mutation audit row
...
// identity-mutation audit path stamps for every auth mode.
//
...
func TestMutationAuditsStampActorClassByAuthMode(t *testing.T) {
	...
	for _, mode := range actorModeCases() {
		t.Run(mode.name, func(t *testing.T) {
			...
			h.audit(req, governanceaudit.EventTypeRoleGrantChange, governanceaudit.DecisionAllowed, "role_grant_changed", "")
			...
			if got := event.ActorClass; got != mode.want {
				t.Errorf("identity mutation actor class = %q, want %q", got, mode.want)
			}
			if event.ActorIDHash != "sha256:abcdef12" {
				t.Errorf("identity mutation actor hash = %q, want the request subject hash", event.ActorIDHash)
			}
			if _, err := governanceaudit.NormalizeEvent(event); err != nil {
				t.Errorf("NormalizeEvent rejected the identity mutation event: %v", err)
			}
		})
	}
}

// TestMutationAuditsWithNoSubjectHash pins the identity-mutation audit row
