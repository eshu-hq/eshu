...
// maxAuditEventLimit bounds the audit-event read window an admin may
...
const identityListLimit = 500

// ReadHandler serves tenant-scoped admin read endpoints for the
...
func (h *ReadHandler) storeReady(w http.ResponseWriter) bool {
	if h == nil || h.Store == nil {
		querycontract.WriteError(w, http.StatusServiceUnavailable, "admin identity read store is unavailable")
		return false
	}
	return true
}

func (h *ReadHandler) handleListInvitations(w http.ResponseWriter, r *http.Request) {
	if !h.storeReady(w) {
		return
	}
	...
	if err != nil {
		...
		querycontract.WriteError(w, http.StatusInternalServerError, "failed to list invitations")
		return
	}
	...
	for _, item := range items {
		...
		audit.AddOptionalTime(row, "accepted_at", item.AcceptedAt)
		audit.AddOptionalTime(row, "revoked_at", item.RevokedAt)
		out = append(out, row)
	}
...
func (h *ReadHandler) handleListRoleAssignments(w http.ResponseWriter, r *http.Request) {
	if !h.storeReady(w) {
		return
	}
	...
	if err != nil {
		...
		querycontract.WriteError(w, http.StatusInternalServerError, "failed to list role assignments")
		return
	}
	out := make([]map[string]any, 0, len(items))
	for _, item := range items {
...
func (h *ReadHandler) handleListRoles(w http.ResponseWriter, r *http.Request) {
	if !h.storeReady(w) {
		return
	}
	...
	if err != nil {
		...
		querycontract.WriteError(w, http.StatusInternalServerError, "failed to list roles")
		return
	}
	out := make([]map[string]any, 0, len(items))
	for _, item := range items {
	...
	querycontract.WriteJSON(w, http.StatusOK, map[string]any{
		...
		"truncated": len(items) == identityListLimit || grantsTruncated,
	})
}

func (h *ReadHandler) handleListIdPProviders(w http.ResponseWriter, r *http.Request) {
	if !h.storeReady(w) {
		return
	}
	...
	if err != nil {
		...
		querycontract.WriteError(w, http.StatusInternalServerError, "failed to list idp providers")
		return
	}
	out := make([]map[string]any, 0, len(items))
	for _, item := range items {
...
func (h *ReadHandler) handleListIdPGroupMappings(w http.ResponseWriter, r *http.Request) {
	if !h.storeReady(w) {
		return
	}
...
func (h *ReadHandler) handleListAPITokens(w http.ResponseWriter, r *http.Request) {
	if !h.storeReady(w) {
		return
	}
