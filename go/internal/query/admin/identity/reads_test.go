...
type fakeAdminIdentityReadStore struct {
	...
	invitations     map[string][]InvitationListItem
	assignments     map[string][]RoleAssignmentListItem
	roles           map[string][]RoleListItem
	providers       map[string][]IdPProviderListItem
	groupMappings   map[string][]IdPGroupMappingListItem
	apiTokens       map[string][]APITokenListItem
	forceListError  error
	grantsTruncated bool
}

func (f *fakeAdminIdentityReadStore) ListAdminInvitations(_ context.Context, tenantID, workspaceID string) ([]InvitationListItem, error) {
...
func (f *fakeAdminIdentityReadStore) ListAdminRoleAssignments(_ context.Context, tenantID, workspaceID, userID string) ([]RoleAssignmentListItem, error) {
	f.gotTenantID, f.gotWorkspaceID, f.gotUserID = tenantID, workspaceID, userID
	return f.assignments[tenantID], nil
}

func (f *fakeAdminIdentityReadStore) ListAdminRoles(_ context.Context, tenantID string) ([]RoleListItem, bool, error) {
...
func (f *fakeAdminIdentityReadStore) ListAdminIdPProviders(_ context.Context, tenantID string) ([]IdPProviderListItem, error) {
	f.gotTenantID = tenantID
...
func (f *fakeAdminAuditReader) ListAuditEvents(_ context.Context, q AuditQuery) ([]governanceaudit.Event, error) {
	f.gotQuery = q
	if f.forceListError != nil {
		return nil, f.forceListError
	}
...
func TestAdminIdentityReadsNeverLeakSecrets(t *testing.T) {
	...
	store := &fakeAdminIdentityReadStore{
		...
		roles: map[string][]RoleListItem{
			"tenant_a": {{RoleID: "developer", Status: "active", BuiltIn: true, Grants: []RoleGrantListItem{{GrantID: "g1", Action: "read", Feature: "code", DataClass: "metadata", ScopeClass: "tenant", Status: "active"}}}},
		},
		providers: map[string][]IdPProviderListItem{
