...
// InputInvalidFactListHandler mounts only the bounded
...
func (h *Handler) listInputInvalidFacts(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(req.ScopeID) == "" || strings.TrimSpace(req.GenerationID) == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "scope_id and generation_id are required")
		return
	}
	if req.Limit <= 0 {
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(ctx.Err(), context.DeadlineExceeded) {
			querycontract.WriteError(w, http.StatusGatewayTimeout, "input-invalid-facts query timed out")
			return
		}
		querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("list input invalid facts: %v", err))
		return
	}
...
func (h *Handler) recordInputInvalidFactsQuery(ctx context.Context, start time.Time, err error) {
...
func writeInputInvalidFactList(w http.ResponseWriter, limit int, truncated bool, items []InputInvalidFact) {
...
func nonNilInputInvalidFacts(items []InputInvalidFact) []InputInvalidFact {
	if items == nil {
		return []InputInvalidFact{}
	}
	return items
}
