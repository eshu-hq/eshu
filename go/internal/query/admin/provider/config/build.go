...
func buildProviderConfigWrite(body providerConfigWriteRequest) (builtProviderConfigWrite, error) {
...
func buildOIDCProviderConfigWrite(body providerConfigWriteRequest) (builtProviderConfigWrite, error) {
	...
	return builtProviderConfigWrite{
		kind:              "external_oidc",
		keyHash:           audit.IdentityHash(issuer + "|" + clientID),
		issuerHash:        audit.IdentityHash(issuer),
		clientIDHash:      audit.IdentityHash(clientID),
		configurationJSON: string(configJSON),
		secretJSON:        string(secretJSON),
	}, nil
}

func buildSAMLProviderConfigWrite(body providerConfigWriteRequest) (builtProviderConfigWrite, error) {
	...
	return builtProviderConfigWrite{
		kind:              "external_saml",
		keyHash:           audit.IdentityHash(entityID),
		entityIDHash:      audit.IdentityHash(entityID),
		metadataURLHash:   audit.IdentityHash(metadataURL),
		configurationJSON: string(configJSON),
		secretJSON:        string(secretJSON),
...
func buildGitHubProviderConfigWrite(body providerConfigWriteRequest) (builtProviderConfigWrite, error) {
	clientID := strings.TrimSpace(body.ClientID)
	clientSecret := body.ClientSecret
	...
	return builtProviderConfigWrite{
		...
		keyHash:           audit.IdentityHash(keySeed + "|" + clientID),
		issuerHash:        audit.IdentityHash(keySeed),
		clientIDHash:      audit.IdentityHash(clientID),
		configurationJSON: string(configJSON),
		secretJSON:        string(secretJSON),
	}, nil
