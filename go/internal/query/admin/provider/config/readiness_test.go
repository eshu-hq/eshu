...
func TestHandleEnableAdminProviderConfigRejectsMissingRedirectURL(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.label, func(t *testing.T) {
			...
			if !hasAuditReason(audit, "provider_config_enable_missing_login_field") {
...
// MutationHandler.ReadStore's doc comment): when ReadStore
...
func TestHandleEnableAdminProviderConfigReadinessGapFailsOpen(t *testing.T) {
	...
	cases := []struct {
		...
		{"provider config not found in read store", &fakeAdminProviderConfigReadStore{details: map[string]Detail{}}},
	}
	for _, tc := range cases {
		t.Run(tc.label, func(t *testing.T) {
			t.Parallel()
