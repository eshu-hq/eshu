...
- `queryauth` owns the context key, `AuthContext`, and `AuthMode`. Do not
...
- This package MUST NOT import `querycontract`. `querycontract` reads
  `queryauth.AuthContext`, and the session import of `queryauth` would
  close a cycle through the reverse edge.

## Verification

From `go/`:
`go test ./internal/query/auth/session/ ./internal/query/queryauth/ -count=1`.
