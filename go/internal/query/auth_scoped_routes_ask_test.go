...
// a non-orchestration whole-graph route such as code/relationships (still in
// the #5167 pendingRowFilteringRoutes backlog, owned by a different family
// workstream) is not. POST /api/v0/code/dead-code was this negative example
// until the #5167 code family bound its candidate scan to the caller's grant
// (deadCodeCandidateRows), and POST /api/v0/code/bundles until it gated a
// scoped caller to visibility = 'public' packages (#5167); code/relationships
// replaces it because its handler still reads no grant.
...
func TestScopedHTTPRoute_Ask(t *testing.T) {
	...
	cases := []struct {
		...
		{http.MethodPost, "/api/v0/code/relationships", false}, // whole-graph, not allowlisted
		{http.MethodGet, "/api/v0/code/relationships", false},  // not allowlisted under any method
