...
// scopedContainerImageTagHistoryRoute reports whether the request targets the
...
// tokens, the same rule scopedContainerImageIdentityRoute applies to images
// with no source correlation.
func scopedContainerImageTagHistoryRoute(r *http.Request) bool {
	return r.Method == http.MethodGet && r.URL.Path == "/api/v0/images/tag-history"
}
