...
// TestMain registers this family's capabilities before any test runs.
//
// In production the registration comes from contract/cicd.go, which
...
func TestMain(m *testing.M) {
	querycontract.RegisterCapabilities(
		...
		querycontract.CapabilityRegistration{Capability: AggregateCapability, Support: Support()},
	)
	os.Exit(m.Run())
}
