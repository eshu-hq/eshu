...
func buildDeadCodeAnalysis(results []map[string]any, excluded []string, stats deadCodePolicyStats) map[string]any {
	...
	return map[string]any{
		...
		"notes": []string{
			...
			"go CLI registrations/signatures, stdlib HTTP registrations/signatures, controller-runtime reconcile signatures, Python FastAPI/Flask/Celery decorator roots, and JavaScript/TypeScript Next.js, Express, Koa, Fastify, NestJS, Node, Hapi, migration, interface, module-contract, and static-registry roots are modeled as derived roots",
