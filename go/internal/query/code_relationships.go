...
` + graphSemanticMetadataProjection() + `
		       ,collect(DISTINCT {direction: 'outgoing', type: type(outgoingRel), call_kind: outgoingRel.call_kind, confidence: outgoingRel.confidence, resolution_method: outgoingRel.resolution_method, reason: outgoingRel.reason, target_name: target.name, target_id: coalesce(target.id, target.uid)}) as outgoing,
		       collect(DISTINCT {direction: 'incoming', type: type(incomingRel), call_kind: incomingRel.call_kind, confidence: incomingRel.confidence, resolution_method: incomingRel.resolution_method, reason: incomingRel.reason, source_name: source.name, source_id: coalesce(source.id, source.uid)}) as incoming
		LIMIT 2
	`
}
