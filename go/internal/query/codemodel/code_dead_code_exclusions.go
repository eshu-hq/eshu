...
// FilterResultsByDecoratorExclusions drops results the decorator exclusions suppress.
...
// ResultMatchesDecoratorExclusion reports whether metadata matches a decorator exclusion.
...
// NormalizeDecoratorName normalizes a decorator name for exclusion matching.
...
// DeadCodeIsTestFile reports whether the candidate lives in a test file.
func DeadCodeIsTestFile(result map[string]any, entity *querycontract.EntityContent) bool {
...
// DeadCodeIsGeneratedCode reports whether the candidate is generated or tool-owned code.
func DeadCodeIsGeneratedCode(result map[string]any, entity *querycontract.EntityContent) bool {
	path := strings.ToLower(deadCodeEntityPath(result, entity))
	base := filepath.Base(path)
	switch {
