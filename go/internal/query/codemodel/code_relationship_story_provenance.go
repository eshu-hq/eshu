...
// RelationshipStoryProvenance derives the provenance envelope for one story row.
func RelationshipStoryProvenance(row map[string]any) map[string]any {
	...
	if querycontract.BoolVal(row, "why_trail_truncated") {
...
func relationshipStoryProvenanceReason(row map[string]any) string {
	if reason := strings.TrimSpace(querycontract.StringVal(row, "reason")); reason != "" {
		return reason
	}
	return "not_reported"
}
...
func relationshipStoryProvenanceSourceFamily(row map[string]any) string {
	if querycontract.StringVal(row, "type") == "TAINT_FLOWS_TO" || strings.TrimSpace(querycontract.StringVal(row, "why_trail_json")) != "" {
		return relationshipStoryProvenanceValueFlowEdge
	}
	if strings.TrimSpace(querycontract.StringVal(row, "resolution_method")) != "" {
		return relationshipStoryProvenanceCodeEdge
	}
	if strings.TrimSpace(querycontract.StringVal(row, "confidence_basis")) != "" ||
		strings.TrimSpace(querycontract.StringVal(row, "resolution_source")) != "" ||
		strings.TrimSpace(querycontract.StringVal(row, "evidence_type")) != "" ||
		len(querycontract.StringSliceVal(row, "evidence_kinds")) > 0 ||
		querycontract.IntVal(row, "evidence_count") > 0 {
...
func relationshipStoryWhyTrail(row map[string]any) []any {
	raw := strings.TrimSpace(querycontract.StringVal(row, "why_trail_json"))
	if raw == "" {
		return nil
	}
	var decoded []any
