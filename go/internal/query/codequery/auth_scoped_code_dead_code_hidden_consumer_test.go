...
func (s *deadCodeHiddenConsumerStore) CodeReachabilityIncomingEntityIDs(
	...
	incoming := make(map[string]deadcode.DeadCodeIncomingEdge, len(entityIDs))
	for _, entityID := range entityIDs {
		...
		incoming[entityID] = deadcode.DeadCodeIncomingEdge{HiddenConsumer: true}
	}
	return incoming, nil
}
...
func TestDeadCodeKeepsACandidateWhoseOnlyConsumerIsOutsideTheGrant(t *testing.T) {
	...
	if got, want := result["classification"], codemodel.DeadCodeClassificationAmbiguous; got != want {
		t.Fatalf("classification = %v, want %q for a candidate whose only consumer is hidden", got, want)
	}
	if hidden, _ := result[codemodel.DeadCodeHiddenConsumerResultKey].(bool); !hidden {
		t.Fatalf("%s missing from the kept candidate: %s", codemodel.DeadCodeHiddenConsumerResultKey, rec.Body.String())
	}
...
func TestDeadCodeInvestigateReportsThePermissionHiddenConsumerReason(t *testing.T) {
	...
	if !slices.Contains(reasons, any(codemodel.DeadCodeHiddenConsumerReason)) {
		t.Fatalf("ambiguity_reasons = %#v, want %q", reasons, codemodel.DeadCodeHiddenConsumerReason)
	}
	if strings.Contains(rec.Body.String(), codeGrantOtherRepo) {
		t.Fatalf("response named the ungranted consumer repository %q: %s", codeGrantOtherRepo, rec.Body.String())
	}
...
// moved to code_dead_code_incoming_probe_test.go (#6060 lane A code L3 PR2):
// both call deadCodeResultsWithGraphIncomingEdges directly to pin the graph
// probe's shape, so they stay white-box tests in package query beside that
// file's other pins of the same method instead of exporting it.
