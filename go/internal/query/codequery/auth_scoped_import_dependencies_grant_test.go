	...
	for _, answer := range g.answers {
		...
		for _, seed := range answer.rows {
			if importGrantRowAdmitted(normalized, params, seed) {
				rows = append(rows, maps.Clone(seed.row))
			}
		}
		return rows, nil
	}
...
// importGrantImportEdge is one IMPORTS row shaped for codemodel.DirectImportRowsCypher,
// codemodel.PackageImportRowsCypher and codemodel.FileImportCycleEdgeRowsCypher, all of which
...
// codemodel.CrossModuleCallRowsCypher, whose pattern binds two Repository aliases.
