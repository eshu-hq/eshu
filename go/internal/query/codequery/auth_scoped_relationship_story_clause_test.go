...
func TestRelationshipStoryBuildersBindTheGrantInTheAnchoringMatch(t *testing.T) {
	...
	req := codemodel.RelationshipStoryRequest{EntityID: storyGrantedAnchor, RelationshipType: "CALLS", Limit: 50}
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			cypher: firstOf(relationshipStoryOverrideRowsCypher(
				codemodel.RelationshipStoryRequest{QueryType: "overrides", RepoID: codeGrantGrantedRepo, Limit: 50}, access)),
			aliases: []string{"source", "target"},
		},
	} {
...
func TestRelationshipStoryInheritanceBoundsInteriorHopsOnCompatOnly(t *testing.T) {
	...
	access := storyScopedAccess()
	req := codemodel.RelationshipStoryRequest{EntityID: storyGrantedAnchor, RelationshipType: "INHERITS", Limit: 50}
