...
func TestDeadCodeScanContinuesAfterFirstLabelSaturates(t *testing.T) {
	...
	scan, err := handler.scanDeadCodeCandidates(context.Background(), deadcode.DeadCodeRequest{
		RepoID: "repo-1",
		Limit:  10,
	})
	if err != nil {
	...
	assertDeadCodeFairnessScanMetadata(
		...
		codeshaping.DeadCodeCandidateScanLimit(10),
	)
}
...
func TestDeadCodeInvestigationContinuesAfterFirstLabelSaturates(t *testing.T) {
	...
	scan, err := handler.scanDeadCodeInvestigation(context.Background(), deadcode.DeadCodeInvestigationRequest{
		RepoID: "repo-1",
		Limit:  10,
	})
	if err != nil {
...
func TestCrossRepoDeadCodeContinuesAfterFirstLabelSaturates(t *testing.T) {
	...
	scan, err := handler.scanCrossRepoDeadCodeCandidates(context.Background(), deadcode.CrossRepoDeadCodeRequest{
		RepoID: "repo-1",
		Limit:  10,
	})
	if err != nil {
