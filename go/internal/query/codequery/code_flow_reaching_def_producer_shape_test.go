...
func TestReachingDefAnswersFromCollectorPayloadShape(t *testing.T) {
	...
	function := codemodel.CodeFlowFunctionFromPayload(payload, "fact-1", "gen-1", "code_dataflow_function", time.Now())
	...
	rows := codeFlowReachingPayloads([]codemodel.CodeFlowFunction{function})
	if len(rows) != 1 {
