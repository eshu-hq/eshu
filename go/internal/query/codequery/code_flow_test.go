...
func TestCodeFlowCFGSummarySurfacesExactParserFactsAndBounds(t *testing.T) {
	...
	store := &fakeCodeFlowStore{model: codemodel.CodeFlowReadModel{
		Freshness: FreshnessFresh,
		Functions: []codemodel.CodeFlowFunction{
			{
	...
	if store.last.Kind != codemodel.CodeFlowKindCFGSummary || store.last.RepoID != "repo-1" || store.last.Limit != 2 {
...
func TestCodeFlowTaintPathSurfacesDerivedEvidence(t *testing.T) {
	...
	store := &fakeCodeFlowStore{model: codemodel.CodeFlowReadModel{
		Freshness: FreshnessFresh,
		TaintPaths: []codemodel.CodeFlowTaintPath{{
			RepoID: "repo-1", RelativePath: "src/handler.go", FunctionName: "handle",
	...
	env := decodeCodeFlowEnvelope(t, rec)
	if env.Truth == nil || env.Truth.Level != querycontract.TruthLevelDerived {
...
type fakeCodeFlowStore struct {
	called bool
	last   codemodel.CodeFlowFilter
	model  codemodel.CodeFlowReadModel
	err    error
}

func (s *fakeCodeFlowStore) ListCodeFlow(ctx context.Context, filter codemodel.CodeFlowFilter) (codemodel.CodeFlowReadModel, error) {
	s.called = true
	s.last = filter
