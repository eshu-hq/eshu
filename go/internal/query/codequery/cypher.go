...
const (
	...
	// readOnlyCypherCapability is the pre-move spelling of
	// codemodel.ReadOnlyCypherCapability, kept for the digest-pinned
	// handleCypherQuery body below.
	readOnlyCypherCapability = codemodel.ReadOnlyCypherCapability

	// visualizationGraphQueryCapability is the pre-move spelling of
	// codemodel.VisualizationGraphQueryCapability, kept for the
	// digest-pinned handleVisualizeQuery body below.
	visualizationGraphQueryCapability = codemodel.VisualizationGraphQueryCapability
)
...
func boundedReadOnlyCypher(query string, requestedLimit int) (string, int, error) {
	if err := codemodel.ValidateReadOnlyCypher(query); err != nil {
		return "", 0, err
	}
...
func (h *CodeHandler) visualizationGraphQueryTruth() *TruthEnvelope {
	return BuildTruthEnvelope(
		h.profile(),
		codemodel.VisualizationGraphQueryCapability,
