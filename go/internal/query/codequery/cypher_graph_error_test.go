...
func TestHandleCypherQueryMapsGraphReadAvailabilityErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unavailable", err: fmt.Errorf("private address: %w", querycontract.ErrGraphUnavailable), wantStatus: http.StatusServiceUnavailable, wantCode: ErrorCodeBackendUnavailable},
		{name: "deadline", err: fmt.Errorf("private query: %w", querycontract.ErrGraphReadDeadline), wantStatus: http.StatusGatewayTimeout, wantCode: ErrorCodeBackendTimeout},
	}
