...
func TestValidateReadOnlyCypher_RejectsMutationKeywords(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			err := codemodel.ValidateReadOnlyCypher(tc.query)
			if err == nil {
...
func TestValidateReadOnlyCypher_RejectsLongQueries(t *testing.T) {
	...
	err := codemodel.ValidateReadOnlyCypher(long)
	if err == nil {
...
func TestHandleCypherQueryRejectsUnsupportedProfileBeforeGraph(t *testing.T) {
	...
	h := &CodeHandler{Neo4j: stub, Profile: querycontract.ProfileLocalLightweight}
	...
	if got, want := envelope.Error.Profiles.Current, querycontract.ProfileLocalLightweight; got != want {
