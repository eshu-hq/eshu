...
func runDeadCodeSaturationScanner(t *testing.T, scanner string, store *deadCodeSaturationProbeStore) {
	...
	switch scanner {
	case "dead-code":
		_, err = handler.scanDeadCodeCandidates(ctx, deadcode.DeadCodeRequest{RepoID: "repo-1", Limit: 100})
	case "investigate":
		_, err = handler.scanDeadCodeInvestigation(ctx, deadcode.DeadCodeInvestigationRequest{RepoID: "repo-1", Limit: 100})
	case "cross-repo":
		_, err = handler.scanCrossRepoDeadCodeCandidates(ctx, deadcode.CrossRepoDeadCodeRequest{RepoID: "repo-1", Limit: 100})
	default:
