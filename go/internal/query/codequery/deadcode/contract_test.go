	...
	handler := &codequery.CodeHandler{
		...
		Neo4j: querytestutil.FakeGraphReader{
			RunFn: func(_ context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
				...
				if got, want := params["limit"], codeshaping.DeadCodeCandidateQueryLimit(codeshaping.DeadCodeDefaultLimit); got != want {
...
func TestHandleDeadCodeCandidateQueryRestrictsCodeEntityLabelsBeforeLimit(t *testing.T) {
	...
	for _, backend := range []querycontract.GraphBackend{querycontract.GraphBackendNeo4j, querycontract.GraphBackendNornicDB} {
		backend := backend
	...
	handler := &codequery.CodeHandler{
		...
		Neo4j: querytestutil.FakeGraphReader{
			RunFn: func(_ context.Context, _ string, params map[string]any) ([]map[string]any, error) {
				...
				if got, want := params["limit"], codeshaping.DeadCodeCandidateQueryLimit(2); got != want {
					t.Fatalf("params[limit] = %#v, want %#v", got, want)
				}
				return []map[string]any{
					{
...
func TestDeadCodeCandidateQueryLimitUsesMinimumPolicyWindowForSmallDisplayLimits(t *testing.T) {
	...
	if got, want := codeshaping.DeadCodeCandidateQueryLimit(5), codeshaping.DeadCodeCandidateQueryMin; got != want {
		t.Fatalf("codeshaping.DeadCodeCandidateQueryLimit(5) = %d, want %d", got, want)
	}
	if got, want := codeshaping.DeadCodeCandidateQueryLimit(deadcode.DeadCodeMaxLimit), codeshaping.DeadCodeCandidateQueryMax; got != want {
		t.Fatalf("codeshaping.DeadCodeCandidateQueryLimit(deadcode.DeadCodeMaxLimit) = %d, want %d", got, want)
	}
}
