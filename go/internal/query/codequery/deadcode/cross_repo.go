...
func (a *Analyzer) HandleCrossRepoDeadCode(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(a.deps.Profile, crossRepoDeadCodeCapability) {
	...
	if !a.deps.ApplySelector(w, r, &req.RepoID, crossRepoDeadCodeCapability) {
		return
	}
	if !a.applyConsumerRepositorySelectors(w, r, req.ConsumerRepoIDs, crossRepoDeadCodeCapability) {
		return
	}
	...
	evidence, hiddenConsumers, evidenceAvailable, err := a.crossRepoDeadCodeConsumerEvidence(
		r.Context(),
		req.RepoID,
		a.deps.ResultEntityIDs(scan.Active),
		req.ConsumerRepoIDs,
	)
	if err != nil {
		a.deps.WriteError(w, http.StatusInternalServerError, err.Error())
...
func (a *Analyzer) applyConsumerRepositorySelectors(
	...
	for i := range consumerRepoIDs {
		if !a.deps.ApplySelector(w, r, &consumerRepoIDs[i], capability) {
			return false
		}
	}
	return true
...
func (a *Analyzer) ScanCrossRepoDeadCodeCandidates(
	ctx context.Context,
...
func (a *Analyzer) bucketCrossRepoDeadCodeResults(
	ctx context.Context,
	req CrossRepoDeadCodeRequest,
	scan CrossRepoDeadCodeScan,
