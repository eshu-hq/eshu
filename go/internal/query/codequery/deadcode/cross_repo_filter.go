...
func (a *Analyzer) filterCrossRepoDeadCodeResultsWithoutProducerLocalIncomingEdges(
	ctx context.Context,
	...
	incoming, err := a.legacyDeadCodeIncomingEntityIDs(ctx, results)
	if err != nil {
		return nil, err
	}
...
// CrossRepoDeadCodeConsumerReadPlan plans the consumer-evidence reads for a
// cross-repo scan. It is exported because the staying consumer-read-plan
// test in codequery names it.
//
