...
func (s *deadCodeSaturationProbeStore) GetEntityContents(
	...
	for _, id := range entityIDs {
		entities[id] = &deadcode.EntityContent{
			EntityID:     id,
			RepoID:       "repo-1",
...
func (*deadCodeSaturationProbeStore) CodeReachabilityCoverage(
	...
	return deadcode.CodeReachabilityCoverage{Available: true}, nil
}
...
func (s *deadCodeSaturationProbeStore) recordIncoming(entityIDs []string) map[string]deadcode.DeadCodeIncomingEdge {
	s.reachabilityCalls++
	s.reachabilityIDs += len(entityIDs)
	incoming := make(map[string]deadcode.DeadCodeIncomingEdge, len(entityIDs))
	for _, id := range entityIDs {
		incoming[id] = deadcode.DeadCodeIncomingEdge{
...
func runDeadCodeSaturationScanner(t *testing.T, scanner string, store *deadCodeSaturationProbeStore) {
	...
	switch scanner {
	case "dead-code":
		_, err = analyzer.ScanDeadCodeCandidates(ctx, deadcode.DeadCodeRequest{RepoID: "repo-1", Limit: 100})
	case "investigate":
		_, err = analyzer.ScanDeadCodeInvestigation(ctx, deadcode.DeadCodeInvestigationRequest{RepoID: "repo-1", Limit: 100})
	case "cross-repo":
		_, err = analyzer.ScanCrossRepoDeadCodeCandidates(ctx, deadcode.CrossRepoDeadCodeRequest{RepoID: "repo-1", Limit: 100})
	default:
