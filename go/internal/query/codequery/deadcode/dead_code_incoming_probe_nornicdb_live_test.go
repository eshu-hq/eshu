...
func liveIncomingAccess() deadcode.RepositoryAccessFilter {
	return deadcode.RepositoryAccessFilter{AllowedRepositoryIDs: []string{codeGrantGrantedRepo}}
}

// deadCodeLiveNornicDBReader is a live-backend GraphQuery for tests that prove
// query text against a real NornicDB build. Copy of codequery's
// live_nornicdb_reader_test.go double, which cannot be imported here.
type deadCodeLiveNornicDBReader struct {
	driver   neo4jdriver.DriverWithContext
	database string
}

func newDeadCodeLiveNornicDBReader(driver neo4jdriver.DriverWithContext, database string) *deadCodeLiveNornicDBReader {
	return &deadCodeLiveNornicDBReader{driver: driver, database: database}
}

// Run executes a read-only Cypher query and returns results as maps.
func (r *deadCodeLiveNornicDBReader) Run(ctx context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
...
func TestLiveNornicDBDeadCodeIncomingProbeSeparatesSameMethodSources(t *testing.T) {
	...
	rows := runLiveDeadCodeIncomingStatement(
		ctx, t, driver,
		deadcode.BuildDeadCodeScopedIncomingBatchProbeCypher("Function", liveIncomingAccess()),
	)
	...
	for _, row := range rows {
		if got, want := querycontract.StringVal(row, "incoming_entity_id"), liveIncomingTarget; got != want {
			t.Fatalf("incoming_entity_id = %q, want %q; the projection was not evaluated", got, want)
		}
		if got, want := querycontract.StringVal(row, "resolution_method"), liveIncomingSameMethod; got != want {
			t.Fatalf("resolution_method = %q, want %q", got, want)
		}
		if querycontract.BoolVal(row, "in_grant") {
			granted++
			continue
		}
		hidden++
		if got, want := querycontract.IntVal(row, "edge_count"), 2; got != want {
...
func TestLiveNornicDBDeadCodeIncomingWithdrawnPairCollapses(t *testing.T) {
	...
	signal := runLiveDeadCodeIncomingStatement(ctx, t, driver, deadcode.BuildDeadCodeIncomingBatchProbeCypher("Function"))
	...
	if querycontract.StringVal(granted[0], "resolution_method") != querycontract.StringVal(signal[0], "resolution_method") {
...
func TestLiveNornicDBDeadCodeIncomingRejectsReturnDistinct(t *testing.T) {
	...
	`,
		...
		if got := querycontract.StringVal(rows[0], testCase.column); got != testCase.wantLiteral {
	...
	`)
	if len(comprehension) != 1 || !querycontract.BoolVal(comprehension[0], "in_grant") {
...
func runLiveDeadCodeIncomingStatement(
	...
	rows, err := newDeadCodeLiveNornicDBReader(driver, "nornic").Run(ctx, cypher, liveIncomingParams())
	if err != nil {
