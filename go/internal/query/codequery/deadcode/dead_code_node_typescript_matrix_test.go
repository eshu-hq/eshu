...
func TestHandleDeadCodeNodeTypeScriptFixtureMatrix(t *testing.T) {
	...
	entities := make(map[string]deadcode.EntityContent, len(matrix.Cases))
	for _, item := range matrix.Cases {
		rows = append(rows, nodeTypeScriptDeadCodeRow(item))
		entity := deadcode.EntityContent{
			EntityID:     item.ID,
			RepoID:       "repo-1",
	...
	if got, want := scanLimits, []int{codeshaping.DeadCodeCandidateQueryLimit(25)}; !equalIntSlices(got, want) {
	...
	for _, result := range results {
		...
		resultIDs[querycontract.StringVal(item, "entity_id")] = struct{}{}
	}
