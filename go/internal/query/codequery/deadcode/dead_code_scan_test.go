...
func TestFilterDuplicateDeadCodeRowsKeepsFirstEntityIDAcrossLabels(t *testing.T) {
	...
	firstPage := deadcode.FilterDuplicateDeadCodeRows([]map[string]any{
		deadCodeScanRow("routine-1", "refresh"),
	}, seen)
	secondPage := deadcode.FilterDuplicateDeadCodeRows([]map[string]any{
		deadCodeScanRow("routine-1", "refresh"),
	...
	if got, want := querycontract.StringVal(secondPage[0], "entity_id"), "routine-2"; got != want {
...
func TestHandleDeadCodeContinuesCandidateScanAfterPolicyExclusions(t *testing.T) {
	...
	pageLimit := codeshaping.DeadCodeCandidateQueryLimit(2)
...
func TestDeadCodeCandidateLabelsForScalaIncludesTraits(t *testing.T) {
	...
	labels := deadcode.DeadCodeCandidateLabelsForLanguage("scala")
	if !slices.Contains(labels, "Trait") {
		t.Fatalf("deadcode.DeadCodeCandidateLabelsForLanguage(scala) = %#v, want Trait", labels)
	}
	if slices.Contains(deadcode.DeadCodeCandidateLabelsForLanguage("sql"), "Trait") {
		t.Fatalf("deadcode.DeadCodeCandidateLabelsForLanguage(sql) includes Trait")
	}
}
...
func TestDeadCodeIncomingEntityIDsGroupsContentLookupsByRepository(t *testing.T) {
	...
	analyzer := newDeadCodeTestAnalyzer(store, nil)

	incoming, err := analyzer.DeadCodeIncomingEntityIDs(context.Background(), []map[string]any{
...
func TestFilterDeadCodeResultsBatchesSQLGraphIncomingProbes(t *testing.T) {
	...
	analyzer := newDeadCodeTestAnalyzer(fakeDeadCodeContentStore{}, graph)
	...
	if got, want := querycontract.StringVal(results[0], "entity_id"), "sql-dead"; got != want {
...
func (s *repoGroupedIncomingStore) DeadCodeIncomingEntityIDs(
	...
	for _, entityID := range entityIDs {
		if s.incomingByRepo[repoID][entityID] {
			incoming[entityID] = deadcode.DeadCodeIncomingEdge{
				MaxConfidence: codeprovenance.LegacyConfidence,
				Method:        codeprovenance.MethodUnspecified,
			}
		}
...
func (f *contentCandidateDeadCodeStore) DeadCodeCandidateRows(
	...
	if language != "" {
		...
		for _, row := range f.rows {
			if strings.EqualFold(querycontract.StringVal(row, "language"), language) {
...
func (f *batchingDeadCodeContentStore) GetEntityContent(ctx context.Context, entityID string) (*deadcode.EntityContent, error) {
	*f.singleCalls++
...
func (f *batchingDeadCodeContentStore) GetEntityContents(
	_ context.Context,
	entityIDs []string,
) (map[string]*deadcode.EntityContent, error) {
	*f.batchCalls++
	*f.batchIDs = append((*f.batchIDs)[:0], entityIDs...)
	entities := make(map[string]*deadcode.EntityContent, len(entityIDs))
	for _, entityID := range entityIDs {
