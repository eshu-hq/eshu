...
func TestDeadCodeIsRubyRootHonorsDowngradeVerdict(t *testing.T) {
	downgraded := codemodel.DeadCodeDowngradedRoots{"orders-index": {codemodel.RubyRailsControllerActionRootKind: {}}}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := codemodel.DeadCodeIsRubyRoot(tt.result, nil, stats, tt.downgraded)
			if got != tt.wantRoot {
				t.Fatalf("codemodel.DeadCodeIsRubyRoot = %v, want %v", got, tt.wantRoot)
			}
		})
	}
}
...
func TestFilterDeadCodeResultsByDefaultPolicyDowngradeFlipsToDead(t *testing.T) {
	...
	keptAlive, _ := deadcode.FilterDeadCodeResultsByDefaultPolicy([]map[string]any{result}, nil, nil)
	if len(keptAlive) != 0 {
		t.Fatalf("without a verdict a controller action must be kept alive (excluded), got %#v", keptAlive)
	}

	downgraded := codemodel.DeadCodeDowngradedRoots{"orders-index": {codemodel.RubyRailsControllerActionRootKind: {}}}
	flagged, _ := deadcode.FilterDeadCodeResultsByDefaultPolicy([]map[string]any{result}, nil, downgraded)
	if len(flagged) != 1 {
		t.Fatalf("a downgraded controller action must survive as a dead candidate, got %#v", flagged)
	}
	if querycontract.StringVal(flagged[0], "entity_id") != "orders-index" {
...
func TestLoadDeadCodeDowngradedRootsFailOpen(t *testing.T) {
	...
	t.Run("store error keeps everything", func(t *testing.T) {
		...
		got := analyzer.LoadDeadCodeDowngradedRoots(context.Background(), results)
		if got != nil {
	...
	t.Run("successful load surfaces downgraded kinds", func(t *testing.T) {
		store := &fakeVerdictContentStore{downgraded: map[string]map[string]struct{}{
			"orders-index": {codemodel.RubyRailsControllerActionRootKind: {}},
		}}
		analyzer := newDeadCodeTestAnalyzer(store, nil)
		got := analyzer.LoadDeadCodeDowngradedRoots(context.Background(), results)
		if !got.IsDowngraded("orders-index", codemodel.RubyRailsControllerActionRootKind) {
