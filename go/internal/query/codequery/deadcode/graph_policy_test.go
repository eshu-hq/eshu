...
func TestBuildDeadCodeGraphCypherKeepsCandidateReadSimple(t *testing.T) {
	...
	cypher := buildDeadCodeGraphCypher(true, querycontract.GraphBackendNornicDB)
...
func TestBuildDeadCodeGraphCypherKeepsTheScopedVariantSimple(t *testing.T) {
	...
	access := deadcode.RepositoryAccessFilter{AllowedRepositoryIDs: []string{"repo://tenant-a/granted-service"}}
	for _, hasRepoID := range []bool{true, false} {
		cypher := deadcode.BuildDeadCodeGraphCypherForLabel(hasRepoID, "Function", "go", access)
...
func TestBuildDeadCodeIncomingProbeCypherUsesBatchedExactEntityLookup(t *testing.T) {
	...
	cypher := deadcode.BuildDeadCodeIncomingBatchProbeCypher("Function")
	for _, want := range []string{
...
func TestDeadCodeCandidateScanLimitUsesFullWindowForSmallDisplayLimits(t *testing.T) {
	...
	if got, want := codeshaping.DeadCodeCandidateScanLimit(50), 2500; got != want {
		t.Fatalf("codeshaping.DeadCodeCandidateScanLimit(50) = %d, want %d", got, want)
	}
}
