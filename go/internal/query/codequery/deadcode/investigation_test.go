...
func TestDeadCodeInvestigationNextCallsMatchCandidateType(t *testing.T) {
	...
	scan := deadcode.DeadCodeInvestigationScan{
	...
	calls := deadcode.InvestigationNextCalls(scan)
...
func hasDeadCodeInvestigationRelationshipCall(calls []map[string]any, entityID string, relationshipType string) bool {
	for _, call := range calls {
		if querycontract.StringVal(call, "tool") != "get_code_relationship_story" {
			continue
		}
		...
		if querycontract.StringVal(args, "entity_id") == entityID && querycontract.StringVal(args, "relationship_type") == relationshipType {
			return true
		}
	}
	return false
...
func hasDeadCodeInvestigationToolCall(calls []map[string]any, tool string, entityID string) bool {
	for _, call := range calls {
		if querycontract.StringVal(call, "tool") != tool {
			continue
		}
		args, ok := call["arguments"].(map[string]any)
		if !ok {
		...
		if strings.Contains(querycontract.StringVal(args, "cypher_query"), entityID) || querycontract.StringVal(args, "entity_id") == entityID {
			return true
		}
	}
	return false
