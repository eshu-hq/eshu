...
type provenanceDeadCodeContentStore struct {
	...
	incomingEdges map[string]deadcode.DeadCodeIncomingEdge
}
...
func goDeadCodeEntity(entityID, name string) deadcode.EntityContent {
...
func TestDeadCodeWeakIncomingEdgeKeepsResultAsAmbiguous(t *testing.T) {
	...
	results := runDeadCodeWithContentIncoming(
		...
		map[string]deadcode.EntityContent{"weak-helper": goDeadCodeEntity("weak-helper", "weakHelper")},
		map[string]deadcode.DeadCodeIncomingEdge{
			"weak-helper": {
	...
	if got, want := results[0]["classification"], codemodel.DeadCodeClassificationAmbiguous; got != want {
	...
	for _, method := range []string{
		...
		t.Run(method, func(t *testing.T) {
			...
			results := runDeadCodeWithContentIncoming(
				...
				map[string]deadcode.EntityContent{"live-helper": goDeadCodeEntity("live-helper", "liveHelper")},
				map[string]deadcode.DeadCodeIncomingEdge{
...
func TestDeadCodeNoIncomingEdgeKeepsResultAsUnused(t *testing.T) {
	...
	results := runDeadCodeWithContentIncoming(
		...
		map[string]deadcode.EntityContent{"dead-helper": goDeadCodeEntity("dead-helper", "deadHelper")},
		map[string]deadcode.DeadCodeIncomingEdge{},
	)
	if got, want := len(results), 1; got != want {
		t.Fatalf("len(results) = %d, want %d results=%#v", got, want, results)
	...
	if got, want := results[0]["classification"], codemodel.DeadCodeClassificationUnused; got != want {
		t.Fatalf("results[0][classification] = %#v, want %#v", got, want)
	}
...
func TestDeadCodeUnspecifiedIncomingMethodTreatedStrong(t *testing.T) {
	...
	results := runDeadCodeWithContentIncoming(
		...
		map[string]deadcode.EntityContent{"legacy-helper": goDeadCodeEntity("legacy-helper", "legacyHelper")},
		map[string]deadcode.DeadCodeIncomingEdge{
...
func TestDeadCodeIncomingEdgeIsWeakBoundary(t *testing.T) {
	...
	if !deadcode.DeadCodeIncomingEdgeIsWeak(weakThreshold) {
		t.Fatalf("deadcode.DeadCodeIncomingEdgeIsWeak(%v) = false, want true", weakThreshold)
	}
	if deadcode.DeadCodeIncomingEdgeIsWeak(codeprovenance.Confidence(codeprovenance.MethodScopeUniqueName)) {
		t.Fatalf("deadcode.DeadCodeIncomingEdgeIsWeak(scope_unique_name) = true, want false")
	}
	if deadcode.DeadCodeIncomingEdgeIsWeak(codeprovenance.LegacyConfidence) {
		t.Fatalf("deadcode.DeadCodeIncomingEdgeIsWeak(LegacyConfidence) = true, want false")
	}
}
...
func TestDeadCodeWeakGraphIncomingEdgeKeepsSQLFunctionAsAmbiguous(t *testing.T) {
	...
	handler := &codequery.CodeHandler{
		...
		Neo4j: querytestutil.FakeGraphReader{
			...
			RunIncomingFn: func(_ context.Context, cypher string, _ map[string]any) ([]map[string]any, error) {
...
func TestDeadCodeStrongGraphIncomingEdgeFiltersSQLFunction(t *testing.T) {
	...
	handler := &codequery.CodeHandler{
		...
		Neo4j: querytestutil.FakeGraphReader{
			...
			RunIncomingFn: func(_ context.Context, _ string, _ map[string]any) ([]map[string]any, error) {
				return []map[string]any{
					{
