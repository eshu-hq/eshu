...
// The codemodel.CodeFlowKind/Filter/ReadModel/Function/TaintPath contract and the page
...
type CodeFlowStore interface {
	ListCodeFlow(context.Context, codemodel.CodeFlowFilter) (codemodel.CodeFlowReadModel, error)
}
...
func (h *CodeHandler) handleTaintPath(w http.ResponseWriter, r *http.Request) {
	h.handleCodeFlow(w, r, codemodel.CodeFlowKindTaintPath, CodeFlowTaintPathCapability)
}

func (h *CodeHandler) handleReachingDef(w http.ResponseWriter, r *http.Request) {
	h.handleCodeFlow(w, r, codemodel.CodeFlowKindReachingDef, CodeFlowReachingDefCapability)
}

func (h *CodeHandler) handleCFGSummary(w http.ResponseWriter, r *http.Request) {
	h.handleCodeFlow(w, r, codemodel.CodeFlowKindCFGSummary, CodeFlowCFGSummaryCapability)
}

func (h *CodeHandler) handlePDGSummary(w http.ResponseWriter, r *http.Request) {
	h.handleCodeFlow(w, r, codemodel.CodeFlowKindPDGSummary, CodeFlowPDGSummaryCapability)
}
...
func (h *CodeHandler) handleCodeFlow(
	...
	if state, ok := unsupportedCodeFlowLanguage(req); ok {
		h.writeCodeFlowResponse(w, r, capability, kind, req, codemodel.CodeFlowReadModel{}, codeFlowPayload{
	...
	filter := codemodel.CodeFlowFilter{
		Kind:     kind,
		RepoID:   req.RepoID,
		Language: req.Language,
		Symbol:   req.Symbol,
...
func buildCodeFlowPayload(kind codemodel.CodeFlowKind, req codeFlowRequest, model codemodel.CodeFlowReadModel) codeFlowPayload {
	truncated := false
	coverageState := "exact"
	...
	if kind == codemodel.CodeFlowKindTaintPath {
	...
	extra := map[string]any{}
	switch kind {
	case codemodel.CodeFlowKindTaintPath:
		extra["paths"] = codeFlowPathPayloads(paths)
	case codemodel.CodeFlowKindReachingDef:
		extra["definitions"] = codeFlowReachingPayloads(functions)
	case codemodel.CodeFlowKindCFGSummary:
		extra["functions"] = codeFlowFunctionPayloads(functions)
	case codemodel.CodeFlowKindPDGSummary:
...
func (h *CodeHandler) writeCodeFlowResponse(
	...
	kind codemodel.CodeFlowKind,
	req codeFlowRequest,
	model codemodel.CodeFlowReadModel,
...
func (r *codeFlowRequest) normalize() {
	...
	if r.Limit <= 0 {
		r.Limit = codemodel.CodeFlowDefaultLimit
	}
	if r.Limit > codemodel.CodeFlowMaxLimit {
		r.Limit = codemodel.CodeFlowMaxLimit
	}
	if r.Line < 0 {
		r.Line = 0
	}
...
func codeFlowFunctionPayloads(functions []codemodel.CodeFlowFunction) []map[string]any {
...
func codeFlowReachingPayloads(functions []codemodel.CodeFlowFunction) []map[string]any {
	out := make([]map[string]any, 0, len(functions))
	for _, function := range functions {
		row := codeFlowFunctionBasePayload(function)
		row["fact_label"] = "exact_parser_fact"
...
func codeFlowPDGPayloads(functions []codemodel.CodeFlowFunction) []map[string]any {
	out := make([]map[string]any, 0, len(functions))
	for _, function := range functions {
		row := codeFlowFunctionBasePayload(function)
...
func codeFlowPathPayloads(paths []codemodel.CodeFlowTaintPath) []map[string]any {
...
func codeFlowFunctionBasePayload(function codemodel.CodeFlowFunction) map[string]any {
	row := map[string]any{
...
func codeFlowAmbiguity(functions []codemodel.CodeFlowFunction, req codeFlowRequest) map[string]any {
...
func codeFlowResultCount(kind codemodel.CodeFlowKind, functions []codemodel.CodeFlowFunction, paths []codemodel.CodeFlowTaintPath) int {
	if kind == codemodel.CodeFlowKindTaintPath {
		return len(paths)
	}
...
func codeFlowTruthBasis(kind codemodel.CodeFlowKind) TruthBasis {
	if kind == codemodel.CodeFlowKindTaintPath || kind == codemodel.CodeFlowKindPDGSummary {
