...
const (
	...
	// CallGraphMetricsEdgeScanLimit aliases the metrics leaf's bound so
	// every existing caller (root seam, tests, the methods below) keeps
	// its spelling with a single source of truth.
	CallGraphMetricsEdgeScanLimit = metrics.CallGraphMetricsEdgeScanLimit
	// callGraphMetricsEdgeScanLimit is the pre-move spelling of
	// CallGraphMetricsEdgeScanLimit, kept for the digest-pinned
...
// callGraphMetricsEdgesCypher forwards to the metrics leaf so
...
// The edge-pass builder lives in the metrics leaf
// (metrics.CallGraphMetricsEdgesCypher); the forwarder above keeps this
// file's digest-pinned method body unchanged.
