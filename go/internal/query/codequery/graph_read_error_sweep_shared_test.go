...
// This is codequery's copy of root's graph_read_error_sweep_shared_test.go.
// A _test.go symbol is not importable across a package boundary, and both
// packages have their own graph-read availability regressions, so each keeps
// its own copy rather than one reaching into the other's test files.
...
func graphReadSweepCases() []graphReadSweepCase {
	return []graphReadSweepCase{
		{name: "unavailable", err: fmt.Errorf("private graph detail: %w", querycontract.ErrGraphUnavailable), wantStatus: http.StatusServiceUnavailable, wantCode: ErrorCodeBackendUnavailable},
		{name: "deadline", err: fmt.Errorf("private graph detail: %w", querycontract.ErrGraphReadDeadline), wantStatus: http.StatusGatewayTimeout, wantCode: ErrorCodeBackendTimeout},
	}
}
