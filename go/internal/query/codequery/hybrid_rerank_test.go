...
func TestCodeSearchContentResultsAreHybridReranked(t *testing.T) {
	...
	handler := &CodeHandler{
		...
		HybridRanker: codemodel.NewCodeHybridRanker(true),
	}
...
func TestCodeHybridRerankNeverInvokesProviderEmbedder(t *testing.T) {
	...
	ranker := codemodel.NewCodeHybridRanker(true)
...
func TestCodeSearchHybridRerankFallsBackToLexicalOrder(t *testing.T) {
	...
	disabled := codemodel.NewCodeHybridRanker(false)
	...
	_, applied = codemodel.NewCodeHybridRanker(true).Rerank(context.Background(), "repo-team-a", "alpha", single)
	if applied {
