...
func TestImportDependencyUniqueModulesPreservesRepositoryAndLanguageIdentity(t *testing.T) {
	...
	modules := codemodel.ImportDependencyUniqueModules([]map[string]any{
...
func TestImportDependencyCypherSupportsCanonicalAndLegacyFileLanguageProperties(t *testing.T) {
	...
	for name, cypher := range map[string]string{
		"direct imports": codemodel.DirectImportRowsCypher(codemodel.ImportDependencyRequest{RepoID: "repo-1", Language: "python"}),
		"cycle edges":    codemodel.FileImportCycleEdgeRowsCypher(codemodel.ImportDependencyRequest{RepoID: "repo-1"}),
		"cross calls": codemodel.CrossModuleCallRowsCypher(
			codemodel.ImportDependencyRequest{RepoID: "repo-1", Language: "python"},
			nil,
			nil,
		),
	} {
...
func TestHandleImportDependencyInvestigationFailsClosedWhenModuleMembershipOverflows(t *testing.T) {
	...
	handler := &CodeHandler{Neo4j: fakeGraphReader{run: func(
		...
		return make([]map[string]any, querycontract.ImportDependencyInternalScanLimit+1), nil
	}}}
