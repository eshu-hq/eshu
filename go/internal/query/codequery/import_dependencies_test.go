...
func TestFileImportCycleEdgeRowsCypherPreservesUnscopedRepositoryShape(t *testing.T) {
	...
	cypher := codemodel.FileImportCycleEdgeRowsCypher(codemodel.ImportDependencyRequest{
	...
	handler := &CodeHandler{
		Neo4j: fakeGraphReader{
			run: func(_ context.Context, _ string, params map[string]any) ([]map[string]any, error) {
				if got, want := params["scan_limit"], querycontract.ImportDependencyInternalScanLimit+1; got != want {
