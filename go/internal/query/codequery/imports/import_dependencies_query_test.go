...
func TestBuildFileImportCycleRowsFindsReciprocalEdgesAndPaginates(t *testing.T) {
	...
	row := rows[0]
	if got, want := codequery.StringVal(row, "source_file"), "src/a.py"; got != want {
		t.Fatalf("source_file = %q, want %q", got, want)
	}
	if got, want := codequery.StringVal(row, "target_file"), "src/b.py"; got != want {
		t.Fatalf("target_file = %q, want %q", got, want)
	}
	if got, want := codequery.IntVal(row, "source_line_number"), 8; got != want {
		t.Fatalf("source_line_number = %d, want earliest %d", got, want)
	}
	if got, want := codequery.IntVal(row, "back_edge_line_number"), 13; got != want {
...
func TestUniquePackageImportRowsOrdersLogicalIdentity(t *testing.T) {
	...
	if got, want := codequery.StringVal(rows[0], "target_module"), "alpha"; got != want {
		t.Fatalf("first target_module = %q, want %q", got, want)
	}
	if got, want := codequery.StringVal(rows[1], "target_module"), "zeta"; got != want {
...
func TestFilterCrossModuleCallRowsDropsCrossRepositoryRowsBeforePaging(t *testing.T) {
	...
	if got, want := codequery.StringVal(filtered[0], "repo_id"), "repo-1"; got != want {
...
} // importDependencyCycleProofEdge builds one cycle-proof edge row. Copy of
// import_dependencies_exactness_test.go, which stays in codequery: a _test.go
// symbol cannot be imported here.
