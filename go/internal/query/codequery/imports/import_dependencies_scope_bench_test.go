...
func BenchmarkImportDependencyScopeLookup(b *testing.B) {
	...
	b.Run("indexed-production", func(b *testing.B) {
		for range b.N {
			if got := len(codemodel.FilterImportDependencyScopeRows(rows, "repo_id", "source_path", scopes)); got != size {
				b.Fatalf("matched rows = %d, want %d", got, size)
			}
		}
	})
...
func linearImportDependencyScopeMatches(rows, scopes []map[string]any) int {
	...
	for _, row := range rows {
		for _, scope := range scopes {
			if codequery.StringVal(row, "repo_id") == codequery.StringVal(scope, "repo_id") &&
				codequery.StringVal(row, "source_path") == codequery.StringVal(scope, "path") {
				matched++
				break
			}
		}
