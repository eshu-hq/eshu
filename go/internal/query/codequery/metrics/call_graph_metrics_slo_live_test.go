...
func TestCallGraphMetricsInteractiveSLO(t *testing.T) {
	...
	handler := &codequery.CodeHandler{Neo4j: newMetricsLiveNornicDBReader(driver, database)}
...
func assertCallGraphMetricsSLOResult(t *testing.T, metricType string, data map[string]any) {
	...
	if got := querycontract.IntVal(data, "count"); got != wantCount {
		t.Fatalf("%s count = %d, want %d", metricType, got, wantCount)
	}
	if got := querycontract.BoolVal(data, "truncated"); got != wantTruncated {
		t.Fatalf("%s truncated = %t, want %t", metricType, got, wantTruncated)
	}
}

// metricsLiveNornicDBReader is the test-only live-backend GraphQuery for this
// leaf's SLO proof. It mirrors codequery's live_nornicdb_reader_test.go helper,
// which is unexported and therefore unreachable from package metrics_test: it
// opens a read session per call and decodes rows with the same Keys/Values
// mapping. It deliberately carries none of the production read policy
// (deadlines, retries, telemetry): the proof pins backend-compatible query
// TEXT, not the policy layer.
type metricsLiveNornicDBReader struct {
	driver   neo4jdriver.DriverWithContext
	database string
}

// newMetricsLiveNornicDBReader constructs a live-backend GraphQuery for the
// SLO proof.
func newMetricsLiveNornicDBReader(driver neo4jdriver.DriverWithContext, database string) *metricsLiveNornicDBReader {
	return &metricsLiveNornicDBReader{driver: driver, database: database}
}

// Run executes a read-only Cypher query and returns results as maps.
func (r *metricsLiveNornicDBReader) Run(ctx context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
...
func (r *metricsLiveNornicDBReader) RunSingle(ctx context.Context, cypher string, params map[string]any) (map[string]any, error) {
