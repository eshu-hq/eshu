...
func TestNornicDBIncomingOneHopCypherSeedsExactTarget(t *testing.T) {
	...
	for _, property := range []string{"uid", "id"} {
		...
		t.Run(property, func(t *testing.T) {
			...
			cypher, params := relationships.OneHopRelationshipsCypher(
				entityID,
				"incoming",
				"CALLS",
				"Function",
			...
			if got, want := params, map[string]any{"entity_id": entityID, "row_limit": relationships.FetchLimit}; !reflect.DeepEqual(got, want) {
...
func TestNornicDBIncomingOneHopRelationshipsPreservesRowBehavior(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got, _, err := relationships.OneHopRelationships(
				context.Background(),
				handler.Neo4j,
...
func TestNornicDBRelationshipsGraphRowDoesNotMutateMetadataRow(t *testing.T) {
	...
	// The root forwarder pre-resolves an empty label for an empty entity id
	// and reads the grant from the context; mirror both here since the
	// unexported reader cannot be named from this package.
	access := querycontract.RepositoryAccessFilterFromContext(ctx).WithCanonicalScopeRepositories()
	got, err := relationships.GraphRow(ctx, handler.Neo4j, "", "handlePayment", "", "", "CALLS", access, "")
	if err != nil {
	...
	if outgoing := relationships.MapRelationships(got["outgoing"]); len(outgoing) != 1 {
		t.Fatalf("got[outgoing] = %#v, want one relationship", got["outgoing"])
	}
	if incoming := relationships.MapRelationships(got["incoming"]); len(incoming) != 1 {
...
func TestNornicDBGraphLabelForContentEntityTypeStaysAlignedWithGraphLabels(t *testing.T) {
	...
	if got := relationships.NornicDBGraphLabelForContentEntityType(" Protocol "); got != "" {
		t.Fatalf("relationships.NornicDBGraphLabelForContentEntityType(%q) = %q, want empty unsupported label", " Protocol ", got)
	}
}
...
func TestNornicDBOneHopRelationshipsCypherUsesIndexedEntityLookup(t *testing.T) {
	...
	if got, want := params, map[string]any{"entity_id": "content-entity:handleRelationships", "row_limit": relationships.FetchLimit}; !reflect.DeepEqual(got, want) {
