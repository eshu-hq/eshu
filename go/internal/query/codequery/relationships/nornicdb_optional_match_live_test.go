...
//	cd go && go test ./internal/query/codequery/relationships -tags live_nornicdb_relationships_proof \
...
// relationshipsLiveNornicDBReader is a live-backend GraphQuery for tests
// that prove query text against a real NornicDB build. Copy of codequery's
// live_nornicdb_reader_test.go double, which cannot be imported here.
type relationshipsLiveNornicDBReader struct {
	driver   neo4jdriver.DriverWithContext
	database string
}

func newRelationshipsLiveNornicDBReader(driver neo4jdriver.DriverWithContext, database string) *relationshipsLiveNornicDBReader {
	return &relationshipsLiveNornicDBReader{driver: driver, database: database}
}

// Run executes a read-only Cypher query and returns results as maps.
func (r *relationshipsLiveNornicDBReader) Run(ctx context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
...
func (r *relationshipsLiveNornicDBReader) RunSingle(ctx context.Context, cypher string, params map[string]any) (map[string]any, error) {
...
// relationshipsLiveEntityLabel mirrors the root pinned label reader for a
// content-less handler: it probes the uid/id identity queries in order and
// returns the row's first known code-entity label.
func relationshipsLiveEntityLabel(ctx context.Context, graph querycontract.GraphQuery, entityID string) (string, error) {
	params := map[string]any{"entity_id": entityID}
	for _, property := range []string{"uid", "id"} {
		rows, err := graph.Run(ctx, relationships.EntityLabelCypher(property, false), params)
		if err != nil {
			return "", err
		}
		if len(rows) == 1 {
			return relationships.PrimaryEntityLabel(rows[0]), nil
		}
		if len(rows) > 1 {
			return "", nil
		}
...
func TestLiveNornicDBRelationshipsSurviveOptionalMatchProjection(t *testing.T) {
	...
	reader := newRelationshipsLiveNornicDBReader(driver, "nornic")

	// The root forwarder pre-resolves the entity label through the pinned
	// reader and reads the grant from the context before the leaf GraphRow
	// runs; mirror both here since the unexported reader cannot be named
	// from this package.
	label, err := relationshipsLiveEntityLabel(ctx, reader, "cls:ServiceDog")
	if err != nil {
		t.Fatalf("resolve entity label: %v", err)
	}
	access := querycontract.RepositoryAccessFilterFromContext(ctx).WithCanonicalScopeRepositories()
	row, err := relationships.GraphRow(ctx, reader, "cls:ServiceDog", "", "", "outgoing", "INHERITS", access, label)
	if err != nil {
	...
	// equalled "INHERITS".
	filtered := relationships.FilterResponse(row, "outgoing", "INHERITS")
	outgoing := relationships.MapRelationships(filtered["outgoing"])
	if len(outgoing) != 3 {
	...
	dog := byTarget["Dog"]
	if got := codequery.StringVal(dog, "target_id"); got != "cls:Dog" {
		t.Fatalf("Dog target_id = %q, want evaluated \"cls:Dog\"", got)
	}
	if got := codequery.StringVal(dog, "target_repo_id"); got != "repo:1" {
		t.Fatalf("Dog target_repo_id = %q, want enriched \"repo:1\"", got)
	}
	if got := codequery.StringVal(dog, "target_file_path"); got != "svc.py" {
		t.Fatalf("Dog target_file_path = %q, want enriched \"svc.py\"", got)
	}
	if got := codequery.StringVal(dog, "target_type"); got != "Class" {
	...
	if got := codequery.StringVal(serialize, "target_id"); got != "cls:SerializeMixin" {
...
func targetNames(rows []map[string]any) []string {
	...
	for _, row := range rows {
		names = append(names, codequery.StringVal(row, "target_name"))
	}
	return names
}
