...
// split out of codequery/relationship_story_resolution.go (#6060). The
// grant scope itself (codeContentGrantScope) stays in codequery: the
// staying resolver computes it from the request context and passes the
// granted repositories in.

// ClassHierarchyCandidates keeps only the candidates whose entity type
// can anchor a class-hierarchy story.
func ClassHierarchyCandidates(candidates []querycontract.EntityContent) []querycontract.EntityContent {
	out := make([]querycontract.EntityContent, 0, len(candidates))
	for _, candidate := range candidates {
		if ClassHierarchyEntityType(candidate.EntityType) {
...
// ClassHierarchyEntityType reports whether an entity type can anchor a
// class-hierarchy story.
func ClassHierarchyEntityType(entityType string) bool {
...
// GrantedCandidates resolves the target-name lookup for the
// relationship story with the caller's repository grant bound at the
// read.
//
...
func GrantedCandidates(
	...
	return ExactCandidatesPerRepository(ctx, content, target, allowed, limit)
}

// ExactCandidatesPerRepository reads the granted repositories
...
func ExactCandidatesPerRepository(
	ctx context.Context,
	content querycontract.ContentStore,
	target string,
	allowed []string,
	...
	candidates := make([]querycontract.EntityContent, 0, limit)
...
// the relationship-story target resolution above. It is exported so the
// twin-dispatch pin (TestSearchEntitiesForGrantMatchesTheLanguageQueryRead in
// package query) can drive it from outside codequery.
//
...
func SearchEntitiesForGrant(
	...
	// which is GrantedCandidates with a language named.
	//
