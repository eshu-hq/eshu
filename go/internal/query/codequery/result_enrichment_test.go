...
func TestEnrichGraphSearchResultsWithContentMetadata(t *testing.T) {
	...
	store := &enrichMetadataFakeStore{entities: []querycontract.EntityContent{{
		...
		EndLine:      20,
		Language:     "python",
		SourceCache:  "async def handler(): ...",
		Metadata:     map[string]any{"decorators": []any{"@route"}, "async": true},
	}}}
...
func TestEnrichGraphResultsWithContentMetadataByEntityIDPreservesPythonGraphMetadata(t *testing.T) {
	...
	store := &enrichMetadataFakeStore{entities: []querycontract.EntityContent{{
		...
		Language:     "python",
		SourceCache:  "async def handler(): ...",
		Metadata:     map[string]any{"docstring": "Handles incoming requests.", "decorators": []any{"@content"}, "async": false},
	}}}
...
// enrichMetadataFakeStore serves canned content entities to the enrichment
// proofs in this package, which pin the metadata mapping rather than the SQL
// feed behind it (that feed is root-owned ContentReader SQL, pinned by root's
...
func (s *enrichMetadataFakeStore) GetEntityContent(_ context.Context, entityID string) (*querycontract.EntityContent, error) {
	for i := range s.entities {
		if s.entities[i].EntityID == entityID {
			entity := s.entities[i]
			return &entity, nil
		}
	}
