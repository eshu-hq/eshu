...
func TestCodeSeamExportsForward(t *testing.T) {
	...
	if deadcode.CrossRepoDeadCodeUngrantedConsumerProbeQuery == "" {
		t.Fatal("deadcode.CrossRepoDeadCodeUngrantedConsumerProbeQuery is empty")
	}
	if !strings.Contains(deadcode.CrossRepoDeadCodeUngrantedConsumerProbeQuery, "code_reachability_rows") {
		t.Fatalf("deadcode.CrossRepoDeadCodeUngrantedConsumerProbeQuery = %q, want it to reference code_reachability_rows", deadcode.CrossRepoDeadCodeUngrantedConsumerProbeQuery)
	}

	if ErrCodeTopicBackendUnavailable == nil || ErrCodeTopicBackendUnavailable.Error() != "code topic investigation backend is unavailable" {
	...
	if got := deadcode.CrossRepoDeadCodeConfidenceLabel(0); got != "unknown" {
		t.Fatalf("deadcode.CrossRepoDeadCodeConfidenceLabel(0) = %q, want unknown", got)
	}
	...
	assertSeamAlias[deadcode.CodeReachabilityCoverage](deadcode.CodeReachabilityCoverage{Available: true})
	...
	assertSeamAlias[deadcode.CrossRepoDeadCodeEvidence](deadcode.CrossRepoDeadCodeEvidence{})
	...
	// Root's ContentReader still satisfies HardcodedSecretInvestigator; that
	// assertion lives in code_seam_crosspackage_test.go (package
	// codequery_test), which may import root package query without a cycle.
...
// its home type: assigning a value of the home type to the alias-typed
// parameter only compiles when the alias holds. Twin of the same-named
// helper in package query (impact_seam_export_test.go): a _test.go symbol is
