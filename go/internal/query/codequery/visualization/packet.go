...
// graph access.
func BuildGraphQueryVisualizationPacket(rows []map[string]any, truth *querycontract.TruthEnvelope) visualization.VisualizationPacket {
	builder := visualization.NewVisualizationBuilder(visualization.VisualizationViewGraphQuery, "graph query result")
	...
	if builder.Empty() {
		return visualization.UnsupportedVisualizationPacket(
			visualization.VisualizationViewGraphQuery,
			truth,
...
func addGraphQueryValue(builder *visualization.VisualizationBuilder, value any) {
	switch v := value.(type) {
...
func addGraphQueryNode(builder *visualization.VisualizationBuilder, node neo4jdriver.Node) string {
	elementID := strings.TrimSpace(node.ElementId)
	if elementID == "" {
		return ""
	}
	nodeID := visualization.VisualizationNodeID("graph", elementID)
	builder.AddNode(visualization.VisualizationNode{
		ID:       nodeID,
...
func addGraphQueryRelationship(builder *visualization.VisualizationBuilder, rel neo4jdriver.Relationship) {
	start := strings.TrimSpace(rel.StartElementId)
	end := strings.TrimSpace(rel.EndElementId)
	if start == "" || end == "" {
		return
	}
	builder.AddEdge(visualization.VisualizationEdge{
		Source:       visualization.VisualizationNodeID("graph", start),
		Target:       visualization.VisualizationNodeID("graph", end),
...
func addGraphQueryPath(builder *visualization.VisualizationBuilder, path neo4jdriver.Path) {
...
func graphQueryNodeLabel(node neo4jdriver.Node) string {
	for _, key := range graphQueryNodeLabelKeys {
		if label := strings.TrimSpace(querycontract.StringVal(node.Props, key)); label != "" {
			return label
		}
	}
