...
func TestFetchWorkloadRuntimeTopologyTruncationSurvivorSetIsOrderIndependentAboveLimit(t *testing.T) {
	...
	const instanceCount = querycontract.ContextStoryItemLimit + 1
	...
	if len(ascendingResult.instances) != querycontract.ContextStoryItemLimit {
		t.Fatalf("ascending survivor count = %d, want %d", len(ascendingResult.instances), querycontract.ContextStoryItemLimit)
	}
	if len(reversedResult.instances) != querycontract.ContextStoryItemLimit {
	...
	wantTopologyEdgeCount := 1 + querycontract.ContextStoryItemLimit
...
func assertRuntimeTopologyEdgeShape(t *testing.T, name string, edges []map[string]any, instanceCount int) {
	...
	if instanceOfCount != querycontract.ContextStoryItemLimit {
		t.Fatalf("%s INSTANCE_OF edge count = %d, want %d (one per retained instance)", name, instanceOfCount, querycontract.ContextStoryItemLimit)
	}
}
...
func buildDeterminismServiceStoryPayloadHash(t *testing.T, shuffle bool) string {
	...
	response := service.BuildServiceStoryResponse("orders-api", workloadContext)
