...
func (h *EntityHandler) enrichServiceStorySupplyChainEvidence(ctx context.Context, workloadContext map[string]any) error {
	...
	truncated := false
	if len(imageRefs) > querycontract.ServiceStoryItemLimit {
		imageRefs = imageRefs[:querycontract.ServiceStoryItemLimit]
		truncated = true
	}
	...
	for _, imageRef := range imageRefs {
		...
		identities, err := h.ContainerImageIdentities.ListContainerImageIdentities(ctx, supplychain.ContainerImageIdentityFilter{
			ImageRef: imageRef,
		...
		attachments, err := h.SBOMAttachments.ListSBOMAttestationAttachments(ctx, supplychain.SBOMAttestationAttachmentFilter{
	...
	sort.Slice(evidence, func(i, j int) bool {
		if left, right := querycontract.StringVal(evidence[i], "digest"), querycontract.StringVal(evidence[j], "digest"); left != right {
			return left < right
		}
		return querycontract.StringVal(evidence[i], "sbom_attachment_id") < querycontract.StringVal(evidence[j], "sbom_attachment_id")
	})
...
func serviceStoryDeploymentImageRefs(workloadContext map[string]any) []string {
	refs := make([]string, 0)
	refs = append(refs, querycontract.StringSliceVal(workloadContext, "image_refs")...)
	...
	return querycontract.UniqueSortedStrings(refs)
}
...
func serviceStoryAdmissibleImageIdentity(rows []supplychain.ContainerImageIdentityRow) (supplychain.ContainerImageIdentityRow, string) {
	if len(rows) == 0 {
		return supplychain.ContainerImageIdentityRow{}, "container_image_identity_missing"
	}
	if len(rows) > serviceStorySupplyChainReadLimit {
		return supplychain.ContainerImageIdentityRow{}, "container_image_identity_ambiguous"
	}
	admissible := make([]supplychain.ContainerImageIdentityRow, 0, len(rows))
	stale := false
	...
	if len(admissible) == 0 {
		if stale {
			return supplychain.ContainerImageIdentityRow{}, "container_image_identity_stale"
		}
		return supplychain.ContainerImageIdentityRow{}, "container_image_identity_not_admissible"
	}
	byDigest := map[string]supplychain.ContainerImageIdentityRow{}
...
func serviceStoryImageIdentityCanonical(row supplychain.ContainerImageIdentityRow) bool {
...
func serviceStoryAdmissibleSBOMAttachments(
	...
	rows []supplychain.SBOMAttestationAttachmentRow,
) ([]supplychain.SBOMAttestationAttachmentRow, string) {
	if len(rows) == 0 {
	...
	out := make([]supplychain.SBOMAttestationAttachmentRow, 0, len(rows))
	stale := false
	for _, row := range rows {
		if !serviceStorySourceFresh(row.SourceFreshness) {
			stale = true
...
func serviceStorySBOMAttachmentCanonical(subjectDigest string, row supplychain.SBOMAttestationAttachmentRow) bool {
...
func serviceStoryImagePackageEvidence(
	...
	sbom supplychain.SBOMAttestationAttachmentRow,
) map[string]any {
	...
	return map[string]any{
		...
		"image_ref":                  querycontract.FirstNonEmptyString(identity.ImageRef, anchorImageRef),
		...
		"evidence_fact_ids":          querycontract.UniqueSortedStrings(evidenceFactIDs),
	}
}

func serviceStorySetSupplyChainImagePackage(workloadContext map[string]any, imagePackage map[string]any) {
	supplyChain := querycontract.MapValue(workloadContext, "supply_chain_evidence")
	if supplyChain == nil {
