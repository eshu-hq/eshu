...
func TestFetchWorkloadContextUsesScalarQueriesForNornicDBOptionalProjectionSafety(t *testing.T) {
		...
		Neo4j: querytestutil.FakeWorkloadGraphReader{
			...
			RunFn: func(_ context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
				...
				case strings.Contains(cypher, "-[runsOn:RUNS_ON]->(p:Platform)"):
					...
					if got := querycontract.StringSliceVal(params, "instance_ids"); !reflect.DeepEqual(got, wantIDs) {
						t.Fatalf("params[instance_ids] = %#v, want %#v", got, wantIDs)
					}
					if got, want := querycontract.StringVal(params, "repo_id"), "repository:datax"; got != want {
	...
	bgProdPlatforms := querycontract.MapSliceValue(instances[0], "platforms")
	...
	if got, want := querycontract.StringVal(bgProdPlatforms[1], "topology_basis"), "direct_runtime"; got != want {
		t.Fatalf("direct topology_basis = %#v, want %#v", got, want)
	}
	directRelationships := querycontract.MapSliceValue(bgProdPlatforms[1], "topology_edges")
	if got, want := len(directRelationships), 1; got != want {
		t.Fatalf("len(instances[0].platforms[1].relationships) = %d, want %d", got, want)
	}
	if got, want := querycontract.StringVal(directRelationships[0], "relationship_type"), "RUNS_ON"; got != want {
		t.Fatalf("direct relationship_type = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringVal(directRelationships[0], "source_id"), "workload-instance:svc-orders:example-prod"; got != want {
		t.Fatalf("direct source_id = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringVal(directRelationships[0], "target_id"), "platform:kubernetes:example-prod"; got != want {
		t.Fatalf("direct target_id = %#v, want %#v", got, want)
	}
	overview := service.BuildServiceDeploymentOverview(ctx)
...
func TestFetchWorkloadContextPrefersInstanceRunsOnTruthOverProvisionedPlatformShortcut(t *testing.T) {
		...
		Neo4j: querytestutil.FakeWorkloadGraphReader{
			...
			RunFn: func(_ context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
				...
				case strings.Contains(cypher, "-[runsOn:RUNS_ON]->(p:Platform)"):
					...
					if got, want := querycontract.StringVal(params, "repo_id"), "repository:r_fdb82379"; got != want {
						t.Fatalf("params[repo_id] = %q, want %q", got, want)
					}
					return []map[string]any{
						{
	...
	opsQAPlatforms := querycontract.MapSliceValue(instances[1], "platforms")
...
func TestFetchDeploymentTraceKeepsProvisionedPlatformSeparateWhenInstanceRunsOnMissing(t *testing.T) {
	...
	platforms := querycontract.MapSliceValue(instances[0], "platforms")
	if len(platforms) != 0 {
		t.Fatalf("instance platforms = %#v, want direct RUNS_ON only", platforms)
	}
	provisioned := querycontract.MapSliceValue(ctx, "provisioned_platforms")
	...
	fallbackRelationships := querycontract.MapSliceValue(provisioned[0], "topology_edges")
	if got, want := len(fallbackRelationships), 2; got != want {
		t.Fatalf("len(fallback relationships) = %d, want %d", got, want)
	}
	if got, want := querycontract.StringVal(fallbackRelationships[0], "relationship_type"), "PROVISIONS_DEPENDENCY_FOR"; got != want {
		t.Fatalf("fallback dependency relationship_type = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringVal(fallbackRelationships[0], "target_id"), "repository:legacy"; got != want {
		t.Fatalf("fallback dependency target_id = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringVal(fallbackRelationships[1], "relationship_type"), "PROVISIONS_PLATFORM"; got != want {
		t.Fatalf("fallback platform relationship_type = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringVal(fallbackRelationships[1], "source_id"), "repository:runtime-config"; got != want {
		t.Fatalf("fallback platform source_id = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringVal(fallbackRelationships[1], "target_id"), "platform:ecs:shared-runtime-cluster"; got != want {
