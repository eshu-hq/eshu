...
func worstCaseWorkloadPlatformRows() []map[string]any {
	...
	for i := rowCount - 1; i >= 0; i-- {
		instance := i % querycontract.ContextStoryItemLimit // 50 distinct instance ids
		group := i / querycontract.ContextStoryItemLimit    // ~51 rows per instance
...
func TestWorstCaseWorkloadPlatformRowsExerciseEveryComparatorKey(t *testing.T) {
	...
	for _, row := range rows {
		instances[querycontract.StringVal(row, "instance_id")]++
		instanceName := querycontract.StringVal(row, "instance_id") + "\x00" + querycontract.StringVal(row, "platform_name")
		if seenInstanceName[instanceName] > 0 {
			nameTies++
		}
		seenInstanceName[instanceName]++
		instanceNameID := instanceName + "\x00" + querycontract.StringVal(row, "platform_id")
	...
	if len(instances) > querycontract.ContextStoryItemLimit {
		t.Fatalf("distinct instance ids = %d, want <= %d (production caps instance_ids at the truncated topology)",
			len(instances), querycontract.ContextStoryItemLimit)
	}
	if nameTies == 0 {
