...
func TestFetchProvisionedPlatformsReportsUniqueSentinel(t *testing.T) {
	...
	if got, want := len(result.rows), querycontract.ContextStoryItemLimit; got != want {
...
func TestFetchProvisionedPlatformsKeepsRepositoryTopologySeparate(t *testing.T) {
	...
	reader := querytestutil.FakeGraphReader{RunFn: func(_ context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
		...
		if got, want := querycontract.IntVal(params, "provisioned_platform_limit"), querycontract.ContextStoryItemLimit+1; got != want {
	...
	if got, want := querycontract.StringVal(result.rows[0], "topology_basis"), "provisioning_fallback"; got != want {
		t.Fatalf("topology_basis = %q, want %q", got, want)
	}
	edges := querycontract.MapSliceValue(result.rows[0], "topology_edges")
...
func TestFetchProvisionedPlatformsOrdersSamePlatformByRepositoryEndpoints(t *testing.T) {
	...
	for index, want := range []string{"repository:a-infra", "repository:z-infra"} {
		edges := querycontract.MapSliceValue(result.rows[index], "topology_edges")
		if got := querycontract.StringVal(edges[0], "source_id"); got != want {
...
func TestFetchProvisionedPlatformsTruncationSurvivorSetIsOrderIndependentAboveLimit(t *testing.T) {
	...
	const platformCount = querycontract.ContextStoryItemLimit + 1
	...
	if len(ascendingResult.rows) != querycontract.ContextStoryItemLimit {
		t.Fatalf("ascending survivor count = %d, want %d", len(ascendingResult.rows), querycontract.ContextStoryItemLimit)
	}
	if len(reversedResult.rows) != querycontract.ContextStoryItemLimit {
		t.Fatalf("reversed survivor count = %d, want %d", len(reversedResult.rows), querycontract.ContextStoryItemLimit)
	}
...
func provisionedPlatformIDs(rows []map[string]any) []string {
	...
	for _, row := range rows {
		ids = append(ids, querycontract.StringVal(row, "platform_id"))
	}
	return ids
}
...
func TestBuildDeploymentTraceResponseDoesNotCopyProvisioningUnderInstances(t *testing.T) {
	...
	if gotPlatforms := querycontract.MapSliceValue(instances[0], "platforms"); len(gotPlatforms) != 0 {
		t.Fatalf("instances[0].platforms = %#v, want direct RUNS_ON only", gotPlatforms)
	}
	if gotProvisioned := querycontract.MapSliceValue(got, "provisioned_platforms"); len(gotProvisioned) != 1 {
