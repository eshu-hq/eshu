...
func boundedTopologyProofEdges() []map[string]any {
	edges := make([]map[string]any, 0, querycontract.ContextStoryItemLimit*2)
	for index := range querycontract.ContextStoryItemLimit {
...
func appendUniqueTopologyEdgeLegacy(rows *[]map[string]any, seen map[string]int, edge map[string]any) {
	...
	if index, exists := seen[key]; exists {
		candidate, _ := json.Marshal(querycontract.MapValue(edge, "properties"))
		existing, _ := json.Marshal(querycontract.MapValue((*rows)[index], "properties"))
