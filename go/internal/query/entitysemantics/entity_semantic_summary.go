...
// buildEntitySemanticSummary renders a one-sentence, entity-type-specific
// prose summary from an entity's promoted metadata. The implementation moved
// from root's entity_summary.go for #6060 so a handler-family subpackage can
// build the same summary without importing root.
func buildEntitySemanticSummary(entity map[string]any) string {
	...
	name := querycontract.StringVal(entity, "name")
	...
	case "TypeAlias":
		...
		if params := querycontract.MetadataStringSlice(metadata, "type_parameters"); len(params) > 0 {
			fragments = append(fragments, "declares type parameters "+strings.Join(params, ", "))
		}
		if len(fragments) > 0 {
			return fmt.Sprintf("%s %s %s.", label, name, querycontract.JoinSentenceFragments(fragments))
		}
	case "TypeAnnotation":
	...
	case "TerragruntConfig":
		...
		includes := querycontract.MetadataStringSlice(metadata, "includes")
		inputs := querycontract.MetadataStringSlice(metadata, "inputs")
		locals := querycontract.MetadataStringSlice(metadata, "locals")
	...
	case "KustomizeOverlay":
		bases := querycontract.MetadataStringSlice(metadata, "bases")
		resourceRefs := querycontract.MetadataStringSlice(metadata, "resource_refs")
		helmRefs := querycontract.MetadataStringSlice(metadata, "helm_refs")
		imageRefs := querycontract.MetadataStringSlice(metadata, "image_refs")
		patchTargets := querycontract.MetadataStringSlice(metadata, "patch_targets")
	...
	case "ArgoCDApplicationSet":
		generatorRepos := querycontract.MetadataStringSlice(metadata, "generator_source_repos")
		templateRepos := querycontract.MetadataStringSlice(metadata, "template_source_repos")
		destServer, _ := metadata["dest_server"].(string)
		destNamespace, _ := metadata["dest_namespace"].(string)
		switch {
	...
	if pythonProfile.TypeAnnotation && label != "TypeAnnotation" {
		...
		case len(pythonProfile.TypeAnnotationKinds) > 1:
			fragments = append(fragments, fmt.Sprintf(
				...
				querycontract.JoinSentenceFragments(pythonProfile.TypeAnnotationKinds),
			))
	...
	if jsSemantics.MethodKind != "" {
		if querycontract.StringVal(entity, "language") == "javascript" {
