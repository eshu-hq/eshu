...
// promoted in query surfaces. The implementation moved from root's
// python_semantics_promotion.go for #6060 so a handler-family subpackage can
// build the same profile without importing root.
...
func PythonSemanticProfileFromMetadata(entityType string, metadata map[string]any) PythonSemanticProfile {
	...
	profile.Docstring = querycontract.MetadataString(metadata, "docstring")
	profile.TypeAnnotationCount = querycontract.IntVal(metadata, "type_annotation_count")
	profile.TypeAnnotationKinds = querycontract.StringSliceFromAny(metadata["type_annotation_kinds"])
	profile.AnnotationKind = querycontract.MetadataString(metadata, "annotation_kind")
	profile.Context = querycontract.MetadataString(metadata, "context")
...
func hasValues(value any) bool {
	...
	case []string:
		return len(querycontract.FilterEmptyStrings(typed)) > 0
	case []any:
		return len(typed) > 0
	default:
		return false
