...
| runtime-context join (EXPLAIN ANALYZE) | — | 0.725 ms execution | 200 candidates (27 corpus repos + 173 padding = enforced max page bound of 200), index-served on `fact_records_collector_status_active_idx`, 0 rows removed by filter |
...
20-repo corpus's natural size). The final query shape — including the
related_scope_ids predicates the Codex review added — runs Index Scan on
`fact_records_collector_status_active_idx` keyed on
(scope_id, generation_id, fact_kind), 64 index searches, 228 shared buffer
hits, execution time 0.725 ms, returning 30 rows (the related_scope_ids
predicates resolve 3 facts the direct-scope predicates miss — facts anchored
to a repository only through that array). Scaling from 27 to 200 candidates
(7.4x) costs 0.035 ms, so the join is comfortably sub-millisecond per page
on the real schema and does not change the findings handler's latency
envelope.
...
runtime-context join: 0.725 ms execution over 200 candidates (the enforced
