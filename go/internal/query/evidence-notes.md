...
Deployment trace keeps config-derived CloudResource rows visible without
promoting them into canonical `cloud_resources`. An explicit
`READS_CONFIG_FROM` artifact can produce a bounded candidate with
`match_basis=deployment_config_read_evidence`, but canonical cloud-resource
truth still requires a materialized
`WorkloadInstance-[:USES]->CloudResource` relationship.
...
-count=1` proves explicit config-read evidence produces a candidate with its
match basis and missing workload relationship disclosed while
`cloud_resources` remains empty.
...
exposes candidate results through `uncorrelated_cloud_resources`, its truncation
flag, `deployment_overview`, and `deployment_fact_summary`.
