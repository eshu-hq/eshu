...
// ManagementStore.{List,Count}UnmanagedCloudResources (management.go)
// -- so a single fakeIaCManagementStore (management_test.go) whose
...
func iacGrantTestFindings() []ManagementFindingRow {
...
// ManagementFilter choke point. get_iac_management_status and
...
func TestIaCManagementFamilyRoutesFilterByScopeGrant(t *testing.T) {
	...
	for _, route := range iacManagementFamilyRoutes() {
		t.Run(route.name, func(t *testing.T) {
			...
			handler := &Handler{Profile: querycontract.ProfileLocalAuthoritative, Management: store}
