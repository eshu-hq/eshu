...
func (h *Handler) handleAWSRuntimeDriftFindings(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), AWSRuntimeDriftFindingsCapability) {
		querycontract.WriteContractError(
			...
			AWSRuntimeDriftFindingsCapability,
			h.profile(),
			querycontract.RequiredProfile(AWSRuntimeDriftFindingsCapability),
		)
		return
	}
	...
	if h == nil || h.Management == nil {
		querycontract.WriteError(w, http.StatusServiceUnavailable, "AWS runtime drift finding store is required")
		return
	}

	// Access scoping (#5167 Group B): ManagementStore/ManagementFilter
...
func writeAWSRuntimeDriftFindings(
	...
	querycontract.WriteSuccess(w, r, http.StatusOK, map[string]any{
		...
		"truncated":            ManagementTruncated(filter.Offset, len(driftFindings), totalFindings),
		"next_offset":          ManagementNextOffset(filter.Offset, len(driftFindings), totalFindings),
...
// drift-findings page.
func normalizeAWSRuntimeDriftFindingsRequest(req managementRequest) (ManagementFilter, error) {
	...
	// the caller named no explicit kind. NormalizeManagementFindingKinds
...
func awsRuntimeDriftFindingRows(findings []ManagementFindingRow) []AWSRuntimeDriftFindingRow {
	out := make([]AWSRuntimeDriftFindingRow, 0, len(findings))
	for _, finding := range findings {
		out = append(out, AWSRuntimeDriftFindingRow{
			ManagementFindingRow: finding,
...
func awsRuntimeDriftOutcome(finding ManagementFindingRow) string {
...
func awsRuntimeDriftPromotionOutcome(finding ManagementFindingRow) string {
...
func awsRuntimeDriftPromotionReason(finding ManagementFindingRow) string {
	if finding.SafetyGate.ReviewRequired {
