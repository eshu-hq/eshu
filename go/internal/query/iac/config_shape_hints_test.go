...
func safeReadyFinding(resourceType, resourceID, arn string) ManagementFindingRow {
...
func TestConfigShapeHintForEachSupportedResourceType(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			candidate := terraformImportPlanCandidateForFinding(tc.finding, ManagementFilter{})
...
func TestConfigShapeHintNeverLeaksSensitiveValues(t *testing.T) {
	...
	candidate := terraformImportPlanCandidateForFinding(finding, ManagementFilter{})
	if candidate.ConfigShapeHint == nil {
		t.Fatal("ready candidate missing config_shape_hint")
	}
...
func TestConfigShapeHintAbsentForRefusedCandidate(t *testing.T) {
	...
	finding.SafetyGate = ManagementSafetyGate{
