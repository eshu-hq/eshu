...
func (h *Handler) handleTerraformImportPlanCandidates(w http.ResponseWriter, r *http.Request) {
	r, span := startQueryHandlerSpan(
		...
		TerraformImportCapability,
	)
	defer span.End()

	if querycontract.CapabilityUnsupported(h.profile(), TerraformImportCapability) {
		querycontract.WriteContractError(
			...
			TerraformImportCapability,
			h.profile(),
			querycontract.RequiredProfile(TerraformImportCapability),
		)
		return
	}
	...
	if resourceID := strings.TrimSpace(req.ResourceID); resourceID != "" && !strings.HasPrefix(resourceID, "arn:aws:") {
		querycontract.WriteError(w, http.StatusBadRequest, "resource_id must be the full AWS ARN for Terraform import-plan candidates")
		return
	}
...
func buildTerraformImportPlanCandidates(findings []ManagementFindingRow, filter ManagementFilter) terraformImportPlanSummary {
...
func terraformImportPlanCandidateForFinding(finding ManagementFindingRow, filter ManagementFilter) terraformImportPlanCandidate {
	...
	candidate := terraformImportPlanCandidate{
		...
		Warnings:     mergeStringSets(finding.WarningFlags, finding.SafetyGate.Warnings),
	...
	if !ok {
		...
		candidate.Warnings = mergeStringSets(candidate.Warnings, []string{"unsupported_terraform_import_mapping"})
		return candidate
	}
...
func terraformImportAccountID(finding ManagementFindingRow, filter ManagementFilter) string {
...
func terraformImportRegion(finding ManagementFindingRow, filter ManagementFilter) string {
...
func terraformImportDestinationHint(finding ManagementFindingRow) string {
...
func terraformImportConfigurationShape(finding ManagementFindingRow) string {
	if strings.TrimSpace(finding.MatchedTerraformModulePath) != "" {
...
func terraformImportEvidenceRefs(finding ManagementFindingRow) []string {
