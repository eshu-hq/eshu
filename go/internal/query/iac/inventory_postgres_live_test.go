...
CREATE TABLE fact_records (
  ...
  payload jsonb NOT NULL
	...
	started := time.Now()
	first, err := store.SearchActive(ctx, InventorySearch{
	...
	second, err := store.SearchActive(ctx, InventorySearch{
		Kind:      resourceKindResource,
	...
	search, err := store.SearchActive(ctx, InventorySearch{
		Kind:  resourceKindResource,
		Query: "logging.tf",
		Limit: 10,
	}, access)
	...
	if summary.Total != 4 || summary.ByKind[resourceKindResource] != 2 ||
		summary.ByKind[resourceKindModule] != 1 ||
		summary.ByKind[resourceKindDataSource] != 1 {
		t.Fatalf("current summary = %#v, want total 4 with 2/1/1 kinds", summary)
	}

	allScopes := querycontract.RepositoryAccessFilter{AllScopes: true}
	newer, err := store.SearchActive(ctx, InventorySearch{
		Kind:  resourceKindResource,
		Query: "replacement",
		Limit: 10,
	}, allScopes)
	...
	historical, err := store.SearchActive(ctx, InventorySearch{
		Kind:  resourceKindResource,
		Query: "historical",
		Limit: 10,
	}, allScopes)
	...
	unauthorized, err := store.SearchActive(ctx, InventorySearch{
		Kind:  resourceKindResource,
		Query: "private",
		Limit: 10,
	}, access)
	...
	literalWildcard, err := store.SearchActive(ctx, InventorySearch{
		Kind:  resourceKindResource,
		Query: "%",
		Limit: 10,
	}, access)
	...
	cancelNow()
	if _, err := store.SearchActive(cancelled, InventorySearch{
		Kind:  resourceKindResource,
		Limit: 10,
	}, allScopes); err == nil {
...
func issue5262ScopedAccess(repositoryID, scopeID string) querycontract.RepositoryAccessFilter {
