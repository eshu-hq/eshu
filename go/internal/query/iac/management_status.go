...
func (i *managementStatusInput) recordEvidence(evidenceType string) {
...
func deriveIaCManagementStatus(input managementStatusInput) string {
...
// P1-1). It is called both by AWSRuntimeDriftRowToManagement
// (list_aws_runtime_drift_findings, find_unmanaged_resources) and by
// AWSCloudRuntimeDriftDerivedStatus (management_transform.go), which the
...
func warningFlagsForManagementFinding(
	...
	return mergeStringSets(warnings, nil)
}
...
type managementEvidenceEnrichment struct {
	stateAddress          string
	configFile            string
	modulePath            string
...
func (e *managementEvidenceEnrichment) recordEvidence(atom postgres.AWSCloudRuntimeDriftEvidenceRow) {
	...
	case evidenceType == "terraform_state_resource" && (key == "resource_address" || key == "address"):
		e.stateAddress = firstNonEmpty(e.stateAddress, value)
	case evidenceType == "terraform_config_resource" && (key == "file_path" || key == "relative_path"):
		e.configFile = firstNonEmpty(e.configFile, value)
	case evidenceType == "terraform_config_resource" && key == "module_path":
		e.modulePath = firstNonEmpty(e.modulePath, value)
	...
	case strings.Contains(evidenceType, "cloudformation") ||
		...
		e.otherIaCSource = firstNonEmpty(e.otherIaCSource, value)
	}
}
...
func mergeStringSets(primary []string, secondary []string) []string {
