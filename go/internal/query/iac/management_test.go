...
// fakeIaCManagementStore is the shared test double for ManagementStore
...
type fakeIaCManagementStore struct {
	...
	observedFilter *ManagementFilter
...
func (f fakeIaCManagementStore) scopedRows(filter ManagementFilter) []ManagementFindingRow {
	if !filter.Scoped {
		return f.rows
	}
	...
	filtered := make([]ManagementFindingRow, 0, len(f.rows))
...
func (f fakeIaCManagementStore) ListUnmanagedCloudResources(
	...
	f.touchDB()
	rows := append([]ManagementFindingRow(nil), f.scopedRows(filter)...)
...
func (f fakeIaCManagementStore) CountUnmanagedCloudResources(_ context.Context, filter ManagementFilter) (int, error) {
	if filter.Scoped && len(filter.AllowedScopeIDs) == 0 {
		return 0, nil
	}
	f.touchDB()
