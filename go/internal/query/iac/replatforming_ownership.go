...
type replatformingOwnershipPacket struct {
	...
	Freshness querycontract.TruthFreshness `json:"freshness"`
...
func buildReplatformingOwnershipPacket(
	...
	packet := replatformingOwnershipPacket{
		ItemID:                       finding.ID,
		Provider:                     firstNonEmpty(finding.Provider, "aws"),
...
func ownershipRepositoryCandidates(finding ManagementFindingRow) ([]ReplatformingOwnerCandidate, []string) {
...
func ownershipAccountID(finding ManagementFindingRow, filter ManagementFilter) string {
...
func ownershipFreshnessForState(state ReplatformingSourceState) querycontract.TruthFreshness {
	switch state {
	case ReplatformingSourceStateStale:
		return querycontract.TruthFreshness{State: querycontract.FreshnessStale}
	case ReplatformingSourceStateUnavailable, ReplatformingSourceStateUnknown:
		return querycontract.TruthFreshness{State: querycontract.FreshnessUnavailable}
	default:
		return querycontract.TruthFreshness{State: querycontract.FreshnessFresh}
	}
}
...
func ownershipRecommendedNextCalls(finding ManagementFindingRow, missing []string) []map[string]any {
	var calls []map[string]any
	if querycontract.ContainsString(missing, ownershipMissingServiceAttribution) ||
		querycontract.ContainsString(missing, ownershipMissingEnvironmentAttribution) {
	...
	if querycontract.ContainsString(missing, ownershipMissingTerraformAddress) {
		calls = append(calls, map[string]any{
...
func ownershipPacketIsUnattributed(packet replatformingOwnershipPacket) bool {
	return querycontract.ContainsString(packet.MissingEvidence, ownershipMissingServiceAttribution) &&
		querycontract.ContainsString(packet.MissingEvidence, ownershipMissingEnvironmentAttribution)
}
