...
func (h *Handler) handleReplatformingOwnershipPackets(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), ReplatformingOwnershipCapability) {
		querycontract.WriteContractError(
			...
			ReplatformingOwnershipCapability,
			h.profile(),
			querycontract.RequiredProfile(ReplatformingOwnershipCapability),
		)
		return
	}
	...
	if h == nil || h.Management == nil {
		querycontract.WriteError(w, http.StatusServiceUnavailable, "IaC management store is required for replatforming ownership packets")
		return
	}
