...
// rollupFinding is this package's own copy of root's identically named
// fixture (replatforming_rollups_handler_test.go, #6642 Part A). Both copies
// exist because replatforming_rollups_handler_test.go stays in root (it
// drives the HTTP route through the root alias) while this file moved with
// the rest of the ownership-packet unit tests; root's copy inlines the ARN
// split instead of calling this package's unexported parseAWSManagementARN,
// which this copy can call directly.
func rollupFinding(arn, status, kind string, services, environments []string) ManagementFindingRow {
	parsed := parseAWSManagementARN(arn)
	return ManagementFindingRow{
		ID:                    "fact:" + arn,
		Provider:              "aws",
...
func ownershipFinding(arn, status, kind string, services, environments []string) ManagementFindingRow {
...
func TestBuildOwnershipPacketCloudOnlyNoServiceMatch(t *testing.T) {
	...
	filter := ManagementFilter{AccountID: "123456789012"}
	...
	if !querycontract.ContainsString(packet.MissingEvidence, "service_attribution") {
		t.Fatalf("missing_evidence must record absent service attribution: %#v", packet.MissingEvidence)
	}
	if !querycontract.ContainsString(packet.MissingEvidence, "environment_attribution") {
...
func TestBuildOwnershipPacketSingleServiceIsDerivedNotExact(t *testing.T) {
	...
	packet := buildReplatformingOwnershipPacket(finding, ManagementFilter{AccountID: "123456789012"})
...
func TestBuildOwnershipPacketTagsAreProvenanceNeverOwner(t *testing.T) {
	...
	if vals := candidateValues(packet, ownershipCandidateKindEnvironment); querycontract.ContainsString(vals, "prod") {
...
func TestBuildOwnershipPacketSummaryReportsCandidatesAndAccount(t *testing.T) {
	...
	summary := buildReplatformingOwnershipSummary([]ManagementFindingRow{finding}, ManagementFilter{AccountID: "123456789012"})
