...
func replatformingPlanItemForFinding(finding ManagementFindingRow, filter ManagementFilter) MigrationPacketItem {
...
func replatformingPromotionRejected(finding ManagementFindingRow) bool {
	if !finding.SafetyGate.ReviewRequired &&
		!terraformImportStringSliceContains(finding.SafetyGate.RefusedActions, managementSafetyActionTerraformImportPlan) {
		return false
	}
...
func replatformingMergeWarnings(finding ManagementFindingRow) []string {
	return mergeStringSets(finding.WarningFlags, finding.SafetyGate.Warnings)
}

func replatformingProvider(finding ManagementFindingRow) string {
...
func replatformingResourceType(finding ManagementFindingRow) string {
...
func replatformingStableID(finding ManagementFindingRow) string {
	if stable := strings.TrimSpace(firstNonEmpty(finding.ARN, finding.ResourceID)); stable != "" {
		return stable
	}
...
func replatformingSourceLayers(finding ManagementFindingRow, state ReplatformingSourceState) []ReplatformingSourceLayerStatus {
...
func replatformingDeclaredDetail(finding ManagementFindingRow) string {
	switch {
...
func replatformingOwnerCandidates(finding ManagementFindingRow, state ReplatformingSourceState) []ReplatformingOwnerCandidate {
...
func replatformingImportCandidate(finding ManagementFindingRow, filter ManagementFilter) *ReplatformingImportCandidate {
