...
type MigrationPacketItem struct {
	...
	Confidence       string                           `json:"confidence,omitempty"`
	Freshness        *querycontract.TruthFreshness    `json:"freshness,omitempty"`
...
func (s ReplatformingSourceState) TruthLevel() querycontract.TruthLevel {
	switch s {
	case ReplatformingSourceStateExact:
		return querycontract.TruthLevelExact
	case ReplatformingSourceStateDerived, ReplatformingSourceStatePartial:
		return querycontract.TruthLevelDerived
	default:
		return querycontract.TruthLevelFallback
	}
}
...
func (p *ReplatformingPlan) RollupTruthLevel() querycontract.TruthLevel {
	if p == nil || len(p.Items) == 0 {
		return querycontract.TruthLevelDerived
	}
	rollup := querycontract.TruthLevelExact
	for _, item := range p.Items {
		rollup = querycontract.MinTruthLevel(rollup, item.SourceState.TruthLevel())
	}
	return rollup
}
