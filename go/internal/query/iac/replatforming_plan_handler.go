...
func (h *Handler) handleReplatformingPlan(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), ReplatformingPlanReadinessCapability) {
		querycontract.WriteContractError(
			...
			ReplatformingPlanReadinessCapability,
			h.profile(),
			querycontract.RequiredProfile(ReplatformingPlanReadinessCapability),
		)
		return
	}
	...
	nextOffset := ManagementNextOffset(filter.Offset, len(findings), totalFindings)
...
func normalizeReplatformingScope(req replatformingPlanRequest) (ReplatformingPlanScope, error) {
	...
	return ReplatformingPlanScope{
		...
		Resource:    strings.TrimSpace(firstNonEmpty(req.ResourceID, req.ARN)),
	}, nil
}
...
func replatformingFilterRequest(req replatformingPlanRequest) managementRequest {
...
func replatformingPlanNextCalls(filter ManagementFilter, nextOffset *int) []map[string]any {
