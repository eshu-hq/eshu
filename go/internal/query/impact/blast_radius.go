...
func BlastRadiusRepositoryQuery(access querycontract.RepositoryAccessFilter) string {
...
func blastRadiusTerraformSourceReposQuery(access querycontract.RepositoryAccessFilter) string {
...
func blastRadiusDependentsByIDQuery(access querycontract.RepositoryAccessFilter) string {
...
func blastRadiusCrossplaneQuery(access querycontract.RepositoryAccessFilter) string {
...
// BlastRadiusSqlTableBranches is exported because staying query tests pin
// it; the home stays this package. See #6060.
...
func BlastRadiusSqlTableQuery(access querycontract.RepositoryAccessFilter) string {
	branchGrant := ""
	anchoredBranchGrant := ""
	if access.Scoped() {
...
func sqlTableBlastRadiusCoverage() (bool, []blastRadiusEdgeCoverage) {
	...
	for _, edgeType := range sqlTableBlastRadiusEdgeTypes {
		c := querycontract.EdgeMaterializationCoverage(edgeType)
		if !c.Materialized {
			complete = false
		}
...
func (h *Handler) findBlastRadius(w http.ResponseWriter, r *http.Request) {
	if querycontract.CapabilityUnsupported(h.profile(), "platform_impact.blast_radius") {
		querycontract.WriteContractError(
			...
			h.profile(),
			querycontract.RequiredProfile("platform_impact.blast_radius"),
		)
		return
	}
	...
	if req.TargetType == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "target_type is required")
		return
	}
	...
	if access.Empty() {
		querycontract.WriteSuccess(w, r, http.StatusOK, map[string]any{
			...
			"limit":          limit,
			"truncated":      false,
		}, querycontract.BuildTruthEnvelope(h.profile(), "platform_impact.blast_radius", querycontract.TruthBasisHybrid, "resolved from platform graph impact analysis"))
		return
	}

	affected, supported, complete, coverage, err := h.blastRadiusAffected(r.Context(), req.TargetType, req.Target, limit+1, access)
	if !supported {
		querycontract.WriteError(w, http.StatusBadRequest, "unsupported target_type: "+req.TargetType)
		return
	}
	if err != nil {
	...
	for _, row := range affected {
		entry := map[string]any{"repo": querycontract.StringVal(row, "repo")}
		if v := querycontract.StringVal(row, "tier"); v != "" {
			entry["tier"] = v
		}
		if v := querycontract.StringVal(row, "risk"); v != "" {
			entry["risk"] = v
		}
		if v := querycontract.IntVal(row, "hops"); v > 0 {
			entry["hops"] = v
		}
...
func (h *Handler) blastRadiusAffected(ctx context.Context, targetType, target string, limit int, access querycontract.RepositoryAccessFilter) ([]map[string]any, bool, bool, []blastRadiusEdgeCoverage, error) {
...
func (h *Handler) enrichBlastRadiusTiers(ctx context.Context, affected []map[string]any) {
