...
// ChangeSurfaceTargetCandidate is a resolved change-surface anchor: the
// repository, workload, or file the traversal fans out from. Exported
// because the root live-backend proof test constructs it; the home stays
...
func (h *Handler) investigateChangeSurface(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), changeSurfaceInvestigationCapability) {
		querycontract.WriteContractError(
			...
			changeSurfaceInvestigationCapability,
			h.profile(),
			querycontract.RequiredProfile(changeSurfaceInvestigationCapability),
		)
		return
	}
	...
	codeSurface, err := h.codeSurface().FetchCodeSurface(r.Context(), h, req, h.changeSurfacePathSymbols)
	if err != nil {
...
func (h *Handler) resolveChangeSurfaceTarget(
	ctx context.Context,
