...
func (h *Handler) findChangeSurface(w http.ResponseWriter, r *http.Request) {
	if querycontract.CapabilityUnsupported(h.profile(), "platform_impact.change_surface") {
		querycontract.WriteContractError(
			...
			h.profile(),
			querycontract.RequiredProfile("platform_impact.change_surface"),
		)
		return
	}
	...
	if selected != nil {
		...
		rows, rowsTruncated, traversalErr := h.FindChangeSurfaceImpactRows(r.Context(), *selected, req.Environment, depth, limit, querycontract.RepositoryAccessFilterFromContext(r.Context()))
		if traversalErr != nil {
	...
	querycontract.WriteSuccess(w, r, http.StatusOK, resp, querycontract.BuildTruthEnvelope(h.profile(), "platform_impact.change_surface", querycontract.TruthBasisHybrid, "resolved from graph and impact relationships"))
}
...
// Exported because the root live-backend proof test calls it; the home stays
// this package. See #6060.
func (h *Handler) FindChangeSurfaceImpactRows(
	ctx context.Context,
	...
	for _, row := range rows {
		...
		base := map[string]any{"id": querycontract.StringVal(row, "id"), "name": querycontract.StringVal(row, "name"), "labels": querycontract.StringSliceVal(row, "labels"), "depth": querycontract.IntVal(row, "depth")}
		if env != "" {
		...
		for _, edge := range changeSurfaceRelEdges(row["rels"]) {
			...
			key := fmt.Sprintf("%s\x00%s\x00%s\x00%s\x00%d", querycontract.StringVal(entry, "id"), edge.relType, confKey, edge.reason, querycontract.IntVal(entry, "depth"))
			if _, ok := seen[key]; ok {
				continue
			}
			seen[key] = struct{}{}
...
func changeSurfaceRelEdges(raw any) []changeSurfaceRelEdge {
	...
	for _, item := range rels {
		...
		case map[string]any:
			...
			edges = append(edges, changeSurfaceRelEdgeFromProps(querycontract.StringVal(rel, "type"), props))
		}
	}
	return edges
}
