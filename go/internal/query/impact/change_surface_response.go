...
// ChangeSurfaceImpactRows resolves the impacted rows for one anchored
// target. Exported because the root live-backend proof test calls it; the
// home stays this package. See #6060.
func (h *Handler) ChangeSurfaceImpactRows(
	ctx context.Context,
...
func changeSurfaceImpactedRowRepoID(row map[string]any) string {
	for _, label := range querycontract.StringSliceVal(row, "labels") {
		if label == "Repository" {
			return querycontract.StringVal(row, "id")
		}
	}
	return querycontract.StringVal(row, "repo_id")
}
...
func (h *Handler) changeSurfaceResponse(
	req ChangeSurfaceInvestigationRequest,
...
func splitImpactRows(rows []map[string]any) ([]map[string]any, []map[string]any) {
	...
	for _, row := range rows {
		...
		if querycontract.IntVal(row, "depth") <= 1 {
...
func changeSurfaceImpactEntry(row map[string]any) map[string]any {
	entry := map[string]any{
		...
		"evidence_handle": map[string]any{"entity_id": querycontract.StringVal(row, "id")},
	}
	if env := querycontract.StringVal(row, "environment"); env != "" {
		entry["environment"] = env
	}
	if repoID := querycontract.StringVal(row, "repo_id"); repoID != "" {
...
func symbolDedupeKey(symbol map[string]any) string {
	parts := []string{
		querycontract.StringVal(symbol, "entity_id"),
		querycontract.StringVal(symbol, "repo_id"),
		querycontract.StringVal(symbol, "relative_path"),
		querycontract.StringVal(symbol, "entity_name"),
	}
...
func changeSurfaceRecommendedNextCalls(
	...
	for _, symbol := range querycontract.MapSliceValue(codeSurface, "touched_symbols") {
		if entityID := querycontract.StringVal(symbol, "entity_id"); entityID != "" {
	...
	if status := querycontract.StringVal(resolution, "status"); status == "resolved" && (len(direct) > 0 || len(transitive) > 0) {
		selected, _ := resolution["selected"].(map[string]any)
		calls = append(calls, map[string]any{"tool": "find_change_surface", "args": map[string]any{"target": querycontract.StringVal(selected, "id"), "limit": req.Limit}})
	}
	return calls
}

func changeSurfaceQueryShape(resolution map[string]any) string {
	switch querycontract.StringVal(resolution, "status") {
	case "resolved":
