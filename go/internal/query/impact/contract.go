...
func (h *Handler) contractImpact(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), contractImpactCapability) {
		querycontract.WriteContractError(
			...
			contractImpactCapability,
			h.profile(),
			querycontract.RequiredProfile(contractImpactCapability),
		)
		return
	}
	...
	querycontract.WriteSuccess(w, r, http.StatusOK, resp, truth)
}
...
func normalizeContractImpactRequest(req contractImpactRequest) (contractImpactRequest, error) {
	...
	req.ProviderRepoID = strings.TrimSpace(querycontract.FirstNonEmptyString(req.ProviderRepoID, req.RepoID))
...
func (h *Handler) contractImpactResponse(
	ctx context.Context,
...
func contractImpactHTTPProviders(rows []map[string]any, req contractImpactRequest) []map[string]any {
	...
	for _, row := range rows {
		...
		for _, method := range methods {
			provider := map[string]any{
				...
				"contract_key":     contractImpactKey(contractImpactFamilyHTTP, querycontract.StringVal(row, "provider_repo_id"), querycontract.StringVal(row, "path"), method),
				"provider_repo_id": querycontract.StringVal(row, "provider_repo_id"),
				"provider_repo":    querycontract.StringVal(row, "provider_repo"),
				"route":            querycontract.StringVal(row, "path"),
				"http_method":      method,
				"endpoint_id":      querycontract.StringVal(row, "endpoint_id"),
				"source_kinds":     querycontract.UniqueSortedStrings(querycontract.StringSliceVal(row, "source_kinds")),
				"source_paths":     querycontract.UniqueSortedStrings(querycontract.StringSliceVal(row, "source_paths")),
				"operation_ids":    querycontract.UniqueSortedStrings(querycontract.StringSliceVal(row, "operation_ids")),
	...
	slices.SortFunc(providers, func(a, b map[string]any) int {
		return strings.Compare(querycontract.StringVal(a, "contract_key"), querycontract.StringVal(b, "contract_key"))
	})
	return providers
}
