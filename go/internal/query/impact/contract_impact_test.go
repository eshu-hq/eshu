...
func TestContractImpactRequiresSupportedProfile(t *testing.T) {
	...
	handler := &ImpactHandler{Neo4j: &recordingContractImpactGraph{}, Profile: querycontract.ProfileLocalLightweight}
	...
	if got, want := envelope.Error.Code, querycontract.ErrorCodeUnsupportedCapability; got != want {
...
func TestContractImpactHTTPReturnsServiceUnavailableWithoutGraph(t *testing.T) {
	...
	handler := &ImpactHandler{Profile: querycontract.ProfileLocalAuthoritative}
	mux := http.NewServeMux()
	handler.Mount(mux)
...
func decodeContractImpactEnvelope(t *testing.T, w *httptest.ResponseRecorder) querycontract.ResponseEnvelope {
	t.Helper()
