...
func (h *ImpactHandler) investigateDeploymentConfigInfluence(w http.ResponseWriter, r *http.Request) {
	if querycontract.CapabilityUnsupported(h.profile(), deploymentConfigInfluenceCapability) {
		querycontract.WriteContractError(
			...
			h.profile(),
			querycontract.RequiredProfile(deploymentConfigInfluenceCapability),
		)
		return
	}
	...
	if req.ServiceName == "" && req.WorkloadID == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "service_name or workload_id is required")
		return
	}
	...
	ctx, err := h.traceContext().FetchServiceTraceContext(r.Context(), h.Neo4j, h.Content, h.Logger, selector, TraceEnrichmentConfig{MaxDepth: 4})
	if err != nil {
	...
	querycontract.WriteSuccess(
		...
		querycontract.BuildTruthEnvelope(h.profile(), deploymentConfigInfluenceCapability, querycontract.TruthBasisHybrid, "resolved from service deployment evidence, topology, and runtime artifacts"),
	)
}
...
func (h *ImpactHandler) enrichDeploymentConfigInfluenceContext(ctx context.Context, workload map[string]any) error {
	...
	go func() {
		result, err := h.FetchK8sResourceResult(ctx, repoID, querycontract.SafeStr(workload, "name"))
...
func buildDeploymentConfigInfluenceResponse(req deploymentConfigInfluenceRequest, workload map[string]any) map[string]any {
	...
	serviceName := querycontract.CanonicalServiceName(querycontract.FirstNonEmptyString(req.ServiceName, req.WorkloadID), workload)
	environment := strings.TrimSpace(req.Environment)
	artifacts := deploymentConfigArtifactsForEnvironment(querycontract.MapSliceValue(querycontract.MapValue(workload, "deployment_evidence"), "artifacts"), environment)
	deploymentSources := querycontract.MapSliceValue(workload, "deployment_sources")
	k8sResources := querycontract.MapSliceValue(workload, "k8s_resources")
	controllerEntities := querycontract.MapSliceValue(workload, "controller_entities")
	imageRefs := querycontract.StringSliceVal(workload, "image_refs")
	...
	deploymentSourceLimits, k8sResourceLimits := querycontract.MapValue(workload, "deployment_source_limits"), querycontract.MapValue(workload, "k8s_resource_limits")
	...
	return map[string]any{
		...
		"workload_id":              querycontract.SafeStr(workload, "id"),
...
func capRows(rows []map[string]any, limit int, alreadyTruncated bool) ([]map[string]any, bool) {
	capped, truncated := querycontract.CapMapRows(rows, limit)
...
func deploymentConfigEnvironmentMatches(row map[string]any, environment string) bool {
	...
	rowEnvironment := querycontract.StringVal(row, "environment")
	if rowEnvironment == "" {
		return true
	}
...
func deploymentConfigPortableRow(row map[string]any) map[string]any {
	relativePath := querycontract.FirstNonEmptyString(querycontract.StringVal(row, "relative_path"), querycontract.StringVal(row, "path"), querycontract.StringVal(row, "file_path"), querycontract.StringVal(row, "config_path"))
	relativePath = strings.TrimPrefix(relativePath, "/")
	repoID := querycontract.FirstNonEmptyString(querycontract.StringVal(row, "source_repo_id"), querycontract.StringVal(row, "repo_id"))
	repoName := querycontract.FirstNonEmptyString(querycontract.StringVal(row, "source_repo_name"), querycontract.StringVal(row, "repo_name"))
	...
	if line := querycontract.IntVal(row, "start_line"); line > 0 {
		result["start_line"] = line
	}
	if line := querycontract.IntVal(row, "end_line"); line > 0 {
...
func deploymentConfigSearchText(row map[string]any) string {
	return strings.ToLower(strings.Join([]string{
		...
		querycontract.StringVal(row, "matched_alias"),
		querycontract.StringVal(row, "matched_value"),
		querycontract.StringVal(row, "relative_path"),
	}, " "))
}
...
func deploymentConfigInfluencingRepositories(workload map[string]any, deploymentSources []map[string]any, artifacts []map[string]any) []map[string]any {
	...
	addRepo := func(repoID string, repoName string, role string) {
		key := querycontract.FirstNonEmptyString(repoID, repoName)
		if key == "" {
			return
		}
		row := seen[key]
		...
		querycontract.AddUniqueStringField(row, "roles", role)
	}
	addRepo(querycontract.SafeStr(workload, "repo_id"), querycontract.SafeStr(workload, "repo_name"), "service_owner")
	for _, source := range deploymentSources {
		addRepo(querycontract.StringVal(source, "repo_id"), querycontract.StringVal(source, "repo_name"), "deployment_source")
	}
	for _, artifact := range artifacts {
		addRepo(querycontract.StringVal(artifact, "repo_id"), querycontract.StringVal(artifact, "repo_name"), "configuration_artifact")
	}
	rows := make([]map[string]any, 0, len(seen))
	for _, row := range seen {
		querycontract.SortStringFields(row, "roles")
		rows = append(rows, row)
	}
	sortDeploymentConfigRows(rows)
	return rows
...
func deploymentConfigReadFirstFiles(artifacts []map[string]any) []map[string]any {
	seen := map[string]map[string]any{}
	for _, artifact := range artifacts {
		repoID := querycontract.StringVal(artifact, "repo_id")
		relativePath := querycontract.StringVal(artifact, "relative_path")
		...
		if row == nil {
			row = map[string]any{
				"repo_id":       repoID,
				"repo_name":     querycontract.StringVal(artifact, "repo_name"),
		...
		if line := querycontract.IntVal(artifact, "start_line"); line > 0 {
			row["start_line"] = line
		}
		if line := querycontract.IntVal(artifact, "end_line"); line > 0 {
			row["end_line"] = line
		}
		querycontract.AddUniqueStringField(row, "evidence_kinds", querycontract.StringVal(artifact, "evidence_kind"))
	}
	rows := make([]map[string]any, 0, len(seen))
	for _, row := range seen {
		querycontract.SortStringFields(row, "evidence_kinds")
		rows = append(rows, row)
	}
	sortDeploymentConfigRows(rows)
	return rows
...
func deploymentConfigSubject(workload map[string]any, serviceName string, environment string) map[string]any {
	return map[string]any{"type": "service", "id": querycontract.SafeStr(workload, "id"), "name": serviceName, "environment": environment}
}
...
func deploymentConfigNextCalls(readFirstFiles []map[string]any, serviceName string, environment string) []string {
	...
	if len(readFirstFiles) > 0 {
		...
		calls = append(calls, "get_file_lines(repo_id: "+querycontract.StringVal(first, "repo_id")+", relative_path: "+querycontract.StringVal(first, "relative_path")+")")
	}
	return calls
}
...
func sortDeploymentConfigRows(rows []map[string]any) {
	sort.SliceStable(rows, func(i, j int) bool {
		left := querycontract.FirstNonEmptyString(querycontract.StringVal(rows[i], "repo_id"), querycontract.StringVal(rows[i], "repo_name")) + "\x00" +
			querycontract.FirstNonEmptyString(querycontract.StringVal(rows[i], "relative_path"), querycontract.StringVal(rows[i], "name"), querycontract.StringVal(rows[i], "entity_name"), querycontract.StringVal(rows[i], "image_ref"))
		right := querycontract.FirstNonEmptyString(querycontract.StringVal(rows[j], "repo_id"), querycontract.StringVal(rows[j], "repo_name")) + "\x00" +
			querycontract.FirstNonEmptyString(querycontract.StringVal(rows[j], "relative_path"), querycontract.StringVal(rows[j], "name"), querycontract.StringVal(rows[j], "entity_name"), querycontract.StringVal(rows[j], "image_ref"))
		return left < right
	})
}
