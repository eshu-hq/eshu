...
func deploymentConfigGitOpsEntities(count int) []querycontract.EntityContent {
	if count == 0 {
		return nil
	}
	...
	for index := 1; index < count; index++ {
		...
		if index == querycontract.ServiceStoryItemLimit+1 {
			image = deploymentConfigBoundedOmittedGitOpsImage
		}
		if count > querycontract.RepositorySemanticEntityLimit && index == count-1 {
...
func (s deploymentConfigInfluenceContentStore) ListRepoEntities(
	...
	return append([]querycontract.EntityContent(nil), rows...), nil
}
...
func TestBuildDeploymentConfigInfluenceResponseReturnsPromptReadyFiles(t *testing.T) {
	...
	if got := querycontract.StringVal(resp, "service_name"); got != "eshu-hqgraph-resolution-engine" {
	...
	readFirst := querycontract.MapSliceValue(resp, "read_first_files")
	if got := querycontract.StringVal(readFirst[0], "repo_id"); got == "" {
		t.Fatalf("read_first_files[0].repo_id is empty")
	}
	if got := querycontract.StringVal(readFirst[0], "relative_path"); got == "" || got[0] == '/' {
		t.Fatalf("read_first_files[0].relative_path = %q, want portable relative path", got)
	}
	if got := querycontract.StringVal(readFirst[0], "next_call"); got != "get_file_lines" {
		t.Fatalf("read_first_files[0].next_call = %q, want get_file_lines", got)
	}

	coverage := querycontract.MapValue(resp, "coverage")
	if got := querycontract.StringVal(coverage, "query_shape"); got != "deployment_config_influence_story" {
		t.Fatalf("coverage.query_shape = %q, want deployment_config_influence_story", got)
	}
	if querycontract.BoolVal(coverage, "truncated") {
		t.Fatalf("coverage.truncated = true, want false")
	}
	if got := querycontract.StringSliceVal(resp, "recommended_next_calls"); len(got) == 0 {
...
func TestBuildDeploymentConfigInfluenceResponseUsesServiceStoryDeploymentEvidence(t *testing.T) {
	...
	valuesLayers := querycontract.MapSliceValue(resp, "values_layers")
	if len(valuesLayers) == 0 {
		t.Fatalf("values_layers is empty, want service story deployment artifacts as config influence")
	}
	coverage := querycontract.MapValue(resp, "coverage")
	if got, want := querycontract.IntVal(coverage, "artifact_candidate_count"), 2; got != want {
...
func TestInvestigateDeploymentConfigInfluenceDisclosesSaturatedUpstreamEvidence(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "saturated_gitops_probe", gitOpsCount: querycontract.RepositorySemanticEntityLimit + 1, wantTruncated: true, wantLowerBound: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			coverage := querycontract.MapValue(response, "coverage")
			if got := querycontract.IntVal(coverage, "rendered_target_count"); got >= 100 {
				t.Fatalf("coverage.rendered_target_count = %d, want fewer than requested limit despite upstream saturation", got)
			}
			if got := querycontract.BoolVal(coverage, "truncated"); got != tt.wantTruncated {
				t.Fatalf("coverage.truncated = %t, want %t; coverage = %#v", got, tt.wantTruncated, coverage)
			}
			if got := querycontract.BoolVal(coverage, "observed_count_is_lower_bound"); got != tt.wantLowerBound {
			...
			if tt.wantTruncated && !querycontract.ContainsString(limitations, "k8s_resource_evidence_truncated") {
			...
			deploymentSourceLimits := querycontract.MapValue(response, "deployment_source_limits")
			k8sResourceLimits := querycontract.MapValue(response, "k8s_resource_limits")
			if got := querycontract.IntVal(deploymentSourceLimits, "query_sentinel_limit"); got != querycontract.ContextStoryItemLimit+1 {
				t.Fatalf("deployment_source_limits = %#v, want preserved sentinel metadata", deploymentSourceLimits)
			}
			if got := querycontract.IntVal(k8sResourceLimits, "deployment_source_query_sentinel_limit"); got != querycontract.RepositorySemanticEntityLimit+1 {
				t.Fatalf("k8s_resource_limits = %#v, want preserved deployment-source sentinel metadata", k8sResourceLimits)
			}
			if got := querycontract.BoolVal(k8sResourceLimits, "truncated"); got != tt.wantTruncated {
				t.Fatalf("k8s_resource_limits.truncated = %t, want %t; limits = %#v", got, tt.wantTruncated, k8sResourceLimits)
			}
			if got := querycontract.BoolVal(k8sResourceLimits, "deployment_source_observed_count_is_lower_bound"); got != tt.wantLowerBound {
