...
// every edge every blastRadiusGateQueries entry traverses is either
...
// blastRadiusGateQuery names one production Cypher constant in
// blast_radius.go the #5335 edge-materialization gate audits.
type blastRadiusGateQuery struct {
	// ConstName is the Go identifier of the `const ... = \`...\`` literal in
	// blast_radius.go, verified present as a literal string constant
...
// blastRadiusGateQueries is the closed list of production Cypher
// constants in blast_radius.go this gate audits: the six
...
// blastRadiusGateQueries audits the edge types traversed by the
...
var blastRadiusGateQueries = func() []blastRadiusGateQuery {
	unscoped := querycontract.RepositoryAccessFilter{AllScopes: true}
	return []blastRadiusGateQuery{
...
func resolveImpactEdgeType(query blastRadiusGateQuery, edgeType string) (ok bool, reason string) {
	for _, owned := range query.CoverageEdgeTypes {
		if owned == edgeType {
			return true, ""
		}
	}
	if querycontract.EdgeMaterializationCoverage(edgeType).Materialized {
		return true, ""
	}
