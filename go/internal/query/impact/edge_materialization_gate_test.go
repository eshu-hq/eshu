...
// gateSourceFile resolves blast_radius.go's path from this test
// file's own directory (both live in go/internal/query).
func gateSourceFile(t *testing.T) string {
	t.Helper()
	_, file, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("runtime.Caller(0) failed")
	}
	return filepath.Join(filepath.Dir(file), "blast_radius.go")
}
...
// blast_radius.go and asserts every blastRadiusGateQueries
...
func TestImpactBlastRadiusGateQueriesAreLiteralConstants(t *testing.T) {
	...
	file, err := parser.ParseFile(fileSet, gateSourceFile(t), nil, 0)
	if err != nil {
		t.Fatalf("parse blast_radius.go: %v", err)
	}
	...
	for _, q := range blastRadiusGateQueries {
		if !literalConsts[q.ConstName] {
			t.Errorf(
				"%s is not declared as a single string-literal const in blast_radius.go -- "+
...
//
// For each production Cypher constant in blastRadiusGateQueries, it
...
//   - Only the Cypher constants in blast_radius.go that feed
...
//     (handler.go's dependency-path explainer, exposure_path.go, etc).
...
func TestImpactBlastRadiusCoverageEdgeTypesAreStillTraversed(t *testing.T) {
	...
	for _, entry := range stale {
		t.Errorf(
			"coverage-edge-type list entry %q is not traversed by any blast_radius.go query "+
