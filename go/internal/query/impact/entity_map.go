...
// EntityMapCandidate is the resolved entity-map anchor.
...
type entityMapResolverQuery struct {
	// Cypher and Params are exported because the root queryplan tests pin the
	// bound resolver query this family builds; the home stays this package.
	// See #6060.
	Cypher string
...
func (h *ImpactHandler) entityMap(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), entityMapCapability) {
		querycontract.WriteContractError(
			...
			entityMapCapability,
			h.profile(),
			querycontract.RequiredProfile(entityMapCapability),
		)
		return
	}
