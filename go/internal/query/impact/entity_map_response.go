...
func entityMapScope(req entityMapRequest) map[string]any {
	scope := querycontract.CompactStringMap(map[string]any{
...
func entityMapIsDefinedBy(row map[string]any) bool {
	relType := querycontract.StringVal(row, "relationship_type")
	return querycontract.StringVal(row, "direction") == "incoming" &&
...
func entityMapIsDependsOn(row map[string]any) bool {
	...
	switch querycontract.StringVal(row, "relationship_type") {
