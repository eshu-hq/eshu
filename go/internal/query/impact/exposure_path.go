...
func (h *Handler) traceExposurePath(w http.ResponseWriter, r *http.Request) {
	if querycontract.CapabilityUnsupported(h.profile(), exposurePathCapability) {
		querycontract.WriteContractError(
			...
			h.profile(),
			querycontract.RequiredProfile(exposurePathCapability),
		)
		return
	}
	...
	if strings.TrimSpace(req.Source) == "" && strings.TrimSpace(req.SourceEntityID) == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "source or source_entity_id is required")
		return
	}
...
func (h *Handler) resolveExposureSource(ctx context.Context, req exposurePathRequest) (exposure.PathNode, exposure.SourceSpec, bool, string, error) {
	...
	spec, ok := exposure.ClassifySource(code.DeadCodeRootKindsFromMetadata(entity.Metadata))
	if !ok {
