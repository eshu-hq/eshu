...
type Handler struct {
	...
	// skipped) so tests can construct Handler without wiring the full
	// telemetry stack.
	...
	// tests inject fakes. B2-FOLLOWUP: collapse back into Handler
...
// so the cmd wirings (which construct Handler with only the store
// fields) keep working unchanged; tests in this package inject fakes
// per-handler and never touch these globals.
var (
	// DefaultCodeSurface backs Handler.CodeSurface when unset.
	DefaultCodeSurface ChangeSurfaceCodeBackend
	// DefaultTraceContext backs Handler.TraceContext when unset.
	DefaultTraceContext DeploymentTraceContextProvider
	// DefaultPathProbe backs Handler.PathProbe when unset.
	DefaultPathProbe PathProbeBackend
)

func (h *Handler) codeSurface() ChangeSurfaceCodeBackend {
...
func (h *Handler) traceContext() DeploymentTraceContextProvider {
...
func (h *Handler) pathProbe() PathProbeBackend {
...
// PathProbeBackend decodes raw graph path projections for the by-id
...
type PathProbeBackend interface {
...
func (h *Handler) traceResourceToCode(w http.ResponseWriter, r *http.Request) {
...
func (h *Handler) explainDependencyPath(w http.ResponseWriter, r *http.Request) {
