...
func (g grantBeforeLimitFake) Run(_ context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
	...
	if strings.Contains(cypher, "IN $allowed_repository_ids") {
		...
		for _, row := range g.corpus {
			if _, ok := allowed[querycontract.StringVal(row, "repo_id")]; ok {
	...
	if limit := querycontract.IntVal(params, "limit"); limit > 0 && len(rows) > limit {
		rows = rows[:limit]
	}
	return rows, nil
}
...
func TestBlastRadiusGrantBoundBeforeLimit(t *testing.T) {
	...
	handler := &ImpactHandler{Neo4j: fake, Profile: querycontract.ProfileLocalAuthoritative}
	...
	noGrantQuery := BlastRadiusRepositoryQuery(querycontract.RepositoryAccessFilter{AllScopes: true})
	...
	for _, row := range affected {
		if !scoped.AllowsRepositoryID(querycontract.StringVal(row, "repo_id")) {
...
func repoNames(rows []map[string]any) map[string]bool {
	...
	for _, row := range rows {
		out[querycontract.StringVal(row, "repo")] = true
	}
	return out
}
