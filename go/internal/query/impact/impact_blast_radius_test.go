...
func TestMergeBlastRadiusRowsMinHopsDedup(t *testing.T) {
	...
	if querycontract.StringVal(got[0], "repo") != "b" || querycontract.IntVal(got[0], "hops") != 1 {
		t.Fatalf("row0 = %#v, want b@1", got[0])
	}
	if querycontract.StringVal(got[0], "repo_id") != "repo-b" {
		t.Fatalf("b lost its repo_id from the first-seen row: %#v", got[0])
	}
	if querycontract.StringVal(got[1], "repo") != "a" || querycontract.IntVal(got[1], "hops") != 2 {
		t.Fatalf("row1 = %#v, want a@2", got[1])
	}
	if querycontract.StringVal(got[1], "claim") != "c-a" {
