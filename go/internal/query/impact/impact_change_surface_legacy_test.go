...
func TestFindChangeSurfaceImpactRowsDedupsConfidenceStably(t *testing.T) {
	...
	rows, _, err := handler.FindChangeSurfaceImpactRows(t.Context(), ChangeSurfaceTargetCandidate{ID: "wl:s", Labels: []string{"Workload"}}, "", 4, 50, querycontract.RepositoryAccessFilter{AllScopes: true})
	if err != nil {
