...
// sloLiveGraphQuery is a minimal live querycontract.GraphQuery for this SLO
// proof. Production uses query.NewNeo4jReader, which this package cannot
// import without an import cycle (root already imports impact for #6060),
// and hoisting the full reader (policy, retries, tracing, telemetry) is
// out of scope for this move. The adapter opens a fresh read session per
// call, matching the GraphQuery contract, and converts rows exactly like
// production (record.Keys zipped with record.Values). It deliberately omits
// production's retry/backoff, deadlines, tracing, and telemetry: any read
// failure fails this proof loudly, and microsecond span overhead is noise
// against a 2s SLO. See #6060.
type sloLiveGraphQuery struct {
	driver   neo4jdriver.DriverWithContext
	database string
}

func (q sloLiveGraphQuery) Run(ctx context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
	session := q.driver.NewSession(ctx, neo4jdriver.SessionConfig{
...
func (q sloLiveGraphQuery) RunSingle(ctx context.Context, cypher string, params map[string]any) (map[string]any, error) {
...
func TestResourceInvestigationSelectorInteractiveSLO(t *testing.T) {
	...
	handler := &ImpactHandler{Neo4j: sloLiveGraphQuery{driver: driver, database: database}}
...
func assertResourceSelectorSLOCandidateIDs(
	...
	for _, row := range rows {
		got = append(got, querycontract.StringVal(row, "id"))
	}
	sort.Strings(got)
