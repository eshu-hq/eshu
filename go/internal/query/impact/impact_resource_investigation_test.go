...
// stubImpactPathProbe is the ImpactPathProbeBackend double for tests whose
// fixture rows carry no relationships(path) projection: production
// resourceInvestigationHopList maps a missing projection to empty, so the
// stub reports empty hops and zero values elsewhere. Per the PathProbe field
// contract, tests in this package inject fakes per-handler instead of
// touching the DefaultPathProbe global (which package query's init wires).
// See #6060.
...
func TestInvestigateResourceResolvesExactCloudARN(t *testing.T) {
	...
	missingEvidence := querycontract.StringSliceVal(data, "missing_evidence")
	for _, want := range []string{"resource_usage_relationship_missing", "repository_provenance_path_missing"} {
		if !slices.Contains(missingEvidence, want) {
...
func TestResourceInvestigationResolverNarrowsQueueAndDatabaseTypes(t *testing.T) {
	...
	queueCypher := ResourceInvestigationSelectorLabelCypher(ResourceInvestigationRequest{
		...
		ResourceType: "queue",
		Limit:        25,
	}, querycontract.RepositoryAccessFilter{AllScopes: true}, "CloudResource", ResourceInvestigationExactSelectorPredicates)
...
func TestResourceInvestigationWorkloadsGrantFiltersBeforeTruncation(t *testing.T) {
	...
	for _, workload := range workloads {
		if id := querycontract.StringVal(workload, "workload_id"); id == "wl-b" {
...
func TestResourceInvestigationRepoPathsGrantFiltersBeforeTruncation(t *testing.T) {
	...
	handler := &ImpactHandler{Neo4j: graph, Profile: querycontract.ProfileLocalAuthoritative, PathProbe: stubImpactPathProbe{}}
	access := querycontract.RepositoryAccessFilterFromContext(queryauth.ContextWithAuthContext(context.Background(), querytestutil.ScopedTestAuthContext("tenant-a", []string{"repo-a"})))
	...
	for _, path := range paths {
		if repoID := querycontract.StringVal(path, "repo_id"); repoID == "repo-b" {
