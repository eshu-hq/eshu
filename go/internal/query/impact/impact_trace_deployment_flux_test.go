...
func (s fluxCrossRepoContentStore) ListRepoEntities(_ context.Context, repoID string, limit int) ([]querycontract.EntityContent, error) {
	rows := append([]querycontract.EntityContent(nil), s.entitiesByRepo[repoID]...)
		...
		Entities: []querycontract.EntityContent{
			{
				...
				EntityType: "FluxKustomization", EntityName: "payments-app",
				StartLine: 1, EndLine: 20, Language: "yaml", SourceCache: "kind: Kustomization",
				Metadata: map[string]any{
					"source_path": "apps/payments/overlays/prod", "source_ref_kind": "GitRepository", "source_ref_name": "app-source",
				},
			},
			{
				EntityID: "flux-helmrelease-1", RepoID: "repo-deploy", RelativePath: "clusters/prod/podinfo-helmrelease.yaml",
				EntityType: "FluxHelmRelease", EntityName: "podinfo",
				StartLine: 1, EndLine: 15, Language: "yaml", SourceCache: "kind: HelmRelease",
				Metadata: map[string]any{
					"chart": "charts/podinfo", "source_ref_kind": "GitRepository", "source_ref_name": "app-source",
				},
			},
			{
				EntityID: "k8s-1", RepoID: "repo-deploy", RelativePath: "apps/payments/overlays/prod/deployment.yaml",
				EntityType: "K8sResource", EntityName: "payments-api",
				StartLine: 1, EndLine: 10, Language: "yaml", SourceCache: "kind: Deployment",
...
func TestFetchDeploymentSourceGitOpsResultAttributesCrossRepoFluxTargetResources(t *testing.T) {
	...
	handler := &ImpactHandler{Content: fluxCrossRepoContentStore{entitiesByRepo: map[string][]querycontract.EntityContent{
		"repo-deploy": {{
	...
	if got, want := querycontract.StringVal(result.k8sResources[0], "repo_id"), "repo-app"; got != want {
...
func TestFetchDeploymentSourceGitOpsResultAttributesCrossRepoFluxTargetResourcesAtRepositoryRoot(t *testing.T) {
	...
	for _, sourcePath := range []string{".", "./"} {
		...
		t.Run(sourcePath, func(t *testing.T) {
			...
			if got, want := querycontract.StringVal(result.k8sResources[0], "entity_id"), "k8s:root-deployment"; got != want {
