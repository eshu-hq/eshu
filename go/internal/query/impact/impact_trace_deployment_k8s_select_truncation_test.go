...
// SELECTS candidate scan hits the querycontract.RepositorySemanticEntityLimit ceiling, the
...
func TestImpactTraceK8sSelectWideningTruncationDisclosure(t *testing.T) {
	...
	// sentinel (querycontract.RepositorySemanticEntityLimit + 1 rows returned by the scan).
	...
	for i := range querycontract.RepositorySemanticEntityLimit + 1 {
	...
	if got := querycontract.BoolVal(result.limits, "k8s_relationships_complete"); got {
		t.Fatalf("k8s_relationships_complete = true, want false at the sentinel")
	}
	if got, want := querycontract.StringVal(result.limits, "k8s_relationships_incomplete_reason"), k8sSelectCandidatePoolTruncationReason; got != want {
