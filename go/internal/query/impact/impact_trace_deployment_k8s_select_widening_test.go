...
func (s k8sSelectWideningStore) SearchEntitiesByName(_ context.Context, repoID, entityType, name string, limit int) ([]querycontract.EntityContent, error) {
	out := make([]querycontract.EntityContent, 0)
...
// k8sEntity builds a K8sResource querycontract.EntityContent in repo-1. metaStrings entries
...
func k8sEntity(id, name, path, kind, namespace string, metaStrings map[string]string) querycontract.EntityContent {
...
func surfacedEntityIDs(rows []map[string]any) map[string]struct{} {
	...
	for _, row := range rows {
		ids[querycontract.StringVal(row, "entity_id")] = struct{}{}
	}
	return ids
}
...
func hasSelectsEdge(relationships []map[string]any, sourceID, targetID, reason string) bool {
	for _, rel := range relationships {
		...
		if querycontract.StringVal(rel, "source_id") == sourceID &&
			querycontract.StringVal(rel, "target_id") == targetID &&
			querycontract.StringVal(rel, "reason") == reason {
			return true
		}
	}
	return false
...
func TestImpactTraceK8sSelectWideningUnderLinkingRegression(t *testing.T) {
	...
	handler := &ImpactHandler{Content: newK8sSelectWideningStore([]querycontract.EntityContent{deployment, service})}
...
func TestImpactTraceK8sSelectWideningFrozenSubSurfaceOnNoMatch(t *testing.T) {
	...
	if got, want := querycontract.IntVal(result.limits, "returned_count"), 1; got != want {
	...
	if got, want := querycontract.IntVal(result.limits, "k8s_select_candidate_sentinel_limit"), querycontract.RepositorySemanticEntityLimit+1; got != want {
