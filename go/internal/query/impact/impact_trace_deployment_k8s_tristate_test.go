...
// drives the real ImpactHandler.fetchK8sResources (not impacttrace.BuildK8sRelationships
...
func TestFetchK8sResourcesPreservesSelectorPodTemplateLabelsNamespaceTriState(t *testing.T) {
	...
	withLabels := querycontract.EntityContent{
	...
	vintage := querycontract.EntityContent{
	...
	h := &ImpactHandler{
		Content: fetchK8sResourcesContentStore{byName: []querycontract.EntityContent{withLabels, vintage}},
	}
	...
	for _, resource := range resources {
		byID[querycontract.StringVal(resource, "entity_id")] = resource
	}
