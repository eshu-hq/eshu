...
func TestFetchWorkloadLiveInstanceSummaryNilHandler(t *testing.T) {
	...
	var h *ImpactHandler
	summary, err := h.fetchWorkloadLiveInstanceSummary(t.Context(), nil, nil, nil, querycontract.RepositoryAccessFilter{})
	if err != nil {
...
func TestFetchWorkloadLiveInstanceSummaryNilStore(t *testing.T) {
	...
	summary, err := h.fetchWorkloadLiveInstanceSummary(
		t.Context(), controllers, resources, []string{"img:latest"}, querycontract.RepositoryAccessFilter{AllScopes: true},
	)
	if err != nil {
		t.Fatalf("error = %v, want nil", err)
	}
...
func TestFetchWorkloadLiveInstanceSummaryEmptyImageRefs(t *testing.T) {
	...
	h := &ImpactHandler{KubernetesPodTemplates: store}
	summary, err := h.fetchWorkloadLiveInstanceSummary(
		t.Context(), controllers, resources, nil, querycontract.RepositoryAccessFilter{AllScopes: true},
	)
	if err != nil {
		t.Fatalf("error = %v, want nil", err)
	}
...
func TestFetchWorkloadLiveInstanceSummaryEmptyAccess(t *testing.T) {
	...
	h := &ImpactHandler{KubernetesPodTemplates: store}
	summary, err := h.fetchWorkloadLiveInstanceSummary(
		t.Context(), controllers, resources, []string{"img@sha256:a"}, querycontract.RepositoryAccessFilter{}, // scoped, no grants
	)
	if err != nil {
		t.Fatalf("error = %v, want nil", err)
	}
...
func TestFetchWorkloadLiveInstanceSummaryReadyZeroIsPresent(t *testing.T) {
	...
	if summary.truncated {
		t.Fatal("truncated = true, want false (single match, far under querycontract.ServiceStoryItemLimit)")
	}
}
