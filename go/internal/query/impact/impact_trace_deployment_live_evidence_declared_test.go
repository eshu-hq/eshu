...
func TestFetchWorkloadLiveEvidenceDeclaredObjectAnchorSharedDigestDistinctWorkloadsNoPromotion(t *testing.T) {
	...
	liveA, err := h.fetchWorkloadLiveEvidence(t.Context(), nil, resourcesA, []string{sharedDigest}, querycontract.RepositoryAccessFilter{AllScopes: true})
	if err != nil {
	...
	liveB, err := h.fetchWorkloadLiveEvidence(t.Context(), nil, resourcesB, []string{sharedDigest}, querycontract.RepositoryAccessFilter{AllScopes: true})
	if err != nil {
