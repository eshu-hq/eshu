...
func TestBuildDeploymentTraceResponseIncludesOCIRegistryTruthInOverview(t *testing.T) {
	...
	if got, want := querycontract.IntVal(overview, "image_registry_match_count"), 1; got != want {
		t.Fatalf("deployment_overview.image_registry_match_count = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(overview, "canonical_image_match_count"), 1; got != want {
