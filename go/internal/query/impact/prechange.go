...
func (h *Handler) preChangeImpact(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), preChangeImpactCapability) {
		querycontract.WriteContractError(
			...
			preChangeImpactCapability,
			h.profile(),
			querycontract.RequiredProfile(preChangeImpactCapability),
		)
		return
	}
	...
	querycontract.WriteSuccess(w, r, http.StatusOK, AttachPreChangeImpactPacket(resp, preChangeSummary(resp), truth), truth)
}
...
func normalizePreChangeImpactRequest(req preChangeImpactRequest) (preChangeImpactRequest, error) {
	...
	req.Topic = strings.TrimSpace(querycontract.FirstNonEmptyString(req.Topic, req.Query))
	req.Query = ""
...
// set. See #6060.
func (h *Handler) PreChangeImpactResponse(
	r *http.Request,
	...
	codeSurface, err := h.codeSurface().FetchCodeSurface(r.Context(), h, changeReq, h.changeSurfacePathSymbols)
	if err != nil {
...
func preChangeImpactData(req preChangeImpactRequest, surface map[string]any) map[string]any {
	data := querycontract.CopyMap(surface)
	...
	data["missing_evidence"] = preChangeMissingEvidence(req, querycontract.MapValue(surface, "code_surface"))
...
func preChangeMissingEvidence(req preChangeImpactRequest, codeSurface map[string]any) []map[string]any {
	...
	for _, symbol := range querycontract.MapSliceValue(codeSurface, "touched_symbols") {
		if path := querycontract.StringVal(symbol, "relative_path"); path != "" {
			matched[path] = struct{}{}
		}
	}
...
func preChangeCoverage(req preChangeImpactRequest, surface map[string]any) map[string]any {
	coverage := querycontract.CopyMap(querycontract.MapValue(surface, "coverage"))
...
func preChangeNextCalls(data map[string]any) []map[string]any {
	calls := querycontract.MapSliceValue(data, "recommended_next_calls")
	if len(calls) == 0 && querycontract.IntVal(data, "changed_file_count") > 0 {
		calls = append(calls, map[string]any{
			"tool":   "investigate_change_surface",
			"reason": "inspect the normalized changed-file surface",
		})
	}
	if len(querycontract.MapSliceValue(data, "missing_evidence")) > 0 {
		calls = append(calls, map[string]any{
...
// already-built impact payload. It is a variable (not a method) because
// packet composition names the root AnswerPacket cluster, which cannot cross
// into this package: package query assigns the production implementation at
// init, so the cmd wirings keep working unchanged, and tests in this package
// inject fakes or exercise data shaping only. It defaults to identity so a
// handler built without wiring still returns the shaped data. It mirrors
// AttachDeveloperChangePlanPacket. See #6060.
var AttachPreChangeImpactPacket = func(data map[string]any, summary string, truth *querycontract.TruthEnvelope) map[string]any {
	return data
}

func preChangeSummary(data map[string]any) string {
	summary := querycontract.MapValue(data, "impact_summary")
	return fmt.Sprintf(
		...
		querycontract.IntVal(querycontract.MapValue(data, "code_surface"), "symbol_count"),
		querycontract.IntVal(summary, "direct_count"),
		querycontract.IntVal(summary, "transitive_count"),
	)
}
