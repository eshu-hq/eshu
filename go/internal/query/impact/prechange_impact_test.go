...
func TestPreChangeImpactAllowsEmptyDiff(t *testing.T) {
	...
	handler := &ImpactHandler{Content: querytestutil.FakePortContentStore{}, Profile: querycontract.ProfileLocalAuthoritative}
	mux := http.NewServeMux()
	handler.Mount(mux)
...
func decodePreChangeEnvelope(t *testing.T, rec *httptest.ResponseRecorder) querycontract.ResponseEnvelope {
	t.Helper()
