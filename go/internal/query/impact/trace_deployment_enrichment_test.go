...
func TestBuildDeploymentTraceResponseIncludesServiceEvidenceConsumersAndProvisioningChains(t *testing.T) {
	...
	// Counts-only contract (#6060 lane B2): the overview carries evidence
	// counts, never backfilled raw lists — the service-story builder owns
	// the normalized list forms, and the remaining lists must stay absent
	// unless the caller provided them.
	for key, want := range map[string]int{
		"hostname_count": 2, "entrypoint_count": 1, "network_path_count": 1,
		"dependent_count":                 1,
		"consumer_repository_count":       1,
		"provisioning_source_chain_count": 1,
	} {
		gotCount, ok := deploymentOverview[key].(int)
		if !ok || gotCount != want {
			t.Fatalf("deployment_overview[%q] = %v, want count %d", key, deploymentOverview[key], want)
		}
	}
	for _, key := range []string{"hostnames", "entrypoints", "network_paths", "api_surface", "dependents", "consumer_repositories", "provisioning_source_chains"} {
		if _, ok := deploymentOverview[key]; ok {
			t.Fatalf("deployment_overview[%q] present, want counts only (no backfilled lists)", key)
		}
	}
...
func TestBuildDeploymentTraceResponseRecognizesGitOpsFromReadModelEvidence(t *testing.T) {
	...
	if !slices.Contains(querycontract.StringSliceVal(gitopsOverview, "tool_families"), "argocd") {
	...
	if !slices.Contains(querycontract.StringSliceVal(controllerOverview, "controller_kinds"), "argocd") {
	...
	for _, path := range controllerDrivenPaths {
		pathsByTarget[querycontract.StringVal(path, "observed_target")] = path
	}
	if gotKind, wantKind := querycontract.StringVal(pathsByTarget["prod"], "controller_kind"), "kubernetes"; gotKind != wantKind {
		t.Fatalf("controller_driven_paths[prod].controller_kind = %q, want %q", gotKind, wantKind)
	}
	if gotKind, wantKind := querycontract.StringVal(pathsByTarget["runtime-ecs"], "controller_kind"), "ecs"; gotKind != wantKind {
