...
func TestTraceDeploymentChainOwnRepoAppOfAposMonorepoDoesNotLeakOtherServiceEvidence(t *testing.T) {
	...
	k8sResources := querycontract.MapSliceValue(body, "k8s_resources")
	for _, resource := range k8sResources {
		if querycontract.StringVal(resource, "entity_name") == "payments-api" {
...
func TestTraceDeploymentChainOwnRepoPartialControllerDiscoveryDoesNotLeakOtherWorkloadEvidence(t *testing.T) {
	...
	if got, want := querycontract.StringVal(summary, "deployment_truth_tier"), "runtime_confirmed"; got == want {
...
// (trace_deployment_controllers.go) claims a query error "fails
