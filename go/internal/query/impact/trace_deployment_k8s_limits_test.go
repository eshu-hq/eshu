...
func TestFetchK8sResourceResultUsesSentinelAndReportsTruncation(t *testing.T) {
	...
	if got, want := store.queryLimit, querycontract.ServiceStoryItemLimit+1; got != want {
	...
	if got, want := len(result.imageRefs), querycontract.ServiceStoryItemLimit; got != want {
		t.Fatalf("len(FetchK8sResourceResult().imageRefs) = %d, want images from returned rows only (%d)", got, want)
	}
	if got, want := querycontract.IntVal(result.limits, "limit"), querycontract.ServiceStoryItemLimit; got != want {
		t.Fatalf("k8s_resource_limits.limit = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(result.limits, "query_sentinel_limit"), querycontract.ServiceStoryItemLimit+1; got != want {
		t.Fatalf("k8s_resource_limits.query_sentinel_limit = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(result.limits, "returned_count"), querycontract.ServiceStoryItemLimit; got != want {
		t.Fatalf("k8s_resource_limits.returned_count = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(result.limits, "observed_count"), querycontract.ServiceStoryItemLimit+1; got != want {
		t.Fatalf("k8s_resource_limits.observed_count = %d, want lower bound %d", got, want)
	}
	if !querycontract.BoolVal(result.limits, "observed_count_is_lower_bound") {
...
func TestFetchK8sResourceResultReportsLowerBoundWhenSubstringRowsFillProbe(t *testing.T) {
	...
	for index := 1; index < querycontract.ServiceStoryItemLimit+1; index++ {
		rows = append(rows, querycontract.EntityContent{
	...
	handler := &Handler{Content: &recordingK8sContentStore{rows: rows}}
	...
	if got, want := querycontract.IntVal(result.limits, "observed_count"), 1; got != want {
		t.Fatalf("k8s_resource_limits.observed_count = %d, want observed exact-name count %d", got, want)
	}
	if !querycontract.BoolVal(result.limits, "observed_count_is_lower_bound") || !querycontract.BoolVal(result.limits, "truncated") {
...
func TestFetchDeploymentSourceGitOpsReportsRepositoryProbeLowerBound(t *testing.T) {
	...
	rows := make([]querycontract.EntityContent, querycontract.RepositorySemanticEntityLimit+1)
	...
	if got, want := store.queryLimit, querycontract.RepositorySemanticEntityLimit+1; got != want {
...
func TestBoundedK8sResourceResultCapsMergedGitOpsRows(t *testing.T) {
	...
	contentRows := make([]map[string]any, 0, querycontract.ServiceStoryItemLimit-1)
	for index := range querycontract.ServiceStoryItemLimit - 1 {
	...
	if got, want := querycontract.IntVal(result.limits, "content_observed_count"), querycontract.ServiceStoryItemLimit-1; got != want {
		t.Fatalf("k8s_resource_limits.content_observed_count = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(result.limits, "deployment_source_observed_count"), len(gitOpsRows); got != want {
...
func TestBoundedK8sResourceResultPropagatesDeploymentSourceLowerBound(t *testing.T) {
	...
	if !querycontract.BoolVal(result.limits, "deployment_source_observed_count_is_lower_bound") {
...
func TestBuildDeploymentTraceResponseIncludesK8sResourceLimits(t *testing.T) {
	...
	want := map[string]any{
		...
		"returned_count":                querycontract.ServiceStoryItemLimit,
		"observed_count":                querycontract.ServiceStoryItemLimit + 1,
