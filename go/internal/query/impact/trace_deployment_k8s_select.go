...
// out of trace_deployment_resources.go to keep both files under the
...
// querycontract.RepositorySemanticEntityLimit ceiling (#5363; real pagination is #5367). It
...
// mergeDeploymentTraceRows (trace_deployment_resources.go) dedups the
...
func k8sResourceWireRow(row querycontract.EntityContent) map[string]any {
	kind, _ := querycontract.MetadataNonEmptyString(row.Metadata, "kind")
	qualifiedName, _ := querycontract.MetadataNonEmptyString(row.Metadata, "qualified_name")
	images := querycontract.MetadataStringSlice(row.Metadata, "container_images")
...
// truncated at the querycontract.RepositorySemanticEntityLimit ceiling.
//
...
// capped at querycontract.ServiceStoryItemLimit (the same public cap as the surfaced pool),
...
func (h *Handler) fetchK8sSelectMatchedServiceIDs(
	ctx context.Context,
	repoID string,
	...
	candidateLimit := querycontract.RepositorySemanticEntityLimit + 1
	...
	if truncated {
		candidates = candidates[:querycontract.RepositorySemanticEntityLimit]
	}

	matched := make([]string, 0, querycontract.ServiceStoryItemLimit)
	seen := make(map[string]struct{}, querycontract.ServiceStoryItemLimit)
	for _, candidate := range candidates {
		if len(matched) >= querycontract.ServiceStoryItemLimit {
			break
		}
...
// outgrew the querycontract.RepositorySemanticEntityLimit ceiling and some SELECTS edges may
...
func (h *Handler) reportK8sSelectCandidatePoolTruncated(ctx context.Context, repoID string) {
	if h.Logger != nil {
