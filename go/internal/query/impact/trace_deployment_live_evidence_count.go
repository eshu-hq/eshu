...
// querycontract.ServiceStoryItemLimit rows, disclosing that the count is a conservative
...
type liveInstanceSummary struct {
	...
	// ListLiveIdentityMatches read returned exactly querycontract.ServiceStoryItemLimit
...
func (h *Handler) fetchWorkloadLiveInstanceSummary(
	ctx context.Context,
	...
	for _, anchor := range anchors {
		...
		// #5663: a full page (exactly querycontract.ServiceStoryItemLimit rows) from this
		...
		if len(matches) == querycontract.ServiceStoryItemLimit {
			truncated = true
		}
