...
// out of trace_deployment_live_evidence_count_test.go to stay under
...
// querycontract.ServiceStoryItemLimit rows must report truncated=true -- a full page is
...
func TestFetchWorkloadLiveInstanceSummaryAnchorAtLimitTruncated(t *testing.T) {
	...
	summary, err := h.fetchWorkloadLiveInstanceSummary(
		t.Context(), controllers, resources, []string{"img@sha256:a"}, querycontract.RepositoryAccessFilter{AllScopes: true},
	)
	if err != nil {
	...
	if !summary.truncated {
		t.Fatal("truncated = false, want true (anchor read hit querycontract.ServiceStoryItemLimit rows)")
	}
}

// TestFetchWorkloadLiveInstanceSummaryUnderLimitNotTruncated is the #5663 RED
// case (b): an anchor read that returns fewer than querycontract.ServiceStoryItemLimit rows
...
func TestFetchWorkloadLiveInstanceSummaryUnderLimitNotTruncated(t *testing.T) {
	...
	if summary.truncated {
		t.Fatal("truncated = true, want false (anchor read returned fewer than querycontract.ServiceStoryItemLimit rows)")
	}
}
...
func TestFetchWorkloadLiveInstanceSummaryAnyAnchorAtLimitTruncatesWholeSummary(t *testing.T) {
	...
	h := &Handler{KubernetesPodTemplates: store}
	summary, err := h.fetchWorkloadLiveInstanceSummary(
		t.Context(), controllers, resources, []string{"img@sha256:shared"}, querycontract.RepositoryAccessFilter{AllScopes: true},
	)
	if err != nil {
		t.Fatalf("error = %v, want nil", err)
	}
...
func TestFetchWorkloadLiveInstanceSummaryNilSummaryStaysNilAtLimit(t *testing.T) {
	...
	h := &Handler{KubernetesPodTemplates: store}
	summary, err := h.fetchWorkloadLiveInstanceSummary(
		t.Context(), nil, nil, nil, querycontract.RepositoryAccessFilter{AllScopes: true},
	)
	if err != nil {
		t.Fatalf("error = %v, want nil", err)
	}
