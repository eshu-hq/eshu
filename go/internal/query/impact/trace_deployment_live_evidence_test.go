...
func TestFetchWorkloadLiveEvidenceNilHandler(t *testing.T) {
	...
	var h *Handler
	live, err := h.fetchWorkloadLiveEvidence(t.Context(), nil, nil, nil, querycontract.RepositoryAccessFilter{})
	if err != nil {
...
func TestFetchWorkloadLiveEvidenceDistinctWorkloadsSharedDigest(t *testing.T) {
	...
	liveA, err := h.fetchWorkloadLiveEvidence(t.Context(), controllersA, resourcesA, []string{sharedDigest}, querycontract.RepositoryAccessFilter{AllScopes: true})
	if err != nil {
	...
	liveB, err := h.fetchWorkloadLiveEvidence(t.Context(), controllersB, resourcesB, []string{sharedDigest}, querycontract.RepositoryAccessFilter{AllScopes: true})
	if err != nil {
...
// live in trace_deployment_live_evidence_declared_test.go (kept
