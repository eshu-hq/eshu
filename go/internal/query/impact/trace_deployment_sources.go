...
func (h *Handler) FetchDeploymentSourceResult(
	ctx context.Context,
	workloadID string,
	repoID string,
...
func FetchDeploymentSourceResultFromGraph(
	...
	rows, mergedTruncated := querycontract.CapMapRows(merged, querycontract.ContextStoryItemLimit)
...
func sortDeploymentSources(rows []map[string]any) {
	sort.SliceStable(rows, func(i, j int) bool {
		...
		leftPriority := deploymentSourceRelationshipPriority(querycontract.StringVal(left, "relationship_type"))
		rightPriority := deploymentSourceRelationshipPriority(querycontract.StringVal(right, "relationship_type"))
		if leftPriority != rightPriority {
			return leftPriority < rightPriority
		}
		leftKey := strings.Join([]string{
			querycontract.StringVal(left, "repo_name"),
			querycontract.StringVal(left, "source_id"),
			querycontract.StringVal(left, "target_id"),
		}, "\x00")
		rightKey := strings.Join([]string{
			querycontract.StringVal(right, "repo_name"),
			querycontract.StringVal(right, "source_id"),
			querycontract.StringVal(right, "target_id"),
		}, "\x00")
...
func normalizedDeploymentSources(rows []map[string]any) ([]map[string]any, error) {
	...
	for _, row := range rows {
		confidence, err := querycontract.FiniteGraphFloat(row, "confidence", "deployment source")
		if err != nil {
			return nil, err
		}
		...
		if querycontract.BoolVal(row, "flux_target_bindings_saturated") {
...
func deploymentSourceRepoIDs(rows []map[string]any) []string {
	...
	for _, row := range rows {
		if id := querycontract.StringVal(row, "source_id"); id != "" {
			ids = append(ids, id)
		}
	}
	return querycontract.UniqueSortedStrings(ids)
}
...
func deploymentSourceRowsWithEndpoints(
	...
	for _, row := range rows {
		...
		if relationshipType == "DEPLOYMENT_SOURCE" {
			row["source_id"] = querycontract.StringVal(row, "instance_id")
			row["target_id"] = querycontract.StringVal(row, "repo_id")
			continue
		}
		row["source_id"] = querycontract.StringVal(row, "repo_id")
...
func deploymentSourceRelationshipKey(row map[string]any) string {
	relationshipType := querycontract.StringVal(row, "relationship_type")
