...
// blastRadiusSqlTableCypher. impact.BlastRadiusSqlTableBranches is the compile-time
...
func TestSQLTableBlastRadiusBranchTableMatchesQuery(t *testing.T) {
	...
	if len(branches) != impact.BlastRadiusSqlTableBranches {
		t.Fatalf("branch fixtures = %d, impact.BlastRadiusSqlTableBranches = %d -- a UNION branch was added or "+
			"removed without updating sqlBlastRadiusBranches, so it has no live proof",
			len(branches), impact.BlastRadiusSqlTableBranches)
	}
