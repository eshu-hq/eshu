...
// selector/enrichment behind ImpactHandler.TraceContext and the full
// trace-deployment-chain handler path. They live in root (not in the impact
...
func TestLoadUncorrelatedCloudResourceCandidatesUsesBoundedServiceSelector(t *testing.T) {
	...
	if got, want := querycontract.StringVal(got[0], "candidate_status"), "uncorrelated"; got != want {
		t.Fatalf("candidate_status = %q, want %q", got, want)
	}
	if got, want := querycontract.StringVal(got[0], "missing_relationship"), "workload_cloud_relationship"; got != want {
...
func TestFetchServiceTraceContextAcceptsQualifiedWorkloadID(t *testing.T) {
	...
	if got, want := querycontract.SafeStr(ctx, "id"), "workload:service-edge-api"; got != want {
		t.Fatalf("context.id = %#v, want %#v", got, want)
	}
	if got, want := querycontract.SafeStr(ctx, "name"), "service-edge-api"; got != want {
...
func TestFetchServiceTraceContextPreservesResolvedWorkloadIDWhenAnotherWorkloadNameCollides(t *testing.T) {
	...
	ctx, err := fetchServiceTraceContext(
		...
		nil,
		"workload:orders",
		impact.TraceEnrichmentOptions(impact.TraceDeploymentChainRequest{ServiceName: "workload:orders"}),
	)
	if err != nil {
		t.Fatalf("fetchServiceTraceContext() error = %v, want nil", err)
	}
	if got, want := querycontract.SafeStr(ctx, "id"), "workload:orders"; got != want {
...
func TestFetchServiceTraceContextIncludesGraphDeploymentEvidenceWithoutContent(t *testing.T) {
	...
	ctx, err := fetchServiceTraceContext(
		...
		nil,
		"checkout-service",
		impact.TraceEnrichmentOptions(impact.TraceDeploymentChainRequest{ServiceName: "checkout-service"}),
	)
	if err != nil {
		t.Fatalf("fetchServiceTraceContext() error = %v, want nil", err)
	}

	evidence := querycontract.MapValue(ctx, "deployment_evidence")
	if len(evidence) == 0 {
...
func TestTraceDeploymentChainKeepsConfigDerivedCloudResourcesAsUncorrelatedCandidates(t *testing.T) {
	...
	resources := querycontract.MapSliceValue(body, "cloud_resources")
	if got, want := len(resources), 0; got != want {
		t.Fatalf("cloud_resources len = %d, want %d; body = %#v", got, want, body)
	}
	candidates := querycontract.MapSliceValue(body, "uncorrelated_cloud_resources")
	if got, want := len(candidates), 1; got != want {
		t.Fatalf("uncorrelated_cloud_resources len = %d, want %d; body = %#v", got, want, body)
	}
	if got, want := querycontract.StringVal(candidates[0], "relationship_basis"), "deployment_config_read_evidence"; got != want {
	...
	if got, want := querycontract.IntVal(overview, "cloud_resource_count"), 0; got != want {
		t.Fatalf("deployment_overview.cloud_resource_count = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(overview, "uncorrelated_cloud_resource_count"), 1; got != want {
		t.Fatalf("deployment_overview.uncorrelated_cloud_resource_count = %d, want %d", got, want)
	}
	summary := querycontract.MapValue(body, "deployment_fact_summary")
	if missing := querycontract.StringSliceVal(summary, "missing_evidence"); !slices.Equal(missing, []string{"workload_cloud_relationship_missing"}) {
