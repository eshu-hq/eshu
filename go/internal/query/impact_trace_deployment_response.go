...
// shape from an already-filtered workload context (see impact_trace_deployment.go
// for the handler and the #5167 W3 grant filtering applied to deployment_sources
// before this response is built). Split out to keep both files under the
...
func buildDeploymentTraceFields(serviceName string, workloadContext map[string]any) deploymentTraceFields {
	...
	f.deploymentFacts = buildDeploymentFacts(f.instances, f.deploymentSources)
...
func buildDeploymentTraceResponse(serviceName string, workloadContext map[string]any) map[string]any {
	...
	response := map[string]any{
		...
		"controller_overview":     buildControllerOverview(f.platforms, f.platformKinds, f.controllerEntities, f.deploymentSources, f.deploymentEvidence),
