...
// This file hosts the indirect-evidence hostname bound behind
// LoadConsumerRepositoryEnrichmentFromCandidates (Issue #6060, lane B B4).
// It moved here from the query root (indirect_evidence_hostnames.go) with
// the enrichment entry points because the service family consumes them and
// a handler-family subpackage cannot import the query root without an
...
// bounded consumer evidence. Pinned by the staying
// indirect_evidence_hostnames_test.go and
// deployment_trace_truncation_disclosure_test.go via the root forwarder,
// and the moving service evidence-bound test.
//
