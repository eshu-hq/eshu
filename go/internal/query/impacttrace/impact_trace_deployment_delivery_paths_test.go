...
func TestBuildDeploymentTraceResponseDeduplicatesRepositoryDeliveryPaths(t *testing.T) {
	...
	if got, want := querycontract.StringVal(deliveryPaths[0], "target"), "deployment-helm"; got != want {
		t.Fatalf("delivery_paths[0].target = %q, want %q", got, want)
	}
	if got, want := querycontract.StringVal(deliveryPaths[1], "type"), "repository_delivery_artifact"; got != want {
...
func TestBuildDeploymentTraceResponseDoesNotEmitStructurallyEmptyDeliveryPaths(t *testing.T) {
	...
	for _, path := range deliveryPaths {
		if querycontract.StringVal(path, "type") == "" {
			t.Fatalf("delivery path missing type: %#v", path)
		}
		if querycontract.StringVal(path, "type") == "" &&
			querycontract.StringVal(path, "target") == "" &&
			querycontract.StringVal(path, "path") == "" &&
			querycontract.StringVal(path, "kind") == "" &&
			querycontract.StringVal(path, "artifact_type") == "" &&
			querycontract.StringVal(path, "evidence_kind") == "" {
...
func TestBuildDeploymentTraceResponseSeparatesControllerIdentityFromObservedTargets(t *testing.T) {
	...
	got := BuildDeploymentTraceResponse("service-edge-api", ctx, map[string]any{})
	...
	if gotControllers := querycontract.StringSliceVal(controllerOverview, "controllers"); len(gotControllers) != 0 {
		t.Fatalf("controller_overview.controllers = %#v, want empty when no controller entities exist", gotControllers)
	}
	if gotTargets, wantTargets := querycontract.StringSliceVal(controllerOverview, "observed_targets"), []string{"modern"}; !reflect.DeepEqual(gotTargets, wantTargets) {
		t.Fatalf("controller_overview.observed_targets = %#v, want %#v", gotTargets, wantTargets)
	}
	if gotKinds, wantKinds := querycontract.StringSliceVal(controllerOverview, "controller_kinds"), []string{"kubernetes"}; !reflect.DeepEqual(gotKinds, wantKinds) {
	...
	if gotController := querycontract.StringVal(controllerDrivenPaths[0], "controller"); gotController != "" {
		t.Fatalf("controller_driven_paths[0].controller = %q, want empty when only observed target is known", gotController)
	}
	if gotTarget, wantTarget := querycontract.StringVal(controllerDrivenPaths[0], "observed_target"), "modern"; gotTarget != wantTarget {
