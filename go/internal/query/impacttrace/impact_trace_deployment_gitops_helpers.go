...
// ArgoCDApplication(Set) plus FluxKustomization/FluxHelmRelease (issue #5483
// C2) share the controller-entity trace surface. It lives here rather than in
// the impact package because this package's controller-entity builder reads
// it, and an impacttrace file cannot name an impact-defined symbol (package
// impact imports impacttrace, so the reverse import would be a cycle).
// Exported because the impact package reads it from outside this package.
// See #6060.
var ControllerEntityTypes = map[string]string{
	...
	// needs the chart-as-path-root rule in deploymentTraceSourceRoots below.
	"FluxKustomization": "flux_kustomization",
	"FluxHelmRelease":   "flux_helm_release",
}

// MetadataNonEmptyStringValue returns the cleaned non-empty metadata string
// for key, or "" when absent. Same home rationale as ControllerEntityTypes:
// this package's controller-entity builder is the primary reader. See #6060.
func MetadataNonEmptyStringValue(metadata map[string]any, key string) string {
	value, _ := querycontract.MetadataNonEmptyString(metadata, key)
	return value
}

// UniqueNonEmptyRepoIDs returns the distinct non-empty repo_ids across
// sources, preserving first-seen order. It lives here because the
// controller-entity surface is this package's primary reader; the impact
// package reaches it from outside this package. See #6060.
func UniqueNonEmptyRepoIDs(sources []map[string]any) []string {
	...
	for _, source := range sources {
		repoID := querycontract.SafeStr(source, "repo_id")
		if repoID == "" {
			continue
		}
...
func BuildDeploymentSourceControllerEntity(entity querycontract.EntityContent) (map[string]any, bool) {
	...
	controller := map[string]any{
		...
		"generator_source_repos": slices.Clone(querycontract.MetadataStringSlice(entity.Metadata, "generator_source_repos")),
		"generator_source_paths": slices.Clone(querycontract.MetadataStringSlice(entity.Metadata, "generator_source_paths")),
		"template_source_repos":  slices.Clone(querycontract.MetadataStringSlice(entity.Metadata, "template_source_repos")),
		"template_source_paths":  slices.Clone(querycontract.MetadataStringSlice(entity.Metadata, "template_source_paths")),
...
func CollectDeploymentSourceK8sResources(
	...
	for _, controller := range controllerEntities {
		repoID := querycontract.StringVal(controller, "repo_id")
		if repoID == "" {
			continue
		}
		controllersByRepo[repoID] = append(controllersByRepo[repoID], controller)
		if targetRepoID := querycontract.StringVal(controller, "flux_target_repo_id"); targetRepoID != "" && targetRepoID != repoID {
	...
	for _, entity := range entities {
		...
		key := entity.EntityID + "|" + sourceRoot + "|" + querycontract.StringVal(controller, "entity_id")
		if _, exists := seen[key]; exists {
			continue
		}
		seen[key] = struct{}{}

		qualifiedName, _ := querycontract.MetadataNonEmptyString(entity.Metadata, "qualified_name")
		containerImages := querycontract.MetadataStringSlice(entity.Metadata, "container_images")
...
func deploymentTraceSourceRoots(entityType string, metadata map[string]any) []string {
	values := append([]string{MetadataNonEmptyStringValue(metadata, "source_path")}, querycontract.MetadataStringSlice(metadata, "template_source_paths")...)
...
func deploymentTraceDiscoveryRoots(metadata map[string]any) []string {
	return deploymentTraceNormalizedRoots(querycontract.MetadataStringSlice(metadata, "generator_source_paths"))
}
...
func deploymentTraceControllerMatchesService(controller map[string]any, normalizedService string) bool {
	...
	identityCandidates := []string{
		querycontract.StringVal(controller, "entity_id"),
		querycontract.StringVal(controller, "entity_name"),
		querycontract.StringVal(controller, "source_repo"),
	}
	...
	pathCandidates := []string{
		querycontract.StringVal(controller, "relative_path"),
		querycontract.StringVal(controller, "source_path"),
		querycontract.StringVal(controller, "source_root"),
	}
	pathCandidates = append(pathCandidates, querycontract.StringSliceMapValue(controller, "source_roots")...)
	pathCandidates = append(pathCandidates, querycontract.StringSliceMapValue(controller, "discovery_roots")...)
...
func matchDeploymentTraceController(entity querycontract.EntityContent, controllers []map[string]any) (map[string]any, string, bool) {
	...
	for index, controller := range controllers {
		for _, root := range querycontract.StringSliceMapValue(controller, "source_roots") {
...
func deploymentTraceEntityKind(entity querycontract.EntityContent) (string, bool) {
...
func SortDeploymentTraceMaps(rows []map[string]any) {
	sort.SliceStable(rows, func(i, j int) bool {
		leftRepoID := querycontract.StringVal(rows[i], "repo_id")
		rightRepoID := querycontract.StringVal(rows[j], "repo_id")
		if leftRepoID != rightRepoID {
			return leftRepoID < rightRepoID
		}
		leftPath := querycontract.StringVal(rows[i], "relative_path")
		rightPath := querycontract.StringVal(rows[j], "relative_path")
		if leftPath != rightPath {
			return leftPath < rightPath
		}
		return querycontract.StringVal(rows[i], "entity_id") < querycontract.StringVal(rows[j], "entity_id")
	})
}
