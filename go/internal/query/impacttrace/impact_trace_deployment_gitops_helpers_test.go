...
func TestSelectRelevantDeploymentSourceControllersFiltersToServiceScopedArgoCDRoots(t *testing.T) {
	...
	gotIDs := []string{querycontract.StringVal(got[0], "entity_id"), querycontract.StringVal(got[1], "entity_id")}
	wantIDs := []string{"app-1", "appset-1"}
	if !reflect.DeepEqual(gotIDs, wantIDs) {
		t.Fatalf("selected controller ids = %#v, want %#v", gotIDs, wantIDs)
	}

	if got, want := querycontract.StringVal(got[0], "controller_kind"), "argocd_application"; got != want {
		t.Fatalf("controllers[0].controller_kind = %q, want %q", got, want)
	}
	if got, want := querycontract.StringVal(got[0], "source_root"), "services/sample-service-api/overlays/prod"; got != want {
		t.Fatalf("controllers[0].source_root = %q, want %q", got, want)
	}
	if got, want := querycontract.StringSliceMapValue(got[1], "source_roots"), []string{"services/sample-service-api/overlays/prod"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("controllers[1].source_roots = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringSliceMapValue(got[1], "discovery_roots"), []string{"services"}; !reflect.DeepEqual(got, want) {
...
func TestSelectRelevantDeploymentSourceControllersTrustsSoleWorkloadOwnRepoController(t *testing.T) {
	...
	if got, want := querycontract.StringVal(got[0], "entity_id"), "argocd-app-1"; got != want {
...
func TestSelectRelevantDeploymentSourceControllersRequiresTokenMatchWhenOwnRepoDefinesMultipleWorkloadsAndBothControllersIndexed(t *testing.T) {
	...
	if got, want := querycontract.StringVal(got[0], "entity_id"), "app-1"; got != want {
...
func TestSelectRelevantDeploymentSourceControllersRejectsShortNameCollisions(t *testing.T) {
	...
	if got, want := querycontract.StringVal(got[0], "entity_id"), "api-app"; got != want {
		t.Fatalf("selected controller id = %q, want %q", got, want)
	}
}
...
func TestCollectDeploymentSourceK8sResourcesIncludesRootScopedResourcesWithAttribution(t *testing.T) {
	...
	first := got[0]
	if got, want := querycontract.StringVal(first, "repo_id"), "repo-helm"; got != want {
		t.Fatalf("resources[0].repo_id = %q, want %q", got, want)
	}
	if got, want := querycontract.StringVal(first, "source_root"), "services/sample-service-api/overlays/prod"; got != want {
		t.Fatalf("resources[0].source_root = %q, want %q", got, want)
	}
	if got, want := querycontract.StringVal(first, "controller_kind"), "argocd_application"; got != want {
		t.Fatalf("resources[0].controller_kind = %q, want %q", got, want)
	}

	resourceKinds := []string{
		querycontract.StringVal(got[0], "kind"),
		querycontract.StringVal(got[1], "kind"),
		querycontract.StringVal(got[2], "kind"),
	}
