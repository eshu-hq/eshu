...
func ExpectedArgoCDTrackingIDs(controllers []map[string]any, k8sResources []map[string]any) []string {
	...
	for _, resource := range k8sResources {
		kind := querycontract.StringVal(resource, "kind")
		name := querycontract.StringVal(resource, "entity_name")
		if kind == "" || name == "" {
			continue
		}
		group := apiVersionGroup(querycontract.StringVal(resource, "api_version"))
		namespace := querycontract.StringVal(resource, "namespace")
...
func argoCDApplicationNames(controllers []map[string]any) []string {
	...
	for _, controller := range controllers {
		if querycontract.StringVal(controller, "controller_kind") != "argocd_application" {
			continue
		}
		name := querycontract.StringVal(controller, "entity_name")
		if name == "" {
			continue
		}
...
// LiveIdentityAnchorDeclaredObject is the same anchor kind, exported because
// staying query tests pin anchor identity in store-exercising fixtures; the
// home stays this package. See #6060.
const LiveIdentityAnchorDeclaredObject = liveIdentityAnchorDeclaredObject
...
// the two probes.
func LiveIdentityAnchorFilter(anchor liveIdentityAnchor, imageRefs []string, access querycontract.RepositoryAccessFilter) KubernetesPodTemplateFilter {
