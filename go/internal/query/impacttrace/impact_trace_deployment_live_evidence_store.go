...
type KubernetesPodTemplateStore interface {
	...
	// to querycontract.ServiceStoryItemLimit rows (existence checks use LIMIT 1; this read
...
const hasLiveKubernetesPodTemplateIdentityQuery = HasLiveKubernetesPodTemplateIdentityQuery

// HasLiveKubernetesPodTemplateIdentityQuery is the same query string, exported because staying query tests pin
// the SQL text; the home stays this package. See #6060.
...
// parameter ($6) rather than a literal so it always matches querycontract.ServiceStoryItemLimit
// without risking drift between the SQL text and the Go constant.
const listLiveKubernetesPodTemplateIdentityMatchesQuery = ListLiveKubernetesPodTemplateIdentityMatchesQuery

// ListLiveKubernetesPodTemplateIdentityMatchesQuery is the same query string, exported because staying query tests pin
// the SQL text; the home stays this package. See #6060.
...
const listLiveKubernetesPodTemplateIdentityMatchesScopedQuery = ListLiveKubernetesPodTemplateIdentityMatchesScopedQuery

// ListLiveKubernetesPodTemplateIdentityMatchesScopedQuery is the same query string, exported because staying query tests pin
// the SQL text; the home stays this package. See #6060.
...
// to querycontract.ServiceStoryItemLimit rows. It shares HasLiveIdentityMatch's fail-closed
...
func scanLiveIdentityMatchRows(rows *sql.Rows) ([]LiveIdentityMatch, error) {
	...
	for rows.Next() {
		...
		if readyReplicas.Valid {
			v := int32(readyReplicas.Int64) // #nosec G115 -- bounded: readyReplicas originates as int32 in the K8s API and round-trips through the int64 column losslessly
