...
func TestBuildDeploymentTraceResponsePromotesWorkflowAndControllerProvenance(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(provenanceOverview, "families"), []string{"ansible", "argocd", "github_actions", "helm", "jenkins"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("provenance_overview.families = %#v, want %#v", got, want)
	}
	if got, want := querycontract.IntVal(provenanceOverview, "artifact_lineage_count"), 3; got != want {
		t.Fatalf("provenance_overview.artifact_lineage_count = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(provenanceOverview, "workflow_artifact_count"), 1; got != want {
		t.Fatalf("provenance_overview.workflow_artifact_count = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(provenanceOverview, "controller_artifact_count"), 1; got != want {
	...
	if got, want := querycontract.StringSliceVal(deploymentOverview, "provenance_families"), []string{"ansible", "argocd", "github_actions", "helm", "jenkins"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("deployment_overview.provenance_families = %#v, want %#v", got, want)
	}
	if got, want := querycontract.IntVal(deploymentOverview, "artifact_lineage_count"), 3; got != want {
	...
	if !querycontract.ContainsAllSubstrings(
		story,
...
func TestBuildDeploymentTraceResponseReportsProvenanceFamiliesWithoutArtifactLineage(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(provenanceOverview, "families"), []string{"github_actions", "helm", "jenkins"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("provenance_overview.families = %#v, want %#v", got, want)
	}
	...
	if got, want := querycontract.StringSliceVal(deploymentOverview, "provenance_families"), []string{"github_actions", "helm", "jenkins"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("deployment_overview.provenance_families = %#v, want %#v", got, want)
	}
	if got, want := querycontract.IntVal(provenanceOverview, "artifact_lineage_count"), 0; got != want {
		t.Fatalf("provenance_overview.artifact_lineage_count = %d, want %d", got, want)
	}
}
