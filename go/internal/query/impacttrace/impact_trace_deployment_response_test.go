...
func TestBuildDeploymentTraceResponseIncludesArtifactBackedDeliveryPaths(t *testing.T) {
	...
	deliveryPaths := querycontract.MapSliceValue(got, "delivery_paths")
	...
	for _, path := range deliveryPaths {
		switch querycontract.StringVal(path, "resolved_id") {
		case "resolved-gitops":
			seenGitOps = true
			if got, want := querycontract.StringVal(path, "type"), "deployment_evidence"; got != want {
				t.Fatalf("gitops delivery path type = %q, want %q", got, want)
			}
			if got, want := querycontract.StringVal(path, "artifact_family"), "argocd"; got != want {
				t.Fatalf("gitops artifact_family = %q, want %q", got, want)
			}
		case "resolved-terraform":
			seenTerraform = true
			if got, want := querycontract.StringVal(path, "relationship_type"), "PROVISIONS_DEPENDENCY_FOR"; got != want {
...
func TestBuildDeploymentTraceResponseExplainsUncorrelatedCloudCandidates(t *testing.T) {
	...
	got := BuildDeploymentTraceResponse("sample-service-api", ctx, map[string]any{})
	if cloudResources := querycontract.MapSliceValue(got, "cloud_resources"); len(cloudResources) != 0 {
		t.Fatalf("cloud_resources = %#v, want no promoted attached resources", cloudResources)
	}
	candidates := querycontract.MapSliceValue(got, "uncorrelated_cloud_resources")
	...
	summary := querycontract.MapValue(got, "deployment_fact_summary")
	missingEvidence := querycontract.StringSliceVal(summary, "missing_evidence")
	if !querycontract.ContainsString(missingEvidence, "workload_cloud_relationship_missing") {
		...
		"instances": []map[string]any{
			{
				...
				"platforms": []map[string]any{{
					"platform_id":   "platform:payments-argocd",
					"platform_name": "payments-argocd",
					"platform_kind": "argocd_application",
					"topology_edges": []map[string]any{{
						"relationship_type": "RUNS_ON",
						"source_id":         "inst-1",
						"target_id":         "platform:payments-argocd",
						"target_name":       "payments-argocd",
					}},
				}},
			},
			{
