...
func incidentContextWorkflow() InvestigationWorkflow {
	return InvestigationWorkflow{
		...
		RequiredInputs: []PlaybookInput{
			...
			{Name: "service_id", Type: PlaybookInputIdentifier, Description: "Optional service or workload selector for service-story, runtime, observability, and incident drilldowns."},
		...
		MissingEvidenceRoutes: []WorkflowMissingEvidenceRoute{
			{
				...
				Calls: []WorkflowNextCall{{
					...
					Params:            []PlaybookParam{inputParam("provider_incident_id", "incident_id"), inputParam("provider", "provider"), inputParam("scope_id", "scope_id"), inputParam("service_id", "service_id"), inputParam("since", "since"), inputParam("until", "until"), limitParam("limit", 25)},
