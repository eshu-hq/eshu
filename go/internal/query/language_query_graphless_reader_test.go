...
func TestHandleLanguageQueryUnconfiguredReaderServesContentBackedEntityType(t *testing.T) {
	...
	// sourceBackendForTruthBasis maps TruthBasisContentIndex to
	// "postgres_content_store" (language/reasons.go), asserted by
	// literal value rather than by reference so this route-level test
	// pins the wire value, not the helper's spelling (#6642).
...
func TestHandleLanguageQueryUnconfiguredReaderReturns501ForGraphOnlyEntityType(t *testing.T) {
	...
	// carry RequiredProfile so querycontract.RequiredProfile reports the tier
	// that actually serves this residue path (local_authoritative, the first
	// tier with a graph sidecar) instead of falling through to its
	// local_full_stack default -- local_lightweight already
