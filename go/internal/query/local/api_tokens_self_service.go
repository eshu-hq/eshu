...
// api_tokens.go to keep that file under the repo's 500-line cap.
// The security contract they implement is exercised by root package query's
// identity_api_tokens_self_service_test.go.
...
func selfServiceTokenOwner(auth auth.AuthContext) string {
	if auth.AllScopes {
		return ""
	}
...
func (h *IdentityHandler) writeSelfServiceTokenNotFound(w http.ResponseWriter, ownerSubjectIDHash string, err error) bool {
	if ownerSubjectIDHash == "" || !errors.Is(err, ErrIdentityAPITokenNotFound) {
		return false
	}
	querycontract.WriteError(w, http.StatusNotFound, "api token not found")
	return true
}
...
func (h *IdentityHandler) enforceSelfServiceTokenCreateScope(
	...
	if !found {
		h.auditLocalIdentity(r, governanceaudit.EventTypeTokenLifecycle, governanceaudit.DecisionDenied, "api_token_self_service_scope_denied", "")
		querycontract.WriteError(w, http.StatusForbidden, "self-service api token creation requires a resolvable local identity")
		return false
	}
	if supplied := strings.TrimSpace(req.UserID); supplied != "" && supplied != userID {
		h.auditLocalIdentity(r, governanceaudit.EventTypeTokenLifecycle, governanceaudit.DecisionDenied, "api_token_self_service_scope_denied", "")
		querycontract.WriteError(w, http.StatusForbidden, "self-service api token creation cannot target another user")
		return false
	}
