...
// IdentityHandler serves production local identity bootstrap, login, MFA,
...
type IdentityHandler struct {
	...
	// requireSSODecision in sign_in_policy_gate.go) and by
...
func (h *IdentityHandler) Mount(mux *http.ServeMux) {
...
func (h *IdentityHandler) handleBootstrap(w http.ResponseWriter, r *http.Request) {
	if !h.ready(w) || !h.requireSharedOperator(w, r) {
		return
	}
	var req localIdentityBootstrapRequest
	if err := querycontract.ReadJSON(r, &req); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "invalid local identity bootstrap request")
		return
	}
	now := h.now()
	passwordHash, err := h.hashPassword(req.Password)
	if err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "invalid local identity password")
		return
	}
	...
	if err != nil {
		querycontract.WriteError(w, http.StatusInternalServerError, "failed to create local identity bootstrap")
		return
	}
	record := IdentityBootstrapRecord{
		TenantID:               strings.TrimSpace(req.TenantID),
		WorkspaceID:            strings.TrimSpace(req.WorkspaceID),
		UserID:                 userID,
		SubjectIDHash:          IdentityHash(req.LoginID),
		ProfileHandleHash:      IdentityHash(req.ProfileHandle),
		PasswordHash:           passwordHash,
		PasswordAlgorithm:      "bcrypt",
		PasswordParametersHash: IdentityHash("bcrypt"),
		...
		RecoveryCodeHashes:     IdentityHashes(req.RecoveryCodes),
	...
	if err := h.Store.BootstrapLocalIdentity(r.Context(), record); err != nil {
		...
		querycontract.WriteError(w, http.StatusBadRequest, "failed to bootstrap local identity")
		return
	}
...
func (h *IdentityHandler) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err := querycontract.ReadJSON(r, &req); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "invalid local identity login request")
		return
	}
	now := h.now()
	result, err := h.Store.AuthenticateLocalIdentity(r.Context(), IdentityAuthenticationAttempt{
		SubjectIDHash:         IdentityHash(req.LoginID),
		Password:              req.Password,
		MFARecoveryCodeHash:   IdentityHash(req.RecoveryCode),
	...
	if err != nil {
		...
		querycontract.WriteError(w, http.StatusUnauthorized, "local identity authentication failed")
		return
	}
	...
	if result.Status == IdentityAuthMFARequired && !result.Auth.AllScopes && result.Auth.TenantID != "" {
		if allowed, decision := h.requireSSODecision(r.Context(), result.Auth); !allowed {
			...
			querycontract.WriteError(w, http.StatusForbidden, "local sign-in is disabled by tenant policy; sign in with SSO, or an admin may use break-glass local sign-in")
			return
		}
	}
...
func (h *IdentityHandler) handleCreateInvitation(w http.ResponseWriter, r *http.Request) {
	...
	if err := querycontract.ReadJSON(r, &req); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "invalid local identity invitation request")
		return
	}
	now := h.now()
	...
	if !h.allowLocalUserCreation(r.Context(), tenantID) {
		...
		querycontract.WriteError(w, http.StatusForbidden, "local user creation is disabled by tenant sign-in policy")
		return
	}
	record := IdentityInvitationRecord{
		InviteID:             h.newID(),
		TenantID:             tenantID,
		WorkspaceID:          workspaceID,
		InviteCodeHash:       IdentityHash(inviteCode),
		InviteeHandleHash:    IdentityHash(req.InviteeHandle),
		InviterSubjectIDHash: authSubjectIDHash(r),
		RoleID:               localIdentityDefault(req.RoleID, "developer"),
		Status:               "active",
		PolicyRevisionHash:   PolicyRevision(tenantID, workspaceID),
		ExpiresAt:            localIdentityExpiry(req.ExpiresAt, now, 24*time.Hour),
		CreatedAt:            now,
		UpdatedAt:            now,
	}
	if record.InviteID == "" || inviteCode == "" {
		querycontract.WriteError(w, http.StatusInternalServerError, "failed to create local identity invitation")
		return
	}
	if err := h.Store.CreateLocalIdentityInvitation(r.Context(), record); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "failed to create local identity invitation")
		return
	}
	querycontract.WriteJSON(w, http.StatusCreated, map[string]any{
...
func (h *IdentityHandler) handleAcceptInvitation(w http.ResponseWriter, r *http.Request) {
	if !h.ready(w) {
		return
	}
	var req localIdentityAcceptInvitationRequest
	if err := querycontract.ReadJSON(r, &req); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "invalid local identity invitation acceptance request")
		return
	}
	passwordHash, err := h.hashPassword(req.Password)
	if err != nil {
	...
	acceptance := IdentityInvitationAcceptance{
		InviteCodeHash:         IdentityHash(req.InviteCode),
		UserID:                 h.newID(),
		SubjectIDHash:          IdentityHash(req.LoginID),
		ProfileHandleHash:      IdentityHash(req.ProfileHandle),
		PasswordHash:           passwordHash,
	...
	if err := h.Store.AcceptLocalIdentityInvitation(r.Context(), acceptance); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "failed to accept local identity invitation")
		return
	}
	querycontract.WriteJSON(w, http.StatusCreated, map[string]any{"status": "accepted", "user_id": acceptance.UserID})
}

func (h *IdentityHandler) handleResetPassword(w http.ResponseWriter, r *http.Request) {
	if !h.ready(w) || !h.requireAllScopeAuth(w, r) {
		return
	}
	if !h.requirePermissionFeature(
	...
	if err := querycontract.ReadJSON(r, &req); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "invalid local identity password reset request")
		return
	}
	passwordHash, err := h.hashPassword(req.Password)
	if err != nil {
	...
	reset := IdentityPasswordReset{
		UserID:                 querycontract.PathParam(r, "user_id"),
	...
	if err := h.Store.ResetLocalIdentityPassword(r.Context(), reset); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "failed to reset local identity password")
		return
	}
...
func (h *IdentityHandler) handleRotatePassword(w http.ResponseWriter, r *http.Request) {
	if !h.ready(w) {
		return
	}
	var req localIdentityPasswordRotationRequest
	if err := querycontract.ReadJSON(r, &req); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "invalid local identity password rotation request")
		return
	}
	...
	now := h.now()
	result, err := h.Store.RotateLocalIdentityPassword(r.Context(), IdentityPasswordRotation{
		SubjectIDHash:             IdentityHash(req.LoginID),
		CurrentPassword:           req.CurrentPassword,
		NewPasswordHash:           passwordHash,
		NewPasswordAlgorithm:      "bcrypt",
		NewPasswordParametersHash: IdentityHash("bcrypt"),
		CredentialID:              h.newID(),
		MFARecoveryCodeHash:       IdentityHash(req.RecoveryCode),
		ConsumeRecoveryCodeAt:     now,
		MFATOTPCode:               req.TOTPCode,
	...
	if err != nil {
		...
		querycontract.WriteError(w, http.StatusUnauthorized, "local identity password rotation failed")
		return
	}
	if !result.Authenticated {
		h.writeLocalIdentityUnauthenticated(w, r, result)
...
func (h *IdentityHandler) handleResetMFA(w http.ResponseWriter, r *http.Request) {
	if !h.ready(w) || !h.requireAllScopeAuth(w, r) {
		return
	}
	if !h.requirePermissionFeature(
	...
	if err := querycontract.ReadJSON(r, &req); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "invalid local identity mfa reset request")
		return
	}
	reset := IdentityMFAReset{
		UserID:              querycontract.PathParam(r, "user_id"),
		MFAFactorID:         h.newID(),
		MFAFactorKind:       localIdentityDefault(req.MFAFactorKind, "recovery_code"),
		MFACredentialHandle: strings.TrimSpace(req.MFACredentialHandle),
	...
	if err := h.Store.ResetLocalIdentityMFA(r.Context(), reset); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "failed to reset local identity mfa")
		return
	}
...
func (h *IdentityHandler) handleDisableUser(w http.ResponseWriter, r *http.Request) {
	if !h.ready(w) || !h.requireAllScopeAuth(w, r) {
		return
	}
	if !h.requirePermissionFeature(
	...
	if err := h.Store.DisableLocalIdentityUser(r.Context(), IdentityDisableUser{
		UserID:     querycontract.PathParam(r, "user_id"),
		DisabledAt: h.now(),
	}); err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "failed to disable local identity user")
		return
	}
