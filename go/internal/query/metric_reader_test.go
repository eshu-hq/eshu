...
//
// The implementation moved to testutil (#6060) so the handler families
// that left this package can install a reader the same way; a _test.go
// declaration is importable from nowhere. See
// testutil.WithPackageMetricReader for the delegate-once ordering, the
...
func withPackageMetricReader(t *testing.T, reset func()) *sdkmetric.ManualReader {
	t.Helper()
	return testutil.WithPackageMetricReader(t, reset)
}
