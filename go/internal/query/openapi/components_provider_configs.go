...
// componentsProviderConfigs documents the DB-backed identity
// provider-config CRUD schemas (#4966, epic #4962), included into
// components' "schemas" object as a separate constant fragment to
// keep components.go under the repository's file-size limit — same
// pattern as componentsReplatforming
// (components_replatforming.go).
const componentsProviderConfigs = `      "AdminProviderConfig": {
        "type": "object",
