...
// ComponentsWorkloadSession documents the WorkloadContext and browser
// -session schemas, included into components' "schemas" object as a
// separate constant fragment to keep components.go comfortably under
// the repository's file-size limit — same pattern as
// componentsReplatforming (components_replatforming.go) and
// componentsProviderConfigs (components_provider_configs.go).
const ComponentsWorkloadSession = `      "WorkloadContext": {
        "type": "object",
        "properties": {
          "id": {"type": "string"},
          "name": {"type": "string"},
          ...
          "hostnames": {"type": "array", "description": "Exact hostnames found in service evidence, capped at 50 rows. result_limits.hostname_count reports the total before the cut; a cut sets result_limits.truncated and adds hostnames_truncated to partial_reasons.", "items": {"type": "object"}},
          ...
          "entrypoints": {"type": "array", "description": "Public and internal service entrypoints, capped at 50 rows. result_limits.entrypoint_count reports the total before the cut; a cut sets result_limits.truncated and adds entrypoints_truncated to partial_reasons.", "items": {"type": "object"}},
          "network_paths": {"type": "array", "description": "Entrypoint-to-runtime paths, one per entrypoint that matches a runtime instance, capped at 50 rows. result_limits.network_path_count reports the total before the cut; a cut sets result_limits.truncated and adds network_paths_truncated to partial_reasons.", "items": {"type": "object"}},
          ...
          "deployment_overview": {"type": "object", "description": "Deployment summary with counts (including hostname_count and entrypoint_count). It does not repeat the hostnames and entrypoints arrays, which ship once at the top level."},
          ...
          "result_limits": {"type": "object", "description": "Additive drilldown block: bounded limit, deterministic ordering, fan-out counts (instance_count, dependent_count, consumer_count, hostname_count, entrypoint_count, network_path_count), truncation flag, and the next prompt tool plus context path. downstream_read_limit reports the tighter 25-row bound underneath the fan-out lists (versus limit, the 50-row rendering cap); evidence_file_read_limit reports the separate 5,000-file bound on the service repository's own indexed-file read that consumer_repositories_truncated also folds in. When truncated is true because of dependents_truncated or provisioning_source_chains_truncated, downstream_read_limit was exceeded; when it is true only because of consumer_repositories_truncated, evidence_file_read_limit or one of the other bounds documented on that flag may be the one that fired instead; truncated is also true when the infrastructure panel's own read landed past its bound (see limitations' infrastructure_truncated).", "additionalProperties": true},
          "limitations": {"type": "array", "description": "Raw limitation reasons attached directly to the context payload before promotion into partial_reasons. Populated on the primary graph-materialized path (not only the repository read-model fallback) when an auxiliary read degrades or lands past its bound: infrastructure_read_degraded (the graph read failed and infrastructure answered empty rather than propagating), infrastructure_truncated (a healthy read landed past its LIMIT bound; more rows may exist), hostnames_truncated, entrypoints_truncated, and network_paths_truncated (the list was cut to the 50-row cap; result_limits carries the total), relationships_read_degraded (the repository dependencies read failed and dependencies answered empty), api_surface_read_degraded (the API surface read failed), languages_read_degraded and source_tool_breakdown_read_degraded (the tech-fingerprint reads failed and the breakdown is absent rather than empty), or workload_identity_not_materialized (repository read-model fallback with no materialized Workload node).", "items": {"type": "string", "enum": ["infrastructure_read_degraded", "infrastructure_truncated", "hostnames_truncated", "entrypoints_truncated", "network_paths_truncated", "relationships_read_degraded", "api_surface_read_degraded", "languages_read_degraded", "source_tool_breakdown_read_degraded", "workload_identity_not_materialized"]}},
          ...
          "runtime_topology_limits": ` + schema.ImpactRuntimeTopologyLimits + `
        }
      },
      "BrowserSessionAuth": {
        "type": "object",
