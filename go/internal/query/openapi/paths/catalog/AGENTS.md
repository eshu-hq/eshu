# Catalog OpenAPI Fragments — Agent Instructions

Scope: `go/internal/query/openapi/paths/catalog/` (package `catalog`).

## Ownership

This leaf owns the OpenAPI 3.0 path documentation for the
deployment-capability surface: `capabilities.go` (`Capabilities`),
`component_extensions.go` (`ComponentExtensions`), `playbooks.go`
(`Playbooks`), and `surface_inventory.go` (`SurfaceInventory`). Each file
is a JSON string constant only — no handler logic, no HTTP wiring.
...
`docs/internal/naming.md` is law: no `catalog_` file prefixes, no
`catalog/catalog.go`, and exported constants lose the family stutter
(`Capabilities`, not CatalogCapabilities).
