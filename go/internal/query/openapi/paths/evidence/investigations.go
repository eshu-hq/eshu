...
// Investigations is the OpenAPI path fragment documenting the 4
// `/api/v0/investigations/*` routes. openapi.Spec concatenates it into the
              ...
              "application/json": {
                "schema": {
                  ...
                  "properties": {
                    ...
                    "coverage_summary": {"type": "object", "description": "Coverage state and reason plus repository/evidence counts, result_limit (the 50-row repository rendering cap), downstream_read_limit (the tighter bound that actually fires on the cross-repository fan-out, 25 by default), evidence_file_read_limit (the separate 5,000-file bound on the service repository's own indexed-file read), and truncated. truncated is true when the repository list was capped OR when the bounded provisioning-candidate, hostname, or consumer-evidence reads behind dependents, consumer_repositories, and provisioning_source_chains hit their own bounds; when only the consumer_repositories read is why, evidence_file_read_limit rather than downstream_read_limit may be the bound that fired."},
