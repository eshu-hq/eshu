...
// Rest is the OpenAPI path fragment documenting the 6 `/api/v0/impact/*`
...
const Rest = `
    ...
    "/api/v0/impact/explain-dependency-path": {
      "post": {
        ...
        "description": "Finds and explains the shortest path between two entities. The walk is bounded: one shortestPath of at most 8 hops. Scoped tokens receive the same shape filtered to their grant (#5167): both endpoints must be owned by the grant (the same per-class rule as trace-resource-to-code; an endpoint given by name resolves to the first matching node the grant owns) before any shortestPath runs, otherwise the response is the same 404 an unknown endpoint gets; and when any node on the shortest path is not owned, the response carries no path, depth, confidence, or reason, exactly as when no path exists. The 403 remains for browser sessions refused by policy.",
