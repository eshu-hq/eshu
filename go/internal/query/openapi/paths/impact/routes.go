...
// Routes is the OpenAPI fragment for the /api/v0/impact routes.
//
              ...
              "application/json": {
                "schema": {
                  ...
                  "properties": {
                    ...
                    "hostnames": {"type": "array", "description": "Exact hostnames found in service evidence, capped at 50 rows; hostname_limits reports the total before the cut.", "items": {"type": "object"}},
                    "hostname_limits": {"type": "object", "description": "Cap metadata for hostnames: limit, pre-cut total, truncated, and the drilldown_tool to call for the workload context read.", "properties": {"limit": {"type": "integer"}, "total": {"type": "integer"}, "truncated": {"type": "boolean"}, "drilldown_tool": {"type": "string"}}},
                    "entrypoints": {"type": "array", "description": "Public and internal service entrypoints, capped at 50 rows; entrypoint_limits reports the total before the cut.", "items": {"type": "object"}},
                    "entrypoint_limits": {"type": "object", "description": "Cap metadata for entrypoints: limit, pre-cut total, truncated, and the drilldown_tool to call for the workload context read.", "properties": {"limit": {"type": "integer"}, "total": {"type": "integer"}, "truncated": {"type": "boolean"}, "drilldown_tool": {"type": "string"}}},
                    "network_paths": {"type": "array", "description": "Entrypoint-to-runtime paths, one per entrypoint that matches a runtime instance, capped at 50 rows; network_path_limits reports the total before the cut.", "items": {"type": "object"}},
                    "network_path_limits": {"type": "object", "description": "Cap metadata for network_paths: limit, pre-cut total, truncated, and the drilldown_tool to call for the workload context read.", "properties": {"limit": {"type": "integer"}, "total": {"type": "integer"}, "truncated": {"type": "boolean"}, "drilldown_tool": {"type": "string"}}},
...
` + K8sResourceLimits + `
                    ...
                    "deployment_overview": {"type": "object", "description": "Deployment summary with counts, including the full hostname_count, entrypoint_count, and network_path_count. It does not repeat the hostnames and entrypoints arrays."},
                    ...
                    "evidence_boundaries": ` + schema.EvidenceBoundaries + `
                  }
                }
              }
            }
