...
const ResourceAggregate = `
    "/api/v0/infra/resources/count": {
      "get": {
        ...
        "description": "Counts the canonical graph population of the infrastructure labels. Once the infra read model backfill has completed and no repository waits for a repair after a write from an older binary, unscoped reads count content-derived nodes from the Postgres infra_resource_entities table and CloudResource and TerraformStateResource nodes from current-generation fact truth, reading the graph only for the Terraform state projector's TerraformModule and TerraformOutput nodes, and report truth basis hybrid. A category that needs no graph read (category=argocd, cloud, crossplane, helm, or k8s) is served from Postgres alone and reports truth basis content_index; category=cloud reads only CloudResource from current-generation fact truth. Scoped tokens, and every other read (before the backfill completes, or while a repository waits for that repair), read only the graph and report truth basis authoritative_graph.",
    ...
    "/api/v0/infra/resources/inventory": {
      "get": {
        ...
        "description": "Groups the canonical graph population of the infrastructure labels by one dimension. Uses the same serving rules as countInfraResources: unscoped reads after the infra read model backfill are truth basis hybrid, except a category that needs no graph read (category=argocd, cloud, crossplane, helm, or k8s), which is served from Postgres alone and is content_index; category=cloud reads only CloudResource from current-generation fact truth. Scoped reads, and reads before the backfill or while a repository waits for a repair, are authoritative_graph.",
