    ...
    "/api/v0/images/tag-history": {
      "get": {
        ...
        "parameters": [
          ...
          {"name": "cursor", "in": "query", "schema": {"type": "string"}, "description": "Continuation token from a truncated page's next_cursor. Pass it back verbatim and treat it as opaque by convention -- it is not sealed, and nothing here claims an edit is detected. It names a row key, not a row position, so it stays valid when you change limit mid-walk and a key that matches no current row is not an error (paging simply continues after that position). It is bound to the image_ref it was issued for; a malformed, unknown-version (which includes every pre-keyset token) or foreign-image_ref token returns 400 rather than a silently reset or empty page. Mutually exclusive with a non-zero offset."},
              ...
              "application/json": {
                "schema": {
                  ...
                  "properties": {
                    ...
                    "repository_id": {"type": "string"},
                    "tag": {"type": "string"},
                    "grant_filtered": {"type": "boolean", "description": "Present and true only for a scoped caller whose page was bound to its repository grant through ContainerImage-[:BUILT_FROM]->Repository. Such a page is refilled to limit visible rows, so count below limit means the history ended or the per-request read cap was reached, not that rows were withheld from this window. On a cap-reached page (truncated true with count below limit) the shortfall does describe the scanned span, which is a constant 800 raw rows: count 0 there means every one of them was withheld."},
                    "next_cursor": {"type": "string", "description": "Present only when truncated is true. Keyset continuation token naming one row's first_observed_at and uid; pass it back as cursor. Do not parse or synthesize it. On a normal page it names the last row this page returned. On a cap-reached page that returned no rows at all it names the last raw row the scan reached, which for a grant-filtered caller may be a row withheld from that caller -- the one continuation point that is not a row you were shown."}
                  },
