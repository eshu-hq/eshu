    ...
    "/api/v0/images/tag-history": {
      "get": {
        ...
        "parameters": [
          ...
          {"name": "cursor", "in": "query", "schema": {"type": "string"}, "description": "Continuation token from a truncated page's next_cursor. Pass it back verbatim: it is an authenticated-encryption envelope over a row key, so only a token this deployment issued for this image_ref opens, and anything else -- edited, synthesized, truncated, issued for another image_ref, sealed under a key a rotation retired, or issued under different repository grants (another caller's token, or your own from before a grant change) -- returns 400 rather than a silently reset or empty page. There is no row position inside it, so it stays valid when you change limit mid-walk, and a key that matches no current row is not an error (paging simply continues after that position). A grant-filtered caller on a deployment with no sealing key configured gets a 503 naming the variable to set instead. Mutually exclusive with a non-zero offset."},
