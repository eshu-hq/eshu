...
// only grow an existing violation. testutil.MustMapField is shared across
...
func TestOpenAPICodeownersOwnershipDescribesEffectiveOwnerSchema(t *testing.T) {
	...
	paths := testutil.MustMapField(t, spec, "paths")
	ownershipPath := testutil.MustMapField(t, paths, "/api/v0/codeowners/ownership")
	ownershipGet := testutil.MustMapField(t, ownershipPath, "get")
	...
	responses := testutil.MustMapField(t, ownershipGet, "responses")
	...
	effectiveOwnerSchema := testutil.MustMapField(t, testutil.MustMapField(t, schema, "effective_owner"), "properties")
