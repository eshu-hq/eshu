...
const openAPIComponentsWorkloadSession = `      "WorkloadContext": {
        ...
        "properties": {
          ...
          "result_limits": {"type": "object", "description": "Additive drilldown block: bounded limit, deterministic ordering, fan-out counts, truncation flag, and the next prompt tool plus context path. downstream_read_limit reports the bound that actually fires on the downstream lists (25 by default), which is tighter than limit (the 50-row rendering cap); when truncated is true it is downstream_read_limit, not limit, that was exceeded.", "additionalProperties": true},
