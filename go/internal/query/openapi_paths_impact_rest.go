...
const openAPIPathsImpactRest = `
    ...
    "/api/v0/impact/explain-dependency-path": {
      "post": {
        ...
        "description": "Finds and explains the shortest path between two entities. The walk is bounded: one shortestPath of at most 8 hops. Scoped tokens, all-scope bearer tokens included, are refused with a 403, and so is every browser session except a tenant-bound all-scope console session, because the anchors and the infrastructure hops on the path carry no repo_id property, so a repository grant has nothing to bind to. The route stays on the #5167 pending row-filtering ledger.",
