...
const openAPIPathsFreshnessServiceChangedSince = `
    "/api/v0/freshness/services/changed-since": {
      "get": {
        ...
        "description": "Returns a bounded service-scope changed-since delta for one service (#1943). It diffs a prior service materialization generation's evidence snapshot set against the current active generation's set, keyed by a generation-independent service_evidence_key, into per-evidence-family counts for added, updated, unchanged, retired, and superseded keys plus bounded, deterministic sample handles per classification. It reports the ownership (#1943), deployment (#1985), runtime (#1986), dependencies (#1987), docs (#1988), incidents (#1989), and vulnerabilities (#1990) evidence families. Supply since_generation_id to diff from an exact prior service generation. An unknown service_id returns service_not_found; a since reference that matches no service generation returns not_found. A service with no current active generation returns an explicit unavailable diff instead of zero deltas. Counts are exact; only the per-classification samples are capped by sample_limit with a per-classification truncated flag. Scoped tokens, all-scope bearer tokens included, are refused with a 403 in every deployment, and so is every browser session except a tenant-bound all-scope console session, because the service lineage tables carry no column naming the tenant a row belongs to; the route stays on the pending row-filtering ledger until #6475. In local_no_policy, hosted_single_tenant, and an unset mode (which defaults to local_no_policy) the browser-session route policy admits that console session, as it does on the routes of the pending row-filtering ledger; hosted_multi_tenant and any other unrecognized mode refuse it.",
