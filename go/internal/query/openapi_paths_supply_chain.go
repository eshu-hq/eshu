    ...
    "/api/v0/supply-chain/impact/findings": {
      "get": {
        ...
        "parameters": [
          ...
          {"name": "profile", "in": "query", "schema": {"type": "string", "enum": ["precise", "comprehensive"], "default": "precise"}, "description": "Detection profile selector. precise (default) returns only findings backed by exact installed-version anchors resolved by supported matchers such as npm, Maven, Cargo, NuGet, or Swift. comprehensive also returns range-only manifest, SBOM/CPE-derived, malformed range, unsupported ecosystem, and missing-version rows. Each row keeps its truth labels (impact_status, confidence, runtime_reachability) and missing-evidence reasons."},
                    ...
                    "findings": {
                      ...
                      "items": {
                        ...
                        "properties": {
                          ...
                          "detection_profile": {"type": "string", "enum": ["precise", "comprehensive"], "description": "Evidence tier the row meets. precise requires an exact installed-version anchor and ecosystem-aware match, including supported npm, Maven, Cargo, NuGet, and Swift paths. comprehensive covers SBOM/CPE-derived, range-only, malformed, unsupported, or missing-version rows that still have an owned anchor."},
