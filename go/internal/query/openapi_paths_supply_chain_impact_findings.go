                    ...
                    "findings": {
                      ...
                      "items": {
                        ...
                        "properties": {
                          ...
                          "environment_evidence": {"type": "object", "additionalProperties": {"type": "string", "enum": ["deploy_event", "declared"]}, "description": "Per-environment-name corroboration state for entries in environments[] (issue #5426). deploy_event means a ci.deployment_event observed at the deploying run's commit supplied that environment; declared means only the CI-declared workflow job gate did, with no deployment-event corroboration. deploy_event always wins a collision from multiple deployments for the same environment name. Omitted entirely when a finding has no recorded environment evidence, including rows written before #5426 landed, rather than reporting a fabricated value. A declared-only deployment still contributes its environment and its cicd_run_correlation evidence hop; only promotion to runtime_reachability=deployed_image is withheld."},
