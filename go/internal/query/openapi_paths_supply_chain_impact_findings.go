                    ...
                    "findings": {
                      ...
                      "items": {
                        ...
                        "properties": {
                          ...
                          "runtime_context": {
                            "type": "object",
                            "description": "Read-time-resolved runtime context joined from the finding's repository_id at query time (#5746): the workloads, services, deployments, environments, and catalog refs that repository currently maps to. Populated on this findings list route only; the explain route embeds the same finding shape but does not resolve runtime context. truth_basis is always read_time_resolved so callers can distinguish these IDs from baked workload_ids/service_ids/environments. The workload_id/service_id/environment filters resolve only the same current active repository mappings (#5747); stale baked values cannot satisfy them. An empty runtime_context is an honest 'no runtime facts landed yet' (fresh ingest) that self-heals on the next read; it is not an error and not 'never scanned'.",
                            "properties": {
