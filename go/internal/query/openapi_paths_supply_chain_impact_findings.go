                    ...
                    "findings": {
                      ...
                      "items": {
                        ...
                        "properties": {
                          ...
                          "runtime_context": {
                            "type": "object",
                            "description": "Read-time-resolved runtime context joined from the finding's repository_id at query time (#5746): the workloads, services, deployments, environments, and catalog refs that repository currently maps to. truth_basis is always read_time_resolved so callers cannot mistake these IDs for the baked workload_ids/service_ids/environments fields the workload_id/service_id/environment FILTERS read — a workload shown here may not yet match those filters. An empty runtime_context is an honest 'no runtime facts landed yet' (fresh ingest) that self-heals on the next read; it is not an error and not 'never scanned'.",
                            "properties": {
