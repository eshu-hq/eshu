                    ...
                    "findings": {
                      ...
                      "items": {
                        ...
                        "properties": {
                          ...
                          "detection_profile": {"type": "string", "enum": ["precise", "comprehensive"], "description": "Evidence tier the row meets. precise requires an exact installed-version anchor and ecosystem-aware match, including supported npm, PyPI, Maven, Cargo, Pub, NuGet, Swift, and vendor-backed RPM, Debian/dpkg, and Alpine/APK paths. comprehensive covers SBOM/CPE-derived, range-only manifest or OS-package range, malformed, or missing-version rows that still have an owned anchor."},
                          ...
                          "environment_evidence": {"type": "object", "additionalProperties": {"type": "string", "enum": ["deploy_event", "declared"]}, "description": "Per-environment-name corroboration state for entries in environments[] (issue #5426). deploy_event means a ci.deployment_event observed at the deploying run's commit supplied that environment; declared means only the CI-declared workflow job gate did, with no deployment-event corroboration. deploy_event always wins a collision from multiple deployments for the same environment name. The two-value vocabulary is a producer guarantee: the reducer normalizes every value it writes, mapping anything it does not recognize to declared rather than inventing corroboration. Omitted entirely when a finding has no recorded environment evidence, including rows written before #5426 landed, rather than reporting a fabricated value. A declared-only deployment still contributes its environment and its cicd_run_correlation evidence hop; only promotion to runtime_reachability=deployed_image is withheld."},
                          ...
                          "kubernetes_runtime_workload_refs": {"type": "array", "description": "Current, caller-authorized Kubernetes workloads observed through an exact digest RUNS_IMAGE edge whose source_digest equals this finding's subject_digest. Each workload owner winner and edge scope/generation is independently current and authorized. Query-time enrichment (#5834); present only when exact live evidence drives deployment_truth_tier=runtime_confirmed. Unlike repository-level runtime_context.workload_ids, these refs are bound to this finding's digest.", "items": {"type": "object", "properties": {"workload_uid": {"type": "string"}, "cluster_id": {"type": "string"}, "namespace": {"type": "string"}, "name": {"type": "string"}}, "required": ["workload_uid"]}},
                          ...
                          "runtime_context": {
                            ...
                            "properties": {
                              ...
                              "environments": {"type": "array", "items": {"type": "string"}},
                              "environment_evidence": {"type": "object", "additionalProperties": {"type": "string", "enum": ["deploy_event", "declared"]}, "description": "Per-environment corroboration resolved from current accepted cicd_run_correlation facts. deploy_event means a ci.deployment_event observed at the run's commit supplied the environment; declared means the CI-declared workflow gate supplied it without deployment-event corroboration. Missing or unknown producer values normalize to declared, and deploy_event wins when multiple current facts name the same environment."},
                          ...
                          "version_resolution_tier": {"type": "string", "enum": ["runtime_confirmed", "provenance_ci_declared", "declared_ref", "config_only"], "description": "Which deployment-truth tier the judged version/digest for this finding (subject_digest/image_ref/observed_version) was resolved from (issue #5469), reusing the truth.DeploymentTruthTier vocabulary verbatim -- the same tier strings as deployment_truth_tier. Can differ from deployment_truth_tier: deployment_truth_tier reports the strongest tier with ANY deployment evidence, while version_resolution_tier requires that tier's evidence to also carry a concrete, ELIGIBLE version/digest claim. provenance_ci_declared requires ci_declared_artifact_digest or ci_declared_image_ref to be non-empty (a strong-branch CI/CD match); a CI-declared hop that matched only via repository+environment+operational anchor (#5426's weak branch) bakes neither field, so it drops through to config_only here (or is omitted entirely if config_only itself has no eligible claim) even though it still holds deployment_truth_tier at provenance_ci_declared. A provenance_ci_declared claim whose ci_declared_artifact_digest CONTRADICTS subject_digest is never eligible to win either -- it still appears in version_resolution_corroboration, but resolution falls through to the next tier with an eligible claim about the finding's own identity (typically config_only). declared_ref never appears: #5393 (DEPLOYS_REF) has no evidence producer yet. Omitted when the finding has no eligible version/digest evidence at all."},
                          "version_resolution_corroboration": {
                            "type": "array",
                            "description": "Every deployment-truth tier other than the winner that ALSO makes a version/digest claim for this finding (issue #5469), including a tier that was ineligible to win (a CI-declared digest contradicting subject_digest, review finding R1) or whose claim disagrees with the winner -- disagreement is disclosed rather than the entry being dropped, so an operator can see two evidence sources disagree instead of only ever seeing confirmation. A provenance_ci_declared entry's digest_or_version is always the CI-declared deployment's OWN identity (ci_declared_artifact_digest/ci_declared_image_ref), never borrowed from subject_digest, so a genuine same-axis digest disagreement between runtime-observed/config-owned and CI-declared evidence is expressible here, not only a cross-axis digest-vs-version mismatch against config_only. Omitted when no other tier makes a claim.",
                            "items": {
                              "type": "object",
                              "properties": {
