                    ...
                    "drift_findings": {
                      ...
                      "items": {
                        ...
                        "properties": {
                          ...
                          "evidence": {"type": "array", "items": {"type": "object"}, "description": "Reducer evidence atoms explaining the finding (address, drift_kind, config, state, and for removed_from_state findings, prior). Each atom's own scope_id names the repository/state-snapshot scope that atom's data came from, which can differ from the finding's own scope_id -- notably the config atom's scope_id is the config repo's scope, not the state-snapshot scope this endpoint is bound to. A scoped caller only sees an atom's scope_id when it is inside its own grant; an out-of-grant scope_id is removed from the atom (the atom itself, and its other fields, are still returned)."}
                        }
                      }
                    }
                  }
