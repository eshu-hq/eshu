...
func TestPackageRegistryAggregateCountScopedForcesPublicVisibility(t *testing.T) {
	t.Parallel()
	store := &stubPackageRegistryAggregateStore{count: AggregateCount{TotalPackages: 3, ByEcosystem: map[string]int{"npm": 3}}}
...
func TestPackageRegistryAggregateSharedKeyCallerUnaffected(t *testing.T) {
	t.Parallel()
	store := &stubPackageRegistryAggregateStore{count: AggregateCount{TotalPackages: 7, ByEcosystem: map[string]int{"npm": 7}}}
	handler := &Handler{Aggregates: store, Profile: querycontract.ProfileProduction}
	mux := http.NewServeMux()
	handler.Mount(mux)
