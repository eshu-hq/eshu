...
func TestGraphPackageRegistryAggregateStoreCountShapeIsHotPathEligible(t *testing.T) {
	...
	store := NewGraphAggregateStore(graph)
	count, err := store.CountPackageRegistryPackages(context.Background(), AggregateFilter{Ecosystem: "npm"})
	if err != nil {
...
func TestGraphPackageRegistryAggregateStoreRejectsUnsafeDimension(t *testing.T) {
	...
	_, err := store.PackageRegistryPackageInventory(
		context.Background(),
		AggregateFilter{},
		InventoryDimension("normalized_name"),
		100,
		0,
	)
	if err == nil {
