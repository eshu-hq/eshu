...
// production Reader (GraphAggregateStore) is exercised
...
type stubPackageRegistryAggregateStore struct {
	...
	inventoryErr  error
	lastFilter    AggregateFilter
	lastDimension InventoryDimension
	lastLimit     int
	lastOffset    int
	countCalls    int
	invCalls      int
...
func (s *stubPackageRegistryAggregateStore) CountPackageRegistryPackages(
	...
	s.lastFilter = filter
	if s.countErr != nil {
		return AggregateCount{}, s.countErr
	}
	return s.count, nil
}
...
func (s *stubPackageRegistryAggregateStore) PackageRegistryPackageInventory(
	...
	return append([]InventoryRow(nil), s.inventory...), nil
}
...
func TestPackageRegistryAggregateInventoryRejectsOversizedLimit(t *testing.T) {
	...
	handler := &Handler{Aggregates: &stubPackageRegistryAggregateStore{}}
	mux := http.NewServeMux()
	handler.Mount(mux)
