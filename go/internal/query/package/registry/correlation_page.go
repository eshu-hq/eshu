...
// the raw row separate from CorrelationRow lets
...
// CorrelationPage is one bounded page of package ownership,
...
type CorrelationPage struct {
	...
	// scoped_access.go (packageRegistryGateForVisibility and
...
func buildPackageRegistryCorrelationPage(
	...
	rows := make([]CorrelationRow, 0, len(window))
	...
	page := CorrelationPage{Rows: rows, Truncated: truncated, WindowFactCount: len(window)}
