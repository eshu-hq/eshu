...
// CorrelationStore reads reducer-owned package correlations.
//
...
// extra fact beyond Limit internally so CorrelationPage's
...
// CorrelationFilter bounds package correlation reads to one
...
type CorrelationFilter struct {
	...
	// lookahead here: PostgresCorrelationStore fetches one
	// extra fact internally and derives CorrelationPage's
	// Truncated/NextCursorCorrelationID from that raw fetch, never from
	// len(Rows) (#5816 finding on #5461).
	Limit int
}

// CorrelationRow is one durable package ownership, publication,
...
// CorrelationQueryer is the minimal Postgres surface the
...
// CorrelationPage's Truncated and NextCursorCorrelationID
...
func (s PostgresCorrelationStore) ListPackageRegistryCorrelations(
	ctx context.Context,
	...
	if filter.PackageID == "" && filter.RepositoryID == "" && len(filter.PackageIDs) == 0 {
		return CorrelationPage{}, fmt.Errorf("package_id, package_ids, or repository_id is required")
	}
	if filter.Limit <= 0 || filter.Limit > packageRegistryMaxLimit {
		return CorrelationPage{}, fmt.Errorf("limit must be between 1 and %d", packageRegistryMaxLimit)
	}
...
// CorrelationRow through the typed factschema seam matching
...
func decodePackageRegistryCorrelationRow(
	...
	payloadBytes []byte,
) (CorrelationRow, bool, error) {
	var payload map[string]any
	if err := json.Unmarshal(payloadBytes, &payload); err != nil {
		return CorrelationRow{}, false, fmt.Errorf("decode package registry correlation: %w", err)
	}
	...
	case packageOwnershipCorrelationFactKind:
		...
		if err != nil {
			...
			return CorrelationRow{}, false, nil
		}
		return CorrelationRow{
			CorrelationID:          factID,
			RelationshipKind:       decode.DerefString(correlation.RelationshipKind),
			PackageID:              correlation.PackageID,
			VersionID:              decode.DerefString(correlation.VersionID),
			RepositoryID:           decode.DerefString(correlation.RepositoryID),
			RepositoryName:         decode.DerefString(correlation.RepositoryName),
			SourceURL:              decode.DerefString(correlation.SourceURL),
			CandidateRepositoryIDs: correlation.CandidateRepositoryIDs,
			Outcome:                decode.DerefString(correlation.Outcome),
			Reason:                 decode.DerefString(correlation.Reason),
			ProvenanceOnly:         derefBool(correlation.ProvenanceOnly),
	...
	case packageConsumptionCorrelationFactKind:
		...
		return CorrelationRow{
			...
			RelationshipKind: decode.DerefString(correlation.RelationshipKind),
			PackageID:        correlation.PackageID,
			Ecosystem:        decode.DerefString(correlation.Ecosystem),
			PackageName:      decode.DerefString(correlation.PackageName),
			RepositoryID:     decode.DerefString(correlation.RepositoryID),
			RepositoryName:   decode.DerefString(correlation.RepositoryName),
			RelativePath:     decode.DerefString(correlation.RelativePath),
			ManifestSection:  decode.DerefString(correlation.ManifestSection),
			DependencyRange:  decode.DerefString(correlation.DependencyRange),
			Outcome:          decode.DerefString(correlation.Outcome),
			Reason:           decode.DerefString(correlation.Reason),
			ProvenanceOnly:   derefBool(correlation.ProvenanceOnly),
			CanonicalWrites:  packageCorrelationDerefInt(correlation.CanonicalWrites),
	...
	case packagePublicationCorrelationFactKind:
		...
		return CorrelationRow{
			...
			PackageID:              correlation.PackageID,
			VersionID:              decode.DerefString(correlation.VersionID),
			Version:                decode.DerefString(correlation.Version),
			PublishedAt:            decode.DerefString(correlation.PublishedAt),
			RepositoryID:           decode.DerefString(correlation.RepositoryID),
			RepositoryName:         decode.DerefString(correlation.RepositoryName),
			SourceURL:              decode.DerefString(correlation.SourceURL),
			CandidateRepositoryIDs: correlation.CandidateRepositoryIDs,
	...
	default:
		return CorrelationRow{}, false, fmt.Errorf("decode package registry correlation: unrecognized fact kind %q", factKind)
	}
}
