...
func decodePackageRegistryCorrelationRow(
	...
	case packageOwnershipCorrelationFactKind:
		...
		return CorrelationRow{
			...
			RelationshipKind:       derefString(correlation.RelationshipKind),
			PackageID:              correlation.PackageID,
			VersionID:              derefString(correlation.VersionID),
			RepositoryID:           derefString(correlation.RepositoryID),
			RepositoryName:         derefString(correlation.RepositoryName),
			SourceURL:              derefString(correlation.SourceURL),
			CandidateRepositoryIDs: correlation.CandidateRepositoryIDs,
			Outcome:                derefString(correlation.Outcome),
			Reason:                 derefString(correlation.Reason),
	...
	case packageConsumptionCorrelationFactKind:
		...
		return CorrelationRow{
			...
			RelationshipKind: derefString(correlation.RelationshipKind),
			PackageID:        correlation.PackageID,
			Ecosystem:        derefString(correlation.Ecosystem),
			PackageName:      derefString(correlation.PackageName),
			RepositoryID:     derefString(correlation.RepositoryID),
			RepositoryName:   derefString(correlation.RepositoryName),
			RelativePath:     derefString(correlation.RelativePath),
			ManifestSection:  derefString(correlation.ManifestSection),
			DependencyRange:  derefString(correlation.DependencyRange),
			Outcome:          derefString(correlation.Outcome),
			Reason:           derefString(correlation.Reason),
			ProvenanceOnly:   derefBool(correlation.ProvenanceOnly),
			CanonicalWrites:  packageCorrelationDerefInt(correlation.CanonicalWrites),
	...
	case packagePublicationCorrelationFactKind:
		...
		return CorrelationRow{
			...
			PackageID:              correlation.PackageID,
			VersionID:              derefString(correlation.VersionID),
			Version:                derefString(correlation.Version),
			PublishedAt:            derefString(correlation.PublishedAt),
			RepositoryID:           derefString(correlation.RepositoryID),
			RepositoryName:         derefString(correlation.RepositoryName),
			SourceURL:              derefString(correlation.SourceURL),
			CandidateRepositoryIDs: correlation.CandidateRepositoryIDs,
