...
// PostgresCorrelationStore's real "+1 lookahead fetch, then
...
// (correlations_pagination_test.go) and
// candidateFactPackageRegistryCorrelationStore
// (scoped_access_windowfactcount_test.go) are the faithful
...
func (s *recordingPackageRegistryCorrelationStore) ListPackageRegistryCorrelations(
	...
	s.lastFilter = filter
	rows := append([]CorrelationRow(nil), s.rows...)
	truncated := filter.Limit > 0 && len(rows) > filter.Limit
	if truncated {
		rows = rows[:filter.Limit]
	}
	page := CorrelationPage{Rows: rows, Truncated: truncated, WindowFactCount: len(rows)}
...
func TestPackageRegistryListCorrelationsRequiresScopeAndLimit(t *testing.T) {
	...
	handler := &Handler{Correlations: &recordingPackageRegistryCorrelationStore{}}
...
// pre-existing raw querycontract.StringVal/querycontract.BoolVal/querycontract.IntVal/querycontract.StringSliceVal path used to
...
// value here too, matching the raw path's querycontract.StringVal("")/nil-slice fallback
// for an absent key.
...
// behavior versus the pre-#5461 raw querycontract.StringVal path, which had no concept of a
