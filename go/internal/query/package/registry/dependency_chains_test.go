...
type fakeChainCorrelationStore struct {
	consumption []CorrelationRow
	publication []CorrelationRow
	calls       []CorrelationFilter
}

// ListPackageRegistryCorrelations mirrors PostgresCorrelationStore's
...
func (s *fakeChainCorrelationStore) ListPackageRegistryCorrelations(
	...
	if filter.RepositoryID != "" {
		...
		out := make([]CorrelationRow, 0, len(s.consumption))
		...
		page := CorrelationPage{Rows: out, Truncated: truncated, WindowFactCount: len(out)}
	...
	out := make([]CorrelationRow, 0, len(s.publication))
	...
	return CorrelationPage{Rows: out, WindowFactCount: len(out)}, nil
}
