...
// Handler exposes graph-backed package registry identity reads.
type Handler struct {
...
// PackageResult is one package identity materialized from
...
// VersionResult is one package version identity materialized
// from registry facts.
...
// DependencyResult is one package-native dependency edge
...
func (h *Handler) listPackages(w http.ResponseWriter, r *http.Request) {
	...
	name := querycontract.QueryParam(r, "name")
	if packageID == "" && name != "" && ecosystem == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "ecosystem is required when name is provided")
		return
	}
	if packageID == "" && ecosystem == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "package_id or ecosystem is required")
		return
	}
...
func (h *Handler) listVersions(w http.ResponseWriter, r *http.Request) {
	...
	packageID := querycontract.QueryParam(r, "package_id")
	if packageID == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "package_id is required")
		return
	}
	...
	results := make([]VersionResult, 0, len(rows))
	for _, row := range rows {
		results = append(results, VersionResult{
			VersionID:      querycontract.StringVal(row, "version_id"),
			PackageID:      querycontract.StringVal(row, "package_id"),
			Version:        querycontract.StringVal(row, "version"),
			PURL:           querycontract.StringVal(row, "purl"),
			BOMRef:         querycontract.StringVal(row, "bom_ref"),
			PackageManager: querycontract.StringVal(row, "package_manager"),
			PublishedAt:    packageRegistryTimestampVal(row, "published_at"),
			IsYanked:       querycontract.BoolVal(row, "is_yanked"),
			IsUnlisted:     querycontract.BoolVal(row, "is_unlisted"),
			IsDeprecated:   querycontract.BoolVal(row, "is_deprecated"),
			IsRetracted:    querycontract.BoolVal(row, "is_retracted"),
		})
	}
	body := map[string]any{
		"versions":  results,
...
func (h *Handler) unsupported(w http.ResponseWriter, r *http.Request, capability string) bool {
...
func requiredPackageRegistryLimit(w http.ResponseWriter, r *http.Request) (int, bool) {
	...
	if err != nil || limit <= 0 || limit > packageRegistryMaxLimit {
		querycontract.WriteError(w, http.StatusBadRequest, fmt.Sprintf("limit must be between 1 and %d", packageRegistryMaxLimit))
		return 0, false
	}
	return limit, true
}
