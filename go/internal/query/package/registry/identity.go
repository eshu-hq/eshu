...
// IdentityIssue describes a package-registry graph row that
...
func packageRegistryPackageResultFromRow(
	row map[string]any,
) (PackageResult, *IdentityIssue) {
	packageID := strings.TrimSpace(querycontract.StringVal(row, "package_id"))
	if packageID == "" {
		issue := packageRegistryIdentityIssueFromRow(row, packageRegistryPackageIDMissingReason)
		return PackageResult{}, &issue
	}
	return PackageResult{
		PackageID:        packageID,
		Ecosystem:        querycontract.StringVal(row, "ecosystem"),
		Registry:         querycontract.StringVal(row, "registry"),
		Namespace:        querycontract.StringVal(row, "namespace"),
		NormalizedName:   querycontract.StringVal(row, "normalized_name"),
		...
		SourceSpecificID: querycontract.StringVal(row, "source_specific_id"),
		Visibility:       querycontract.StringVal(row, "visibility"),
		SourceConfidence: querycontract.StringVal(row, "source_confidence"),
		VersionCount:     querycontract.IntVal(row, "version_count"),
	}, nil
}
...
func redactPackageRegistryIdentityIssueMetadata(issue *IdentityIssue) {
	issue.Registry = ""
	issue.Namespace = ""
	issue.PURL = ""
	issue.BOMRef = ""
...
func packageRegistryIdentityIssueFromRow(row map[string]any, reason string) IdentityIssue {
