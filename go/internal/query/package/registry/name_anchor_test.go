...
func (s *keyedPackageRegistryCorrelationStore) ListPackageRegistryCorrelations(
	...
	for _, id := range requested {
		if s.grantedPackageIDs[id] {
			rows = append(rows, CorrelationRow{PackageID: id})
		}
	}
	...
	return CorrelationPage{Rows: rows, WindowFactCount: len(rows)}, nil
}
...
func TestPackageRegistryPackagesByNamePreservesEveryGrantedCandidate(t *testing.T) {
	...
	byID := make(map[string]PackageResult, len(body.Packages))
...
func TestPackageRegistryPackagesByNameRedactsIdentityIssueMetadata(t *testing.T) {
	...
	var body struct {
		Packages       []PackageResult `json:"packages"`
		IdentityIssues []IdentityIssue `json:"identity_issues"`
	}
	if err := json.Unmarshal(rec.Body.Bytes(), &body); err != nil {
		t.Fatalf("decode: %v; body = %s", err, rec.Body.String())
	}
...
func (s *crowdingPackageRegistryCorrelationStore) ListPackageRegistryCorrelations(
	...
	if len(filter.PackageIDs) > 0 {
		s.batchCalls++
		rows := make([]CorrelationRow, packageRegistryMaxLimit)
		for i := range rows {
			rows[i] = CorrelationRow{PackageID: s.crowderPackageID}
		}
	...
	if s.grantedPackageIDs[filter.PackageID] {
		return CorrelationPage{
			Rows:            []CorrelationRow{{PackageID: filter.PackageID}},
			WindowFactCount: 1,
		}, nil
	}
