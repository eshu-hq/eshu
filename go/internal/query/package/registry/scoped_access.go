...
func packageRegistryGateForVisibility(
	...
	page, err := correlations.ListPackageRegistryCorrelations(ctx, CorrelationFilter{
		PackageID:            packageID,
		Limit:                1,
...
func packageRegistryAnchorVisibility(ctx context.Context, graph querycontract.GraphQuery, packageID string) (string, error) {
	if graph == nil {
	...
	return querycontract.StringVal(rows[0], "visibility"), nil
}
...
func packageRegistryNameAnchorCandidates(
	...
	for _, row := range rows {
		candidates = append(candidates, packageRegistryNameCandidate{
			...
			Visibility: querycontract.StringVal(row, "visibility"),
		})
	}
...
func packageRegistryVersionAnchorPackageID(ctx context.Context, graph querycontract.GraphQuery, versionID string) (string, error) {
	if graph == nil {
		return "", fmt.Errorf("package registry graph is required")
	}
	...
	return querycontract.StringVal(rows[0], "package_id"), nil
}
...
func writeEmptyPackageRegistryPackagesPage(w http.ResponseWriter, r *http.Request, h *Handler, limit int) {
	body := map[string]any{
...
func writeEmptyPackageRegistryVersionsPage(w http.ResponseWriter, r *http.Request, h *Handler, limit int) {
	body := map[string]any{
...
func writeEmptyPackageRegistryDependenciesPage(w http.ResponseWriter, r *http.Request, h *Handler, limit int) {
	body := map[string]any{
...
) (out AggregateFilter, emptyResult bool) {
