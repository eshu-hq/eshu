...
// packageRegistryScopedGraphFake is a querycontract.GraphQuery test double for the scoped
...
func (f *packageRegistryScopedGraphFake) Run(_ context.Context, cypher string, params map[string]any) ([]map[string]any, error) {
	...
	case packageRegistryAnchorVisibilityCypher:
		visibility, ok := f.visibilityByPackageID[querycontract.StringVal(params, "package_id")]
		if !ok {
			return nil, nil
		}
		return []map[string]any{{"visibility": visibility}}, nil
	case packageRegistryVersionAnchorPackageIDCypher:
		packageID, ok := f.packageIDByVersionID[querycontract.StringVal(params, "version_id")]
		if !ok {
			return nil, nil
		}
		return []map[string]any{{"package_id": packageID}}, nil
	case packageRegistryNameAnchorVisibilityCypher:
		rows, ok := f.nameAnchorByKey[querycontract.StringVal(params, "ecosystem")+"\x00"+querycontract.StringVal(params, "name")]
		if !ok {
			return nil, nil
		}
		return rows, nil
	default:
		...
		if strings.HasPrefix(cypher, "MATCH (p:Package {uid: $package_id})\n") {
			packageID := querycontract.StringVal(params, "package_id")
			filtered := make([]map[string]any, 0, 1)
			for _, row := range f.rows {
				if querycontract.StringVal(row, "package_id") == packageID {
...
func TestPackageRegistryPackagesScopedVisibilityAndGrant(t *testing.T) {
	...
	t.Run("public row visible and redacted for any scoped caller", func(t *testing.T) {
		...
		handler := &Handler{Neo4j: graph, Correlations: &fatalOnCallPackageRegistryCorrelationStore{t: t}, Profile: querycontract.ProfileProduction}
	...
	t.Run("private row visible without redaction for the granted tenant", func(t *testing.T) {
		...
		correlations := &recordingPackageRegistryCorrelationStore{
			rows: []CorrelationRow{{CorrelationID: "corr-a", PackageID: "pkg:npm:private-lib", RepositoryID: "repo-a"}},
		}
		handler := &Handler{Neo4j: graph, Correlations: correlations, Profile: querycontract.ProfileProduction}
		mux := http.NewServeMux()
		handler.Mount(mux)
