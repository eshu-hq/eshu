...
func packageRegistryPackagesGate(
	...
	if h.Neo4j == nil {
		querycontract.WriteContractError(
			...
			h.profile(),
			querycontract.RequiredProfile(packageRegistryPackagesCapability),
		)
		return result, true
	}
	if !access.Scoped() {
	...
	case name != "":
		...
		// PackageResult, so it has no package_id to look up in
...
func packageRegistryVersionsGate(
	...
	if h.Neo4j == nil {
		querycontract.WriteContractError(
			...
			h.profile(),
			querycontract.RequiredProfile(packageRegistryVersionsCapability),
		)
		return true
	}
	if !access.Scoped() {
...
func packageRegistryDependenciesGate(
	...
	if h.Neo4j == nil {
		querycontract.WriteContractError(
			...
			h.profile(),
			querycontract.RequiredProfile(packageRegistryDependenciesCapability),
		)
		return true
	}
	if !access.Scoped() {
