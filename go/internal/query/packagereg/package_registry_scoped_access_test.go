...
func TestPackageRegistryPackagesScopedVisibilityAndGrant(t *testing.T) {
	...
	t.Run("public row visible and redacted for any scoped caller", func(t *testing.T) {
		...
		handler := &PackageRegistryHandler{Neo4j: graph, Correlations: &fatalOnCallPackageRegistryCorrelationStore{t: t}, Profile: querycontract.ProfileProduction}
	...
	t.Run("private row visible without redaction for the granted tenant", func(t *testing.T) {
		...
		handler := &PackageRegistryHandler{Neo4j: graph, Correlations: correlations, Profile: querycontract.ProfileProduction}
		mux := http.NewServeMux()
		handler.Mount(mux)
