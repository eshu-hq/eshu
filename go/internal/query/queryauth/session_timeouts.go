...
// session's lifetime or block login -- the same fail-open convention root
// package query's requireSSODecision uses for the require_sso gate.
//
// Moved from root package query's session_timeout_policy.go
// (resolveSessionTimeouts, #6642) so a handler-family subpackage can resolve
...
func ResolveSessionTimeouts(
	...
	// absolute expiry always fires first); clamp rather than reject,
	// mirroring root's issueBrowserSessionWithExternalAuth's existing
	// idleExpiresAt-after-absoluteExpiresAt clamp.
	if idle > absolute {
		idle = absolute
	}
	return idle, absolute
