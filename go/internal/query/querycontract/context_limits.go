...
// Context result-limits and partial-reason shaping shared by the query root
// and the handler-family subpackages (#6060 lane B5). The entity family
// moved out of package query, and a family subpackage cannot import the
// root package back without an import cycle, because root names family
// symbols in its compatibility aliases. These helpers carry no dependency
// on anything beyond the standard library, so they move without behavior
// change. Package query keeps forwarding wrappers under the original
// names, so its own callers compile unchanged.

// ContextPartialReasons promotes the context payload's limitations into an
// explicit partial_reasons array. Callers see missing or unsupported
// evidence directly instead of inferring completeness from an absent field.
// The result is always non-nil so the envelope shape is stable across
...
// WorkloadContextResultLimits builds the shared result_limits drilldown
// block for a workload context or story payload. The surface argument
// selects whether the drilldown handle points callers at the workload story
// or back at the workload context route.
func WorkloadContextResultLimits(ctx map[string]any, workloadID, surface string) map[string]any {
	...
	// (service/story_dossier.go) already ORs the same flags into the service
