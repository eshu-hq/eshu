...
sibling leaves `querycontract/answer` (its packets name
`EvidenceCitationHandle`) and `querycontract/visualization` (its nodes carry
citation handles). The parent `querycontract` does not import it.
