...
// NewVisualizationBuilder starts an empty builder for view, titled title.
...
// Empty reports whether no node or edge has been added yet. A graph-query
// caller uses this to detect a Cypher result with no renderable graph
// entities before paying for Finalize's sort and truncation pass.
func (b *VisualizationBuilder) Empty() bool {
	return len(b.nodes) == 0 && len(b.edges) == 0
}

// EdgeCount reports how many edges have been added so far.
func (b *VisualizationBuilder) EdgeCount() int {
	return len(b.edges)
}
...
func (b *VisualizationBuilder) Finalize() VisualizationPacket {
	...
	if truncation.Truncated {
		packet.Limitations = appendVisualizationReason(packet.Limitations,
...
func appendVisualizationReason(reasons []string, reason string) []string {
