# querytestutil/content — agent instructions
...
only in a test binary — root's own tests import `querytestutil`, so the cycle
surfaces when `internal/query`'s tests build. `go build
./internal/query/querytestutil/content` still succeeds on its own, so a green
