...
// ReaderQueryContainsInOrder reports whether query contains every
...
// queued result, and it is the same check ReaderQueryResult's
// QueryContainsInOrder runs.
func ReaderQueryContainsInOrder(query string, fragments []string) error {
	offset := 0
...
// ReaderCheckArgs asserts args carries exactly the values in want, in
...
func ReaderCheckArgs(args []driver.NamedValue, want []driver.Value) error {
	if want == nil {
		return nil
	}
