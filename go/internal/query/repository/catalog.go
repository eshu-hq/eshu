...
func (h *RepositoryHandler) listCatalog(w http.ResponseWriter, r *http.Request) {
	...
	if h == nil {
		response.finish()
		querycontract.WriteSuccess(w, r, http.StatusOK, response, catalogTruth(querycontract.ProfileProduction, querycontract.TruthBasisHybrid))
		return
	}
	...
	if h.Neo4j == nil {
		...
		response.finish()
		querycontract.WriteSuccess(w, r, http.StatusOK, response, catalogTruth(h.profile(), querycontract.TruthBasisContentIndex))
		return
	}
	...
	if err != nil {
		...
		querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("query workloads: %v", err))
		return
	}
...
func (h *RepositoryHandler) enrichCatalogWorkloadsFromCorrelations(
	...
	rows, err := h.ServiceCatalogCorrelations.ListServiceCatalogCorrelations(ctx, querycontract.ServiceCatalogCorrelationFilter{
	...
	byRepo := make(map[string]querycontract.ServiceCatalogCorrelationRow, len(rows))
	for _, row := range rows {
		if row.RepositoryID != "" {
...
func catalogLimitFromRequest(r *http.Request) int {
	limit := querycontract.QueryParamInt(r, "limit", defaultCatalogLimit)
	if limit <= 0 {
...
func catalogTruth(profile querycontract.QueryProfile, basis querycontract.TruthBasis) *querycontract.TruthEnvelope {
