...
// CatalogWorkloadRepoCypher is the catalog workload query text. Exported for #6060 so the
// cross-family queryplan binding test can name it from outside this package.
...
// CatalogWorkloadInstanceEnvironmentCypher is the catalog workload query text. Exported for #6060 so the
// cross-family queryplan binding test can name it from outside this package.
...
// keeps the result rows identical while cold-planning in single-digit
// milliseconds.
// CatalogWorkloadEvidenceEnvironmentCypher is the catalog workload query text. Exported for #6060 so the
// cross-family queryplan binding test can name it from outside this package.
...
func (h *RepositoryHandler) assembleCatalogWorkloadsFromGraph(
	...
	for _, row := range baseRows {
		if id := querycontract.StringVal(row, "id"); id != "" {
			ids = append(ids, id)
		}
	}
	...
	for _, row := range baseRows {
		...
		workload := catalogWorkload{
			...
			Kind: normalizedCatalogWorkloadKind(querycontract.StringVal(row, "kind")),
		}
...
func (h *RepositoryHandler) catalogWorkloadEnrichments(
	...
	boundedIDs := querycontract.UniqueSortedStrings(ids)
	if len(boundedIDs) == 0 {
	...
	for _, row := range repoRows {
		...
		if entry.repoID == "" {
			entry.repoID = querycontract.StringVal(row, "repo_id")
			entry.repoName = querycontract.StringVal(row, "repo_name")
		}
	}
	...
	for _, row := range instanceRows {
		...
		entry := catalogWorkloadEnrichmentFor(enrichments, id)
		entry.instanceCount = querycontract.IntVal(row, "instance_count")
		entry.instanceEnvs = querycontract.StringSliceVal(row, "environments")
	}
...
const catalogWorkloadRepoCypher = CatalogWorkloadRepoCypher

const catalogWorkloadInstanceEnvironmentCypher = CatalogWorkloadInstanceEnvironmentCypher

const catalogWorkloadEvidenceEnvironmentCypher = CatalogWorkloadEvidenceEnvironmentCypher
