...
func buildOverviewTopologyStory(deliveryPaths []map[string]any, sharedConfigPaths []map[string]any) []string {
	...
	for _, row := range deliveryPaths {
		switch querycontract.StringVal(row, "kind") {
		case "controller_artifact":
			...
			if entryPoints := querycontract.StringSliceValue(row, "entry_points"); len(entryPoints) > 0 {
				details = append(details, "entry points "+strings.Join(entryPoints, ", "))
			}
			if sharedLibraries := querycontract.StringSliceValue(row, "shared_libraries"); len(sharedLibraries) > 0 {
				details = append(details, "shared libraries "+strings.Join(sharedLibraries, ", "))
			}
			if pipelineCalls := querycontract.StringSliceValue(row, "pipeline_calls"); len(pipelineCalls) > 0 {
			...
			if hints := querycontract.MapSliceValue(row, "ansible_playbook_hints"); len(hints) > 0 {
				...
				for _, hint := range hints {
					if playbook := strings.TrimSpace(querycontract.StringVal(hint, "playbook")); playbook != "" {
			...
			if inventories := querycontract.StringSliceValue(row, "ansible_inventories"); len(inventories) > 0 {
				details = append(details, "ansible inventories "+strings.Join(inventories, ", "))
			}
			if varFiles := querycontract.StringSliceValue(row, "ansible_var_files"); len(varFiles) > 0 {
				details = append(details, "ansible vars "+strings.Join(varFiles, ", "))
			}
			if taskEntrypoints := querycontract.StringSliceValue(row, "ansible_task_entrypoints"); len(taskEntrypoints) > 0 {
		...
		case "runtime_artifact":
			...
			artifactName := strings.TrimSpace(querycontract.StringVal(row, "artifact_name"))
			serviceName := strings.TrimSpace(querycontract.StringVal(row, "service_name"))
			baseImage := strings.TrimSpace(querycontract.StringVal(row, "base_image"))
			cmd := strings.TrimSpace(querycontract.StringVal(row, "cmd"))
			buildContext := strings.TrimSpace(querycontract.StringVal(row, "build_context"))
			envFiles := querycontract.StringSliceValue(row, "env_files")
			configs := querycontract.StringSliceValue(row, "configs")
			secrets := querycontract.StringSliceValue(row, "secrets")
			signals := querycontract.StringSliceValue(row, "signals")
			...
			if len(runtimeDetails) > 0 {
				line += " with " + querycontract.JoinSentenceFragments(runtimeDetails)
			}
			if len(signals) > 0 {
		...
		case "workflow_artifact":
			path := strings.TrimSpace(querycontract.StringVal(row, "path"))
			artifactType := strings.TrimSpace(querycontract.StringVal(row, "artifact_type"))
			workflowName := strings.TrimSpace(querycontract.StringVal(row, "workflow_name"))
			if path == "" || artifactType == "" {
				continue
			}
			...
			if triggerEvents := querycontract.StringSliceValue(row, "trigger_events"); len(triggerEvents) > 0 {
				line += fmt.Sprintf(" triggered by %s", strings.Join(triggerEvents, ", "))
			}
			if workflowInputs := querycontract.StringSliceValue(row, "workflow_inputs"); len(workflowInputs) > 0 {
				line += fmt.Sprintf(" with workflow inputs %s", strings.Join(workflowInputs, ", "))
			}
			hasGovernance := false
			if permissionScopes := querycontract.StringSliceValue(row, "permission_scopes"); len(permissionScopes) > 0 {
				line += fmt.Sprintf(" with permissions %s", strings.Join(permissionScopes, ", "))
				hasGovernance = true
			}
			if concurrencyGroups := querycontract.StringSliceValue(row, "concurrency_groups"); len(concurrencyGroups) > 0 {
				if hasGovernance {
			...
			if environments := querycontract.StringSliceValue(row, "environments"); len(environments) > 0 {
				if hasGovernance {
			...
			if jobTimeouts := querycontract.StringSliceValue(row, "job_timeout_minutes"); len(jobTimeouts) > 0 {
				if hasGovernance {
			...
			if matrixKeys := querycontract.StringSliceValue(row, "matrix_keys"); len(matrixKeys) > 0 {
			...
			if deliveryLocalPaths := querycontract.StringSliceValue(row, "delivery_local_paths"); len(deliveryLocalPaths) > 0 {
				line += fmt.Sprintf(" using local paths %s", strings.Join(deliveryLocalPaths, ", "))
			}
			if deliveryFamilies := querycontract.StringSliceValue(row, "delivery_command_families"); len(deliveryFamilies) > 0 {
				line += fmt.Sprintf(" and delivery families %s", strings.Join(deliveryFamilies, ", "))
			}
			if gatingConditions := querycontract.StringSliceValue(row, "gating_conditions"); len(gatingConditions) > 0 {
				line += fmt.Sprintf(", gating conditions %s", strings.Join(gatingConditions, "; "))
			}
			if needsDependencies := querycontract.StringSliceValue(row, "needs_dependencies"); len(needsDependencies) > 0 {
				line += fmt.Sprintf(", and needs %s", strings.Join(needsDependencies, ", "))
			}
			if localWorkflowPaths := querycontract.StringSliceValue(row, "local_reusable_workflow_paths"); len(localWorkflowPaths) > 0 {
				line += fmt.Sprintf(" via local reusable workflow paths %s", strings.Join(localWorkflowPaths, ", "))
			}
			if reusableWorkflows := querycontract.StringSliceValue(row, "reusable_workflow_repositories"); len(reusableWorkflows) > 0 {
			...
			if checkoutRepos := querycontract.StringSliceValue(row, "checkout_repositories"); len(checkoutRepos) > 0 {
			...
			if actionRepos := querycontract.StringSliceValue(row, "action_repositories"); len(actionRepos) > 0 {
				switch {
			...
			if workflowInputRepos := querycontract.StringSliceValue(row, "workflow_input_repositories"); len(workflowInputRepos) > 0 {
				if strings.Contains(line, " via reusable workflow repos ") ||
					strings.Contains(line, " via checkout repos ") ||
					strings.Contains(line, " and checkout repos ") ||
	...
	if len(sharedConfigPaths) > 0 {
		...
		for _, row := range sharedConfigPaths {
			path := strings.TrimSpace(querycontract.StringVal(row, "path"))
			sourceRepos := querycontract.StringSliceValue(row, "source_repositories")
		...
		if len(families) > 0 {
			story = append(story, "Shared config families span "+querycontract.JoinSentenceFragments(families)+".")
		}
	}
...
func buildOverviewDeliveryFamilyStory(deliveryFamilyPaths []map[string]any) []string {
	...
	for _, row := range deliveryFamilyPaths {
		switch querycontract.StringVal(row, "family") {
		case "cloudformation":
			path := strings.TrimSpace(querycontract.StringVal(row, "path"))
			artifactType := strings.TrimSpace(querycontract.StringVal(row, "artifact_type"))
			if path == "" || artifactType == "" {
		...
		case "docker_compose":
			...
			story = append(story, line)
		case "gitops":
			relType := strings.TrimSpace(querycontract.StringVal(row, "type"))
			targetName := strings.TrimSpace(querycontract.StringVal(row, "target_name"))
