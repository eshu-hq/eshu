...
func BuildRepositoryInfrastructureOverview(
	...
	for _, item := range infrastructure {
		entityType := querycontract.StringVal(item, "type")
		if entityType == "" {
			continue
		}
...
// buildRepositoryInfrastructureOverview keeps the in-package spelling after
// the #6060 export; root stayers name BuildRepositoryInfrastructureOverview.
func buildRepositoryInfrastructureOverview(infrastructure []map[string]any, files []querycontract.FileContent) map[string]any {
	return BuildRepositoryInfrastructureOverview(infrastructure, files)
}
