...
func noteRepositoryFrameworkSignal(
	...
	relativePath = artifacts.CleanRepositoryRelativePath(relativePath)
...
func isRepositoryNarrativeCandidate(relativePath string) bool {
	lowerPath := strings.ToLower(artifacts.CleanRepositoryRelativePath(relativePath))
	if lowerPath == "" {
		return false
	}
	...
	if querycontract.IsServiceEvidenceCandidate(querycontract.FileContent{RelativePath: lowerPath}, "") {
		return true
	}
