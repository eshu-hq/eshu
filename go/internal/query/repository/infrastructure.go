...
func QueryRepoInfrastructureFromContent(ctx context.Context, content querycontract.ContentStore, repoID string) ([]map[string]any, bool) {
	...
	sort.SliceStable(result, func(i, j int) bool {
		leftType := querycontract.StringVal(result[i], "type")
		rightType := querycontract.StringVal(result[j], "type")
		if leftType != rightType {
			return leftType < rightType
		}
		leftName := querycontract.StringVal(result[i], "name")
		rightName := querycontract.StringVal(result[j], "name")
		if leftName != rightName {
			return leftName < rightName
		}
		return querycontract.StringVal(result[i], "file_path") < querycontract.StringVal(result[j], "file_path")
	})
...
func QueryRepoInfrastructureFromGraph(ctx context.Context, reader querycontract.GraphQuery, params map[string]any) ([]map[string]any, bool, error) {
	...
	for _, row := range rows {
		...
		if !isRepositoryInfrastructureType(querycontract.StringVal(entry, "type")) {
			continue
		}
		result = append(result, entry)
	}
...
func repositoryInfrastructureEntryFromRow(row map[string]any) map[string]any {
	...
	if kind := querycontract.StringVal(row, "kind"); kind != "" {
		entry["kind"] = kind
	}
	if source := querycontract.StringVal(row, "source"); source != "" {
		entry["source"] = source
	}
	if terraformSource := querycontract.StringVal(row, "terraform_source"); terraformSource != "" {
		entry["terraform_source"] = terraformSource
	}
	if configPath := querycontract.StringVal(row, "config_path"); configPath != "" {
...
func repositoryInfrastructureEntryFromContent(entity querycontract.EntityContent) (map[string]any, bool) {
	entry := map[string]any{
	...
	case "TerraformModule":
		...
		if deploymentName, ok := querycontract.MetadataNonEmptyString(entity.Metadata, "deployment_name"); ok {
			entry["deployment_name"] = deploymentName
		}
	case "TerragruntConfig":
		if terraformSource, ok := querycontract.MetadataNonEmptyString(entity.Metadata, "terraform_source"); ok {
			entry["terraform_source"] = terraformSource
			entry["source"] = terraformSource
		}
		if includes := querycontract.MetadataStringSlice(entity.Metadata, "includes"); len(includes) > 0 {
			entry["includes"] = includes
		}
		if inputs := querycontract.MetadataStringSlice(entity.Metadata, "inputs"); len(inputs) > 0 {
			entry["inputs"] = inputs
		}
		if locals := querycontract.MetadataStringSlice(entity.Metadata, "locals"); len(locals) > 0 {
			entry["locals"] = locals
		}
	...
	case "ArgoCDApplication", "ArgoCDApplicationSet", "KustomizeOverlay", "HelmChart",
		...
		if kind, ok := querycontract.MetadataNonEmptyString(entity.Metadata, "kind"); ok {
			entry["kind"] = kind
		}
...
func copyInfrastructureClassification(entry map[string]any, source map[string]any) {
	for _, key := range []string{"provider", "resource_type", "resource_service", "resource_category"} {
		if value := querycontract.StringVal(source, key); value != "" {
			entry[key] = value
		}
	}
}
...
func repositoryInfrastructureEntryKey(entry map[string]any) string {
	return querycontract.StringVal(entry, "type") + "|" + querycontract.StringVal(entry, "name") + "|" + querycontract.StringVal(entry, "file_path")
}

// queryRepoInfrastructureFromContent and queryRepoInfrastructureFromGraph keep
// the in-package spelling after the #6060 export; root stayers name the
// exported spelling.
func queryRepoInfrastructureFromContent(ctx context.Context, content querycontract.ContentStore, repoID string) ([]map[string]any, bool) {
	return QueryRepoInfrastructureFromContent(ctx, content, repoID)
}
