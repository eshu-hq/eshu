...
const RefPageDefaultLimit = 100

// RefPageMaxLimit is the largest limit a caller may request.
const RefPageMaxLimit = 500

// RefPageCursorVersion is the only cursor payload version this
...
const RefPageCursorVersion = 2

// RefPageCursor is the forward-only keyset cursor for the combined
...
// empty or reset page.
func DecodeRepositoryRefPageCursor(raw, repoID string) (RefPageCursor, error) {
	var cursor RefPageCursor
	...
	if cursor.Version != RefPageCursorVersion {
...
// RefKeyLess reports whether key a sorts strictly before key b
...
func RefKeyLess(a, b RefPageCursor) bool {
	if a.Kind != b.Kind {
...
// RefPageWindow returns the bounded page of refs strictly after
...
// RefWindowEntries splits one paged window of refs into branch and
...
func RefWindowEntries(window []querycontract.RepositoryRef) (branches, tags []map[string]any) {
	...
	for _, ref := range window {
		switch ref.Kind {
		case "tag":
			tags = append(tags, RefEntry(ref, false))
		default:
			branches = append(branches, RefEntry(ref, true))
		}
	}
