...
func queryRepositoryContextCount(ctx context.Context, reader querycontract.GraphQuery, params map[string]any, fallbackKey string, cypher string, fallback map[string]any) (int, error) {
	rows, err := reader.Run(ctx, cypher, params)
	if err != nil {
		return 0, err
	}
	if len(rows) == 0 {
		return querycontract.IntVal(fallback, fallbackKey), nil
	}
...
func repositoryLanguageNamesFromCoverage(contentCoverage *querycontract.RepositoryContentCoverage) ([]string, bool) {
	if contentCoverage == nil || !contentCoverage.Available {
		return nil, false
	}
