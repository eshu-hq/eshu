...
func TestBuildRepositoryDeploymentOverviewIncludesComposeRuntimeLinkageMetadata(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "env_files"), []string{".env", "deploy/api.env"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("delivery_paths[0].env_files = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "configs"), []string{"app-config", "api-runtime"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("delivery_paths[0].configs = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "secrets"), []string{"db-password", "api-token"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
...
func TestBuildRepositoryDeploymentOverviewIncludesWorkflowInputRepositoriesInDeliveryPaths(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "workflow_input_repositories"), []string{"example-org/automation-fallback", "example-org/shared-automation"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
...
func TestBuildRepositoryDeploymentOverviewIncludesCheckoutRepositoriesInDeliveryPaths(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "checkout_repositories"), []string{"example-org/deployment-helm", "example-org/deployment-kustomize"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
...
func TestBuildRepositoryDeploymentOverviewIncludesActionRepositoriesInDeliveryPaths(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "action_repositories"), []string{"hashicorp/setup-terraform", "peter-evans/create-pull-request"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
...
func TestBuildRepositoryDeploymentOverviewIncludesWorkflowTriggerAndMatrixMetadata(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "trigger_events"), []string{"push", "workflow_dispatch"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("delivery_paths[0].trigger_events = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "workflow_inputs"), []string{"deploy_enabled"}; len(got) != len(want) || got[0] != want[0] {
		t.Fatalf("delivery_paths[0].workflow_inputs = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "matrix_keys"), []string{"region", "runtime"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
...
func TestBuildRepositoryDeploymentOverviewIncludesWorkflowGovernanceMetadataInDeliveryPaths(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "permission_scopes"), []string{"contents:read", "deployments:write", "id-token:write"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] || got[2] != want[2] {
		t.Fatalf("delivery_paths[0].permission_scopes = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "concurrency_groups"), []string{"deploy-${{ github.ref }}", "deploy-production"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
		t.Fatalf("delivery_paths[0].concurrency_groups = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "environments"), []string{"production"}; len(got) != len(want) || got[0] != want[0] {
		t.Fatalf("delivery_paths[0].environments = %#v, want %#v", got, want)
	}
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "job_timeout_minutes"), []string{"deploy:30"}; len(got) != len(want) || got[0] != want[0] {
...
func TestBuildRepositoryDeploymentOverviewIncludesLocalWorkflowCallPaths(t *testing.T) {
	...
	if got, want := querycontract.StringSliceVal(deliveryPaths[0], "local_reusable_workflow_paths"), []string{".github/workflows/release.yaml", ".github/workflows/verify.yaml"}; len(got) != len(want) || got[0] != want[0] || got[1] != want[1] {
