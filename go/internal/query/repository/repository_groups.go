...
func deriveRepositoryGroupEvidence(repo map[string]any) repositoryGroupEvidence {
	if querycontract.BoolVal(repo, "is_dependency") {
	...
	if namespace := repositorySlugNamespace(querycontract.StringVal(repo, "repo_slug")); namespace != "" {
		return repositoryGroupEvidence{
	...
	if owner := repositoryRemoteOwner(querycontract.StringVal(repo, "remote_url")); owner != "" {
		return repositoryGroupEvidence{
...
func repositoryGroupEvidenceMissing(repos []map[string]any) bool {
	for _, repo := range repos {
		if querycontract.StringVal(repo, "group_source") == repositoryGroupSourceMissing {
			return true
		}
	}
	return false
