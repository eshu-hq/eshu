...
func TestQueryRepoInfrastructureFiltersNonInfrastructureGraphRows(t *testing.T) {
	...
	if got, want := querycontract.StringVal(got[0], "type"), "K8sResource"; got != want {
		t.Fatalf("type = %q, want %q", got, want)
	}
	if got, want := querycontract.StringVal(got[0], "name"), "api"; got != want {
...
func TestQueryRepoInfrastructureUsesContentRowsBeforeGraph(t *testing.T) {
	...
	if got, want := querycontract.StringVal(got[0], "kind"), "Deployment"; got != want {
...
func (s *limitCapturingContentStore) ListRepoEntitiesByTypes(_ context.Context, _ string, _ []string, limit int) ([]querycontract.EntityContent, error) {
...
func TestQueryRepoInfrastructureFromContentSignalsTruncationAtLimit(t *testing.T) {
	...
	entities := make([]querycontract.EntityContent, repositoryInfrastructureEntityLimit+1)
...
func TestQueryRepoInfrastructureFromContentNoTruncationAtLimit(t *testing.T) {
	...
	entities := make([]querycontract.EntityContent, repositoryInfrastructureEntityLimit)
	for i := range entities {
		entities[i] = querycontract.EntityContent{
			EntityType:   "K8sResource",
			EntityName:   fmt.Sprintf("res-%d", i),
...
func TestQueryRepoInfrastructureFromContentIgnoresNonInfrastructureEntityCount(t *testing.T) {
	...
	entities := make([]querycontract.EntityContent, 0, nonInfraCount+3)
	...
	for _, entry := range got {
		if got, want := querycontract.StringVal(entry, "type"), "TerraformResource"; got != want {
			t.Fatalf("type = %q, want %q", got, want)
		}
	}
}
