...
// StoryRowsTruncatedReason marks a story whose row set hit its LIMIT bound.
// Exported for #6060 so root tests can name it from outside this package.
...
func queryRepositoryStoryStringRows(
	...
	if len(rows) == 0 {
		return querycontract.StringSliceVal(fallback, fallbackKey), false, nil
	}
	...
	for _, row := range rows {
		value := querycontract.StringVal(row, valueKey)
		if value == "" {
			continue
		}
...
const storyRowsTruncatedReason = StoryRowsTruncatedReason
