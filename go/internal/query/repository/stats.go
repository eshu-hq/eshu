...
func (h *Handler) repositoryStatsContentCoverage(
	...
	if h == nil || h.Content == nil {
		return querycontract.RepositoryContentCoverage{}, nil
	}
	coverage, err := h.Content.RepositoryCoverage(ctx, repoID)
	if err != nil {
		return querycontract.RepositoryContentCoverage{}, err
	}
...
func repositoryStatsTruthBasis(coverageMap map[string]any) querycontract.TruthBasis {
	if querycontract.StringVal(coverageMap, "source_backend") == "content_store" {
		return querycontract.TruthBasisContentIndex
	}
	return querycontract.TruthBasisHybrid
}
...
func repositoryStatsCoverageLogAttrs(coverageMap map[string]any, coverageErr error) []slog.Attr {
	return []slog.Attr{
		...
		slog.String("source_backend", querycontract.StringVal(coverageMap, "source_backend")),
		slog.String("query_shape", querycontract.StringVal(coverageMap, "query_shape")),
		slog.Bool("counts_available", querycontract.BoolVal(coverageMap, "counts_available")),
		slog.Bool("entity_types_available", querycontract.BoolVal(coverageMap, "entity_types_available")),
		slog.Bool("whole_graph_traversal", querycontract.BoolVal(coverageMap, "whole_graph_traversal")),
		slog.Bool("partial_results", querycontract.BoolVal(coverageMap, "partial_results")),
		slog.Bool("truncated", querycontract.BoolVal(coverageMap, "truncated")),
		slog.Bool("timeout", querycontract.BoolVal(coverageMap, "timeout")),
		slog.Any("missing_evidence", coverageMap["missing_evidence"]),
	}
}

func repositoryStatsLanguageNames(languages []querycontract.RepositoryLanguageCount) []string {
...
func repositoryStatsEntityTypeNames(entityTypes []querycontract.RepositoryEntityTypeCount) []string {
...
func repositoryStatsCoverageHasEvidence(coverage querycontract.RepositoryContentCoverage) bool {
