...
type repoFileLanguageLister interface {
	ListRepoFilesByLanguage(ctx context.Context, repoID string, languages []string, pathPrefix string, limit int) ([]querycontract.FileContent, error)
...
func (h *Handler) getRepositoryTree(w http.ResponseWriter, r *http.Request) {
	...
	if lister, ok := h.Content.(repoFileLanguageLister); ok && len(languageList) > 0 {
		...
		if ctxErr != nil {
			querycontract.WriteError(w, http.StatusInternalServerError, fmt.Sprintf("list repository files failed: %v", ctxErr))
			return
		}
		indexedRef = ref
	...
	if status, message, err := readmodel.ValidateSelectedRepositoryRef(ctx, h.Content, repoID, requestedRef, indexedRef); err != nil {
	...
	if requestPath != "" && !matched {
		querycontract.WriteError(w, http.StatusNotFound, "path not found")
		return
	}
...
func repositoryTreeRef(files []querycontract.FileContent) string {
...
func repositoryTreeFileEntry(file querycontract.FileContent, name, relativePath string) map[string]any {
...
func sortRepositoryTreeEntries(entries []map[string]any) {
	sort.SliceStable(entries, func(i, j int) bool {
		...
		return querycontract.StringVal(left, "path") < querycontract.StringVal(right, "path")
	})
}
