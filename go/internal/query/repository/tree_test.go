...
func requestRepositoryTree(t *testing.T, handler *Handler, target string) *httptest.ResponseRecorder {
	t.Helper()
...
func TestGetRepositoryTree_LocalLightweightReturnsTree(t *testing.T) {
	...
	if got, want := string(env.Truth.Profile), string(querycontract.ProfileLocalLightweight); got != want {
		t.Fatalf("truth profile = %s, want %s", got, want)
	}
	if got, want := string(env.Truth.Level), string(querycontract.TruthLevelDerived); got != want {
