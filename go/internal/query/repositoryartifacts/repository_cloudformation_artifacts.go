...
func BuildRepositoryCloudFormationRuntimeArtifacts(infrastructure []map[string]any) map[string]any {
	...
	for _, row := range infrastructure {
		if strings.TrimSpace(querycontract.StringVal(row, "type")) != "CloudFormationResource" {
			continue
		}
		resourceType := strings.TrimSpace(querycontract.StringVal(row, "resource_type"))
		if !isCloudFormationServerlessResource(resourceType) {
			continue
		}
		path := strings.TrimSpace(querycontract.StringVal(row, "file_path"))
		name := strings.TrimSpace(querycontract.StringVal(row, "name"))
		if path == "" || name == "" {
			continue
		}
	...
	sort.SliceStable(artifacts, func(i, j int) bool {
		if left, right := querycontract.StringVal(artifacts[i], "relative_path"), querycontract.StringVal(artifacts[j], "relative_path"); left != right {
			return left < right
		}
		return querycontract.StringVal(artifacts[i], "artifact_name") < querycontract.StringVal(artifacts[j], "artifact_name")
	})
...
// buildRepositoryCloudFormationRuntimeArtifacts keeps the in-package spelling
// after the #6060 export; root tests name
// BuildRepositoryCloudFormationRuntimeArtifacts.
func buildRepositoryCloudFormationRuntimeArtifacts(infrastructure []map[string]any) map[string]any {
	return BuildRepositoryCloudFormationRuntimeArtifacts(infrastructure)
}
