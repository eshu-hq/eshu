...
func extractKustomizeConfigPathRows(repoName string, files []querycontract.FileContent) []map[string]any {
	index := make(map[string]querycontract.FileContent, len(files))
...
func collectKustomizeResourceFiles(index map[string]querycontract.FileContent, kustomizationPath string) []querycontract.FileContent {
	resourceFiles := make([]querycontract.FileContent, 0)
...
func resolveKustomizationTarget(index map[string]querycontract.FileContent, resolved string) (string, bool) {
