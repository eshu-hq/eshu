...
func TestBuildRepositoryControllerArtifactsExtractsJenkinsPipelineSignals(t *testing.T) {
	...
	got := BuildRepositoryControllerArtifacts("controller-repo", []querycontract.FileContent{
		{
...
`,
	...
	artifacts := querycontract.MapSliceValue(got, "controller_artifacts")
	if len(artifacts) != 2 {
	...
	hints := querycontract.MapSliceValue(first, "ansible_playbook_hints")
	if len(hints) != 1 {
	...
	inventories := querycontract.StringSliceVal(first, "ansible_inventories")
	...
	varFiles := querycontract.StringSliceVal(first, "ansible_var_files")
	...
	taskEntrypoints := querycontract.StringSliceVal(first, "ansible_task_entrypoints")
	...
	rolePaths := querycontract.StringSliceVal(first, "ansible_role_paths")
