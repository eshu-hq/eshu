...
func BuildRepositoryRuntimeArtifacts(files []querycontract.FileContent) map[string]any {
...
func loadRepositoryRuntimeArtifacts(
	...
	hydratedCandidates, err := HydrateRepositoryCandidateFiles(ctx, reader, repoID, candidates, func(file querycontract.FileContent) bool {
...
func MergeDeploymentArtifactMaps(left map[string]any, right map[string]any) map[string]any {
	...
	for key, value := range right {
		if existingRows, ok := querycontract.MapSliceValue(merged, key), true; ok && len(existingRows) > 0 {
			incomingRows := querycontract.MapSliceValue(right, key)
...
func IsDockerComposeArtifact(file querycontract.FileContent) bool {
...
func composeArtifactType(file querycontract.FileContent) string {
