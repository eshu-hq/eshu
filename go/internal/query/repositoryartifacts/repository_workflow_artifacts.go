...
func WorkflowArtifactDetails(content string) ([]string, []string, []string, []string, []string, []string, []string, []string, []string, []string, []string, []string, []string, []string, []string, int, []string) {
	...
	for _, document := range documents {
		...
		for jobName, rawJob := range jobs {
			...
			if condition := strings.TrimSpace(querycontract.StringVal(job, "if")); condition != "" {
			...
			for _, rawStep := range steps {
				...
				runCommand := strings.TrimSpace(querycontract.StringVal(step, "run"))
				if condition := strings.TrimSpace(querycontract.StringVal(step, "if")); condition != "" {
...
func GithubActionsLocalReusableWorkflowPath(value string) string {
	return ghactionsref.LocalReusableWorkflowPath(querycontract.TrimGitHubActionsScalar(value))
}
...
func githubActionsConcurrencyGroups(rawConcurrency any) []string {
	...
	case map[string]any:
		if group := strings.TrimSpace(querycontract.StringVal(typed, "group")); group != "" {
			return []string{group}
		}
	}
	return nil
...
func githubActionsEnvironmentNames(rawEnvironment any) []string {
	...
	case map[string]any:
		if name := strings.TrimSpace(querycontract.StringVal(typed, "name")); name != "" {
			return []string{name}
		}
	}
	return nil
...
func githubActionsCheckoutRepositories(step map[string]any) []string {
	...
	refs = append(refs, querycontract.MetadataStringSlice(step, "repository")...)
	if with, ok := step["with"].(map[string]any); ok {
		refs = append(refs, querycontract.MetadataStringSlice(with, "repository")...)
	}
	return refs
}
...
func githubActionsNeedsDependencies(jobName string, rawNeeds any) []string {
	...
	case []any:
		for _, item := range value {
			trimmed := strings.TrimSpace(querycontract.StringVal(map[string]any{"needs": item}, "needs"))
			if trimmed == "" {
				continue
			}
			needs = append(needs, jobName+"<-"+trimmed)
...
func workflowStepName(step map[string]any) string {
	if name := strings.TrimSpace(querycontract.StringVal(step, "name")); name != "" {
		return name
	}
	if uses := strings.TrimSpace(querycontract.StringVal(step, "uses")); uses != "" {
		return uses
	}
	if run := strings.TrimSpace(querycontract.StringVal(step, "run")); run != "" {
		return run
	}
	return "step"
}
