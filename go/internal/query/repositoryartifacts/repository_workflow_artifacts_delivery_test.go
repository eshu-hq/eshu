...
func TestBuildRepositoryWorkflowArtifactsClassifiesDeliveryCommandFamilies(t *testing.T) {
	...
	artifacts := BuildRepositoryWorkflowArtifacts([]querycontract.FileContent{
		{
...
`,
	...
	row := rows[0]
	if got, want := querycontract.StringSliceVal(row, "delivery_command_families"), []string{
		"ansible",
		"argocd",
		"docker",
		"docker_compose",
	...
	if got, want := querycontract.StringSliceVal(row, "signals"), []string{
		"workflow_file",
		"run_commands",
		"workflow_triggers",
...
`,
	...
	row := rows[0]
	if got := querycontract.StringSliceVal(row, "action_repositories"); len(got) != 1 || got[0] != "hashicorp/setup-terraform" {
		t.Fatalf("action_repositories = %#v, want hashicorp/setup-terraform", got)
	}
	if got := querycontract.StringSliceVal(row, "delivery_command_families"); len(got) != 0 {
...
`,
	...
	row := rows[0]
	if got, want := querycontract.StringSliceVal(row, "delivery_local_paths"), []string{
		"charts/edge-api",
