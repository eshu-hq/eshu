...
// TestCountSBOMAttestationAttachmentsMapsGraphReadAvailabilityErrors proves
...
// countSBOMAttestationAttachments maps to the stable 503/504 envelope
// instead of a bare 500. The selector "my-repo-name" is intentionally not
...
func TestCountSBOMAttestationAttachmentsMapsGraphReadAvailabilityErrors(t *testing.T) {
	t.Parallel()
	...
	for _, test := range graphReadSweepCases() {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			handler := &SupplyChainHandler{
				Neo4j:                    graph,
				Content:                  nil,
				SBOMAttachmentAggregates: &stubSBOMAttestationAttachmentAggregateStore{},
			}
			...
			req := httptest.NewRequest(
				http.MethodGet,
				"/api/v0/supply-chain/sbom-attestations/attachments/count?repository_id=my-repo-name",
				nil,
			)
