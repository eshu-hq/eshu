...
func TestSemanticSearchHandlerRejectsAmbiguousRepositoryScope(t *testing.T) {
	...
	if envelope.Error == nil || envelope.Error.Code != querycontract.ErrorCodeAmbiguous {
		t.Fatalf("error = %#v, want code %q", envelope.Error, querycontract.ErrorCodeAmbiguous)
	}
	if index.calls != 0 {
