...
// TestSemanticSearchHandlerEmitsDegradedOnIndexUnready proves a hybrid request
// that fell back to keyword because the persisted vector index is not ready
// (retrieval_state index_unready) increments the counter with the requested
// query_type and reason=index_unready.
func TestSemanticSearchHandlerEmitsDegradedOnIndexUnready(t *testing.T) {
	reader := withDegradedCounterReader(t)
	index := &fakeSemanticSearchIndexStore{
		result: SemanticSearchIndexResult{
			IndexedDocumentCount: 1,
	...
	rm := collectDegradedMetrics(t, reader)
	if got := degradedCounterValue(t, rm, map[string]string{"query_type": "hybrid", "reason": "index_unready"}); got != 1 {
		t.Fatalf("degraded counter = %d, want 1", got)
	}
}
...
func TestSemanticSearchHandlerEmitsDegradedOnSemanticNoEmbedder(t *testing.T) {
	...
	handler := &SemanticSearchHandler{Index: &fakeSemanticSearchIndexStore{}, Profile: querycontract.ProfileProduction}
	mux := http.NewServeMux()
	handler.Mount(mux)
...
func TestSemanticSearchDegradation(t *testing.T) {
	...
	cases := []struct {
		...
		{"hybrid degraded to bm25", searchbench.ModeHybrid, "hybrid_degraded", true, "hybrid", "no_embedder"},
		{"semantic unavailable", searchbench.ModeSemantic, "semantic_unavailable", true, "semantic", "no_embedder"},
		{"hybrid index unready", searchbench.ModeHybrid, "index_unready", true, "hybrid", "index_unready"},
		{"semantic index unready", searchbench.ModeSemantic, "index_unready", true, "semantic", "index_unready"},
		{"keyword index unready not degraded", searchbench.ModeKeyword, "index_unready", false, "", ""},
		{"hybrid active", searchbench.ModeHybrid, "hybrid_active", false, "", ""},
		{"semantic active", searchbench.ModeSemantic, "semantic_active", false, "", ""},
		{"keyword requested", searchbench.ModeKeyword, "keyword_only", false, "", ""},
		{"empty state", searchbench.ModeHybrid, "", false, "", ""},
		{"unknown state", searchbench.ModeHybrid, "something_else", false, "", ""},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			degraded, queryType, reason := semanticSearchDegradation(tc.mode, tc.retrievalState)
