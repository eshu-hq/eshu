...
func TestSemanticSearchHandlerKeywordModeIgnoresPendingSearchVector(t *testing.T) {
	...
	handler := &SemanticSearchHandler{Index: index, Profile: querycontract.ProfileProduction, SearchVectorReady: ready}
...
func TestSemanticSearchHandlerHybridModeAppliesPendingSearchVector(t *testing.T) {
	...
	if got, want := envelope.Truth.Freshness.State, querycontract.FreshnessBuilding; got != want {
		t.Fatalf("hybrid mode freshness state = %q, want %q", got, want)
	}
	if got, want := envelope.Truth.Freshness.Cause, querycontract.FreshnessCausePendingSearchVector; got != want {
...
func decodeSemanticSearchEnvelope(t *testing.T, rec *httptest.ResponseRecorder) querycontract.ResponseEnvelope {
	t.Helper()
