...
//
// It is exported so root package query's compatibility forwarder can name it
// in NewPostgresSearchVectorReadyStore's signature (#6060). The surface is
// still one method: a caller passes its *sql.DB or an instrumented wrapper,
// and this store cannot write through it.
