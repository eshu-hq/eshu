...
func TestServiceInvestigationCoverageDisclosesProvisioningReadTruncation(t *testing.T) {
	...
	t.Run("names the downstream read bound", func(t *testing.T) {
		t.Parallel()
		coverage := coverageFor(t, nil)
		if got, want := querycontract.IntVal(coverage, "downstream_read_limit"), querycontract.DefaultIndirectEvidenceSearchLimit; got != want {
			t.Fatalf("coverage_summary[downstream_read_limit] = %d, want %d", got, want)
		}
		if got, want := querycontract.IntVal(coverage, "result_limit"), serviceStoryItemLimit; got != want {
...
func TestBuildServiceResultLimitsReportsTheDownstreamReadBound(t *testing.T) {
	...
	if got, want := querycontract.IntVal(limits, "downstream_read_limit"), querycontract.DefaultIndirectEvidenceSearchLimit; got != want {
		t.Fatalf("result_limits[downstream_read_limit] = %d, want %d", got, want)
	}
	if got, want := querycontract.IntVal(limits, "limit"), serviceStoryItemLimit; got != want {
	...
	if querycontract.IntVal(limits, "downstream_read_limit") >= querycontract.IntVal(limits, "limit") {
		t.Fatalf(
			...
			querycontract.IntVal(limits, "downstream_read_limit"), querycontract.IntVal(limits, "limit"),
		)
	}
}
