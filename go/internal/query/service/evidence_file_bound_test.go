...
// it does not.
func serviceEvidenceFileCorpus(count int) []querycontract.FileContent {
	files := make([]querycontract.FileContent, 0, count)
...
func (r limitHonoringServiceEvidenceReader) ListRepoFiles(
	...
	if limit > 0 && limit < len(r.files) {
		return append([]querycontract.FileContent(nil), r.files[:limit]...), nil
	}
	return append([]querycontract.FileContent(nil), r.files...), nil
}
...
// comes from querycontract.BoundedTraceEnrichmentLimit, whose smallest result is 10, so the
...
func TestTraceDeploymentChainDisclosesTheServiceEvidenceFileBound(t *testing.T) {
	...
	t.Run("a full evidence file page discloses only the consumer flag", func(t *testing.T) {
		...
		if got, want := len(querycontract.MapSliceValue(body, "consumer_repositories")), serviceEvidenceCandidateRowCount; got != want {
	...
	t.Run("one file fewer discloses nothing", func(t *testing.T) {
		...
		if got := len(querycontract.MapSliceValue(body, "consumer_repositories")); got == 0 {
