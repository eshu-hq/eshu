...
func TestBuildServiceStoryResponseExposesNonEntrypointCandidates(t *testing.T) {
	...
	entrypoints := querycontract.MapSliceValue(got, "entrypoints")
	if len(entrypoints) != 2 {
		t.Fatalf("entrypoints = %#v, want docs route and exact hostname only", entrypoints)
	}
	for _, entrypoint := range entrypoints {
		if querycontract.StringVal(entrypoint, "target") == "app.config.retry.count" {
			t.Fatalf("rejected config key was promoted to service entrypoint: %#v", entrypoints)
		}
	}

	candidates := querycontract.MapSliceValue(got, "entrypoint_candidates")
	if len(candidates) != 1 {
		t.Fatalf("entrypoint_candidates = %#v, want rejected supporting evidence", candidates)
	}
	if got, want := querycontract.StringVal(candidates[0], "reason"), "dotted_config_key"; got != want {
...
func TestRepositoryStoryReadbackKeepsDocsRoutesWithoutHostnameEntrypoints(t *testing.T) {
	...
	repository.EnrichRepositoryStoryResponseWithEvidence(response, nil, []querycontract.FileContent{
		{
...
`,
	...
	documentation := querycontract.MapValue(response, "documentation_overview")
	if got, want := querycontract.StringSliceVal(documentation, "docs_routes"), []string{"/docs"}; !reflect.DeepEqual(got, want) {
		t.Fatalf("documentation_overview.docs_routes = %#v, want %#v", got, want)
	}
	if _, ok := querycontract.MapValue(response, "deployment_overview")["hostnames"]; ok {
