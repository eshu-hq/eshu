...
func TestBuildServiceStoryResponseIncludesInvestigationPacket(t *testing.T) {
	...
	if got, want := querycontract.StringVal(investigation, "service_name"), "sample-service-api"; got != want {
		t.Fatalf("investigation.service_name = %q, want %q", got, want)
	}
	repositories := querycontract.MapSliceValue(investigation, "repositories_considered")
	...
	families := querycontract.StringSliceVal(investigation, "evidence_families_found")
	for _, want := range []string{"api_surface", "deployment_lanes", "documentation", "downstream_consumers", "upstream_dependencies"} {
		if !slices.Contains(families, want) {
			t.Fatalf("evidence_families_found = %#v, missing %q", families, want)
		}
	}
	coverage := querycontract.MapValue(investigation, "coverage_summary")
	if got := querycontract.StringVal(coverage, "state"); got == "" || got == "complete" {
		t.Fatalf("coverage_summary.state = %q, want truthful non-complete coverage", got)
	}
	if nextCalls := querycontract.MapSliceValue(investigation, "recommended_next_calls"); len(nextCalls) == 0 {
