		...
		RunFn: func(_ context.Context, cypher string, _ map[string]any) ([]map[string]any, error) {
			...
			case strings.Contains(cypher, "<-[:DEFINES]-(r:Repository)"):
				...
				return []map[string]any{{
					"repo_id":   querycontract.StringVal(workload, "repo_id"),
					"repo_name": querycontract.StringVal(workload, "repo_name"),
				}}, nil
			default:
				return nil, nil
			}
...
func (s fullPageConsumerSearchContentStore) SearchFileContentAnyRepoExactCase(
	...
	rows := make([]querycontract.FileContent, 0, limit)
...
func TestTraceDeploymentChainDisclosesProvisioningReadTruncation(t *testing.T) {
	...
	t.Run("truncated read discloses all three fields", func(t *testing.T) {
		...
		// The route resolves max_depth 0 to querycontract.DefaultIndirectEvidenceSearchLimit,
		// so one row past that bound is what the over-fetch probe detects.
		body := runProvisioningTruncationTrace(t, querycontract.DefaultIndirectEvidenceSearchLimit+1)
		for _, field := range provisioningTruncationResponseFields {
			if !querycontract.BoolVal(body, field) {
				t.Fatalf(
					"%s = %#v, want true (the provisioning-candidate read returned more than querycontract.DefaultIndirectEvidenceSearchLimit = %d rows)",
					field, body[field], querycontract.DefaultIndirectEvidenceSearchLimit,
				)
			}
		}
		// The disclosed lists must still be bounded at the limit, so a caller
		// cannot infer completeness from the count alone.
		if got, want := len(querycontract.MapSliceValue(body, "dependents")), querycontract.DefaultIndirectEvidenceSearchLimit; got != want {
	...
	t.Run("untruncated read omits all three fields", func(t *testing.T) {
		...
		body := runProvisioningTruncationTrace(t, querycontract.DefaultIndirectEvidenceSearchLimit)
		for _, field := range provisioningTruncationResponseFields {
			if value, present := body[field]; present {
				t.Fatalf(
					"%s = %#v, want the key absent (the read returned exactly querycontract.DefaultIndirectEvidenceSearchLimit = %d rows, nothing was dropped)",
					field, value, querycontract.DefaultIndirectEvidenceSearchLimit,
				)
			}
		}
		...
		if got := len(querycontract.MapSliceValue(body, "dependents")); got == 0 {
...
func TestTraceDeploymentChainDistinguishesConsumerTruncationFromCandidateTruncation(t *testing.T) {
	...
	if !querycontract.BoolVal(body, "consumer_repositories_truncated") {
		t.Fatalf(
			...
			body["consumer_repositories_truncated"], querycontract.DefaultIndirectEvidenceSearchLimit,
		)
	}
	for _, field := range []string{"dependents_truncated", "provisioning_source_chains_truncated"} {
		if value, present := body[field]; present {
			t.Fatalf(
				...
				field, value, candidateRowCount, querycontract.DefaultIndirectEvidenceSearchLimit,
			)
		}
	}
	...
	if got, want := len(querycontract.MapSliceValue(body, "consumer_repositories")), candidateRowCount+1; got != want {
		t.Fatalf("len(consumer_repositories) = %d, want %d", got, want)
	}
	if got, want := len(querycontract.MapSliceValue(body, "dependents")), candidateRowCount; got != want {
		t.Fatalf("len(dependents) = %d, want %d", got, want)
	}
}
...
func TestEnrichServiceQueryContextSetsProvisioningTruncationKeys(t *testing.T) {
	...
	t.Run("truncated read sets all three keys", func(t *testing.T) {
		...
		workloadContext := enrichWithProvisioningCandidateRows(t, context.Background(), querycontract.DefaultIndirectEvidenceSearchLimit+1)
		for _, key := range provisioningTruncationResponseFields {
			if !querycontract.BoolVal(workloadContext, key) {
	...
	t.Run("untruncated read sets none of them", func(t *testing.T) {
		...
		workloadContext := enrichWithProvisioningCandidateRows(t, context.Background(), querycontract.DefaultIndirectEvidenceSearchLimit)
		for _, key := range provisioningTruncationResponseFields {
			if value, present := workloadContext[key]; present {
				t.Fatalf("workloadContext[%s] = %#v, want the key absent", key, value)
			}
		}
		if got := len(querycontract.MapSliceValue(workloadContext, "dependents")); got == 0 {
...
func TestEnrichServiceQueryContextDisclosesTruncationToScopedCallers(t *testing.T) {
	...
	t.Run("grant emptied by the access filter still discloses", func(t *testing.T) {
		...
		workloadContext := enrichWithProvisioningCandidateRows(
			...
			querycontract.DefaultIndirectEvidenceSearchLimit+1,
		)
		if got := len(querycontract.MapSliceValue(workloadContext, "dependents")); got != 0 {
	...
	t.Run("surviving grant still discloses", func(t *testing.T) {
		...
		if got, want := len(querycontract.MapSliceValue(workloadContext, "dependents")), 1; got != want {
