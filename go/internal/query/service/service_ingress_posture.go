...
func edgeResourcesFromCloudResources(cloudResources []map[string]any) []ingressEdgeResource {
	...
	for _, resource := range cloudResources {
		resourceType := querycontract.FirstNonEmptyString(
			querycontract.StringVal(resource, "resource_type"),
			querycontract.StringVal(resource, "kind"),
		)
		if _, ok := ingressEdgeResourceTypes[resourceType]; !ok {
			continue
		}
		id := querycontract.FirstNonEmptyString(
			querycontract.StringVal(resource, "id"),
			querycontract.StringVal(resource, "resource_id"),
			querycontract.StringVal(resource, "arn"),
			querycontract.StringVal(resource, "name"),
		)
		if id == "" {
			continue
		}
		...
		edges = append(edges, ingressEdgeResource{
			id:           id,
			name:         querycontract.StringVal(resource, "name"),
...
func ingressPostureEdgeSet(ctx context.Context, graph querycontract.GraphQuery, cypher string, params map[string]any) (map[string]bool, error) {
	...
	for _, row := range rows {
		if id := querycontract.StringVal(row, "edge_id"); id != "" {
			set[id] = true
		}
	}
	return set, nil
