...
func TestBuildIngressPostureUnprovenWithNoEdges(t *testing.T) {
	...
	if got := querycontract.StringVal(posture, "waf_coverage"); got != ingressPostureUnproven {
		t.Fatalf("waf_coverage = %q, want %q", got, ingressPostureUnproven)
	}
	if got := querycontract.StringVal(posture, "tls_termination"); got != ingressPostureUnproven {
		t.Fatalf("tls_termination = %q, want %q", got, ingressPostureUnproven)
	}
	if got := querycontract.IntVal(posture, "edge_count"); got != 0 {
...
func TestBuildIngressPostureProtectedAndTerminated(t *testing.T) {
	...
	if got := querycontract.StringVal(posture, "waf_coverage"); got != ingressPostureProtected {
		t.Fatalf("waf_coverage = %q, want %q", got, ingressPostureProtected)
	}
	if got := querycontract.StringVal(posture, "tls_termination"); got != ingressPostureTerminated {
		t.Fatalf("tls_termination = %q, want %q", got, ingressPostureTerminated)
	}
	if got := querycontract.IntVal(posture, "waf_protected"); got != 1 {
...
func TestBuildIngressPostureUnprotectedWhenEdgeMissingProtection(t *testing.T) {
	...
	if got := querycontract.StringVal(posture, "waf_coverage"); got != ingressPostureUnprotected {
		t.Fatalf("waf_coverage = %q, want %q", got, ingressPostureUnprotected)
	}
	if got := querycontract.StringVal(posture, "tls_termination"); got != ingressPostureNotTerminated {
		t.Fatalf("tls_termination = %q, want %q", got, ingressPostureNotTerminated)
	}
	if got := querycontract.IntVal(posture, "waf_protected"); got != 1 {
		t.Fatalf("waf_protected = %d, want 1", got)
	}
	edgeRows := querycontract.MapSliceValue(posture, "edges")
	if len(edgeRows) != 2 {
...
func TestBuildIngressPostureUnprovenWhenCollectorAbsent(t *testing.T) {
	...
	if got := querycontract.StringVal(edgeRows[0], "waf_coverage"); got != ingressPostureUnproven {
