...
func buildServiceAPISurface(evidence ServiceQueryEvidence) map[string]any {
	...
	sort.Slice(endpoints, func(i, j int) bool {
		if querycontract.StringVal(endpoints[i], "path") != querycontract.StringVal(endpoints[j], "path") {
			return querycontract.StringVal(endpoints[i], "path") < querycontract.StringVal(endpoints[j], "path")
		}
		return querycontract.StringVal(endpoints[i], "spec_path") < querycontract.StringVal(endpoints[j], "spec_path")
	})
	...
	result := map[string]any{
		...
		"spec_paths":         querycontract.UniqueSortedStrings(specPaths),
...
func frameworkRouteEndpoints(fr FrameworkRouteEvidence) []frameworkRouteEndpoint {
	...
	for _, path := range paths {
		endpoints = append(endpoints, frameworkRouteEndpoint{
			Path:    path,
			Methods: querycontract.UniqueSortedStrings(methodsByPath[path]),
		})
	}
	return endpoints
}
