...
func buildServiceEvidenceGraph(workloadContext map[string]any) map[string]any {
	...
	addEvidenceNode(nodes, serviceID, querycontract.SafeStr(workloadContext, "name"), "service", "service", serviceEvidenceNodeFields{
		role: "workload",
	})
	addEvidenceNode(nodes, serviceRepoID, querycontract.SafeStr(workloadContext, "repo_name"), "repository", "source", serviceEvidenceNodeFields{
	...
	for _, instance := range querycontract.MapSliceValue(workloadContext, "instances") {
		addEvidenceNode(
			nodes,
			querycontract.StringVal(instance, "instance_id"),
			querycontract.FirstNonEmptyString(querycontract.StringVal(instance, "platform_name"), querycontract.StringVal(instance, "instance_id")),
			"runtime",
			"runtime",
	...
	for _, consumer := range querycontract.MapSliceValue(workloadContext, "consumer_repositories") {
		addEvidenceNode(nodes, querycontract.StringVal(consumer, "repo_id"), querycontract.StringVal(consumer, "repository"), "repository", "downstream", serviceEvidenceNodeFields{
			role: "downstream_consumer",
		})
	}
	for _, dependent := range querycontract.MapSliceValue(workloadContext, "dependents") {
		addEvidenceNode(nodes, querycontract.StringVal(dependent, "repo_id"), querycontract.StringVal(dependent, "repository"), "repository", "downstream", serviceEvidenceNodeFields{
			role: "downstream_consumer",
		})
	}
...
func serviceEvidenceGraphEdges(workloadContext map[string]any) ([]map[string]any, int, bool) {
	...
	for _, artifact := range serviceDeploymentArtifacts(workloadContext) {
		...
		edge := map[string]any{
			"id":                resolvedID,
			"source":            querycontract.StringVal(artifact, "source_repo_id"),
			"target":            querycontract.StringVal(artifact, "target_repo_id"),
	...
	serviceID := querycontract.SafeStr(workloadContext, "id")
	for _, instance := range querycontract.MapSliceValue(workloadContext, "instances") {
		instanceID := querycontract.StringVal(instance, "instance_id")
	...
	slices.SortFunc(edges, func(left, right map[string]any) int {
		if relationshipOrder := cmp.Compare(querycontract.StringVal(left, "relationship_type"), querycontract.StringVal(right, "relationship_type")); relationshipOrder != 0 {
			return relationshipOrder
		}
		for _, key := range []string{"resolved_id", "source", "target", "id"} {
			if fieldOrder := cmp.Compare(querycontract.StringVal(left, key), querycontract.StringVal(right, key)); fieldOrder != 0 {
				return fieldOrder
			}
		}
		return 0
...
func addServiceRepositoryEvidenceNode(
	...
	id := querycontract.StringVal(artifact, endpoint+"_repo_id")
	...
	addEvidenceNode(
		nodes,
		id,
		querycontract.StringVal(artifact, endpoint+"_repo_name"),
		"repository",
		category,
		serviceEvidenceNodeFields{
			role:         role,
			canonicalKey: querycontract.StringVal(artifact, endpoint+"_repo_canonical_id"),
			scopeKey:     querycontract.StringVal(artifact, endpoint+"_repo_scope_key"),
		},
	)
}
...
func addEvidenceNode(
	...
	if existing := nodes[id]; existing != nil {
		if serviceEvidenceNodeRolePriority(querycontract.StringVal(existing, "role")) > serviceEvidenceNodeRolePriority(querycontract.StringVal(node, "role")) {
			for key, value := range node {
				if querycontract.StringVal(existing, key) == "" {
					existing[key] = value
				}
			}
			nodes[id] = existing
		...
		for key, value := range existing {
			if querycontract.StringVal(node, key) == "" {
				node[key] = value
			}
		}
	}
