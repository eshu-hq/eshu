...
func TestGetOperationsScopedWithGrantsSeesOnlyGrantedRowsIdentityRedacted(t *testing.T) {
	...
	if got, want := reader.calledAllowedRepositoryIDs, []string{"repo-a"}; !slices.Equal(got, want) {
		t.Fatalf("reader called with AllowedRepositoryIDs = %#v, want %#v", got, want)
	}
	if got, want := reader.calledAllowedScopeIDs, []string{"scope-a"}; !slices.Equal(got, want) {
