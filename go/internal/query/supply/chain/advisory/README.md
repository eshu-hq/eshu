...
- `internal/storage/postgres/array` — the array scan/build surface the
...
moved with their wrappers into `factschema_decode_test.go`,
...
- A dropped `Sources` entry is a dead-lettered malformed fact
  ...
  `factschema_decode.go` for which fields stay on the raw path
  and why.
