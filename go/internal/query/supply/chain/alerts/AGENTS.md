...
Scope: `go/internal/query/supply/chain/alerts/` (package `alerts`).
...
- MUST NOT import root package `query` or `supplychain` would import this
  package for the compatibility aliases in `compat_supply_chain.go`,
...
- `Queryer` and `AggregateQueryer` are exported because they are
  ...
  `compat_supply_chain.go` must be able to name them, and through those,
...
- `StringMapVal` is exported because root's `sbom_attestation_attachments.go`
  ...
  `stringMapVal` forward in `compat_supply_chain.go`. `mapVal` and
...
- The `var _ supplychain.SecurityAlertReconciliation(Aggregate)?Store = ...`
  ...
  `compat_supply_chain.go` (#6642) -- keep them beside the type they pin
...
`compat_supply_chain.go` keeps every old exported spelling, and the
