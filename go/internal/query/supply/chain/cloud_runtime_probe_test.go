...
// probe keys off subject digests, never off this entry's value, so the
// copy is opaque fixture text, not a contract. If the cicd family renames
// the kind, update this mirror (rg both names).
const cloudRuntimeProbeTestCICDFactKind = "reducer_ci_cd_run_correlation"
...
func TestProbeSupplyChainCloudRuntimeResourcesRecordsTruthfulCounts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := &Handler{CloudResourceInventory: tt.inventory}
...
func TestApplySupplyChainCloudRuntimeEvidencePromotesRunningDigest(t *testing.T) {
	...
	graph := &graph.FakeCloudRuntimeGraph{
		RowsByDigest: map[string][]map[string]any{
	...
	handler := &Handler{Neo4j: graph, CloudResourceInventory: inventory}

	rows := []impact.FindingRow{
		{FindingID: "f-running", SubjectDigest: runningDigest, EvidencePath: []string{cloudRuntimeProbeTestCICDFactKind}},
		{FindingID: "f-notrunning", SubjectDigest: "sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", EvidencePath: []string{cloudRuntimeProbeTestCICDFactKind}},
	}
	...
	if tier := impact.BuildFindingResult(&rows[0]).DeploymentTruthTier; tier != "runtime_confirmed" {
	...
	if tier := impact.BuildFindingResult(&rows[1]).DeploymentTruthTier; tier != "provenance_ci_declared" {
...
func TestApplySupplyChainCloudRuntimeEvidenceExcludesStaleOrUnauthorized(t *testing.T) {
	...
	rows := []impact.FindingRow{{FindingID: "f", SubjectDigest: runningDigest, EvidencePath: []string{cloudRuntimeProbeTestCICDFactKind}}}
	if err := handler.applySupplyChainCloudRuntimeEvidence(context.Background(), querycontract.RepositoryAccessFilter{AllScopes: true}, rows); err != nil {
		t.Fatalf("applySupplyChainCloudRuntimeEvidence() error = %v, want nil", err)
	}
	...
	if tier := impact.BuildFindingResult(&rows[0]).DeploymentTruthTier; tier != "provenance_ci_declared" {
...
func TestApplySupplyChainCloudRuntimeEvidenceScopedCallerGetsAuthorized(t *testing.T) {
	...
	scoped := querycontract.RepositoryAccessFilter{AllowedRepositoryIDs: []string{"repository:r_granted"}}
	rows := []impact.FindingRow{{FindingID: "f", SubjectDigest: runningDigest}}
...
func TestApplySupplyChainCloudRuntimeEvidenceDoesNotReadGraph(t *testing.T) {
	...
	graph := &graph.FakeCloudRuntimeGraph{Err: errors.New("graph unavailable")}
...
func TestApplySupplyChainCloudRuntimeEvidencePropagatesLedgerError(t *testing.T) {
	...
	graph := &graph.FakeCloudRuntimeGraph{RowsByDigest: map[string][]map[string]any{digest: {cloudResourceGraphRow("uid-x", digest, "arn-x")}}}
	inventory := &stubCloudInventory{
...
func TestApplySupplyChainCloudRuntimeEvidenceNilStoresAreNoOp(t *testing.T) {
	...
	rows := []impact.FindingRow{{FindingID: "f", SubjectDigest: "sha256:cc"}}
	// Nil graph remains safe because runtime evidence resolves from Postgres.
	if err := (&Handler{CloudResourceInventory: &stubCloudInventory{}}).
	...
	if err := (&Handler{Neo4j: &graph.FakeCloudRuntimeGraph{}}).
		applySupplyChainCloudRuntimeEvidence(context.Background(), querycontract.RepositoryAccessFilter{AllScopes: true}, rows); err != nil {
