...
const WinnersReadEnv = "ESHU_SUPPLY_CHAIN_IMPACT_WINNERS_READ"

// WinnersReadEnabled parses the Phase 2 read gate value using
...
func WinnersReadEnabled(value string) bool {
	enabled, err := strconv.ParseBool(strings.TrimSpace(value))
	return err == nil && enabled
}

// FindingStore reads reducer-owned vulnerability impact
// findings.
type FindingStore interface {
	ListSupplyChainImpactFindings(context.Context, FindingFilter) ([]FindingRow, error)
}

// FindingFilter bounds impact reads to concrete evidence,
...
type FindingFilter struct {
	...
	ImageRef                               string
	Status, Ecosystem, Severity            string
...
// FindingRow is one durable impact finding decoded from
// reducer-owned facts.
type FindingRow struct {
	...
	PriorityContributions []PriorityContribution
	...
	// position as FindingResult so the
	// FindingResult(row) conversion stays valid.
	...
	// same field position as FindingResult so the
	// FindingResult(row) conversion stays valid.
	...
	// the default view.
	Suppression *SuppressionDecisionRow
	...
	VersionResolutionCorroboration []VersionResolutionCorroboration
}

// SuppressionDecisionRow is the API-shaped suppression decision
// attached to one impact finding. The reducer produces an "active" row when
// no suppression matched.
type SuppressionDecisionRow struct {
...
// Provenance preserves per-source advisory provenance for one
...
type Provenance struct {
	...
	AlternateSeverities        []AlternateSeverity  `json:"alternate_severities,omitempty"`
	FixedVersionBranches       []FixedVersionBranch `json:"fixed_version_branches,omitempty"`
	AdvisorySources            []AdvisorySource     `json:"advisory_sources,omitempty"`
}

// AlternateSeverity is one non-selected source severity preserved
// for explainability.
...
// FixedVersionBranch is one source-attributed fixed-version branch
...
// AdvisorySource is one source-attributed advisory observation
...
// WinnersFreshness reports the freshness of the maintained
...
func (s PostgresFindingStore) SupplyChainImpactWinnersWatermark(
	...
	if !rows.Next() {
		...
		return WinnersFreshness{ServingFromWinners: true, Present: false}, nil
	}
	var materializedAt time.Time
	if err := rows.Scan(&materializedAt); err != nil {
		return WinnersFreshness{ServingFromWinners: true}, fmt.Errorf("scan supply chain impact winners watermark: %w", err)
	}
	return WinnersFreshness{ServingFromWinners: true, Present: true, MaterializedAt: materializedAt}, nil
}
...
func (s PostgresFindingStore) ListSupplyChainImpactFindings(
	...
	out := make([]FindingRow, 0, filter.Limit)
	for rows.Next() {
		var factID string
...
func (f FindingFilter) HasScope() bool {
	return f.CVEID != "" || f.AdvisoryID != "" || f.PackageID != "" ||
		f.RepositoryID != "" || f.SubjectDigest != "" || f.ImageRef != "" || f.Status != "" ||
