...
func TestApplySupplyChainRuntimeContextThreadsScopedGrants(t *testing.T) {
	...
	if got, want := store.AllowedRepoIDs, access.AllowedRepositoryIDs; !slices.Equal(got, want) {
		t.Fatalf("allowed repository ids = %#v, want %#v", got, want)
	}
	if got, want := store.AllowedScopeIDs, access.AllowedScopeIDs; !slices.Equal(got, want) {
...
func TestSupplyChainImpactRuntimeContextHandlerThreadsScopeOnlyGrant(t *testing.T) {
	...
	if got, want := store.AllowedScopeIDs, []string{"scope:5747:tenant-a"}; !slices.Equal(got, want) {
...
func TestApplySupplyChainRuntimeContextKeepsRepeatedDigestEvidenceWithinRowPlan(t *testing.T) {
	...
	store := &graph.FakeRuntimeContextFindingStore{
		...
		ByDigest: map[string]map[string]string{
			first.SubjectDigest: {
				"production": impact.RuntimeEnvironmentEvidenceDeployEvent,
				"staging":    impact.RuntimeEnvironmentEvidenceDeclared,
			},
		},
	}
	rows := []impact.FindingRow{first, second}
	...
	for index, want := range []struct {
		...
		{environment: "production", evidence: impact.RuntimeEnvironmentEvidenceDeployEvent},
		{environment: "staging", evidence: impact.RuntimeEnvironmentEvidenceDeclared},
	} {
...
func TestApplySupplyChainRuntimeContextCapsRepeatedDigestPageEvidenceAtCandidateBudget(t *testing.T) {
	...
	rows := make([]impact.FindingRow, rowCount)
	...
	for index, row := range rows {
		...
		if got := row.RuntimeContext.EnvironmentEvidence; len(got) != 1 || got[wantEnvironment] != impact.RuntimeEnvironmentEvidenceDeployEvent {
...
// findingsOnlyStore implements just the findings list port, standing in for
// a legacy store that predates the runtime-context reader. It is local to
// this test (not a twin of root's recordingSupplyChainImpactFindingStore,
// which also records filters for route-level assertions this package does
// not need).
type findingsOnlyStore struct{}

func (findingsOnlyStore) ListSupplyChainImpactFindings(
	context.Context,
