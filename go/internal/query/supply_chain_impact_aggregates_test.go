...
type stubSupplyChainImpactAggregateStore struct {
	...
	inventoryErr    error
	lastFilter      impact.AggregateFilter
	lastCountFilter impact.AggregateFilter
	lastInvFilter   impact.AggregateFilter
	lastDimension   impact.InventoryDimension
	lastLimit       int
	lastOffset      int
	callCountCount  int
	callInvCount    int
...
func (s *stubSupplyChainImpactAggregateStore) SupplyChainImpactInventory(
	...
	return append([]impact.InventoryRow(nil), s.inventory...), nil
}
...
func TestSupplyChainImpactAggregatePriorityQueryQualifiesBucket(t *testing.T) {
	...
	if strings.Contains(impact.AggregatePriorityCountQuery, "NULLIF(priority_bucket") {
		t.Fatalf("priority aggregate query must qualify bucket references:\n%s", impact.AggregatePriorityCountQuery)
	}
	if !strings.Contains(impact.AggregatePriorityCountQuery, "NULLIF(fact.priority_bucket") {
...
func TestSupplyChainImpactAggregateQueriesCountCanonicalFindings(t *testing.T) {
	...
	for name, query := range map[string]string{
		"totals":    impact.AggregateCountQuery,
		"priority":  impact.AggregatePriorityCountQuery,
		"severity":  impact.AggregateSeverityCountQuery,
		"inventory": impact.InventoryQueryTemplate,
	} {
...
func TestSupplyChainImpactAggregateQueriesUseListProfileAndSuppressionPredicates(t *testing.T) {
	...
	if !strings.Contains(impact.InventoryQueryTemplate, "LIMIT $21 OFFSET $22") {
		t.Fatalf("inventory query must keep limit/offset after filter parameters:\n%s", impact.InventoryQueryTemplate)
	}
}
...
func TestSupplyChainImpactAggregateInventoryReportsTruncated(t *testing.T) {
	...
	rows := make([]impact.InventoryRow, 6)
	for i := range rows {
		rows[i] = impact.InventoryRow{
