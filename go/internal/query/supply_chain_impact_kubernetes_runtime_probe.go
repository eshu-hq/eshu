...
func (h *SupplyChainHandler) applySupplyChainKubernetesRuntimeEvidence(
	...
	byDigest := make(map[string][]KubernetesRuntimeWorkloadRef)
	...
	refsByDigest := make(map[string][]KubernetesRuntimeWorkloadRef, len(fanout.slots))
	metadataByDigest := make(map[string]*KubernetesRuntimeProbeMetadata, len(fanout.slots))
	...
	for _, slot := range fanout.slots {
		...
		metadata := &KubernetesRuntimeProbeMetadata{CandidateLimit: slot.plan.Quota}
		if allScopes {
			switch {
...
func kubernetesRuntimeCandidates(rows []map[string]any) []KubernetesRuntimeCandidate {
	...
	for _, row := range rows {
		candidate := KubernetesRuntimeCandidate{
			WorkloadUID:      strings.TrimSpace(StringVal(row, "workload_uid")),
			Digest:           strings.TrimSpace(StringVal(row, "matched_digest")),
			EdgeScopeID:      strings.TrimSpace(StringVal(row, "edge_scope_id")),
			EdgeGenerationID: strings.TrimSpace(StringVal(row, "edge_generation_id")),
		}
