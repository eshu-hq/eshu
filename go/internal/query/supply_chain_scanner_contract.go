...
func vulnerabilityScannerReadContract() map[string]any {
	return map[string]any{
		...
		"filters": []vulnerabilityScannerFilterContract{
			...
			{Name: "image_digest", Support: "exact", Semantics: []string{"exact", "missing-evidence driven"}, Parameters: []string{"subject_digest", "digest"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory", "impact_explain", "scanner_report"}, Backing: "subject_digest predicates over impact, SBOM, and container-image read models"},
			{Name: "workload", Support: "derived", Semantics: []string{"derived", "missing-evidence driven"}, Parameters: []string{"workload_id"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory"}, Backing: "reducer impact workload_ids array; missing runtime mapping remains missing evidence"},
			{Name: "service", Support: "derived", Semantics: []string{"derived", "missing-evidence driven"}, Parameters: []string{"service_id"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory"}, Backing: "reducer impact service_ids array; service comes only from admitted service/workload evidence"},
...
func vulnerabilityScannerRouteContracts() []vulnerabilityScannerRouteContract {
	return []vulnerabilityScannerRouteContract{
		...
		{Name: "impact_explain", Path: "/api/v0/supply-chain/impact/explain", Tool: "explain_supply_chain_impact", TruthCapability: supplyChainImpactExplanationCapability, LimitRequired: false, Timeout: "request context; one finding or one bounded advisory/package/repository/image path", Ordering: "one row only; ambiguous scopes return conflict", TruncatedField: "not applicable", MissingEvidence: "explanation.missing_evidence and readiness.missing_evidence"},
