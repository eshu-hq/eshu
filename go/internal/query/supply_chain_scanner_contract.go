...
func vulnerabilityScannerReadContract() map[string]any {
	return map[string]any{
		...
		"filters": []vulnerabilityScannerFilterContract{
			...
			{Name: "workload", Support: "derived", Semantics: []string{"derived", "missing-evidence driven"}, Parameters: []string{"workload_id"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory", "impact_explain"}, Backing: "baked reducer workload_ids plus current active workload-identity/service-catalog repository mapping at read time; missing runtime mapping remains missing evidence"},
			{Name: "service", Support: "derived", Semantics: []string{"derived", "missing-evidence driven"}, Parameters: []string{"service_id"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory", "impact_explain"}, Backing: "baked reducer service_ids plus current accepted service-catalog repository mapping at read time; service comes only from admitted evidence"},
			{Name: "environment", Support: "derived", Semantics: []string{"derived", "missing-evidence driven"}, Parameters: []string{"environment"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory"}, Backing: "baked reducer environments plus current accepted CI/CD repository mapping at read time; environment aliases are not guessed"},
		...
		"response_fields": []map[string]string{
			{
				...
				"notes": "read-time-resolved workloads, services, deployments, environments, and catalog refs joined from the finding's repository_id at query time (truth_basis: read_time_resolved), populated on the impact-findings list route only (the explain route embeds the same finding shape but does not resolve runtime context). workload_id, service_id, and environment filters use the same current active repository mappings, while retaining baked payload values for compatibility. An empty runtime_context is an honest 'no runtime facts landed yet' that self-heals on the next read.",
			},
		},
