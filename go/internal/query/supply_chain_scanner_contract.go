		...
		"response_fields": []map[string]string{
			{
				...
				"notes": "read-time-resolved workloads, services, deployments, environments, per-environment corroboration, and catalog refs (truth_basis: read_time_resolved), populated on findings list and impact explain routes; the transformed investigation packet omits it. Repository mappings remain current repository truth. environment_evidence uses deploy_event or declared from current accepted correlations and can also confirm already-visible finding environment names against exact-subject-digest CI/CD correlations across builder/deployer repository seams; this is artifact deployment context, never repository ownership, and baked evidence values are not copied. environment_evidence_probe exposes the bounded page-weighted confirmation budget and visible-candidate truncation. workload_id, service_id, and environment filters use current active repository mappings; stale baked values cannot satisfy them.",
			},
		},
...
func vulnerabilityScannerRouteContracts() []vulnerabilityScannerRouteContract {
	return []vulnerabilityScannerRouteContract{
		{Name: "impact_findings", Path: "/api/v0/supply-chain/impact/findings", Tool: "list_supply_chain_impact_findings", TruthCapability: supplyChainImpactFindingsCapability, LimitRequired: true, Timeout: "request context; bounded Postgres finding, cloud-runtime, runtime-context, readiness, and Kubernetes authorization reads; Kubernetes graph enrichment performs at most 200 exact-digest reads with at most 32 concurrent", Ordering: "finding_id asc by default; priority sorts tie-break by finding_id", TruncatedField: "truncated plus next_cursor.after_finding_id", MissingEvidence: "readiness.missing_evidence and finding.missing_evidence"},
