...
func parseAdvisoryCatalogKEVOnly(w http.ResponseWriter, r *http.Request) (bool, bool) {
	raw := querycontract.QueryParam(r, "kev")
	if raw == "" {
		return false, true
	}
	switch raw {
	...
	case "false":
		return false, true
	default:
		querycontract.WriteError(w, http.StatusBadRequest, "kev must be true or false")
		return false, false
	}
}
...
func parseAdvisoryCatalogCursor(w http.ResponseWriter, r *http.Request) (float64, string, bool) {
	rawCVSS := querycontract.QueryParam(r, "after_cvss")
	rawKey := querycontract.QueryParam(r, "after_advisory_key")
	if rawCVSS == "" && rawKey == "" {
		return 0, "", true
	}
	if rawCVSS == "" || rawKey == "" {
		querycontract.WriteError(w, http.StatusBadRequest, "after_cvss and after_advisory_key must be provided together")
		return 0, "", false
	}
	cvss, err := strconv.ParseFloat(rawCVSS, 64)
	if err != nil {
		querycontract.WriteError(w, http.StatusBadRequest, "after_cvss must be a number")
		return 0, "", false
	}
