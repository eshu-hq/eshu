...
func (h *SupplyChainHandler) countContainerImageIdentities(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), ContainerImageIdentityAggregateCapability) {
		querycontract.WriteContractError(
			...
			ContainerImageIdentityAggregateCapability,
			h.profile(),
			querycontract.RequiredProfile(ContainerImageIdentityAggregateCapability),
		)
		return
	}
	...
	sourceRepositoryID, ok := h.resolveContainerImageSourceRepositorySelector(w, r, querycontract.QueryParam(r, "source_repository_id"), access, ContainerImageIdentityAggregateCapability)
	if !ok {
		return
	}
...
func (h *SupplyChainHandler) containerImageIdentityInventory(w http.ResponseWriter, r *http.Request) {
	...
	dimension := ContainerImageIdentityInventoryDimension(querycontract.QueryParam(r, "group_by"))
	if dimension == "" {
		dimension = ContainerImageIdentityInventoryByOutcome
	}
	if !isSupportedContainerImageIdentityDimension(dimension) {
		querycontract.WriteError(w, http.StatusBadRequest, "group_by must be one of outcome, identity_strength, repository_id")
		return
	}
