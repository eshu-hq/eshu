	...
	for _, fact := range row.EvidenceFacts {
		...
		if out.VulnerableRange == "" {
			out.VulnerableRange = firstNonEmptyString(
				querycontract.StringVal(fact.Payload, "affected_range"),
				querycontract.StringVal(fact.Payload, "vulnerable_range"),
			)
		}
		if out.AdvisoryID == "" {
			out.AdvisoryID = querycontract.StringVal(fact.Payload, "advisory_id")
		}
		out.References = append(out.References, querycontract.StringSliceVal(fact.Payload, "references")...)
		out.References = append(out.References, querycontract.StringSliceVal(fact.Payload, "reference_urls")...)
		if url := querycontract.StringVal(fact.Payload, "url"); url != "" {
...
func buildSupplyChainComponentExplanation(
	...
	for _, fact := range row.EvidenceFacts {
		...
		if out.ManifestRange == "" {
			...
			case !evidence.Matched:
				...
				out.ManifestRange = firstNonEmptyString(
					querycontract.StringVal(fact.Payload, "dependency_range"),
					querycontract.StringVal(fact.Payload, "manifest_range"),
				)
			}
		}
		if out.ObservedVersion == "" {
			...
			case !evidence.Matched:
				out.ObservedVersion = querycontract.StringVal(fact.Payload, "version")
			}
		}
		if out.PURL == "" {
			switch {
			...
			case !evidence.Matched:
				out.PURL = querycontract.StringVal(fact.Payload, "purl")
			}
		}
	}
	return out
...
func buildSupplyChainVersionExplanation(
	...
	if version.FixedVersion == "" {
		for _, fact := range row.EvidenceFacts {
			if versions := querycontract.StringSliceVal(fact.Payload, "fixed_versions"); len(versions) > 0 {
				version.FixedVersion = versions[0]
				break
			}
			if fixed := querycontract.StringVal(fact.Payload, "fixed_version"); fixed != "" {
...
func buildSupplyChainDependencyChain(
	...
	for _, fact := range facts {
		if len(out.Path) == 0 {
			out.Path = querycontract.StringSliceVal(fact.Payload, "dependency_path")
		}
		if out.Depth == 0 {
			out.Depth = querycontract.IntVal(fact.Payload, "dependency_depth")
		}
...
func buildSupplyChainExplanationAnchors(
	...
	for _, fact := range row.EvidenceFacts {
		...
		appendPathAnchors(&out, querycontract.StringVal(fact.Payload, "relative_path"))
		appendPathAnchors(&out, querycontract.StringVal(fact.Payload, "manifest_path"))
		appendPathAnchors(&out, querycontract.StringVal(fact.Payload, "source_path"))
		appendUniqueString(&out.ImageDigests, querycontract.StringVal(fact.Payload, "digest"))
		appendUniqueString(&out.ImageRefs, querycontract.StringVal(fact.Payload, "image_ref"))
		appendUniqueString(&out.Workloads, querycontract.StringVal(fact.Payload, "workload_id"))
		appendDeploymentAnchors(&out, fact.Payload)
		appendUniqueString(&out.Services, querycontract.StringVal(fact.Payload, "service_id"))
		appendUniqueString(&out.Environments, querycontract.StringVal(fact.Payload, "environment"))
		switch {
		case evidence.Matched && evidence.Err == nil:
		...
		case !evidence.Matched:
			appendLockfilePathAnchors(&out, querycontract.StringVal(fact.Payload, "lockfile_path"))
			appendUniqueString(&out.SBOMDocuments, querycontract.StringVal(fact.Payload, "document_id"))
			appendUniqueString(&out.ImageDigests, querycontract.StringVal(fact.Payload, "subject_digest"))
			appendUniqueString(&out.CatalogEntities, querycontract.StringVal(fact.Payload, "entity_ref"))
			appendUniqueString(&out.CatalogOwners, querycontract.StringVal(fact.Payload, "owner_ref"))
			if out.SubjectDigest == "" {
				out.SubjectDigest = querycontract.StringVal(fact.Payload, "subject_digest")
			}
		}
		if out.RepositoryID == "" {
			out.RepositoryID = querycontract.StringVal(fact.Payload, "repository_id")
		}
...
func providerAlertAnchor(fact SupplyChainImpactEvidenceFact) SupplyChainProviderAlertAnchor {
	...
	return SupplyChainProviderAlertAnchor{
		...
		ManifestPath: firstNonEmptyString(querycontract.StringVal(fact.Payload, "manifest_path"), querycontract.StringVal(fact.Payload, "relative_path")),
	}
}
