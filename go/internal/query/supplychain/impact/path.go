...
func evidenceFactIDsForSemanticHop(
	...
	for _, fact := range row.EvidenceFacts {
		switch hop {
		case "repository":
			if querycontract.StringVal(fact.Payload, "repository_id") != "" {
				factIDs = append(factIDs, fact.FactID)
			}
		case "image":
			if querycontract.StringVal(fact.Payload, "subject_digest") != "" ||
				querycontract.StringVal(fact.Payload, "digest") != "" ||
				querycontract.StringVal(fact.Payload, "image_ref") != "" ||
				querycontract.StringVal(fact.Payload, "artifact_digest") != "" {
				factIDs = append(factIDs, fact.FactID)
			}
		case "workload":
			if querycontract.StringVal(fact.Payload, "workload_id") != "" ||
				payloadEntityKeyHasPrefix(fact.Payload, "workload:") {
				factIDs = append(factIDs, fact.FactID)
			}
		case "deployment":
			if querycontract.StringVal(fact.Payload, "deployment_id") != "" ||
		...
		case "service":
			...
			if querycontract.StringVal(fact.Payload, "service_id") != "" ||
				(fact.FactKind == serviceCatalogCorrelationFactKind &&
					querycontract.StringVal(fact.Payload, "entity_ref") != "") {
				factIDs = append(factIDs, fact.FactID)
			}
		case "environment":
			if querycontract.StringVal(fact.Payload, "environment") != "" {
				factIDs = append(factIDs, fact.FactID)
			}
		}
	}
