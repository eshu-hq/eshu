...
// runtimeFilterLiveServiceCatalogFactKind is fixture content for seeded fact
// rows. It mirrors root's runtimeFilterLiveServiceCatalogFactKind: the filter
// assertions key off scope and repository ids, never off this kind string,
// so the copy is opaque fixture text, not a contract. If the service-catalog
// family renames the kind, update this mirror (rg both names).
const runtimeFilterLiveServiceCatalogFactKind = "reducer_service_catalog_correlation"
...
INSERT INTO scope_generations (
  ...
  status, superseded_at, payload
	...
	insertSupplyChainRuntimeFilterFact(t, ctx, tx, "fact:5747:service:allowed", runtimeFilterLiveScopeA, runtimeFilterLiveGenA,
		runtimeFilterLiveServiceCatalogFactKind, false, map[string]any{
			"repository_id": runtimeFilterLiveRepository,
	...
	insertSupplyChainRuntimeFilterFact(t, ctx, tx, "fact:5747:environment:allowed", runtimeFilterLiveScopeA, runtimeFilterLiveGenA,
		cloudRuntimeProbeTestCICDFactKind, false, map[string]any{
			"repository_id": runtimeFilterLiveRepository,
	...
	insertSupplyChainRuntimeFilterFact(t, ctx, tx, "fact:5747:service:tombstone", runtimeFilterLiveScopeA, runtimeFilterLiveGenA,
		runtimeFilterLiveServiceCatalogFactKind, true, map[string]any{
			"repository_id": runtimeFilterLiveRepository,
