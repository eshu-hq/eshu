...
func assertRuntimeRepositoryPrecedenceHydration(
	...
	hasSelector := (tc.workloadID != "" && querycontract.ContainsAuthString(resolved.WorkloadIDs, tc.workloadID)) ||
		(tc.serviceID != "" && querycontract.ContainsAuthString(resolved.ServiceIDs, tc.serviceID)) ||
		(tc.environment != "" && querycontract.ContainsAuthString(resolved.Environments, tc.environment))
...
func seedSupplyChainRuntimeRepositoryPrecedenceLiveFacts(
	...
	insertSupplyChainRuntimeFilterFact(
		...
		runtimeFilterLiveDecoyRepo,
		runtimeFilterLiveGenDecoy,
		runtimeFilterLiveServiceCatalogFactKind,
		false,
		map[string]any{
			"scope_id":          runtimePrecedenceRawRepository,
