...
func (h *SupplyChainHandler) listSBOMAttachments(w http.ResponseWriter, r *http.Request) {
	...
	if querycontract.CapabilityUnsupported(h.profile(), SBOMAttestationAttachmentsCapability) {
		querycontract.WriteContractError(
			...
			SBOMAttestationAttachmentsCapability,
			h.profile(),
			querycontract.RequiredProfile(SBOMAttestationAttachmentsCapability),
		)
		return
	}
	...
	repositoryID, ok := h.resolveSBOMAttachmentRepositorySelector(w, r, querycontract.QueryParam(r, "repository_id"), access, SBOMAttestationAttachmentsCapability)
	if !ok {
		return
	}
	filter := SBOMAttestationAttachmentFilter{
		...
		AfterAttachmentID:          querycontract.QueryParam(r, "after_attachment_id"),
		Limit:                      limit + 1,
		AllowedSourceRepositoryIDs: access.RepositorySearchIDs(),
	}
	if !filter.HasScope() {
		querycontract.WriteError(w, http.StatusBadRequest, "subject_digest, document_id, document_digest, repository_id, workload_id, or service_id is required")
		return
	}
...
func requiredSBOMAttestationAttachmentLimit(w http.ResponseWriter, r *http.Request) (int, bool) {
	...
	if err != nil || limit <= 0 || limit > SBOMAttestationAttachmentMaxLimit {
		querycontract.WriteError(w, http.StatusBadRequest, fmt.Sprintf("limit must be between 1 and %d", SBOMAttestationAttachmentMaxLimit))
		return 0, false
	}
	return limit, true
}
