...
func (h *Handler) getVulnerabilityScannerReadContract(w http.ResponseWriter, r *http.Request) {
	route := querycontract.QueryParam(r, "route")
	contract := vulnerabilityScannerReadContract()
	if route != "" {
		filtered := contractForScannerRoute(contract, route)
		if len(filtered) == 0 {
			querycontract.WriteError(w, http.StatusBadRequest, "route must be one of impact_findings, impact_count, impact_inventory, impact_explain, security_alert_reconciliations, security_alert_count, security_alert_inventory, scanner_report")
			return
		}
		contract["routes"] = filtered
	}
	querycontract.WriteSuccess(w, r, http.StatusOK, contract, querycontract.BuildTruthEnvelope(
		h.profile(),
...
func vulnerabilityScannerReadContract() map[string]any {
	return map[string]any{
		...
		"filters": []vulnerabilityScannerFilterContract{
			...
			{Name: "workload", Support: "derived", Semantics: []string{"derived", "missing-evidence driven"}, Parameters: []string{"workload_id"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory", "impact_explain"}, Backing: "current active workload-identity/service-catalog repository mapping at read time; stale baked workload_ids do not satisfy the filter"},
			{Name: "service", Support: "derived", Semantics: []string{"derived", "missing-evidence driven"}, Parameters: []string{"service_id"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory", "impact_explain"}, Backing: "current accepted service-catalog repository mapping at read time; stale baked service_ids do not satisfy the filter"},
			{Name: "environment", Support: "derived", Semantics: []string{"derived", "missing-evidence driven"}, Parameters: []string{"environment"}, Routes: []string{"impact_findings", "impact_count", "impact_inventory"}, Backing: "current accepted CI/CD repository mapping at read time; stale baked environments and guessed aliases do not satisfy the filter"},
...
func vulnerabilityScannerRouteContracts() []vulnerabilityScannerRouteContract {
	return []vulnerabilityScannerRouteContract{
		{Name: "impact_findings", Path: "/api/v0/supply-chain/impact/findings", Tool: "list_supply_chain_impact_findings", TruthCapability: ImpactFindingsCapability, LimitRequired: true, Timeout: "request context; bounded Postgres finding, cloud-runtime, runtime-context, readiness, and Kubernetes authorization reads; Kubernetes graph enrichment performs at most 200 exact-digest reads with at most 32 concurrent", Ordering: "finding_id asc by default; priority sorts tie-break by finding_id", TruncatedField: "truncated plus next_cursor.after_finding_id", MissingEvidence: "readiness.missing_evidence and finding.missing_evidence"},
		{Name: "impact_count", Path: "/api/v0/supply-chain/impact/findings/count", Tool: "count_supply_chain_impact_findings", TruthCapability: ImpactAggregateCapability, LimitRequired: false, Timeout: "request context; aggregate read model predicates only", Ordering: "not applicable", TruncatedField: "not applicable", MissingEvidence: "same scope semantics as impact_findings; use list/explain for readiness detail"},
		{Name: "impact_inventory", Path: "/api/v0/supply-chain/impact/inventory", Tool: "get_supply_chain_impact_inventory", TruthCapability: ImpactAggregateCapability, LimitRequired: false, Timeout: "request context; aggregate read model predicates only", Ordering: "count desc then bucket asc", TruncatedField: "truncated plus next_offset", MissingEvidence: "same scope semantics as impact_findings; use list/explain for readiness detail"},
		{Name: "impact_explain", Path: "/api/v0/supply-chain/impact/explain", Tool: "explain_supply_chain_impact", TruthCapability: ImpactExplanationCapability, LimitRequired: false, Timeout: "request context; one finding or one bounded advisory/package/repository/image/workload/service path", Ordering: "one row only; ambiguous scopes return outcome=ambiguous_scope refusal envelopes", TruncatedField: "not applicable", MissingEvidence: "explanation.missing_evidence and readiness.missing_evidence"},
		...
		{Name: "scanner_report", Path: "eshu vuln-scan repo --json", TruthCapability: ImpactFindingsCapability, LimitRequired: true, Timeout: "CLI/API request context; report is assembled from impact_findings readiness", Ordering: "same as impact_findings", TruncatedField: "report.summary.truncated mirrors API truncated", MissingEvidence: "report.readiness and report.scope_plan missing_evidence"},
	}
}
