...
// BuildEvidenceCitationPacket derives a visualization packet from
...
func BuildEvidenceCitationPacket(response querycontract.EvidenceCitationResponse, truth *querycontract.TruthEnvelope) Packet {
	...
	builder := newVisualizationBuilder(ViewEvidenceCitation, strings.TrimSpace(response.Question))
	...
	for _, citation := range response.Citations {
		...
		builder.AddNode(Node{
			...
			Category:       querycontract.FirstNonEmptyString(citation.EvidenceFamily, family),
...
// BuildIncidentContextPacket derives a visualization packet from an
...
func BuildIncidentContextPacket(response model.IncidentContextResponse, truth *querycontract.TruthEnvelope) Packet {
	...
	builder := newVisualizationBuilder(ViewIncidentContext, incidentVisualizationTitle(response.Incident))
	builder.SetTruth(truth)
...
func addIncidentAnchorNode(builder *visualizationBuilder, incident model.IncidentContextIncident) string {
...
func incidentVisualizationAnchor(incident model.IncidentContextIncident) string {
...
func incidentVisualizationTitle(incident model.IncidentContextIncident) string {
