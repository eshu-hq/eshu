...
// workitemHandlerTracer is this package's tracer AND the seam its span tests
...
// go/internal/query/incident/handler.go for the identical seam (also used by
// the sibling #6642 language move's language/handler_tracing.go).
var workitemHandlerTracer = queryspan.HandlerTracer()
...
func startQueryHandlerSpan(r *http.Request, spanName, route, capability string) (*http.Request, trace.Span) {
	return queryspan.StartHandlerSpanWith(workitemHandlerTracer, r, spanName, route, capability)
}
