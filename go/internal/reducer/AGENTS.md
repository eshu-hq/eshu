...
Performance Evidence: measured, not assumed — `go test ./internal/reducer/ -run ^$ -bench 'BenchmarkBestSupplyChainImageIdentitiesByDigest(Bare|Consensus)$' -benchmem -benchtime=2s -count=5` (`supply_chain_impact_anchor_consensus_bench_test.go`, Apple M1 Max), 50 digests x 20 rows = 1000 `reducer_container_image_identity` envelopes, wider than the live corpus's single digest with 20 rows: pre-#5887 baseline (`bareBestSupplyChainImageIdentitiesByDigest`, the real retained `preferSupplyChainImageIdentity` bare fold) averages 293us/op, 154.7KB/op, 3059 allocs/op; the #5887 fix (`bestSupplyChainImageIdentitiesByDigest` with the consensus pre-pass) averages 600us/op, 305.3KB/op, 6068 allocs/op — roughly 2.0x time and allocations, not negligible in relative terms. In absolute terms this stage runs once per reducer intent's index build (not per edge, not in a nested loop), so 600 microseconds for 1000 envelopes is far below the Postgres round-trips the same intent already performs (reducerBatchInsertFacts, active-evidence reads); this call site (`buildSupplyChainImpactIndexWithQuarantine`, `supply_chain_impact.go:252`) is the only real caller today. The added cost is bounded because `buildSupplyChainImageIdentityConsensus` is one more single O(N) linear pass with map inserts over the same envelope batch the surrounding loop already iterates — no nested loop over digests or rows, so the 2x factor holds at any N rather than growing with corpus size.
