...
No-Observability-Change: pure in-memory reducer computation — `buildSupplyChainImageIdentityConsensus` is one more O(N) pass over the same envelope batch `bestSupplyChainImageIdentitiesByDigest`/`buildSupplyChainImpactIndexWithQuarantine` already iterate, using a `map[struct]int` bounded by rows-per-digest (~20 in the real corpus). No new metric instrument, metric label, span, structured log field, queue domain, worker, lease, or runtime knob. No Cypher or Postgres path touched. The `supply_chain_impact` domain remains covered by the existing `eshu_dp_reducer_executions_total`, `eshu_dp_reducer_run_duration_seconds`, and `eshu_dp_supply_chain_impact_findings_total` instruments.
