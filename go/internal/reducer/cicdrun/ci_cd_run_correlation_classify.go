...
func classifyCICDRunEvidence(ev *cicdRunEvidence, imageIndex map[string][]cicdImageIdentity) CICDRunCorrelationDecision {
	...
	decision := CICDRunCorrelationDecision{
		...
		RunAttempt:       defaultCICDRunAttempt(payloadcore.DerefString(run.RunAttempt)),
	...
	if env, factID, ok := classifyCICDDeploymentEventEnvironment(ev.deploymentEvents); ok {
		...
		decision.EnvironmentEvidence = reducercontract.CICDRunCorrelationEnvironmentEvidenceDeployEvent
		...
		decision.EnvironmentEvidence = reducercontract.CICDRunCorrelationEnvironmentEvidenceDeclared
	}
