...
// strPtr returns a pointer to s, for constructing the optional (pointer)
// fields cicdrunv1's typed decode produces.
...
func TestClassifyRunEvidenceFallsBackWhenDeploymentEnvironmentBlank(t *testing.T) {
	...
	decisions, _, _, _ := buildCICDRunCorrelationDecisionsWithQuarantine([]facts.Envelope{
		{
			FactID:   "run",
