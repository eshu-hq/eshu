...
// (payloadcore.PayloadString in go/internal/reducer/payloadcore/payload.go). The typed
...
// pre-migration payloadcore.PayloadString-trimmed behavior. It FAILS on the raw-typed HEAD
...
func TestBuildCICDRunCorrelationDecisionsTrimsIdentityFieldsLikePayloadString(t *testing.T) {
	...
	// pre-migration payloadcore.PayloadString behavior and keeping the persisted reducer
	// fact identity stable regardless of collector-side whitespace.
	if got.Provider != "github_actions" {
		t.Fatalf("Provider = %q, want %q (trimmed like the old payloadcore.PayloadString path)", got.Provider, "github_actions")
	}
	if got.RunID != "run-1" {
		t.Fatalf("RunID = %q, want %q (trimmed like the old payloadcore.PayloadString path)", got.RunID, "run-1")
	}
	if got.RunAttempt != "1" {
		t.Fatalf("RunAttempt = %q, want %q (trimmed like the old payloadcore.PayloadString path)", got.RunAttempt, "1")
	}
	if got.RepositoryID != "repo-api" {
		t.Fatalf("RepositoryID = %q, want %q (trimmed like the old payloadcore.PayloadString path)", got.RepositoryID, "repo-api")
	}
...
// fields the pre-migration path also read through the trimming payloadcore.PayloadString.
...
func TestBuildCICDRunCorrelationDecisionsTrimsArtifactAndWorkflowImageEvidence(t *testing.T) {
	...
	t.Run("padded environment surfaces trimmed", func(t *testing.T) {
		...
		if got.Environment != "prod" {
			t.Fatalf("Environment = %q, want %q (trimmed like the old payloadcore.PayloadString path)", got.Environment, "prod")
		}
	})
