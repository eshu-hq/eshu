...
No-Regression Evidence: `go test ./internal/reducer -run TestCodeCallResolutionMethodElixirAliasImportBinding -count=1` failed before `alias Demo.Basic; Basic.greet()` could resolve across files, then passed after the Elixir resolver bound parser-emitted alias rows to the prescan `imports_map` module path and resolved the module-qualified function as `import_binding`. `go test ./internal/resolutionparity -run TestGoldenCallGraphCorrectnessHarness/elixir_import_binding -count=1` failed before the source-backed fixture emitted the expected edge, then passed with the exact `Demo.Basic.greet` target while a same-name `Demo.Other.greet` decoy stayed unselected.
