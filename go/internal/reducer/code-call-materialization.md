...
No-Regression Evidence: `go test ./internal/resolutionparity -run TestGoldenCallGraphCorrectnessHarness/typescript_import_binding -count=1` failed before direct unqualified TypeScript imports were considered before repo-wide fallback, reporting `repo_unique_name` instead of `import_binding`, then passed after JavaScript-family unqualified imports used parser import rows before weak repo fallback. `go test ./internal/reducer/code/call -run TestExtractCodeCallRowsBlocksTypeScriptDirectImportFallbackToRepoUnique -count=1` failed before unresolved direct imports could block an unrelated repo-unique helper, then passed after parser-proven import bindings returned unresolved instead of falling through to weak repo fallback. `go test ./internal/reducer ./internal/reducer/code/call ./internal/reducer/code/call/python ./internal/reducer/code/call/shared -run 'TypeScript|Import|ReExport' -count=1`, `go test ./internal/reducer -count=1`, and `go test ./internal/resolutionparity -count=1` prove existing TypeScript interface, baseUrl, namespace import, and static re-export behavior still holds.
...
## Python import-bound fallback precision (issue #3198)

No-Regression Evidence: `go test ./internal/reducer/code/call -run TestCallResolutionGoldensPython -count=1 -v` failed before the fix with the Python #3156 corpus still resolving `from third_party.db import connect; connect()` to an unrelated local `connect` via `repo_unique_name`, and still labeling `from pkg import handler; handler()` as `repo_unique_name` instead of the explicit `import_binding` tier. After the fix, `go test ./internal/reducer/code/call -run 'TestCallResolutionGoldensPython|TestExtractCodeCallRows(PrefersPythonAliasedImportBeforeRepoFallback|ResolvesPythonModuleAndFromImports|BlocksTypeScriptDirectImportFallbackToRepoUnique)' -count=1 -v` passed, proving Python import-map symbol evidence resolves re-exported local targets before weak fallback, unresolved explicit imports block weak repo-wide guesses, and existing TypeScript fallback blocking still holds. `go test ./internal/resolutionparity -count=1` passed after the checked-in tier golden moved the Python source corpus from weak `repo_unique_name` rows to `import_binding`.
...
## Elixir alias import resolution (issue #3006)

No-Regression Evidence: `go test ./internal/reducer/code/call -run TestCodeCallResolutionMethodElixirAliasImportBinding -count=1` failed before `alias Demo.Basic; Basic.greet()` could resolve across files, then passed after the Elixir resolver bound parser-emitted alias rows to the prescan `imports_map` module path and resolved the module-qualified function as `import_binding`. `go test ./internal/resolutionparity -run TestGoldenCallGraphCorrectnessHarness/elixir_import_binding -count=1` failed before the source-backed fixture emitted the expected edge, then passed with the exact `Demo.Basic.greet` target while a same-name `Demo.Other.greet` decoy stayed unselected.

No-Observability-Change: Elixir alias import resolution is an in-memory resolver branch over existing parser import rows, repository prescan import maps, and the existing code entity index. It adds no graph query, graph write shape, queue table, worker, lease, batch setting, runtime knob, metric instrument, metric label, span, route, or log key. Operators still diagnose code-call extraction through the existing `code call materialization completed` log fields and reducer execution spans/counters.

## Dart URI import resolution (issue #3006)

No-Regression Evidence: `go test ./internal/reducer/code/call ./internal/reducer/code/call/shared -run 'TestCodeCall.*Dart|Dart.*Import' -count=1 -v` failed before Dart URI imports could bind a bare call to the imported file and before unresolved imported calls blocked weak repository fallback, then passed after the Dart resolver used parser import rows plus the prescan `imports_map` to resolve one imported target as `import_binding`. The focused gate also proves uppercase class constructor calls do not fall through to a repo-unique decoy when their lowercase Dart import file cannot be resolved, export-only Dart directives do not resolve as lexical imports, package imports resolve only under the caller package root, and external package imports do not bind to coincidentally named local files. `go test ./internal/parser/dart -run TestParseCapturesDartBuckets -count=1 -v` failed before Dart parser import rows identified `library_import` versus `library_export`, then passed after rows carried `import_type=import` or `import_type=export`. `go test ./internal/resolutionparity -run TestGoldenCallGraphCorrectnessHarness/dart_import_binding -count=1 -v` proves the source-backed parser plus reducer path emits the exact imported class-constructor edge while a same-name decoy file stays unselected.
...
## Haskell qualified import resolution (issue #3108)

No-Regression Evidence: `go test ./internal/reducer/code/call -run 'Haskell.*Import' -count=1` failed before `import qualified Data.Text as T; T.pack value` could resolve across files, then passed after the Haskell resolver bound parser-emitted qualified import aliases to the prescan `imports_map` module path and resolved the call as `import_binding`. The same focused gate failed before duplicate qualified aliases such as `import qualified Data.Text as T` plus `import qualified Other.Text as T` left `T.pack` unresolved, then passed after the resolver gathered all matching imported modules and required exactly one owner before materializing a CALLS row. `go test ./internal/resolutionparity -run TestGoldenCallGraphCorrectnessHarness/haskell_import_binding -count=1` failed before the source-backed fixture emitted the expected edge, then passed with the exact `Data.Text.pack` target while a same-name `Other.Text.pack` decoy stayed unselected.

No-Observability-Change: Haskell qualified import resolution and duplicate-alias ambiguity handling are in-memory resolver branches over existing parser import rows, repository prescan import maps, and the existing code entity index. They add no graph query, graph write shape, queue table, worker, lease, batch setting, runtime knob, metric instrument, metric label, span, route, or log key. Operators still diagnose code-call extraction through the existing `code call materialization completed` log fields and reducer execution spans/counters.
