...
func TestExtractCodeCallRowsResolvesCrossRepoExportedGoFunction(t *testing.T) {
	...
	for _, row := range rows {
		if payloadcore.AnyToString(row["callee_entity_id"]) == "content-entity:repoa-process" {
			if got, want := payloadcore.AnyToString(row["caller_entity_id"]), "content-entity:repob-main"; got != want {
				t.Fatalf("caller_entity_id = %q, want %q", got, want)
			}
			if got, want := payloadcore.AnyToString(row["callee_file"]), "pkg/process.go"; got != want {
...
func TestExtractCodeCallRowsLeavesCrossRepoUnexportedGoFunctionUnresolved(t *testing.T) {
	...
	_, rows := ExtractRows(envelopes)
	for _, row := range rows {
		if payloadcore.AnyToString(row["callee_entity_id"]) == "content-entity:repoa-process-unexported" {
...
func TestExtractCodeCallRowsLeavesAmbiguousCrossRepoExportUnresolved(t *testing.T) {
	...
	_, rows := ExtractRows(envelopes)
	for _, row := range rows {
		callee := payloadcore.AnyToString(row["callee_entity_id"])
...
func TestExtractCodeCallRowsCrossRepoExportIsReprojectionStable(t *testing.T) {
	...
	calleeFor := func(rows []map[string]any) string {
		for _, row := range rows {
			if payloadcore.AnyToString(row["caller_entity_id"]) == "content-entity:repob-main" &&
				payloadcore.AnyToString(row["resolution_method"]) == codeprovenance.MethodCrossRepoExportPackage {
				return payloadcore.AnyToString(row["callee_entity_id"])
			}
		}
		return ""
	}

	_, firstRows := ExtractRows(build())
	_, secondRows := ExtractRows(build())
