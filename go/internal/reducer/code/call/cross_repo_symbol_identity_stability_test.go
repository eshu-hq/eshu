...
func crossRepoIdentityForCaller(t *testing.T, rows []map[string]any, callerEntityID string) crossRepoIdentity {
	...
	for _, row := range rows {
		if payloadcore.AnyToString(row["caller_entity_id"]) != callerEntityID {
			continue
		}
		method := payloadcore.AnyToString(row["resolution_method"])
		return crossRepoIdentity{
			calleeEntityID:   payloadcore.AnyToString(row["callee_entity_id"]),
...
func stampGeneration(envelopes []facts.Envelope, generationID string) []facts.Envelope {
	for i := range envelopes {
		...
		envelopes[i].FactID = "fact:" + generationID + ":" + payloadcore.AnyToString(envelopes[i].Payload["repo_id"])
...
func TestExtractCodeCallRowsCrossRepoSCIPSymbolIdentityStableAcrossGenerations(t *testing.T) {
	...
	_, firstRows := ExtractRows(stampGeneration(build(), "gen-1"))
	_, secondRows := ExtractRows(stampGeneration(build(), "gen-2"))
