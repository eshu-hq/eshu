...
func assertCodeCallRowWithKind(
	...
	for _, row := range rows {
		if payloadcore.AnyToString(row["caller_entity_id"]) == callerID &&
			payloadcore.AnyToString(row["callee_entity_id"]) == calleeID &&
			payloadcore.AnyToString(row["call_kind"]) == callKind {
			if got := payloadcore.AnyToString(row["relationship_type"]); got != "REFERENCES" {
