...
func goInterfaceReceiverChainGoldenEdges(rows []map[string]any) []goldenaudit.Edge {
	...
	for _, row := range rows {
		if payloadcore.AnyToString(row["caller_entity_id"]) != "content-entity:execute" ||
			payloadcore.AnyToString(row["callee_entity_id"]) != "content-entity:get-action-instance" {
			continue
		}
		relationshipType := payloadcore.AnyToString(row["relationship_type"])
		if relationshipType == "" {
