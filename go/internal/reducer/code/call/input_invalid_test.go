...
func TestDecodeCodegraphTreatsAbsentSchemaVersionAsLatestMajor(t *testing.T) {
	...
	file, err := schemadecode.DecodeCodegraphFile(validFile)
	if err != nil {
	...
	repo, err := schemadecode.DecodeCodegraphRepository(validRepo)
	if err != nil {
	...
	if _, err := schemadecode.DecodeCodegraphFile(missingRepoID); err == nil {
		t.Fatalf("decodeCodegraphFile(missing repo_id) error = nil, want an input_invalid *factDecodeError")
	} else {
		q, quarantinable, fatal := factdecode.PartitionDecodeFailures(missingRepoID, err)
		if !quarantinable {
