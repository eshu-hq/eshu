...
// relative_path while extractPythonMetaclassRowsWithIndex (and BuildEntityIndex)
...
// (ExtractAllRelationshipRowsWithIndex) and drops the malformed file from
...
func TestExtractAllCodeRelationshipRowsQuarantinesFileFromBothExtractors(t *testing.T) {
	...
	_, _, _, metaclassRows, _, quarantined := ExtractAllRelationshipRowsWithIndex([]facts.Envelope{malformed, valid})
