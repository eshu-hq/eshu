...
func resolutionMethodForCallee(t *testing.T, rows []map[string]any, calleeID string) string {
	...
	for _, row := range rows {
		if payloadcore.AnyToString(row["callee_entity_id"]) == calleeID {
			return payloadcore.AnyToString(row["resolution_method"])
		}
	}
...
func TestPythonMetaclassRowsCarryDeclaredResolutionMethod(t *testing.T) {
	...
	_, _, _, metaclassRows := ExtractAllRelationshipRows(envelopes)
	if len(metaclassRows) != 1 {
		t.Fatalf("len(metaclassRows) = %d, want 1", len(metaclassRows))
	}
	if got := payloadcore.AnyToString(metaclassRows[0]["resolution_method"]); got != codeprovenance.MethodDeclared {
