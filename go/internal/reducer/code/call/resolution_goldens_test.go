...
// exercising the reducer call-resolution path through ExtractRows. The
...
func runCallResolutionGoldens(t *testing.T, goldens []callResolutionGolden) {
	...
	for _, g := range goldens {
		...
		t.Run(string(g.category)+"/"+g.name, func(t *testing.T) {
			...
			_, rows := ExtractRows(g.envelopes)

			resolvedForbidden := func(target string) (string, bool) {
				for _, row := range rows {
					if payloadcore.AnyToString(row["callee_entity_id"]) == target {
						return payloadcore.AnyToString(row["resolution_method"]), true
					}
				}
				return "", false
			}
			...
			for _, row := range rows {
				if payloadcore.AnyToString(row["callee_entity_id"]) == g.wantCallee {
					gotMethod, found = payloadcore.AnyToString(row["resolution_method"]), true
					break
				}
			}
			if !found {
