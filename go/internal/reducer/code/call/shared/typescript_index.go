...
func addTypeScriptClassMethodCandidate(
	...
	className := strings.TrimSpace(payloadcore.AnyToString(item["class_context"]))
	methodName := strings.TrimSpace(payloadcore.AnyToString(item["name"]))
...
func addTypeScriptInterfaceImplementer(
	...
	className := strings.TrimSpace(payloadcore.AnyToString(item["name"]))
	if className == "" {
		return
	}
	interfaces := MetadataStringSlice(item, "implemented_interfaces")
	if len(interfaces) == 0 {
		return
	}
...
func addTypeScriptInterfaceDeclaration(
	...
	interfaceName := strings.TrimSpace(payloadcore.AnyToString(item["name"]))
	if interfaceName == "" {
		return
	}
...
func codeCallTypeScriptEntity(item map[string]any) bool {
	switch strings.TrimSpace(payloadcore.AnyToString(item["lang"])) {
	case "typescript", "tsx":
		return true
	default:
		return false
