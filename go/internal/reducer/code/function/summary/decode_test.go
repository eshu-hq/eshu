...
// payload shapes decode through the typed contracts seam into parsed.Effects
...
func TestCodeFunctionSummaryEffectsMapsAllFields(t *testing.T) {
	...
	if id != parsed.FunctionID("repo-1:pkg::view") {
	...
	if len(effects.ParamToCallArg) != 1 || effects.ParamToCallArg[0].Callee != parsed.FunctionID("repo-1:pkg::query") || effects.ParamToCallArg[0].Arg != 1 {
...
// becomes a parsed.FunctionID, so the durable summary keys its callee edge on
...
func TestCodeFunctionSummaryEffectsTrimsCallee(t *testing.T) {
	...
	if len(effects.ParamToCallArg) != 1 || effects.ParamToCallArg[0].Callee != parsed.FunctionID("repo-1:pkg::query") {
...
// error (routed to an input_invalid dead-letter by ExtractEffects), rather
...
func TestCodeFunctionGraphIDMapsFields(t *testing.T) {
	...
	if err != nil || !ok || id != parsed.FunctionID("repo-1:pkg::view") || uid != "uid:view-fn" {
