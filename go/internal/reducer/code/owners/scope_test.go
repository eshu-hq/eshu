...
// fail to force a whole-repository retract for a location the collector now
// emits facts for, or vice versa (issue #5419). Order is significant because
// it is GitHub's location precedence, so the comparison is exact including
// order.
func TestCodeownersOwnershipCandidatePathsMatchCollector(t *testing.T) {
	...
	if !reflect.DeepEqual(got, want) {
		t.Fatalf(
			"owners candidatePaths = %v, want collector codeowners.CandidatePaths() = %v — "+
