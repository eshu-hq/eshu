...
// coupled, not just co-located: [EvidenceHandler]
// decodes through the same typed-contracts seam
// (DecodeEvidenceInput/DecodeInterprocEvidenceInput,
// ExtractEvidenceRowsWithQuarantine/
// ExtractInterprocEvidenceRowsWithQuarantine) as
// [InterprocEvidenceHandler], and the sibling value
// package's value-flow fixpoint solver (code/value/fixpoint_evidence_loader.go)
// composes [InterprocEvidenceInput] and
// [ExtractInterprocFixpointEvidenceRows] directly. Moving either family
// alone would just relocate a two-package import cycle back into the
// reducer root.
//
// [EvidenceHandler] and
// [InterprocEvidenceHandler] each: load raw fact
...
// [ProjectedNodeLedger] or [InterprocProjectedEdgeLedger]
...
//
// [ProjectedNodeBackfiller] and
// [InterprocProjectedEdgeBackfiller] are one-time, idempotent startup
