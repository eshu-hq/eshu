...
// DecodeEvidenceInput decodes one code_taint_evidence envelope
// through the contracts seam (schemadecode.DecodeCodeTaintEvidence) and projects it into
// the reducer's EvidenceInput row, RETURNING the decode error rather
...
// *factDecodeError so the caller (ExtractEvidenceRowsWithQuarantine)
...
// root.
func DecodeEvidenceInput(envelope facts.Envelope) (EvidenceInput, error) {
...
// DecodeInterprocEvidenceInput decodes one code_interproc_evidence
// envelope through the contracts seam (schemadecode.DecodeCodeInterprocEvidence) and
// projects it into the reducer's InterprocEvidenceInput row, RETURNING the
// decode error so the caller (ExtractInterprocEvidenceRowsWithQuarantine)
...
// as DecodeEvidenceInput. Exported for the same reason.
func DecodeInterprocEvidenceInput(envelope facts.Envelope) (InterprocEvidenceInput, error) {
	evidence, err := schemadecode.DecodeCodeInterprocEvidence(envelope)
	if err != nil {
		return InterprocEvidenceInput{}, err
	}
...
// ExtractEvidenceRowsWithQuarantine decodes each code_taint_evidence
...
func ExtractEvidenceRowsWithQuarantine(envelopes []facts.Envelope) ([]map[string]any, []factdecode.QuarantinedFact, error) {
	inputs := make([]EvidenceInput, 0, len(envelopes))
	var quarantined []factdecode.QuarantinedFact
	for _, env := range envelopes {
		if env.IsTombstone {
			continue
		}
		input, err := DecodeEvidenceInput(env)
		if err != nil {
	...
	return ExtractEvidenceRows(inputs), quarantined, nil
}

// ExtractInterprocEvidenceRowsWithQuarantine is the interproc counterpart
// of ExtractEvidenceRowsWithQuarantine: it decodes each
...
func ExtractInterprocEvidenceRowsWithQuarantine(envelopes []facts.Envelope) ([]map[string]any, []factdecode.QuarantinedFact, error) {
	inputs := make([]InterprocEvidenceInput, 0, len(envelopes))
	var quarantined []factdecode.QuarantinedFact
	for _, env := range envelopes {
		if env.IsTombstone {
			continue
		}
		input, err := DecodeInterprocEvidenceInput(env)
		if err != nil {
			q, isQuarantine, fatal := factdecode.PartitionDecodeFailures(env, err)
			if !isQuarantine {
				return nil, nil, fatal
	...
	return ExtractInterprocEvidenceRows(inputs), quarantined, nil
}
