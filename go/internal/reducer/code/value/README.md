...
| `GraphCloudSinkTargetLoader` / `CloudSinkWorkloadRowsCypher` / `CloudSinkTargetsByPairCypher` / `CloudSinkTarget` | the graph-backed cloud sink target loader and its two pinned statements: raw (function, action, workload) rows, a Go-side single-workload check, then an `UNWIND $pairs` single-hop chain to the reachable cloud resources that also returns each function's current workloads, so a pair whose `RUNS_IN` changed between the two reads is dropped (#6690). The backend-conformance corpus asserts on both statements by equality. The loader logs `value-flow cloud sink targets loaded` with the multi-workload, unresolved and revalidation pair drop counts |
