...
func TestExtractCodeCallRowsResolvesKotlinSuspendFunctionCallsUsingInferredObjectType(t *testing.T) {
	t.Parallel()
...
`)
	...
	if functions, ok := callerPayload["functions"].([]map[string]any); ok {
		for _, function := range functions {
			...
			switch {
			case name == "usage":
				suspendFlag, ok := function["suspend"].(bool)
				if !ok {
					t.Fatalf(`functions["usage"]["suspend"] = %T, want bool`, function["suspend"])
				}
				if !suspendFlag {
					t.Fatalf(`functions["usage"]["suspend"] = false, want true`)
				}
				function["end_line"] = 9
