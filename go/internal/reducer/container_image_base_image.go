...
func dockerfileStageBaseRef(stage map[string]any) (string, bool) {
	image := strings.TrimSpace(payloadStr(stage, "base_image"))
	...
	if tag := strings.TrimSpace(payloadStr(stage, "base_tag")); tag != "" {
...
func dockerfileStageAlias(stage map[string]any) string {
	return strings.TrimSpace(payloadStr(stage, "alias"))
}
