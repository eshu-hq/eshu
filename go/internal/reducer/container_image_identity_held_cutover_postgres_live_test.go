...
func TestPostgresContainerImageIdentityHeldCanonicalStillCreatesCutover(t *testing.T) {
	...
	if err := reducerBatchInsertFacts(
		ctx,
		db,
		[]reducerFactRow{containerImageIdentityLegacyLiveRow(heldLegacyID, 1, false)},
	); err != nil {
	...
	assertContainerImageIdentityLegacyStatementRejected(
		t,
		reducerBatchInsertFacts(
			ctx,
			db,
			[]reducerFactRow{containerImageIdentityLegacyLiveRow(lateLegacyID, 2, false)},
		),
	)
