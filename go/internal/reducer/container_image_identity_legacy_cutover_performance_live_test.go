...
func TestContainerImageIdentityLegacyCutoverPerformanceLive(t *testing.T) {
	...
	scenarios := []containerImageIdentityLegacyPerfScenario{
		{name: "all_insert_representative", rows: reducerFactBatchSize, iterations: 10},
		{name: "all_conflict_representative", rows: reducerFactBatchSize, iterations: 10, preseedFraction: 1},
		{name: "mixed_representative", rows: reducerFactBatchSize, iterations: 10, preseedFraction: 0.5},
		{name: "unrelated_representative", rows: reducerFactBatchSize, iterations: 10, unrelated: true},
	...
	for _, scenario := range scenarios {
		...
		maxTriggerCalls := int64(2 * ((scenario.rows + reducerFactBatchSize - 1) / reducerFactBatchSize))
...
func explainContainerImageIdentityLegacyPerf(
	...
	for start := 0; start < len(rows); start += reducerFactBatchSize {
		end := min(start+reducerFactBatchSize, len(rows))
		var raw []byte
		if err := db.QueryRowContext(
			ctx,
			"EXPLAIN (ANALYZE, BUFFERS, WAL, FORMAT JSON) "+reducerFactBatchInsertQuery,
			reducerFactChunkArgs(rows[start:end])...,
		).Scan(&raw); err != nil {
			t.Fatalf("explain legacy writer chunk %d: %v", start/reducerFactBatchSize, err)
		}
		var plans []containerImageIdentityLegacyExplain
		if err := json.Unmarshal(raw, &plans); err != nil {
			t.Fatalf("decode legacy writer EXPLAIN chunk %d: %v", start/reducerFactBatchSize, err)
		}
		if len(plans) != 1 {
			t.Fatalf(
				"legacy writer EXPLAIN chunk %d plans = %d, want 1",
				start/reducerFactBatchSize,
				len(plans),
			)
		}
...
func proveContainerImageIdentityLegacyPerfContention(
	...
	go func() {
		targetDone <- reducerBatchInsertFacts(ctx, db, targetRows)
	}()
	select {
	case err := <-targetDone:
