...
func TestCIRunDigestBuildProvenanceLetsCorrelationEscapeProvenanceOnly(t *testing.T) {
	...
	// decisions for this digest share one fact ID, and factwrite.BatchInsertFacts
	...
	decisions := cicdrun.BuildCICDRunCorrelationDecisions(envelopes)
	if len(decisions) != 1 {
		t.Fatalf("decisions = %d, want 1: %#v", len(decisions), decisions)
	}
	decision := decisions[0]
	if decision.Outcome != cicdrun.CICDRunCorrelationExact {
		t.Fatalf("Outcome = %q (%s), want %q", decision.Outcome, decision.Reason, cicdrun.CICDRunCorrelationExact)
	}
