		...
		},
		{
			...
			decisions: []ContainerImageIdentityDecision{
				...
				func() ContainerImageIdentityDecision {
					...
					d.Outcome = reducercontract.ContainerImageIdentityTagResolved
					return d
				}(),
			},
			want: nil,
		},
		{
			...
			decisions: []ContainerImageIdentityDecision{
				func() ContainerImageIdentityDecision {
					...
					d.Outcome = reducercontract.ContainerImageIdentityAmbiguousTag
					return d
				}(),
				base("sha256:base", "repo-a"),
			},
...
func TestProjectContainerImageDerivedFromEdgesNoOpWithoutWriter(t *testing.T) {
	...
	if err := h.projectContainerImageDerivedFromEdges(
		context.Background(), reducercontract.Intent{ScopeID: "scope-1", GenerationID: "gen-1"}, nil,
	); err != nil {
...
func TestProjectContainerImageDerivedFromEdgesRetractsFirstThenWrites(t *testing.T) {
	...
	decisions := []ContainerImageIdentityDecision{
		{Digest: "sha256:child", SourceRepositoryIDs: []string{"repository:repo-1"}, BuildProvenanceRepositoryIDs: []string{"repository:repo-1"}, Outcome: reducercontract.ContainerImageIdentityExactDigest},
		{Digest: "sha256:base", BaseImageForRepositoryIDs: []string{"repository:repo-1"}, Outcome: reducercontract.ContainerImageIdentityExactDigest},
	}

	if err := h.projectContainerImageDerivedFromEdges(
		context.Background(), reducercontract.Intent{ScopeID: "git-repository-scope:repository:repo-1", GenerationID: "gen-1"}, decisions,
	); err != nil {
...
func TestProjectContainerImageDerivedFromEdgesNonRepoScopeWritesNothing(t *testing.T) {
	...
	if err := h.projectContainerImageDerivedFromEdges(
		context.Background(),
		reducercontract.Intent{ScopeID: "oci_registry:ghcr.io:team:api", GenerationID: "gen-1"},
		decisions,
	); err != nil {
		t.Fatalf("projectContainerImageDerivedFromEdges returned error: %v", err)
	}
...
func TestProjectContainerImageDerivedFromEdgesRetractsEvenWhenNoRowsToWrite(t *testing.T) {
	...
	if err := h.projectContainerImageDerivedFromEdges(
		context.Background(), reducercontract.Intent{ScopeID: "scope-1", GenerationID: "gen-2"}, nil,
	); err != nil {
		t.Fatalf("projectContainerImageDerivedFromEdges returned error: %v", err)
	}
	if len(writer.retractCalls) != 1 {
...
func TestProjectContainerImageDerivedFromEdgesPropagatesWriterErrors(t *testing.T) {
	...
	intent := reducercontract.Intent{ScopeID: "git-repository-scope:repository:repo-1", GenerationID: "gen-1"}
...
func TestContainerImageDerivedFromRowsStayEmptyForCIRunScope(t *testing.T) {
	...
	owningRepositoryID := payloadcore.RepositoryIDFromReducerScope(ciScopeID)
	if owningRepositoryID != "" {
		t.Fatalf("payloadcore.RepositoryIDFromReducerScope(%q) = %q, want empty: a CI-run scope owns no repository", ciScopeID, owningRepositoryID)
	}
