...
// factload.LoadFactsForKinds, which also keeps quarantine accounting for a malformed
...
func (h ContainerImageIdentityHandler) loadActiveContainerImageCIFacts(
	...
	ownerRepositoryID := payloadcore.RepositoryIDFromReducerScope(scopeID)
...
func filterContainerImageCIFactsForOwner(
	...
	// redundant part), and schemadecode.DecodeCICDRun is a pure function of the immutable
	...
	for _, envelope := range envelopes {
		...
		if cicdrun.TrimmedCICDPtr(run.RepositoryID) != ownerRepositoryID {
			continue
		}
...
// cicdRunScopeKey extends cicdrun.CICDRunKeyFromParts with the fact's own scope_id
...
// cicdrun.CICDRunKeyFromParts key. Every other cicdrun.CICDRunKeyFromParts caller
...
// factload.LoadFactsForKinds, or this function's already-filtered output), so the
...
func cicdRunScopeKey(scopeID, provider, runID string, runAttempt *string) string {
	key := cicdrun.CICDRunKeyFromParts(provider, runID, runAttempt)
	if key == "" {
		return ""
	}
...
// scope-local ci.run/ci.artifact facts (loaded via factload.LoadFactsForKinds) overlap
