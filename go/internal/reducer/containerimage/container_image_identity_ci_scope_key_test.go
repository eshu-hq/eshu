...
// provider/run_id/run_attempt alone (cicdrun.CICDRunKeyFromParts). Two INDEPENDENT
...
func TestFilterContainerImageCIFactsForOwnerRunKeyIncludesFactScope(t *testing.T) {
	...
	for _, envelope := range kept {
		...
		if err != nil {
			t.Fatalf("schemadecode.DecodeCICDArtifact(%q) error = %v", envelope.FactID, err)
		}
		keptDigests[cicdrun.TrimmedCICDPtr(artifact.ArtifactDigest)] = true
	}
