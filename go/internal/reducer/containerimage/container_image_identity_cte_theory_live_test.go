...
func TestContainerImageIdentitySingleStatementCleanupTheoryLive(t *testing.T) {
	...
	if err := factwrite.ExecChunk(ctx, db, currentRows); err != nil {
		t.Fatalf("seed current-path logical rows: %v", err)
	}
	if err := factwrite.ExecChunk(ctx, db, candidateRows); err != nil {
...
func executeContainerImageIdentityCurrentTheory(
	...
	if err := factwrite.ExecChunk(ctx, tx, rows); err != nil {
...
func containerImageIdentityTheoryRowArgs(rows []factwrite.Row) []any {
	n := len(rows)
...
func containerImageIdentityTheoryRowsFor(
	...
	generationID string,
	variant string,
) ([]factwrite.Row, []string) {
	rows := make([]factwrite.Row, containerImageIdentityTheoryRows)
...
func seedContainerImageIdentityTheoryLegacyRows(
	...
	ctx context.Context,
	db *sql.DB,
	template []factwrite.Row,
	legacyIDs []string,
) {
	t.Helper()
	rows := append([]factwrite.Row(nil), template...)
	for i := range rows {
		rows[i].FactID = legacyIDs[i]
		rows[i].StableFactKey += ":tag_resolved"
	}
	if err := factwrite.ExecChunk(ctx, db, rows); err != nil {
