...
func TestPostgresContainerImageIdentityLegacyWriterFailsClosedAboveReadCommitted(
	...
	err = factwrite.BatchInsertFacts(
		ctx,
		tx,
		[]factwrite.Row{containerImageIdentityLegacyLiveRow(factID, 0, false)},
	)
...
`, otherScope, otherGeneration); err != nil {
	...
	go func() {
		done <- factwrite.BatchInsertFacts(ctx, db, []factwrite.Row{row})
	}()
	select {
	case err := <-done:
		if err != nil {
