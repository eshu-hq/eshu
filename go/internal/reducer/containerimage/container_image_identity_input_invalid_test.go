...
// whatever payloadcore.PayloadStr returned for "region" (including "" for an absent key),
...
) (reducercontract.Result, *recordingContainerImageIdentityWriter) {
	t.Helper()
...
// input_invalid quarantine on the intent's reducercontract.Result.SubSignals (each
// quarantined fact is also on the eshu_dp_reducer_input_invalid_facts_total
// counter and a structured error log — see factdecode.RecordQuarantinedFacts).
func assertOneInputInvalidFact(t *testing.T, result reducercontract.Result) {
	t.Helper()
