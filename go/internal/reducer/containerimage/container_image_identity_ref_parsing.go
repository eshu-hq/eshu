...
func dockerfileBaseImageFromFileFact(payload map[string]any) (string, bool) {
	if !strings.EqualFold(strings.TrimSpace(payloadcore.PayloadStr(payload, "language")), "dockerfile") {
		return "", false
	}
...
func containerImageServiceIDsFromPayload(payload map[string]any) []string {
	...
	if serviceID := payloadcore.PayloadStr(payload, "service_id"); serviceID != "" {
	...
	return payloadcore.UniqueSortedStrings(serviceIDs)
}
...
func stringListValue(value any) []string {
	switch typed := value.(type) {
	case []string:
		return payloadcore.CleanFactFilterValues(typed)
	case []any:
		out := make([]string, 0, len(typed))
		for _, item := range typed {
			out = append(out, strings.TrimSpace(fmt.Sprint(item)))
		}
		return payloadcore.CleanFactFilterValues(out)
	case string:
		return payloadcore.CleanFactFilterValues([]string{typed})
	default:
		return nil
	}
}

// ParseContainerImageRef splits a raw image reference into its repository key
// plus either a digest (`repo@sha256:...`) or a tag (`repo:tag`) component. It
// reports false for a blank reference or one with neither a digest nor a tag
// separator. Exported so root-staying Kubernetes and AWS running-image
// correlation can parse the same way this family does (issue #6061).
...
// NormalizeContainerRepositoryKey lowercases and trims a raw repository path
// so digest and tag observations for the same repository join on one key
// regardless of surrounding slashes or case. Exported for the same
// cross-family reason as [ParseContainerImageRef].
...
// DigestFromImageRef returns the digest component of raw if it parses as a
// digest-addressed reference, or "" otherwise (including for a tag-addressed
// or unparseable reference). Exported for the same cross-family reason as
