...
func classifyContainerImageRef(
	...
	decision := ContainerImageIdentityDecision{
		...
		SourceRepositoryIDs:          payloadcore.UniqueSortedStrings(ref.sourceRepositoryIDs),
		BuildProvenanceRepositoryIDs: payloadcore.UniqueSortedStrings(ref.buildProvenanceRepositoryIDs),
		BaseImageForRepositoryIDs:    payloadcore.UniqueSortedStrings(ref.baseImageForRepositoryIDs),
		SourceRevision:               sourceRevision,
		SourceRevisionProvenance:     sourceRevisionProvenance,
		WorkloadIDs:                  payloadcore.UniqueSortedStrings(ref.workloadIDs),
		ServiceIDs:                   payloadcore.UniqueSortedStrings(ref.serviceIDs),
		...
		EvidenceFactIDs:              payloadcore.UniqueSortedStrings(ref.factIDs),
	}
		...
		if repositoryID == "" {
			...
			if len(observations) != 1 {
				if len(observations) > 1 {
					...
					decision.Outcome = reducercontract.ContainerImageIdentityAmbiguousTag
					decision.Reason = "artifact digest matched multiple registry repositories"
					decision.EvidenceFactIDs = payloadcore.UniqueSortedStrings(decision.EvidenceFactIDs)
				}
				return decision
			}
			obs := observations[0]
			...
			decision.Outcome = reducercontract.ContainerImageIdentityExactDigest
			...
			decision.EvidenceFactIDs = payloadcore.UniqueSortedStrings(append(decision.EvidenceFactIDs, obs.factIDs...))
			return decision
		}
	...
	for _, tag := range tags {
		if ref.resolvedDigest != "" && tag.mutated && tag.previousDigest == ref.resolvedDigest {
			...
			decision.Outcome = reducercontract.ContainerImageIdentityStaleTag
			decision.Reason = "runtime resolved the tag to a digest registry facts report as previous"
			decision.EvidenceFactIDs = payloadcore.UniqueSortedStrings(append(decision.EvidenceFactIDs, tag.factID))
			return decision
		}
	}
	...
	if len(distinctDigests) == 1 {
		...
		decision.Digest = tag.digest
		decision.RepositoryID = tag.repositoryID
		decision.Outcome = reducercontract.ContainerImageIdentityTagResolved
...
// factschema seam (never a raw payloadcore.PayloadStr read for the digest), then resolves the
...
// (never a raw payloadcore.PayloadStr read for tag/resolved_digest/previous_digest/mutated),
...
func ociTagObservation(envelope facts.Envelope) (containerImageTagObservation, bool) {
	observation, ok := schemadecode.DecodeOCIImageTagObservationForIndex(envelope)
	if !ok {
	...
	return containerImageTagObservation{
		tag:            tag,
		digest:         digest,
		previousDigest: payloadcore.DerefString(observation.PreviousDigest),
		repositoryID:   repositoryID,
...
func applyCIRunDigestRevision(
	...
	if len(anchor.sourceRepositoryIDs) > 0 {
		decision.SourceRepositoryIDs = payloadcore.UniqueSortedStrings(
			append(decision.SourceRepositoryIDs, anchor.sourceRepositoryIDs...),
		)
		decision.BuildProvenanceRepositoryIDs = payloadcore.UniqueSortedStrings(
			append(decision.BuildProvenanceRepositoryIDs, anchor.sourceRepositoryIDs...),
		)
	}
	if len(anchor.factIDs) > 0 {
		decision.EvidenceFactIDs = payloadcore.UniqueSortedStrings(
