...
// statement_id) is routed through factdecode.PartitionDecodeFailures for quarantine,
...
func extractSLSADigestAnchorsWithQuarantine(
...
) (map[string]slsaDigestAnchor, []factdecode.QuarantinedFact, error) {
	...
	for _, envelope := range envelopes {
		...
		case facts.AttestationStatementFactKind:
			...
			digests := payloadcore.UniqueSortedStrings(append(
				[]string{payloadcore.DerefString(statement.SubjectDigest)},
		...
		case facts.AttestationSignatureVerificationFactKind:
			...
			status := sbomattest.NormalizedVerificationStatus(payloadcore.FirstNonBlank(
				payloadcore.DerefString(verification.VerificationResult),
				payloadcore.DerefString(verification.VerificationStatus),
			))
	...
	for _, envelope := range envelopes {
		...
		anchor.commits = payloadcore.UniqueSortedStrings(append(anchor.commits, commit))
		anchor.factIDs = payloadcore.UniqueSortedStrings(append(
			anchor.factIDs,
			envelope.FactID,
		...
		if sourceURL != "" {
			if match, ok := matchOCIConfigSourceRepository(sourceURL, repositories); ok {
				anchor.sourceRepositoryIDs = payloadcore.UniqueSortedStrings(append(anchor.sourceRepositoryIDs, match.RepositoryID))
			}
		}
...
func slsaProvenanceCommitAndSource(provenance sbomv1.SLSAProvenance) (commit string, sourceURL string) {
	if provenance.ConfigSource != nil {
		configSourceURL := payloadcore.DerefString(provenance.ConfigSource.URI)
	...
	for _, material := range provenance.Materials {
		uri := payloadcore.DerefString(material.URI)
