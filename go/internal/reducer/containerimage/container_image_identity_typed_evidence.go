...
// factdecode.PartitionDecodeFailures (returned to the caller for quarantine); an
...
func addWorkflowImageEvidenceRef(byRef map[string]containerImageRefEvidence, envelope facts.Envelope) (factdecode.QuarantinedFact, bool, error) {
	evidence, err := schemadecode.DecodeCICDWorkflowImageEvidence(envelope)
	if err != nil {
		return factdecode.PartitionDecodeFailures(envelope, err)
	}
	if cicdrun.TrimmedCICDPtr(evidence.EvidenceClass) != "workflow_image_ref" {
		return factdecode.QuarantinedFact{}, false, nil
	}
	addContainerImageRef(
		byRef,
		cicdrun.TrimmedCICDPtr(evidence.ImageRef),
		"",
...
// (cicdrun.CICDRunKeyFromParts/ci_cd_run_correlation_decode.go) the sibling
...
// required provider/run_id join key is routed through factdecode.PartitionDecodeFailures
// for quarantine rather than silently collapsing onto (or contributing
// nothing under) an empty-string key.
func containerImageCIRuns(envelopes []facts.Envelope) (map[string]containerImageCIRunAnchor, []factdecode.QuarantinedFact, error) {
	...
	for _, envelope := range envelopes {
		...
		// cicdrun.CICDRunKeyFromParts still yields a non-empty key like
		...
		key := cicdrun.CICDRunKeyFromParts(run.Provider, run.RunID, run.RunAttempt)
		repositoryID := cicdrun.TrimmedCICDPtr(run.RepositoryID)
		if key == "" || repositoryID == "" {
			continue
		}
		out[key] = containerImageCIRunAnchor{
			repositoryID: repositoryID,
			commitSHA:    cicdrun.TrimmedCICDPtr(run.CommitSHA),
...
// image_digest, manifest_digest) is routed through factdecode.PartitionDecodeFailures for
...
// defensive checks.
func addAWSImageReference(byRef map[string]containerImageRefEvidence, envelope facts.Envelope) (factdecode.QuarantinedFact, bool, error) {
	reference, err := schemadecode.DecodeAWSImageReference(envelope)
	if err != nil {
		return factdecode.PartitionDecodeFailures(envelope, err)
	}
	...
	registryID := payloadcore.FirstNonBlank(payloadcore.DerefString(reference.RegistryID), reference.AccountID)
	if registryID == "" {
		return factdecode.QuarantinedFact{}, false, nil
	}
...
func addAzureImageReference(byRef map[string]containerImageRefEvidence, envelope facts.Envelope) (factdecode.QuarantinedFact, bool, error) {
	...
	imageRef := payloadcore.DerefString(reference.ImageReference)
	digest := payloadcore.DerefString(reference.ImageDigest)
...
// tag_digest_confidence) is routed through factdecode.PartitionDecodeFailures for
...
func addGCPImageReference(byRef map[string]containerImageRefEvidence, envelope facts.Envelope) (factdecode.QuarantinedFact, bool, error) {
	reference, err := schemadecode.DecodeGCPImageReference(envelope)
	if err != nil {
		return factdecode.PartitionDecodeFailures(envelope, err)
	}
	imageRef := payloadcore.DerefString(reference.ImageReference)
...
// factdecode.PartitionDecodeFailures for quarantine. The real ci.artifact collector
...
func addCICDArtifactImageReference(
	...
	if cicdrun.TrimmedCICDPtr(artifact.ArtifactType) != "container_image" {
		return factdecode.QuarantinedFact{}, false, nil
	}
	digest := cicdrun.TrimmedCICDPtr(artifact.ArtifactDigest)
	if digest == "" {
		return factdecode.QuarantinedFact{}, false, nil
	}
	anchors := containerImageAnchorsFromEnvelope(envelope)
	evidenceFactIDs := []string{envelope.FactID}
	run := runs[cicdrun.CICDRunKeyFromParts(artifact.Provider, artifact.RunID, artifact.RunAttempt)]
	if run.repositoryID != "" {
