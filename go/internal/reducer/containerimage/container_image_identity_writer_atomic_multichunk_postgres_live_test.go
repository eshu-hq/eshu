...
func TestPostgresContainerImageIdentityMultiChunkWriterSerializesMatchingKeyAndCleansInterleavedLegacyWrite(
	...
	go func() {
		conflictingDone <- factwrite.BatchInsertFacts(
			ctx,
			db,
			[]factwrite.Row{containerImageIdentityLiveRow(firstFactID, freshToken, false)},
		)
	}()
	select {
	...
	go func() {
		legacyDone <- factwrite.BatchInsertFacts(
			ctx,
			db,
			[]factwrite.Row{containerImageIdentityLegacyLiveRow(legacyFactID, 0, false)},
		)
	}()
	select {
	case err := <-legacyDone:
