...
// materialization_diagnostics.go carries the per-domain diagnostic signals
...
//
// The helper lives in the shared contract tier rather than the reducer root so
// the materialization families that moved into subpackages (issue #6061) emit
// the identical key set as the domains still in the root.
