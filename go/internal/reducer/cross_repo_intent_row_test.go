...
// never source_revision or source_ref directly. The hand-built literal
// validated a code path the real pipeline never exercises, which hid a P0:
// buildResolvedEdgeIntentRow read r.Details["source_revision"] and always
// got "". The fix moved source_revision/first_party_ref_version onto typed
...
// pipeline. A third field, destination_namespace, was investigated the same
// way and deliberately removed before merge (#5441 review round 2): it has
// no evidence producer on any of the five widened relationship types — see
// the Candidate doc comment in internal/relationships/models.go.
...
// fabricate a non-empty value for either of the two new payload keys, and
// must not carry a destination_namespace key at all (removed before merge,
// #5441 review round 2 — see the Candidate doc comment in
// internal/relationships/models.go).
