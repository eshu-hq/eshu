...
indirection at runtime. Measured against baseline `origin/main` at `630115dc5b` (this branch's merge-base):
`go build ./...` and `go vet ./...` both exit 0 on the branch, and `go test
./internal/reducer/... -count=1` exits 0 with 20 packages ok, including this
