...
- Baseline (origin/main): the `HANDLES_ROUTE` intent-materialization pipeline
  (`buildCodeCallProjectionContexts` → `buildCodeEntityIndex` (now
  `shared.BuildEntityIndex`) → `buildHandlesRouteIntentRows`) is reached only
  by the production caller `symbol_runtime_refresh_intents.go` (now
  `code/call/materialization/refresh.go`) and the package-local test helper
...
- After (this PR): `BuildHandlesRouteIntentRowsForQueryProof` is a thin exported
  ...
  (`markRowsRetractViaRefresh` (now `sharedintent.MarkRowsRetractViaRefresh`),
  `buildRepoWideRetractRefreshIntents`) that is irrelevant to the edge payload
  fields the proof reads.
