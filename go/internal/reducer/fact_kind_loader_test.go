...
func TestSQLRelationshipHandlerUsesKindFilteredFactLoader(t *testing.T) {
	...
	if got, want := len(loader.kindCalls), 3; got != want {
	...
	if got, want := strings.Join(loader.kindCalls[2], ","), "file"; got != want {
		t.Fatalf("ListFactsByKind() third kinds = %q, want %q", got, want)
	}
}
