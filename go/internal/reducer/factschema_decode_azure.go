...
// (#4686). The family's remaining consumed kinds (azure_identity_observation,
// azure_resource_change, azure_image_reference) are still intentionally NOT
// decoded through any typed seam: their read-side consumers are a shared
// cross-provider surface or an unconverted Azure-specific storage loader, so a
// decode wrapper for them would be dead code with no caller (and a hollow,
// never-validated contract). They migrate with the surface that reads them.
