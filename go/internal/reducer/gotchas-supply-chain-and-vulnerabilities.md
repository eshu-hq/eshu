...
Container image identity's digest-matching, fencing-token, and test-evidence
detail moved to [`container-image-identity.md`](container-image-identity.md)
for the same reason.
...
- **Multi-cloud runtime drift shares one path keyed on `cloud_resource_uid`, but
  ...
  `multidrift.BuildReducerIntent` enqueues it
