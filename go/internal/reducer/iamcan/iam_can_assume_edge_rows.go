...
// resources.
func buildIAMRoleUserJoinIndex(envelopes []facts.Envelope) (iamRoleUserJoinIndex, []factdecode.QuarantinedFact, error) {
...
func iamPrincipalKindForResourceType(resourceType string) string {
	switch resourceType {
	case iampolicy.ResourceTypeRole:
		return iamCanAssumePrincipalKindRole
	case iampolicy.ResourceTypeUser:
...
func ExtractIAMCanAssumeEdgeRows(
...
) ([]map[string]any, iamCanAssumeEdgeTally, []factdecode.QuarantinedFact, error) {
	...
	type assumeEdgeKey struct {
		principal string
		role      string
	}
	seen := make(map[assumeEdgeKey]struct{}, len(permissionEnvelopes))
	...
	for _, env := range permissionEnvelopes {
		...
		for _, principal := range permission.AssumePrincipals {
			...
			key := assumeEdgeKey{principal: principalNode.uid, role: roleNode.uid}
	...
	sort.Slice(rows, func(a, b int) bool {
		left := payloadcore.AnyToString(rows[a]["principal_uid"]) + "->" + payloadcore.AnyToString(rows[a]["role_uid"])
		right := payloadcore.AnyToString(rows[b]["principal_uid"]) + "->" + payloadcore.AnyToString(rows[b]["role_uid"])
		return left < right
	})
