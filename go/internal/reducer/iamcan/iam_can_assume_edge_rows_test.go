...
func TestExtractIAMCanAssumeEdgeRowsResolvesRoleAndUser(t *testing.T) {
	...
	roleUID := cloudjoin.CloudResourceUID(acct, "aws-global", "aws_iam_role", roleARN)
	wantPrincipalUIDs := map[string]string{
		cloudjoin.CloudResourceUID(acct, "aws-global", "aws_iam_role", assumingRoleARN): iamCanAssumePrincipalKindRole,
		cloudjoin.CloudResourceUID(acct, "aws-global", "aws_iam_user", assumingUserARN): iamCanAssumePrincipalKindUser,
	}
	for _, row := range rows {
		if got := payloadcore.AnyToString(row["role_uid"]); got != roleUID {
			t.Fatalf("role_uid = %q, want %q", got, roleUID)
		}
		principalUID := payloadcore.AnyToString(row["principal_uid"])
		wantKind, ok := wantPrincipalUIDs[principalUID]
		if !ok {
			t.Fatalf("unexpected principal_uid %q", principalUID)
		}
		if got := payloadcore.AnyToString(row["principal_kind"]); got != wantKind {
			t.Fatalf("principal_kind = %q, want %q for %q", got, wantKind, principalUID)
		}
		if got := payloadcore.AnyToString(row["resolution_mode"]); got != iamCanAssumeModeARN {
...
func TestExtractIAMCanAssumeEdgeRowsSelfAssumeAndDuplicateDedupe(t *testing.T) {
	...
	wantPrincipal := cloudjoin.CloudResourceUID(acct, "aws-global", "aws_iam_role", assumingARN)
	if got := payloadcore.AnyToString(rows[0]["principal_uid"]); got != wantPrincipal {
...
func TestExtractIAMCanAssumeEdgeRowsCrossAccountResolvesWhenScanned(t *testing.T) {
	...
	wantPrincipal := cloudjoin.CloudResourceUID(otherAcct, "aws-global", "aws_iam_role", crossAccountRole)
	if got := payloadcore.AnyToString(rows[0]["principal_uid"]); got != wantPrincipal {
		t.Fatalf("principal_uid = %q, want %q", got, wantPrincipal)
	}
}
