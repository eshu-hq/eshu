...
// least one catalog action, matching iampolicy.PrincipalGrant.allows precedence: the
...
func addIAMCanPerformEdge(
	...
	key := iampolicy.EdgeKey{PrincipalUID: principalUID, TargetUID: resourceUID}
	acc := edges[key]
	if acc == nil {
...
func buildIAMCanPerformEdgeRows(
	...
	sort.Slice(rows, func(a, b int) bool {
		left := payloadcore.AnyToString(rows[a]["principal_uid"]) + "->" + payloadcore.AnyToString(rows[a]["resource_uid"])
		right := payloadcore.AnyToString(rows[b]["principal_uid"]) + "->" + payloadcore.AnyToString(rows[b]["resource_uid"])
		return left < right
	})
	return rows
}
