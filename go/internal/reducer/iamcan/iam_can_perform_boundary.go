...
func groupIAMCanPerformBoundaryEvidence(
...
) (map[string]iamCanPerformBoundaryEvidence, []factdecode.QuarantinedFact, error) {
	...
	for _, env := range envelopes {
		...
		case facts.AWSIAMPermissionBoundaryFactKind:
			...
			principalARN := payloadcore.PayloadString(env.Payload, "principal_arn")
			policyARN := payloadcore.PayloadString(env.Payload, "boundary_policy_arn")
		...
		case facts.AWSIAMPermissionFactKind:
			...
			policyARN := payloadcore.DerefString(permission.PolicyARN)
...
func evaluateIAMCanPerformBoundaryPolicy(
	...
	for _, permission := range statements {
		...
		actionTouched := iampolicy.AllowStatementTouches(actions, action) || hasNotActions
		if !actionTouched {
			continue
		}
		...
		if payloadcore.DerefBool(permission.HasConditions) {
			if effect == "Deny" {
