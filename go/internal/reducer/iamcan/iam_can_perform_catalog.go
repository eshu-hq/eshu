...
// Action is one curated, reviewed, high-value sensitive IAM action
...
var iamCanPerformCatalog = []Action{
...
// CatalogByAction indexes the catalog by lowercase action so the
...
func CatalogByAction() map[string]Action {
	out := make(map[string]Action, len(iamCanPerformCatalog))
...
func iamCanPerformCatalogActionsFromCatalog(catalog map[string]Action) map[string]struct{} {
