...
func TestIAMCanPerformResourcePolicyExactPrincipalEmitsEdge(t *testing.T) {
	...
	edge := canPerformEdgeFor(result.Edges, uidOf(iampolicy.ResourceTypeUser, attackerUserARN), canPerformUID(iamCanPerformResourceTypeS3Bucket, canPerformBucketARN))
	if edge == nil {
...
func TestIAMCanPerformResourcePolicyKMSWildcardResourceTargetsAttachedKey(t *testing.T) {
	...
	edge := canPerformEdgeFor(result.Edges, uidOf(iampolicy.ResourceTypeUser, attackerUserARN), canPerformUID(iamCanPerformResourceTypeKMSKey, canPerformKMSKeyARN))
	if edge == nil {
...
func TestIAMCanPerformResourcePolicyUsesProvidedCatalog(t *testing.T) {
	...
	edges := make(map[iampolicy.EdgeKey]*iamCanPerformEdgeAccumulator)
	var tally iamCanPerformTally

	index, _, err := cloudjoin.BuildCloudResourceJoinIndex(resources)
	if err != nil {
		t.Fatalf("cloudjoin.BuildCloudResourceJoinIndex() error = %v, want nil", err)
	}
	...
	edge := canPerformEdgeFor(rows, uidOf(iampolicy.ResourceTypeUser, attackerUserARN), canPerformUID(iamCanPerformResourceTypeS3Bucket, canPerformBucketARN))
	if edge == nil {
