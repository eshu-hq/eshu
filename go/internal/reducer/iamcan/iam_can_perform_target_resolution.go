...
// iampolicy.ResourceTypeOfARN classifier from IAM to the S3/KMS/SecretsManager/SSM/
...
func resolveIAMCanPerformTarget(
	...
	case len(exactMatches) == 1:
		return singleUID(exactMatches), iamCanPerformResolutionExactARN, iampolicy.TargetResolved
	case len(exactMatches) > 1:
	...
	switch {
	case len(merged) == 1:
		return singleUID(merged), iamCanPerformResolutionSingleGlob, iampolicy.TargetResolved
	case len(merged) > 1:
		return "", "", iampolicy.TargetAmbiguous
	case sawWildcard:
