...
// iampolicy.Statement values the grant builders now accept, exercising the
...
func iamPermissionStatementsForTest(t *testing.T, envelopes ...facts.Envelope) []iampolicy.Statement {
	t.Helper()
	statements := make([]iampolicy.Statement, 0, len(envelopes))
	for _, env := range envelopes {
		permission, err := schemadecode.DecodeAWSIAMPermission(env)
		if err != nil {
			t.Fatalf("schemadecode.DecodeAWSIAMPermission() error = %v, want nil", err)
		}
		statements = append(statements, iampolicy.Statement{FactID: env.FactID, Permission: permission})
	}
	return statements
}
...
func canPerformUID(resourceType, arn string) string {
	return cloudjoin.CloudResourceUID(iamEscAccount, "us-east-1", resourceType, arn)
}
