...
// decodeBatchedFactCalls test doubles the reducer root still keeps in
// workload_identity_writer_test.go and reducer_fact_batch_insert_test_helpers_test.go
// for the other batch-insert writer families that have not moved out of the
// root yet (issue #6061). Those root helpers are shared by many files across
// several other families; moving or exporting them was out of scope for this
// move-only split, so incident's tests carry their own copy scoped to only the
// non-versioned insert shape it uses, wired to the exported
// [factwrite.BatchInsertQuery] / [factwrite.BatchSize] leaves instead of the
// root's compat aliases.
