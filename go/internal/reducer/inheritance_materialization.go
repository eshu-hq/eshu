...
func (h InheritanceMaterializationHandler) Handle(
	...
	// vs entities_with_declared_parent vs edge_count distinguish the failure mode:
	// a low content-entity count signals the handler ran against a partial fact
	// set (upstream ordering), while entities_with_declared_parent > 0 with
	// edge_count = 0 signals declared parents that resolved to no in-corpus
	// entity. This is logged here, BEFORE the empty/stall early-return branches
	// below, so every exit path (not just the success path) carries the counts.
	contentEntityFacts, entitiesWithDeclaredParent := countInheritanceFactInputs(envelopes)
	slog.InfoContext(
		ctx, "inheritance materialization fact inputs",
		log.ScopeID(intent.ScopeID),
		log.GenerationID(intent.GenerationID),
		slog.Int("content_entity_facts", contentEntityFacts),
		slog.Int("entities_with_declared_parent", entitiesWithDeclaredParent),
		slog.Int("edge_count", len(rows)),
		slog.Int("repo_count", len(repoIDs)),
		slog.Int("projection_contexts", len(contextByRepoID)),
	)
