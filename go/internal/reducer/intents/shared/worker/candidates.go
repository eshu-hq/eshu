...
func loadPartitionRows(
	...
	indexed bool,
) (partitionRows []sharedintent.Row, loadedCount, unhashedFallback int, err error) {
	if indexed {
	...
	return sharedintent.RowsForPartition(pending, partitionID, partitionCount), len(pending), 0, nil
}
...
func appendUnhashedSharedCandidates(
	...
	merged = hashed
	for _, row := range sharedintent.RowsForPartition(legacy, partitionID, partitionCount) {
	...
	sort.SliceStable(merged, func(i, j int) bool {
		ri, rj := sharedintent.IsRepoRefreshRow(merged[i]), sharedintent.IsRepoRefreshRow(merged[j])
		if ri != rj {
