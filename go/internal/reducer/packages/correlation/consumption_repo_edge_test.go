...
func findUpsertIntent(rows []sharedintent.Row, consumer, owner string) *sharedintent.Row {
	for i := range rows {
		row := rows[i]
		if payloadcore.AnyToString(row.Payload["repo_id"]) == consumer &&
			payloadcore.AnyToString(row.Payload["target_repo_id"]) == owner {
			return &rows[i]
		}
	}
	return nil
...
func TestBuildPackageConsumptionRepoDependencyIntentsProjectsOwnerEdge(t *testing.T) {
	...
	if intent.ProjectionDomain != reducercontract.DomainRepoDependency {
		t.Fatalf("projection domain = %q, want %q", intent.ProjectionDomain, reducercontract.DomainRepoDependency)
	}
	if got := payloadcore.AnyToString(intent.Payload["evidence_source"]); got != PackageConsumptionEvidenceSource {
		t.Fatalf("evidence_source = %q, want %q", got, PackageConsumptionEvidenceSource)
	}
	if got := payloadcore.AnyToString(intent.Payload["relationship_type"]); got != "DEPENDS_ON" {
		t.Fatalf("relationship_type = %q, want DEPENDS_ON", got)
	}
	if got := payloadcore.AnyToString(intent.Payload["action"]); got != "upsert" {
...
func TestBuildPackageConsumptionRepoDependencyIntentsStableAcceptanceKeyAcrossGenerations(t *testing.T) {
	...
	// comment 3455350029).
	build := func(generationID string) []sharedintent.Row {
...
func TestBuildPackageConsumptionRepoEdgeRefreshIntentsRetractsDisappearedOwner(t *testing.T) {
	...
	row := refreshRows[0]
	if got := payloadcore.AnyToString(row.Payload["action"]); got != "retract" {
		t.Fatalf("refresh action = %q, want retract", got)
	}
	if got := payloadcore.AnyToString(row.Payload["evidence_source"]); got != PackageConsumptionEvidenceSource {
		t.Fatalf("refresh evidence_source = %q, want %q", got, PackageConsumptionEvidenceSource)
	}
	if got := payloadcore.AnyToString(row.Payload["repo_id"]); got != "repo-consumer" {
		t.Fatalf("refresh repo_id = %q, want repo-consumer", got)
	}
	if got := payloadcore.AnyToString(row.Payload["target_repo_id"]); got != "" {
	...
	if row.ProjectionDomain != reducercontract.DomainRepoDependency {
		t.Fatalf("refresh domain = %q, want %q", row.ProjectionDomain, reducercontract.DomainRepoDependency)
	}
...
func TestBuildPackageConsumptionRepoEdgeRefreshIntentsRetractsSelfReferenceOnlyConsumer(t *testing.T) {
	...
	if got := payloadcore.AnyToString(rows[0].Payload["action"]); got != "retract" {
		t.Fatalf("refresh action = %q, want retract", got)
	}
	if got := payloadcore.AnyToString(rows[0].Payload["repo_id"]); got != "repo-same" {
