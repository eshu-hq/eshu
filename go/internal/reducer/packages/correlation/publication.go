	...
	for _, version := range versions {
		for _, hint := range hints.forVersion(version) {
			...
			decisions = append(decisions, PackagePublicationDecision{
				...
				CandidateRepositoryIDs: payloadcore.UniqueSortedStrings(sourceDecision.CandidateRepositoryIDs),
				Outcome:                sourceDecision.Outcome,
				Reason:                 sourceDecision.Reason,
				ProvenanceOnly:         true,
				CanonicalWrites:        0,
				EvidenceFactIDs:        payloadcore.UniqueSortedStrings(append(sourceDecision.EvidenceFactIDs, version.FactID)),
			})
		}
	}
...
func extractPackageRegistryVersions(envelopes []facts.Envelope) []packageRegistryVersionIdentity {
	...
	for _, envelope := range envelopes {
		...
		version := packageRegistryVersionIdentity{
			...
			Version:     payloadcore.PayloadStr(envelope.Payload, "version"),
			PublishedAt: payloadcore.PayloadStr(envelope.Payload, "published_at"),
		}
