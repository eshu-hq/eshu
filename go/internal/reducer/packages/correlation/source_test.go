...
func TestPackageSourceCorrelationHandlerLoadsActiveRepositoriesAndEmitsCounters(t *testing.T) {
	...
	if got := reducerCounterValue(t, rm, "eshu_dp_package_source_correlations_total", map[string]string{
		telemetry.MetricDimensionDomain:  string(reducercontract.DomainPackageSourceCorrelation),
...
// payloadStringAny is a family-local copy of the same-named helper the
// reducer root's workload tests keep (workload_dependency_reconciliation_test.go);
// Go test files cannot share unexported symbols across a package boundary, so
// this family test keeps a local copy (issue #6061).
