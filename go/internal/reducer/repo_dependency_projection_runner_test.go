...
// TestRepoDependencyProjectionRunnerRecordsQuiescenceBlockedCycle is the
// owner-review P2 for the #6184 quiescence gate: a cycle blocked on
// canonical-code quiescence must leave operator-visible evidence (the
// per-cycle write-duration point, plus the blocked log line), mirroring the
// code-call lane's recordCodeCallTiming on its blocked path. Without it a
// lane held shut for hours is indistinguishable from a dead lane.
