...
// fakeWorkloadIdentityExecer, decodeBatchedFactCalls, expectedBatchedExecCount,
// and their supporting helpers below are local copies of the reducer root's
// shared batch-insert test infrastructure (see workload_identity_writer_test.go
// and reducer_fact_batch_insert_test_helpers_test.go there, used by 33 other
// root writer tests). They carry no sbom_attestation- or workload-identity-
// specific logic — they exercise [factwrite]'s own batched-insert wire format
// (factwrite.BatchInsertQuery, factwrite.ChunkArgs) — but Go test files cannot
// share unexported symbols across package boundaries, so they are duplicated
// here rather than exported cross-package for test-only use.
