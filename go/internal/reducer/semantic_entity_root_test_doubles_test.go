...
// test suite defined before it moved to internal/reducer/code/semantic
...
// code/semantic and requalify every call site. Mirrors
// container_image_identity_root_test_doubles_test.go for the same reason.

// recordingSemanticEntityWriter is a semantic.EntityWriter
// double that records every write request it receives.
type recordingSemanticEntityWriter struct {
	writes []semantic.EntityWrite
	result semantic.EntityWriteResult
}
