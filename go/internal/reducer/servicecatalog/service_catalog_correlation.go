...
func (h ServiceCatalogCorrelationHandler) Handle(ctx context.Context, intent reducercontract.Intent) (reducercontract.Result, error) {
	if intent.Domain != reducercontract.DomainServiceCatalogCorrelation {
		return reducercontract.Result{}, fmt.Errorf("service_catalog_correlation handler does not accept domain %q", intent.Domain)
	}
	if h.FactLoader == nil {
		return reducercontract.Result{}, fmt.Errorf("service catalog correlation fact loader is required")
	}
	if h.Writer == nil {
		return reducercontract.Result{}, fmt.Errorf("service catalog correlation writer is required")
	}

	envelopes, err := factload.LoadFactsForKinds(ctx, h.FactLoader, intent.ScopeID, intent.GenerationID, serviceCatalogCorrelationFactKinds())
	if err != nil {
		return reducercontract.Result{}, fmt.Errorf("load service catalog correlation facts: %w", err)
	}
	...
	if fatalErr != nil {
		...
		return reducercontract.Result{}, fmt.Errorf("decode service catalog correlation facts: %w", fatalErr)
	}
	inputInvalidCount := factdecode.RecordQuarantinedFacts(ctx, h.Instruments, reducercontract.DomainServiceCatalogCorrelation, intent.ScopeID, intent.GenerationID, quarantined)
	...
	if err != nil {
		return reducercontract.Result{}, fmt.Errorf("write service catalog correlations: %w", err)
	}
