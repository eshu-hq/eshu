...
// newServiceCatalogCorrelationInstruments builds a fresh manual-reader
// telemetry.Instruments for one test's counter assertions. This is this
// package's own copy of the small helper the reducer root's
// package_source_correlation_test.go also defines under an unqualified name
// -- both are test-only setup with no family-specific logic, so each family
// keeps its own rather than one importing the other's tests (issue #6061).
func newServiceCatalogCorrelationInstruments(t *testing.T) (*telemetry.Instruments, sdkmetric.Reader) {
	t.Helper()
...
func TestServiceCatalogCorrelationCountersSeparateGuardrails(t *testing.T) {
	...
	inst, reader := newServiceCatalogCorrelationInstruments(t)
	loader := &stubServiceCatalogCorrelationFactLoader{
		scopeFacts: []facts.Envelope{
			serviceCatalogEntityFact("entity-shared", "component:default/shared", "Shared"),
			serviceCatalogRepositoryLinkFact("repo-link-shared", "component:default/shared", "https://github.com/acme/shared.git"),
	...
	if got := reducerCounterValue(t, rm, "eshu_dp_service_catalog_correlations_total", map[string]string{
		telemetry.MetricDimensionDomain:  string(reducercontract.DomainServiceCatalogCorrelation),
