...
// contracts seam (schemadecode.DecodeServiceCatalogEntity, schemadecode.DecodeServiceCatalogOwnership,
// schemadecode.DecodeServiceCatalogRepositoryLink) and the codegraph "repository" envelope
// through schemadecode.DecodeCodegraphRepository (reused from Wave 4f S1), returning
...
// recorded as a factdecode.QuarantinedFact and EXCLUDED from the index — exactly the set
...
// reachable class here) — is NOT quarantined: factdecode.PartitionDecodeFailures returns
...
) (serviceCatalogCorrelationIndex, []factdecode.QuarantinedFact, error) {
...
// repository) decode error through factdecode.PartitionDecodeFailures, returning
// (factdecode.QuarantinedFact, ok, fatalErr). When ok is true the fact is a quarantinable
...
// quarantine. The residual factdecode.QuarantinedFact carries the decode error's own
...
//
// This tightens the earlier wrapper, which discarded factdecode.PartitionDecodeFailures's
// fatal third result and fell back to schemadecode.ServiceCatalogDecodeQuarantine for it —
...
func serviceCatalogQuarantine(envelope facts.Envelope, err error) (factdecode.QuarantinedFact, bool, error) {
	if q, ok, _ := factdecode.PartitionDecodeFailures(envelope, err); ok {
		return q, true, nil
	}
	return schemadecode.ServiceCatalogDecodeQuarantine(envelope, err), false, err
}
...
func serviceCatalogEntityFromFact(envelope facts.Envelope) (serviceCatalogEntityEvidence, error) {
	entity, err := schemadecode.DecodeServiceCatalogEntity(envelope)
	if err != nil {
...
func serviceCatalogOwnershipFromFact(envelope facts.Envelope) (serviceCatalogOwnershipEvidence, error) {
	ownership, err := schemadecode.DecodeServiceCatalogOwnership(envelope)
	if err != nil {
...
func serviceCatalogRepositoryLinkFromFact(envelope facts.Envelope) (serviceCatalogRepositoryLinkEvidence, error) {
	link, err := schemadecode.DecodeServiceCatalogRepositoryLink(envelope)
	if err != nil {
