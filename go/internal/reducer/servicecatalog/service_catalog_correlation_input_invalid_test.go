...
// factdecode.PartitionDecodeFailures, NOT a silent empty-string catalog identity.
//
...
// factdecode.QuarantinedFact was ever recorded. After the migration, the index decodes
// each "service_catalog.entity" fact's outer envelope through
// factschema.DecodeServiceCatalogEntity (schemadecode.DecodeServiceCatalogEntity); the
// malformed fact yields a classified *factDecodeError that
// factdecode.PartitionDecodeFailures routes to an explicit factdecode.QuarantinedFact naming the
...
func TestPartitionServiceCatalogFactsQuarantinesMalformedEntity(t *testing.T) {
	...
	if len(quarantined) != 1 {
		t.Fatalf("len(quarantined) = %d, want 1; the missing-entity_ref entity fact must be quarantined via factdecode.PartitionDecodeFailures: %#v", len(quarantined), quarantined)
	}
...
// kinds this is a reachable class. factdecode.PartitionDecodeFailures returns it as the
