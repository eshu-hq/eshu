...
func TestDecodeServiceCatalogAcceptsPersistedVersionlessSchemaVersion(t *testing.T) {
	...
	entity, err := schemadecode.DecodeServiceCatalogEntity(persistedEntity)
	if err != nil {
		t.Fatalf("schemadecode.DecodeServiceCatalogEntity(SchemaVersion=0.0.0) error = %v, want nil; the persisted version-less sentinel must decode as the latest major, not dead-letter", err)
	}
	if entity.EntityRef != "component:default/checkout" {
		t.Fatalf("schemadecode.DecodeServiceCatalogEntity EntityRef = %q, want component:default/checkout", entity.EntityRef)
	}
	...
	if ownership, err := schemadecode.DecodeServiceCatalogOwnership(persistedOwnership); err != nil {
		t.Fatalf("schemadecode.DecodeServiceCatalogOwnership(SchemaVersion=0.0.0) error = %v, want nil", err)
	} else if ownership.EntityRef != "component:default/checkout" {
		t.Fatalf("schemadecode.DecodeServiceCatalogOwnership EntityRef = %q, want component:default/checkout", ownership.EntityRef)
	}
	...
	if link, err := schemadecode.DecodeServiceCatalogRepositoryLink(persistedLink); err != nil {
		t.Fatalf("schemadecode.DecodeServiceCatalogRepositoryLink(SchemaVersion=0.0.0) error = %v, want nil", err)
	} else if link.EntityRef != "component:default/checkout" {
		t.Fatalf("schemadecode.DecodeServiceCatalogRepositoryLink EntityRef = %q, want component:default/checkout", link.EntityRef)
	}
}
...
// schemadecode.DecodeCodegraphRepository seam (Wave 4f S1) through this family's own
