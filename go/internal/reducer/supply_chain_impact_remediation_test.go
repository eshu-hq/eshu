...
// proves that ecosystems outside the npm starting scope of issue #595 report
// an unknown-confidence "package_manager_unsupported" remediation so callers
// see explicitly that Eshu has not yet computed a safe upgrade path for that
// ecosystem.
func TestBuildSupplyChainImpactFindingsRemediationPackageManagerUnsupported(t *testing.T) {
	t.Parallel()

	findings := BuildSupplyChainImpactFindings([]facts.Envelope{
		vulnerabilityCVEFact("cve-maven", "CVE-2026-90006", 7.7),
	...
	finding := supplyChainImpactFindingsByCVE(findings)["CVE-2026-90006"]
	remediation := finding.Remediation
	...
	if remediation.Ecosystem != "maven" {
		t.Fatalf("Ecosystem = %q, want maven", remediation.Ecosystem)
	}
	if remediation.ManifestAllowsFix != "unknown" {
