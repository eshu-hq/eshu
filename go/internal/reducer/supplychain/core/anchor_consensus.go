...
// containerimage.ContainerImageIdentityFormatImageRef), so
...
func dedupeContainerImageIdentityByLogicalRow(envelopes []facts.Envelope) []supplyChainImageIdentity {
	...
	for _, envelope := range envelopes {
		...
		candidate := containerImageIdentityLogicalCandidate{
			row:  row,
			isV2: payloadcore.PayloadStr(envelope.Payload, "identity_format") == containerimage.ContainerImageIdentityFormatImageRef,
		}
...
// observed_at (go/internal/collector/repo/git/git_source_processing.go:
