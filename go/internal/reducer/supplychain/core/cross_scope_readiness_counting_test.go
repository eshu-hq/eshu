...
func TestSupplyChainImpactProducerFactKindsCoverEveryDeclaredProducer(t *testing.T) {
	...
	dependencies := crossscope.DependenciesForRegistration(reducercontract.DomainSupplyChainImpact)
	if len(dependencies) != 1 {
		t.Fatalf("crossscope.DependenciesForRegistration(%s) returned %d entries, want 1", reducercontract.DomainSupplyChainImpact, len(dependencies))
	}
