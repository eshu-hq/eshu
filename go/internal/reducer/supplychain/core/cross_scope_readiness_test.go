...
// scopeOnlySupplyChainImpactFactLoader implements the base factload.FactLoader but NOT
...
func supplyChainImpactIntentUnderTheFloor(cause string) reducercontract.Intent {
...
// evidence alone.
func readinessWithOnlyUnready(unready reducercontract.Domain) *fixedCrossScopeReadiness {
	return &fixedCrossScopeReadiness{
		ready:           true,
		readyByProducer: map[reducercontract.Domain]bool{unready: false},
	}
}
...
func TestSupplyChainImpactDefersWhenProducerScopesHaveNotActivated(t *testing.T) {
	...
	var notReady crossscope.ProducerNotReadyError
	if !errors.As(err, &notReady) {
		t.Fatalf("Handle() error = %#v, want crossscope.ProducerNotReadyError", err)
	}
	if got := notReady.FailureClass(); got != crossscope.ProducerNotReadyFailureClass {
		t.Fatalf("FailureClass() = %q, want %q", got, crossscope.ProducerNotReadyFailureClass)
	}
...
// floor stays opt-in by capability. A factload.FactLoader that does not implement
...
func TestSupplyChainImpactDefersWhenOnlyOneProducerResolved(t *testing.T) {
	...
	if !slices.Contains(notReady.ProducerDomains, reducercontract.DomainCICDRunCorrelation) {
		t.Fatalf("ProducerDomains = %v, want the unresolved ci_cd_run_correlation producer", notReady.ProducerDomains)
	}
	if slices.Contains(notReady.ProducerDomains, reducercontract.DomainContainerImageIdentity) {
		t.Fatalf(
			"ProducerDomains = %v, want container_image_identity omitted: it resolved output on this pass",
			notReady.ProducerDomains,
		)
	}
	if got := logs.String(); !strings.Contains(got, string(reducercontract.DomainCICDRunCorrelation)) ||
		strings.Contains(got, string(reducercontract.DomainContainerImageIdentity)) {
