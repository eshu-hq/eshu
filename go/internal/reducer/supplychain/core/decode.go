...
) ([]vulnerabilitySuppression, []factdecode.QuarantinedFact, error) {
...
func decodeVulnerabilitySuppression(envelope facts.Envelope) (vulnerabilitySuppression, error) {
	value, err := factschema.DecodeVulnerabilitySuppression(schemadecode.FactschemaEnvelope(envelope))
	if err != nil {
		return vulnerabilitySuppression{}, factdecode.NewFactDecodeError(factschema.FactKindVulnerabilitySuppression, err)
	}
...
func invalidVulnerabilitySuppressionField(field string) error {
	return factdecode.NewFactDecodeError(
