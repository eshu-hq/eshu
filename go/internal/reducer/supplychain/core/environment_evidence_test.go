...
func TestSupplyChainDeploymentContextFromEnvelopeDecodesEnvironmentEvidence(t *testing.T) {
	...
	cases := []struct {
		...
		// Padding is already stripped by payloadcore.PayloadStr before normalize sees it,
...
// payloadcore.UniqueSortedStrings drops blanks from Environments, leaving the two
...
// normalize does its own trimming even though payloadcore.PayloadStr already trims on the
