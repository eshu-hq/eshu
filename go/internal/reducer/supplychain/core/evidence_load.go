...
func (h SupplyChainImpactHandler) loadSupplyChainImpactEvidence(
	...
	envelopes, err := factload.LoadFactsForKinds(ctx, h.FactLoader, intent.ScopeID, intent.GenerationID, supplyChainImpactFactKinds())
...
type supplyChainImpactCrossScopeFloor struct {
	signal    crossscope.ProducerReadinessSignal
	sampledAt time.Time
	...
	// producer's delta.
	producerFactsBefore map[reducercontract.Domain]int
}
...
var supplyChainImpactProducerFactKindByDomain = map[reducercontract.Domain]string{
	reducercontract.DomainContainerImageIdentity: reducercontract.ContainerImageIdentityFactKind,
	reducercontract.DomainCICDRunCorrelation:     cicdrun.CICDRunCorrelationFactKind,
}
...
func countSupplyChainImpactCrossScopeProducerFacts(envelopes []facts.Envelope) map[reducercontract.Domain]int {
	producerByKind := make(map[string]reducercontract.Domain, len(supplyChainImpactProducerFactKindByDomain))
	for _, dependency := range crossscope.DependenciesForRegistration(reducercontract.DomainSupplyChainImpact) {
	...
	resolved := make(map[reducercontract.Domain]int, len(producerByKind))
