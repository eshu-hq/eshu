...
func buildJSTSPackageReachabilityIndex(envelopes []facts.Envelope) jsTSPackageReachabilityIndex {
	...
	for _, envelope := range envelopes {
		if envelope.IsTombstone || envelope.FactKind != factload.FactKindFile {
			continue
		}
...
func (r *jsTSRepositoryReachability) recordImports(fileData map[string]any, factID string) {
	...
	for _, entry := range payloadcore.MapSlice(fileData["imports"]) {
		source := payloadcore.PayloadStr(entry, "source")
		packageRoot, ok := npmPackageRootFromImportSource(source)
		if !ok {
			continue
		}
		evidence := jsTSPackageAPIImportEvidence
		if strings.EqualFold(payloadcore.PayloadStr(entry, "import_type"), "reexport") {
	...
	for _, call := range payloadcore.MapSlice(fileData["function_calls"]) {
		callName := payloadcore.FirstNonBlank(payloadcore.PayloadStr(call, "full_name"), payloadcore.PayloadStr(call, "name"))
...
func (r *jsTSRepositoryReachability) recordSCIPEdges(fileData map[string]any, factID string) {
	for _, edge := range payloadcore.MapSlice(fileData["function_calls_scip"]) {
		for _, key := range []string{"package", "package_name", "import_source", "callee_package", "callee_symbol"} {
			packageRoot, ok := npmPackageRootFromSCIPValue(payloadcore.PayloadStr(edge, key))
			if !ok {
				continue
			}
			r.recordUsage(packageRoot, jsTSPackageAPIUsage{
...
func isJSTSParsedFile(fileData map[string]any) bool {
	switch strings.ToLower(payloadcore.FirstNonBlank(
		payloadcore.PayloadStr(fileData, "language"),
		payloadcore.PayloadStr(fileData, "lang"),
	)) {
...
func jsTSImportAliases(entry map[string]any) []string {
	return payloadcore.UniqueSortedStrings([]string{
