...
func supplyChainReachabilityForFinding(finding SupplyChainImpactFinding) SupplyChainReachability {
	...
	case "package_manifest", string(GoVulnReachabilityModuleOnly), string(GoVulnReachabilityUnknown), "known_fixed":
		return fallbackSupplyChainReachability(finding, ecosystem, maturity, detail)
	default:
		return fallbackSupplyChainReachability(finding, ecosystem, maturity, detail)
	}
}

// fallbackSupplyChainReachability resolves dependency-only evidence
// (package manifest, module-only/unknown Go reachability, known-fixed) and
// any detail no earlier case of supplyChainReachabilityForFinding claims.
// Pure code motion for the funlen cap; conditions, order, and bodies are
// unchanged.
func fallbackSupplyChainReachability(finding SupplyChainImpactFinding, ecosystem, maturity, detail string) SupplyChainReachability {
	if detail == "package_manifest" || detail == string(GoVulnReachabilityModuleOnly) || detail == string(GoVulnReachabilityUnknown) || detail == "known_fixed" {
		if ecosystem == "pypi" {
...
func longTailSupplyChainReachability(
	...
	missing := payloadcore.UniqueSortedStrings(append(
...
func supplyChainReachabilityMissingEvidence(
	...
	missing := payloadcore.UniqueSortedStrings(finding.MissingEvidence)
