...
func TestSupplyChainImpactHandlerEndToEndCrossScopeCIRunDoesNotBlankRepositoryID(t *testing.T) {
	...
	identityHandler := containerimage.ContainerImageIdentityHandler{FactLoader: identityLoader, Writer: identityWriter}

	if _, err := identityHandler.Handle(context.Background(), reducercontract.Intent{
	...
	identityFact := facts.Envelope{
		...
		Payload:  containerimage.ContainerImageIdentityPayload(identityWriter.write, decision, "canonical-5810ci-e2e"),
	}
