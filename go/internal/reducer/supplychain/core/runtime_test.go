...
func TestBuildSupplyChainImpactFindingsConnectsRuntimeEvidencePath(t *testing.T) {
	...
	findings := BuildSupplyChainImpactFindings([]facts.Envelope{
		...
		containerImageIdentityImpactFactWithOutcome(
			...
			string(reducercontract.ContainerImageIdentityExactDigest),
		),
		cicdRunCorrelationImpactFact(
			...
			testImpactRepositoryID,
			testImpactEnv,
			string(cicdrun.CICDRunCorrelationExact),
		),
		serviceCatalogCorrelationImpactFact(
			...
			string(servicecatalog.ServiceCatalogCorrelationExact),
			"matches",
			false,
		),
	})
	...
	assertContainsString(t, got.EvidencePath, cicdrun.CICDRunCorrelationFactKind)
	assertContainsString(t, got.EvidencePath, reducercontract.ServiceCatalogCorrelationFactKind)
...
func TestBuildSupplyChainImpactFindingsAttachesWorkloadIdentityWithoutServiceCatalog(t *testing.T) {
	...
	assertSupplyChainImpactStatus(t, got, SupplyChainImpactAffectedExact)
	assertContainsString(t, got.WorkloadIDs, testImpactWorkloadID)
	assertContainsString(t, got.EvidencePath, reducercontract.WorkloadIdentityFactKind)
...
func TestBuildSupplyChainImpactFindingsReportsScopedUnresolvedServiceCatalogEvidence(t *testing.T) {
	...
	findings := BuildSupplyChainImpactFindings([]facts.Envelope{
		...
		serviceCatalogCorrelationRepositoryScopeImpactFact(
			"catalog-1",
			testImpactRepositoryID,
			string(servicecatalog.ServiceCatalogCorrelationUnresolved),
			"missing",
			true,
		),
	})
	...
	assertContainsString(t, got.WorkloadIDs, testImpactWorkloadID)
	assertContainsString(t, got.EvidencePath, reducercontract.WorkloadIdentityFactKind)
	assertNotContainsString(t, got.EvidencePath, reducercontract.ServiceCatalogCorrelationFactKind)
...
func TestBuildSupplyChainImpactFindingsDoesNotPromoteAmbiguousImageIdentity(t *testing.T) {
	...
	assertNotContainsString(t, got.EvidencePath, reducercontract.ContainerImageIdentityFactKind)
...
func TestBuildSupplyChainImpactFindingsKeepsStaleServiceEvidenceMissing(t *testing.T) {
	...
	findings := BuildSupplyChainImpactFindings([]facts.Envelope{
		...
		serviceCatalogCorrelationImpactFact(
			...
			testImpactServiceID,
			testImpactWorkloadID,
			string(servicecatalog.ServiceCatalogCorrelationStale),
			"stale",
			true,
		),
	})
...
func serviceCatalogCorrelationImpactFact(
	...
	return facts.Envelope{
		FactID:   factID,
		FactKind: reducercontract.ServiceCatalogCorrelationFactKind,
		Payload: map[string]any{
			"repository_id":     repositoryID,
...
func workloadIdentityImpactFact(factID string, repositoryID string, workloadID string) facts.Envelope {
	return facts.Envelope{
		FactID:   factID,
		FactKind: reducercontract.WorkloadIdentityFactKind,
...
func platformMaterializationImpactFact(factID string, repositoryID string, deploymentID string) facts.Envelope {
	...
	return facts.Envelope{
		FactID:   factID,
		FactKind: reducercontract.PlatformMaterializationFactKind,
		ScopeID:  scopeID,
		Payload: map[string]any{
			"scope_id":          scopeID,
...
func containerImageIdentityImpactFactWithOutcome(
	...
	if outcome == string(reducercontract.ContainerImageIdentityAmbiguousTag) || outcome == string(reducercontract.ContainerImageIdentityStaleTag) {
