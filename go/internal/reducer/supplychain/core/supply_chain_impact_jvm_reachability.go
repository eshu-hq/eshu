...
func jvmReachabilityFactFilter(envelopes []facts.Envelope) JVMReachabilityFactFilter {
	...
	return JVMReachabilityFactFilter{
		...
		APIPackages:   payloadcore.UniqueSortedStrings(apiPackages),
	}
}
...
func buildJVMReachabilityIndex(envelopes []facts.Envelope) jvmReachabilityIndex {
	...
	for _, envelope := range envelopes {
		...
		if !ok || !jvmParsedFile(fileData, payloadcore.PayloadStr(envelope.Payload, "relative_path")) {
			continue
		}
...
func jvmReachabilityUsagesFromFile(factID string, fileData map[string]any) []jvmReachabilityUsage {
	...
	for _, item := range payloadcore.MapSlice(fileData["function_calls"]) {
	...
	for _, item := range payloadcore.MapSlice(fileData["function_calls_scip"]) {
...
func applyJVMSupplyChainReachability(
	...
	finding.EvidencePath = append(finding.EvidencePath, factload.FactKindFile, usage.evidenceKind, consumption.PackageAPIIdentitySource)
...
func jvmParsedFile(fileData map[string]any, relativePath string) bool {
	switch strings.ToLower(strings.TrimSpace(payloadcore.AnyToString(fileData["lang"]))) {
...
func jvmAPIPackageFromValues(values ...any) string {
	for _, value := range values {
		if apiPackage := jvmAPIPackageFromText(payloadcore.AnyToString(value)); apiPackage != "" {
			return apiPackage
		}
	}
	return ""
