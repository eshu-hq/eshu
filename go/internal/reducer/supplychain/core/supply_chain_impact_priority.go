...
func buildSupplyChainImpactPriorityContributions(
	...
	if finding.Reachability != nil {
		...
		case SupplyChainReachabilityUnknown, SupplyChainReachabilityUnavailable:
			// Unknown and unavailable reachability contribute no priority
			// signal: nothing proven to reward or penalize. Matches the
			// pre-move default (no contribution); listed for exhaustiveness.
		}
	}
...
func deploymentPriorityValue(finding SupplyChainImpactFinding) string {
	...
	return strings.Join(payloadcore.UniqueSortedStrings(values), ",")
}
...
func supplyChainPriorityReasonCodes(contributions []SupplyChainImpactPriorityContribution) []string {
	...
	return payloadcore.UniqueSortedStrings(codes)
}
