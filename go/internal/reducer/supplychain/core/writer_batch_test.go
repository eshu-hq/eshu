...
// factwrite.BatchInsertVersionedFacts bulk-insert path (issue #5317) rather than
...
func TestWriteSupplyChainImpactFindingsBoundedExecCount(t *testing.T) {
	...
	wantExecs := testutil.ExpectedBatchedExecCount(findingCount)
	...
	if rows := testutil.DecodeBatchedVersionedFactCalls(t, db.Execs); len(rows) != findingCount {
