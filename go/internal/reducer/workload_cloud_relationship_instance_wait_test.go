...
func (l *usesWaitLedger) UpsertReadinessWait(_ context.Context, wait crossscope.ReadinessWait) error {
	l.upserts++
...
func (l *usesWaitLedger) ClearReadinessWait(_ context.Context, wait crossscope.ReadinessWait) error {
	key := wait.ScopeID + "|" + string(wait.Domain)
...
func TestWorkloadCloudRelationshipCommitsBeforeWaitingOnInstances(t *testing.T) {
	...
	for key, row := range ledger.rows {
		if !row.Cleared() || row.MissingCount != 0 {
			t.Fatalf("ledger row %s = %+v, want a cleared tombstone once nothing is missing", key, row)
		}
	}
}
