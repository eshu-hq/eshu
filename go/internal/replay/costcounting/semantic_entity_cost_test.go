...
// operates over flat semantic.EntityRow values, not a
...
func semanticEntityFixtureRows() []semantic.EntityRow {
...
func TestCostBudget_SemanticEntityMaterialization_N1_ExceedsBudget(t *testing.T) {
	...
	for _, row := range rows {
		if _, err := writer.WriteSemanticEntities(context.Background(), semantic.EntityWrite{
			RepoIDs: []string{row.RepoID},
			Rows:    []semantic.EntityRow{row},
		}); err != nil {
