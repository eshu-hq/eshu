...
// path (INHERITS/IMPLEMENTS) deletes only the in-scope edges on a real NornicDB.
...
func TestReducerInheritanceEdgeRetractGraphTruth(t *testing.T) {
	...
	for _, uid := range []string{inhChildA, inhParentA, inhChildB, inhParentB, inhChildC, inhParentC} {
