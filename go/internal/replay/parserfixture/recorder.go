...
func Record(ctx context.Context, opts RecordOptions) error {
	...
	return writeFixture(*captured, opts.Path)
}
...
// writeFixture builds the fixture file, canonicalizes it, verifies it still loads
// as a valid fixture, and writes it to disk. The load-back is a guard: the
// recorder must never emit a fixture the replay loader would reject.
func writeFixture(captured captureState, path string) error {
	file := File{
