...
`replaycoverage` is the assertion core of the **C-1/C-8/C-9 replay coverage
...
`EnumerateSupported` flattens the six registries into a deterministic
...
surfaces, CLI surfaces, parsers, capability claims, and product claims all come
from the same generated registries, snapshot, and ledgers those gates already
own.
...
the command. CI now passes the single blocking flag after the C-2..C-9 burn-down,
