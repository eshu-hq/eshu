...
**does every non-exempt surface and required scenario-depth class Eshu claims
to support map to a committed replay scenario and an enforceable proof gate —
and will CI notice when a new one doesn't?** Exempt rows carry an audited reason
that the proof class does not apply. The inventory check is credential-free and
