...
type Inputs struct {
	...
	Resolver Resolver
	// Blocking flips every coverage finding from advisory to required. The gate
	// ships with Blocking=false (advisory) and is flipped to true once C-2..C-6
	// burn the coverage gaps down.
	Blocking bool
}
