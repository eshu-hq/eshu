...
// boundaryFreeTextTail sits AFTER the escape, so a walk that ends the value at
// the escape removes boundaryFreeTextSentinel and leaves this behind. A single
// sentinel cannot tell "removed the whole value" from "cut it at the escape".
const boundaryFreeTextTail = "FREETEXT-TAIL-3a90f2"
...
// TestRedactFreeTextKeepsAnEscapedProseTerminatorInsideAnEncodedValue covers the
// other half of the terminator set at the OTHER depth, which is the half the
// depth rule got wrong in the opposite direction.
//
// At the encoded depth the scan counted every member of
// freeTextValueTerminators in its escaped form. Only "?", "&" and ";" are URL
// structure there. Space, tab, CR, LF, a quote and a backtick are PROSE
// delimiters: an encoder writes "%20" precisely because the space is inside a
// value, so the escaped spelling is evidence of content, not of a boundary.
// Reading it as one cut the credential and shipped the tail — the same partial
// leak the literal depth had, one layer down, on the canonical nested-callback
// URL an HTTP client writes.
func TestRedactFreeTextKeepsAnEscapedProseTerminatorInsideAnEncodedValue(t *testing.T) {
	t.Parallel()

	// The pair's own "=" arrived as "%3D", so this text sits one layer down and
	// the escape under test is inside the value.
	nested := func(escape string) string {
		return "curl 'https://h/cb?redirect_uri=%2Fx%3Faccess_token%3D" +
			boundaryFreeTextSentinel + escape + boundaryFreeTextTail + "'"
	}
	...
	tests := []struct {
		...
		{name: "escaped space", note: nested("%20"), want: "curl 'https://h/cb?redirect_uri=%2Fx%3F[redacted]'"},
		{name: "escaped tab", note: nested("%09"), want: "curl 'https://h/cb?redirect_uri=%2Fx%3F[redacted]'"},
		{name: "escaped carriage return", note: nested("%0D"), want: "curl 'https://h/cb?redirect_uri=%2Fx%3F[redacted]'"},
		{name: "escaped line feed", note: nested("%0A"), want: "curl 'https://h/cb?redirect_uri=%2Fx%3F[redacted]'"},
		{name: "escaped double quote", note: nested("%22"), want: "curl 'https://h/cb?redirect_uri=%2Fx%3F[redacted]'"},
		{name: "escaped single quote", note: nested("%27"), want: "curl 'https://h/cb?redirect_uri=%2Fx%3F[redacted]'"},
		{name: "escaped backtick", note: nested("%60"), want: "curl 'https://h/cb?redirect_uri=%2Fx%3F[redacted]'"},
		{
			// The control. A separator IS structure at this depth, so the value
			// still ends at it and "repo%3Ddemo" survives. Without this row the
			// fix above could be "stop reading escapes at the encoded depth",
			// which would put the whole-credential leak back.
			name: "an escaped ampersand still ends the value",
			note: "curl 'https://h/cb?redirect_uri=%2Fx%3Faccess_token%3D" +
				boundaryFreeTextSentinel + "%26repo%3Ddemo'",
			want: "curl 'https://h/cb?redirect_uri=%2Fx%3F[redacted]%26repo%3Ddemo'",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(got, boundaryFreeTextSentinel) || strings.Contains(got, boundaryFreeTextTail) {
				t.Fatalf("a credential fragment survived the walk of %q", tt.note)
			}
			if got != tt.want {
				t.Fatalf("redactFreeText(%q)\n got %q\nwant %q", tt.note, got, tt.want)
			}
