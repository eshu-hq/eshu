...
- `ReadinessProbe` / `ReadinessProbesForDependencies(db, driver)` /
  ...
  body. The core status-schema probe always runs as the baseline. The reader
  must implement `status.ReadinessChecker`; construction fails if it does not.
...
- `compose_nornicdb_image_test.go` enforces that Compose pulls the immutable
  eshu-hq self-built NornicDB image by default, keeps Compose, Helm, and R-5 on the same
