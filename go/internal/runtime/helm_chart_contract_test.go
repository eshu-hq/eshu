...
func TestHelmVulnerabilityIntelligenceCollectorDeployment(t *testing.T) {
	t.Parallel()

	defaultManifests := renderHelmChart(t)
	if helmManifestExists(defaultManifests, "Deployment", "eshu-vulnerability-intelligence-collector") {
		t.Fatal("default chart render included vulnerability intelligence collector deployment")
	}

	valuesPath := filepath.Join(t.TempDir(), "vuln-values.yaml")
	values := []byte(`
contentStore:
  dsn: postgresql://eshu:secret@postgres:5432/eshu
neo4j:
...
vulnerabilityIntelligenceCollector:
  ...
  extraEnv:
    - name: NVD_API_KEY
      valueFrom:
        secretKeyRef:
          name: eshu-vuln-secrets
          key: nvd-api-key
`)
	if err := os.WriteFile(valuesPath, values, 0o600); err != nil {
		t.Fatalf("write vulnerability collector values: %v", err)
	}

	manifests := renderHelmChart(t, "-f", valuesPath)
	deployment := requireHelmManifest(t, manifests, "Deployment", "eshu-vulnerability-intelligence-collector")
	container := requireHelmContainer(t, deployment, "vulnerability-intelligence-collector")
	command := helmStringSlice(container["command"])
	if !stringSlicesEqual(command, []string{"/usr/local/bin/eshu-collector-vulnerability-intelligence"}) {
		t.Fatalf("vulnerability collector command = %#v", command)
	}

	env := helmEnvByName(container)
	assertHelmLiteralEnv(t, env, "ESHU_VULNERABILITY_INTELLIGENCE_COLLECTOR_INSTANCE_ID", "vulnerability-intelligence-primary")
	assertHelmLiteralEnv(t, env, "ESHU_VULNERABILITY_INTELLIGENCE_POLL_INTERVAL", "5s")
	assertHelmLiteralEnv(t, env, "ESHU_VULNERABILITY_INTELLIGENCE_CLAIM_LEASE_TTL", "90s")
	assertHelmLiteralEnv(t, env, "ESHU_VULNERABILITY_INTELLIGENCE_HEARTBEAT_INTERVAL", "20s")
	instances := helmString(env["ESHU_COLLECTOR_INSTANCES_JSON"]["value"])
	for _, want := range []string{`"collector_kind":"vulnerability_intelligence"`, `"source":"cisa_kev"`, `"source":"nvd"`, `"api_key_env":"NVD_API_KEY"`} {
		if !strings.Contains(instances, want) {
			t.Fatalf("ESHU_COLLECTOR_INSTANCES_JSON = %q, missing %s", instances, want)
		}
	}
	if _, ok := env["NVD_API_KEY"]["valueFrom"]; !ok {
		t.Fatalf("NVD_API_KEY env = %#v, want valueFrom secret ref", env["NVD_API_KEY"])
	}

	requireHelmManifest(t, manifests, "Service", "eshu-vulnerability-intelligence-collector-metrics")
	requireHelmManifest(t, manifests, "ServiceMonitor", "eshu-vulnerability-intelligence-collector-metrics")
	requireHelmManifest(t, manifests, "NetworkPolicy", "eshu-vulnerability-intelligence-collector")
	requireHelmManifest(t, manifests, "PodDisruptionBudget", "eshu-vulnerability-intelligence-collector")
}

func TestHelmVulnerabilityIntelligenceCollectorRequiresMatchingCoordinatorInstance(t *testing.T) {
	t.Parallel()

	valuesPath := filepath.Join(t.TempDir(), "vuln-missing-coordinator-instance.yaml")
	values := []byte(`
workflowCoordinator:
  enabled: true
  deploymentMode: active
...
`)
	...
	if !strings.Contains(output, "workflowCoordinator.collectorInstances must contain an enabled claim-driven vulnerability_intelligence instance") {
		t.Fatalf("helm template error = %q, want matching coordinator instance requirement", output)
	}
}

func TestHelmVulnerabilityIntelligenceCollectorRequiresMatchingLocalInstance(t *testing.T) {
	t.Parallel()

	valuesPath := filepath.Join(t.TempDir(), "vuln-mismatch.yaml")
	values := []byte(`
workflowCoordinator:
  enabled: true
  deploymentMode: active
