...
func suggestInvestigations(report *Report, spec sectionSpec, input SectionInput, section ReportSection) {
	for _, candidate := range sectionInvestigations(spec, input, section) {
...
func sectionInvestigations(spec sectionSpec, input SectionInput, section ReportSection) []SuggestedInvestigation {
	...
	if section.Status == StatusUnsupported && !ambiguous {
		out = append(out, unsupportedLaneInvestigation(spec, section))
	}
...
func unsupportedLaneInvestigation(spec sectionSpec, section ReportSection) SuggestedInvestigation {
	return SuggestedInvestigation{
		ID:                 investigationID(spec.Kind, BasisUnsupportedLane, fallbackTarget(spec.Fallback)),
		Section:            spec.Kind,
		...
		NextCall:           spec.Fallback,
		ExpectedTruthClass: expectedTruthClass(section, spec.Fallback),
	}
}
