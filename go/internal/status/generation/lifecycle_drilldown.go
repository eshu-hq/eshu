...
// MaxLifecycleLimit is the hard upper bound on rows a single
...
const MaxLifecycleLimit = 500

// DefaultLifecycleLimit is the page size used when a caller does not
// specify a limit.
const DefaultLifecycleLimit = 50
...
// range. A zero or negative Limit becomes DefaultLifecycleLimit and a
// value above MaxLifecycleLimit is clamped to the cap.
func (f LifecycleFilter) Normalize() LifecycleFilter {
	...
	if f.Limit <= 0 {
		f.Limit = DefaultLifecycleLimit
	}
	if f.Limit > MaxLifecycleLimit {
		f.Limit = MaxLifecycleLimit
	}
	return f
}
...
func (f LifecycleFilter) HasScopeSelector() bool {
	return strings.TrimSpace(f.ScopeID) != "" ||
		strings.TrimSpace(f.Repository) != "" ||
		strings.TrimSpace(f.GenerationID) != ""
}

// LifecycleRecord is one scope-generation lifecycle row joined with
...
type LifecycleRecord struct {
	...
	// failure class.
	LatestFailure *LatestFailure `json:"latest_failure,omitempty"`
}

// QueueStatus summarizes the fact_work_items queue rows that belong to
...
// LatestFailure carries the most recent failure class and message
...
// LifecycleTimestamp formats a database timestamp as RFC3339 UTC, or
