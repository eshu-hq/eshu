...
- **RetryingExecutor.ExecuteGroup retries on replay-safe groups** — both
  ...
  Exact v1.3.3 RUNS_ON transaction timeouts follow rollback and defer once
  to the bounded queue as `graph_write_timeout`, without a local retry.
...
- **CanonicalNodeWriter.Write wraps escaping errors as retryable** — every
  ...
  other graph writer in this package (`edge/writer/writer.go`, `cloud_resource_node_writer.go`,
...
- **Add a new shared projection domain (EdgeWriter)** → add the domain constant
  ...
  case in `edge/writer/writer.go`; add tests in `edge/writer/dispatch_test.go`. Verify the new
  UNWIND Cypher template against both Neo4j and NornicDB if both backends are
  active.

- **Change SQL relationship writes** → update `edge/writer/sql.go`,
