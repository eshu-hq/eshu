...
const retractInheritanceEdgesCypher = `MATCH (child)-[rel:INHERITS|IMPLEMENTS|OVERRIDES|ALIASES]->()
...
const retractCodeCallFallbackEdgesCypher = `MATCH (source:Function|Class|Struct|Interface|TypeAlias|File)-[rel:CALLS|REFERENCES|INSTANTIATES|USES_METACLASS]->()
WHERE source.repo_id IN $repo_ids
  AND rel.evidence_source = $evidence_source
DELETE rel`
