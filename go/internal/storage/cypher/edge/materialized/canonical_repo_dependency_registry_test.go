...
//
// The live path is edgewriter.BuildRepoDependencySplitRetractStatements, which splits into
...
func TestRepoDependencyRegistryMatchesTheLiveRetractRoles(t *testing.T) {
	...
	stmts := edgewriter.BuildRepoDependencySplitRetractStatements([]string{"repo-1"}, "resolver/cross-repo")
	if len(stmts) == 0 {
...
func TestRepoDependencyRegistryDerivesTheAlternationRatherThanRelistingIt(t *testing.T) {
	...
	registry := RepoDependencyMaterializedEdgeTypes()
	for _, edgeType := range strings.Split(sourcecypher.RepoDependencyRelationshipEdgeTypes, "|") {
	...
	if len(registry) != len(strings.Split(sourcecypher.RepoDependencyRelationshipEdgeTypes, "|"))+1 {
