...
func codeCallResolutionMethod(payload map[string]any) codeprovenance.Method {
	if method := sourcecypher.PayloadString(payload, "resolution_method"); method != "" {
		return method
	}
...
func buildCodeCallRowMap(
	...
	if relationshipType := sourcecypher.PayloadString(payload, "relationship_type"); relationshipType == "USES_METACLASS" {
		...
		return sourcecypher.BatchCanonicalMetaclassUpsertCypher, rowMap, true
	}

	callerEntityID := sourcecypher.PayloadString(payload, "caller_entity_id")
	calleeEntityID := sourcecypher.PayloadString(payload, "callee_entity_id")
	...
	if sourcecypher.PayloadString(payload, "relationship_type") == "INSTANTIATES" {
		...
		return sourcecypher.BatchCanonicalInstantiatesUpsertCypher, rowMap, true
	}
	if rowMap["call_kind"] == "jsx_component" || sourcecypher.PayloadString(payload, "relationship_type") == "REFERENCES" {
		...
		return sourcecypher.BatchCanonicalCodeReferenceUpsertCypher, rowMap, true
	}
...
func annotateEdgeStatementSummaries(domain string, cypher string, stmts []sourcecypher.Statement) {
	for index := range stmts {
		rowCount, _ := sourcecypher.StatementRowsCount(stmts[index])
		...
		stmts[index].Parameters[sourcecypher.StatementMetadataSummaryKey] = summary
	}
}
