...
func TestCrossRepoResolutionDispatchesTypedRelationshipsIntoNeo4jWrites(t *testing.T) {
	...
	typedRepoWrites := map[string]*sourcecypher.Statement{}
	var runsOnWrite *sourcecypher.Statement
	...
	for _, want := range []string{
		...
		if typedRepoWrite.Operation != sourcecypher.OperationCanonicalUpsert {
			t.Fatalf("typed repo write operation = %q, want %q", typedRepoWrite.Operation, sourcecypher.OperationCanonicalUpsert)
		}
	...
	if runsOnWrite.Operation != sourcecypher.OperationCanonicalUpsert {
		t.Fatalf("runs_on write operation = %q, want %q", runsOnWrite.Operation, sourcecypher.OperationCanonicalUpsert)
	}
