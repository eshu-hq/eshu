...
func NewEdgeWriter(executor sourcecypher.Executor, batchSize int) *EdgeWriter {
...
func (w *EdgeWriter) WriteEdges(
	...
	for _, row := range rows {
		...
		if domain == reducer.DomainRepoDependency {
			for _, artifactRow := range repoEvidenceArtifactRowsFromIntent(row, evidenceSource) {
				artifactCypher := sourcecypher.BatchCanonicalRepoEvidenceArtifactUpsertCypher
				if sourcecypher.PayloadString(artifactRow, "environment") != "" {
					artifactCypher = sourcecypher.BatchCanonicalRepoEvidenceArtifactWithEnvironmentUpsertCypher
				}
	...
	var artifactStmts []sourcecypher.Statement
	bs := w.batchSizeForDomain(domain)
	for _, cypher := range routeOrder {
		routeStatements := sourcecypher.BuildEdgeRouteStatements(cypher, routedRows[cypher], bs)
	...
	for _, cypher := range artifactRouteOrder {
		routeStatements := sourcecypher.BuildBatchedStatements(cypher, artifactRows[cypher], bs)
		annotateEdgeStatementSummaries(domain, cypher, routeStatements)
	...
	if ge, ok := w.executor.(sourcecypher.GroupExecutor); ok && !sequentialSQL {
...
func batchCypherForDomain(domain string) (string, error) {
	...
	case reducer.DomainRepoDependency:
		return sourcecypher.BatchCanonicalRepoDependencyUpsertCypher, nil
	case reducer.DomainWorkloadDependency:
		return sourcecypher.BatchCanonicalWorkloadDependencyUpsertCypher, nil
	case reducer.DomainCodeCalls:
		return sourcecypher.BatchCanonicalCodeCallUpsertCypher, nil
	case reducer.DomainInheritanceEdges:
		return sourcecypher.BatchCanonicalInheritanceEdgeUpsertCypher, nil
	case reducer.DomainDocumentationEdges:
		return sourcecypher.BatchCanonicalDocumentationEntityEdgeCypher, nil
	case reducer.DomainRationaleEdges:
		return sourcecypher.BatchCanonicalRationaleExplainsEdgeCypher, nil
	case reducer.DomainSQLRelationships:
	...
	case reducer.DomainDeployableUnitEdges:
		return sourcecypher.BatchCanonicalDeployableUnitCorrelationUpsertCypher, nil
	case reducer.DomainHandlesRoute:
		return sourcecypher.BatchCanonicalHandlesRouteEdgeUpsertCypher, nil
	case reducer.DomainRunsIn:
		return sourcecypher.BatchCanonicalRunsInEdgeUpsertCypher, nil
	case reducer.DomainInvokesCloudAction:
		return sourcecypher.BatchCanonicalInvokesCloudActionUpsertCypher, nil
	case reducer.DomainCodeownersOwnershipEdges:
		return sourcecypher.BatchCanonicalCodeownersOwnershipEdgeCypher, nil
	case reducer.DomainSubmodulePinEdges:
		return sourcecypher.BatchCanonicalSubmodulePinEdgeCypher, nil
	default:
...
func buildRowMap(
	...
	switch domain {
	case reducer.DomainRepoDependency:
		repoID := sourcecypher.PayloadString(row.Payload, "repo_id")
		targetRepoID := sourcecypher.PayloadString(row.Payload, "target_repo_id")
		relationshipType := sourcecypher.PayloadString(row.Payload, "relationship_type")
		if repoID == "" {
			return "", nil, false
		}
		if relationshipType == string(edgetype.RunsOn) {
			platformID := sourcecypher.PayloadString(row.Payload, "platform_id")
			if platformID == "" {
				return "", nil, false
			}
	...
	case reducer.DomainWorkloadDependency:
		workloadID := sourcecypher.PayloadString(row.Payload, "workload_id")
		targetWorkloadID := sourcecypher.PayloadString(row.Payload, "target_workload_id")
		if workloadID == "" || targetWorkloadID == "" {
			return "", nil, false
		}
		return sourcecypher.BatchCanonicalWorkloadDependencyUpsertCypher, map[string]any{
	...
	case reducer.DomainRunsIn:
		return sourcecypher.BuildRunsInRowMap(row.Payload, evidenceSource)

	case reducer.DomainInvokesCloudAction:
		return sourcecypher.BuildInvokesCloudActionRowMap(row.Payload, evidenceSource)

	case reducer.DomainCodeownersOwnershipEdges:
		return sourcecypher.BuildCodeownersOwnershipRowMap(row.Payload, evidenceSource)

	case reducer.DomainSubmodulePinEdges:
		return sourcecypher.BuildSubmodulePinRowMap(row.Payload, evidenceSource)
