...
func TestBuildInheritanceRowMapRoutesImplementsCanonicalFallback(t *testing.T) {
	...
	if cypher != sourcecypher.BatchCanonicalImplementsEdgeUpsertCypher {
...
func TestRetractInheritanceEdgesCoversImplements(t *testing.T) {
	stmts := sourcecypher.BuildRetractInheritanceEdgeStatements([]string{"r"}, "reducer/inheritance")
	found := false
...
func TestBuildCodeCallRowMapRoutesInstantiatesCanonicalFallback(t *testing.T) {
	...
	if cypher != sourcecypher.BatchCanonicalInstantiatesUpsertCypher {
...
func TestRetractCodeCallEdgesCoversInstantiates(t *testing.T) {
	for _, tc := range []struct {
		...
		stmts := sourcecypher.BuildRetractCodeCallEdgeStatements([]string{"r"}, tc.evidenceSource)
		found := false
		for _, stmt := range stmts {
