...
func applyRetractWrite(
	existing []sourcecypher.GraphDenormalizedEdge,
	retractAnchors map[string]struct{},
	authoritativeWrites []sourcecypher.GraphDenormalizedEdge,
) []sourcecypher.GraphDenormalizedEdge {
	converged := make([]sourcecypher.GraphDenormalizedEdge, 0, len(existing)+len(authoritativeWrites))
...
func TestReconciliationConvergesStaleGenerationAfterPartialFailure(t *testing.T) {
	...
	postReport := sourcecypher.ClassifyReconciliationDrift(authoritative, converged)
	if !postReport.Converged() {
		t.Fatalf("graph did not converge after retract+write, counts %v", postReport.Counts)
	}
	if got := postReport.Counts[sourcecypher.ReconciliationDriftInSync]; got != 1 {
...
func summarizeStatements(stmts []sourcecypher.Statement) string {
