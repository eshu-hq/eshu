...
func TestRepairAnchorsYieldDomainRetractAnchorsNotEdgeKeys(t *testing.T) {
	...
	authoritative := []sourcecypher.AuthoritativePostgresGeneration{{
		Identity:     sourcecypher.AcceptanceIdentity{ScopeID: "scope-1", AcceptanceUnitID: "repo-a", SourceRunID: "run-1"},
		GenerationID: "gen-2",
		ResolvedIDs:  resolvedIDSet("resolved-2"),
	}}
	graph := []sourcecypher.GraphDenormalizedEdge{{
		EdgeKey:       "edge-gen1",
	...
	report := sourcecypher.ClassifyReconciliationDrift(authoritative, graph)
...
func TestRepairAnchorsScopeAnchoredDomainUsesScopeID(t *testing.T) {
	...
	authoritative := []sourcecypher.AuthoritativePostgresGeneration{{
		Identity:     sourcecypher.AcceptanceIdentity{ScopeID: "scope-doc", AcceptanceUnitID: "unit-doc", SourceRunID: "run-1"},
		GenerationID: "gen-2",
	}}
	graph := []sourcecypher.GraphDenormalizedEdge{{
		EdgeKey:       "edge-doc",
...
func TestClassifyReconciliationDriftExactAcceptanceIdentityNoCollapse(t *testing.T) {
	...
	authoritative := []sourcecypher.AuthoritativePostgresGeneration{
		{
			Identity:     sourcecypher.AcceptanceIdentity{ScopeID: "scope-A", AcceptanceUnitID: "repo-shared", SourceRunID: "run-A"},
			GenerationID: "gen-A2",
			ResolvedIDs:  resolvedIDSet("resolved-A"),
		},
		{
			Identity:     sourcecypher.AcceptanceIdentity{ScopeID: "scope-B", AcceptanceUnitID: "repo-shared", SourceRunID: "run-B"},
			GenerationID: "gen-B5",
			ResolvedIDs:  resolvedIDSet("resolved-B"),
		},
	}

	edges := []sourcecypher.GraphDenormalizedEdge{
		{
	...
	report := sourcecypher.ClassifyReconciliationDrift(authoritative, edges)
	...
	if got := report.Counts[sourcecypher.ReconciliationDriftInSync]; got != 2 {
...
func TestClassifyReconciliationDriftExactKeyDetectsScopeSpecificDrift(t *testing.T) {
	...
	if got := report.Counts[sourcecypher.ReconciliationDriftInSync]; got != 1 {
		t.Fatalf("in_sync count = %d, want 1", got)
	}
	if got := report.Counts[sourcecypher.ReconciliationDriftStaleGeneration]; got != 1 {
		t.Fatalf("stale_generation count = %d, want 1", got)
	}
	if report.Findings[0].Class != sourcecypher.ReconciliationDriftInSync {
		t.Fatalf("scope-A edge class = %q, want in_sync", report.Findings[0].Class)
	}
	if report.Findings[1].Class != sourcecypher.ReconciliationDriftStaleGeneration {
