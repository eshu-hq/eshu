...
func TestKubernetesNamespaceNodeWriterEmptyRowsIsNoOp(t *testing.T) {
	...
	writer := sourcecypher.NewKubernetesNamespaceNodeWriter(executor, 0)
...
func findUpsertCall(t *testing.T, calls []sourcecypher.Statement) sourcecypher.Statement {
	t.Helper()
	var upserts []sourcecypher.Statement
...
func TestKubernetesNamespaceNodeWriterSplitsMixedBatch(t *testing.T) {
	...
	var upsertCalls []sourcecypher.Statement
...
func TestKubernetesNamespaceNodeWriterUsesGroupExecutorAtomically(t *testing.T) {
	...
	for _, stmt := range executor.groupCalls[0] {
		if stmt.Operation != sourcecypher.OperationCanonicalUpsert {
			t.Fatalf("grouped statement Operation = %q, want %q (retract must never enter the group)", stmt.Operation, sourcecypher.OperationCanonicalUpsert)
		}
	}
}
...
func TestKubernetesNamespaceNodeWriterSatisfiesReducerInterface(t *testing.T) {
	...
	} = sourcecypher.NewKubernetesNamespaceNodeWriter(&recordingExecutor{}, 0)
}
...
func TestKubernetesNamespaceNodeWriterEmitsRetractBeforeUpsert(t *testing.T) {
	...
	if retractCall.Operation != sourcecypher.OperationCanonicalRetract {
		t.Fatalf("calls[0].Operation = %q, want %q (retract must be emitted first)", retractCall.Operation, sourcecypher.OperationCanonicalRetract)
	}
	...
	if upsertCall.Operation != sourcecypher.OperationCanonicalUpsert {
		t.Fatalf("calls[1].Operation = %q, want %q (upsert must follow the retract)", upsertCall.Operation, sourcecypher.OperationCanonicalUpsert)
	}
}
...
func (g *namespaceTargetsEnvironmentGraph) apply(t *testing.T, stmt sourcecypher.Statement) {
	t.Helper()
	switch stmt.Cypher {
	case sourcecypher.RetractKubernetesNamespaceStaleTargetsEnvironmentCypher:
	...
	case sourcecypher.CanonicalKubernetesNamespaceWithEnvironmentUpsertCypher:
		rows, _ := stmt.Parameters["rows"].([]map[string]any)
		for _, row := range rows {
			uid, _ := row["uid"].(string)
	...
	case sourcecypher.CanonicalKubernetesNamespaceUpsertCypher:
