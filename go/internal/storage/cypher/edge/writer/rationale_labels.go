...
func buildRationaleRowMap(
	...
	rationaleUID := sourcecypher.PayloadString(payload, "rationale_uid")
	...
	rowMap := map[string]any{
		...
		"repo_id":          sourcecypher.PayloadString(payload, "repo_id"),
		"comment_kind":     sourcecypher.PayloadString(payload, "comment_kind"),
		"excerpt_hash":     sourcecypher.PayloadString(payload, "excerpt_hash"),
		"evidence_source":  evidenceSource,
	}
	return sourcecypher.BatchCanonicalRationaleExplainsEdgeCypher, rowMap, true
}

// BuildRetractRationaleEdges builds a repo-scoped EXPLAINS edge retraction
// statement.
func BuildRetractRationaleEdges(repoIDs []string, evidenceSource string) sourcecypher.Statement {
	...
	if len(sources) > 1 {
		return sourcecypher.Statement{
			...
			Cypher:    sourcecypher.RetractCanonicalRationaleEdgesCypher,
	...
	return sourcecypher.Statement{
		Operation: sourcecypher.OperationCanonicalRetract,
		Cypher:    sourcecypher.RetractRationaleEdgesCypher,
		Parameters: map[string]any{
			"repo_ids":        repoIDs,
			"evidence_source": evidenceSource,
		},
...
// retry.
func BuildProbeRationaleEdges(repoIDs []string, evidenceSource string) sourcecypher.Statement {
	sources := rationaleRetractEvidenceSources(evidenceSource)
	if len(sources) > 1 {
		return sourcecypher.Statement{
			Operation: sourcecypher.OperationCanonicalProbe,
			Cypher:    sourcecypher.ProbeCanonicalRationaleEdgesCypher,
			Parameters: map[string]any{
				"repo_ids":         repoIDs,
				"evidence_sources": sources,
			},
	...
	return sourcecypher.Statement{
		Operation: sourcecypher.OperationCanonicalProbe,
		Cypher:    sourcecypher.ProbeRationaleEdgesCypher,
		Parameters: map[string]any{
			"repo_ids":        repoIDs,
			"evidence_source": evidenceSource,
		},
...
func (w *EdgeWriter) retractRationaleEdgesWithProbe(
	...
	pe, ok := w.executor.(sourcecypher.ProbeExecutor)
	if !ok {
		w.observeRationaleRetractProbe(ctx, rationaleRetractProbeOutcomeUnsupported, repoIDs, rationaleWholeScopeProbeScope, 0, nil)
		return sourcecypher.WrapRetryableNeo4jError(w.executor.Execute(ctx, deleteStmt))
	}
...
func BuildRetractRationaleEdgeStatementsByFilePath(filePaths []string, evidenceSource string) []sourcecypher.Statement {
	...
	return buildRationaleDeltaStatementsByFilePath(
		...
		sourcecypher.OperationCanonicalRetract, rationaleDeltaRetractTerminalClause,
	)
}
...
func BuildProbeRationaleEdgeStatementsByFilePath(filePaths []string, evidenceSource string) []sourcecypher.Statement {
	evidencePredicate, evidenceParameters := rationaleDeltaEvidencePredicate(evidenceSource)

	return buildRationaleDeltaStatementsByFilePath(
		filePaths, evidencePredicate, evidenceParameters,
		sourcecypher.OperationCanonicalProbe, rationaleDeltaProbeTerminalClause,
	)
}
...
func buildRationaleDeltaStatementsByFilePath(
	...
	stmts := make([]sourcecypher.Statement, 0, len(sourcecypher.RationaleExplainsTargetLabels))
	for _, label := range sourcecypher.RationaleExplainsTargetLabels {
