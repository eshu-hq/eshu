...
func TestBuildProbeRationaleEdgesMirrorsRetractShape(t *testing.T) {
	...
	t.Run("multi-source canonical", func(t *testing.T) {
		...
		if probe.Cypher != sourcecypher.ProbeCanonicalRationaleEdgesCypher {
		...
		if retract.Operation != sourcecypher.OperationCanonicalRetract {
			t.Fatalf("retract.Operation = %q, want OperationCanonicalRetract", retract.Operation)
		}
		if probe.Operation != sourcecypher.OperationCanonicalProbe {
	...
	t.Run("single-source custom", func(t *testing.T) {
		...
		if probe.Cypher != sourcecypher.ProbeRationaleEdgesCypher {
...
type probeGuardRecordingExecutor struct {
	executeCalls []sourcecypher.Statement
	probeCalls   []sourcecypher.Statement
	probeFound   bool
	probeErr     error
}

func (e *probeGuardRecordingExecutor) Execute(_ context.Context, stmt sourcecypher.Statement) error {
	e.executeCalls = append(e.executeCalls, stmt)
	return nil
}

func (e *probeGuardRecordingExecutor) ExecuteProbe(_ context.Context, stmt sourcecypher.Statement) (bool, error) {
...
func TestEdgeWriterRetractEdgesRationaleProbeUsesSameParametersAsDelete(t *testing.T) {
	...
	if deleteStmt.Operation != sourcecypher.OperationCanonicalRetract {
		t.Fatalf("delete Operation = %q, want OperationCanonicalRetract", deleteStmt.Operation)
	}
	if probeStmt.Operation != sourcecypher.OperationCanonicalProbe {
