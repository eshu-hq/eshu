...
func (w *EdgeWriter) RetractEdges(
	...
	if domain == reducer.DomainCodeCalls {
		...
		if hasDeltaScope {
			stmts := sourcecypher.BuildRetractCodeCallEdgeStatementsByFilePath(filePaths, evidenceSource)
	...
	if domain == reducer.DomainDocumentationEdges {
		...
		return sourcecypher.WrapRetryableNeo4jError(
	...
	if domain == reducer.DomainCodeCalls {
		...
		stmts := sourcecypher.BuildRetractCodeCallEdgeStatements(repoIDs, evidenceSource)
		return w.executeCodeCallRetractStatements(ctx, stmts)
	}
	...
	if domain == reducer.DomainCodeownersOwnershipEdges {
		...
		if hasDeltaScope {
			stmt := sourcecypher.BuildRetractCodeownersOwnershipEdgesByFilePath(repoIDs, filePaths, evidenceSource)
			return sourcecypher.WrapRetryableNeo4jError(w.executor.Execute(ctx, stmt))
		}
	}
...
func (w *EdgeWriter) executeCodeCallRetractStatements(ctx context.Context, stmts []sourcecypher.Statement) error {
...
// is safe.
func (w *EdgeWriter) executeInheritanceRetractStatements(ctx context.Context, stmts []sourcecypher.Statement) error {
	return w.executeSequentialRetractStatements(ctx, stmts)
}

func (w *EdgeWriter) executeSQLRelationshipRetractStatements(ctx context.Context, stmts []sourcecypher.Statement) error {
...
func (w *EdgeWriter) executeGuardedRationaleDeltaRetracts(
	ctx context.Context,
	probes []sourcecypher.Statement,
	retracts []sourcecypher.Statement,
	repoIDs []string,
) error {
	...
	pe, canProbe := w.executor.(sourcecypher.ProbeExecutor)
...
func (w *EdgeWriter) executeSequentialRetractStatements(ctx context.Context, stmts []sourcecypher.Statement) error {
...
func (w *EdgeWriter) executeDocumentationRetractStatements(ctx context.Context, stmts []sourcecypher.Statement) error {
	return w.executeSequentialRetractStatements(ctx, stmts)
}
...
func buildRetractStatement(
	...
	case reducer.DomainWorkloadDependency:
		return sourcecypher.BuildRetractWorkloadDependencyEdges(repoIDs, evidenceSource), nil
	...
	case reducer.DomainDeployableUnitEdges:
		return sourcecypher.BuildRetractDeployableUnitCorrelationEdges(repoIDs, evidenceSource), nil
	case reducer.DomainHandlesRoute:
		return sourcecypher.Statement{
			Operation: sourcecypher.OperationCanonicalRetract,
			Cypher:    sourcecypher.RetractHandlesRouteEdgesCypher,
	...
	case reducer.DomainRunsIn:
		return sourcecypher.Statement{
			Operation: sourcecypher.OperationCanonicalRetract,
			Cypher:    sourcecypher.RetractRunsInEdgesCypher,
			Parameters: map[string]any{
				"repo_ids":        repoIDs,
				"evidence_source": evidenceSource,
			},
	...
	case reducer.DomainInvokesCloudAction:
		return sourcecypher.Statement{
			Operation: sourcecypher.OperationCanonicalRetract,
			Cypher:    sourcecypher.RetractInvokesCloudActionEdgesCypher,
			Parameters: map[string]any{
				"repo_ids":        repoIDs,
				"evidence_source": evidenceSource,
			},
	...
	case reducer.DomainCodeownersOwnershipEdges:
		return sourcecypher.BuildRetractCodeownersOwnershipEdges(repoIDs, evidenceSource), nil
	...
	case reducer.DomainSubmodulePinEdges:
		return sourcecypher.BuildRetractSubmodulePinEdges(repoIDs, evidenceSource), nil
	default:
		return sourcecypher.Statement{}, fmt.Errorf("unsupported domain for retract: %q", domain)
	}
}
...
func (w *EdgeWriter) retractFencedRepoWideDomain(
	...
	if domain == reducer.DomainInheritanceEdges {
		...
		if hasDeltaScope {
			stmts := sourcecypher.BuildRetractInheritanceEdgeStatementsByFilePath(filePaths, evidenceSource)
			...
			return true, w.executeInheritanceRetractStatements(
				ctx,
				sourcecypher.BuildRetractInheritanceEdgeStatements(wholeScopeRepoIDs, evidenceSource),
			)
		}
		...
		stmts := sourcecypher.BuildRetractInheritanceEdgeStatements(wholeScopeRepoIDs, evidenceSource)
