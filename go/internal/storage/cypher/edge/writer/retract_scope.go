...
func collectWholeScopeRefreshRepoIDs(rows []reducer.SharedProjectionIntentRow) []string {
	...
	var result []string
	for _, row := range rows {
		if sourcecypher.PayloadBool(row.Payload, "delta_projection") {
			continue
		}
		if sourcecypher.PayloadString(row.Payload, "intent_type") != reducer.RepoRefreshIntentType {
			continue
		}
		repoID := row.RepositoryID
		if repoID == "" {
...
func collectScopeIDs(rows []reducer.SharedProjectionIntentRow) []string {
	...
	for _, row := range rows {
		...
		if scopeID == "" {
			scopeID = strings.TrimSpace(sourcecypher.PayloadString(row.Payload, "scope_id"))
		}
		if scopeID == "" {
			continue
		}
...
func retractRowRepositoryID(row reducer.SharedProjectionIntentRow) string {
	...
	return sourcecypher.PayloadString(row.Payload, "repo_id")
}
...
func collectDeltaFilePaths(rows []reducer.SharedProjectionIntentRow) ([]string, bool, error) {
	...
	for _, row := range rows {
		...
		hasDeltaScope = true
		deltaRowCount++
		rowFilePaths := sourcecypher.PayloadStringSlice(row.Payload, "delta_file_paths")
...
func collectDocumentationDeltaScope(rows []reducer.SharedProjectionIntentRow) (documentationRetractScope, bool, error) {
	...
	for _, row := range rows {
		...
		hasDeltaScope = true
		rowDocumentIDs := sourcecypher.PayloadStringSlice(row.Payload, "document_ids")
		...
		for _, sectionUID := range sourcecypher.PayloadStringSlice(row.Payload, "section_uids") {
...
func buildDocumentationDeltaRetractStatements(
	...
	stmts := make([]sourcecypher.Statement, 0, 2)
