...
func copyRepoRelationshipMetadata(rowMap map[string]any, payload map[string]any, rowGenerationID string) {
	rowMap["resolved_id"] = sourcecypher.PayloadString(payload, "resolved_id")
	generationID := sourcecypher.PayloadString(payload, "generation_id")
	if generationID == "" {
		generationID = rowGenerationID
	}
	rowMap["generation_id"] = generationID
	rowMap["evidence_count"] = sourcecypher.PayloadInt(payload, "evidence_count")
	rowMap["evidence_kinds"] = sourcecypher.PayloadStringSlice(payload, "evidence_kinds")
	rowMap["resolution_source"] = sourcecypher.PayloadString(payload, "resolution_source")
	rowMap["confidence"] = sourcecypher.RepoRelationshipConfidence(sourcecypher.PayloadFloat(payload, "confidence"))
	rowMap["rationale"] = sourcecypher.PayloadString(payload, "rationale")
	rowMap["source_revision"] = sourcecypher.PayloadString(payload, "source_revision")
	rowMap["first_party_ref_version"] = sourcecypher.PayloadString(payload, "first_party_ref_version")
}
...
func repoEvidenceArtifactRowsFromIntent(
	...
	targetRepoID := sourcecypher.PayloadString(payload, "target_repo_id")
	if repoID == "" || targetRepoID == "" {
		return nil
	}
	artifacts := sourcecypher.PayloadMapSlice(payload, "evidence_artifacts")
	if len(artifacts) == 0 {
		return nil
	}

	relationshipType := sourcecypher.PayloadString(payload, "relationship_type")
	resolvedID := sourcecypher.PayloadString(payload, "resolved_id")
	generationID := sourcecypher.PayloadString(payload, "generation_id")
	if generationID == "" {
	...
	for _, artifact := range artifacts {
		evidenceKind := sourcecypher.PayloadString(artifact, "evidence_kind")
		path := sourcecypher.PayloadString(artifact, "path")
		matchedValue := sourcecypher.PayloadString(artifact, "matched_value")
		fluxName := sourcecypher.PayloadString(artifact, "flux_git_repository_name")
		fluxNamespace := sourcecypher.PayloadString(artifact, "flux_git_repository_namespace")
		name := path
		if name == "" {
			name = evidenceKind
		}
		...
		row := map[string]any{
			...
			"artifact_family":               sourcecypher.PayloadString(artifact, "artifact_family"),
			"path":                          path,
			"extractor":                     sourcecypher.PayloadString(artifact, "extractor"),
			"environment":                   sourcecypher.PayloadString(artifact, "environment"),
			"runtime_platform_kind":         sourcecypher.PayloadString(artifact, "runtime_platform_kind"),
			"matched_alias":                 sourcecypher.PayloadString(artifact, "matched_alias"),
			"matched_value":                 matchedValue,
			"flux_git_repository_name":      fluxName,
			"flux_git_repository_namespace": fluxNamespace,
			"confidence":                    sourcecypher.PayloadFloat(artifact, "confidence"),
