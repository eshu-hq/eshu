...
func (e *runsOnEdgeConflictExecutor) Execute(context.Context, sourcecypher.Statement) error {
	return errors.New("RUNS_ON edge write must use an atomic group")
}

func (e *runsOnEdgeConflictExecutor) ExecuteGroup(_ context.Context, statements []sourcecypher.Statement) error {
	e.groupAttempts++
	if len(statements) != 2 ||
