...
// edges still in the graph from the SqlView/SqlFunction source_tables
...
func buildSQLRelationshipRetractStatements(unwindParam, alias, anchorProp string, scopeValues []string, evidenceSource string) []sourcecypher.Statement {
	stmts := make([]sourcecypher.Statement, 0, len(sqlRelationshipRetractSourceLabels))
...
func buildSQLRelationshipRowMap(
	...
	sourceEntityID := sourcecypher.PayloadString(payload, "source_entity_id")
	...
	sourceLabel := sourcecypher.PayloadString(payload, "source_entity_type")
	targetLabel := sourcecypher.PayloadString(payload, "target_entity_type")
	...
	case "QUERIES_TABLE":
		return sourcecypher.BatchCanonicalSQLQueriesTableUpsertCypher, rowMap, true
	case "HAS_COLUMN":
		return sourcecypher.BatchCanonicalSQLHasColumnUpsertCypher, rowMap, true
	case "TRIGGERS":
		return sourcecypher.BatchCanonicalSQLTriggersUpsertCypher, rowMap, true
	case "EXECUTES":
		return sourcecypher.BatchCanonicalSQLExecutesUpsertCypher, rowMap, true
	default:
		return "", nil, false
	}
}
...
// given repositories.
func BuildRetractSQLRelationshipEdgeStatements(repoIDs []string, evidenceSource string) []sourcecypher.Statement {
...
func BuildRetractSQLRelationshipEdgeStatementsByFilePath(filePaths []string, evidenceSource string) []sourcecypher.Statement {
