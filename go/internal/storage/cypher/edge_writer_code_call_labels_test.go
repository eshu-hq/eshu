...
func TestEdgeWriterWriteEdgesInstantiatesUsesExactEndpointLabels(t *testing.T) {
	t.Parallel()
	...
	if strings.Contains(cypher, "Function|Class|File") || strings.Contains(cypher, "Class|Struct|Enum") {
		t.Fatalf("cypher = %q, want no multi-label fallback for typed INSTANTIATES endpoints", cypher)
	}
}
