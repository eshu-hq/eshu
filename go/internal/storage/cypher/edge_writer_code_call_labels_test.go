...
func TestEdgeWriterWriteEdgesCodeInstantiationsUsesExactEndpointLabels(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(cypher, "MATCH (target:Class {uid: row.callee_entity_id})") {
		t.Fatalf("cypher = %q, want Class target uid anchor", cypher)
	}
	if !strings.Contains(cypher, "MERGE (source)-[rel:INSTANTIATES]->(target)") {
		t.Fatalf("cypher = %q, want INSTANTIATES relationship", cypher)
	}
	summary, _ := executor.calls[0].Parameters[StatementMetadataSummaryKey].(string)
	if !strings.Contains(summary, "relationship=INSTANTIATES") {
		t.Fatalf("statement summary = %q, want INSTANTIATES relationship", summary)
	}
}
