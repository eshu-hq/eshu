...
func buildParameterizedSemanticEntityStatement(row semantic.EntityRow) (Statement, bool) {
...
func buildSemanticEntityPropertyRowMap(row semantic.EntityRow) (map[string]any, bool) {
	rowMap, ok := buildSemanticEntityRowMap(row)
	if !ok {
		return nil, false
	}
...
func buildSemanticEntityRowMap(row semantic.EntityRow) (map[string]any, bool) {
...
func buildRustImplBlockOwnershipRows(rows []semantic.EntityRow) []map[string]any {
	if len(rows) == 0 {
		return nil
	}
