...
- **Projector scope ordering** — `ProjectorQueue.Claim` must preserve one
  ...
  on one. #7115: the claim locks and bumps the scope's `projector_scope_claim_fences` row and never locks `ingestion_scopes`; only the claim may lock or update fence rows (the insert trigger, backfill and cascade delete aside), and no table may reference them (see `claimProjectorWorkQuery`'s doc comment). Keep the stale-generation coalescing CTEs together; they move older
  ...
  generation; an `active` one keeps the scope pointer until successor Ack. #7130: a `superseded` generation is terminal for Claim (second `supersedable_projector_generations` branch sweeps its pending, retrying and expired-lease rows; keep the branches un-ORed and SKIP LOCKED), Heartbeat (`projector_heartbeat_generation_superseded`), Ack and replay; see `docs/internal/evidence/7130-ack-superseded-generation-guard.md`.
...
`taintstore.CodeInterprocProjectedEdgeStore` (`code_interproc_projected_edge`,
migration 044) and `taintstore.CodeTaintEvidenceProjectedNodeStore`
(`code_taint_evidence_projected_node`, migration 045), in
`internal/storage/postgres/code/taint`, record the source-Function uid /
CodeTaintEvidence node uid of every projected value-flow artifact so the
reducer retracts by indexed uid instead of scanning the whole graph on
NornicDB. Both follow the store conventions here: `ExecQueryer`, idempotent
batched upsert with in-batch dedupe, `SchemaSQL` const mirrored by the
migration, and `SELECT DISTINCT ... ORDER BY` enumeration / bounded `DELETE`
prune, all parameterized. Written before the graph write (superset
invariant); see `go/internal/reducer/AGENTS.md` (#4893).
...
enumeration query shapes, and prune shapes. Full runtime evidence in
