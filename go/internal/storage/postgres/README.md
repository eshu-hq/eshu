...
No-Regression Evidence: `go test ./internal/storage/postgres -run 'TestReducerQueueClaimGatesIAMPermissionMaterializationOnCanonicalCloudResourceReadiness|TestReducerQueueBlockagesReportIAMPermissionReadinessWait' -count=1` failed before the reducer claim and blockage SQL included `iam_escalation_materialization` and `iam_can_perform_materialization` in the CloudResource readiness-gated domain set, then passed once those IAM edge domains stayed unclaimed until `cloud_resource_uid/canonical_nodes_committed` was present and surfaced the missing prerequisite as a readiness blockage.
