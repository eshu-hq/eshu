...
- Projector claims preserve one active source-local generation per `scope_id`,
  ...
  the scope pointer, and marking work succeeded. A superseded generation is
  terminal (#7130): Claim sweeps its claimable rows to `superseded` instead of
  claiming them, Heartbeat stops running work on it with
  `ErrWorkSuperseded`, and Ack refuses to re-activate it. Each refusal counts
  on `eshu_dp_superseded_generation_fence_total` by `failure_class`, except
  the claim sweep, which marks the row itself.
...
- Graph projection phase, shared projection intent, acceptance, freshness, and
  ...
  `generation_ingested_at` (migration 125, no backfill), and an upsert
  carrying a generation that sorts before the stored one by
  ...
  refreshes the row. A NULL stored key (a row from before #6679) is resolved
  from `scope_generations` inside the guard and filled by the next applied
  write. Batches are written in primary-key order so overlapping writers
  cannot deadlock.
