...
// array.StringArray grant value. The handler-layer fakeIaCManagementStore proofs
...
func TestAWSCloudRuntimeDriftFindingStoreScopedGrantBindsScopePredicate(t *testing.T) {
	...
	if got, want := db.queries[0].args[2], array.StringArray(grant); !reflect.DeepEqual(got, want) {
