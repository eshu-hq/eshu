...
func TestAWSScanStatusStoreCommitClearsFailureDetailsOnCommittedSuccess(t *testing.T) {
	t.Parallel()
	...
	for _, want := range []string{
		...
		if !strings.Contains(query, want) {
			t.Fatalf("CommitAWSScan() query missing committed-success failure reset %q:\n%s", want, query)
		}
	}
}
