...
func TestContainerImageIdentitySupportStoreSchemaContract(t *testing.T) {
	...
	sql := MigrationSQL("container_image_identity_support_store") +
		MigrationSQL("container_image_identity_support_store_backfill") +
...
func TestContainerImageIdentityCutoverFilesAreAdjacentAndFailClosed(t *testing.T) {
	...
	backfill := MigrationSQL("container_image_identity_support_store_backfill")
	if !strings.Contains(backfill, "WITH legacy_supports AS MATERIALIZED") ||
		!strings.Contains(backfill, "WHERE state.active_set_id IS NULL") ||
		strings.Contains(backfill, "DELETE FROM fact_records") {
		t.Fatalf("092a must retain legacy rows behind a per-scope authority switch:\n%s", backfill)
	}
}
