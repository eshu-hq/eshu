...
failed before `deduplicateEnvelopes` considered `FencingToken`, because the
last array position won even when it carried a lower token. It passed after the
helper chose the highest token while preserving last-position behavior when
tokens tie. The changed code stays inside the existing in-memory pre-batch dedup
step that Postgres requires before `INSERT ... ON CONFLICT DO UPDATE`; it adds
no SQL, index, batch-size, worker, lease, queue, graph-write, or
