...
// runs against the real Postgres schema: two scopes each with a generation and
// facts, one scope carrying two facts in the same generation, so the DISTINCT
// collapses the duplicate and the enumeration returns exactly one hydrated work
// per scope generation, ordered by (scope_id, generation_id). Gated on
// ESHU_SCOPE_GENERATION_ENUM_PROOF_DSN; the hermetic scan/shape test
// (TestListScopeGenerationWorkEnumeratesCorpus) is the always-run CI guard.
func TestListScopeGenerationWorkLive(t *testing.T) {
	...
	if got, want := len(works), 2; got != want {
		t.Fatalf("work count = %d, want %d (DISTINCT must collapse the duplicate fact)", got, want)
	}
...
func seedScopeGenerationEnumFixture(t *testing.T, ctx context.Context, db *sql.DB, observed, ingested time.Time) {
	...
	// scope-a carries two facts in the same generation; DISTINCT must collapse to
	// one enumerated work.
