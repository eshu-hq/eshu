...
const listActiveContainerImageIdentityWarningsQuery = `
SELECT
    fact.fact_id,
    fact.scope_id,
    fact.generation_id,
...
// ListActiveContainerImageIdentityWarnings loads the active OCI registry
// incompleteness declarations that can hold identity retirement. It remains
// separate from the cached identity-evidence loader so warning churn does not
// invalidate or enlarge the shared evidence cache.
func (s *FactStore) ListActiveContainerImageIdentityWarnings(
	...
	for {
		page, err := s.listActiveContainerImageIdentityWarningsPage(
			ctx,
			cursorObservedAt,
			cursorFactID,
		)
...
func (s *FactStore) listActiveContainerImageIdentityWarningsPage(
	ctx context.Context,
	...
	if err != nil {
		return nil, fmt.Errorf("list active container image identity warnings: %w", err)
	}
	...
	for rows.Next() {
		...
		if scanErr != nil {
			return nil, fmt.Errorf("list active container image identity warnings: %w", scanErr)
		}
