...
func (database *governanceAuditTenantMemoryDB) ExecContext(_ context.Context, query string, args ...any) (sql.Result, error) {
	database.execs = append(database.execs, fake.ExecCall{Query: query, Args: args})
...
// Event-id uniqueness tests (TestGovernanceAuditEventIDDistinctAcrossTenants
// and friends) live in tenant_eventid_test.go, split out to keep this file
