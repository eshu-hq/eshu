...
// the login-time require_mfa_for_all_users read fails CLOSED: a read error
// denies the login (no session issued) rather than silently skipping the
// check, mirroring the require_sso non-admin fail-closed stance in
// go/internal/query/local_identity_sign_in_policy_gate.go.
...
func TestAuthenticateLocalIdentityMFAEnforcementIsASingleSharedCodePath(t *testing.T) {
	...
	if got := strings.Count(text, "LocalIdentityAuthMFARequired}, nil"); got != 1 {
