...
func assertDeferredSelfExclusionArgs(t *testing.T, args []any) {
	...
	nonRepoIDTerms, ok := args[0].(array.StringArray)
	if !ok {
		t.Fatalf("deferred query arg[0] type = %T, want array.StringArray", args[0])
	}
	repoIDTerms, ok := args[1].(array.StringArray)
	if !ok {
		t.Fatalf("deferred query arg[1] type = %T, want array.StringArray", args[1])
	}
	...
	repoIDKeys, ok := args[6].(array.StringArray)
	if !ok {
		t.Fatalf("deferred query arg[6] ($7 repo_id reference keys) type = %T, want array.StringArray", args[6])
	}
