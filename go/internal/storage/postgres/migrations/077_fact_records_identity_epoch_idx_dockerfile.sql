...
--
-- Migration 069 matched a content_entity only when its payload carried
...
-- parsed_file_data.dockerfile_stages. Base-image lineage widened
-- identityFactFilterSQL to admit that shape, so this predicate must match it --
-- a partial index only covers a query when its predicate is a subset match of
-- the query's filter, so leaving the narrower predicate in place would silently
-- drop the epoch probe and the identity fact load back to a full scan.
