...
func TestStoreUpsertBindsRowAndAnchorMode(t *testing.T) {
	t.Parallel()
	fake := &fakeDB{}
	store := Store{DB: fake}
	wait := crossscope.ReadinessWait{
		...
		UpdatedAt: storeTestT0,
	}
	if err := store.UpsertReadinessWait(context.Background(), wait, true); err != nil {
	...
	if args[11] != true {
		t.Fatalf("reset anchor arg = %v, want true", args[11])
	}
}
...
func TestStoreSurfacesDatabaseErrors(t *testing.T) {
	...
	if err := store.ClearReadinessWait(context.Background(), "s", "d"); err == nil {
		t.Fatal("ClearReadinessWait() error = nil, want the database error")
	}
	if err := (Store{}).UpsertReadinessWait(context.Background(), crossscope.ReadinessWait{}, false); err == nil {
