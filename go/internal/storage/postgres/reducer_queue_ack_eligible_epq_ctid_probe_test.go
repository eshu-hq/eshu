...
// Compare shipped ordered-ID lookup with the proposed physical-tuple lookup.
// This diagnostic branch is deliberately test-only: a failing ctid arm rejects
// that optimization before it can replace the production statement.
func TestReducerContentionGateAckEligibleEPQCtidProbe(t *testing.T) {
	for _, domain := range []reducer.Domain{reducer.DomainSupplyChainImpact, reducer.DomainContainerImageIdentity, reducer.DomainCICDRunCorrelation} {
		for _, shape := range []string{"ordered_id", "ctid", "locked_id_only"} {
			t.Run(fmt.Sprintf("%s/%s", domain, shape), func(t *testing.T) {
				...
				switch shape {
				case "ctid":
					query = eligibleEPQCtidRewrite(t, query)
				case "locked_id_only":
					query = eligibleEPQLockedIDRewrite(t, query)
				}
				type outcome struct {
					result sql.Result
					err    error
				...
				t.Logf("shape=%s before=%s heartbeat=%s affected=%d status=%s owner=%q", shape, oldTID, newTID, affected, status, leaseOwner)
				if affected != 1 || status != "succeeded" || leaseOwner != "" {
					t.Fatalf("lost still-eligible ACK after EPQ: shape=%s affected=%d status=%s owner=%q", shape, affected, status, leaseOwner)
				}
				var emitted int64
...
// Derive only the physical target lookup from the shipped statement. Preserve
// the complete locking-level and target eligibility fences and count barrier.
func eligibleEPQCtidRewrite(t *testing.T, query string) string {
	t.Helper()
	const projection = "SELECT work_item_id\n    FROM fact_work_items"
	if strings.Count(query, projection) != 1 {
		t.Fatal("production locking projection boundary changed")
	}
	query = strings.Replace(query, projection, "SELECT work_item_id, ctid AS locked_tid\n    FROM fact_work_items", 1)
	replaced := false
	for _, alias := range []string{"work.", ""} {
		from := alias + "work_item_id IN (SELECT work_item_id FROM locked_work)"
		if strings.Contains(query, from) {
			query = strings.Replace(query, from, alias+"ctid = ANY(ARRAY(SELECT locked_tid FROM locked_work))", 1)
			replaced = true
			break
		}
	}
	if !replaced {
		t.Fatal("production locked-ID target boundary changed")
	}
	return query
}

// The locking SELECT keeps every predicate and owns the rows until completion.
// Remove only redundant outer filters, leaving immutable ID and drain barrier.
func eligibleEPQLockedIDRewrite(t *testing.T, query string) string {
	t.Helper()
	const barrier = "  AND (SELECT count(*) FROM locked_work) > 0\n"
	if strings.Count(query, barrier) != 1 {
		t.Fatal("production lock-drain boundary changed")
	}
	start := strings.Index(query, barrier) + len(barrier)
	end := strings.Index(query[start:], "\nRETURNING")
	if end < 0 {
		return query[:start]
	}
	return query[:start] + query[start+end:]
}
