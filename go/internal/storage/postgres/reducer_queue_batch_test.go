...
func TestClaimBatchGatesAWSRelationshipsOnCanonicalCloudResourceReadiness(t *testing.T) {
	...
	query := db.queries[0].query
	for _, want := range []string{
		"domain NOT IN ('aws_relationship_materialization', 'observability_coverage_materialization', 'iam_can_assume_materialization', 's3_logs_to_materialization', 'rds_posture_materialization')",
		...
		"same.domain NOT IN ('aws_relationship_materialization', 'observability_coverage_materialization', 'iam_can_assume_materialization', 's3_logs_to_materialization', 'rds_posture_materialization')",
