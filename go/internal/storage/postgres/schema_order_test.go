...
var orderedBootstrapDefinitionNames = []string{
	...
	// migrations 125/126 (#6679) add and backfill
	// shared_projection_acceptance.generation_ingested_at, the stored
	// generation's ordering key the advance-only upsert compares row-locally.
	// Split so 125's ACCESS EXCLUSIVE ALTER is not held through 126's rewrite.
	"shared_projection_acceptance_generation_key",
	"shared_projection_acceptance_generation_key_backfill",
}
