...
// scope with no partition key, so both call this one function. RepoIDFromScopeID derives the lowercased repo_id from a
// git-repository-scope:<repo_id> scope ID and returns "" for every other
// scope shape; the deferred relationship backfill uses it only as a
// performance hint, never as a correctness input.
//
