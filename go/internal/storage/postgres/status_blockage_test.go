...
func TestReducerQueueBlockagesReportAWSRelationshipReadinessWait(t *testing.T) {
	...
	for _, want := range []string{
		...
		"eligible.domain IN ('aws_relationship_materialization', 'observability_coverage_materialization', 'iam_can_assume_materialization', 's3_logs_to_materialization', 's3_external_principal_grant_materialization', 'rds_posture_materialization', 's3_internet_exposure_materialization')",
