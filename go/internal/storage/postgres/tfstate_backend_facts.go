...
// and converts each entry through the typed resolver. The query only returns
// payloads with a non-null array, but we still tolerate empty rows in case a
// fact was emitted with an empty bucket.
func scanTerragruntRemoteStateCandidates(rows Rows) ([]terraformstate.DiscoveryCandidate, error) {
	var repoID string
	var rawRemoteStates []byte
	if err := rows.Scan(&repoID, &rawRemoteStates); err != nil {
		return nil, err
	}
	...
	for _, entry := range entries {
		candidate, ok := terraformstate.TerragruntRemoteStateCandidate(repoID, entry)
		if !ok {
			continue
		}
		candidates = append(candidates, candidate)
