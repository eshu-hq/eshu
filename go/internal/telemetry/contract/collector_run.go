...
// Extracted as a sibling file so the frozen root contract.go does not grow,
// mirroring collector_stage.go and bootstrap_ingestion.go.
//
...
// to the specific collector family that held the claim. The outcome label uses
// the existing MetricDimensionOutcome ("outcome") key already in the root
// registry — no new dimension key is needed. Root registration.go appends this
// span name to the ordered spanNames slice.
