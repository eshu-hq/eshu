...
const (
	...
	// literal is replaced before hashing, so it identifies the statement shape
	// without leaking a bound parameter or an inline literal value.
	...
	// marker appended when the statement exceeds it. Every literal is replaced
	// with <REDACTED>; it never carries a parameter or an inline literal value.
