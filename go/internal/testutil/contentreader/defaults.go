...
// query sets.
func contentReaderDefaultRows(query string, results []ReaderQueryResult) driver.Rows {
...
func contentReaderReadModelDefaultRows(query string, results []ReaderQueryResult) driver.Rows {
	if strings.Contains(query, "WITH scoped_relationships AS") &&
		...
		!contentReaderHeadHasColumns(results, ReaderDeploymentEvidenceColumns()) {
		return &contentReaderRows{columns: ReaderDeploymentEvidenceColumns(), rows: nil}
	}
	if strings.Contains(query, "WITH scoped_relationships AS") &&
		strings.Contains(query, "r.evidence_count") &&
		!contentReaderHeadHasColumns(results, ReaderRelationshipReadModelColumns()) {
		return &contentReaderRows{columns: ReaderRelationshipReadModelColumns(), rows: nil}
	}
	if strings.Contains(query, "WHERE r.resolved_id = $1") &&
		!contentReaderHeadHasColumns(results, ReaderRelationshipEvidenceColumns()) {
		return &contentReaderRows{columns: ReaderRelationshipEvidenceColumns(), rows: nil}
	}
	...
	if strings.Contains(query, "FROM content_entities") &&
		...
		!contentReaderHeadHasColumns(results, ReaderDeadCodeCandidateColumns()) {
		return &contentReaderRows{columns: ReaderDeadCodeCandidateColumns(), rows: nil}
	}
	return nil
}
...
func contentReaderHeadHasColumns(results []ReaderQueryResult, columns []string) bool {
	if len(results) == 0 {
		return false
	}
...
// columns, in order.
func contentReaderResultColumnsEqual(result ReaderQueryResult, columns []string) bool {
