...
// OpenReaderTestDB, plus the assertions to run against the query that
// consumes it.
//
...
// OpenReaderTestDB returns a *sql.DB backed by a fake driver that answers
...
func OpenReaderTestDB(t *testing.T, results []ReaderQueryResult) *sql.DB {
	t.Helper()
...
func (d *contentReaderDriver) Open(string) (driver.Conn, error) {
	return &contentReaderConn{results: append([]ReaderQueryResult(nil), d.results...)}, nil
}
...
func (c *contentReaderConn) QueryContext(_ context.Context, query string, args []driver.NamedValue) (driver.Rows, error) {
	...
	if err := ReaderQueryContainsInOrder(query, result.QueryContainsInOrder); err != nil {
		return nil, err
	}
	if err := ReaderCheckArgs(args, result.WantArgs); err != nil {
		return nil, err
	}
	if result.Err != nil {
