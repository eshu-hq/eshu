...
-- every active vulnerability.cve fact (the catalog spine) and every active
-- vulnerability.known_exploited fact (the KEV CTE) with no cve_id anchor. The
-- payload-leading vulnerability indexes above cannot bound that no-anchor
-- enumeration, so the planner scans all of fact_records at collector scale.
-- These per-fact_kind partial indexes' predicates bound the scan to exactly one
-- kind's active tuples, and their (scope_id, generation_id) leading keys resolve
-- the active-generation join straight from the index (index-only when the heap
-- is vacuum-fresh, a bounded index scan otherwise).
