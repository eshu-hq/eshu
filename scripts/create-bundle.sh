...
# Helper script to create .eshu bundles from GitHub repositories
...
check_requirements() {
    ...
    if ! command -v eshu &> /dev/null; then
        print_error "eshu is not installed. Build it from source with: cd go && go build -o ../eshu ./cmd/eshu"
        exit 1
    fi
...
# Export to bundle
BUNDLE_NAME="${OUTPUT_NAME}-${TAG}-${COMMIT_SHORT}.eshu"
print_info "Exporting to bundle: $BUNDLE_NAME"

if ! eshu export "$BUNDLE_NAME" --repo .; then
