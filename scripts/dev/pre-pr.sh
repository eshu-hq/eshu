...
run_or_defer() {
	...
	# Materialize the path list first, then test rg output emptiness. A direct
	# `changed_paths | rg -q` lets rg exit on its first match and SIGPIPE the
	# upstream git/sort; under `pipefail` that nonzero would hit `|| return 0`
	# and misclassify a genuinely-triggered gate as untriggered — skipping its
	# live proof while still stamping the commit. A here-string (<<<) is not the
	# answer either: it deadlocks on bash >= 5.3. Without -q rg consumes all
	# input, so no early-exit signal exists on either side.
	local _changed; _changed="$(changed_paths)"
	[ -n "$(printf '%s\n' "${_changed}" | rg "${trigger}")" ] || return 0
