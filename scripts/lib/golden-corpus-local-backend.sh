...
#   1. reads that fixture's local_path from Postgres,
...
stage_local_backend_cassette() {
	...
	repo_local_path="$(pg "SELECT payload->>'local_path' FROM ingestion_scopes WHERE scope_kind = 'repository' AND payload->>'repo_name' = '${local_backend_fixture_repo_name}' ORDER BY observed_at DESC LIMIT 1;" | tr -d '[:space:]')"
