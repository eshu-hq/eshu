...
# whole codebase write and only THIS one this gate drives (recorded at
# scripts/lib/ifa_fault_injection_deployable_unit_lock.sh:100-107). The fault
# cell holds that table in ACCESS EXCLUSIVE MODE
# (ifa_deployable_unit_start_admission_decisions_lock, same file) =>
# blocker_kind=table_lock:admission_decisions. Confirmed live:
# scripts/lib/ifa_fault_injection_deployable_unit_cells.sh:287 calls
...
# scripts/verify-ifa-fault-injection.sh:309
...
# ordering guard in scripts/lib/test-ifa-determinism-family-cases.sh:82-95
...
# (scripts/lib/ifa_fault_generic_cells.sh:472's `table_lock:*)` arm) -- but
# the live driver has NOT been migrated to dispatch this family through it:
# scripts/verify-ifa-fault-injection.sh:482-483 still calls
