...
cell_baseline() {
	...
	baseline_rationale_retried="$(ifa_fault_count_retried "${FAULT_COMPOSE_PROJECT}" "${use_compose}" "${ESHU_POSTGRES_DSN}" "${compose_file}" "rationale_materialization")"
	baseline_rationale_retried="${baseline_rationale_retried:-0}"
	printf 'baseline: fault-free rationale_materialization retried rows (attempt_count>1): %s\n' "${baseline_rationale_retried}"
...
cell_killworker() {
	...
	ifa_det_stop_join_untrack_bg_pid "${reducer_pid_before}" KILL \
		|| die "kill-worker-after-claim: could not stop, join, and untrack the killed reducer"
