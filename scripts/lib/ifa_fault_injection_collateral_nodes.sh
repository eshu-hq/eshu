...
# the mutable SQL identity map keep their typed stable key plus every label and
# property after validating the side-specific generation. The one additional
# fixture-owned transition is the SqlIndex retarget already proved by the exact
# SQL-v2 edge assertion that runs before this comparator. Unmapped nodes remain
# byte-exact canonical records.
	...
	while IFS= read -r node_record; do
		...
		normalized="$(printf '%s\n' "${node_record}" | jq -c \
			...
			if (($identity | type) != "object")
				...
				(if $comparison_side == "baseline" then "gen-1" else "gen-2" end) as $expected_generation
...
ifa_fault_compare_collateral_nodes() {
	...
	ifa_fault_write_collateral_nodes \
		"${baseline_dump}" "${baseline_identities}" "${baseline_nodes}" baseline || return 2
	ifa_fault_write_collateral_nodes \
		"${changed_dump}" "${changed_identities}" "${changed_nodes}" changed || return 2
	local diff_rc=0
