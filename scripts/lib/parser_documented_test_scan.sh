...
validate_documented_parser_test_commands() {
  ...
  local backtick_fenced_code_candidate tilde_fenced_code_candidate
  local fenced_code_candidate inline_code_candidate
  ...
  backtick_fenced_code_candidate="(?m:^[ ]{0,3}(?<backtick_fence>\`{3,})[^\r\n]*\r?\n(?=(?:(?!^[ ]{0,3}\\k<backtick_fence>\`*[ \t]*\r?$)[\s\S])*${documented_test_signal})(?:(?!^[ ]{0,3}\\k<backtick_fence>\`*[ \t]*\r?$)[\s\S])+^[ ]{0,3}\\k<backtick_fence>\`*[ \t]*\r?$)"
  tilde_fenced_code_candidate="(?m:^[ ]{0,3}(?<tilde_fence>~{3,})[^\r\n]*\r?\n(?=(?:(?!^[ ]{0,3}\\k<tilde_fence>~*[ \t]*\r?$)[\s\S])*${documented_test_signal})(?:(?!^[ ]{0,3}\\k<tilde_fence>~*[ \t]*\r?$)[\s\S])+^[ ]{0,3}\\k<tilde_fence>~*[ \t]*\r?$)"
  fenced_code_candidate="(?:${backtick_fenced_code_candidate}|${tilde_fenced_code_candidate})"
  inline_code_candidate="(?<!\\x60)(?<inline_ticks>\\x60+)(?!\\x60)(?=(?:(?!\\k<inline_ticks>)[\\s\\S])*${documented_test_signal})(?:(?!\\k<inline_ticks>)[\\s\\S])+\\k<inline_ticks>(?!\\x60)"
  candidate_pattern="(?s)(?:${fenced_code_candidate}|${inline_code_candidate}|${env_split_candidate}|${assign_export_goflags_candidate}|${export_goflags_candidate}|${cd_goflags_candidate}|${prefixed_goflags_candidate}|${cd_candidate})"
