...
target_story_validate_expected_security_alert_rows() {
	...
	jq -e '
		...
		all(expected_alerts[]?; ((.provider_alert_id // "") | length) > 0 or (.provider_alert_number? != null))
	' "${expected_file}" >/dev/null || {
		echo "target security_alert_expected_rows requires provider_alert_id or provider_alert_number for every row" >&2
		return 1
	}
}
...
target_story_compare_security_alert_expected_rows() {
	...
	jq -nr --slurpfile expected "${expected_file}" --slurpfile actual "${actual_file}" '
		...
		def actual_installed_version($row):
			($row.eshu_impact.installed_version //
			 $row.eshu_impact.observed_version //
			 $row.eshu_impact.package_version //
			 $row.provider_alert.installed_version //
			 "");
		...
		def expected_installed_version($row):
		...
		def field_mismatches($want; $row):
			[
				...
				(if (expected_installed_version($want) | length) > 0 then (actual_installed_version($row) == expected_installed_version($want)) else true end),
		...
		def has_evidence_or_reason($want; $row):
			if (($want.requires_evidence // true) == false) then true
				...
				or ((($row.reason // "") | ascii_downcase) | test("missing|unavailable|unsupported|provider.only|provider_only|stale"))
