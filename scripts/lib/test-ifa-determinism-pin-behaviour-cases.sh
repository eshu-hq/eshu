...
run_ifa_determinism_pin_behaviour_cases() {
	...
	while IFS= read -r det_fn; do
		...
		_ifa_det_pin_probe_run "${det_fn}" "${det_probe_dir}/real_code.sh" "${det_needle}" \
			|| fail "${det_fn}() rejected a needle that IS live code -- the probe cannot distinguish binding from broken"
		for det_probe in comment_only heredoc_only; do
			if _ifa_det_pin_probe_run "${det_fn}" "${det_probe_dir}/${det_probe}.sh" "${det_needle}"; then
