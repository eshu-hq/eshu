...
#
# The pin-helper meta-gate that EXECUTES these helpers against synthetic probe
# files -- _ifa_pin_probe_run, IFA_PIN_PROSE_HELPERS and
# assert_pin_helpers_bind_code -- lives in
# scripts/lib/test-ifa-fault-injection-pin-probe.sh since #6261. This file sat
# at exactly 499/500 for every commit of #6161's review cycle and the ceiling
# forced both packing and inlining, which is where two P1s came from; the probe
# block was the coherent unit to move.
