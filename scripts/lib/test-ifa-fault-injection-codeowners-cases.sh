...
# Focused behavioral regressions for the codeowners_ownership_edges
# fault-injection cells (#5992). Split from
...
codeowners_cells_lib="${repo_root}/scripts/lib/ifa_fault_injection_codeowners_cells.sh"
...
test_ifa_codeowners_intent_lock_is_fail_closed() (
	source "${det_lib}"
	source "${codeowners_cells_lib}"
	...
	log_dir="$(mktemp -d -t ifa-codeowners-lock.XXXXXX)"
...
test_ifa_codeowners_released_lock_holder_is_not_torn_down_twice() (
	source "${det_lib}"
	source "${driver_lib}"
	source "${codeowners_cells_lib}"
	...
	case_dir="$(mktemp -d -t ifa-codeowners-lock-owner.XXXXXX)"
	...
	if rg --line-regexp --quiet -- "${holder_pid}" "${case_dir}/kill.log"; then
		fail "teardown signaled the joined codeowners lock-holder PID; PID reuse could target an unrelated process"
	fi
