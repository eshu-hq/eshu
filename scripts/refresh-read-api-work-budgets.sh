...
# the same formulas to the max over every other exercised route. Numbers are
...
table="$(jq -s -r --argjson named "${named_json}" --arg n "${#reports[@]}" '
	...
	| "# read-api-route-work-budgets.txt -- GENERATED by scripts/refresh-read-api-work-budgets.sh",
	  ...
	  "# blks = ceil(max*3.0), rows = ceil(max*2.0).",
	  "",
	  "default\t\(row($d))",
	  "",
	  ($named_rows | sort_by(.route)[] | "\(.route)\t\(row(.))\tGREEN-derived work guard")
