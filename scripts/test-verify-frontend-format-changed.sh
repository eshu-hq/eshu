...
git_init() {
  ...
  git -C "${repo_dir}" config user.name "Eshu Test"
  ...
  git -C "${repo_dir}" switch -q -c feature
}
...
test_no_changed_js_ts_files_skips() {
  ...
  git -C "${repo_dir}" commit -q -m "docs"
...
test_changed_unformatted_js_ts_fails() {
  ...
  git -C "${repo_dir}" add apps/console/src/bad.ts
  git -C "${repo_dir}" commit -q -m "bad format"
...
test_changed_formatted_js_ts_passes() {
  ...
  git -C "${repo_dir}" add src/good.ts
  git -C "${repo_dir}" commit -q -m "good format"
  run_verifier "${repo_dir}" "${out}"
...
test_no_merge_base_uses_two_dot_fallback() {
  ...
  git -C "${repo_dir}" add apps/console/src/good.tsx
  git -C "${repo_dir}" commit -q -m "feature format"
  local feature_head
  feature_head="$(git -C "${repo_dir}" rev-parse HEAD)"
  git -C "${repo_dir}" switch -q --orphan unrelated-main
  git -C "${repo_dir}" rm -qr --ignore-unmatch .
  printf 'unrelated\n' >"${repo_dir}/unrelated.txt"
  git -C "${repo_dir}" add unrelated.txt
  git -C "${repo_dir}" commit -q -m "unrelated main"
  git -C "${repo_dir}" update-ref refs/remotes/origin/main HEAD
  git -C "${repo_dir}" switch -q --detach "${feature_head}"
  run_verifier "${repo_dir}" "${out}"
