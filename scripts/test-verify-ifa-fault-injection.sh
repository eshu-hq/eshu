...
# first thing to exercise live. The driver script itself was split into
# scripts/lib/ifa_fault_injection_driver.sh (shared per-cell plumbing) and
# scripts/lib/ifa_fault_injection_cells.sh (the five cells) to stay under the
# repo's 500-line cap; checks below point at whichever file now actually
# holds the content.
set -euo pipefail
...
for f in "${script}" "${fault_lib}" "${det_lib}" "${driver_lib}" "${cells_lib}"; do
	[[ -f "${f}" ]] || fail "missing ${f}"
done
...
# Both cassettes, generated synth-multiscope once, and the drive verb (now in
# the driver lib's drive_all_cassettes helper).
...
for f in "${script}" "${fault_lib}" "${driver_lib}" "${cells_lib}"; do
