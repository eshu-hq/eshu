#!/usr/bin/env bash
# C-1/C-8 replay coverage manifest + lockstep gate (#4173, #4187, epic #4172).
...
# without failing the command. CI passes --blocking now that C-2..C-8 burned the
...
printf '\nPASS: C-1/C-8 replay coverage gate (%s); report at %s\n' "${blocking:-advisory}" "${report_abs}"
