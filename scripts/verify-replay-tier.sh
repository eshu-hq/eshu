...
(
	...
	go test ./internal/replay/offlinetier/ \
		-run 'TestOfflineReplayTierGraphTruth|TestDeltaTombstone|TestDeltaEntityRetractGraphTruth|TestEntityRetractManifestBinding|TestDeltaSurvivorScopedRetractGraphTruth|TestDeltaEdgeRetractGraphTruth' -count=1 -v
)
tier_status=$?
tier_end="$(date +%s)"
