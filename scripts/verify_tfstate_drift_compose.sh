...
# this script grows past six scenarios, consider threading expected via
...
echo "==> Confirming Phase 3.5 enqueued the six seeded state_snapshot scopes"
...
if [[ -z "$phase35_count" || "$phase35_count" -lt 6 ]]; then
	echo "Phase 3.5 enqueued count=${phase35_count:-<missing>}, expected >=6" >&2
	tail -n 40 "$PHASE_35_LOG_FILE" >&2
	exit 1
fi
...
for kind in added_in_state added_in_config removed_from_state attribute_drift removed_from_config; do
...
for address in aws_s3_bucket.unmanaged aws_s3_bucket.declared aws_s3_bucket.was_there aws_s3_bucket.logs aws_iam_policy.legacy; do
	tfstate_drift_assert_log_line "$DRIFT_LOGS_FILE" \
