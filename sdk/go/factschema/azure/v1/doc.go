...
// CloudRelationship (azure_cloud_relationship), TagObservation
// (azure_tag_observation), IdentityObservation (azure_identity_observation),
// ResourceChange (azure_resource_change), DNSRecord (azure_dns_record),
// ImageReference (azure_image_reference), and CollectionWarning
// (azure_collection_warning). Each struct's required fields are non-pointer
...
// ImageReference, and CollectionWarning are each scoped to one fact kind with
// a known, closed field set and carry no Attributes pass-through: the
