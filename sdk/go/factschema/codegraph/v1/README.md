...
collector (`go/internal/collector/repo/git/fact_builder.go` `repositoryFactEnvelope` and
`gitcontent.FileFactEnvelope` in `go/internal/collector/git/content/envelopes.go`). This package types only the OUTER envelope identity
...
these payloads (`go/internal/collector/repo/git/fact_builder.go`,
`go/internal/collector/repo/git/refs.go`), which also live outside this module.
...
- `go/internal/collector/repo/git/fact_builder.go`,
  `go/internal/collector/repo/git/refs.go` — the collector-side emitters for these
  two fact kinds.
