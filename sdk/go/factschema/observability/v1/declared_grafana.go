...
// DeclaredFolder is the schema-version-1 typed payload for the 'observability.declared_folder' fact
// kind: one declared Grafana folder from an IaC or GitOps manifest.
//
...
// DeclaredDashboard is the schema-version-1 typed payload for the 'observability.declared_dashboard' fact
// kind: one declared Grafana dashboard from an IaC or GitOps manifest.
//
// SourceInstanceID is the only required field; every other field is an
// optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
type DeclaredDashboard struct {
	// SourceInstanceID is the collector-derived source instance identity
	// (repo_id:relative_path for the git-declared lane, the live collector's
	// source_instance_id for the observed/applied lane). Required — the one
	// identity field EVERY observability collector injects on EVERY kind in
...
// DeclaredDatasource is the schema-version-1 typed payload for the 'observability.declared_datasource' fact
// kind: one declared Grafana datasource from an IaC or GitOps manifest.
//
// SourceInstanceID is the only required field; every other field is an
// optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
type DeclaredDatasource struct {
	// SourceInstanceID is the collector-derived source instance identity
	// (repo_id:relative_path for the git-declared lane, the live collector's
	// source_instance_id for the observed/applied lane). Required — the one
	// identity field EVERY observability collector injects on EVERY kind in
...
// DeclaredAlertRule is the schema-version-1 typed payload for the 'observability.declared_alert_rule' fact
// kind: one declared Grafana alert rule from an IaC or GitOps manifest.
//
// SourceInstanceID is the only required field; every other field is an
// optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
type DeclaredAlertRule struct {
	// SourceInstanceID is the collector-derived source instance identity
	// (repo_id:relative_path for the git-declared lane, the live collector's
	// source_instance_id for the observed/applied lane). Required — the one
	// identity field EVERY observability collector injects on EVERY kind in
