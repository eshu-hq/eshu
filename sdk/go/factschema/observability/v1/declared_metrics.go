...
// DeclaredScrapeConfig is the schema-version-1 typed payload for the 'observability.declared_scrape_config' fact
// kind: one declared Prometheus/Mimir scrape configuration.
//
...
type DeclaredScrapeConfig struct {
...
// DeclaredMetricRule is the schema-version-1 typed payload for the 'observability.declared_metric_rule' fact
// kind: one declared Prometheus/Mimir rule.
//
// SourceInstanceID is the only required field; every other field is an
// optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
type DeclaredMetricRule struct {
	// SourceInstanceID is the collector-derived source instance identity
	// (repo_id:relative_path for the git-declared lane, the live collector's
	// source_instance_id for the observed/applied lane). Required — the one
	// identity field EVERY observability collector injects on EVERY kind in
...
// DeclaredMetricRoute is the schema-version-1 typed payload for the 'observability.declared_metric_route' fact
// kind: one declared metric pipeline route.
//
// SourceInstanceID is the only required field; every other field is an
// optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
type DeclaredMetricRoute struct {
	// SourceInstanceID is the collector-derived source instance identity
	// (repo_id:relative_path for the git-declared lane, the live collector's
	// source_instance_id for the observed/applied lane). Required — the one
	// identity field EVERY observability collector injects on EVERY kind in
