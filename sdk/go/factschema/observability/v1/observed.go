...
// ObservedDashboard is the schema-version-1 typed payload for the 'observability.observed_dashboard' fact
// kind: one live Grafana dashboard/folder/datasource/alert object.
//
// SourceInstanceID and ProviderObjectUID are required; every other field is
// an optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
type ObservedDashboard struct {
	...
	// ProviderObjectUID is the live provider object uid. Required for this
	// kind: its sole emitter validates the uid non-blank and always writes
	// provider_object_uid, so a fact missing it is malformed. Also the first
	// object-ref candidate the reducer reads.
	ProviderObjectUID string `json:"provider_object_uid"`
...
// ObservedTarget is the schema-version-1 typed payload for the 'observability.observed_target' fact
// kind: one live Prometheus/Mimir scrape target.
//
// SourceInstanceID and ProviderObjectUID are required; every other field is
// an optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
...
// ObservedRule is the schema-version-1 typed payload for the 'observability.observed_rule' fact
// kind: one live Prometheus/Mimir/Loki/Grafana rule. provider_object_uid stays optional: the Grafana emitter identifies the rule by alert_rule_uid, so requiring it would dead-letter every Grafana observed_rule.
//
