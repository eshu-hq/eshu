...
// ObservedLogSignal is the schema-version-1 typed payload for the 'observability.observed_log_signal' fact
// kind: one bounded live Loki log signal (no log lines).
//
...
// ObservedTraceSignal is the schema-version-1 typed payload for the 'observability.observed_trace_signal' fact
// kind: one bounded live Tempo trace signal (no spans).
//
// SourceInstanceID and ProviderObjectUID are required; every other field is
// an optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
type ObservedTraceSignal struct {
	// SourceInstanceID is the collector-derived source instance identity
	// (repo_id:relative_path for the git-declared lane, the live collector's
	// source_instance_id for the observed/applied lane). Required — the one
	// identity field EVERY observability collector injects on EVERY kind in
