...
// SourceInstance is the schema-version-1 typed payload for the 'observability.source_instance' fact
// kind: one configured declared/applied/observed observability evidence source. The coverage-metadata classifier skips this kind, but it is typed for a uniform decode seam and source_instance_id remains the required anchor.
//
...
// CoverageWarning is the schema-version-1 typed payload for the 'observability.coverage_warning' fact
// kind: one source-local coverage/redaction/unsupported/stale/permission-hidden warning. provider_object_uid is only set when the warning names a concrete resource, so it stays optional.
//
// SourceInstanceID is the only required field; every other field is an
// optional pointer the coverage-metadata classifier reads through the
// shared candidate-key fallback chain.
