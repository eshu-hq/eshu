...
describe("evaluateMarketingReview", () => {
  it("passes when every viewport meets the full contract", () => {
    ...
    expect(result.findings.every((finding) => finding.severity === "pass")).toBe(
      true
    );
  ...
  it("flags a CTA whose href regressed", () => {
    ...
    expect(result.passed).toBe(false);
    expect(
      result.findings.find((item) => item.check === "cta:Try it locally")
        ?.severity
    ).toBe("fail");
  });
  ...
  it("flags missing Ask Eshu positioning copy", () => {
    ...
    expect(result.passed).toBe(false);
    expect(
      result.findings.filter((item) => item.severity === "fail").length
    ).toBe(contract.askEshuPositioning.length);
  });
  ...
  it("flags accessibility regressions: missing alt text and duplicate h1", () => {
    ...
    expect(result.passed).toBe(false);
    expect(
      result.findings.find((item) => item.check === "a11y:img-alt")?.severity
    ).toBe("fail");
    expect(
      result.findings.find((item) => item.check === "a11y:single-h1")?.severity
    ).toBe("fail");
  });
  ...
  it("flags an unreachable external link", () => {
    ...
    expect(
      result.findings.find(
        (item) => item.check === "links:external-reachable"
      )?.severity
    ).toBe("fail");
  });
  ...
  it("flags a slow load that exceeds the performance budget", () => {
    ...
    expect(
      result.findings.find(
        (item) => item.check === "perf:dom-content-loaded"
      )?.severity
    ).toBe("fail");
  });
});
